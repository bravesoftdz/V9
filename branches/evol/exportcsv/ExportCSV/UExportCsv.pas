...
begin
  result := DatetimeToStr(NowH)+ ' Envoie du fichier '+fExportName+' sur '+ADDRSERVER+' ERREUR --> ('+nettoie(TheMessage)+')' ;
end;


function TForm1.ConstitueMessageOk : string;
begin
  result := DatetimeToStr(NowH)+ ' Envoie du fichier '+fExportName+' sur '+ADDRSERVER+'  OK' ;
end;
...
begin
  ...
  Sql := 'SELECT GP_DATEPIECE,'+
         ...
         'WHERE GP_NATUREPIECEG=''DBT'' AND GP_LIBREPIECE1=''ENC'' AND GP_DATEPIECE >= '''+USDATETIME(flastDate)+''' AND GP_DATEPIECE <= '''+USDATETIME(fnewDate)+'''';
...
begin
  ...
  TRY
    ...
    begin
      ...
      fIdFTP.Host := ADDRSERVER;
      fidFTP.Port := 21;
      fIDFTP.Username := 'lse';
      fidFTP.Password := 'doNotDisC3rn11';
      fidFTP.Passive := true;
