...
begin
  ...
  begin
    if not JaiLeDroitConcept(TConcept(ccAuxModif), False) then A := taConsult;
...
begin
  ...
  A := taModif;
  if SAJAL = nil then Exit;
  if not JaiLeDroitConcept(TConcept(ccGenModif), False) then A := taConsult; //FicheGene(Nil,'',SAJAL.TRESO.CPT,A,0) ;//XMG 20/04/04
