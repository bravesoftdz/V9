...
uses
  ...
  QR, hmsgbox, HQuickrp, DB, DBTables, StdCtrls, Buttons, Hctrls, ExtCtrls,
  Mask, Hcompte, ComCtrls, Ent1, HEnt1, QrRupt, CritEdt, UtilEdt, HQry,
  EdtLegal, CpteUtil, ParamDBG, HSysMenu, Menus, DBCtrls, HTB97, HPanel, UiUtil, tCalcCum ;

procedure EcheancierED ;
procedure EcheancierEDZoom(Crit : TCritEdt) ;
...
type
  ...
  private    { Déclarations privées }
    ...
    Cod, Lib, StReportEche   : String ;
    Last, Cpt, NbEch, NumR   : integer ;
...
BEGIN
PP:=FindInsidePanel ;
QR:=TFQREcheED.Create(Application) ;
Edition.Etat:=etEche ;
QR.QRP.QRPrinter.OnSynZoom:=QR.EcheanZoom ;
QR.InitType (nbAux,neEch,msRien,'QRECHEANCEED','',TRUE,FALSE,FALSE,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
try
 QR.QRP.QRPrinter.OnSynZoom:=QR.EcheanZoom ;
 QR.CritEdt:=Crit ;
 QR.InitType (nbAux,neEch,msRien,'QRECHEANCEED','',FALSE,TRUE,FALSE,Edition) ;
 finally
 QR.Free ;
 end ;
Screen.Cursor:=SyncrDefault ;
...
Function TFQREcheED.QuoiMvt : String ;
BEGIN
...
Procedure TFQREcheED.EcheanZoom(Quoi : String) ;
Var Lp,i: Integer ;
BEGIN
...
procedure TFQREcheED.FinirPrint;
Var Solde : double ;
BEGIN
Inherited ;
VideGroup(LMulti) ;
...
if OkMajEdt Then
   ...
   if Solde<0
      then MajEdition('ECH', CritEdt.Exo.Code, DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date2),
                      '', TotGeneral[1].TotDebit, TotGeneral[1].TotCredit, Solde, 0)
      else MajEdition('ECH', CritEdt.Exo.Code, DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date2),
                      '', TotGeneral[1].TotDebit, TotGeneral[1].TotCredit, 0, Solde) ;
   END ;
END ;

procedure TFQREcheED.InitDivers ;
BEGIN
Inherited ;
...
procedure TFQREcheED.GenereSQL ;
Var WhereGen,WhereAux,WhereColl,WhereJal,St,StSupCouv : String ;
...
Case CritEdt.Monnaie of
  ...
  1 : BEGIN
      ...
      END ;
  2 : BEGIN
      Q.SQL.Add(' E_DEBITEURO as DEBIT, E_CREDITEURO as CREDIT, E_COUVERTUREEURO as COUVERTURE ') ;
      StSupCouv:='AND (NOT (E_ETATLETTRAGE="PL" AND ((E_DEBIT+E_CREDIT)=E_COUVERTUREEURO)))'
      END ;
  end ;
...
Case CritEdt.Ech.Sens of
  0 : BEGIN
      Case CritEdt.Monnaie of
        ...
        2 : BEGIN Q.SQL.Add(' And E_CREDITEURO<>0 ') ; END ;
       end ;
      END ;
  1 : BEGIN
      Case CritEdt.Monnaie of
        ...
        2 : BEGIN Q.SQL.Add(' And E_DEBITEURO<>0 ') ; END ;
       end ;
      END ;
 end ;
...
if CritEdt.Ech.RuptPaie then Q.SQL.Add(' Order By E_DATEECHEANCE, E_MODEPAIE, E_AUXILIAIRE, E_GENERAL ') Else
  BEGIN
//  If CritEdt.Eche
  Q.SQL.Add(' Order By E_DATEECHEANCE, E_AUXILIAIRE, E_GENERAL') ;
  END ;
ChangeSQL(Q) ; Q.Open ;
END ;

procedure TFQREcheED.RenseigneCritere ;
...
procedure TFQREcheED.ChoixEdition ;
BEGIN
Inherited ;
...
procedure TFQREcheED.RecupCritEdt ;
Var St3, St4 : String ;
BEGIN
Inherited ;
With CritEdt Do
...
function  TFQREcheED.CritOk : Boolean ;
BEGIN
...
function TFQREcheED.Calculs : Boolean ;
Var MReport : TabTRep ;
    Reste   : Double ;
BEGIN
...
procedure TFQREcheED.FormShow(Sender: TObject);
begin
HelpContext:=7541000 ;
...
procedure TFQREcheED.FColl1Change(Sender: TObject);
...
procedure TFQREcheED.TOPREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQREcheED.BEcheHBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQREcheED.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
// XX OkEche:=True ;
...
procedure TFQREcheED.BSDMultiBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
Case NumR of
...
procedure TFQREcheED.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQREcheED.BOTTOMREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var MReport        : TabTRep ;
begin
  inherited;
...
procedure TFQREcheED.QRDLMultiNeedData(var MoreData: Boolean);
...
procedure TFQREcheED.DLEcheHNeedData(var MoreData: Boolean);
begin
  inherited;
...
procedure TFQREcheED.FEcheancierChange(Sender: TObject);
begin
...
procedure TFQREcheED.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFQREcheED.FGenTCTD1Change(Sender: TObject);
Var AvecJokerS : Boolean ;
begin
  inherited;
...
procedure TFQREcheED.QRDLGGNeedData(var MoreData: Boolean);
...
procedure TFQREcheED.QRBandGGAfterPrint(BandPrinted: Boolean);
begin
  inherited;
QRBandGG.ForceNewPage:=FALSE ;
end;

procedure TFQREcheED.EntetePageAfterPrint(BandPrinted: Boolean);
begin
  inherited;
if CritEdt.Ech.RecapMp then if Q.Eof then TOPREPORT.Enabled:=False ;
end;

procedure TFQREcheED.FRecapPaieClick(Sender: TObject);
begin
  inherited;
FSautRecapMP.Enabled:=FRecapPaie.Checked ;
end;

procedure TFQREcheED.EntetePageBeforePrint(var PrintBand: Boolean;
  var Quoi: string);
begin
  inherited;
...
procedure TFQREcheED.FNatureCptChange(Sender: TObject);
begin
  inherited;
...
procedure TFQREcheED.QRSUBTOTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQREcheED.FNatureCptTCTDChange(Sender: TObject);
begin
  inherited;
If Not QRLoading then
   BEGIN
...
procedure TFQREcheED.BSDMultiAfterPrint(BandPrinted: Boolean);
begin
  inherited;
if NumR=0 then InitReport([2],CritEdt.Ech.FormatPrint.Report) ;
end;

procedure TFQREcheED.FEditionChange(Sender: TObject);
begin
  inherited;
...
procedure TFQREcheED.BNouvRechClick(Sender: TObject);
begin
  inherited;
...
procedure TFQREcheED.FFiltresChange(Sender: TObject);
begin
  inherited;
//If FEcheancier.ItemIndex=0 then ;
end;
