...
if not JaiLeDroitConcept(TConcept(ccSaisEcritures), TRUE) then begin Result:=FALSE ; Exit ; end ;
...
{$ENDIF}
 ...
 if not JaiLeDroitConcept(TConcept(ccSaisEcritures), FALSE) then FAction:=taConsult ;
 LookLesDocks(Self) ;
 InitSaisie(FALSE) ;
...
if FAction = taCreat then
 Action := taModif
   ...
   Action:=FAction ;
if not JaiLeDroitConcept(TConcept(ccGenModif), FALSE) then Action:=taConsult ;
...
if FAction = taCreat then
 Action := taModif
   ...
   Action:=FAction ;
if not JaiLeDroitConcept(TConcept(ccJalModif), FALSE) then Action:=taConsult ;
...
if FAction = taCreat then
 Action := taModif
   ...
   Action:=FAction ;
if not JaiLeDroitConcept(TConcept(ccAuxModif), FALSE) then Action:=taConsult ;
...
begin
if not BZoomTiers.Enabled then Exit ;
if not JaiLeDroitConcept(TConcept(ccAuxModif), FALSE) then exit ;
if (memJal.Code='') or (GS.Cells[SF_AUX, GS.Row]='') then Exit ;
