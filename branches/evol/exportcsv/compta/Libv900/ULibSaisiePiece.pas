...
{$ENDIF SAISIEDOC}
      ...
      function  ValideCellule( vChamp : String ; vRow : Integer ) : boolean ;
...
//      procedure PutGridToTob( aCol, aRow : integer );
      ...
      function  RecupFormule        ( vStChamp : String ; vStGrille : String ) : String;
      function  RecupCondition      ( vStChamp : String ; vStGrille : String ) : String;
...
{$IFDEF SAISIEDOC}
      ...
      procedure InitPopupViewer       ( vPopup : TPopupMenu ; vBouton : TToolBarButton97 ) ;
...
{$IFDEF SAISIEDOC}
  FTSDoc            := TSaisieDoc.Create( FEcran, FTobPiece ) ;
...
{$IFDEF SAISIEDOC}
  if (Piece.ModeSaisie=msBor) and ViewerActif and (not MajDocEnCours) then
  // and  TSP.ViewerVisible ???
    begin
...
begin
  ...
  // Cut-off
  if not Piece.isOut( ARow ) and (lStCol = 'E_NUMLIGNE') then
    begin
    lTobEcr   := Piece.Detail[ ARow-1 ] ;
    lTobCompl := CGetTOBCompl( lTobEcr ) ;
    if (lTobCompl<>nil) and (lTobCompl.GetDateTime('EC_CUTOFFDEB')>iDate1900) then
      begin
...
begin
  ...
  if Not JaiLeDroitConcept(TConcept(ccSaisProrata),True) then Exit ;
...
//    Exit ;
  ...
  // ------------------ Condition -------------------
  // ------------------------------------------------
  Condition := RecupCondition(vStColName,'SAISIE');
  if Condition <> '' then
  begin
     FGrille   := 'SAISIE';
...
function TSaisiePiece.ValideCellule( vChamp : String ; vRow : Integer ) : boolean ;
...
begin
  ...
  begin
    FGrille := vStGrille;
    FTob    := vTobEcr;
    Result  := GFormule( RecupFormule(vStChamp,vStGrille), GetFormule, nil, 1 );
  end
...
function TSaisiePiece.RecupFormule(vStChamp : String ; vStGrille : String ) : String;
var
  TobFormule : TOB;
begin
  Result := '';
  TobFormule := RecupChampGrille(vStChamp,vStGrille);
  if TobFormule = nil then Exit;
  if TobFormule.FieldExists('FORMULE') then
     Result := TobFormule.GetString('FORMULE');
end;

function TSaisiePiece.RecupCondition(vStChamp : String ; vStGrille : String ) : String;
var
  TobCondition : TOB;
begin
  Result := '';
  TobCondition := RecupChampGrille(vStChamp,vStGrille);
  if TobCondition = nil then Exit;
  if TobCondition.FieldExists('CONDITION') then
     Result := TobCondition.GetString('CONDITION');
end;
...
{$IFDEF SAISIEDOC}
procedure TSaisiePiece.paramViewer;
begin
  TSDoc.ActiverViewer( GetTobMasqueParam ) ;
  ...
  if TSDoc.IsActif and TSDoc.IsVisible then
    Case TSDoc.Position Of
      ...
      vpDroite, vpGauche  :  begin
                             TSDoc.PanelActif.Width  := (LaGrille.Width + TSDoc.PanelActif.Width) div 2  ;
//                             if TSDoc.FitToLargeOk then
...
begin
  ...
  if not assigned(TSDoc) then
    FTSDoc := TSaisieDoc.Create( FEcran, FTobPiece ) ;

  // Référencement des composants pour le gestionnaire du viewer
  TSDoc.InitViewer( vPanelB, vPanelR, vPanelL, vPanelT, vSplitB, vSplitR, vSplitL, vSplitT ) ;
...
begin
  ...
  if THSPlitter(Sender).Visible then
    ...
    if (TSDoc.Position in [ vpDroite, vpGauche]) then
      begin
      // On force le resize des colonnes de la grille en cas de déplacement latéral
      Resize ;
...
procedure TSaisiePiece.InitPopupViewer(vPopup: TPopupMenu ; vBouton : TToolBarButton97 ) ;
begin
  TSDoc.SetMenuPop( vPopup, vBouton ) ;
  vPopup.OnPopup := TSDoc.Onpopupviewer ;
  TSDoc.InitPopupViewer(False);
end;
