...
Suite ........ : MVG 16/01/2004 FQ19514
Suite ........ : MBO 02/10/2007 - ajout paramètre en appel planInfo.Calcul : calcul pour cession
Suite ........ : MBO 29/10/07 - fq 21754 - ajout paramètre en appel planInfo.calcul : prise en cpte ou non du jour de cession
Mots clefs ... :
*****************************************************************}
unit HalOLE;
...
uses SysUtils, WinTypes, WinProcs, Messages, Classes, Graphics, Controls, Forms,
     ...
     HEnt1, hmsgbox, OLEAuto,ParamSoc,
     UtilSoc,
{$IFDEF EAGLCLIENT}
     UtileAGL,
     Rubrique_TOM,
...
{$IFDEF VER150}
 ...
 {$ENDIF}
    ...
    CPSection_TOM,
    ExcelToEcr,
    HSysMenu,MajTable,Ent1,CalcOLE,HCompte,
    uTofConsEcr,
    UTob, SoldeCpt
    ,GalOutil
{$IFDEF AMORTISSEMENT}
...
//    FMainForm : TMainForm ;
  ...
  automated
    ...
    Function GetBalanceTableau(nExo : Integer; Extent : string) : Variant ;
    ...
    *)
    ...
    Function GetDerogatoire(CpteInf,CpteSup, Methode : string) : Variant ; // FQ 19514
    ...
    function TraiteEcrituresComptesRevision ( LeJournal,LeFolio : String; CreatCompte:string='TRUE') : Variant ;
    ...
    // CA - 29/10/2007 - Demandes KPMG
    function InfoPeriode(const pstPeriode: Widestring): Variant;
    function FourchetteDenombrer (const pstFourchette : Widestring) : variant;
    function FourchetteEnumerer (const pstFourchette : Widestring) : variant;
    function FourchetteControler (const pstFourchette : Widestring ) : variant;
    function FourchetteComplement(const pstFourchette, pstCpteMin, pstCpteMax: WideString) : variant;
    function CumulAgregat (const pstTypeAgregat, pstAgregat, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise : Widestring ) : variant;
    function CumulAgregatDetail (const pstTypeAgregat, pstAgregat, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise, pstCount : Widestring ) : variant;
    function CumulAgregatDetailHisto(const pstTypeAgregat, pstAgregat, pstTypeEcr, pstEtablissement, pstDevise, pstCount: Widestring): variant;
    function CumulAgregatFourchetteComplement (const pstAgregat, pstCpteMin, pstCpteMax, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise : Widestring ) : variant;
    function CumulAgregatComplementDetailHisto(const pstAgregat, pstCpteMin, pstCpteMax, pstTypeEcr, pstEtablissement, pstDevise, pstCount: Widestring): variant;

    function OuvrirCumulAgregat : variant;
    function FermerCumulAgregat : variant;
...
Result := JaiLeDroit;
if Result <> '' then exit;
if V_PGI.Driver in [dbINTRBASE,dbMSSQL,dbMSACCESS,dbPARADOX,dbSQLBASE,dbMssql2005] //10/08/2006 YMO Ajout test SQL2005
   then
   {$IFNDEF EAGLCLIENT}
...
Créé le ...... : 26/09/2007
Modifié le ... : 26/09/2007
Description .. : La liste de la balance sous forme  d'un tableau à deux
Suite ........ : dimensions
Suite ........ : Compte  libellé  total débit  total crédit  solde débit  solde
Suite ........ : crédit
Suite ........ : 2906000 PROV.DEPRE.DROIT AU BAIL 100 0 100 0
Suite ........ : 2907000 PROV.DEPRE.FONDS COMMERCE 0 0 0 0
Suite ........ : 2677000 REPORT AN 01/01/2004 13119140 0 13119140
Suite ........ : 0
Suite ........ : 2678000 REPORT AN 01/01/2004 3234366.48 0
Suite ........ : 3234366.48 0
Suite ........ : en VB
Suite ........ : Function GetBalance(NbExo$)
Suite ........ : Dim s As Variant
Suite ........ : Connecte
Suite ........ : s = V.GetBalanceTableau(NbExo$, "G")
Suite ........ : GetBalance = s(0)(1)
Suite ........ : End Function
Mots clefs ... :
*****************************************************************}

Function THalleyWindow.GetBalanceTableau(nExo : Integer; Extent : string) : Variant ;
var Q : TQuery ; bPrec, bN1C : Boolean ;
    s,sChampDEB,sChampCRE,sExo : string ;
    TotDeb,TotCre,SoldeDeb,SoldeCre : Double ;
    V, Vi : Variant;
...
Result := JaiLeDroit;
if Result <> '' then exit;
if Extent  = '' then begin Result := 'Extention G ou T n''est pas renseigné'; exit; end;
s:='' ; bPrec:=FALSE ;
i := 0;
if (nExo = 2 ) then
begin
...
Table := ''; SQL := '';
if nExo <> -1 then Case Extent[1] of
'G' : begin
      Table := 'GENERAUX';
      SQL   := 'Select G_GENERAL,G_LIBELLE,G_COLLECTIF,G_TOTDEBE,G_TOTCREE,G_TOTDEBP,G_TOTCREP,G_TOTDEBS,G_TOTCRES FROM GENERAUX ORDER BY G_GENERAL'
      end;
'T' : begin
      Table := 'TIERS';
      SQL   := 'Select T_AUXILIAIRE,T_LIBELLE,T_TOTDEBE,T_TOTCREE,T_TOTDEBP,T_TOTCREP,T_TOTDEBS,T_TOTCRES FROM TIERS ORDER BY T_AUXILIAIRE'
      end;
end;

case nExo of
     0 : begin
         sChampDEB:=Extent+'_TOTDEBE' ;
         sChampCRE:=Extent+'_TOTCREE' ;
         sExo:='N' ;
         end ;
    -1 : begin
             sChampDEB:=Extent+'_TOTDEBP' ;
             sChampCRE:=Extent+'_TOTCREP' ;
             sExo  :='N-' ;
             bPrec := not (ExisteSQL ('SELECT E_EXERCICE FROM ECRITURE WHERE E_EXERCICE="'+VH^.Precedent.Code+'"'));
             if Extent = 'G' then
             begin
                   Q:=OpenSQL('SELECT count(HB_COMPTE1) FROM HISTOBAL LEFT JOIN GENERAUX ON HB_COMPTE1=G_GENERAL WHERE HB_TYPEBAL="N1C" OR HB_TYPEBAL="N1A" ',False) ;
                   V:=VarArrayCreate([0, Q.Fields[0].asinteger],varVariant);
                   Ferme (Q);
                   SQL   := 'SELECT HB_COMPTE1, G_LIBELLE, HB_DEBIT, HB_CREDIT FROM HISTOBAL LEFT JOIN GENERAUX ON HB_COMPTE1=G_GENERAL WHERE HB_TYPEBAL="N1C" OR HB_TYPEBAL="N1A"'
             end
             else
             begin
                   Q:=OpenSQL('SELECT count(HB_COMPTE2) FROM HISTOBAL LEFT JOIN TIERS ON HB_COMPTE2=T_AUXILIAIRE WHERE HB_TYPEBAL="N1C" OR HB_TYPEBAL="N1A" ',False) ;
                   V:=VarArrayCreate([0, Q.Fields[0].asinteger],varVariant);
                   Ferme (Q);
                   SQL   := 'SELECT HB_COMPTE2, T_LIBELLE, HB_DEBIT, HB_CREDIT FROM HISTOBAL LEFT JOIN TIERS ON HB_COMPTE2=T_AUXILIAIRE WHERE HB_TYPEBAL="N1C" OR HB_TYPEBAL="N1A"'
             end
         end ;
     1 : begin
         sChampDEB:=Extent+'_TOTDEBS' ;
         sChampCRE:=Extent+'_TOTCRES' ;
         sExo:='N+' ;
         end ;
     end ;
if bPrec then
begin
  Q:=OpenSQL('SELECT HB_COMPTE1, G_LIBELLE, HB_DEBIT, HB_CREDIT FROM HISTOBAL LEFT JOIN GENERAUX ON HB_COMPTE1=G_GENERAL WHERE HB_TYPEBAL="N1C" OR HB_TYPEBAL="N1A"'+
                ' ORDER BY HB_TYPEBAL,HB_COMPTE1,HB_COMPTE2', TRUE) ;
  bN1C:=not Q.EOF ;
  while not Q.EOF do
  begin
    Vi :=VarArrayCreate([0, 5], varVariant);
    Vi[0] := VirePV(Q.FindField('HB_COMPTE1').AsString);
    Vi[1] := VirePV(Q.FindField('G_LIBELLE').AsString) + ';' ;
    Vi[2] := Strfpoint(0);
    Vi[3] := Strfpoint(0);
    Vi[4] := Strfpoint(Q.FindField('HB_DEBIT').AsFloat);
    Vi[5] := Strfpoint(Q.FindField('HB_CREDIT').AsFloat) ;
    V[i] := Vi;
    Q.Next ;  inc(i);
  end ;
  Ferme(Q) ;
...
Q:=OpenSQL('Select COUNT(*) FROM  '+ Table,False) ;
V:=VarArrayCreate([0, Q.Fields[0].asinteger],varVariant);
Ferme (Q);
...
While not Q.EOF do
   begin
    Vi :=VarArrayCreate([0, 5], varVariant);
    if Extent = 'G' then
      Vi[0] := VirePV(Q.FindField('G_GENERAL').AsString)
    else
      Vi[0] := VirePV(Q.FindField('T_AUXILIAIRE').AsString);

     Vi[1] := VirePV(Q.FindField(Extent+'_LIBELLE').AsString);
     TotDeb:=Q.FindField(sChampDEB).AsFloat ;
     TotCre:=Q.FindField(sChampCRE).AsFloat ;
     SoldeDeb:=0 ; SoldeCre:=0 ;
     if (Extent = 'G') and (Q.FindField('G_COLLECTIF').AsString='X') then
     begin
        Get_CumulColl(Q.FindField('G_GENERAL').AsString,sExo,SoldeDeb,SoldeCre) ;
        end else
        begin
     ...
     Vi[2] := Strfpoint(TotDeb);
     Vi[3] := Strfpoint(TotCre);
     Vi[4] := Strfpoint(SoldeDeb);
     Vi[5] := Strfpoint(SoldeCre);
     V[i] := Vi;
   Q.Next ;  inc(i);
   end ;
Ferme(Q) ;
...
// MVG 16/01/2007 - FQ19514
Function THalleyWindow.GetDerogatoire(CpteInf,CpteSup, Methode : string) : Variant ;
begin
Result := JaiLeDroit;
if Result <> '' then exit;
  result := '#Erreur : Non connecté' ;
  if Not V_PGI.OKOuvert then Exit ;
  Result:=AmGetDerogatoire(CpteInf,CpteSup, Methode);
end ;
...
// CA - 29/11/2001 - Génère écriture avec création des comptes                       //fiche 18108
Function THalleyWindow.TraiteEcrituresComptesRevision ( LeJournal,LeFolio : string; CreatCompte:string='TRUE' ) : Variant ;
var
Creat : Boolean;
begin
Result := JaiLeDroit;
if Result <> '' then exit;
Creat := (CreatCompte='TRUE');  //fiche 18108
Result:=IntegreEcritureRevisionPGI(LeJournal,LeFolio,Creat) ;
end;
...
if Result <> '' then exit;
Result:=varEmpty ;
It:=HTStringList.Create ;
Va:=HTStringList.Create ;
...
procedure THalleyWindow.OnTheLast(var shutdown: boolean);
begin
...
begin
  if not JaiLeDroitConcept(ccOLE, FALSE) then
...
////////////////////////////////////////////////////////////////////////////////
// VERBES OLE - DEMANDES KPMG
////////////////////////////////////////////////////////////////////////////////
{***********A.G.L.***********************************************
Auteur  ...... : Christophe AYEL
Créé le ...... : 26/10/2007
Modifié le ... : 26/10/2007
Description .. : Renvoie les informations d'une période dans un tableau :
Suite ........ : (Date de début, Date de fin, Nombre de mois, Libellé de la
Suite ........ : période)
Mots clefs ... :
*****************************************************************}
function THalleyWindow.InfoPeriode (const pstPeriode : Widestring) : Variant;
var
  lDateDebut, lDateFin : TDateTime;
  Err : integer;
  Exo : TExoDate;
  Vi : Variant;
begin
  ...
  Result := JaiLeDroit;
  if Result <> '' then exit;
  Result := TraduireMemoire('#Erreur : Non connecté');
  if not V_PGI.OKOuvert then exit ;

  // Traitement
  if WhatDate(pstPeriode, lDateDebut, lDateFin, Err, Exo) then
  begin
    Vi :=VarArrayCreate([0, 3], varVariant);
    Vi[0] := Exo.Deb;
    Vi[1] := Exo.Fin;
    Vi[2] := Exo.NombrePeriode;
    Vi[3] := GetColonneSQL('EXERCICE','EX_LIBELLE','EX_EXERCICE="'+Exo.Code+'"');
    Result := Vi;
  end else Result := '';
end;
...
Créé le ...... : 29/10/2007
Modifié le ... :   /  /    
Description .. : Calcul le nombre de fourchettes présentes dans la chaîne 
Suite ........ : de caractères passée en paramètre
Mots clefs ... :
*****************************************************************}
function THalleyWindow.FourchetteDenombrer (const pstFourchette : Widestring) : variant;
var
  Ens : TEnsembleFourchettes;
begin
  // Contrôle préalables
  Result := JaiLeDroit;
  if Result <> '' then exit;
  ...
  Ens := TEnsembleFourchettes.Create(GetInfoCpta (fbGene).Lg);
  try
    Ens.FourchettesString := pstFourchette;
    Result := Ens.Count;
  finally
    Ens.Free;
  end;
...
Créé le ...... : 29/10/2007
Modifié le ... :   /  /
Description .. : Renvoie dans un tableau la liste des fourchettes passée en
Suite ........ : paramètre
Mots clefs ... :
*****************************************************************}
function THalleyWindow.FourchetteEnumerer (const pstFourchette : Widestring) : variant;
var
  i : integer;
  Ens : TEnsembleFourchettes;
begin
  ...
  try
    ...
    if Ens.Count > 0 then
    begin
      Result := VarArrayCreate([0, Ens.Count-1, 0, 1], varOleStr);
      for i:=0 to Ens.Count-1 do
      begin
        Result[i, 0] := Ens.Fourchettes[i].CompteDebut;
        Result[i, 1] := Ens.Fourchettes[i].CompteFin;
      end;
    end
    else
      Result := Null;
...
Créé le ...... : 30/10/2007
Modifié le ... :   /  /
Description .. : Contrôle l'intersection enventuelle de fourchettes,  et
Suite ........ : renvoie les les fourchettes incréminées
Mots clefs ... :
*****************************************************************}
function THalleyWindow.FourchetteControler (const pstFourchette : Widestring ) : variant;
var
  Ens : TEnsembleFourchettes;
  Res : TStringList;
begin
  ...
  try
    Ens.FourchettesString := pstFourchette;
    Ens.Intersections(Res);
    { l'ensemble des fourchettes incréminés sont dans ma liste
      pour ne pas dupliquer le code de FourchetteEnumerer, je réaffecte mon ensemble
      puis j'appelle FourchetteEnumerer }
    Ens.SetFourchettesStrings(Res);
    Result := FourchetteEnumerer(Ens.FourchettesString);
  finally
    Res.Free;
    Ens.Free;
  end;
...
Créé le ...... : 30/10/2007
Modifié le ... :   /  /
Description .. : Renvoie le complément d'une fourchette
Mots clefs ... :
*****************************************************************}
function THalleyWindow.FourchetteComplement(const pstFourchette, pstCpteMin,
      pstCpteMax: WideString) : variant;
var
  Ens : TEnsembleFourchettes;
begin
  // Contrôle préalables
  ...
  try
    Ens.FourchettesString := pstFourchette;
    if pstCpteMin <> '' then
      Ens.CompteMin := pstCpteMin;
    if pstCpteMax <> '' then
      Ens.CompteMax := pstCpteMax;
    Result := Ens.FourchettesStringComplement;
  finally
    Ens.Free;
  end;
end;
...
Description .. : Calcul de la valeur d'un agrégat. Cet agrégat peut être de 2
Suite ........ : types, rubrique ou fourchette.
Suite ........ : Paramètres :
Suite ........ : ------------
Suite ........ : _pstTypeAgregat : RUBRIQUE ou FOURCHETTE
Suite ........ : _psAgregat : Agrégat à calculer (dans le cas d'une
Suite ........ :    fourchette, on peut ajouter |SM, |SD ou |SC pour indiquer le
Suite ........ :    type de solde à calculer
Suite ........ : _pstPeriode : période de calcul
Suite ........ : _pstTypeEcr : Type d'écriture (permet aussi de passer le
Suite ........ :    type de solde)
Suite ........ : _pstEtablissement : Etablissement
Suite ........ : _pstDevise : Devise
Mots clefs ... :
*****************************************************************}
function THalleyWindow.CumulAgregat (const pstTypeAgregat, pstAgregat, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise : Widestring ) : variant;
var
  lDateDebut, lDateFin : TDateTime;
  Err : integer;
  Exo : TExoDate;
...
begin
  ...
  if WhatDate(pstPeriode, lDateDebut, lDateFin, Err, Exo) then
  begin
    if FCumul <> nil then LeCumul := FCumul
    else LeCumul := TZCumul.Create;
    try
      LeCumul.ModeFonc := mfMemory;
      LeCumul.InitCriteres(pstTypeEcr,pstEtablissement,pstDevise,pstPeriode);
      Result := LeCumul.GetValeur(pstTypeAgregat,pstAgregat,nil);
    finally
      if FCumul = nil then LeCumul.Free;
    end;
  end else Result := TraduireMemoire('#Erreur : problème de date');
end;
...
Mots clefs ... :
*****************************************************************}
function THalleyWindow.CumulAgregatDetail (const pstTypeAgregat, pstAgregat, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise, pstCount  : widestring ) : variant;
var
  lDateDebut, lDateFin : TDateTime;
  Err : integer;
  Exo : TExoDate;
...
begin
  ...
  begin
    ...
    try
      ...
      LeCumul.InitCriteres(pstTypeEcr,pstEtablissement,pstDevise,pstPeriode);
      LeCumul.AvecLibelle := True;
      LeCumul.GetValeur(pstTypeAgregat,pstAgregat,LesCptes);
      if (pstCount='1') then
      begin
        Result := LesCptes.Count;
        exit;
      end;
      V :=VarArrayCreate([0, LesCptes.Count-1], varVariant);
      for i := 0 to LesCptes.Count - 1 do
      begin
        lstCompte := LesCptes[i];
        Vi :=VarArrayCreate([0, 2], varVariant);
        lSt := lstCompte;
        Vi[0] := ReadTokenPipe(lSt,':'); // Numéro de compte
        // SM
        if not ((Pos('/',pstTypeEcr)>0) or (Pos('\',pstTypeEcr)>0)) then
          Vi[2] := Valeur(ReadTokenPipe(lSt,':'))
          else ReadTokenPipe(lSt,':');
        // SC
        if (Pos('\',pstTypeEcr)>0) then Vi[2] := Valeur(ReadTokenPipe(lSt,':'))
          else ReadTokenPipe(lSt,':');
        if (Pos('/',pstTypeEcr)>0) then Vi[2] := Valeur(ReadTokenPipe(lSt,':'))
          else ReadTokenPipe(lSt,':');

        // Détermination du libellé du compte
        pLib := Pos (#9,lSt);
        if pLib > 0 then
        Vi[1] := Copy(lSt,pLib+1,Length(lSt)-pLib);
...
function THalleyWindow.CumulAgregatDetailHisto(const pstTypeAgregat,
  pstAgregat, pstTypeEcr, pstEtablissement, pstDevise, pstCount : Widestring): variant;

  procedure _ChargeLeDetail (LesCptes : TStringList; pstPeriode : string; pT : TOB ; pstTypeSolde : string);
  var
      i, pLib : integer;
      lT : TOB;
      lSt : string;
  begin
    ...
    begin
      ...
      lT := TOB.Create ('',pT,-1);
      lSt := LesCptes[i];
      lT.AddChampSupValeur('COMPTE',ReadTokenPipe(lSt,':'));

      // Solde Mixte
      if (pstTypeSolde = 'SM') then
        lT.AddChampSupValeur('VALEUR', Valeur(ReadTokenPipe(lSt,':')))
      else ReadTokenPipe(lSt,':');

      // Solde créditeur
      if (pstTypeSolde = 'SC') then
        lT.AddChampSupValeur('VALEUR', Valeur(ReadTokenPipe(lSt,':')))
      else ReadTokenPipe(lSt,':');

      // Solde débiteur
      if (pstTypeSolde = 'SD') then
        lT.AddChampSupValeur('VALEUR', Valeur(ReadTokenPipe(lSt,':')))
      else ReadTokenPipe(lSt,':');
      ...
      pLib := Pos (#9,lSt);
      if pLib > 0 then
        lT.AddChampSupValeur('LIBELLE', Copy(lSt,pLib+1,Length(lSt)-pLib));
    end;
  end;

  procedure _RangeLeDetail ( pTHisto, pT : TOB; pstNiveau : string);
    procedure __InitLaLigne (lTLigne : TOB ; pstCompte, pstLibelle : string );
    begin
      lTLigne.AddChampSupValeur('COMPTE',pstCompte);
      lTLigne.AddChampSupValeur('LIBELLE',pstLibelle);
      lTLigne.AddChampSupValeur('N----',0);
      lTLigne.AddChampSupValeur('N---',0);
      lTLigne.AddChampSupValeur('N--',0);
      lTLigne.AddChampSupValeur('N-',0);
      lTLigne.AddChampSupValeur('N',0);
  ...
  begin
    ...
    begin
      bTraite := False;
      lstCpte := pT.Detail[i].GetValue('COMPTE');
      // On insère systématiquement le premier enregistrement dans l'historique
      if pTHisto.Detail.Count = 0 then
      begin
        lT := TOB.Create ('',pTHisto,-1);
        __InitLaLigne (lT,pT.Detail[i].GetValue('COMPTE'),pT.Detail[i].GetValue('LIBELLE'));
      end;
      // On recherche la position du nouvel enregistrement à insérer ou mettre à jour
      for k := memK to pTHisto.Detail.Count - 1 do
      begin
        if (pTHisto.Detail[k].GetValue('COMPTE') < lstCpte) then continue
        else if (pTHisto.Detail[k].GetValue('COMPTE') = lstCpte) then
        begin
          // J'ai trouvé le compte dans la liste ==> je mets à jour
          pTHisto.Detail[k].AddChampSupValeur(pstNiveau,pT.Detail[i].GetValue('VALEUR'));
          // Je mémorise la position
          memK := k;
          // On sort
          bTraite := True;
          break;
        ...
        begin
          // On a dépassé la position, il faut donc ajouter la ligne
          lT := TOB.Create ('',pTHisto,memK);
          __InitLaLigne (lT, pT.Detail[i].GetValue('COMPTE'),pT.Detail[i].GetValue('LIBELLE'));
          lT.PutValue(pstNiveau, pT.Detail[i].GetValue('VALEUR'));
          bTraite := True;
          break;
        end;
      end;
      // Si le compte n'a pas été traité, on l'ajoute à la fin de la liste
      if not bTraite then
      begin
        lT := TOB.Create ('',pTHisto,-1);
        __InitLaLigne (lT, pT.Detail[i].GetValue('COMPTE'),pT.Detail[i].GetValue('LIBELLE'));
  ...
  procedure _CalculHistorique ( pCumul : TZCumul; psTypeAgregat, pstAgregat, pstPeriode, pstTypeEcr,pstEtablissement,pstDevise : string ; pTHisto : TOB);
  var
    LesCptes : TStringList;
    lT : TOB;
    lStTypeSolde : string;
  begin
    LesCptes := TStringList.Create;
    if pCumul.InitCriteres(pstTypeEcr,pstEtablissement,pstDevise,pstPeriode) then
    begin
      // Détermination du type de solde (SM, SC ou SD)
      if (Pos('\',pstTypeEcr)>0) then lstTypeSolde := 'SC'
      else if (Pos('/',pstTypeEcr)>0) then lstTypeSolde := 'SD'
      else lstTypeSolde := 'SM';
      // Récupération de la valeur de l'agrégat via le TZCumul
      pCumul.AvecLibelle := True;
      pCumul.GetValeur(pstTypeAgregat,pstAgregat,LesCptes);
      // Enregistrement des informations en mémoire
      lT := TOB.Create ('',nil,-1);
      _ChargeLeDetail ( LesCptes, pstPeriode, lT, lstTypeSolde);
      // Classement des informations
      _RangeLeDetail (pTHisto, lT, pstPeriode);
      // On libère
      lT.Free;
      LesCptes.Free;
    end;
...
begin
  ...
  LeCumul.ModeFonc := mfMemory;
  lTHisto := TOB.Create ('',nil,-1);
  try
    // On calcul les valeurs de l'historique
    _CalculHistorique ( LeCumul, pstTypeAgregat, pstAgregat, 'N+', pstTypeEcr,
                                          pstEtablissement,pstDevise, lTHisto);
    _CalculHistorique ( LeCumul, pstTypeAgregat, pstAgregat, 'N', pstTypeEcr,
                                          pstEtablissement,pstDevise, lTHisto);
    _CalculHistorique ( LeCumul, pstTypeAgregat, pstAgregat, 'N-', pstTypeEcr,
                                          pstEtablissement,pstDevise, lTHisto);
    ...
    _CalculHistorique ( LeCumul, pstTypeAgregat, pstAgregat, 'N----', pstTypeEcr,
                                          pstEtablissement,pstDevise, lTHisto);
    // On déverse le tout dans un tableau
    if pstCount = '1' then Result := lTHisto.Detail.Count
    else
    begin
      V := VarArrayCreate([0, lTHisto.Detail.Count - 1], varVariant);
      for i := 0 to lTHisto.Detail.Count - 1 do
      begin
        Vi := VarArrayCreate([0, 7], varVariant);
        Vi[0] := lTHisto.Detail[i].GetValue('COMPTE');
        Vi[1] := lTHisto.Detail[i].GetValue('LIBELLE');
        Vi[2] := lTHisto.Detail[i].GetValue('N----');
        Vi[3] := lTHisto.Detail[i].GetValue('N---');
        Vi[4] := lTHisto.Detail[i].GetValue('N--');
        Vi[5] := lTHisto.Detail[i].GetValue('N-');
        Vi[6] := lTHisto.Detail[i].GetValue('N');
        Vi[7] := lTHisto.Detail[i].GetValue('N+');
        V[i] := Vi;
      end;
      Result := V;
    end;
...
Créé le ...... : 30/10/2007
Modifié le ... :   /  /
Description .. : Calcul d'un agrégat à partir du complément de la fourchette
Mots clefs ... :
*****************************************************************}
function THalleyWindow.CumulAgregatFourchetteComplement( const pstAgregat,
  pstCpteMin, pstCpteMax, pstPeriode, pstTypeEcr, pstEtablissement, pstDevise: widestring): variant;
begin
  // Contrôle préalables
  Result := JaiLeDroit;
  if Result <> '' then exit;
  ...
  // Traitement
  Result := CumulAgregat ('FOURCHETTE', FourchetteComplement(pstAgregat, pstCpteMin, pstCpteMax),
      pstPeriode, pstTypeEcr, pstEtablissement, pstDevise);
end;

function THalleyWindow.CumulAgregatComplementDetailHisto(
  const pstAgregat, pstCpteMin, pstCpteMax, pstTypeEcr, pstEtablissement,
  pstDevise, pstCount: Widestring): variant;
begin
  Result := CumulAgregatDetailHisto('FOURCHETTE',
    FourchetteComplement(pstAgregat,pstCpteMin,pstCpteMax), pstTypeEcr, pstEtablissement, pstDevise, pstCount);
end;

function THalleyWindow.FermerCumulAgregat: variant;
begin
  // Contrôle préalables
  Result := JaiLeDroit;
  if Result <> '' then exit;
...
function THalleyWindow.OuvrirCumulAgregat: variant;
begin
  // Contrôle préalables
  Result := JaiLeDroit;
  if Result <> '' then exit;
  ...
  if FCumul = nil then FCumul := TZCumul.Create;
  Result := '';
end;
