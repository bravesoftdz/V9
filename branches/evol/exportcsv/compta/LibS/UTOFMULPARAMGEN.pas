...
{$ENDIF EAGLCLIENT}
 ...
 FBoToucheCtrl := GetKeyState(VK_CONTROL) < 0 ;
  ...
  begin
    ...
    {$IFNDEF COMPTAPAIE}
    SetControlVisible ('BINSERT', ( JaiLeDroitConcept(TConcept(ccGenCreat),False) ) and ( gszFonction='C' ) and ( not SaisieLancer ) ) ;
    {$ELSE}
  ...
  begin
    SetControlVisible ('BINSERT', ( JaiLeDroitConcept(TConcept(ccJalCreat),False) ) and ( not SaisieLancer ) );
  ...
  begin
    ...
    if ( gszFonction = 'M' ) then
     begin
      SetControlVisible ('BINSERT', ( JaiLeDroitConcept(TConcept(ccAuxCreat),False) ) ) ;
      TToolBarButton97( GetControl('BOUVRCPTE1') ).Hint := TraduireMemoire('Valider') ;
     end
    else
      SetControlVisible ('BINSERT', ( JaiLeDroitConcept(TConcept(ccAuxCreat),False) ) and ( not SaisieLancer ) ) ;
...
var lStAxe   : String ;
    ...
    lInForm  : Longint ;
begin
  if (GetNomEcran = 'MULGENERAUX') and Not JaiLeDroitConcept(TConcept(ccGenModif),True) then Exit ;
  if (GetNomEcran = 'MULJOURNAL') and Not JaiLeDroitConcept(TConcept(ccJalModif),True) then Exit ;
  if (GetNomEcran = 'MULTIERS') and Not JaiLeDroitConcept(TConcept(ccAuxModif),True) then Exit ;
  lInForm  := Longint( Ecran ) ;
  lStAxe   := GetAxe ;
  lStCpt   := GetField(GetFieldName) ;
  lStTable := GetTableName ;
