...
begin
  ...
  begin
    ...
    NumTable := TableToNum(PrefixeToTable(Prefixe));
    {$IFDEF EAGLCLIENT}
    if High(V_PGI.DEChamps[NumTable]) <= 0 then
      ChargeDeChamps(NumTable, Prefixe);
    {$ENDIF EAGLCLIENT}
    if NumTable > 0 then
    begin
      {$IFDEF EAGLSERVER}
      for NumChamp := Low(V_PGI.HDeChamps[LookupCurrentSession.SocNum, NumTable]) to High(V_Pgi.HDeChamps[LookupCurrentSession.SocNum, NumTable]) do
      {$ELSE  EAGLSERVER}
      for NumChamp := Low(V_PGI.DeChamps[NumTable]) to High(V_Pgi.DeChamps[NumTable]) do
      {$ENDIF EAGLSERVER}
      begin
        NomChamp := V_PGI.DEChamps[NumTable, NumChamp].Nom;
        IsBlob := (pos(V_PGI.DEChamps[NumTable, NumChamp].tipe, 'BLOB;DATA') > 0);
