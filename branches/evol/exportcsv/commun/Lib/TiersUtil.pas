...
begin
  Result := '';
  QQ := OpenSQL('SELECT T_TIERS FROM TIERS WHERE T_IDSPIGAO='+InttoStr(IDSPIGAO)+' AND T_NATUREAUXI IN ("CLI","PRO")',True,1,'',true);
  if Not QQ.eof then
  begin
...
var QQ : TQuery;
begin
  ExecuteSQL('UPDATE TIERS SET T_IDSPIGAO='' WHERE T_IDSPIGAO="'+InttOstr(IDSPIGAO)+'" AND T_NATUREAUXI IN ("CLI","PRO")');
  ExecuteSQL('UPDATE TIERS SET T_IDSPIGAO='+InttOstr(IDSPIGAO)+' WHERE T_TIERS="'+CodeTiers+'" AND T_NATUREAUXI IN ("CLI","PRO")');
end;
...
begin
 Result :=
  ((NatureAuxi = 'CLI') and JaiLeDroitConcept(TConcept(gcCLICreat),False) and JaiLeDroitConcept(TConcept(gcCLIModif),False))
  or
  ((NatureAuxi = 'FOU') and JaiLeDroitConcept(TConcept(gcFOUCreat),False) and JaiLeDroitConcept(TConcept(gcFOUModif),False))
    or
  ((NatureAuxi = 'PRO') and JaiLeDroitConcept(TConcept(gcProspectCreat),False) and JaiLeDroitConcept(TConcept(gcProspectModif),False));
end;
