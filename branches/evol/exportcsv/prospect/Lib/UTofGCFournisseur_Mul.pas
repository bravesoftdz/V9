...
begin
  for k:=1 to 30 do
  begin
    i := PrefixeToNum(ExtractPrefixe(TabChamp[k]));
    j := ChampToNum(TabChamp[k]);
    if (i>-1) and (j>-1) then
    begin
      if del then
      begin
        TabControl[k]:=V_PGI.DEChamps[i, j].Control;
        V_PGI.DEChamps[i, j].Control:='';
      end
      else V_PGI.DEChamps[i, j].Control:=TabControl[k];
    end;
  end;
end;
...
(*
  ...
  if JaiLeDroitConcept(TConcept(bt511),False) then
  begin
