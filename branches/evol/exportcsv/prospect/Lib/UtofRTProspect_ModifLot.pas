...
if (F.Name = 'RTQUALITE') or (F.Name = 'RTQUALITE_SUSP') then
   ...
   if (Argument = 'CLI') or (Argument = 'FOU') then
       ...
       for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
           ...
           if Pos(V_PGI.DEChamps[iTable,iChamp].nom,V_PGI.DEVues[VueToNum('RTTIERS')].SQL ) = 0 then continue;

//           if V_PGI.Dechamps[iTable,iChamp].libelle[1] <> '.' then
           if Copy(V_PGI.Dechamps[iTable,iChamp].libelle,1,1) <> '.' then
              begin
              ListeChamp.add(V_PGI.DEChamps[iTable,iChamp].nom);
              ListeLibelle.add (V_PGI.Dechamps[iTable,iChamp].libelle);
              ListeChamp1.add(V_PGI.DEChamps[iTable,iChamp].nom);
              ListeLibelle1.add (V_PGI.Dechamps[iTable,iChamp].libelle);
              end;
           end;
       ...
       if (Argument = 'PRO') then
         ...
         begin
         iTable := PrefixeToNum ('RSU');
         ChargeDeChamps(iTable,'RSU') ;
         for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
             begin
             if pos( ctrl,V_PGI.DEChamps[iTable,iChamp].Control)=0 then continue ;
