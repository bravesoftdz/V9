...
{$ENDIF EAGLSERVER}
  ...
  else SetField('RAC_HEUREACTION', Time);
  if Planning and (TobAct.GetValue('RAC_TYPEACTION') <> '') then
  begin
  ...
  if Planning then
  	stIntervenant := TobAct.GetValue('RAC_INTERVENANT');
...
//    SetField ('RAC_INTERVINT',StIntervint);
  ...
  if iPerspective <> 0 then
     SetField('RAC_PERSPECTIVE',iPerspective);
...
{$ENDIF EAGLSERVER}
  if stAffaire <> '' then
     SetField('RAC_AFFAIRE',stAffaire);
  SetField ('RAC_DUREEACTION',30);
  if iIdentParc <> 0 then
    SetField('RAC_IDENTPARC',iIdentParc);
{$IFDEF QUALITE}
  if stProduitPGI <> 'QNC' then
  begin
  	if (Ecran is TFFiche) then
  ...
  Repeat
      ...
      begin
          ...
          begin
             ...
             if ChampMul='RAC_PROJET' then
             begin
                stProjet := ValMul;
             end;
             if ChampMul='RAC_OPERATION' then
             begin
                stOperation := ValMul;
             end;
             if ChampMul='RAC_TIERS' then
             begin
                stTiers := ValMul;
             end;
             if ChampMul='DUPLICATION' then
             begin
                x:=pos('|',ValMul);
                stTiersDuplic := copy(ValMul,1,x-1);
                iNumActDuplic := strToInt(copy(ValMul,x+1,length(ValMul)));
             end;
             if ChampMul='RAC_PERSPECTIVE' then
             begin
                iPerspective := strToInt(ValMul);
                SetControlEnabled('NUM_PERSPECTIVE',FALSE) ;
             end;
             if ChampMul='RAC_NUMEROCONTACT' then
             begin
                iNumeroContact := strToInt(ValMul);
             end;
             if ChampMul='RAC_INTERVENANT' then
                stIntervenant := ValMul;
             if ChampMul='RAC_NUMCHAINAGE' then
                iNumChainage := StrToInt(ValMul);
             if ChampMul='RAC_AFFAIRE' then
                stAffaire := ValMul;
             if ChampMul='PRODUITPGI' then
                stProduitpgi := ValMul;
             if ChampMul='RAC_IDENTPARC' then
                iIdentParc := Valeuri(ValMul);
{$IFDEF QUALITE}
             if ChampMul='RAC_QNCNUM' then
                iQncNum := Valeuri(ValMul);
             if ChampMul='RAC_QPLANCORRNUM' then
                iPlanCorrNum := Valeuri(ValMul);
             if ChampMul='RQN_QLITIGIEUSE' then
                iQLitigieuse := Valeuri(ValMul);
             if ChampMul='ACTION' then
             		sAction := ValMul;
             if ChampMul='QNCORIGINE' then
                sQNCOrigine := ValMul;
             if ChampMul='CLOTUREENCOURS' then
                ClotureEncours := StrToBool_(ValMul);
{$ENDIF QUALITE}
             if ChampMul='ORIGINE' then
             		sOrigine := ValMul;
          end;
          x:=pos('NOCHANGEPROSPECT',Critere);
          if x<>0 then
             NoChangeProspect := True ;
          x:=pos('NOCHANGEPROJET',Critere);
          if x<>0 then
             NoChangeProjet := True ;
          x:=pos('NOCHANGEAFFAIRE',Critere);
          if x<>0 then
            //AffaireEnabled(false);
            NoChangeAffaire:=true;
          x:=pos('FICHETIERS',Critere);
          if x<>0 then
          begin
{$IFNDEF EAGLSERVER}
             if ecran is TFFiche then
...
{$ENDIF EAGLSERVER}
          ...
          x:=pos('NOCHANGECHAINAGE',Critere);  //Liste Chainage
          if x<>0 then NoChangeChainage := true;
          x:=pos('FICHECHAINAGE',Critere);  //Liste Chainage
          if x<>0 then FicheChainage := true;
          x:=pos('CREATPLANNING',Critere);  //Planning
          if x<>0 then
          begin
             Planning := true;
{$IFNDEF EAGLSERVER}
...
{$ENDIF EAGLSERVER}
          ...
          x:=pos('MODIFPLANNING',Critere);  //Planning
          if x<>0 then
          begin
             Planning := true;
          end;
          x:=pos('NOCREAT',Critere);  // Analyse dynamique des actions
          if x<>0 then
             NoCreat := True ;
      end;
  until  Critere='';
...
{$IFNDEF EAGLSERVER}
  InitGrids ;
{$ENDIF EAGLSERVER}
if (stProduitpgi<>'GRF') and (stProduitPGI<>'QNC') then
	stProduitpgi:='GRC';
...
begin
  ...
  AddRDV(getfield ('RAC_LIBELLE'),GetControlText('TRAC_TIERS'),Memo.Text,getfield ('RAC_DATEACTION'),StrToDateTime (Heure) ,ActFormatDuree) ;
  end;
  if Ajout='TACHE' then
  begin
