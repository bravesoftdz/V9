...
procedure DonneCodeChpJoinPGINbNivLib(TabCodeAxe:array of hString;
          niveauI:integer;TablePrinc:string;
...
procedure ChercheCodeSqlChpAxeLibPGI(nivMax:integer; InitTree:boolean;
          TabCodeAxe:array of hString;TableP:string;
          var chpSql,joinSql:hString);
...
function SessionObjInitDelai(const session:hString):boolean;

procedure dateDebFinSesssion(CodeS:hString;var DateD,DateF:TDateTime);

implementation

uses Controls,SysUtils,HMsgBox,HCtrls,
     {$IFNDEF EAGLCLIENT}
     {$IFNDEF DBXPRESS}dbtables{BDE},{$ELSE}uDbxDataSet,{$ENDIF}
     {$else} uTob, {$ENDIF}
     {$IFDEF PAIEGRH}EntPaie,{$ENDIF PAIEGRH}
     UCtx,UCtxTrait,CstCommun,UUtil,BPUtil,DateUtils,BPFctSession;
...
begin
  ...
  begin
    ...
    if ((ContextBP=3) AND (StrToInt(CodeAxe) in [12,13,14,15])) then chp:='PSA'+Q.fields[0].asString
    else
    begin
      if joinX=''
...
begin
  ...
  SetLength(TabJoin,nivMax+1);
  for j:=1 to nivMax do
  begin
    if TabCodeAxe[j] = '' then continue;
    ...
    begin
      ...
      { EV5 / Gestion des booléens libres PAIE }
      if ((ContextBP=3) AND (StrToInt(TabCodeAxe[j]) in [12,13,14,15])) then
      begin
        if Q.fields[6].asString = '' then nomRequete:='PSA'+Q.fields[5].asString
        else
        begin
          if InitTree then nomRequete:='Z'+IntToStr(nivTree)+'.PSA'+Q.fields[5].asString
          else nomRequete:='Z'+IntToStr(j)+'.PSA'+Q.fields[5].asString
        end;
      end
      else
      begin
      ...
      if ((TableP<>'') AND (TableP=table)) then
      begin
        if chpSql<>'' then chpSql:=chpSql+' ,'+chpLib
        else chpSql:=chpLib;
      end
      else
      begin
      ...
      { EV5 / Correction : remise à '' du codewhere }
      if (nomchp2<>'') and (valchp2<>'') then codewhere:=' AND A'+IntToStr(j)+'.'+nomchp2+'="'+valchp2+'" '
      else codewhere:='';

      TabJoin[j]:=' left join '+table+' A'+IntToStr(j)+' on A'+IntToStr(j)+'.'+chpcode+'='+nomRequete+' '+codewhere;
      if ((ContextBP=3) AND (TabCodeAxe[j]='011')) then IndiceSalarie:=j;
    end;
    ferme(Q);
  end;

  if IndiceSalarie<>0 then joinSql:=TabJoin[IndiceSalarie];

  for i:=1 to nivmax do
  begin
    if i<>IndiceSalarie then joinSql:=joinSql+TabJoin[i];
  end;

  { EVI / Correction : Left Join Self }
  if ((TableP<>'') AND (TableP=table)) then joinSql:='';

  if chpsql<>'' then chpsql:=','+chpsql;
end;
...
begin
 ...
 for i:=1 to niveauI do
  begin
   ...
   chpSS:='';
   if i=niveauI then
     ChercheCodeSqlChpAxeLibPGI(niveauI,true,['',TabCodeAxe[i]],TablePrinc,chpSS,joinSS);
...
begin
  ...
  begin
    ...
    if table <>'' then
    begin
      if (nomchp2<>'') and (valchp2<>'') then codewhere:=' AND '+nomchp2+'="'+valchp2+'" ';
      Q2:=MOPenSql('SELECT '+chplib+' FROM '+table+' WHERE '+
                   chpcode+'="'+ValeurAxe+'" '+codewhere,
                   'BPBasic (DonneLibelleValeurAxe).',true);
      if not Q2.eof then result:=Q2.fields[0].asString;
      ferme(Q2);
      if ((result='') AND (table='SALARIES')) then
      begin
        //Nouvelle Embauche pour Paie
        Q2:=OPenSql('SELECT CC_LIBELLE FROM CHOIXCOD WHERE CC_CODE="'+copy(ValeurAxe,0,2)+'" '+
                     'AND CC_TYPE="PTK"',true);
        if not Q2.eof then result:=Q2.fields[0].asString + ' ' + IntToStr(StrToInt(Copy(ValeurAxe,3,8)));
        ferme(Q2);
      end;
    end;
  end
...
begin
  ...
  begin
    ...
    begin
      ...
      if table='' then
      begin
        if chpSql<>'' then chpSql:=chpSql+',A'+IntToStr(j)+'.QBA_LIBVALEURAXE '
        else chpSql:=' A'+IntToStr(j)+'.QBA_LIBVALEURAXE ';
        if j=nivMax then joinSql:=joinSql+' left join QBPAXE A'+IntToStr(j)+
      ...
      begin
        ...
        if j=nivMax
        then
        TabJoin[j]:=' left join '+table+' A'+IntToStr(j)+' on A'+IntToStr(j)+'.'+chpcode+'=QBR_VALEURAXE '+codewhere
        else
        TabJoin[j]:=' left join '+table+' A'+IntToStr(j)+' on A'+IntToStr(j)+'.'+chpcode+'=QBR_VALAXENIV'+IntToStr(j)+codewhere;
        if ((ContextBP=3) AND (TabCodeAxe[j]='011')) then IndiceSalarie:=j;
      end;
    end;
    ferme(Q);
...
var codeOP,nbVal:hString;
    ...
    TabValAffPaie: array [0..7] of hString;
begin
  codeOP:='Prévision';
  ...
  begin
    ...
    Case ContextBP of
      ...
      3 : begin //Paie
            ...
            for i:= 1 to StrToInt(TabValAffPaie[0]) do
            begin
              if i=1 then
              begin
                //Quantité
                ChangeLibelleChamp('QBR_QTEREF','Valeur réference ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_QTEREFPRCT','% référence ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_QTEC',CodeOP+' ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_QTECPRCT','Nouveau % répartition ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_SAISIQTE',CodeOP+' saisi ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_EVOLQTE','Montant évolution ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_EVOLQTEPRCT','% évolution ('+TabValAffPaie[1]+')');
                ChangeLibelleChamp('QBR_PRCTVARIATIONQ','% variation ('+TabValAffPaie[1]+')');
              end
              else
              begin
                ChangeLibelleChamp('QBR_REF'+IntToStr(i-1),'Valeurs réference ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_REFPRCT'+IntToStr(i-1),'% référence ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_OP'+IntToStr(i-1),CodeOP+' ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_OPPRCT'+IntToStr(i-1),'Nouveau % répartition ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_SAISI'+IntToStr(i-1),CodeOP+' saisi ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_EVOLVAL'+IntToStr(i-1),'Montant évolution ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_EVOLPRCT'+IntToStr(i-1),'% évolution ('+TabValAffPaie[i]+')');
                ChangeLibelleChamp('QBR_PRCTVARIATION'+IntToStr(i-1),'% variation ('+TabValAffPaie[i]+')');
              end;
            end;
            
            for i:= 2 to StrToInt(TabValAffPaie[0]) do
            begin
              ChangeControlChamp('QBR_REF'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_REFPRCT'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_OP'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_OPPRCT'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_SAISI'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_EVOLVAL'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_EVOLPRCT'+IntToStr(i-1),'L');
              ChangeControlChamp('QBR_PRCTVARIATION'+IntToStr(i-1),'L');
            end;

            for i:= StrToInt(TabValAffPaie[0]) to 6 do
            begin
              ChangeControlChamp('QBR_REF'+IntToStr(i),'');
              ChangeControlChamp('QBR_REFPRCT'+IntToStr(i),'');
              ChangeControlChamp('QBR_OP'+IntToStr(i),'');
              ChangeControlChamp('QBR_OPPRCT'+IntToStr(i),'');
              ChangeControlChamp('QBR_SAISI'+IntToStr(i),'');
              ChangeControlChamp('QBR_EVOLVAL'+IntToStr(i),'');
              ChangeControlChamp('QBR_EVOLPRCT'+IntToStr(i),'');
              ChangeControlChamp('QBR_PRCTVARIATION'+IntToStr(i),'');
            end;
...
begin
  ...
  begin
    ...
    if codeaxe='010' then result:=VH_Paie.PGLibCodeStat;
    if codeaxe='012' then result:=VH_Paie.PgLibCoche1;
    if codeaxe='013' then result:=VH_Paie.PgLibCoche2;
    if codeaxe='014' then result:=VH_Paie.PgLibCoche3;
    if codeaxe='015' then result:=VH_Paie.PgLibCoche4;
  end;
...
var Q: tQuery;
begin
  Q:=MOPenSql('SELECT QBS_DATEDEBC,QBS_DATEFINC FROM QBPSESSIONBP WHERE QBS_CODESESSION="'+CodeS+'"',
              'QUFSLARBRE_TOF (DateDebFinSession).',true);
  if not Q.eof then
  begin
    DateD := Q.Fields[0].AsDateTime;
    DateF := Q.Fields[1].AsDateTime;
  end;
  Ferme(Q);
end;

//test si la session est initialisée
function SessionObjInitDelai(const session:hString):boolean;
var Q:TQuery;
begin
 result:=false;
 Q:=MOPenSql('SELECT QBS_SESSIONINIT FROM QBPSESSIONBP '+
             'WHERE QBS_CODESESSION="'+session+'" ',
             'BPBasic (SessionBPInitialise).',true);
 if not Q.eof then
 begin
   if Q.fields[0].asString='X' then result:=true else result:=false;
 end;
 ferme(Q);
end;
