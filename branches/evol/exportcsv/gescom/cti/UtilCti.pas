...
begin
  ...
  QQ := OpenSQL(Req,True);
	...
	if Not QQ.EOF then
     ...
     Req := Req + 'ADR_ADRESSE1, ADR_ADRESSE2, ADR_ADRESSE3, ';
     Req := Req + 'ADR_CODEPOSTAL, ADR_VILLE, ADR_LIBELLE ';
     Req := Req + 'From ADRESSES Where ADR_CLETELEPHONE="';
     Req := Req + CleTelephone(StNotel) + '"';
     Req := Req + ' And ADR_INT="X"';
