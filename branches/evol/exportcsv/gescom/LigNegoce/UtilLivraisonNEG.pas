...
begin
  ...
  TRY
    ...
    SQL := 'SELECT GL_QTEFACT,GL_QUALIFQTEACH,GL_PUHTDEV AS DPA, '+
           ...
           'GL_NATUREPIECEG IN ("BLF","FF") AND GL_QTERESTE > 0 AND GL_VIVANTE="X"';
    ...
    SQL := 'SELECT SUM(GL_QTEFACT) FROM LIGNE WHERE GL_PIECEORIGINE LIKE "'+EncodeFindRefOrigine(Cledoc)+'"';
           'GL_NATUREPIECEG IN ("BLC","FBC") AND GL_QTERESTE > 0 AND GL_VIVANTE="X" AND NOT ('+WherePiece(CledocPC,ttdLigne,False)+')';
    QQ := OpenSQL(SQL,True,-1,'',true);
    if not QQ.eof then
    begin
