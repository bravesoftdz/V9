  ...
  TModegestion = (TmgIntervenant,TmgCotraitance,TmgSousTraitance);
	...
	TPieceCotrait = class (Tobject)
      ...
      procedure GSAffectSoustrait (Sender : tobject);
    	procedure AffecteSousTraitanceLigSDetail(TOBL : TOB; Fournisseur, ModePaie,LibelleFou,NatureTravail: string);
    	procedure AffecteSousTraitanceOuvrage(TOBL: TOB; Fournisseur, LibelleFou,ModePaie,NatureTravail: string;Var NextLig : integer; OkInc : boolean; PrixBloque,FromExcel : boolean);
    	procedure AffecteSousTraitanceSousDetail(TOBPere,TOBOUV: TOB; IndiceNomen : Integer;Fournisseur,ModePaie,LibelleFou,NatureTravail: string;Var NextLig : integer; OkINc : boolean; PrixBloque,FromExcel : boolean);
    	procedure AffecteSousTraitanceSousDetail1(TOBOUV: TOB;indiceNomen: integer; Fournisseur, ModePaie,LibelleFou,NatureTravail: string; PrixBloque,FromExcel : boolean);
    	procedure SetInfoSoustraitancelig(TOBL: TOB; Fournisseur,ModePaie,LibelleFou: string; var Ligne : integer; PrixBloque,FromExcel : boolean);
    	procedure AffecteSousTraitanceLigne(Fournisseur,ModePaie: string; var Ligne: integer);
      //
...
procedure CalculeReglementsIntervenants(TOBSousTrait,TOBPiece,TOBPieceRG,TOBAcomptes : TOB; CodeAffaire : string;TOBPieceTrait : TOB; DEv : Rdevise; lastfacture : boolean=false);
...
var TOBPT         : TOB;
    ...
    NumCaution    : String;
    EcartRegl,GainSt,RGSt,SommePaie,NetApayer,MtAcomptesSoc : double;
...
//  if TOBPieceTrait.detail[0].GetBoolean('BPE_REGLSAISIE')= true then exit; // evite de recalculer les reglemets si deja saisi
  ...
  begin
    ...
    begin
      ...
      *)
      ...
      //
    	//if TOBPT.getValue('BPE_TYPEINTERV')='Y00' then
      //begin
      GainST := GainSt + (TOBPT.GetValue('BPE_MONTANTREGL') - TOBPT.GetValue('BPE_MONTANTPA'));
      GainST := ARRONDI(GainST,DEv.Decimale);
      if lastFacture then
      begin
    		TOBPT.putvalue('BPE_REGLSAISIE','X');
      end;
  ...
  //
  NetApayer := TOBPiece.GetValue('GP_TOTALTTCDEV') - XD;
...
begin
	result := false;
  if pos (TOBPiece.GetString('GP_NATUREPIECEG'),'ETU;BCE;DBT;FBT;PBT')= 0 then exit;
...
begin
  ...
  case Numtag of
    ...
    325310 : BEGIN
    			   ...
    			   END;
    325320 : {AGLLanceFiche('BTP', 'BTPARAMMAIL', '', '','TYPEMAIL=S');//}BEGIN SaisirHorsInside('BTPARAMMAIL', 'TYPEMAIL=C'); Result := -1 ; end;
...
begin
  ...
  Case Numtag of
    ...
    // Devis
    327211 : AGLLanceFiche('BTP','BTDEVISSTRAITE','GP_NATUREPIECEG=DBT;GP_VENTEACHAT=VEN','','MODIFICATION;SOUSTRAITANCE;STATUT=AFF') ;  // Devis
    327212 : AGLLanceFiche('BTP','BTACCDEVCOT_MUL','GP_NATUREPIECEG=DBT','','MODIFICATION;SOUSTRAITANCE;STATUT=AFF') ;                   // Devis
    327213 : AGLLanceFiche('BTP','BTACCDEVCOT_MUL','GP_NATUREPIECEG=DBT','','MODIFICATION;SOUSTRAITANCE;ACCEPTINVERSE=X;STATUT=AFF') ;   // Devis
...
begin
  ...
  if not TOBSSTrait.InsertDB(nil) then
  begin
    MessageValid := 'Erreur mise à jour des PIECEINTERV';
...
procedure TPieceCotrait.AffecteSousTraitanceLigSDetail( TOBL : TOB; Fournisseur, ModePaie,LibelleFou, NatureTravail: string);
var TOBOUV,TOBOL,TOBLP : TOB;
		Indice : integer;
    indiceNomen,UniqueBlo : integer;
    found : boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if TOBOL.detail.count > 0 then
      begin
        AffecteSousTraitanceSousDetail1 (TOBOL,indiceNomen,Fournisseur,ModePaie,LibelleFou,NatureTravail,PrixBloque,FromExcel);
    		InitTableau (Valeurs);
...
procedure TPieceCotrait.AffecteSousTraitanceOuvrage(TOBL: TOB; Fournisseur,LibelleFou, ModePaie,NatureTravail: string; var NextLig: integer;OkInc: boolean; PrixBloque,FromExcel : boolean);
var TOBOUV : TOB;
		IndiceNomen : integer;
    valeurs:T_Valeurs ;
begin
  ...
  begin
    TOBOUV := TOBOuvrage.detail[IndiceNomen-1];
    AffecteSousTraitanceSousDetail(TOBL,TOBOUV,IndiceNomen,Fournisseur,ModePaie,LibelleFou,NatureTravail,nextLig,OkInc,PrixBloque,FromExcel);
    InitTableau (Valeurs);
    CalculeOuvrageDoc (TOBOUV,1,1,true,fDEV,valeurs,(TOBL.getValue('GL_FACTUREHT')='X'),true,true);
    SetValeurs(TOBL,valeurs);
  end;
...
procedure TPieceCotrait.AffecteSousTraitanceSousDetail1(TOBOUV: TOB;indiceNomen: integer; Fournisseur, ModePaie,LibelleFou, NatureTravail: string; PrixBloque,FromExcel : boolean);
var TOBOL : TOB;
		II : Integer;
    valeurs : t_valeurs;
begin
...
procedure TPieceCotrait.AffecteSousTraitanceSousDetail(TOBPere,TOBOUV: TOB; IndiceNomen: Integer; Fournisseur, ModePaie,LibelleFou,NatureTravail: string; var NextLig: integer; OkINc: boolean; PrixBloque,FromExcel : boolean);
var TOBOL : TOB;
		II: integer;
    IncrementLigne : boolean;
    valeurs : T_Valeurs;
...
procedure TPieceCotrait.SetInfoSoustraitancelig(TOBL: TOB; Fournisseur,ModePaie,LibelleFou: string; var Ligne: integer; PrixBloque,FromExcel : boolean);
...
begin
  ...
  begin
    ...
    if IsOuvrage(TOBL) then
    begin
      AffecteSousTraitanceOuvrage (TOBL,Fournisseur,LibelleFou,ModePaie,'002',Ligne,true,PrixBloque,FromExcel);
    end else Inc(Ligne);
  end else
  *)
  begin
...
procedure TPieceCotrait.AffecteSousTraitanceLigne(Fournisseur,ModePaie: string;var Ligne: integer);
var TOBL : TOB;
		LibelleFou : string;
...
begin
  ...
  begin
    ...
    if IsSousDetail (TOBL) then
    begin
      AffectesousTraitanceLigSDetail (TOBL,Fournisseur,ModePaie,LibelleFou,'002');
      Inc(Ligne);
      exit;
    end;
    SetInfoSoustraitancelig (TOBL,Fournisseur,ModePaie,LibelleFou,Ligne,PrixBloque,FromExcel);
    //
  end;
end;
...
begin
  ...
  TFFacture(FF).HMTrad.ResizeGridColumns(GS);
