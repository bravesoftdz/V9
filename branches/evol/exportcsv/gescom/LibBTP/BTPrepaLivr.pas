...
//  result := result + 'AND ((GL_QTERESTE <> 0 AND ((GL_TYPELIGNE="ART") OR (GL_TYPELIGNE="ARV"))) OR GL_TYPELIGNE <> "ART")';
  result := result + 'AND ((GL_QTERESTE <> 0 AND ((GL_TYPELIGNE="ART") OR (GL_TYPELIGNE="ARV"))) OR ((GL_TYPELIGNE <> "ART") AND (GL_TYPELIGNE <> "ARV")) )';
      ...
      begin
        ...
        begin
          ...
          Requete := 'SELECT LIG.*,PIE.GP_REPRESENTANT,PIE.GP_REFINTERNE,PIE.GP_NUMADRESSEFACT,PIE.GP_NUMADRESSELIVR,PHA.BLP_PHASETRA FROM LIGNE LIG '+
                     ...
                     '((GL_TYPELIGNE IN ("ART","ARV")  AND (GL_QTERESTE<>0) AND (GL_DATELIVRAISON <= "'+UsDateTime(DateFin)+'")) OR ( NOT GL_TYPELIGNE IN ("ART","ARV"))) ORDER BY GL_NUMLIGNE';
...
var chaine : string;
begin
  chaine := 'UPDATE LIGNE SET GL_QTERESTE=0 WHERE GL_NATUREPIECEG="'+cledoc.NaturePiece+'" AND ' +
