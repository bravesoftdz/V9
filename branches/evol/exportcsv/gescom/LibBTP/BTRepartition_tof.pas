...
Type
  ...
  private
  	SG_SEL,SG_AFFAIRE,SG_NATUREPIECE,SG_NUMERO,SG_DATELIVPREVU,SG_GL_QTEFACT,SG_GL_QTERELIQUAT : integer;
  	LastSaisie : string;
  	PHaut,PBas : THpanel;
    LARTICLE : THLabel;
  ...
  repeat
    ...
    begin
    	...
    	if LeChamps = 'AFFAIRE' then SG_AFFAIRE := Indice else
  		...
  		if LeChamps = 'GL_QTERELIQUAT' then SG_GL_QTERELIQUAT := indice;
      inc (indice);
  		leChamps := READTOKENST (TheChamps);
    end;
  until LeChamps = '';
...
begin
  ...
  begin
    LesChamps := 'SEL;AFFAIRE;NATUREPIECE;NUMERO;DATELIVPREVU;GL_QTEFACT;GL_QTERELIQUAT';
    NbrCols := 7;
    Ecran.caption := 'Détail de la ligne '+IntToStr(MyOwnTOB.getValue('LIGNE'));
  end;
...
begin
  ...
  begin
  	sok := false;
  	if (PGIAsk('Qté répartie différente de celle demandé. Confirmez-vous ?', 'Répartition') = mrYes) Then
    begin
    	Sok := true;
      Force := true;
    end;
  ...
  begin
    ...
    TheTOB := LaTOB;
    if force then TheTOB.putValue('RETOUR',1) else TheTOB.putValue('RETOUR',0);
  end else TFVierge(ecran).ModalResult := 0;
end;
