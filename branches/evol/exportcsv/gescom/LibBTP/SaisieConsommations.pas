...
begin
  ...
  begin
    ...
    begin
      ...
      if TOBL.GetValue('NEW')='X' Then
      //if TOBL.GetValue('BCO_NUMMOUV') = 0 Then
      begin
        TheRetour := GetNumUniqueConso (UnNumero);
        if TheRetour = gncAbort then BEGIN V_PGI.IOError := oeUnknown; Exit; END;
        TOBL.putValue ('BCO_NUMMOUV',UnNumero);
...
begin
  ...
  if TOBLIENEQUIPE_O.detail.count > 0 then
  begin
    for Indice := 0 to TOBLIENEQUIPE_O.detail.count -1 do
    begin
      For II := 0 to TOBLIENEQUIPE_O.detail[Indice].detail.count -1 do
      begin
        TOBL := TOBLIENEQUIPE_O.detail[Indice].detail[II];
        TOBL.DeleteDB(false);
      end;
    end;
  end;
...
begin
  ...
  Begin
    ...
    begin
      ...
      else if (TobApp.GetString('AFF_ETATAFFAIRE')<>'REA') then
...
begin
  ...
  begin
    TToolBarButton97(GetControl('BDUPLIC')).visible      := True;
...
begin
  ...
  if CHOIX_APPEL.checked then
     Sql := 'SELECT AFF_AFFAIRE, AFF_LIBELLE, AFF_TIERS, AFF_RESPONSABLE, AFF_AFFAIREINIT, AFF_STATUTAFFAIRE, AFF_ETATAFFAIRE, AFF_DOMAINE FROM AFFAIRE WHERE AFF_AFFAIRE="'+ Ch_CHANTIER.text +'"'
  else
     Sql := 'SELECT AFF_LIBELLE, AFF_DOMAINE, AFF_ETATAFFAIRE FROM AFFAIRE WHERE AFF_AFFAIRE="'+ Ch_CHANTIER.text +'"';
...
begin
  ...
  StSQL := 'SELECT AFF_AFFAIRE FROM AFFAIRE WHERE AFF_AFFAIRE LIKE "' + CodeAffaire + '%"';
