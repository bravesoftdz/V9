...
begin
  ...
  if (Pos(NaturePiece ,'FBT;ABT;FAC;AVC;FBC;FBP;')>0) Then
  begin
...
begin
  ...
  if (Pos(NaturePiece,'FBT;FBP')>0) or (NaturePiece = 'FAC') Then
     TOBTMP.PutValue('FACTURE_SALARIE', TOBTMP.GetValue('FACTURE_SALARIE')+MontantPV)
  else if (NaturePiece = 'ABT') or (NaturePiece = 'AVC') or (NaturePiece = 'ABP')  then
     TOBTMP.PutValue('FACTURE_SALARIE', TOBTMP.GetValue('FACTURE_SALARIE')+MontantPV)
  else
  begin
...
begin
  ...
  if (Pos(NaturePiece,'FBT;FBP')>0) or (Pos(NaturePiece,'ABT;ABP')>0) or (NaturePiece = 'FAC') or (NaturePiece = 'AVC') Then
  begin
...
// BRL 13/08 : ces 3 montants ne concernent que la prévision de chantier PBT
  ...
  //
  if TOBTMP.FieldExists('PREVU_'+NaturePiece+'_PA') then TOBTMP.PutValue('PREVU_' + NaturePiece +'_PA', TotalMtPA);
  if TOBTMP.FieldExists('PREVU_'+NaturePiece+'_PR') then TOBTMP.PutValue('PREVU_' + NaturePiece +'_PR', TotalMtPR);
  if TOBTMP.FieldExists('PREVU_'+NaturePiece+'_PV') then TOBTMP.PutValue('PREVU_' + NaturePiece +'_PV', TotalMtPV);
  //
  ...
  if not ((Pos(NaturePiece,'FBT;FBP')>0) or (Pos(NaturePiece,'ABT;ABP')>0) or (NaturePiece = 'FAC') or (NaturePiece = 'AVC')) Then
  begin
    if (TypeRes <> 'FOU') and (TOBTMP.GetValue('CEclatNatPrest') = 'X') then
    begin
...
begin
  ...
  begin
    ...
    begin
  	  WherePiece := ' ((GP_NATUREPIECEG="'+NaturePiece+'") OR ((GP_NATUREPIECEG="FBP") AND (GP_VIVANTE="X"))) '+
