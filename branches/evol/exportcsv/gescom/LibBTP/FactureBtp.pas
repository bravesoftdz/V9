...
function TraitePrixOuvrage(TOBPiece, TOBL, TOBBases, TOBBasesL,TOBOuvrage: TOB; EnHt: boolean; PuFix,PuReference: double; DEV: Rdevise; EnPA: boolean = false; FromReference : boolean=false; AffecteEcartLig : Boolean=true): boolean;
...
begin
  ...
  //Begin
  //  PGIError(TraduireMemoire('Impression impossible. Pièce non visée !'),TraduireMemoire('Gestion des Visas'));
  //  Exit;
  //end;
  //
  FFAct := TFFacture( FF);
...
begin
  ...
  begin
    ...
    if Not JaiLeDroitConcept(TConcept(bt513),False) then FFact.GP_REMISEPIED.Enabled:=False; //confidentialité , concept acces remise
    if Not JaiLeDroitConcept(TConcept(bt514),False) then FFact.GP_ESCOMPTE.Enabled:=False; //confidentialité , concept acces escompte
    if Not JaiLeDroitConcept(TConcept(bt515),False) then FFact.SIMULATIONRENTABILIT1.Enabled:=False; //confidentialité , concept acces simulation
		{$ENDIF}
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AttribueEcart(TobPiece,TOBL,TobNomen, ArtEcart, ARRONDI(PuSaisi - PuFix,V_PGI.OkDecP), DEV, ENHt, EnPA);
        end;
        PuFix := PuSaisi;
      end;
    end;
...
//		requete := 'SELECT GL_QTERESTE,GL_QUALIFQTEACH,GL_PUHT,GL_COEFCONVQTE FROM LIGNE WHERE' + WherePiece (cledoc,ttdligne,true,true)+ ' AND GL_QTERESTE >0' ;
		requete := 'SELECT GL_QTEFACT,GL_QTERESTE,GL_QUALIFQTEACH,GL_PUHT,GL_COEFCONVQTE FROM LIGNE WHERE' + WherePiece (cledoc,ttdligne,true,true); // + ' AND GL_QTERESTE >0' ;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TOBL.putValue('BLO_COEFFG',Arrondi((TOBL.getValue('BLO_DPR')/TOBL.getvalue('BLO_DPA'))-1,4));
      end;
...
function PieceUniteautorisee (Nature : string) : boolean;
begin
  result := (Pos(Nature,'ETU;DBT;FBT;ABT;ABP;AFF;BCE;FPR;FAC;DEF;CF;BLF;FF;CFR;LFR;')>0);
end;
...
begin
  ...
  if OkVisa then
  begin
    if EtatVisa = 'ATT' then
      Result := False
    else if MontantPiece >= MontantVisa then
      Result := False;
  end;
