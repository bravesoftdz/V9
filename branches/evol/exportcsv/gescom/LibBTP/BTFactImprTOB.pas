...
//    function GetLastNumOrdre : integer;
    ...
    procedure DefiniLignePortFraisDet(ThePiece, TOBFraisDet: TOB;TypePort: string);
    procedure DefiniLigneTotalHtDet(ThePiece, TOBFDetaille: TOB);
    procedure DefiniLignesTVADet(ThePiece, TOBTVAPre, TOBFDetaille: TOB);
    procedure DefiniLigneTOTALTTCInterm(ThePiece,TOBFDetaille: TOB; CumSitTVA,
      CumPreTVA, CumRGSit, CumRgPre, TVARGSIt, TVARGPre: double; var TTCSIt, TTCPRE: double);
    procedure DefiniLignePIECEREPART(ThePiece: TOB; EscompteCum,RemiseCum,PortCum,CumPortSit : double);
  public
...
uses UtilMetres,FactVariante,FactAdresse,FactNomen,FactTob,OptimizeOuv,FactComm,FactOuvrage,Affaireutil,
      ...
      FactTimbres,FactRG,UtilReglementAffaire;
{$IFDEF BTP}
...
begin
  ...
  begin
  	TypePresent := TOBParent.GetValue('BPD_TYPEPRESINIT');
  end;
...
begin
  ...
  begin
    ...
    if (TOBI.GetString('BPE_TYPEINTERV') = 'Y00') and (TOBI.GetString('TYPEPAIE')='001')then
    begin
      TOBMERE.AddChampSupValeur ('SSTRAITANCE','X');
      if I < 5 then
      begin
        inc(I);
        TOBMEre.PutValue ('SST_FOURNISSEUR'+inttoStr(I),TOBI.GetString('LIBELLE'));
      ...
      begin
        TotHTDev   := TotHTDEV  + TOBI.GetDouble('BPE_TOTALHTDEV');
        TotTTCDev  := TotTTCDEV + TOBI.GetDouble('BPE_TOTALTTCDEV');
        TotMtPA    := TotMTPA   + TOBI.GetDouble('BPE_MONTANTPA');
        TotMtFac   := TotMtFac  + TOBI.GetDouble('BPE_MONTANTFAC');
        TotMtREgl  := TotMTRegl + TOBI.GetDouble('BPE_MONTANTREGL');
        if I = 5 then
          TOBMEre.PutValue ('SST_FOURNISSEUR'+inttoStr(Num),TOBI.GetString('LIBELLE'))
        else
          TOBMEre.PutValue ('SST_FOURNISSEUR'+inttoStr(5),'Sous-Traitant Divers');
        TOBMEre.PutValue ('SST_TOTALHTDEV' +inttoStr(5),TotHTDev);
        TOBMEre.PutValue ('SST_TOTALTTCDEV'+inttoStr(5),TotTTCDev);
        TOBMEre.PutValue ('SST_MONTANTPA'  +inttoStr(5),TotMtPA);
        TOBMEre.PutValue ('SST_MONTANTFAC' +inttoStr(5),TotMtFac);
        TOBMEre.PutValue ('SST_MONTANTREGL'+inttoStr(5),TotMtRegl);
        TotSsTrait  := TotSsTrait + TOBI.GetDouble('BPE_MONTANTREGL');
      end;
    end;
  end;
  ...
  //
  (*
  Diff := Arrondi (Totfacture - TotSsTrait,V_PGI.OkDecV);
  if (Diff <> 0) and (IndEnt <> -1 ) then
  begin
    TOBMEre.PutValue ('SST_MONTANTREGL'+inttoStr(IndEnt),TOBMEre.GetValue ('SST_MONTANTREGL'+inttoStr(IndEnt))+Diff);
  end;
  *)
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TotMtREgl  := TotMTRegl + TOBI.GetDouble('BPE_MONTANTREGL');
        if I = 5 then
          TOBMEre.PutValue ('SST_FOURNISSEUR'+inttoStr(Num),TOBI.GetString('LIBTIERS'))
        else
          TOBMEre.PutValue ('SST_FOURNISSEUR'+inttoStr(5),'Sous-Traitant Divers');
        TOBMEre.PutValue ('SST_TOTALHTDEV' +inttoStr(5),TotHTDev);
        TOBMEre.PutValue ('SST_TOTALTTCDEV'+inttoStr(5),TotTTCDev);
...
begin
  ...
  //
  TOBMEre.AddChampSupValeur ('BPD_TYPEPRESINIT',LATOBOrig.GetValue('BPD_TYPEPRES'));
  //
end;
...
//
  ...
  begin
    ...
    for Indice := 1 to high(V_PGI.DeChamps[Itable]) do
    begin
      NewChamp := 'BLF'+Copy(V_PGI.Dechamps[iTable, Indice].Nom,4,255);
      if TOBMere.FieldExists (newChamp) then
      begin
        TOBMere.PutValue (NewChamp,TOBAInsere.GetValue(V_PGI.Dechamps[iTable, Indice].Nom));
      end;
...
begin
  ...
  req := 'SELECT '+
         ...
         'S.BST_NUMEROFAC='+ThePiece.GetString('GP_NUMERO')+') '+
  ...
  begin
    ...
    begin
      ...
      TOBL.AddChampSupValeur('MONTANTHTPRE',QQ.fields[3].asfloat);
      TOBL.AddChampSupValeur('MONTANTTTCPRE',QQ.fields[4].asfloat);
      TOBL.AddChampSupValeur('TVAPRE',TOBL.GetDouble('MONTANTTTCPRE')-TOBL.GetDouble('MONTANTHTPRE'));
  ...
  *)
  ...
  if Repartition then
    if ThePiece.GetValue('BPD_IMPDETAVE')='X' then
      DefiniLignePieceRepart(ThePiece,EscompteCum,RemiseCum,PortCum,CumPortSit)
    else
  ...
  DefiniLignePortFraisDet (ThePiece,TOBFDetaille,'HT');

  if (EscompteCum<>0) or (RemiseCum<>0) or (nbrHt<>0) then
  begin
  ...
  DefiniLignesTVADet  (ThePiece,TOBTVAPre,TOBFDetaille);
  ...
  DefiniLigneTOTALTTCInterm (ThePiece,TOBFDetaille,CumSitTVA,CumPreTVA,CumRGSit,CumRgPre,TVARGSIt,TVARGPre,TTCSit,TTCPre);
  DefiniLignePortFraisDet (ThePiece,TOBFDetaille,'TTC');
  DefiniLigneTOTALTTC (ThePiece,TOBfDetaille,CumSitTVA,CumPreTVA,CumRGSit,CumRgPre,TVARGSIt,TVARGPre,TTCSit,TTCPre);
  ...
  if (Arrondi(Abs(AcomptesSit),2)<>0) or (Arrondi(Abs(AcomptesPre),2)<>0) or
     ...
     (Timbres <> 0) then
  BEGIN
    DefiniLigneTOTALNET (ThePiece,TTCSit,TTCPre,ReliquatRGSit,reliquatRGPre,AcomptesSit,AcomptesPre,Timbres);
  END;
  TOBTVAPre.free;
  TOBRGPre.free;
  TOBFDetaille.free;
...
procedure TImprPieceViaTOB.DefiniLigneTOTALTTCInterm (ThePiece,TOBFDetaille : TOB; CumSitTVA,CumPreTVA,CumRGSit,CumRgPre,TVARGSIt,TVARGPre : double; var TTCSIt,TTCPRE : double);
  ...
  begin
    ...
    begin
      ...
      MTPRE := MTPRE + TOBF.GetDouble('MONTANTTTCPRE');
    end;
...
begin
  ...
  begin
    TOBL.putValue('MONTANTSIT',ThePiece.getValue('GP_TOTALHTDEV')+CumSitTVA+CumRGSit-TVARGSit-CUMSIT);
    TOBL.putValue('MONTANTPRE',ThePiece.getValue('TOTSITPREC')+CumPreTVA+CumRGPre-TVARGPre-CUMPRE);
  end else
  begin
    TOBL.putValue('MONTANTSIT',ThePiece.getValue('GP_TOTALHTDEV')+CumSitTVA-CUMSIT);
    TOBL.putValue('MONTANTPRE',ThePiece.getValue('TOTSITPREC')+CumPreTVA-CUMPRE);
  end;
  TOBL.putValue('ISLIGNEPOR','X');
end;

procedure TImprPieceViaTOB.DefiniLigneTOTALTTC (ThePiece,TOBfDetaille : TOB; CumSitTVA,CumPreTVA,CumRGSit,CumRgPre,TVARGSIt,TVARGPre : double; var TTCSIt,TTCPRE : double);
var TOBL : TOB;
begin
  TOBL := InsereTOB ('LIGNE',ThePiece);
...
begin
  ...
  if TVARGSIt+TVARGPre = 0 then TOBL.putValue('LIBELLECUM','CAUTION');
...
begin
  ...
  begin
    Req := 'SELECT BST_NATUREPIECE,BST_SOUCHE,BST_NUMEROFAC,BST_NUMEROSIT,PRG_NUMLIGNE,PRG_FOURN,'+
           ...
           'BST_VIVANTE="X" AND '+
           'BST_NUMEROSIT <= '+IntToStr(GetNumSituation(fTOBPiece))+' ORDER BY GL_PIECEORIGINE,PRG_FOURN,BST_NUMEROSIT';
  end else
  begin
    Req := 'SELECT BST_NATUREPIECE,BST_SOUCHE,BST_NUMEROFAC,BST_NUMEROSIT,PRG_NUMLIGNE,PRG_FOURN,'+
           'PRG_MTHTRG,PRG_MTHTRGDEV,PRG_MTTTCRGDEV,PRG_MTTTCRG,PRG_NUMCAUTION,PRG_CAUTIONMT,PRG_CAUTIONMTDEV,'+
           ...
           'WHERE BST_SSAFFAIRE="'+ThePiece.GetValue('GP_AFFAIREDEVIS')+'" AND '+
           'BST_VIVANTE="X" AND '+
           'BST_NUMEROFAC < '+IntToStr(ThePiece.GetValue('GP_NUMERO')) +' '+
  ...
  begin
    ...
    TOBP := TOBRGPre.findFirst(['PIECEPRECEDENTE','PRG_FOURN'],
    					[TOBL.GetValue('GL_PIECEORIGINE'),TOBL.GetValue('PRG_FOURN')],true);
    if TOBP = nil then
    begin
...
//           '" AND BST_SOUCHE="'+ThePiece.GetValue('GP_SOUCHE')+
  ...
  begin
    ...
    TVAC.putValue('BASETAXEPRE',TVAC.GetValue('BASETAXEPRE')+TVAS.getValue('GPB_BASEDEV'));
...
procedure TImprPieceViaTOB.DefiniLignesTVADet (ThePiece,TOBTVAPre,TOBFDetaille : TOB);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (fIsDGD) then
      begin
        LibTva := LibTva + ' - Base : '+StrF00(TOBT.getValue('BASETAXEPRE')-Basepre, V_PGI.OkDecV)+ ' '+Symbole ;
      end else
      begin
        if (TOBTVAPRE.detail.count > 1) and (TOBT.getValue('BASETAXE')>0) then
        begin
            LibTva := LibTva + ' - Base Sit. : '+StrF00(TOBT.getValue('BASETAXE')-Base, V_PGI.OkDecV)+ ' '+Symbole ;
        end;
      end;
      ReliqTvaSit := Arrondi(TOBT.GetValue('VALEURTAXESIT') - MTTVA,2);
      ReliqTvaPre := ARRONDI(TOBT.GetValue('VALEURTAXEPRE') - MTTVAPRE,2);
      if (reliqTvaSit<>0) or (reliqTvapre<>0) then
      begin
        TOBL.putValue('LIBELLE',libTva);
        TOBL.putValue('MONTANTSIT',TOBT.GetValue('VALEURTAXESIT') - MTTVA);
        TOBL.putValue('MONTANTPRE',TOBT.GetValue('VALEURTAXEPRE') - MTTVAPRE);
        TOBL.putValue('ISLIGNEPOR','X');
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (fIsDGD) then
      begin
        LibTva := LibTva + ' - Base : '+StrF00(TOBT.getValue('BASETAXEPRE'), V_PGI.OkDecV)+ ' '+Symbole ;
      end else
      begin
        if (TOBTVAPRE.detail.count > 1) and (TOBT.getValue('BASETAXE')>0) then
        begin
            LibTva := LibTva + ' - Base Sit. : '+StrF00(TOBT.getValue('BASETAXE'), V_PGI.OkDecV)+ ' '+Symbole ;
        end;
      end;
...
procedure TImprPieceViaTOB.DefiniLignePIECEREPART (ThePiece : Tob; EscompteCum,RemiseCum,PortCum,CumPortSit : double);
Var Nature  : String;
    Souche  : String;
    Numero  : Integer;
    IndiceG : Integer;
...
begin
  ...
  begin
    ...
    begin
      TOBL := InsereTOB ('LIGNE',ThePiece);
      //
      TOBL.putValue('LIBELLE','MARCHE DE BASE');
...
procedure TImprPieceViaTOB.DefiniLignePortFraisDet(ThePiece,TOBFraisDet :TOB; TypePort : string );
var TOBL,TOBF : TOB;
    II : Integer;
begin
  ...
  begin
    ...
    if TypePort = 'HT' then
    begin
      TOBL.putValue('MONTANTSIT',TOBF.GetDouble('MONTANTHTSIT'));
      TOBL.putValue('MONTANTPRE',TOBF.GetDouble('MONTANTHTPRE'));
    end else
    begin
      TOBL.putValue('MONTANTSIT',TOBF.GetDouble('MONTANTTTCSIT'));
      TOBL.putValue('MONTANTPRE',TOBF.GetDouble('MONTANTTTCPRE'));
    end;
    TOBL.putValue('ISLIGNEPOR','X');
    (*
