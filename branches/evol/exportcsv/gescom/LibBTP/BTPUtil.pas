...
if CledocAffaire.Naturepiece = 'DAP' then Codessaff := CodeAffaire
    ...
    begin
      ...
      Req := 'UPDATE AFFAIRE SET AFF_OKSIZERO="'+TOBPiece.getValue('AFF_OKSIZERO')+'",AFF_GENERAUTO ='+'"'+ TOBPiece.getValue('AFF_GENERAUTO') +'",AFF_AFFAIREREF ='+'"'+ CodeAffaire +'"'+',AFF_AFFAIREINIT ='+'"'+ CodeAffaire +'"'+',AFF_ACOMPTE='+StringReplace(FloatToStr(acompte),',','.',[rfReplaceAll])+' WHERE AFF_AFFAIRE ='+'"'+Codessaff+'"';
      ExecuteSQL(Req);
    end;
...
begin
  ...
  TableauSituation:='-';
  if (Pos(CleDoc.NaturePiece ,'FBT;DAC;FBP;BAC;')>0) and
  ...
  Begin
    Repeat
      RelancerEtat:=False;
    	if Modele <> '' then StartPdfBatch; // inutile si pas de modèle et impression du tableau seul
      ...
      Begin
        ...
        begin
          ...
          UneTOB := ImpressionViaTOB.TOBRECAP;
    			if Modele <> '' then LastPdfBatch ; // inutile si pas de modèle et impression du tableau seul
          if LanceEtatTOB ('E','GPJ',ModeleR,UneTOB,Bapercu,false,false,nil,'','',false) < 0 then V_PGI.IoError:=oeUnknown ;
        end;
      end
      else
      Begin
        if Modele <> '' then CancelPDFBatch ;  // inutile si pas de modèle et impression du tableau seul
      end;
      {$IFNDEF EAGLCLIENT}
      // RH le 14/10/2003 ne passe pas à la compile WebAcces
      if Modele <> '' then PreviewPDFFile('',GetMultiPDFPath,True); // inutile si pas de modèle et impression du tableau seul
      {$ENDIF}
