...
begin
  ...
  begin
    ...
    if OkLib then
    begin
      if (Ecran.Name = 'BTMATERIEL') then
      begin
        If (Nam = 'BEM_BTETAT')                       then fGS.Colformats[NoCol] := 'CB=BTTYPEACTIONMAT'
        else if Copy(Nam,5,Length(Nam))='RESSOURCE'   then fGS.Colformats[NoCol] := 'CB=BTRESSOURCE'
        else if Copy(Nam,5,Length(Nam))='CODEFAMILLE' then fGS.Colformats[NoCol] := 'CB=BTFAMILLEMAT'
        else
        begin
          TypeFormat := ChargementCombo(Copy(Nam,5,Length(Nam)));
          if TypeFormat <> ''  then fGS.Colformats[NoCol] := 'CB=' + TypeFormat;
        end;
      end
      Else
      begin
