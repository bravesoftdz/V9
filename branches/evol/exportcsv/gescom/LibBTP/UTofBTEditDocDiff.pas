...
{$IFDEF NOMADE}
  if VenteAchat = 'VEN' then NaturePieceG.Plus := NaturePieceG.Plus + GetNaturePOP('GPP_NATUREPIECEG');
{$ENDIF} // NOMADE
...
begin
  ...
  begin
    //
    Avancement := pos(RenvoieTypeFact (TobPiece.detail[iind].GetValue('GP_AFFAIREDEVIS')), 'AVA;DAC')>0;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          OkEtatAvancDirect := TOBPiece.detail[IInd].GetBoolean('OKETAT');
        end else
        begin
          SetInfoPieces (TOBPiece.detail[iInd],
                        ChoixAvancDirect + ModeleAvancDirect,OkEtatAvancDirect,
                        NbExemplaires);
        end;
      end
      else
      begin
        ...
        begin
          ...
          OKEtatAvancNonDirect := TOBPiece.detail[IInd].GetBoolean('OKETAT');
        end else
        begin
          SetInfoPieces (TOBPiece.detail[iInd],
                        ChoixAvancNonDirect + ModeleAvancNonDirect,OKEtatAvancNonDirect,
                        NbExemplaires);
        end;
      end;
    end
  ...
  TRY
    ...
    begin
      ...
      if (Pos(TobPiece.detail[iind].GetValue('GP_NATUREPIECEG'),'FBT;DAC;FBP;BAC')>0) and (pos(RenvoieTypeFact (TobPiece.detail[iind].GetValue('GP_AFFAIREDEVIS')), 'AVA;DAC')>0) then
      begin
        recapSituations := GetInfoRecapFromModele(TobPiece.detail[iind]);
      end;
      if (Pos(TOBPiece.detail[iInd].getValue('GP_NATUREPIECEG'),'FBT;FBP')>0) then
      begin
        ...
        BEGIN
          ...
          begin
            ...
            begin
              ...
              if LanceEtatTOB ('E','GPJ',ModeleR,UneTOB,TRUE,false,false,nil,'','',Bduplicata) < 0 then
                V_PGI.IoError:=oeUnknown ;
            end;
          end;
          // ---
