...
procedure AttribueEcart (TOBPiece,TOBL,TobOuvrage : TOB;ArtEcart:string;Montantecart: double;DEV:RDEVISE;EnHt:Boolean;EnPA : boolean=false;OnlyPa:boolean=false);
...
{$IFDEF BTP}
,UMetreArticle,UtilMetres,PiecesRecalculs,BTStructChampSup,Ucotraitance
{$ENDIF}
;
...
//Récupration du fichier XLS dans le cas d'un Article
procedure DocMetreBiblioEntete(TOBL: TOB; CodeArt,TypeArt:String);
Var FicArt: String;
    MetreArticle: TMetreArt;
begin
...
var IndOuv,IndiceNomen,IndiceLig,TypePresent,NiveauImbric : integer;
    ...
    QteDuDetail,QteDUPv,Montant,MontantAchat,CumMontant,MontantOuv,MontantLig,MontantPaOuv,MontantLPa,QteFact : double;
...
begin
  ...
  begin
    ...
    begin
      ...
      (*
      if (WithAvanc) and (TOBOUV.FieldExists ('BLF_QTESITUATION')) then
      begin
        QteFact := TOBOUV.GetValue('BLF_QTESITUATION');
      end else
      begin
      *)
...
//        MontantAchat := arrondi(TOBOUV.GetValue('BLO_DPA')* TOBL.getvalue('GL_QTEFACT') * (TOBOUV.GetValue('BLO_QTEFACT')/ (QteDuPv * QteDuDetail)),V_PGI.OkdecP);
        ...
        begin
          ...
          if (WithAvanc) and (TOBOUV.FieldExists ('BLF_QTESITUATION')) then
          begin
          	Montant := TOBOUV.GetValue('BLF_MTSITUATION');
          end else
          begin
          *)
      ...
      begin
        ...
        begin
          (*
					if (WithAvanc) and (TOBOUV.FieldExists ('BLF_QTESITUATION')) then
          begin
	          TOBL1.PutValue('GL_QTEFACT',TOBOUV.GetValue('BLF_QTESITUATION'));
          end else
          begin
...
var indice : integer;
    ...
    VoirDetail : boolean;
    TypePresent : integer;
    ModifSousDetail,ModifLeSousDetail,AffSousDetail : boolean;
begin
  ModifSousDetail :=  (GetParamSocSecur('SO_BTMODIFSDETAIL',false));
  ...
  //  TOBPiece := TOBL.Parent;
    ...
    begin
      ...
      begin
        ...
        begin
          TOBIns.putValue('BLF_NATUREPIECEG',TOBLoc.parent.getvalue('BLF_NATUREPIECEG'));
          if EnHt then
          	TOBIns.Putvalue('BLF_MTSITUATION',TOBINS.getvalue(prefixe+'_MONTANTHTDEV') )
          else TOBIns.Putvalue('BLF_MTSITUATION',TOBINS.getvalue(prefixe+'_MONTANTTTCDEV') );
        end;
      end;
    end;
  FINALLY
