...
{$ENDIF}
  ...
  hmsgbox,M3FP, TntStdCtrls, TntComCtrls, TntGrids;
...
implementation
uses FactBordereau,NomenUtil,FactureBTP,UtilPgi;
{$R *.DFM}
...
BEGIN
  ...
  if not (JaiLeDroitConcept(TConcept(bt517),False)) then
  begin
...
//
  ...
  //FV1 : 30/10/2013 -FS#731 - POUCHAIN : simulation de rentabilité, ajout autorisation d'accès au coef de FG.
  if (JaiLeDroitConcept(TConcept(bt517),False)) then
    Enabled(COEFFG,True)
  else
    Enabled(COEFFG,False);
  //
