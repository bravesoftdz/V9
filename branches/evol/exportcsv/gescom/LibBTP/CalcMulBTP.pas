...
begin
  ...
  if (pos(typearticle, 'ARP')>0) then
  begin
    ValoriseOuvrage (CodeArt,nil,nil,valeurs);
...
begin
  ...
  begin
    ...
    Result := GetPuOuvHT(TT.findfield('GA_ARTICLE').AsString,TT.findfield('GA_TYPEARTICLE').AsString,TT.findfield('GA_PVHT').Asfloat);
  end
