...
type
    TGenFacAvanc = class (Tobject)
      private
        ...
        //
        TOBDevis,TOBFacture,TOBOptDevis,TOBTablette,TOBPieceTrait,TOBPieceTraitDevis,TOBAffaireInterv,TOBSousTrait : TOB;
...
function GenereFactureFromAvanc (TOBSOUSTraits,TOBSource,TOBDetailDevis,TOBouvrages,TobAcomptes_O,TOBAcomptes : TOB; ClotureFacturation : boolean; var Erreur : string; DateFac : TdateTime; MontantMarche,DejaFacture,MtHorsDevis : Double; lastFacture : boolean=false; Cotraitance : boolean=false; ModifSituation : Boolean=false) : boolean;

implementation

uses factvariante,FactOuvrage,Ucotraitance,FactTimbres, Math;
...
begin
  ...
  begin
    if GetParamSocSecur('SO_BTSITPROVISOIRE',false) then NewNature := 'FBP' else NewNature := 'FBT'
  end else
  begin
...
begin
  ...
  TRY
    ...
    begin
      ...
      (* ---------------------------------------------- *)
      if Result then  Result := ValidelaPiece; if not result then exit else AjouteEvent ;
    end;
...
//  result := false;
  ...
  CalculFacture(nil, TOBFacture,TOBPieceTrait,TOBSousTrait,TOBOuvrage,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV,false,tamodif,false,-1,True) ;
  ...
  //
  CalculeReglementsIntervenants(TOBSousTrait, TOBFacture,TOBPieceRG,TOBAcomptes, TOBFacture.getValue('GP_AFFAIRE'),TOBPieceTrait,DEV,fLastfacture);
  {Echéances}
  TOBEches.ClearDetail;
  GereEcheancesGC(TOBFacture,TOBTiers,TOBEches,TOBAcomptes,TOBPieceRG,TOBPieceTrait,taCreat,DEV,False);
  result := true;
end;
...
BEGIN
  ...
  if Not PassationComptable(TOBFacture,TOBOuvrage ,TOBOuvragesP,TOBBases,TOBBasesL,TOBEches,TOBPieceTrait,TOBAffaireInterv,TOBTiers,TOBArticles,TOBCpta,TOBAcomptes,TOBPorcs,TOBPIECERG,TOBBasesRG,TOBAnaP,TOBAnaS,TOBSousTrait,DEV,OldEcr,OldStk,false)
