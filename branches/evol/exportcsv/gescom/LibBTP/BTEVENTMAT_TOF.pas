...
begin
  ...
  begin
    ...
    //NumEvent := StrToInt(NumEventMat.caption);
    ...
    begin
      ...
      LaTob.PutValue('CODEETAT',        CodeEtat.Value);
      LaTob.PutValue('DATEDEB',         DateDeb.text);
      LaTob.PutValue('DATEFIN',         DateFin.text);
      LaTob.PutValue('DUREE',           NbHeure.text);
      LaTob.PutValue('AFFAIRE',         Affaire.text);
      LaTob.PutValue('TIERS',           CodeClient.text);
...
begin
  ...
  //if StrToDate(DateDeb.text) = StrToDate(DateFin.text) then exit;

  if StrToDate(Datedeb.text) = OldDdeb then exit;

  Nbheure.text := CalculNbHeure(StrToDateTime(DateDeb.text), StrToDateTime(DateFin.text));
...
begin
  ...
  if not ControleCoherence(StrToDate(DateDeb.text),StrToDate(DateFin.text),StrToInt(Nbheure.Text), CodeMateriel.Text) then exit;

  if StrToDate(DateFin.text) = idate2099 then exit;

  if StrToDate(DateFin.text) = OldDFin   then exit;

  if OldDFin <> idate2099 then
  begin
    if StrToDate(DateDeb.text) = StrToDate(DateFin.text) then
    begin
      Nbheure.text := CalculNbHeure(StrToDateTime(DateDeb.text), StrToDateTime(DateFin.text));
...
begin
  ...
  Datefin.text := Calculdatefin(NbHJour, StrToDateTime(DateDeb.text), StrToFloat(Nbheure.text));
...
begin
  ...
  DateDeb.Text        := TobEventMat.GetString('BEM_DATEDEB');
  DateFin.Text        := TobEventMat.GetString('BEM_DATEFIN');
  //
  OldDDeb := StrToDate(DateDeb.Text);
  OldDFin := StrtoDate(DateFin.Text);
  //
...
begin
  ...
  TobEventMat.PutValue('BEM_DATEDEB',       DateDeb.Text);
  TobEventMat.PutValue('BEM_DATEFIN',       DateFin.Text);
  //
...
begin
  ...
  //
  CodeMateriel.Text := LaTob.GetValue('MATERIEL');
  CodeEtat.Value    := LaTob.GetValue('CODEETAT');
  DateDeb.Text      := LaTob.GetValue('DATEDEB');
  DateFin.Text      := LaTob.GetValue('DATEFIN');
  
  OldDDeb           := StrToDate(DateDeb.Text);
  OldDFin           := StrtoDate(Datefin.Text);

  NbHeure.Text      := LaTob.GetValue('DUREE');
  //
