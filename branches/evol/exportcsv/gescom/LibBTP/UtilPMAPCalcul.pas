...
function CalculsTableTempo(fNomTable : string; Depot, Article : String; var PmapUS,QTEStockUS : double) : boolean;
...
function ConstituetableTemp(fNomTable : string; Article, Depot: String; Date1, Date2: TDateTime) : boolean;
  ...
  var QQ : TQuery;
  begin
    QQ := OpenSql ('SELECT * FROM '+fNomTable+' WHERE NATUREPIECEG="LBT"',false);
    if not QQ.eof then
    begin
      QQ.First;
      While not QQ.eof do
...
begin
  ...
  TRY
    ...
    begin
      ...
      if DPA <> 0 then CoefPU  := (PUHT/DPR) else CoefPU := 0;
      // Calcul de la ligne
