...
uses
  ...
  Grids, StdCtrls, ExtCtrls, Buttons, UTob,
{$IFNDEF EAGLCLIENT}
     DB,
     {$IFNDEF DBXPRESS} dbTables, {$ELSE} uDbxDataSet, {$ENDIF}
{$ENDIF}
  UiUtil, Hctrls,
  ComCtrls, IniFiles, AssistUtilBtp,SaisieAssTablette,HMsgBox,Hent1,UtilImport;

type
  TFAssocImportBtp = class(TForm)
    Edit1: TEdit;
    StringGridFichier: THgrid;
    StringGridTable: THgrid;
    StringGridAsso: THgrid;
    StringGrid1: THgrid;
    Edit2: TEdit;
    OpenDialog1: TOpenDialog;
    StringGridAssoStruct: THgrid;
    Panel1: TPanel;
    ListeTable: TListBox;
    ListeAsso: TListBox;
    ...
    procedure ListeAssoChMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure BtAssocTablClick(Sender: TObject);
  private
...
var
  FAssocImportBtp: TFAssocImportBtp;


implementation

uses SaisieTexte, SaisieFormule, SaisieConditionBtp {, AssistImport};
...
procedure TFAssocImportBtp.Charge_Dictionnaire;
var
    Tob1, Tob2 : TOB;
    Query1 : TQuery;
    Fichier: textfile;
...
//  Ajout des parametres saisis à la liste du fichier .seq
    ...
    for i_ind2 := 0 to ParamGenCle.Count - 1 do
        ...
        StringGridFichier.Cells[2, i_ind1] := FicIni.ReadString('General', ParamGenCle.Strings[i_ind2], '');
        i_ind1 := Succ(i_ind1);
        end;
    FicIni.Free;
...
//----------------------------------------------------------------------------------
function TFAssocImportBtp.VerifCorrespondance (i_nb_champs : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.Charge_Section(FicIni : TIniFile; st_racine : string;
                                       i_col : integer; var i_ligne : integer);
var
    st_enreg, st_trav1, st_trav2, st_file, st_maj : string;
    i_ind1, i_ind2, i_ind3, i_ind4, i_indtext, i_nrow : integer;
...
//  Ajout des parametres saisis à la liste du fichier .seq
    ...
    FicParam.ReadSection('General', ParamGenCle);
    for i_ind2 := 0 to ParamGenCle.Count - 1 do
        begin
        ...
        StringGridFichier.Cells[2, i_ind1] := FicParam.ReadString('General', ParamGenCle.Strings[i_ind2], '');
        i_ind1 := Succ(i_ind1);
        end;
    FicParam.Free;
            ...
            begin
                ...
                begin
                    if ChampTablette(StringGridFichier.Cells[2, i_nrow]) <> ChampTablette (st_trav2) then
                       ...
                       StringGridAssoStruct.Cells[i_ind2, i_ligne]:= ValRet;
                       end else StringGridAssoStruct.Cells[i_ind2, i_ligne] := StringGridFichier.Cells[2, i_nrow];
                end;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn1Click(Sender: TObject);
var
    i_ind1 : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn3Click(Sender: TObject);
var
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn2Click(Sender: TObject);
var
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn4Click(Sender: TObject);
var
    i_ind1, i_ind2, i_ind3 : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn5Click(Sender: TObject);
var
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.BitBtn6Click(Sender: TObject);
var
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.ListeTableDblClick(Sender: TObject);
begin
    BitBtn1Click(Sender);
end;

//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.ListeDictFichier;
var
    i_ind1 : integer;
    st_enreg : string;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.ListeDetailAsso;
var
    i_ind1, i_ind2 : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.RadioGroup1Click(Sender: TObject);
var
    i_ind1, i_ind2 : integer;
    bTrouve : boolean;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.ListeFichierDblClick(Sender: TObject);
begin
    BitBtn3Click(Sender);
end;

//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.StringGrid1DblClick(Sender: TObject);
var
    i_ind1 : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.RadioGroup2Click(Sender: TObject);
begin
    case RadioGroup2.ItemIndex of
    0 :
        begin
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.Enregistre_Inifile(i_col,i_row : integer);
var
    i_ind1, i_ind2, i_ind3 : integer;
...
//----------------------------------------------------------------------------------
procedure TFAssocImportBtp.TabControl1Change(Sender: TObject);
var
    i_ind1 : Integer;
...
procedure TFAssocImportBtp.RadioGroup3Click(Sender: TObject);
var
    i_ind1 : integer;
...
procedure TFAssocImportBtp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    Enregistre_Inifile(i_TabIndexPrec + 1, StringGrid1.Row);
end;

procedure TFAssocImportBtp.FormCreate(Sender: TObject);
begin
    FSaisieTexte := TFSaisieTexte.Create(Application);
    FSaisieFormule := TFSaisieFormule.Create(Application);
    Edit3.Text := '';
    BtAssocTabl.Enabled := false;
end;

procedure TFAssocImportBtp.ListeAssoClick(Sender: TObject);
begin
    Maj_Edit3;
end;

procedure TFAssocImportBtp.ListeAssoChClick(Sender: TObject);
begin
    Maj_Edit3;
end;

procedure TFAssocImportBtp.ListeAssoStructClick(Sender: TObject);
begin
    Maj_Edit3;
end;

procedure TFAssocImportBtp.Maj_Edit3;
var assoc : string;
    assocOk : boolean;
begin
...
if (copy (Edit3.text,1,8)='Tablette') then
   begin
   if (pos ('=',copy(Edit3.text,9,255)) = 0) Then AssocOk := false
                                            else AssocOk := true;
   if (AssocOk) then BtAssocTabl.Enabled := true
                else BtAssocTabl.Enabled := false;
   end else BtAssocTabl.Enabled := false;
end;

procedure TFAssocImportBtp.BitBtn7Click(Sender: TObject);
var
    i_ind1 : integer;
...
procedure TFAssocImportBtp.BitBtn8Click(Sender: TObject);
var
    i_ind1 : integer;
...
procedure TFAssocImportBtp.BitBtn9Click(Sender: TObject);
begin
...
procedure TFAssocImportBtp.ListeTableEnter(Sender: TObject);
begin
{    if Sender is TListBox then
        st_ListeTable_Rech := '';  }
end;

procedure TFAssocImportBtp.ListeTableEntree(Sender: TObject; var Key: Word);
begin
...
procedure TFAssocImportBtp.ListeFichierKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
    i_ind1 : integer;
...
procedure TFAssocImportBtp.RadioGroup4Click(Sender: TObject);
var
    i_ind1 : integer;
...
procedure TFAssocImportBtp.BitBtn10Click(Sender: TObject);
var
...
procedure TFAssocImportBtp.BitBtn11Click(Sender: TObject);
var
    i_ind1, i_ItemIndexSave : integer;
    st1, st2 : string;
...
//
    ...
    st2 := Entree_ConditionBtp([Table, Fichier, TableCode, FichierCode, st1], 5);
    if st2 <> '' then
    begin
//  Recupere le nom du champ dans la ligne centrale
        i_ind1 := ListeAsso.ItemIndex;
...
procedure TFAssocImportBtp.ListeAssoChMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var  APoint : Tpoint;
     Index : Integer;
     TLocal : TListBox;
begin
Apoint.X := X;
APoint.Y := Y;
if ListeAssoCh.Visible then TLocal := ListeAssoCh;
if ListeAsso.Visible then TLocal := ListeAsso;
if ListeAssoStruct.Visible then TLocal := ListeAssoStruct;

Tlocal.ShowHInt := false;


Index := Tlocal.ItemAtPos (Apoint,true);
if (Index >= 0) then
   begin
   Tlocal.hint := Tlocal.Items [Index];
   Tlocal.ShowHint := true;
   end else Tlocal.ShowHInt := false;
end;

procedure TFAssocImportBtp.BtAssocTablClick(Sender: TObject);
var F:TFSelAssociation;
    Champ,ChampIni,ChampTxt,ValRet : string;
    indice,IndiceTable : integer;
begin
F:=TFSelAssociation.Create (Self);
Champ := copy (ListeTable.Items.Strings[0],1,Pos('_',ListeTable.Items.Strings[0])-1);
indiceTable:=PrefixeToNum(Champ);

for indice := 1 to High (V_PGI.dechamps[IndiceTable]) do
    begin
    if V_PGI.Dechamps[IndiceTable,Indice].tipe = 'COMBO' then
        begin
        ChampIni := V_PGI.Dechamps[IndiceTable,Indice].Nom ;
        ChampTxt := V_PGI.Dechamps[IndiceTable,Indice].Libelle;
        Champ := copy (ChampIni,Pos('_',ChampIni)+1,255);
        if indice = 0 then
           begin
           F.ListeTable.Items.Text := Champ;
           F.ListeTableNam.Items.Text := ChampTxt;
           end else
           begin
           F.ListeTable.Items.Add(Champ);
           F.ListeTableNam.Items.Add(ChampTxt);
           end;
        end;
    end;
try
  F.ShowModal ;
   ...
   if F.ValRetour <> '' then
      begin
      ValRet := ListeAssoStruct.Items.Strings[ListeAsso.ItemIndex];
      AffecteTablette(ValRet,F.ValRetour);
      ListeAssoStruct.Items.Strings[ListeAsso.ItemIndex] := ValRet;
      end;
   F.Free ;
end ;
end;
