...
Type
  ...
  Private
    ...
    ColBudgetMT,ColBudgetMTPA : Integer;
    ColEngage    : Integer;
    ColTheoQte   : Integer;
    ColTheoMT,ColTheoMTPA : Integer;
    //
    ColResteQTE       : Integer;
    ColResteMT        : Integer;
    ...
    procedure LoadTOBOuvrages(TOBLIGNES,TOBOUVRAGES: TOB);
...
begin
  ...
  if not DateArreteEncours then ThEdit(GetControl('DATEARRETEE')).enabled := false;
  //
  ColBudgetQte := -1;
  ...
  if not (JaiLeDroitConcept(TConcept(bt519),False)) then
...
begin
  ...
  begin
    ...
    if  (TOBL.GetDouble('RESTEMT') <> 0) or (TOBL.GetDouble('RESTEQTE') <> 0 ) then
    begin
...
Begin
  ...
  if ExisteSQL(StSQL) then Result := false;
...
begin
  ...
  begin
    ...
    else if nam = 'REALISEMTPA'       then ColRealiseMtPA  := Indice
    else if nam = 'REALISEDEPUISPA'   then ColDepuisMtPA   := Indice
    else if nam = 'REALISEDEPUIS'     then ColDepuisMt   := Indice
    else if nam = 'REALISEQTE'        then ColRealiseQte := Indice
    else if nam = 'REALISEQTEDEPUIS'  then ColDepuisQte  := Indice
    else if nam = 'BUDGETQTE'         then ColBudgetQte  := Indice
    else if nam = 'BUDGETMTPA'        then ColBudgetMTPA   := Indice
    else if nam = 'BUDGETMT'          then ColBudgetMT   := Indice
    else if nam = 'ENGAGE'            then ColEngage     := Indice
    else if nam = 'THEOQTE'           then ColTheoQte    := Indice
    else if nam = 'THEOMT'            then ColTheoMT     := Indice
...
begin
  ...
  begin
    ...
    begin
      ...
      fColNamesGS := 'SEL;LIBTYPEARTICLE;BUDGETQTE;BUDGETMTPA;REALISEQTE;REALISEMTPA;REALISEQTEDEPUIS;REALISEDEPUISPA;ENGAGE;THEOQTE;THEOMTPA;RESTEQTE;RESTEMT;FINAFFAIREPA';
      Falignement := 'C.0  ---;G.0  ---;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-';
      Ftitre := ' ;Libelle;Budget Qté;Budget Mt;Réalisé Qté;Réalisé Mt;Qté Depuis;Mt Depuis;Engagé;Théo. Qté;Théo. Mt;Reste Qté;Reste Mt;Fin d''affaire';
      fLargeur := '2;120;30;30;30;30;30;30;30;30;30;30;30;30';
    end else
    begin
      // Définition de la liste de saisie pour la grille Détail
      fColNamesGS := 'SEL;LIBTYPEARTICLE;BUDGETQTE;BUDGETMT;REALISEQTE;REALISEMT;REALISEQTEDEPUIS;REALISEDEPUIS;ENGAGE;THEOQTE;THEOMT;RESTEQTE;RESTEMT;FINAFFAIRE';
      Falignement := 'C.0  ---;G.0  ---;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-;D/2  -X-';
      Ftitre := ' ;Libelle;Budget Qté;Budget Mt;Réalisé Qté;Réalisé Mt;Qté Depuis;Mt Depuis;Engagé;Théo. Qté;Théo. Mt;Reste Qté;Reste Mt;Fin d''affaire';
      fLargeur := '2;120;30;30;30;30;30;30;30;30;30;30;30;30';
    end;
...
Procedure TOF_BTSAISIERESTDEP.LoadTOBOuvrages (TOBLIGNES,TOBOUVRAGES : TOB);
  ...
  begin
    // Initialisation
    Lig := 0; numero := 0; souche := ''; indiceg := 0;
    //IndiceOuv := 0;
    TOBNouv := nil;
    //
...
begin
  ...
  TRY
    StSql := 'SELECT BLO_NATUREPIECEG,BLO_SOUCHE,BLO_NUMERO,BLO_INDICEG,BLO_NUMLIGNE,BLO_N1,BLO_N2,BLO_N3,BLO_N4,BLO_N5,'+
             'BLO_TYPEARTICLE,BLO_CODEARTICLE,BLO_LIBELLE,BLO_DPA,BLO_DPR,BLO_AFFAIRE,BLO_QTEDUDETAIL,BLO_QTEFACT,'+
             '"TYPERESSOURCE" = CASE BLO_NATURETRAVAIL WHEN "002" THEN "ST" ELSE BNP_TYPERESSOURCE END, '+
             '"NATUREPRESTATION" = CASE GA_NATUREPRES WHEN " " THEN GA_TYPEARTICLE ELSE GA_NATUREPRES END,'+
             'GA_TYPEARTICLE     AS TYPEARTICLE '+
             'FROM LIGNEOUV '+
             ...
             'LEFT JOIN NATUREPREST ON BNP_NATUREPRES=GA_NATUREPRES '+
             'WHERE '+
             'BLO_NATUREPIECEG = "BCE" AND BLO_AFFAIRE="' + AFFAIRE.Text + '" '+
             'ORDER BY BLO_NATUREPIECEG,BLO_SOUCHE,BLO_NUMERO,BLO_INDICEG,BLO_NUMLIGNE,BLO_N1,BLO_N2,BLO_N3,BLO_N4,BLO_N5';
  ...
  TRY
    ...
    TRY
      ...
      begin
        ...
        TRY
          LoadTOBOuvrages (TOBLIGNESO,TOBOUVRAGES);
...
begin
  ...
  begin
    ...
    if PieceFrais <> '' then ChargementBudgetFrais(PieceFrais);
  end;
...
begin
  ...
  TRY
    ...
    StSQl := StSQl + 'GL_TYPELIGNE       AS TYPELIGNE,';
    StSQl := StSQl + 'GL_CODEARTICLE     AS CODEARTICLE,';
    StSQl := StSQl + 'GL_LIBELLE         AS LIBELLEART,';
    StSQl := StSQl + 'GL_QTEFACT         AS BUDGETQTE,';
    StSQl := StSQl + 'ROUND(GL_QTEFACT*GL_DPA,4)  AS BUDGETMTPA,';
    StSQl := StSQl + 'ROUND(GL_QTEFACT*GL_DPR,4)  AS BUDGETMT,';
    StSQl := StSQl + 'GL_AFFAIRE         AS CODEAFFAIRE, ';
    StSQl := StSQl + 'GL_DPR             AS REVIENT ';
    StSQl := StSQl + 'FROM LIGNE LEFT JOIN ARTICLE ON GA_ARTICLE=GL_ARTICLE ';
    StSQl := StSQl + 'LEFT JOIN NATUREPREST ON BNP_NATUREPRES=GA_NATUREPRES ';
    StSQl := StSQl + 'LEFT JOIN LIGNECOMPL  ON GLC_NATUREPIECEG=GL_NATUREPIECEG  AND ';
    StSQl := StSQl + 'GLC_SOUCHE=GL_SOUCHE AND GLC_NUMERO=GL_NUMERO AND GLC_INDICEG=GL_INDICEG AND GLC_NUMORDRE=GL_NUMORDRE ';
    StSQl := StSQl + 'WHERE GL_NATUREPIECEG = "' + Nature + '" AND GL_SOUCHE = "' + Souche + '" ';
    StSQl := StSQl + '  AND GL_NUMERO =' + IntToStr(Numero) + ' AND GL_INDICEG = ' + IntToStr(Indice);
    StSQl := StSQl + '  AND GL_TYPELIGNE="ART" AND ';
    StSQl := StSQl + '((NOT GL_TYPEARTICLE IN ("OUV","ARP")) OR (GL_INDICENOMEN=0)) AND ';
    StSQl := StSQl + 'GL_AFFAIRE="' + AFFAIRE.Text + '" ';
    ...
    End;
    //
    StSQl := 'SELECT GL_NATUREPIECEG,GL_SOUCHE,GL_NUMERO,GL_INDICEG,GL_NUMLIGNE,GL_NUMORDRE,GL_INDICENOMEN,GL_QTEFACT ';
    StSQl := StSQl + ' FROM LIGNE ';
    StSQl := StSQl + 'WHERE GL_NATUREPIECEG = "' + Nature + '" AND GL_SOUCHE = "' + Souche + '" ';
    StSQl := StSQl + '  AND GL_NUMERO =' + IntToStr(Numero) + ' AND GL_INDICEG = ' + IntToStr(Indice);
    StSQL := StSQL + '  AND GL_TYPELIGNE = "ART" AND (GL_TYPEARTICLE IN ("OUV","ARP")) AND (GL_INDICENOMEN <> 0) ';
    StSQl := StSQl + '  AND GL_AFFAIRE = "' + AFFAIRE.Text + '" ORDER BY GL_NATUREPIECEG,GL_SOUCHE,GL_NUMERO,GL_INDICEG,GL_NUMLIGNE';
...
begin
  ...
  Begin
    ...
    begin
      ...
      begin
        if IsLivChantier(TOBL.GetString('GL_PIECEPRECEDENTE'),TOBL.GetString('GL_PIECEORIGINE')) then
        begin
      ...
      end else if TOBL.GetString('GL_NATUREPIECEG')='' then
      begin
          if TypeElt = 'CONSOFOU' then TOBL.setString('TYPEELT','STOCK') else
          if TypeElt = 'CONSOFOUDEPUIS' then TOBL.setString('TYPEELT','STOCKDEPUIS') else
      end else
    ...
    Else IF TOBL.GetString('TYPERESSOURCE') = 'FRS' then
    Begin
...
begin
  ...
  //
  CreateLigneSsTotalToGrille('Frais de Chantier', 'FRS');
  ...
  //
  CreateLigneSsTotalToGrille('Provisions', 'PRV');
...
begin
  ...
  TotalBudgetDPR    := 0; TotalBudgetDPRPA    := 0;
  TotalRealiseDPR   := 0; TotalRealiseDPRPA   := 0;
  TotalReaDepuisDPR := 0; TotalReaDepuisDPRPA := 0;
  TotalTheoDPR      := 0; TotalTheoDPRPA      := 0;
  TotalResteDPR     := 0;
  TotalFinAffDPR    := 0; TotalFinAffDPRPA    := 0;
  TotalEngageDpr    := 0; 
...
begin
  ...
  TOBGrilleLig.AddChampSupValeur('REALISEMTPA', '');
  TOBGrilleLig.AddChampSupValeur('REALISEMT', '');
  TOBGrilleLig.AddChampSupValeur('ENGAGE', '');
  TOBGrilleLig.AddChampSupValeur('REALISEDEPUISPA', '');
  TOBGrilleLig.AddChampSupValeur('REALISEDEPUIS', '');
  TOBGrilleLig.AddChampSupValeur('REALISEQTEDEPUIS', '');
  TOBGrilleLig.AddChampSupValeur('THEOQTE','');
...
begin
    ...
    begin
      ...
      TOBL.PutValue('FINAFFAIREPA',    TotalRealiseDPRPA + TotalResteDPR);
...
procedure TOF_BTSAISIERESTDEP.TraitelesOuvrages(TOBLIGNESO, TOBOUVRAGES,TOBBudget: TOB);
  ...
  begin
    ...
    TOBB.AddChampSupValeur('TYPEARTICLE',TOBOuv.GetString('BLO_TYPEARTICLE'));
