...
Function  CalculDateFin(NbHJour : TDatetime; DDeb : TdateTime; NbHeure : Double) : String; OverLoad
...
Function  ControleCoherence(DateDeb,DateFin : TDateTime; Duree : double; Materiel : string) : boolean;
...
begin
  ...
  HeureDebAM := TimeToFloat(GetParamSocSecur('SO_BTAMDEBUT', '08:30'));
  HeureFinAM := TimeToFloat(GetParamSocSecur('SO_BTAMFIN',   '08:30'));
  HeureDebPM := TimeToFloat(GetParamSocSecur('SO_BTPMDEBUT', '08:30'));
  HeureFinPM := TimeToFloat(GetParamSocSecur('SO_BTPMFIN',   '08:30'));
...
Mots clefs ... : 
*****************************************************************}
Function CalculDateFin(NBHJour : TDateTime;DDeb : TdateTime; NbHeure : Double) : String;
var NBJour    : Integer;
    //
    DFin      : TDateTime;
    //
...
begin
  ...
  begin
    ...
    NBJour    := Trunc(NBHeure/hour);
    if NBJour <= 1 then
      Dfin    := DDeb
    else
      DFin    := IncDay(DDeb, NBJour);
  end;
