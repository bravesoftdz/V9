...
uses FactCpta,Facture,UtilTOBpiece,UCotraitance,
     ...
     FactTiers,factSituations,factComm,Splash
     ;
...
procedure TRectifSituation.ConstitueAvoir;

  procedure AjouteCommentaireProvenance;
  var Comment : string;
      TOBB : TOB;
  begin
    if NumSituation <> 0 then
    begin
      Comment := 'Avoir sur Situation N° '+InttoStr(NumSituation)+' / Facture N° '+TOBPiece.getString('GP_NUMERO')+' du '+ DateToStr(TOBpiece.GetDateTime('GP_DATEPIECE'));
    end else
    begin
      Comment := 'Avoir sur Facture N° '+TOBPiece.getString('GP_NUMERO')+' du '+ DateToStr(TOBpiece.GetDateTime('GP_DATEPIECE'));
    end;
...
begin
  ...
  TRY
    ...
    if not PassationComptable ( XTOBPiece,XTOBOUvrage,XTOBOuvragesP, XTOBBases,XTOBBasesL,
                               ...
                               XTOBPorcs, XTOBPIECERG, XTOBBASESRG, XTOBanaP,XTOBanaS,XTOBSSTRAIT, DEV, OldEcr,
...
begin
  ...
  CalculFacture(XTOBAffaire,XTOBPiece,XTOBPieceTrait,XTOBSSTRAIT,XTOBOUvrage, XTOBOuvragesP,XTOBBases, XTOBBasesL,XTOBTiers, XTOBArticles, XTOBPorcs, XTOBPieceRG, XTOBBasesRG, DEV, false, taCreat,false);
  if XTOBTimbres.Detail.Count > 0 then
  begin
    GereEcheancesGC(XTOBPiece, XTOBTiers, XTOBEches, XTOBAcomptes, XTOBPieceRG,XTOBPieceTrait,taCreat, DEV, False);
    CalculeTimbres (XTOBTimbres,XTOBpiece,XTOBEches);
  end;
  CalculeReglementsIntervenants(XTOBSSTRAIT, XTOBPiece,XTOBPIECERG,XTOBAcomptes,XTOBPiece.getValue('GP_AFFAIRE'),XTOBPieceTrait,DEV);
  GereEcheancesGC(XTOBPiece, XTOBTiers, XTOBEches, XTOBAcomptes, XTOBPieceRG,XTOBPieceTrait,tacreat, DEV, False);
end;
...
begin
  ...
  CalculFacture(TOBAffaire,TOBPiece,TOBPieceTrait,TOBSSTRAIT,TOBOUvrage, TOBOuvragesP,TOBBases, TOBBasesL,TOBTiers, TOBArticles, TOBPorcs, TOBPieceRG, TOBBasesRG, DEV, false, taCreat,false);
  if TOBTimbres.Detail.Count > 0 then
  begin
    GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBPieceTrait,taCreat, DEV, False);
...
procedure TRectifSituation.RectifieAvancement;
  ...
  begin
    ...
    TOBLS.setDouble('BLF_POURCENTAVANC', Arrondi(TOBLS.getDouble('BLF_MTCUMULEFACT')/TOBLS.getDouble('BLF_MTMARCHE')*100,2));
  end;
...
begin
  ...
  TRY
    ...
    //
    if V_PGI.IOError = OeOk then ConstitueAvoir;
