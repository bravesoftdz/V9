...
stListSelect:= 'GL_DEPOT,GL_DATELIVRAISON,GL_NATUREPIECEG,GL_NUMERO,GL_NUMLIGNE,GL_SOUCHE,GL_INDICEG,GL_DATEPIECE,'+
                             'GL_QUALIFQTEACH,GL_QUALIFQTEVTE,GL_QUALIFQTESTO,GL_QTERESTE';
InitialiseGrille;
...
for iIndex := 0 to TobVue.Detail.Count - 1 do
    ...
    if TobV.GetValue('NOMEN') = 'X' then TobV.PutValue ('RATIO',GetRatio (TobV,TobV, trsStock))
                                    else TobV.PutValue ('RATIO',GetRatio (TobV, nil, trsStock));
    if TobV.GetValue('VA') = 'ACH' then
       begin
...
begin
  ...
  CodeArt := AGLLanceFiche('BTP', 'BTARTICLE_RECH', '', '', StChamps + ';GA_TENUESTOCK=X;ACTION=CONSULTATION');
