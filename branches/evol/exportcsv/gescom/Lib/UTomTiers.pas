...
{$ENDIF}
  ...
  begin
    if Assigned(GetControl('T_CODEBARRE'))       Then CodeBarre      := THEdit(GetControl('T_CODEBARRE'));
    if Assigned(GetControl('T_QUALIFCODEBARRE')) Then TypeCodeBarre  := THValComboBox(GetControl('T_QUALIFCODEBARRE'));
...
// confidentialité , concept Modif & Creation Prospects
            ...
            BEGIN
              if (Not JaiLeDroitConcept(TConcept(bt510),False)) and (TFFiche(Ecran).fTypeAction<>taCreat) then  //confidentialité , concept Modif Prospect
              begin
...
{$ENDIF}
  ...
  BEGIN
    if ((StNatureAuxi = 'PRO') and (Not JaiLeDroitConcept(TConcept(gcProspectCreat),False))) or
       ((StNatureAuxi = 'CLI') and (Not JaiLeDroitConcept(TConcept(gcCLICreat),False))) Or
       ((StNatureAuxi = 'FOU') and (Not JaiLeDroitConcept(TConcept(gcFouCreat),False))) then
    BEGIN
  ...
  if TFFiche(Ecran).fTypeAction=TaModif then
  BEGIN
    if ((StNatureAuxi = 'PRO') and (Not JaiLeDroitConcept(TConcept(bt510),False))) or
       ((StNatureAuxi = 'CLI') and (Not JaiLeDroitConcept(TConcept(bt511),False))) Or
       ((StNatureAuxi = 'FOU') and (Not JaiLeDroitConcept(TConcept(bt518),False))) then
    Begin
...
begin
   ...
   begin
    ...
    begin
      //il faut prendre les 8 derniers caractère et pas les 8 premiers... (sic)
      Tampon := Copy(Tampon,Length(Tampon)-(lgcode+1),Length(Tampon));
...
begin
  ...
  if not QQ.Eof then
  begin
    SetField('T_CODEBARRE',       QQ.FindField('BCB_CODEBARRE').AsString);
    Setfield('T_QUALIFCODEBARRE', QQ.FindField('BCB_QUALIFCODEBARRE').AsString);
 end;
