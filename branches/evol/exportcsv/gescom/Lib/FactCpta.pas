...
Function  PassationComptable ( TOBPiece,TOBOuvrages,TOBOuvragesP,TOBBases,TOBBasesL,TOBEches,TOBPieceTrait,TOBAffInterv,TOBTiers,TOBArticles,TOBCpta,TOBAcomptes,TOBPorcs,TOBPieceRG,TOBBasesRG,TOBAnaP,TOBAnaS,TOBPieceInterv  : TOB ; DEV : Rdevise ; OldEcr,OldStk : RMVT ; bParle : Boolean ; NewPiece : Boolean = True ) : boolean ;
...
function TraitementVOuvrage (TOBEcr,TOBPieceinterv,TOBAFFInterv,TOBPiece,TOBL,TOBOuvragesP,TOBTiers,TOBArticles,TOBCpta,TOBPorcs: TOB;MM : RMVT ; NbDec : integer; EnHt : boolean; DEV : Rdevise) : T_RetCpta;
...
procedure RePackBase (TObbases : TOB);
var indice : integer;
    TOBB,TOBBE : TOB;
begin
...
BEGIN
  ...
  BEGIN
    ...
    begin
      ...
      TOBTL.Free;
      continue;
    end else if (FamTaxe = 'INT') and (Achat) then
    begin
      TOBTL := TOB.Create('PIEDBASE',nil,-1);
      TOBTL.Dupliquer(TOBB,False,true);
      if RecalcTva(TOBPiece,TOBTL) then
...
Function CreerLignesTiers ( TOBEcr,TOBPiece,TOBEches,TOBTiers,TOBAcomptes,TOBpieceRG,TOBBasesRG,TOBPieceTrait : TOB; DEV : RDevise; var MM : RMVT ) : T_RetCpta ;
...
BEGIN
  ...
  begin
    // -- sous traitance --
    FamArt := getParamSocSecur('SO_BTVENTILSOUSTRAIT','');  {Famille comptable article}
  end;
  // --
  CodeAffaire := TOBL.GetValue(prefixe+'_AFFAIRE');
  if Copy(CodeAffaire,1,1)='W' then
...
Function CreerLignesHTTVA ( TOBEcr,TOBAFFInterv,TOBPieceinterv,TOBPiece,TOBOuvragesP,TOBBases,TOBTiers,TOBArticles,TOBCpta,TOBPorcs,TOBAnaP,TOBAnaS : TOB ; MM : RMVT ; NbDec : integer; DEV : Rdevise ) : T_RetCpta ;
...
BEGIN
  ...
  BEGIN
    ...
    begin
      result := TraitementVOuvrage (TOBEcr,TOBPieceinterv,TOBAFFInterv,TOBPiece,TOBL,TOBOuvragesP,TOBTiers,TOBArticles,TOBCpta,TOBPorcs,MM, NbDec,EnHt,DEV);
    end else
    begin
...
Mots clefs ... : LIAISON;COMPTABILITE;PONT;PASSATION;
*****************************************************************}
Function PasseEnCompta ( TOBPiece,TOBOuvragesP,TOBBases,TOBEches,TOBpieceTrait,TOBAFFInterv,TOBTiers,TOBArticles,TOBCpta,TOBAcomptes,TOBPorcs,TOBPieceRG,TOBBasesRG,TOBAnaP,TOBAnaS,TOBPieceinterv : TOB ; NbDec : integer ; OldEcr : RMVT ; DEV : RDevise; NewPiece : Boolean) : T_RetCpta ;
...
begin
  {Lignes de Tiers}
  Result:=CreerLignesTiers(TOBEcr,TOBPiece,TOBEches,TOBTiers,TOBAcomptes,TOBPieceRg,TOBbasesRG,TOBpieceTrait,DEV, MM) ;
  {Lignes HT}
  if Result=rcOk then Result:=CreerLignesHTTva(TOBEcr,TOBAFFInterv,TOBPieceinterv,TOBPiece,TOBOuvragesP,TOBBases,TOBTiers,TOBArticles,TOBCpta,TOBPorcs,TOBanaP,TOBAnaS,MM,NbDec,DEV) ;
  {Taxes}
...
procedure CalculeLaPieceCpta (TOBPiece,TOBAcomptes,TOBEches,TOBArticles, TOBBases, TOBOuvrages,TOBOuvragesP,TOBtiers,TOBAFFInterv,TOBPorcs,TOBPieceRG,TOBBasesRG,TOBPieceTrait,TOBPieceinterv,TOBOuvPlatMere : TOB; DEV : Rdevise);
var EnHt : boolean;
...
begin
  ...
  repeat
    ...
    if IsCentralisateurBesoin(TOBL) then begin inc(i); Continue ; end;
  ...
  TOBBasesL.clearDetail;
  //
  RePackBase (TObbases); // du fait que les bases soient scindés via co ou sous traitants
...
Var Res : T_RetCpta ;
    Tex : String ;
    TOBTiersCpta : TOB ;
    TOBEchesCpta,TOBBasesCpta,TOBPieceCpta,TOBPieceRGCpta,TOBBaseRGCpta,TOBPorcsCpta,TOBAcomptesCpta,TOBOuvragesPCpta,TOBPieceTraitCpta : TOB;
    I : Integer;
BEGIN
...
//
if IsComptabilisationInterv (TOBPieceTrait) then // comptabilisation co traitance et/ou sous traitance
begin
...
//  RecupReglementsEnt (TOBAcomptesCpta,TOBAcomptes);
  ...
  CalculeLaPieceCpta (TOBpieceCpta,TOBAcomptesCpta,TOBEchesCpta,TOBArticles,TOBBasesCpta,TOBOuvrages,TOBOuvragesPCpta,TOBTiers,TOBAFFinterv,TOBPorcsCpta,TOBPieceRGCpta,TOBBaseRGCpta,TOBPieceTrait,TOBPieceInterv,TOBOuvragesP, DEV);
  //
end else
begin
...
Res:=PasseEnCompta(TOBPieceCpta,TOBOuvragesPCpta,TOBBasesCpta,TOBEchesCpta,TOBPieceTrait,TOBAFFInterv,TOBTiersCpta,TOBArticles,TOBCpta,TOBAcomptesCpta ,TOBPorcsCpta,TOBPieceRGCpta,TOBBaseRGCpta ,TOBanaP,TOBAnaS,TOBPieceinterv,DEV.Decimale,OldEcr,DEV,NewPiece) ;
if Res=rcOk then Res:=PasseEnStock(TOBPiece,TOBTiers,TOBArticles,TOBCpta,DEV.Decimale,OldStk) ;
TOBBasesCpta.free;
if IsComptabilisationInterv (TOBPieceTrait)  then  // gestion co traitance et sous traitance
begin
...
begin
  ...
  begin
    ...
    TOBA := TOBarticles.findFirst(['GA_ARTICLE'],[refUnique],true);
    if TOBA=Nil then Continue ;
    if TOBPP.detail.count < 2 then TOB.Create (' ',TOBPP,-1);
    if TOBPP.detail.count < 2 then TOB.Create (' ',TOBPP,-1);
