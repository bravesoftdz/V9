...
type
  ...
  private
    ...
    function  GetQteDetail(TOBLigneEtNomen : TOB) : Double;
...
uses InvUtil,StockUtil,UTofAjustStock,UTofConsultStock,DateUtils,Ulog,uEntCommun,factComm;
...
begin
  ...
  begin
    if (TobDispo.GetValue('GQ_PHYSIQUE') <> PrevQtePhys) then
    begin
      ChargeTobRecap(TobDispo,'Qté Physique',PrevQtePhys,0);
      change := true;
    end;
    if (TobDispo.GetValue('GQ_LIVRECLIENT') <> PrevLivCli) then
    begin
      ChargeTobRecap(TobDispo,'Livré client',PrevLivCli,0);
      change := true;
    end;
    if (TobDispo.GetValue('GQ_LIVREFOU') <> PrevLivFou) then
    begin
      ChargeTobRecap(TobDispo,'Livré fournisseur',PrevLivFou,0);
      change := true;
    end;
    if (TobDispo.GetValue('GQ_RESERVECLI') <> PrevResCli) then
    begin
      ChargeTobRecap(TobDispo,'Reservé client',PrevResCli,0);
      change := true;
    end;
    if (TobDispo.GetValue('GQ_RESERVEFOU') <> PrevResFou) then
    begin
      ChargeTobRecap(TobDispo,'Reservé fournisseur',PrevResFou,0);
      change := true;
    end;
    if (TobDispo.GetValue('GQ_PREPACLI') <> PrevPrepCli) then
    begin
      ChargeTobRecap(TobDispo,'Preparé client',PrevPrepCli,0);
    end;
  end;
  TobLigne.Free;
  Result := Change;
...
function TFStockAjust.GetQteDetail(TOBLigneEtNomen : TOB) : Double;
var Q : TQuery;
    TOBParent : TOB;
    StSqlNivPere : string;
begin
  ...
  begin
    ...
    Result := result * GetQteDetail(TOBParent);
    TOBParent.Free;
  end;
  Ferme(Q);
end;
