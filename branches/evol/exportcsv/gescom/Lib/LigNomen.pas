...
{$ENDIF}
  ...
  xmldom, XMLIntf, msxmldom, XMLDoc, TntExtCtrls, TntStdCtrls, TntGrids
  ;
...
{$IFDEF BTP}
uses factOuvrage,UtilMetres,UMetreArticle,UImportLigneNomen,UCotraitance,BTSAISIEPAGEMETRE_TOF;
{$ENDIF}
...
*)
    ...
    //
    if TobArticle.getValue('GA_PRIXPASMODIF')<>'X' Then AppliqueCoefDomaineActOuv (TOBOuv,'A',RecupPrix);
    //
...
{$IFDEF BTP}
  ...
  //FV1 : 22/01/2016 - FS#1845 - Pb en gestion de métrés si option "métrés dans EXCEL" non cochée en paramètres
  if GetParamSocSecur('SO_METRESEXCEL',true) then
  begin
    if TheMetreShare.controlMetre then
    begin
  ...
  begin
    result := GereMetre (TOBO,TOBmetres,Action,'BLO');
  end;
