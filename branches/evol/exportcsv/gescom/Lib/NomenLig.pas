...
BEGIN
  ...
  Repeat
    ...
    BEGIN
      ...
      BEGIN
{$IFDEF BTP}
        if (Pos('GNL_',NomCol) > 0) and (Pos('X',V_PGI.Dechamps[iTableLigne,ichamp].Control)>0) then
          ColsInter[icol] := True
        else if (Pos('GNL_',NomCol) = 0) then (* Pour les autres tables *)
          ColsInter[icol] := True;
{$ELSE}
...
begin
  ...
  //begin
     ...
     begin
        renseigneValoArt (TOBL,TobInd,nil);
     end;
