...
{$ENDIF}
  ...
  ,FactDomaines
  {$ENDIF}
  ,uEntCommun, TntStdCtrls, UCotraitance, FactDetOuvrage, FactTimbres, BTSpigao,
  FactRemplTypeLigne,UfactGestionAff,UAuditPerf,UtilsRapport, TntButtons,
  TntExtCtrls, TntGrids, TntComCtrls,
  UtilNumParag,UrectifSituations
  ;
type
...
function TransformePiece(CleDoc: R_CleDoc; NewNat: string; SaisieAveug: Boolean = False;LaDate : string = ''): boolean; //Saisie Aveugle
...
{$ENDIF}
  ...
  public
    ...
    DEV: RDEVISE;
    TransfoPiece, DuplicPiece, TransfoMultiple,ReajusteSitwAvoir: boolean;
...
function TransformePiece(CleDoc: R_CleDoc; NewNat: string; SaisieAveug: Boolean = False;LaDate : string=''): boolean;
var X: TFFacture;
  PP: THPanel;
begin
  Result := True;
...
begin
  ...
  if CleDoc.NaturePiece <> '' then TransformePiece(CleDoc, NewNat, SaisieAveugle,LaDate);
(*
  {$IFDEF BTP}
...
{$IFDEF BTP}
  if (Not TransfoMultiple ) and (not TransfoPiece) and (not DuplicPiece) and (Action = TaModif)  then
  begin
...
begin
	GardeFg := true;
  if (not TransfoMultiple ) and (not TransfoPiece) and (not DuplicPiece) then
  BEGIN
...
//    ZeroMontantPorts (TOBPorcs);
  ...
  begin
    ...
    else GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBPieceTrait, Action, DEV, False);
...
//    FactReCalculPv := true;
  ...
  {$ENDIF}
  *)
  if DuplicPiece or TransfoPiece or TransfoMultiple then
  begin
...
begin
  ...
  NumMess := 9;
  if (not TransfoPiece) and (not TransfoMultiple)  and (not IsModifiable (TOBPiece)) then
  begin
    Action := taConsult;
  end;

  if (Not TransfoMultiple) and (not TransfoPiece) and (not CanModifyPiece(TobPiece)) then { NEWPIECE : Pas de modification de piece V421 }
  begin
    Action := taConsult;
  ...
  if Morte then
  begin
    if (TransfoPiece) or (TransfoMultiple) then
    begin
      TransfoPiece := False;
  ...
  begin
    ...
    BDelete.Visible := (not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece) and (JaiLeDroitConcept(TConcept(bt516),False));
  end;
end;
...
//      GS.height := (GS.rowHeights[1] * GS.Rowcount) + (GS.GridLineWidth * GS.Rowcount);
    ...
    // MODIF LS Pour MODIF AVANCEMENET
  if (((not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece)) and (IsModeAvancement(TOBPiece))) or (SaisieTypeAvanc) then
  begin
  ...
  begin
    if (Pos(NaturePiece,'FBT;FBP')>0) and
       ...
       ((not TransfoPiece) and (not DuplicPiece)) then
    begin
...
//    FactReCalculPv := true;
  ...
  begin
    ...
    begin
      ...
      //
  		if ((not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece)) and (modifavanc) then TraiteAvancOuv := true;
      //
  ...
  begin
  	...
  	LoadLesLibDetail(TOBPiece, TOBNomenclature, TobOuvrage, TOBTiers, TOBAffaire, DEV, ChargeLesDetail);
...
begin
	if (Action = TaModif) and ((Not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece)) then
  begin
    if GS.CanFocus then
    begin
      Arow := 1;
...
begin
	...
	BintegreExcel.visible := (Action<>taConsult) and (not DuplicPiece) and
  												 (not TransfoMultiple) and (not TransfoPiece) and (not OrigineExcel) ;
...
{$ENDIF}
        ...
        if ((not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece) and (CleDoc.Indice > 0)) then GP_DATEPIECE.Enabled := False;
...
//  			TypeFacturation := RenvoieTypeFact(TOBPiece.GetValue('GP_AFFAIREDEVIS'));
        ...
        if (GetInfoParSouche(Cledoc.Souche, 'SH_NUMMOISPIECE') = 'X') and (not TransfoMultiple) and (not TransfoPiece) and (not DuplicPiece) then
        begin
  ...
  begin
    ...
    begin
      if (cledoc.NaturePiece = GetParamSoc('SO_AFNATAFFAIRE')) or
         (Pos(cledoc.NaturePiece,'FBT;FBP')>0) or
...
//  PGIInfo('Etape 13');
  ...
  begin
    ...
    DefiniPieceTrait (TOBBases,TOBpiece,TOBAffaire,TOBPieceTrait,TOBSSTRAIT,DEV);
  	CalculeReglementsIntervenants(TOBSSTRAIT, TOBPiece,TOBPIECERG,TOBAcomptes,TOBPiece.getValue('GP_AFFAIRE'),TOBPieceTrait,DEV);
  end;
  //
//  PGIInfo('Etape 14');
...
//        AppelleDim(GS.Row);
    ...
    {Ctrl+T} 84: if Shift = [ssCtrl] then
      begin
        Key := 0;
        if Not IsAutoLiquidationTva (TOBPiece) then
        begin
        	ChangeTva;
        end else
        begin
          PgiInfo ('Impossible vous êtes en Autoliquidation de TVA',self.Caption);
        end;
      end;
    {$IFDEF BTP}
...
//    HMTrad.ResizeGridColumns(GS);
  ...
  begin
	...
	if (SG_MTPRODUCTION > 0) and
     ...
     (not JaiLeDroitConcept(521,false))) then
  begin
...
{$IFDEF BTP}
  ...
  //if (ACol=SG_QF) and (ISFromExcel(TOBL)) and (not IsSousDetail(TOBL)) and (TOBL.GeTDouble('GL_QTEFACT')<>0) then BEGIN Result := False; Exit end;
  ...
  //if (Acol=SG_MTPRODUCTION) and IsSousDetail(TOBL) then begin result := false; exit; end;
(*
...
{$ENDIF}
  ...
  begin
    ...
    Result := not ((Action = taModif) and (not TransfoMultiple) and (not TransfoPiece) and (TOBL.GetValue('GL_QTEFACT') >= 0)
...
{$ENDIF}
  ...
  *)
  ...
  if result then
  begin
    if ACol <> SG_RefArt then Result := CanModifyLigne(TOBL, (TransfoPiece or TransfoMultiple), TobPiece_O,WarningSuppression,GestionReliquat,Action);
  end;
end;
...
//  if ARow > GS.TopRow + GS.VisibleRowCount - 1 then Exit;
  ...
  begin
    ...
    if TOBL = nil then Exit;
  	...
  	if GetParamSocSecur('SO_METRESEXCEL',true) then
    begin
      if (not ISFromExcel(TOBL)) and (TheMetre <> nil) and (TheMetre.MetreAttached(TOBL)) then
      begin
...
{$IFDEF BTP} // LS
  ...
  begin
    ...
    if (pos(TOBL.GetValue('GL_TYPEARTICLE'),'ARP;ARV;OUV') = 0) and
       (copy(TOBL.getString('GL_TYPELIGNE'),1,2) <> 'TP') then
       Canvas.FillRect(ARect);
  end;
...
{$IFDEF BTP}
  if (((action = taModif) and (not TransfoMultiple) and (not TransfoPiece)) or (action = taConsult)) and EstLigneArticle(TOBL) and EstLigneSoldee(TOBL) and
      Not(SaisieTypeAvanc) then // MODIF BRL AVANCEMENTS CHANTIERS
{$ELSE}
  if (((action = taModif) and (not TransfoMultiple) and  (not TransfoPiece)) or (action = taConsult)) and EstLigneArticle(TOBL) and EstLigneSoldee(TOBL) then
{$ENDIF}
  begin
    Canvas.Font.Style := Canvas.Font.Style + [fsStrikeOut];
...
begin
  ...
  begin
    ...
    if (not OrigineEXCEL) and ((Cledoc.NaturePiece = GetParamSoc('SO_AFNATAFFAIRE')) or
       ...
       (Pos(cledoc.NaturePiece,'FBT;FBP')>0)) then
    begin
      if GetParamSocSecur('SO_METRESEXCEL',true) then
      begin
...
{$ENDIF}
  ...
  if (GetParamSoc('SO_PREFSYSTTARIF')) and (not TransfoPiece) and (not TransfoMultiple) then
  begin
...
begin
  ...
  begin
    TOBL := GetTOBLigne(TOBPiece, GS.Row);
    if (not ISFromExcel(TOBL)) and ((Cledoc.NaturePiece =GetParamSoc('SO_AFNATAFFAIRE')) or
    	 (Pos(cledoc.NaturePiece,'FBT;FBP')>0) or
       (Cledoc.NaturePiece = GetParamSoc('SO_AFNATPROPOSITION'))) then
    begin
    	if GetParamSocSecur('SO_METRESEXCEL',true) then
...
begin
  ...
  //
  if VH_GC.GCIfDefCEGID then
    if ((Action = taModif) and (not TransfoPiece) and (not TransfoMultiple) and (not DuplicPiece) and
    ...
    taModif:
      begin
        if (DuplicPiece) or (Force) then AvecT := True else
          if (((TransfoPiece) or (TransfoMultiple)) and (GetInfoParPiece(NewNature, 'GPP_TYPEECRCPTA') = 'NOR')) then AvecT := True;
      end;
  end;
  if AvecT then
  begin
...
begin
  ...
  if ((Action = taCreat) or (DuplicPiece) or (TransfoPiece) or (TransfoMultiple) ) then Result := TOBTiers.GetValue('T_ETATRISQUE');
  ...
  begin
    ...
    TotR := RisqueTiers + TOBPiece.GetValue('GP_TOTALTTC');
    if ((Action = taCreat) or (DuplicPiece) or (TransfoPiece) or (TransfoMultiple) ) then
    begin
...
{$ENDIF}
  ...
  if ((Rouge) and (GeneCharge) and ((TransfoPiece) or (TransfoMultiple)) ) then
  begin
    Action := taConsult;
    TransfoPiece := False;
    NewNature := TOBPiece.GetValue('GP_NATUREPIECEG');
...
begin
  if GeneCharge then Exit;
  if Action = taConsult then Exit;
  if ((Action = taModif) and (not DuplicPiece) and (not TransfoPiece) and (not TransfoMultiple) ) then Exit;
  OldTaux := DEV.Taux;
  GP_DEVISEChange(nil);
...
{$ENDIF}
    ...
    begin
      ...
      if ((CalcRupt = 'DIS') or (TOBL.GetValue('GL_ENCONTREMARQUE') = 'X') or ((not TransfoPiece) and (not TransfoMultiple)) ) then
      begin
...
begin
  ...
  if GetParamSocSecur('SO_METRESEXCEL',true) then
  begin
    if (not ISFromExcel(TOBL)) and (ATraiterQte) then
    begin
...
{$ENDIF}
    begin
      ...
      lCtrlMinimum := (not TransfoPiece) and (not TransfoMultiple)  and (TobL.GetValue('GL_PIECEPRECEDENTE')='');
      if      (VenteAchat = 'VEN') then
...
{$ENDIF}
  ...
  if GetParamSocSecur('SO_METRESEXCEL',true) then
  begin
    if (not ISFromExcel(TOBL)) and (VenteAchat = 'VEN') then
    begin
...
begin
  ...
  if Action = taConsult then Exit;
  if ((TransfoPiece) or (TransfoMultiple) or (DuplicPiece)) then Exit;
...
begin
  ...
  if TOBTimbres.Detail.Count > 0 then
  begin
    GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBPieceTrait,Action, DEV, False);
...
//      GetcumultaxesRG(TOBBasesRG, TOBPieceRG, TXD, TXP, DEV);
  ...
  begin
    ...
    begin
      vasy := true;
      //GetcumultaxesRG(TOBBasesRG, TOBPieceRG, TXD, TXP, DEV);
      GetTVARg (TOBPIECERG,(TOBPiece.GetValue('GP_FACTUREHT')='X'),True,TXP,TXD);
...
begin
  ...
  BZoomPiece.Enabled := (not TransfoPiece) and (not TransfoMultiple) and (action in [taModif, taConsult]);
...
(*    {$IFDEF CCS3} // DBR Fiche 10202
  ...
  begin
    ...
    if (Action = taModif) and (not TransfoPiece) and (not TransfoMultiple) then
    begin
    ...
    MBDetailNomen.Enabled := ((OkArt) and (TOBL.GetValue('GL_INDICENOMEN') > 0) and (JaiLeDroitConcept(TConcept(bt512),False)) );
    {$IFNDEF CCS3}
...
begin
  ...
  if (ISFromExcel(TOBL)) or (SPIGAOOBJ.TestAffaire (TOBPiece.GetValue('GP_AFFAIRE'))) then
  begin
    BouttonInVisible;
...
begin
  ...
  Ok := GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBPieceTrait,CurrAction, DEV, True);
...
//	Enabled := False;
  ...
  //Begin
  //  PGIError(TraduireMemoire('Impression impossible demande de visa sur la pièce !'),TraduireMemoire('Gestion des Visas'));
  //  Exit;
  //end;
  //
...
begin
  ...
  begin
    PGIBox('ATTENTION : Ce document est issu d''un devis');
  end;
  if IsDejaFacture then
  begin
    PgiError(Traduirememoire('ATTENTION : une facturation a déjà été effectuée.'));
  end;
  *)
  //
  ClickPorcs;
...
begin
  ...
  {$ENDIF}
  if CanDeletePiece(TobPiece, TobPiece_O, (TransfoPiece or TransfoMultiple), NumL,Action) then
  begin
...
begin
  ...
  if (Action <> taConsult) and (VH_GC.GCIfDefCEGID) and // DBR ligne de la NGI intouchable
     (not transfopiece) and (not TransfoMultiple) and (GetChampLigne(TOBPiece, 'GL_REFCOLIS', GS.Row) <> '') then exit;
{$IFDEF BTP}
	if IsLigneReferenceLivr (GetTOBLigne(TOBpiece,GS.row)) then exit;
...
//  if BTypeArticle.Visible then BTypeArticle.Visible := false;
  ...
  if not CanModifyLigne(TobLigne, (TransfoPiece or TransfoMultiple ), TobPiece_O,WarningSuppression,gestionReliquat,Action) then
  begin
...
begin
  ...
  begin
    ...
    IsCalculnecessaire := AppliqueChangementEnteteBtp (self,TheTob,TobPiece,TobOuvrage,TOBporcs,TOBBases,ToBbasesL,RegimeTaxe,SavModApplicFg,SavModApplicFC,ModeRegle_O);
...
begin
  ...
  {$IFDEF BTP}
  if Not JaiLeDroitConcept(TConcept(bt512),True) then Exit; //confidentialité , concept acces sous-détail
  addicted := false;
  {$ENDIF}
...
begin
  ...
  if (result = 1) and (not ControleSaisiePieceBtp (Tobpiece,GP_AFFAIRE.Text,Action,(TransfoPiece or TransfoMultiple),DuplicPiece,false,IsBordereau)) then
  begin
  	result := 0;
  end;
  ...
  *}
  ...
  begin
    ...
    BEGIN
      ...
      TiersChanged := true;
      GP_TIERS.text := AFF_TIERS.Text;
      if ((not TransfoPiece) and  (not TransfoMultiple) and (not DuplicPiece)) and (Action <> Tacreat) then nature := TOBPiece.getValue('GP_NATUREPIECEG')
                                                                            else Nature := newnature;
      RemplirTOBTiers(TOBTiers, GP_TIERS.Text, Nature, False);
      IncidenceTiers;
      TiersChanged := false;
...
begin
  ...
  if not PassationComptable( TOBPiece,TOBOUvrage,TOBOuvragesP, TOBBases,TOBBasesL, TOBEches,TOBPieceTrait,TOBAffaireInterv,TOBTiers, TOBArticles, TOBCpta, TOBAcomptes, TOBPorcs, TOBPIECERG, TOBBASESRG, TOBanaP,TOBanaS,TOBSSTRAIT, DEV, OldEcr,
    OldStk, True, ((TransfoPiece = True) or (TransfoMultiple) or (DuplicPiece = True)) ) then
  begin
...
begin
  if Action <> taCreat then if ((not DuplicPiece) and (not TransfoPiece) and (not TransfoMultiple) ) then Exit;
...
begin
  ...
  if ((Action = taCreat) or (TransfoPiece) or (TransfoMultiple) or (DuplicPiece)) then
  begin
...
begin
  ...
  if (((not TransfoPiece) and (not TransfoMultiple)) or (not HistoPiece(TOBP))) then
  begin
...
//    if (tModeBlocNotes in SaContexte) then ValideLesLiensOle(TOBPiece, TOBPiece_O, TOBLienOLE);
  ...
  begin
    ...
    begin
      ...
      for II := 0 to TOBListPieces.detail.count -1 do
      begin
        if not TOBListPieces.detail[II].UpdateDB(False) then
        BEGIN
          MessageValid := 'Erreur mise à jour des pièces précédente (UPDATE)';
          V_PGI.IoError := oeSaisie; { NEWPIECE }
        end;
      end;
  ...
  if (V_PGI.IoError = oeOk) and (Action = taModif) and (not DuplicPiece) and (not TransfoPiece) and (not TransfoMultiple) then
  begin
...
//       (Action<>taConsult) and (GetParamSoc ('SO_GCCONTROLEFACTURE')) then
  ...
  begin
    ...
    if DocArecalc.NumeroPiece <> 0 then
    begin
...
//    wShowMeTheTob(TobLigneTarif);
  ...
  if (V_PGI.IOError = OeOk) Then GestionConso.GenerelesPhases(TOBPiece,TOBCONSODEL,(TransfoPiece or TransfoMultiple),DuplicPiece,false,Action);
  ...
  if (V_PGI.IOError = OeOk) Then GenereLivraisonClients(TOBPiece,Action,(transfoPiece or TransfoMultiple),DuplicPiece,false);
...
var io: TIOErr;
  ...
  begin
    ...
    if (Action = taConsult) or
      ((Action = taModif) and ((not DuplicPiece) and (not TransfoPiece) and (not TransfoMultiple) )) then exit;
    ...
    begin
      ...
      if (Action = taModif) and ((TransfoPiece) or (TransfoMultiple))then
    begin
...
//     (Action<>taConsult) and (GetParamSoc ('SO_GCCONTROLEFACTURE')) then
  ...
  else Ok := GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPIECERG,TOBPieceTrait, Action, DEV, bOuvreEche);
  if not Ok then
  begin
//    if (VenteAchat = 'ACH') and (GetInfoParPiece(NewNature,'GPP_TYPEECRCPTA') = 'NOR') and
...
// sortie de la saisie en cas d'erreur de la validation
  ...
  begin
    if ((Action = taCreat) or (Action = taModif) or (DuplicPiece) or (TransfoPiece) or (TransfoMultiple) ) and
  ...
  if Action <> taCreat then
  begin
    if ((DuplicPiece) or (TransfoPiece) or (TransfoMultiple) ) then BEGIN MontreNumero(TOBPiece);  END;
    InitPasModif;
    Close;
  end else
  begin
...
begin
  ...
  if (Action = taModif) and (not TransfoPiece) and (Not TransfoMultiple)  and (not DuplicPiece) and (Date >= CleDoc.DatePiece + 1) then
  begin
    Action := taConsult;
...
//  if TypePresent = DOU_AUCUN then Exit;
  ...
  begin
    ...
    begin
      ...
      (*
      if (WithAvanc) and (TOBLOC.FieldExists ('BLF_QTESITUATION')) then
      begin
        QteFact := TOBLOC.GetValue('BLF_QTESITUATION');
      end else
      begin
...
//        MontantPA :=Arrondi((TOBLOC.GetValue('BLO_QTEFACT')*TOBLOC.GetValue('BLO_DPA'))/TOBLOC.GetValue('BLO_PRIXPOURQTE'),V_PGI.okdecP);
        ...
        begin
          ...
          begin
            ...
            if (WithAvanc) and (TOBLOC.FieldExists ('BLF_QTESITUATION')) then
            begin
            	MontantHtDev :=TOBLOC.GetValue('BLF_MTSITUATION');
              Montant := TOBLOC.GetValue('BLF_MTSITUATION');
            end else
            begin
            *)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (WithAvanc) and (TOBLoc.FieldExists ('BLF_QTESITUATION')) then
            begin
          		TOBL.PutValue('GL_QTEFACT', TOBLOC.GetValue('BLF_QTESITUATION'));
            end else
            begin
          	*)
...
//    GS.height := (GS.rowHeights[1] * (GS.Rowcount-TheRgpBesoin.NbLignes)) + (GS.GridLineWidth * (GS.Rowcount-TheRgpBesoin.NbLignes));
    numerotelignesGc(nil, TOBPiece);
    for Indice := Rowreference-1 to TOBPiece.Detail.Count - 1 do AfficheLaLigne(Indice + 1);
*)
  end;
  GS.Row := RowReference;
end;
...
begin
  ...
  *)
  ...
  if IsMultiIntervRg (TOBpieceTrait) then
  begin
    GestionRGIntervenants (self,TobPiece, TOBpieceTrait,TOBBases,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV);
    RecalculeRG(TOBPorcs,TOBPiece, TOBPieceRG, TOBBases, TOBBasesRG,TOBpieceTrait, DEV);
    GetMontantRGReliquat(TOBPIeceRG, XD, XP);

    TOTALRGDEV.Value := XD;
    if TOBPieceRG.detail.count > 0 then TOBRG := TOBPieceRG.Detail[0]
    															 else TOBRG := nil;
    if TOBRG <> nil then
    begin
...
begin
  ...
  if Assigned(Tobl) then
  begin
    if (TransfoPiece or TransfoMultiple) then { Transformation d'une pièce en cours }
    begin
...
{$IFDEF GPAO}
  ...
  begin
    ...
    if (IdentWOL <> 0) and (not transfopiece) and (not TransfoMultiple) then
    begin
    ...
    else if (IdentWOL <> 0) and (transfopiece or TransfoMultiple)  then
    begin
    ...
    else if (IdentWOL = 0) and (not transfopiece) and (not TransfoMultiple) and (GetInfoParpiece(cleDoc.NaturePiece, 'GPP_INSERTLIG') = '-') then
    begin
      gs.col := SG_PX;
      canselect := false;
    end
...
begin
{$IFDEF BTP}
  if IsAutoLiquidationTva (TOBPiece) then
  begin
		PgiInfo ('Impossible vous êtes en Autoliquidation de TVA',self.Caption);
  end;
  if (ModifAvanc) and (Pos(TOBpiece.GetValue('GP_NATUREPIECEG'),'FBT;FBP')>0) then
...
begin
  ...
  begin
    ...
    TRY
      ...
      Nbmax := texte.lines.Count;
      if NbMax > 15 then NbMax := 15;
      Height := Nbmax * (GS.Canvas.TextHeight ('W') + 2);
    finally
      texte.Free;
    END;
  end else
