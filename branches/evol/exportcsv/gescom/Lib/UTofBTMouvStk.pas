...
begin
  ...
  if Article.Text <> '' then StChamps := 'GA_CODEARTICLE=' + Trim(Copy(Article.text, 1, 18)) + ';XX_WHERE=' + sWhere
	...
	TheArticle  := AGLLanceFiche('BTP', 'BTARTICLE_RECH', '', '', StChamps+';RECHERCHEARTICLE');
...
var QQ : TQuery;
begin
  QQ := OpenSql ('SELECT * FROM BTTMPMOUVSTK WHERE BTM_UID="'+fUid+'" AND BTM_NATUREPIECEG="LBT"',false);
  if not QQ.eof then
  begin
    QQ.First;
    While not QQ.eof do
