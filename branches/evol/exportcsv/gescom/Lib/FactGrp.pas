...
if Not PassationComptable(TOBGenere,TOBOuvrage, TOBOuvragesP,TOBBases,TOBBasesL,TOBEches,TOBPieceTrait,nil,TOBTiers,TOBArticles,TOBCpta,TOBAcomptes,TOBPorcs,TOBPIECERG,TOBBasesRG,TOBAnaP,TOBAnaS,nil,DEV,OldEcr,OldStk,Not(bContinuOnError))
...
CalculFacture(TOBAffaire,TOBGenere,TOBPieceTrait,TOBSSTrait,TOBOuvrage,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV) ;
{$IFDEF BTP}
...
if NeedVisaEnd then
   ...
   END ;
{Echéances}
TOBEches.ClearDetail ; GereEcheancesGC(TOBGenere,TOBTiers,TOBEches,TOBAcomptes,TOBPieceRG,nil,taCreat,DEV,False) ;
G_InitToutModif ;
G_TraiteEuro ;
// Modif MODE
...
GereEcheancesGC(TOBGenere,TOBTiers,TOBEches,TOBAcomptes,TOBPieceRG,TOBPieceTrait,taCreat,DEV,False) ;
G_InitToutModif ;
G_TraiteEuro ;
...
// MODIF BTP -- LS
if (V_PGI.IoError=oeOk) and (FromLigneDetail) and ((ModeGeneration = 'CBTTOLIVC') or (ModeGeneration = 'RFOTOLIVC')) then
begin
...
if ModeGeneration ='GENERECF' then
begin
...
var TOBSource:TOB;
    ...
    RatioStk, Qte, ratioVA,RatioVente:double;
    NumPiece : integer;
    // Modif BTP
    UpdateDev : boolean;
...
{$IFDEF BTP}
      ...
      if (GenereMode = 'CBTTOLIVC') or (GenereMode = 'RFOTOLIVC') then
      begin
...
{$ENDIF}
    ...
    if (GenereMode <> 'CBTTOLIVC') and (GenereMode <> 'RFOTOLIVC') then
    begin
