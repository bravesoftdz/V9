...
{$ENDIF}
    function GetLibelleChampsCtx(iTable,iChamp  : integer): string;
{$IFDEF GRC}
...
while stTable<>'' do
   begin
   iTable := PrefixeToNum (TableToPrefixe (stTable));
   for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
       begin
//       if pos( ctrl,V_PGI.DEChamps[iTable,iChamp].Control)=0 then continue ;
       if V_PGI.DEChamps[iTable,iChamp].libelle <> '' then // DBR Fiche 10093 au cas ou le libelle n'est pas chargé
                                                           // dans V_PGI.DECHamps, on ne peut pas tester la position 1
        if V_PGI.DEChamps[iTable,iChamp].libelle[1]='.' then continue ; // pas les champs libres si le nom commence par un point
{$IFDEF GESCOM}
       if copy(V_PGI.DEChamps[iTable,iChamp].nom,1,12)='GPK_TOXAPPEL' then continue ; // pas les champs D'heure d'appel transmission caisse
{$ENDIF}
       // Modif BTP
       if (Contexte <> '') and (not ChampDansContexte (StTable,V_PGI.DEChamps[iTable,iChamp].nom)) then Continue;
       //
       FListe.RowCount:=R+1; FListe.Row:=R;
       St:='';
       FListe.Cells[0,R]:=V_PGI.DEChamps[iTable,iChamp].nom;
       FListe.Cells[1,R]:=V_PGI.DEChamps[iTable,iChamp].tipe;
       if ( copy(V_PGI.DEChamps[iTable,iChamp].nom,1,19) = 'YTC_TABLELIBRETIERS' ) or
          ( copy(V_PGI.DEChamps[iTable,iChamp].nom,1,17) = 'YTC_TABLELIBREFOU' ) then
          FListe.Cells[1,R]:='COMBO';
       if pos('LIBMUL',FListe.Cells[0,R])>0 then FListe.Cells[1,R]:='MULTI';
       FListe.Cells[2,R]:=GetLibelleChampsCtx(iTable,iChamp);
       if FListe.Cells[2,R]='' then FListe.Cells[2,R]:=V_PGI.DEChamps[iTable,iChamp].nom;
       inc(R);
       end;
   stTable:=ReadTokenSt(fTable);
   end;
...
if Contexte = 'MAR' then
    ...
    Autorise := 'GA_FAMILLENIV1;GA_FAMILLENIV2;GA_FAMILLENIV3;GA_COMPTAARTICLE;'+
                ...
                'GA_PRIXPASMODIF;GA_DOMAINE;GA_GEREDEMPRIX;GA_FANTOME;GA_REMISELIGNE;GA_GERESAV;';
    end else
if Contexte = 'ARP' then
    ...
    Autorise := 'GA_FAMILLENIV1;GA_FAMILLENIV2;GA_FAMILLENIV3;GA_COMPTAARTICLE;'+
                ...
                'GA_COEFFG;GA_CALCPRIXHT;GA_CALCPRIXTTC;GA_COEFCALCHT;GA_COEFCALCTTC;'+
                'GA_CALCAUTOHT;GA_CALCAUTOTTC;GA_FOURNPRINC;GA_DPRAUTO;GA_DOMAINE;GA_CALCPRIXPR;GA_NATUREPRES;'+
                'GA_PRIXPASMODIF;GA_GEREDEMPRIX;GA_FANTOME;GA_REMISELIGNE;GA_GERESAV;';
    end else
if Contexte = 'PRE' then
    ...
    Autorise := 'GA_FAMILLENIV1;GA_FAMILLENIV2;GA_FAMILLENIV3;GA_COMPTAARTICLE;'+
                ...
                'GA_PRIXPASMODIF;GA_DOMAINE;GA_GEREDEMPRIX;GA_FANTOME;GA_GEREANAL;GA_SECTION;GA_REMISELIGNE;GA_GERESAV;';
    end else
...
Function TGCFModifZS.GetLibelleChampsCtx (iTable,iChamp  : integer) : string;
begin
  if V_PGI.DEChamps[iTable,iChamp].Nom  = 'GA_PRIXPASMODIF' then
  begin
