...
// Modif BTP
function GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBPieceTrait: TOB; Action: TActionFiche; DEV: RDevise; Ouv: boolean): Boolean;
// -----
{$IFDEF CHR}
...
// modif BTP
procedure ProrateEchesGC(TOBPiece, TOBEches, TOBAcomptes, TOBPieceRG, TOBPieceTrait: TOB; DEV: RDEVISE);
// --
...
begin
  ...
  if Nb <> 1 then
  BEGIN
    MessageValid := 'Erreur écriture Tiers';
...
function InitModeGC(TOBPiece, TOBTiers, TOBAcomptes, TOBPieceRG, TOBpieceTrait : TOB; DEV: RDEVISE): T_ModeRegl;
var Mode: T_ModeRegl;
  XP, XD : Double;
  DD: TDateTime;
  // Modif BTP
...
//  GetCumulRG(TOBPIeceRG, RGP, RGD);
  // ---
  MtAprendreEncompte := Arrondi(TOBPiece.GetValue('GP_TOTALTTCDEV') - XP - RGP,DEV.decimale);
...
function FabriqueEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBpieceTrait : TOB; DEV: RDEVISE): T_ModeRegl;
var Mode: T_ModeRegl;
    nbacpt : Integer;
begin
  Mode := InitModeGC(TOBPiece, TOBTiers, TOBAcomptes, TOBPieceRG,TOBpieceTrait, DEV);
...
function RenseigneEchesGC (TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPIECERG, TOBpieceTrait: TOB;
...
begin
  ...
  begin
  	ProrateEchesGC(TOBPiece, TOBEches, TOBAcomptes, TOBPieceRG, TOBPieceTrait,DEV);
  end;
...
begin
  ...
  if (TOBpieceTrait <> nil) and (TOBpieceTrait.detail.count > 0) and (ExisteReglDirect(TOBpieceTrait)) then
  begin
		Mode := FabriqueEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBpieceTrait,  DEV)
  end else
  begin
    if TOBEches.Detail.Count <= 0 then Mode := FabriqueEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBpieceTrait,  DEV)
                                	else Mode := RenseigneEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG,TOBpieceTrait, DEV, Action);
  end;
  if Ouv then
  begin
...
//                TobLignePiecePrecedente := FindCleDocInTobPiece(CleDocPrecedente, TobPiecePrecedente);
                ...
                begin
                  ...
                  Sql := 'UPDATE LIGNE SET GL_QTERESTE = GL_QTERESTE ' + Signe + ' ' + StrFPoint(Diff)
                    + ',GL_VIVANTE="X" WHERE ' + WherePiece(CleDocPrecedente, ttdLigne, True, True);
...
//                        NewQteReste := Max(0, Double(TobLignePiecePrecedente.G('GL_QTERESTE')) + Diff);
            ...
            end else if (((CleDoc.NaturePiece <> CleDocPrecedente.NaturePiece)
                        or (CleDoc.Souche <> CleDocPrecedente.Souche)
                        or (CleDoc.NumeroPiece <> CleDocPrecedente.NumeroPiece))) and
                        (GetInfoParPiece (Cledoc.NaturePiece,'GPP_RELIQUAT')='-') then
            begin
              Sql := 'UPDATE LIGNE SET GL_VIVANTE="X",GL_QTERESTE=GL_QTEFACT WHERE ' + WherePiece(CleDocPrecedente, ttdLigne, True, True);
              Result := (ExecuteSQL(Sql) = 1);
              if Result then
              begin
...
begin
  ...
  begin
    ...
    *)
    if not TOBEvtsmat.UpdateDB (true) then
    begin
      MessageValid := 'Erreur écriture BTEVENTMAT (UPD)';
      V_PGI.Ioerror := OeUnknown;
      Exit;
    end;
    TobNEVTMAT.free;
