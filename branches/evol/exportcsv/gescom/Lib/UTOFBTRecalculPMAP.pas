...
begin
  ...
  if CodeArticle.Text <> '' then StChamps := 'GA_CODEARTICLE=' + Trim(Copy(CodeArticle.text, 1, 18)) + ';XX_WHERE=' + sWhere
	...
	CodeArticle.text  := AGLLanceFiche('BTP', 'BTARTICLE_RECH', '', '', StChamps+';RECHERCHEARTICLE');
  ArticleOnExit(Self);
end;
...
begin
  ...
  TRY
    ...
    if (TOBChoix.GetString('OKGO')='X') and ((TOBChoix.GetString('LES LIV')='X') OR (TOBChoix.GetString('LES CONSOS')='X'))then
    begin
