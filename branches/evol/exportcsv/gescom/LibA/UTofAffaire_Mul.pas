...
Begin
  ...
  begin
    ...
    Begin
      ...
      ETATAFFAIRE.Enabled := false;
      THedit(GetCOntrol('XX_WHERE')).Text :=  ' AND (NOT AFF_ETATAFFAIRE IN ("ANN","CL1","ECR","FIN","TER","FAC"))';
      SetControlproperty('FListe', 'Multiselection', true);
    end Else
    begin
...
begin
   ...
   if Action='ACTION=RECH' then
      ...
      begin
      if (Pos(EtatAffaire,'TER;CLO')>0) and (not V_PGI.SAV) then Action := 'ACTION=CONSULTATION';
      //if pos('APP',StatutAff.value)>0 then
      if pos('W',Affaire)>0 then
...
{$ENDIF}
  ...
  begin
  	retourAff := AGLLanceFiche('BTP',NomFiche,'','','ACTION=CREATION;STATUT:'+ StatutAffaire + ';' + Tiers + ';'+ EtatAffaire + ';NATURE:'+natpiece+ModeGestionCotrait);
