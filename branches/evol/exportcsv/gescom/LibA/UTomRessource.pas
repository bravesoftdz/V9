...
const
  ...
  TexteMsgRessource: array[1..31] of string = (
    ...
    {26}, 'La famille de ressource saisie n''existe pas.'
...
//chargement option calcul auto du prix de revient...
  ...
  if GetControlText('ARS_TYPERESSOURCE')<>'SAL' then SetControlVisible('ARS_GERESAV',false)
                                                else SetControlVisible('ARS_GERESAV',true);
...
{$IFDEF GPAO}
  if (Ensaisie) and (LastError = 0) and (GetControl('ARS_RESSOURCELIE') <> nil) and (GetField('ARS_RESSOURCELIE') <> '') then
     begin
     if not wExistFamRessource(GetField('ARS_RESSOURCELIE')) then
        begin
        LastError := 23;
        LastErrorMsg := TexteMsgRessource[LastError];
        Exit;
