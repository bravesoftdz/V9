...
uses
  ...
  UtilEdt, Printers, EdtLegal, Filtre, CritEDT,CpteUtil,UtilEdt1, HTB97, HPanel, UiUtil, tCalcCum ;

procedure GLivreAgeTP ;
procedure GLivreAgeZoomTP(Crit : TCritEdt) ;

type
  TFQRGLAgeTP = class(TFQR)
    HLabel3: THLabel;
...
Function TFQRGLAgeTP.QuoiCpt(i : Integer) : String ;
...
Procedure TFQRGLAgeTP.GLAgeZoom(Quoi : String) ;
Var Lp,i: Integer ;
BEGIN
...
BEGIN
PP:=FindInsidePanel ;
QR:=TFQRGLAgeTP.Create(Application) ;
Edition.Etat:=etGlAge ;
QR.QRP.QRPrinter.OnSynZoom:=QR.GLAgeZoom ;
QR.InitType (nbAux,neGlV,msRien,'QRGLAGEETP','',TRUE,FALSE,FALSE,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
procedure TFQRGLAgeTP.InitDivers ;
// Var ll : Integer ;
BEGIN
Inherited ;
TLE_VALIDE.Caption:='' ;
...
procedure TFQRGLAgeTP.GenereSQL ;
BEGIN
Inherited ;
GenereSQLTiers(neGL,Age,CritEdt,Q,TRUE,AvecRevision.State) ;
GenereSQLSub ;
END ;

procedure TFQRGLAgeTP.GenereSQLSUB ;
BEGIN
Inherited ;
GenereSQLSUBTiers(neGL,Age,CritEdt,QEcr,TRUE) ;
END ;

procedure TFQRGLAgeTP.RenseigneCritere ;
...
procedure TFQRGLAgeTP.ChoixEdition ;
begin
Inherited ;
...
Procedure TFQRGLAgeTP.FinirPrint ;
Var Solde   : Double ;
BEGIN
Inherited ;
QEcr.Close ;
...
if OkMajEdt Then
   ...
   if Solde<0
      then MajEdition('GLA', '', DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date1),
                      '', TotEdt[6].TotDebit, TotEdt[6].TotCredit, Solde, 0)
      else MajEdition('GLA', '', DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date1),
                      '', TotEdt[6].TotDebit, TotEdt[6].TotCredit, 0, Solde) ;
   END ;
END ;

function TFQRGLAgeTP.CritOk : Boolean ;
BEGIN
...
procedure TFQRGLAgeTP.RecupCritEdt ;
...
Procedure TFQRGLAgeTP.Calculs(Solde : TQRLabel; var Report1,Report2 : TReport; i : integer; Bool1,Bool2 : Boolean ) ;
...
procedure TFQRGLAgeTP.SaisieDates ;
...
procedure TFQRGLAgeTP.FormShow(Sender: TObject);
begin
//HelpContext:=7550010 ;
...
procedure TFQRGLAgeTP.FChoixEcartClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.FDateCompta1Exit(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFQRGLAgeTP.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var Cpt : String ;
begin
  inherited;
...
procedure TFQRGLAgeTP.BMdpBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLAgeTP.BFCompteAuxBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
PrintBand:=Affiche ;
...
procedure TFQRGLAgeTP.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLAgeTP.FP5Exit(Sender: TObject);
Var St : String ;
    A  : Char   ;
begin
  inherited;
...
procedure TFQRGLAgeTP.tabSupEnter(Sender: TObject);
begin
  inherited;
FPeriodicite.SetFocus ;
end;

procedure TFQRGLAgeTP.FPeriodiciteChange(Sender: TObject);
Var i    : Integer ;
    TabD : TTabDate8 ;
begin
  inherited;
...
procedure TFQRGLAgeTP.TOPREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLAgeTP.BOTTOMREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
begin
  inherited;
...
procedure TFQRGLAgeTP.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFQRGLAgeTP.QEcrAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFQRGLAgeTP.BDetail2BeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport  : TabTRep ;
    OkCalCul : Boolean ;
...
procedure TFQRGLAgeTP.FNatureCptChange(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.BFCompteAuxAfterPrint(BandPrinted: Boolean);
begin
  inherited;
InitReport([2],CritEdt.GlV.FormatPrint.Report) ;
end;

procedure TFQRGLAgeTP.FColl1Change(Sender: TObject);
...
procedure TFQRGLAgeTP.BNouvRechClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.DLRuptNeedData(var MoreData: Boolean);
var TotRupt          : Array[0..13] of Double ;
...
procedure TFQRGLAgeTP.BRuptBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=(CritEdt.Rupture<>rRien) ;
end;

procedure TFQRGLAgeTP.FTypeParClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.FSansRuptClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.FRupturesClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLAgeTP.BPIEDTPBeforePrint(var PrintBand: Boolean;  var Quoi: String);
begin
  inherited;
...
procedure TFQRGLAgeTP.BENTETETPBeforePrint(var PrintBand: Boolean; var Quoi: String);
Var St : String ;
begin
  inherited;
