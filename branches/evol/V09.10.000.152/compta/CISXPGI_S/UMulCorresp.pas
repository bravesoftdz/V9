...
var
  tbcharger: TADOTable;
begin
  tbcharger := DMImport.GzImpCorresp;
  ...
  inherited;
    ...
    begin
                 ...
                 try
                   ...
                   begin
                        Open;
                        if Locate('Ident;Tablename;Profile',VarArrayOf([Q.FindField('Ident').asinteger, Q.FindField('Tablename').AsString, Q.FindField('Profile').Asstring]), [loCaseInsensitive]) then
                        begin
...
begin
  inherited;
    TCorr := TTable.create(Application);
    TCorr.TableName := DMImport.GzImpCorresp.TableName;
    TCorr.DatabaseName := DMImport.DBGlobal.Name;
    ...
    Finally
        ...
        begin
             ...
             begin
                  if Active then Close;
                  Open;
                  if FindKey([FCor^.Ident, FCor^.Tablename, FCor^.Profile]) then
                  begin
...
begin
            ...
            Q1.SQL.Add ('SELECT Max (Ident) Identification from '+DMImport.GzImpCorresp.TableName+
            ' Where Profile="'+ CorrespDlg.Profile.Text+'"');
            try Q1.Open; except Q1.free; exit; end;
            if not Q1.EOF then
...
begin
		  ...
		  AStream.Free;
                  //TMemoField(ABlobField).SaveToFile (CurrentDossier+'xxxxx.txt');
                  Post;
                  Close;
                  ScriptCorr.Destroy;
             end;
