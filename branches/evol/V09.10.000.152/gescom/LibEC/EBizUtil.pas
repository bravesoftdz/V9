...
{$ENDIF}
function ChampType(Field : String) : String;
...
begin
  result := '';
  PrefX := Copy(Field, 1, Pos('_', Field)-1);
  iTable := PrefixeToNum(PrefX);

  for iChamp := 1 to High(V_PGI.DeChamps[iTable]) do
   if V_PGI.DEChamps[iTable,iChamp].Nom = Field then
   begin
     result := V_PGI.DEChamps[iTable,iChamp].tipe;
     break;
   end;
end;
