...
begin
  ...
  StCondi := TraduitPourLigne(stWhere);
  if Stcondi = '' then Stcondi := ' WHERE  GL_QTERESTE <> 0' Else Stcondi := Stcondi + ' AND GL_QTERESTE <> 0';
  ...
  SQl := 'INSERT INTO BTTMPMOUVSTK '+
        ...
        'FROM BTLISTMSTOCK ' + StCondi;
  //
  ExecuteSql(Sql);
  // Chargement des Détail d'ouvrages
  stCondi := TraduitPourOuvrage(stWhere);
