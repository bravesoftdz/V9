...
{$ELSE}
     MainEagl,
{$ENDIF}
     AGLUtilOLE, M3FP, FileCtrl, {$IFNDEF AGL570}PGIEnv, {$ENDIF}extctrls,
...
{$IFNDEF AGL570}
		 if V_PGI_ENV.ModeFonc <> 'MONO' then
     		strReq := strReq + 'AND (ADE_DOSSIER="$STD" OR ADE_DOSSIER="$DAT" OR ADE_DOSSIER="' + V_PGI_ENV.NoDossier + '") ';
{$ELSE}
...
{$ELSE}
{$IFNDEF EAGLCLIENT}
          THEdit (GetControl ('EPATHRESULT')).Text    := V_PGI_Env.PathDos;
{$ELSE}
...
{$IFNDEF AGL570}
     if sOrigine = V_PGI_ENV.NoDossier then
         sNomFic := V_PGI_ENV.PathDos + '\'+ snomfic
{$ELSE}
