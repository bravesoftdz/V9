...
begin
  ...
  begin
    ...
    iTable := TableToNum ( stTable ) ;
    stPrefixe := TableToPrefixe (stTable);
    iChamp := ChampToNum (stPrefixe + '_' + TobOrigine.GetNomChamp (iIndChamp));
    if (iChamp <> -1) and (V_PGI.DEChamps[iTable,iChamp].tipe = 'DATE') then
    ...
    else if (iChamp <> -1) and (V_PGI.DEChamps[iTable,iChamp].tipe = 'DOUBLE') then
    ...
    else if (iChamp <> -1) and (V_PGI.DEChamps[iTable,iChamp].tipe = 'INTEGER') then
