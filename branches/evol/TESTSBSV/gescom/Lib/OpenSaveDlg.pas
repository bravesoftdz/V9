...
iTableLigne := PrefixeToNum('LI');
for iChamp := Low(V_PGI.DEChamps[iTableLigne]) to High(V_PGI.DEChamps[iTableLigne]) do
  begin
  s := V_PGI.DEChamps[iTableLigne, iChamp].tipe;
  Delete(s, 1, 8); Delete(S, Length(s), 1);
  if V_PGI.DEChamps[iTableLigne, iChamp].Nom = FCCode then ed_code.MaxLength := StrToIntDef(s,17);
  if V_PGI.DEChamps[iTableLigne, iChamp].Nom = FCLib then ed_lib.MaxLength := StrToIntDef(s,17);
  end;
end;
