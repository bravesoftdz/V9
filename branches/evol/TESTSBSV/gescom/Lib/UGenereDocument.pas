  ...
  begin
    ...
    begin
      ...
      begin
         ...
         begin
           if (ExecuteSQL('UPDATE PIECE SET GP_VIVANTE="-",GP_DEVENIRPIECE="'+encoderefPiece(TOBPiece)+'"') <= 0) then exit;
         end else
         begin
           if (ExecuteSQL('UPDATE PIECE SET GP_DEVENIRPIECE="'+encoderefPiece(TOBPiece)+'"') <=0) then exit;
         end;
      end else
      begin
        if (ExecuteSQL('UPDATE PIECE SET GP_VIVANTE="-",GP_DEVENIRPIECE="'+encoderefPiece(TOBPiece)+'"') <=0) then exit;
...
begin
  ...
  begin
    ...
    SQLBIS := SQLBIS + 'SELECT GL_ARTICLE AS REFARTICLE FROM LIGNE '+
                      ...
                      'GL_NUMERO='+TOBPiecePrec.detail[II].getString('NUMERO')+ ' AND '+
                      'GL_INDICEG='+TOBPiecePrec.detail[II].getString('INDICEG');
    SQLBIS := SQLBIS + ' UNION ';
    SQLBIS := SQLBIS + 'SELECT BLO_ARTICLE AS REFARTICLE FROM LIGNEOUV '+
                      ...
                      'BLO_NUMERO='+TOBPiecePrec.detail[II].getString('NUMERO')+ ' AND '+
                      'BLO_INDICEG='+TOBPiecePrec.detail[II].getString('INDICEG');
  end;
