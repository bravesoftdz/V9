...
uses
  ...
  FactSpec, AffaireUtil, BTPUtil, UgenereDocument,Urapport,forms
  ;
...
implementation
uses FactComm,FactTOB,FactUtil,FactArticle,ENt1,UtilTOBPiece;
...
begin
  ...
  try
    ...
    TOBLIGNEs.ClearDetail;
    //
    if IMPBSV.Atraite then IMPBSV.finalise; // traietement permettant de regrouper les documents en fonction du type receptionné pour générer la pièce  (ex : reception de CDE FOU --> génération de la réception)
    //
  finally
  ...
  TRY
    ...
    begin
      ...
      begin
        ...
        if (TOBP.GetString('GP_AFFAIRE')='') and (TOBL.GetString('GL_AFFAIRE')<>'') then
        begin
...
begin
  ...
  DateDoc := TOBEntete.GetDateTime('B10_DATEDOC');
...
begin
  ...
  TOBLigne.SetString('GL_REFARTSAISIE',TOBSCAN.GetString(''));
...
//
  ...
  TobLigne.PutValue('GL_PUHTDEV', TOBSCAN.GetValue('B11_PRIX'));
...
begin
  ...
  if (TOBL.GetString('GL_NATUREPIECEG')='FF') then TOBL.SetDouble('GL_PUHTDEV',TOBSCAN.GetDouble('B11_PRIX'));
end;
