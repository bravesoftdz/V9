...
--------------------------------------------------------------------------------------
 6.2X.xxx.xxx    04/11/04   JP   Création de l'unité  : Graph de la fiche de suivi des
                                 portefeuilles, centré sur les rendements de cer derniers
 8.00.001.018    05/06/07   JP   FQ 10469 : désactvation des filtres et des boutons
--------------------------------------------------------------------------------------}
unit TRGRAPHOPCVMREND_TOF;
...
uses
  ...
  FE_Main,
  {$ENDIF}
  SysUtils, UTob, HCtrls;

type

  TOF_TRGRAPHOPCVMREND = class (TOF)
...
procedure TRLanceFiche_GraphOpcvmRend(Arguments : string);

implementation

uses
  UObjFiltres, HTB97, GRS1, Series, GraphUtil, Graphics, TeEngine, Chart;

{---------------------------------------------------------------------------------------}
procedure TRLanceFiche_GraphOpcvmRend(Arguments : string);
{---------------------------------------------------------------------------------------}
begin
  AglLanceFiche('TR', 'TRGRAPHOPCVMREND', '', '', Arguments);
end;

{---------------------------------------------------------------------------------------}
procedure TOF_TRGRAPHOPCVMREND.OnUpdate ;
{---------------------------------------------------------------------------------------}
var
  n : Integer;
  F : TFGRS1;
...
begin
  ...
  try
    ...
    F := TFGRS1(Ecran);
    TstTitres.Add ('Courbes des plus-values par portefeuille');
...
{---------------------------------------------------------------------------------------}
procedure TOF_TRGRAPHOPCVMREND.OnArgument (S : String ) ;
{---------------------------------------------------------------------------------------}
begin
  inherited;
...
{---------------------------------------------------------------------------------------}
procedure TOF_TRGRAPHOPCVMREND.DessineLigne(Sender : TObject);
{---------------------------------------------------------------------------------------}
var
  YPosition  : Longint;
  Ecart, Inc : Double;
...
initialization
  RegisterClasses([TOF_TRGRAPHOPCVMREND]);
