...
{$ENDIF}
    ...
    comctrls, extctrls,
    UTOM, UTOB,
    HmsgBox, Hctrls, Ent1, HEnt1, SaisUtil;
...
begin
  inherited;
      ...
      D_MONNAIEIN  := TDBCheckBox(GetControl('D_MONNAIEIN'));
      if D_MONNAIEIN <> nil then
         D_MONNAIEIN.OnClick := MonnaieInClick;
      D_FONGIBLE  := TDBCheckBox(GetControl('D_FONGIBLE'));
      D_CPTLETTRDEBIT    := THDBEdit(GetControl('D_CPTLETTRDEBIT'));
      D_CPTLETTRCREDIT   := THDBEdit(GetControl('D_CPTLETTRCREDIT'));
      D_CPTPROVDEBIT     := THDBEdit(GetControl('D_CPTPROVDEBIT'));
      D_CPTPROVCREDIT    := THDBEdit(GetControl('D_CPTPROVCREDIT'));
      ...
      D_QUOTITE          := THDBEdit(GetControl('D_QUOTITE'));
      D_DECIMALE         := THDBSpinEdit(GetControl('D_DECIMALE'));
      D_PARITEEURO       := THDBEdit(GetControl('D_PARITEEURO'));
...
begin
     ...
     AGLLanceFiche('YY','CHANCEL',GetField('D_DEVISE'),'','ACTION=MODIFICATION;SUREURO=FALSE');
...
begin
     ...
     AGLLanceFiche('YY','CHANCEL',GetField('D_DEVISE'),'','ACTION=MODIFICATION;SUREURO=TRUE');
...
//            FicheChancel(D_DEVISE.Text,False,0,taCreat,True) ;
            AGLLanceFiche('YY','CHANCEL',GetField('D_DEVISE'),'','ACTION=CREATION;SUREURO=TRUE');
...
begin
  ...
  H_Devise  := THDBValComboBox(GetControl('H_Devise'));
