...
uses
  ...
  CpteUtil, EdtLegal, CritEDT, HQry,UtilEdt1, QRRupt, HTB97, HPanel, UiUtil ;

procedure TresoLumiere ( Previ : boolean ) ;

type
  TFReportTreso = class(TFQR)
    HLabel3: THLabel;
    FTypeGL: TRadioGroup;
    HLabel9: THLabel;
    ...
    procedure BENTETETPBeforePrint(var PrintBand: Boolean;
      var Quoi: String);
  private
    { Déclarations privées }
    LibTPEnCours                   : String ;
    Affiche,Previ                  : Boolean ;
    LMdp, LRupt                    : TStringList ;
    TotEdt, TotAux, TotMdp,TotTP   : TabTot ;
    TabDate                        : TTabDate4 ;
    Qr1T_AUXILIAIRE, Qr1T_LIBELLE,
    Qr1T_CORRESP1,Qr1T_CORRESP2    : TStringField ;
    QR2E_MODEPAIE                  : TStringField ;
    QR2E_DATEECHEANCE,QR2E_DATECOMPTABLE : TDateTimeField ;
    QR2COUVERTURE,
...
Function TrouveLibRubT(St : String ; OkLib : Boolean) : String ;
Var StLib : String ;
    QAux : TQuery ;
begin
StLib:='' ;
If St<>'' then
  BEGIN
  QAux:=OpenSQL('SELECT YX_LIBELLE FROM CHOIXEXT WHERE YX_TYPE="CLG" AND YX_ABREGE="'+St+'" ',TRUE) ;
...
Function TFReportTreso.QuoiCpt(i : Integer) : String ;
BEGIN
Case CritEdt.GlV.TypePar of
  0 : BEGIN
      Case i Of
        0 : Result:=Q.FindField('Q1GENERAL').AsString+' '+Q.FindField('G_LIBELLE').AsString+'#'+' '+'@'+'1' ;
        end ;
      END ;
  1 : BEGIN
      Case i Of
...
Procedure TFReportTreso.BalVentZoom(Quoi : String) ;
Var Lp,i: Integer ;
BEGIN
...
procedure TresoLumiere ( Previ : boolean ) ;
var QR : TFReportTreso ;
    Edition : TEdition ;
    PP : THPanel ;
BEGIN
PP:=FindInsidePanel ;
QR:=TFReportTreso.Create(Application) ;
Edition.Etat:=etBalVen ;
QR.Previ:=Previ ; 
QR.QRP.QRPrinter.OnSynZoom:=QR.BalVentZoom ;
QR.InitType (nbAux,neGlV,msRien,'QRBALVENTP','',TRUE,FALSE,FALSE,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
procedure TFReportTreso.FinirPrint ;
Var SOlde : Double ;
BEGIN
Inherited ;
QEcr.Close ;
...
procedure TFReportTreso.InitDivers ;
BEGIN
Inherited ;
...
Fillchar(TotTP,SizeOf(TotTP),#0) ;
END ;

Procedure genereSQLTreso(NE : TNatureEtat ; NET : TNatureEtatTiers ; Var CritEdt : TCritEdt ; Q : TQuery ; OkTP : Boolean) ;
Var St,StSup : String ;
BEGIN
Q.Close ; Q.SQL.Clear ;
Q.SQL.Add(' select e_naturetreso AS Q1NATURETRESO, e_general AS Q1GENERAL, G_LIBELLE from ecriture Q') ;
Q.SQL.Add(' left join journal on e_journal=j_journal ') ;
Case CritEdt.GlV.TypeGL Of
  0 : Q.SQL.Add(' left join TIERS on e_contrepartieAux=t_auxiliaire ') ;
  1 : Q.SQL.Add(' left join TIERS on e_AUXILIAIRE=t_auxiliaire ') ;
  END ;
Q.SQL.Add(' Left join choixext on e_NAtureTreso=yx_abrege ') ;
Q.SQL.Add(' Left join GENERAUX on e_GENERAL=G_GENERAL ') ;
Case CritEdt.GlV.TypeGL Of
  0 : BEGIN
      Q.SQL.Add(' where e_naturetreso<>"" and e_auxiliaire="" and yx_type="CLG" ') ;
      Q.SQL.Add(' and exists(select * from ecriture where E_numeropiece=Q.E_numeropiece and E_AUXILIAIRE<>"" and E_etatlettrage="AL") ') ;
      END ;
  1 : Q.SQL.Add(' where e_naturetreso<>"" and j_naturejal="BQE" and e_auxiliaire<>"" and yx_type="CLA" ') ;
  END ;
{ Construction de la clause Where de la SQL }
Q.SQL.Add(' AND T_AUXILIAIRE <> "'+w_w+'" ') ;
...
Q.SQL.Add(' group by e_naturetreso,e_general, G_LIBELLE ') ;
ChangeSQL(Q) ; Q.Open ;
END ;

Procedure genereSQLSubTreso(NE : TNatureEtat ; NET : TNatureEtatTiers ; Var CritEdt : TCritEdt ; Q : TQuery ; OkTP : Boolean) ;
Var St,StSup : String ;
BEGIN
Q.Close ; Q.SQL.Clear ;
Q.SQL.Add('Select E_AUXILIAIRE, E_MODEPAIE, E_DATECOMPTABLE, E_NUMEROPIECE, E_NUMLIGNE, E_GENERAL, ') ;
Q.SQL.Add('E_VALIDE, E_REFINTERNE, E_JOURNAL, E_DATEECHEANCE, E_NUMECHE, E_EXERCICE,E_NATURETRESO, ') ;
//      Q.SQL.Add(' left join journal on e_journal=j_journal ') ;
Case CritEdt.Monnaie of
  ...
  2 : BEGIN Q.SQL.Add(' E_DEBITEURO as DEBIT, E_CREDITEURO as CREDIT, E_COUVERTUREEURO as COUVERTURE ') ; END ;
 end ;
...
Q.SQL.Add(' left join journal on e_journal=j_journal ') ;
Q.SQL.Add(' Left join choixext on e_NAtureTreso=yx_abrege ') ;
//      Q.SQL.Add(' where e_naturetreso<>"" and j_naturejal="ACH" and e_auxiliaire="" and yx_type="CLG" ') ;
Q.SQL.Add('Where E_NATURETRESO=:Q1NATURETRESO  AND E_GENERAL=:Q1GENERAL ') ;
(*
Q.SQL.Add(' And e_naturetreso<>"" and e_auxiliaire="" and yx_type="CLG" ') ;
Q.SQL.Add(' and exists(select * from ecriture where E_numeropiece=Q.E_numeropiece and E_AUXILIAIRE<>"" and E_etatlettrage="AL") ') ;
*)
Case CritEdt.GlV.TypeGL Of
  0 : BEGIN
      ...
      Q.SQL.Add(' and exists(select * from ecriture where E_numeropiece=Q.E_numeropiece and E_AUXILIAIRE<>"" and E_etatlettrage="AL") ') ;
      END ;
  1 : Q.SQL.Add(' and e_naturetreso<>"" and j_naturejal="BQE" and e_auxiliaire<>"" and yx_type="CLA" ') ;
  END ;
{ Construction de la clause Where de la SQL }
Q.SQL.Add(' AND T_AUXILIAIRE <> "'+w_w+'" ') ;
if CritEdt.Joker then Q.SQL.Add(' AND T_AUXILIAIRE like "'+TraduitJoker(CritEdt.Cpt1)+'" ') Else
...
if (OkV2 and (V_PGI.Confidentiel<>'1')) then Q.SQL.Add(' AND T_CONFIDENTIEL<>"1" ') ;
{ Construction de la clause Order By de la SQL }
Q.SQL.Add(' Order By E_DATEECHEANCE, E_NUMEROPIECE, E_NUMLIGNE, E_NUMECHE ') ;
ChangeSQL(Q) ; Q.Open ;
END ;

procedure TFReportTreso.GenereSQL ;
{ Construction de la requête SQL en fonction du multicritère }
BEGIN
Inherited ;
GenereSQLTreso(neBal,Ventile,CritEdt,Q,FALSE) ;
GenereSQLSub ;
END ;

procedure TFReportTreso.GenereSQLSub ;
{ Construction de la requête SQL en fonction du multicritère }
BEGIN
Inherited ;
GenereSQLSUBTreso(neBal,Ventile,CritEdt,QEcr,FALSE) ;
END ;

procedure TFReportTreso.RenseigneCritere ;
...
PositionneFourchetteST(FTP1,FTP2,St11,St22) ; RTP1.Caption:=St11 ; RTP2.Caption:=St22 ;
END ;

procedure TFReportTreso.ChoixEdition ;
BEGIN
Inherited ;
...
procedure TFReportTreso.FColl1Change(Sender: TObject);
...
procedure TFReportTreso.RecupCritEdt ;
...
With CritEdt Do
  ...
  if SJoker Then
     ...
     END ;
  GlV.TP1:=FTP1.Text ; GlV.TP2:=FTP2.Text ;
...
function TFReportTreso.CritOk : Boolean ;
BEGIN
...
Procedure TFReportTreso.Calculs(var Report1,Report2 : TReport; i : integer; Bool1, Bool2 : Boolean ) ;
...
procedure TFReportTreso.SaisieDates ;
Var i : byte ;
...
procedure TFReportTreso.FormShow(Sender: TObject);
begin
HelpContext:=7556010 ;
...
FTypeGL.ItemIndex:=Ord(Not Previ) ; 
end;

procedure TFReportTreso.BSubDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
    OkCalcul : Boolean ;
begin
Inherited ;
...
If OkCalcul then
   ...
   Case CritEdt.GlV.TypeGL of
     0 : BEGIN
         ...
         if (QR2E_DateCOMPTABLE.AsDateTime>CritEdt.Date1) And (QR2E_DateCOMPTABLE.AsDateTime<=TabDate[1])
            then Calculs(MReport[2], MReport[7], 4, PrintBand, True) ;
         if (QR2E_DateCOMPTABLE.AsDateTime>TabDate[1]) And (QR2E_DateCOMPTABLE.AsDateTime<=TabDate[2])
            then Calculs(MReport[3], MReport[7], 3, PrintBand, True) ;
         if (QR2E_DateCOMPTABLE.AsDateTime>TabDate[2]) And (QR2E_DateCOMPTABLE.AsDateTime<=TabDate[3])
            then Calculs(MReport[4], MReport[7], 2, PrintBand, True) ;
         if (QR2E_DateCOMPTABLE.AsDateTime>TabDate[3]) And (QR2E_DateCOMPTABLE.AsDateTime<=TabDate[4])
...
procedure TFReportTreso.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var LeTot : TabTot ;
begin
  inherited;
(*
...
procedure TFReportTreso.BMdpBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
Procedure AlimTotTP(Var TotAux,TotTP : TabTot ; Var CritEdt : TCritEdt) ;
Var i : Integer ;
BEGIN
For i:=0 To 6 Do
  BEGIN
  TotTP[i].TotDebit:= Arrondi(TotTP[i].TotDebit+TotAux[i].TotDebit, CritEdt.Decimale) ;
  TotTP[i].TotCredit:= Arrondi(TotTP[i].TotCredit+TotAux[i].TotCredit, CritEdt.Decimale) ;
  END ;
END ;

procedure TFReportTreso.BFCompteAuxBeforePrint(var PrintBand: Boolean;  var Quoi: string);
...
If PrintBand then
   ...
   AlimTotTP(TotAux,TotTP,CritEdt) ;
...
procedure TFReportTreso.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFReportTreso.FTypeGLClick(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFReportTreso.FDateCompta1Exit(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFReportTreso.FChoixEcartClick(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.FP5Exit(Sender: TObject);
Var St : String ;
    A  : Char   ;
begin
  inherited;
...
procedure TFReportTreso.tabSupEnter(Sender: TObject);
begin
  inherited;
FPeriodicite.SetFocus ;
end;

procedure TFReportTreso.FTypeParClick(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.FPeriodiciteChange(Sender: TObject);
Var i : Integer ;
    TabD : TTabDate8 ;
begin
  inherited;
...
procedure TFReportTreso.TOPREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFReportTreso.BOTTOMREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
    TB : TabTot ;
    i : Integer ;
begin
...
procedure TFReportTreso.QEcrAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFReportTreso.FCpte1Change(Sender: TObject);
//Var AvecJokerS : Boolean ;
begin
  inherited;
...
procedure TFReportTreso.FNatureCptChange(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.BNouvRechClick(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.DLRuptNeedData(var MoreData: Boolean);
var TotRupt          : Array[0..13] of Double ;
...
procedure TFReportTreso.BRuptBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=(CritEdt.Rupture<>rRien) ;
end;

procedure TFReportTreso.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
If (CritEdt.GlV.TypePar=0) Then
  BEGIN
...
procedure TFReportTreso.FSansRuptClick(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.FRupturesClick(Sender: TObject);
begin
  inherited;
...
procedure TFReportTreso.BFCompteAuxAfterPrint(BandPrinted: Boolean);
begin
  inherited;
BFCompteAux.ForceNewPage:=FALSE ;
end;

procedure TFReportTreso.BPIEDTPBeforePrint(var PrintBand: Boolean;  var Quoi: String);
begin
  inherited;
LIBTP2.Caption:=TraduireMemoire(' TOTAL  :   ')+Q.FindField('Q1NATURETRESO').AsString+'   '+LibTPEnCours ;
AfficheNewMontant(CritEdt,TotTP[5],TPCum5) ;
AfficheNewMontant(CritEdt,TotTP[4],TPCum4) ;
AfficheNewMontant(CritEdt,TotTP[3],TPCum3) ;
AfficheNewMontant(CritEdt,TotTP[2],TPCum2) ;
AfficheNewMontant(CritEdt,TotTP[1],TPCum1) ;
AfficheNewMontant(CritEdt,TotTP[0],TPCum0)  ;
AfficheNewMontant(CritEdt,TotTP[6],TPTotal) ;
Fillchar(TotTP,SizeOf(TotTP),#0) ;
end;

procedure TFReportTreso.BENTETETPBeforePrint(var PrintBand: Boolean; var Quoi: String);
Var St,StLib : String ;
begin
  inherited;
St:=Q.FindField('Q1NATURETRESO').AsString ;
LibTPEnCours:=TrouveLibRubT(St,TRUE) ;
LIBTP1.Caption:=St+'   '+LibTPEnCours ;
end;
