...
procedure LanceAssistRSISCO ;
procedure MajNomFicSISCO(StFichier : String) ;

type
  TFAssistRSISCO = class(TFAssist)
    Welcome: TTabSheet;
    HLabel1: THLabel;
    ...
    Sauve: TSaveDialog;
    HLabel15: THLabel;
    BJalSISCO: TToolbarButton97;
    BFicSISCO: TToolbarButton97;
    ...
    procedure BJalSISCOClick(Sender: TObject);
    procedure BFicSISCOClick(Sender: TObject);
...
procedure LanceAssistRSISCO ;
Var X : TFAssistRSISCO ;
BEGIN
// GP le 02/09/98 if Not BlocageMonoPoste(True) then Exit ;
SourisSablier ;
X:=TFAssistRSISCO.Create(Application) ;
 Try
  X.ShowModal ;
 Finally
  X.Free ;
...
procedure TFAssistRSISCO.RestoreButton(b : TToolbarButton97) ;
BEGIN
//b.Restore ;
END ;

function TFAssistRSISCO.FirstPage : TTabSheet  ;
var i : Integer ;
BEGIN
//Result := Inherited FirstPage ;
If RecupSISCO Then result := PNomFic Else
  If RecupSERVANT Then result := Welcome ; // positionnement sur la première étape non terminée
i := 1 ; while Steps[i] do Inc(i) ;
if i>1 then result := TTabSheet(P.Pages[i-1]) ;
END ;

procedure TFAssistRSISCO.ParamLaSoc(p : integer) ;
Var b : Boolean ;
...
b := (p>=5) ;
{$IFDEF SPEC302}
FicheSocieteSISCO(taModif,p,b,FileName.text) ;
{$ELSE}
sSeule:='' ; sVirer:='' ;
Case p of
...
procedure TFAssistRSISCO.bSoc1Click(Sender: TObject);
begin
ParamLaSoc(0) ;
RestoreButton(bSoc1) ;
end;

procedure TFAssistRSISCO.bSoc5Click(Sender: TObject);
begin
ParamLaSoc(1) ;
RestoreButton(bSoc5) ;
end;

procedure TFAssistRSISCO.bSoc3Click(Sender: TObject);
begin
ParamFourchetteImport(FileName.Text) ; // fourchettes
RestoreButton(bSoc3) ;
end;

procedure TFAssistRSISCO.bSoc4Click(Sender: TObject);
begin
...
procedure TFAssistRSISCO.FormShow(Sender: TObject);
var i  : Integer ;
    mi : TMenuItem ;
...
Ferme(Q) ;
sLibelle:=GetParamSoc('SO_LIBELLE') ; sRecup:=GetParamSoc('SO_RECUPCPTA') ;
{$ENDIF}
...
If RecupSISCO Then Caption:=HM.Mess[0] Else
  If RecupSERVANT Then
    BEGIN
    Caption:=HM.Mess[1] ; PNomFic.TabVisible:=FALSE ; P.ActivePage:=Welcome ;
    END ;
...
If RecupSISCO Then
  ...
  Ferme(Q) ;
  END ;
BJalSISCO.Visible:=(FileName.text<>'') And FileExists(FileName.Text) And RecupSISCO ;
SourisNormale ;
end;

procedure TFAssistRSISCO.bUsersClick(Sender: TObject);
begin
FicheUser('') ;
RestoreButton(bUsers) ;
end;

procedure TFAssistRSISCO.bUserGroupsClick(Sender: TObject);
begin
FicheUserGrp ;
RestoreButton(bUserGroups) ;
end;

procedure TFAssistRSISCO.bEtabClick(Sender: TObject);
begin
FicheEtablissement_AGL(taCreat) ;
RestoreButton(bEtab) ;
end;

procedure TFAssistRSISCO.bExosClick(Sender: TObject);
begin
FicheExerciceSISCO('',taCreat,1) ;
RestoreButton(bExos) ;
end;

procedure TFAssistRSISCO.bDevisesClick(Sender: TObject);
begin
FicheDevise('',taModif,True) ;
RestoreButton(bDevises) ;
end;

procedure TFAssistRSISCO.StepMenuClick(Sender: TObject);
var i : Integer ;
begin
RestorePage ;
...
procedure TFAssistRSISCO.PChange(Sender: TObject);
begin
inherited;
...
procedure TFAssistRSISCO.ChangeStepBitmap ;
begin
if Steps[P.ActivePage.PageIndex + 1] then bStep.Glyph := ip2.Glyph
                                     else bStep.Glyph := ip1.Glyph ;
end ;

procedure TFAssistRSISCO.FinEtapeClick(Sender: TObject);
begin
...
procedure TFAssistRSISCO.bModePaiementClick(Sender: TObject);
begin
FicheModePaie_AGL('');
RestoreButton(bModePaiement) ;
end;

procedure TFAssistRSISCO.bModeReglClick(Sender: TObject);
begin
FicheRegle_AGL('',FALSE,taModif) ;
RestoreButton(bModeRegl) ;
end;

procedure TFAssistRSISCO.bAna2Click(Sender: TObject);
begin
ParamPlanAnal('') ;
RestoreButton(bAna2) ;
end;

procedure TFAssistRSISCO.bCpt2Click(Sender: TObject);
begin
FicheGene(Nil,'','',taCreatEnSerie,0) ;
RestoreButton(bCpt2) ;
end;

procedure TFAssistRSISCO.bTaxes1Click(Sender: TObject);
begin
ParamTable('TTREGIMETVA',taCreat,1165000,Nil) ;
RestoreButton(bTaxes1) ;
end;

procedure TFAssistRSISCO.bTaxes2Click(Sender: TObject);
begin
ParamTvaTpf(True) ;
RestoreButton(bTaxes2) ;
end;

procedure TFAssistRSISCO.bTaxes3Click(Sender: TObject);
begin
ParamTvaTpf(False) ;
RestoreButton(bTaxes3) ;
end;

procedure TFAssistRSISCO.bTiers9Click(Sender: TObject);
begin
FicheTiers(Nil,'','',taCreatEnSerie,1) ;
RestoreButton(bTiers9) ;
end;

procedure TFAssistRSISCO.bDivers1Click(Sender: TObject);
begin
FicheJournal(Nil,'','',taCreatEnSerie,0) ;
RestoreButton(bDivers1) ;
end;

procedure TFAssistRSISCO.bDivers2Click(Sender: TObject);
begin
YYLanceFiche_Souche('CPT','','ACTION=MODIFICATION;CPT'); //Compteurs
RestoreButton(bDivers2) ;
end;

procedure TFAssistRSISCO.SauverLaSoc ( AvecEtapes : boolean ) ;
var s : String ;
    i : Integer ;
BEGIN
s:='' ;
...
procedure TFAssistRSISCO.bFinClick(Sender: TObject);
var s : String ;
    i : Integer ;
begin
inherited;
...
procedure TFAssistRSISCO.bAna1Click(Sender: TObject);
begin
FicheAxeAna ('') ;
RestoreButton(bAna1) ;
end;

procedure TFAssistRSISCO.bSoc7Click(Sender: TObject);
begin
{$IFDEF SPEC302}
ParamLaSoc(4) ;
{$ELSE}
...
procedure TFAssistRSISCO.RechFileClick(Sender: TObject);
begin
  inherited;
...
Var Q : TQuery ;
BEGIN
If Not RecupSISCO Then Exit ;
If Trim(StFichier)<>'' Then
  BEGIN
  ExecuteSQL('DELETE FROM CHOIXCOD WHERE  CC_TYPE="SIS" AND CC_CODE="NOM"') ;
  Q:=OpenSQL('SELECT * FROM CHOIXCOD WHERE CC_TYPE="SIS" AND CC_CODE="NOM"',FALSE) ;
  Q.Insert ; InitNew(Q) ;
  Q.FindField('CC_TYPE').ASString:='SIS' ;
  Q.FindField('CC_CODE').ASString:='NOM' ;
  Q.FindField('CC_LIBRE').ASString:=StFichier ;
  Q.Post ;
  Ferme(Q) ;
  END ;
END ;

procedure TFAssistRSISCO.FileNameExit(Sender: TObject);
begin
  inherited;
MajNomFicSISCO(FileName.Text) ;
BJalSISCO.Visible:=(FileName.text<>'') And FileExists(FileName.Text) ;
end;

procedure TFAssistRSISCO.BJalSISCOClick(Sender: TObject);
begin
  inherited;
If RecupSISCO Then VisuJalSISCO(FileName.text) ;
end;

procedure TFAssistRSISCO.BFicSISCOClick(Sender: TObject);
begin
  inherited;
If RecupSISCO Then
  BEGIN
...
procedure TFAssistRSISCO.ToolbarButton971Click(Sender: TObject);
begin
  inherited;
If RecupSISCO Then AlimParDefautSISCO(FileName.Text) ;
end;
