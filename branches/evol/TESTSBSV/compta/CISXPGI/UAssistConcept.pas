...
type
  TFAssistExt = class(TFAssist)
    ...
    Label17: TLabel;
    DELIMLIB: TEdit;
    ChargementDelim: TToolbarButton97;
...
{$IFNDEF CISXPGI}
           ...
           begin
                DBODBC.LoginPrompt  := false;
                if pos ('Access', cbOrigine.Items[cbOrigine.ItemIndex ]) <> 0 then
                begin
                     DBODBC.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0;User ID='+EdUtilisateur.Text+';Data Source='+Chemin.text+
                ...
                begin
                //exemple  Provider=SQLNCLI.1;Password=cegid;User ID=sa;Initial Catalog=DB000000;Data Source=LYO-ENTRESSANGL\SQLEXPRESS;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=LYO-ENTRESSANGL;Use Encryption for Data=False;Tag with column collation when possible=False;MARS Connection=False;DataTypeCompatibility=80;Trust Server Certificate=False
                  if pos ('SQL Server', cbOrigine.Items[cbOrigine.ItemIndex ]) <> 0 then
                    DBODBC.ConnectionString := 'Provider=SQLNCLI.1;Data Source='+ cbAliasODBC.text+';User ID='+EdUtilisateur.Text+
                    ';Password='+EdMotdepasse.text+';Initial Catalog='+Chemin.text;
                 if Chemin.Text <> '' then
                  ...
                  DBODBC.Params.Add('DEFAULT DRIVER='+cbAliasODBC.text);
...
' Where Domaine="'+DomaineParam+'"', DMImport.Db);
...
if Q = nil then Q := TQuery.create(nil);
...
var
  B                         : TBookmark;
	...
	procedure InitTable;
	begin
    T.Connection :=  DMImport.DbDonnee as TADOConnection;
   ...
   with FScriptRequete do begin
    ...
    begin
          ...
          begin
						with Items[N] do
						begin
              if (FScriptRequete.TypeSortie = tsODBC) and (DataType = ftCurrency) then
               ...
               s:=s + ' ' + Name;
               s:=s + ' ' + AccessType(DataType, Size);
               s:=s + ',';
						end;
          end;
         s[Length(s)]:=')';
         try
            DMImport.DBDonnee.Execute('DROP TABLE ' + Code.Text);
         except end;

         try;
          DMImport.DBDonnee.Execute('CREATE TABLE ' + Code.Text + ' (' + S);
...
begin
  inherited;
      ...
      begin
           ...
           DriverName     := cbOrigine.Text;
           AssignSql(Z_SQL);
           if tbSauver.Locate('CLE', Code.text, [loCaseInsensitive]) then
           begin
              if tbSauver.FindField('CLEPAR').asstring  <> 'SQL'  then
              begin
              PGIBox('Au niveau Conception, un Script '+ NomFichier + ' existe déjà, changez le nom du code de votre requête', 'Conception') ;
              tbSauver.Close; tbSauver.free;
              exit;
              end;
              ...
              begin
                   ...
                   'Conception')<> mryes) then  begin tbSauver.Close; tbSauver.free; exit; end;
              end
              else
              begin
                   S := Format('Confirmez-vous les modifications de la requête %s ?', [Name]);
                   if (PGIAsk(S, 'Requête')<> mryes) then  begin tbSauver.Close; tbSauver.free; exit; end;
              end;
              tbSauver.Edit;
           end else tbSauver.Insert;
...
{$IFNDEF CISXPGI}
     tbSauver := TADOTable.Create(Application);
     tbSauver.Connection :=  DMImport.Db as TADOConnection;
     tbSauver.TableName := DMImport.GzImpDelim.TableName;
{$ELSE}
...
{$IFNDEF CISXPGI}
		if Active then
                Close;
		Open;
                if Locate('CLE', FScriptDel.Name, [loCaseInsensitive]) then
                begin
...
{$ENDIF}
	...
	begin
{$IFNDEF CISXPGI}
            if not Active then begin try Open; except Close; free; Exit; end; end;
            if not Locate('CLE', Codedelim, [loCaseInsensitive])
            then
            begin
                   if FichS = '' then
...
begin
  inherited;
  If (Code.Text <> '') and (Char(Code.Text[1]) in ['0'..'9']) Then
  begin
