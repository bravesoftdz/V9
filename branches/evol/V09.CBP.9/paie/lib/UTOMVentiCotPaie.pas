...
PT8 GGS 05/02/2007 V_80 Journal événements
...
begin
  ...
  begin
    ...
    begin
      PGIBox('Vous ne pouvez pas ventiler de rubrique de cotisation prédéfini CEGID', 'Accès refusé');
    ...
    begin
      PGIBox('Vous ne pouvez ventiler de rubrique de cotisation prédéfini Standard', 'Accès refusé');
      SetControlProperty('PVT_PREDEFINI', 'Value', 'DOS');
      SetField('PVT_PREDEFINI', 'DOS');
      MsgError := 1;
    end;
...
//PT8-3
      ...
      Trace.add (st);
      st := 'Création de la rubrique '+ GetField('PVT_RUBRIQUE');
      Trace.add (st);
      EnDupl := 'OUI';
...
//Fin PT8-3
      ...
      HShowMessage('5;Ventilation des cotisations :;La duplication est impossible, la ventilation existe déjà.;W;O;O;O;;;', '', '');
  end;
  mode := '';
end;
