...
Mots clefs ... : cCptWA
*****************************************************************}
class function cCptWA.si(test: boolean; alors, sinon: string): string;
begin
  if test then
    result := alors
  else
...
Créé le ...... : 10/05/2004
Modifié le ... :   /  /
Description .. : Affiche la trace des actions dans la trace
Mots clefs ... : cCptWA
*****************************************************************}
class procedure cCptWA.TraceExecution(st: string);
begin
trace.traceinformation('S1',st) ;
end;
{***********A.G.L.***********************************************
Auteur  ...... : Yann-Cyril PELUD
Créé le ...... : 10/05/2004
...
Mots clefs ... : cCptWA
*****************************************************************}
procedure cCptWA.setAServer2(valeur: THttpServer);
begin
  setAserver(valeur);
end;

procedure cCptWA.setAServer(valeur: THttpServer);
begin
...
Mots clefs ... : cCptWA
*****************************************************************}
function cCptWA.getaServer2: THttpServer;
begin
  result := getAServer();
end;

function cCptWA.getaServer: THttpServer;
begin
{$IFDEF EAGLCLIENT}
...
Mots clefs ... : cCptWA
*****************************************************************}
procedure cCptWA.setActionMessage(valeur: string);
begin
...
Mots clefs ... : cCptWA
*****************************************************************}
procedure cCptWA.setErrorMessage(valeur: string);
begin
  if (valeur <> '') and parle then
    PgiBox(valeur);
...
Mots clefs ... : cCptWA
*****************************************************************}
procedure cCptWA.setModeEagl2(valeur: boolean);
begin
  setModeEagl(valeur)
end;

procedure cCptWA.setModeEagl(valeur: boolean);
begin
{$IFDEF EAGLCLIENT}
  MSDEModeEAGL := true;
{$ELSE}
...
Mots clefs ... : cCptWA
*****************************************************************}
function cCptWA.getModeEagl2: boolean;
begin
  result := getModeEagl();
end;

function cCptWA.getModeEagl: boolean;
begin
{$IFDEF EAGLCLIENT}
  result := true;
{$ELSE}
...
Mots clefs ... :
*****************************************************************}
class procedure cCptWA.AjouteUneTob(tobDestination, tobSource: TOB; forceNom: string = '');
var
  tmpTOB: tob;
begin
...
Mots clefs ... :
*****************************************************************}
class function cCptWA.RecupereUneTob(tobSource: TOB; nom: string): TOB;
var
  i: integer;
begin
  result := nil;
...
Mots clefs ... : cCptWA
*****************************************************************}
procedure cCptWA.setRetour(var ResponseTOB: TOB; result: boolean; T1: tob; TT: HTStringList; st: string);
var
  i: integer;
  TmpTOB: TOB;
begin
...
Mots clefs ... : cCptWA
*****************************************************************}
function cCptWA.GetRetour(ResponseTOB: TOB): boolean;
var
  st: string;
begin
...
function cCptWA.GetRetour(ResponseTOB: TOB; T1: tob): boolean;
var
  st: string;
begin
...
function cCptWA.GetRetour(ResponseTOB: TOB; TT: HTStringList): boolean;
var
  st: string;
begin
...
function cCptWA.GetRetour(ResponseTOB: TOB; var st: string): boolean;
begin
  result := GetRetour(ResponseTOB, nil, nil, st);
end;

function cCptWA.GetRetour(ResponseTOB: TOB; T1: tob; TT: HTStringList; var st: string): boolean;
var
  i: integer;
  tmpTob: tob;
begin
...
Mots clefs ... : cCptWA
*****************************************************************}
function cCptWA.Request(Verb, Complement: string; QueryTob: TOb; StackId1, StackId2: string): TOB;
var
  it: TOb;
  sa, sf, St: string;
begin
if pos('.cCptWA',uppercase(Verb))>0 then
  Verb:=StringReplace(Verb,'.cCptWA','.CWA',[rfIgnoreCase]) ;
{$IFDEF EAGLSERVER}
...
{$IFDEF EAGLSERVER}
class procedure cCptWA.MessagesAuClient(Action, fonction, st: string);
var
  LaSession: tISession;
  Req: TOB;
begin
