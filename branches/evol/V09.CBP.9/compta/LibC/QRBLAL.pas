...
procedure BalanceLibreAna ;
procedure BalanceLibreAnaZoom(Crit : TCritEdt) ;
...
type
  ...
  private { Déclarations privées }
    ...
    Procedure BalLibAnaZoom(Quoi : String) ;
...
Procedure TFBLAL.BalLibAnaZoom(Quoi : String) ;
BEGIN
ZoomEdt(1,Quoi) ;
END ;
...
BEGIN
PP:=FindInsidePanel ;
QR:= TFBLAL.Create(Application) ;
Edition.Etat:=etBalLibEcr ;
QR.QRP.QRPrinter.OnSynZoom:=QR.BalLibAnaZoom ;
QR.IsLegal:=FALSE ;
QR.ChargeComboCptes ;
QR.InitType (nbLibAna,neBalLib,msGenAna,'QRBLAL','',False,False,False,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
try
 ...
 QR.IsLegal:=FALSE ;
 QR.ChargeComboCptes ;
 QR.InitType (nbLibAna,neBalLib,msGenAna,'QRBLAL','',False,True,False,Edition) ;
 finally
 QR.Free ;
 end ;
Screen.Cursor:=SyncrDefault ;
...
procedure TFBLAL.RenseigneCritere;
BEGIN
InHerited ;
...
Function TFBLAL.CritOk : Boolean ;
...
Function TFBLAL.ColonnesOK : Boolean ;
BEGIN
...
procedure TFBLAL.CalculDetail(var PrintBand : Boolean ; var Quoi : String) ;
...
procedure TFBLAL.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
inherited ;
...
procedure TFBLAL.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFBLAL.TOPREPORTBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
...
procedure TFBLAL.BOTTOMREPORTBeforePrint(var PrintBand: Boolean; var Quoi: string);
Var MReport : TabTRep ;
begin
  inherited;
...
procedure TFBLAL.FormShow(Sender: TObject);
begin
  inherited;
Floading:=FALSE ;
...
procedure TFBLAL.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFBLAL.Q1AfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFBLAL.FExerciceChange(Sender: TObject);
begin
FLoading:=TRUE ;
  inherited;
FLoading:=FALSE ;
...
procedure TFBLAL.FDateCompta1Change(Sender: TObject);
begin
  inherited;
If FLoading Then Exit ;
end;

procedure TFBLAL.FCpte3Change(Sender: TObject);
begin
...
procedure TFBLAL.FP1Exit(Sender: TObject);
Var St : String ;
    A  : Char   ;
begin
  inherited;
...
procedure TFBLAL.ChargeComboCptes ;
var Q : TQuery ;
BEGIN
...
Q:=OpenSQL('Select CO_CODE,CO_LIBELLE From COMMUN Where CO_TYPE="NAT" And CO_CODE Like "A%" ORDER BY CO_CODE',True) ;
While not Q.Eof do
  BEGIN
...
procedure TFBLAL.TypeChange(Sender: TObject);
var Num : byte ;
    Ok : boolean ;
    T : THValCombobox ;
    L1,L2 : TLabel ;
...
if not Ok then
  ...
  Case Num Of
    0 : C1.ZoomTable:=tzNatEcrA0 ;
    1 : C1.ZoomTable:=tzNatEcrA1 ;
    2 : C1.ZoomTable:=tzNatEcrA2 ;
    3 : C1.ZoomTable:=tzNatEcrA3 ;
    END ;
...
procedure TFBLAL.FPeriodiciteChange(Sender: TObject);
Var i : Integer ;
    TabD : TTabDate8 ;
begin
  inherited;
...
procedure TFBLAL.BFDetailBeforePrint(var PrintBand: Boolean;
  var Quoi: string);
begin
inherited ;
...
procedure TFBLAL.BSecondBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
if CritEdt.Composite and not Q1.Findfield('NT_NATURE').IsNull then CalculDetail(PrintBand,Quoi) else PrintBand:=False ;
end;

procedure TFBLAL.Q1BeforeOpen(DataSet: TDataSet);
begin
  inherited;
