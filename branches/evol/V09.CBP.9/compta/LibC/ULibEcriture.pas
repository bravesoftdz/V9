...
//   property    CompteAuto : string read GetCompteAuto ;
 ...
 // Gestion des modes de règlements et de paiement dans le TInfoEcriture (SBO 22/01/2007)
...
// Nouvelles fonctions de calcul des échéances s'appuiyant sur le TInfoEcriture
...
begin
  ...
  begin
    ...
    for j:=low(ExceptFields) to high(ExceptFields) do
	    if NomChamps=UpperCase(ExceptFields[j]) then begin GarderChamps:=false; break ; end ;
    if GarderChamps then result:= result+NomChamps+',';
  end; // for;
  if result = '' then
  begin
    if WithOutFrom then
...
begin
 ...
 vListe.Add('66;?caption?;Votre ecriture est incorrecte : établissement inexistant.... ! ;E;O;O;O;') ;
...
// ajout me fiche 19032 vListe.Add('85;?caption?;Ce bordereau est clôturé, seule la consultation est possible;W;O;O;O;');
 vListe.Add('85;?caption?;Cette période est clôturée, seule la consultation est possible;W;O;O;O;');
 ...
 vListe.Add('106;?caption?;L''écriture fait référence à la section "$$" qui n''existe pas. Veuillez corriger votre paramètrage;E;O;O;O;') ;
 vListe.Add('107;?caption?;Saisie multidevise impossible en mode libre;E;O;O;O;') ;
 vListe.Add('108;?caption?;La date d''échéance doit respecter la plage de saisie autorisée.;E;O;O;O;') ;
 vListe.Add('109;?caption?;Votre saisie comporte plusieurs établissements !.;E;O;O;O;') ;
 vListe.Add('110;?caption?;la condition de règlement du tiers est inconnue !' + '#10#13' +'Veuillez la renseigner dans l''onglet "Règlements" du paramétrage du compte auxiliaire.;E;O;O;O;') ;
 vListe.Add('111;?caption?;la date n''est pas modifiable, une écriture de la pièce est lettrée !.;E;O;O;O;') ;
 vListe.Add('112;?caption?;Vous ne pouvez pas saisir sur cette établissement !.;E;O;O;O;') ;
 vListe.Add('113;?caption?;Suppression impossible, la pièce a été modifiée !.;E;O;O;O;') ;
 vListe.Add('114;?caption?;Impossible d''enregistrer la pièce de suppression !.;E;O;O;O;') ;
end;
...
begin
 ...
 if RC_Error.RC_Error > vListe.Count then
  begin
   PGIInfo('Le numéro ' + intToStr(RC_Error.RC_Error) + ' est inconnu ', vStTitre ) ;
   exit ;
  end; // if
...
begin
 ...
 // SBO 22/01/2007 : La date de cloture périodique doit être cohérente sur une session de saisie
 // Faire un test en amont dans les traitements qui sont amenés à modifier cette date si besoin...
 {$IFDEF EAGLCLIENT}
...
begin
  ...
  if not lQReq.Eof then
    begin
    // comparaison avec tob deja chargée
...
begin
 ...
 lBoTiers := ( ( vInfo.Compte.IsCollectif and ( vInfo.GetString('T_LETTRABLE')='X' ) ) // tiers lettrable
             or ( vInfo.Compte.IsPointable and ( vInfo.GetString('G_NATUREGENE')<>'DIV' ) )                                           // général pointable
...
begin
 ...
 if result <> RC_PASERREUR then exit ;       // Correction LGE 29/01/2007 : saisie sur auxi fermés
...
Auteur  ...... : SBO
Créé le ...... : 27/11/2003
Modifié le ... : 26/10/2006
Description .. : 
Suite ........ : Permet la mise en place de traitements sur une ligne
Suite ........ : d'écriture comptable avant la maj en base de données.
Suite ........ : 
Suite ........ : Utilisation de V_PGI.IoError pour gestion des erreurs
Suite ........ : - LG - 26/10/2006 - mise à jour du parametre societe des 
...
{$IFNDEF TRESO}
  {JP 14/06/06 : FQ TRESO 10365 : Pour le moment on désactive la saisie bordereau, car il faut repenser
   ...
   JP 11/10/06 : Après modification du numéro de transaction en Tréso, je tente de réactivé la saisie Boredereau}
...
begin
  ...
  try
    ...
    // ============================================================
    // === Cas spécifique des divers lettrables : MONO ECHEANCE ===
    // ============================================================
    ...
    lTobTmp     := Tob.Create('CALCUL_ECHE', nil, -1) ;            // conteneur temporaire des échéances
    if vBoMono                                                     // Calcul du nombre d'échéances
      then lInNbEche := 1
    ...
    For lInEche := 1 to lInNbEche do
      ...
      if lTobEche.GetDateTime('E_DATEVALEUR')=iDate1900 then
        ...
        if vInfo.GetString('G_POINTABLE')='X'
          ...
          else lTobEche.PutValue('E_DATEVALEUR', lDtDepart ) ;  // Comptes lettrable  --> Date d'échéances
        end ;
    ...
    for lInEche := lTobTmp.Detail.count - 1 downto 0 do
      if lInEche = 0
        then vTobpiece.Detail[vIndex].Dupliquer( lTobTmp.Detail[0], True, True )  // sur Lgned de référence, copie des valeurs
...
begin
  ...
  // recherche du mode de règlement du compte
  if vInfo.LoadCompte( vTobEcr.GetValue('E_GENERAL') ) then
    ...
    //   --> Initialisation sur MR par défaut pour les compte divers lettrables ( FQ 16136 SBO 23/08/2005 )
    else if vInfo.GetString('G_NATUREGENE') <> 'DIV'
       then result := vInfo.GetString('G_MODEREGLE') ;
    end ;

  // si rien de renseigné dans le tiers, mode de règlement par défaut
  if result = '' then
...
begin
  ...
  // Détermination du mode de gestion ( avec ou sans éché...)
  vInfo.LoadCompte( vTobEcr.GetString('E_GENERAL') ) ;
  lBoAvecEche := // Auxi lettrable
                 ...
                 // Compte géné lettrable ou pointable
  ...
  if lBoAvecEche then
    begin
    // MAJ Indicateurs + init des champs associées si besoin
    ...
    // Calcul si les montants sont renseignés
...
begin
  ...
  // Arrondi à la quinzaine (Modif SBO)
...
begin
	...
	MCD := TMCD.GetMcd;
	if not mcd.loaded then mcd.WaitLoaded();
  Table := Mcd.GetTable(vTob.NomTable);

  if V_PGI.Debug then
    Trace.TraceInformation('CTOB', '##### CTobInsertDB =====> Tob nomTable : ' + vTob.NomTable + ', niveau : ' + IntToStr(vTob.niveau) );

  result  := True ;
  lStMess := '' ;

  lBoBlobModifie := False;
  if ( vTob.NumTable >= 0) and ( Table.NumChampBlob > 0 ) then
    begin

    lStNomBlob := (Table.GetField( Table.NumChampBlob ) as IFieldCOM).Name ;
    if vTob.IsFieldModified( lStNomBlob ) then
      lBoBlobModifie := True ;
    end;

  // Le blob est modifié on laisse la main à l'AGL
  if lBoBlobModifie then
    begin
    if vBoByNivel
      then begin
           if V_PGI.Debug then
             Trace.TraceInformation('CTOB', '##### CTobInsertDB =====> InsertDBByNivel' );
           if vBoMultiNiv
           ...
           if V_PGI.Debug then
             Trace.TraceInformation('CTOB', '##### CTobInsertDB =====> InsertDB' );
           result := vTob.InsertDB( nil ) ;
           end ;
    if not result then
      Trace.TraceInformation('CTOB', '##### CTobInsertDB =====> RESULT AGL = False !!!' ) ;
    end
  // Sinon on fait nous meme l'insert
    ...
    if lStMess <> '' then
      begin
      if V_PGI.Debug then
        Trace.TraceInformation('CTOB', lStMess );
      if ( V_PGI.SAV ) then
        PGIError( lStMess );
      end ;
