...
procedure DropVuesStandard ;
procedure DropEtatsStandard;
procedure DropListesStandard;
...
Uses HEnt1,HCtrls,MajTable,StdCtrls,ComCtrls,Forms,
    ...
    {$ENDIF}
     ...
     DBCtrls,ParamSoc, Dialogs,UTOB,Ent1,hMsgBox,utilpgi
  ,CbpMCD
  ,CbpEnumerator
      ;
...
BEGIN
  ...
  Table := mcd.GetTable(sNomtable);
  Result := Table.NumVersion ;
END ;
...
begin
  TRY
    ExecuteSQL('DELETE * FROM DEVUES WHERE DV_UTILISATEUR IN ("","000","CEG")')
  EXCEPT
  END;
end;
...
procedure DropListesStandard;
begin
  ExecuteSQLContOnExcept ('DELETE FROM LISTE WHERE LI_UTILISATEUR IN ("CEG","---")');
end;
...
var QQ : Tquery;
		TOBetats : TOB;
		indice : integer;
    TypeEtat,NatureEtat,CodeEtat : string;
begin
  TOBetats := TOB.Create ('LES ETATS',nil,-1);
  TRY
    QQ := OpenSql ('SELECT MO_CODE,MO_TYPE,MO_NATURE FROM MODELES WHERE MO_MODELE="X"',true,-1,'',true);
    TOBetats.LoadDetailDB('MODELES','','',QQ,false);
    ferme (QQ);
    for Indice := 0 to TOBetats.detail.count -1 do
    begin
      TypeEtat := TOBEtats.detail[Indice].getValue('MO_TYPE');
      NatureEtat := TOBEtats.detail[Indice].getValue('MO_NATURE');
      CodeEtat := TOBEtats.detail[Indice].getValue('MO_CODE');
      TRY
        ExecuteSQLContOnExcept('DELETE FROM MODELES WHERE MO_TYPE="'+TypeEtat+'" AND MO_NATURE="'+NatureEtat+'" AND MO_CODE="'+CodeEtat+'"');
        ExecuteSQLContOnExcept('DELETE FROM MODEDATA WHERE MD_CLE LIKE "'+TypeEtat+NatureEtat+CodeEtat+'%"');
      EXCEPT
      END;
    end;
  FINALLY
...
procedure MajBTMAJSTRUCTURES;
begin
	ExecuteSQLContOnExcept('DELETE FROM BTMAJSTRUCTURES');
	ExecuteSQLContOnExcept('INSERT INTO BTMAJSTRUCTURES (BTV_IDBTP, BTV_VERSIONBASEB, BTV_TYPEELT, BTV_NOMELT, BTV_WITHDATA, BTV_FORCE) '
                +'VALUES(0, "998.0", "VERSION", "","-","-")');
	ExecuteSQLContOnExcept('INSERT INTO BTMAJSTRUCTURES (BTV_IDBTP, BTV_VERSIONBASEB, BTV_TYPEELT, BTV_NOMELT, BTV_WITHDATA, BTV_FORCE) '
                +'VALUES(5000, "998.0", "TABLE", "BTMAJSTRUCTURES","X","X")');
end;
