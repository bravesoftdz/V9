...
uses
  ...
  Forms, Dialogs, Vierge, uTob,
  HSysMenu, HTB97, Menus, Grids,
  Hctrls, ExtCtrls, HPanel, uTom,
  ImgList, ComCtrls, StdCtrls,
  wCommuns, wTom, TntMenus, TntExtCtrls, TntGrids;
...
type
  ...
  private
    ...
    FCurrentRow,
    FCurrentCol,
    FDoControl, FDoRefresh,
    FDoUpdate, FLastError : Integer;
    FShowRoot,
    FInitialized,
    FTomEnabled,
    FConsultation         : Boolean;
    FDBListe              : String;
    FTreeTob, FTobFields,
    FTreeTobSaved,
    FTob2Delete,
    FTreeTobBeforeModif,
    FCurrentProcessedTob,
    FSysTob,
    FTobCaptions          : Tob;
    FTom                  : TTreeTom;
    FListe                : TWListe;
    ...
    FOnNewRecord,
    FOnBeforeRefresh,
    FOnCollapseNode       : TTreeGridEvent;
    ...
    FBtParamListe,
    FBtAddChildRow,
    FBtDeleteRow, FBtUndo,
    FBtSearch, FBtAddRow,
    FBtSelectAll,
    FBtSelectNode,
    FBtExpand, FBtLoupe   : TToolBarButton97;
    ...
    //GP_20071228_TS_GP14676
    procedure DrawTvImg(const Canvas: TCanvas; const X, Y: Integer; Ttti: TTreeTobTvImg);
    ...
    { Relatif TreeTob }
    //GP_20071217_TS_GP14539
    procedure GetTobFields(const AvertirCacheFormules: Boolean = True);
    ...
    function SynchronizeSqlFormule(const wField: TWFieldListe): Boolean;
    //GP_20071217_TS_GP14539 <<<
  ...
  public
    ...
    //GP_20071217_TS_GP14539
    procedure DisplayTreeTob(const AvertirCacheFormules: Boolean = True);
    procedure RefreshDBListe;
    //GP_20071217_TS_GP14539
    procedure RefreshTreeTob(const AvertirCacheFormules: Boolean = True);
    ...
    //GP_20071226_TS_GP14676 >>>
    { Impression }
    procedure Print(const Nature, Etat: String; const Apercu: Boolean = True; const Criteres: String = ''; const LanguePrinc: String = '');
    //GP_20071226_TS_GP14676 <<<
    { Get }
...
function ModernDrawing: Boolean;
begin
  Result := V_Pgi.Draw2003 or V_Pgi.DrawAero
end;
...
//GP_20071217_TS_GP14539 >>>
{ Traitement des champs de type SQL de la liste de paramétrage }
function TFFrameTreeTob.SynchronizeSqlFormule(const wField: TWFieldListe): Boolean;
var
  TobFormule: Tob;

  { compose le Where Sql de la formule en parcourant la TreeTob }
  function SetWhere(const T: Tob; var Sql: String): Boolean;
  var
    i: Integer;
    s, sValue, FieldName: String;
  begin
    Result := True;
    s := wField.SqlWhere;
    i := Low(wField.SqlValues);
    while Result and (i <= High(wField.SqlValues)) do
    begin
      FieldName := wField.SqlValues[i];
      Result := T.FieldExists(FieldName);
      if Result then
      begin
        case wGetSimpleTypeField(FieldName) of
          'D': begin
                 if T.GetDouble(FieldName) - Trunc(T.GetDouble(FieldName)) > 0 then
                   sValue := UsDateTime(T.GetDateTime(FieldName))
                 else
                   sValue := UsDateTime(Trunc(T.GetDateTime(FieldName)));
               end;
          'N': sValue := StrFPoint(T.GetDouble(FieldName));
        else { booléens, integer et chaînes }
          sValue := T.GetString(FieldName)
        end;
        s := StringReplace(s, '[' + FieldName + ']', sValue, [rfIgnoreCase, rfReplaceAll])
      end;
      Inc(i);
    end;
    ...
    begin
      ...
      for i := 0 to Pred(T.Detail.Count) do
        SetWhere(T.Detail[i], Sql);
    end
  end;

  { synchronisation des données TreeTob/Résultats formule }
  procedure SynchroTreeTob(const T: Tob);
  var
    iRang, i: Integer;

    procedure InitValueFormule(const Value: Variant);
    begin
      if T.FieldExists(wField.Name) then
        T.PutValue(wField.Name, Value)
      else
        T.AddChampSupValeur(wField.Name, Value, False)
    end;

  begin
    iRang := FindRangOnTobSorted(TobFormule, wField.SqlFields, TobToArrayValue(T, wField.SqlValues));
    if iRang <> -1 then
    begin
      if TobFormule.Detail[iRang].FieldExists(wField.Name) then
        InitValueFormule(TobFormule.Detail[iRang].GetValue(wField.Name))
      else { premier champ ramené }
        InitValueFormule(TobFormule.Detail[iRang].GetValeur(1000))
    end;
    for i := 0 to Pred(T.Detail.Count) do
      SynchroTreeTob(T.Detail[i])
  end;
...
var
  ...
  Where: String;
begin
  Result := Assigned(wField) and (wField.SqlWhere <> '');
  if Result then
  begin
    Where := '';
    { composition de la requête }
    if FShowRoot then
      Result := SetWhere(FTreeTob, Where)
    else
    begin
      i := 0;
      while Result and (i <= Pred(FTreeTob.Detail.Count)) do
      begin
        Result := SetWhere(FTreeTob.Detail[i], Where);
        Inc(i)
      end
    end;
    if Result then
    begin
      ...
      if wRight(Where, 4) = ' OR ' then
        Delete(Where, Length(Where) - 3, 4);
      { Chargement de la Tob }
      TobFormule := Tob.Create('_FormuleSql_', nil, -1);
      try
        if TobFormule.LoadDetailDbFromSql('_FormuleSql_', Format(wField.Sql, [Where])) then
        begin
          { Tri la Tob pour recherche dichotomique }
          TobFormule.Detail.Sort(wArrayToString(wField.SqlFields, ';'));
          { Synchronisation avec la TreeTob }
          if FShowRoot then
            SynchroTreeTob(FTreeTob)
          else
            for i := 0 to Pred(FTreeTob.Detail.Count) do
              SynchroTreeTob(FTreeTob.Detail[i]);
        end
      finally
        TobFormule.Free
      end
...
{ Remplissage d'une ligne de grille (iRow) via le TreeManagerItem }
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              //GP_20071217_TS_GP14539 >>>
              begin
                { optimisation en cache du libellé complet }
                if not T.FieldExists('TTF' + sField) or (T.GetString('TTFVALUE' + sField) <> T.GetString(sField)) then
                begin
                  T.AddChampSupValeur('TTF' + sField, RechDom(TT, T.GetString(sField), False), CSTString);
                  T.AddChampSupValeur('TTFVALUE' + sField, T.GetString(sField), CSTString);
                end;
                TreeGrid.CellValues[i, iRow] := T.GetString('TTF' + sField)
              end
              //GP_20071217_TS_GP14539 <<<
              else
...
//GP_20071217_TS_GP14539
procedure TFFrameTreeTob.DisplayTreeTob(const AvertirCacheFormules: Boolean = True);
var
  i, j, iPosRow: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      //GP_20071217_TS_GP14539
      GetTobFields(AvertirCacheFormules);
...
//GP_20071217_TS_GP14539 >>>
procedure TFFrameTreeTob.GetTobFields(const AvertirCacheFormules: Boolean = True);
var
  i, CptFields: Integer;
  wField: TWFieldListe;
  FormulesSql: TStringList;
begin
  FormulesSql := TStringList.Create();
  try
    { Optimisation : on stock la liste des formules SQL qui ne sont pas à retraiter }
    if not AvertirCacheFormules then
    begin
      for i := 1000 to 1000 + Pred(FTobFields.ChampsSup.Count) do
      begin
        wField := FListe.ChampsByName[VarToStr(FTobFields.GetValeur(i))];
        if Assigned(wField) and wField.isSql() then
          FormulesSql.Add(wField.Name)
      end
    end;
    ...
    begin
      wField := FListe.Champs[i];
      if wField.Visible then
      begin
        FTobFields.AddChampSupValeur('FIELD_' + IntToStr(CptFields), wField.Name);
        FTobCaptions.AddChampSupValeur(wField.Name, wField.Caption);
        Inc(CptFields);

        { Traitement des formules type SQL }
        if wField.isSql() and (FormulesSql.IndexOf(wField.Name) = -1) then
          SynchronizeSqlFormule(wField)
      end
    end
  finally
    FormulesSql.Free
...
//GP_20071228_TS_GP14676 >>>
{ Dessin de l'image du TreeView ------------------------------------------------ }
procedure TFFrameTreeTob.DrawTvImg(const Canvas: TCanvas; const X, Y: Integer; Ttti: TTreeTobTvImg);
begin
  { Type de pinceau }
  if ModernDrawing() then
...
var
  ...
  begin
    if IM.Expanded then
      Result := iif(V_Pgi.DrawAero, 7, iif(V_Pgi.Draw2003, 3, 1))
    else
      Result := iif(V_Pgi.DrawAero, 6, iif(V_Pgi.Draw2003, 2, 0))
  end;
...
//GP_20080627_TS_GP15252
  ...
  begin
//GP_20080627_TS_GP15252
    Result := TreeGrid.TwoColors and (TreeGrid.RowHeights[ARow] > 0) and (ACol >= Grid_FixedCols) and (ARow > TreeGrid.FixedRows)
  end;
  ...
  begin
    { Booléen }
    //GP_20071217_TS_GP14539
    if (TreeGrid.ColTypes[ACol] = 'B') and (TreeGrid.Cells[ACol, ARow] <> '') then
...
const
  COL_GRADXP1STATE1 = $001595EE;  //selected foncé
  COL_GRADXP2STATE1 = $007FD9F8;  //selected clair
  COL_GRADXP1STATE2 = $007ED1F9;  //not select foncé
  COL_GRADXP2STATE2 = $00DBFEFE;  //not select clair
  COL_GRADAERO1STATE1 = $0025A4F9;//$003C8CFB;//selected foncé
  COL_GRADAERO2STATE1 = $00A0D6FA;//$0064D3FE;//selected clair
  COL_GRADAERO1STATE2 = $001DD0FE;//$003FABFF;//not select foncé
  COL_GRADAERO2STATE2 = $00A2E9FB;//$007AE1FE;//not select clair
begin
  ...
  begin
    ...
    if V_Pgi.DrawAero then
    begin
      Canvas.Brush.Color := COL_GRADAERO2STATE1;
      Canvas.Pen.Color := COL_GRADAERO1STATE1;
    end
    else
    begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if not V_Pgi.DrawAero then
          for i := 0 to Pred(IM.CountImages) do
            if TreeGrid.GridLineWidth > 0 then
              DrawTvImg(Canvas, XOffSet + GetX(i + 1), GetY, IM.TreeImages[i]);
        { Dessin du + ou - }
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if V_Pgi.DrawAero then
          begin
            DrawVGradiant(Canvas, ARect, wEclaircirCouleur(TColor(iif(IsRowSelected, COL_GRADAERO2STATE1, COL_GRADAERO2STATE2)), 15),
                                         wEclaircirCouleur(TColor(iif(IsRowSelected, COL_GRADAERO1STATE1, COL_GRADAERO1STATE2)), 15));
            ARect.Top := ARect.Bottom;// + 1;
            ARect.Bottom := iTmp - iif(TreeGrid.RowHeights[ARow] mod 2 = 0, 0, 1);
            Canvas.Pen.Style := psClear;
            Canvas.Brush.Color := TColor(iif(IsRowSelected, COL_GRADAERO1STATE1, COL_GRADAERO1STATE2));
            Canvas.FillRect(ARect);
          end
          else
          begin
...
//GP_20080409_TS_GP14539
  FListe := TWListe.Create(FDBListe, Self, False, True);
end;

{ Recharge et réaffiche la tob  ------------------------------------------------ }
//GP_20071217_TS_GP14539
procedure TFFrameTreeTob.RefreshTreeTob(const AvertirCacheFormules: Boolean = True);
var
  Cancel: Boolean;
begin
  Cancel := False;
  ...
  begin
    ...
    try
      ...
      { Affichage }
      //GP_20071217_TS_GP14539
      DisplayTreeTob(AvertirCacheFormules);
...
begin
  if Assigned(FListe) then
  begin
    Field := FListe.ChampsByName[FieldName];
    Result := not Assigned(Field) or not Field.isSql()
  end
  else
    Result := True;
  Result := Result and not FConsultation and Assigned(FEditableFields) and ((FEditableFields.Count = 0) or (FEditableFields.IndexOf(FieldName) > -1));
end;
//GP_20071217_TS_GP14539 <<<
...
{ Permet de savoir si le noeud courant est étendu ---------------------------- }
function TFFrameTreeTob.GetExpanded: Boolean;
begin
  Result := NodeIsExpanded(CurrentItemManager)
end;

{ Permet de savoir si le noeud de la ligne "iRow" courante est étendu -------- }
...
begin
  ...
  begin
    ...
    begin
      TreeGrid.ValCombo.Height := TreeGrid.RowHeights[TreeGrid.Row] + iif(ModernDrawing(), 1, 0);
      TreeGrid.ValCombo.Width  := TreeGrid.ColWidths [TreeGrid.Col] + iif(ModernDrawing(), 1, 0);
    end;
...
var
  ...
  begin
    ...
    begin
      ...
      //GP_20071217_TS_GP14539 >>>
      if (FVisibilityFields.IndexOfName(FieldName) = -1) or StrToBool_(FVisibilityFields.Values[FieldName]) then
        TreeGrid.ColWidths[i] := FieldParams.Width
      else
        TreeGrid.ColWidths[i] := -1;
      //GP_20071217_TS_GP14539 <<<
      sField := FieldName;
      if (Pos('(', sField) = 1) and (Pos(')', sField) = Length(sField)) then
      begin
...
begin
  ...
  //GP_20071217_TS_GP14539
  FVisibilityFields       := TStringList.Create;
  InitCellFound();
  ...
  begin
    ...
    begin
      ...
      if Field.Tipe = 'RATE' then FRatedFields.Add(Field.Name)
    end;
  end;
...
begin
  ...
  if Assigned(FRatedFields) then
    FRatedFields.Free;
  //GP_20071217_TS_GP14539 >>>
  if Assigned(FVisibilityFields) then
    FVisibilityFields.Free;
  //GP_20071217_TS_GP14539 <<<
...
{ Ramène le nom du champ associé à la colonne ---------------------------------- }
...
{ Ramène l'index absolu d'une tob par rapport à son parent direct -------------- }
...
{ Valide la saisie en cours. Si DoTransactions, on gère la base de données ----- }
...
begin
  if iCol > -1 then
  begin
    FieldName := FieldCol[iCol];
    if FVisibilityFields.IndexOfName(FieldName) > -1 then
      FVisibilityFields.Values[FieldName] := BoolToStr_(Value)
    else
      FVisibilityFields.Add(FieldName + '=' + BoolToStr_(Value));

    if Value then
      TreeGrid.ColWidths[iCol] := FListe.ChampsByName[FieldName].Width
    else
...
//GP_20071226_TS_GP14676 >>>
{ Impression de la Tob }
procedure TFFrameTreeTob.Print(const Nature, Etat: String; const Apercu: Boolean; const Criteres: String; const LanguePrinc: String);
var
  TobToPrint: Tob;
  MaxLevel: Integer;
  //Bmp: TBitmap;

  procedure PrepareTobToPrint(const T: Tob);
  var
    i: Integer;
    IM: TTreeManagerItem;
    //BmpData: String;
    ...
    function TreeTobTvImg2String(const TVImg: TTreeTobTvImg): String;
    begin
      case TVImg of
        tttiCBG: Result := 'CBG';
        tttiTEG: Result := 'TEG';
  ...
  begin
    ...
    with Tob.Create('_TreeTobPrint_', TobToPrint, -1) do
    begin
      Dupliquer(T, False, True);
      { Niveau }
      AddChampSupValeur('NIVEAU', T.Niveau, CSTInteger);
      { Images du TreeView }
      IM := FTreeManager.IndexOf(T);
      for i := Low(IM.FTreeImages) to MaxLevel + Low(IM.FTreeImages) do
        AddChampSup('IMAGETV' + IntToStr(i), False, CSTString);
      for i := Low(IM.FTreeImages) to High(IM.FTreeImages) do
      begin
        (*Stream := TMemoryStream.Create();
        try
          BmpData := '';
          DrawTvImg(Bmp.Canvas, 0, 0, IM.FTreeImages[i]);
          Bmp.SaveToStream(Stream);
          Stream.Position := 0;
          SetLength(BmpData, Stream.Size);
          Stream.Read(PChar(BmpData)^, Stream.Size);
          AddChampSupValeur('IMAGETV' + IntToStr(i), BmpData, CSTBase64Binary);
        finally
          Stream.Free
        end*)
        SetString('IMAGETV' + IntToStr(i), TreeTobTvImg2String(IM.FTreeImages[i]));
      end;
      for i := 0 to Pred(T.Detail.Count) do
        PrepareTobToPrint(T.Detail[i])
    end
  end;
...
begin
  //Bmp := TBitmap.Create();
  TobToPrint := Tob.Create('_TreeTobPrint_', nil, -1);
  try
    MaxLevel := FTreeTob.MaxNiveau;
    //Bmp.Width := 16;
    ...
    for i := 0 to Pred(FTreeTob.Detail.Count) do
      PrepareTobToPrint(FTreeTob.Detail[i]);
    LanceEtatTob('E', Nature, Etat, TobToPrint, Apercu, False, False, nil, '', Self.Caption,
                 False, 0, Criteres,{$IFNDEF EAGLCLIENT} nil,{$ENDIF} 0, '', LanguePrinc);
  finally
    TobToPrint.Free;
    //Bmp.Free;
  end
