...
begin
  ...
  iTable := PrefixeToNum ('RD4');
  for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
     if ( V_PGI.Dechamps[iTable,iChamp].libelle[1] <> '.') and
        ( (copy(V_PGI.Dechamps[iTable,iChamp].nom,5,3) = 'RD4') or
          (copy(V_PGI.Dechamps[iTable,iChamp].nom,5,8) = 'BLOCNOTE') ) and
        ( V_PGI.Dechamps[iTable,iChamp].tipe=stTypeChamp) then
        begin
        ListeChamp.add(V_PGI.DEChamps[iTable,iChamp].nom);
        ListeLibelle.add (V_PGI.Dechamps[iTable,iChamp].libelle);
        end;
...
{ calcul du nombre maxi de combos, de champs et du nombre de lignes dans le grid }
  ...
  for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
  begin
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,8) = 'LIBTABLE' then inc(MaxCombo);
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,8) = 'LIBTEXTE' then inc(MaxTexte);
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,6) = 'LIBMUL' then inc(MaxMultiChoix);
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,6) = 'LIBVAL' then inc(MaxValeur);
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,7) = 'LIBBOOL' then inc(MaxBool);
    if copy(V_PGI.Dechamps[iTable,iChamp].nom,8,7) = 'LIBDATE' then inc(MaxDate);
  end;
