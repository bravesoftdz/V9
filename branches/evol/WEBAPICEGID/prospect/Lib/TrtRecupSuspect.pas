...
uses
  ...
  EntGc     // cinClientKpmg
  {$IFNDEF EAGLCLIENT}
  {$IFNDEF DBXPRESS},dbtables{BDE}{$ELSE},uDbxDataSet, DBGrids, HDB{$ENDIF}
  {$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    iTable := PrefixeToNum ('RSC');
    for iChamp:=1 to High(V_PGI.DEChamps[iTable]) do
    begin
      if ((copy(V_PGI.DEChamps[iTable,iChamp].Nom,1,7)='RSC_RSC') or (copy(V_PGI.DEChamps[iTable,iChamp].Nom,1,8)='RSC_6RSC'))
      and (V_PGI.DEChamps[iTable,iChamp].Libelle<>'')
      and (V_PGI.DEChamps[iTable,iChamp].Libelle[1]<>'.') and (V_PGI.DEChamps[iTable,iChamp].Libelle<>'Error')  then
      begin
         if not ((TLChamp.RowCount = 2) and (TLChamp.Cells[0,1] = '')) then   //1ère ligne vide
            TLChamp.RowCount := TLChamp.RowCount + 1;
        ARow := TLChamp.RowCount -1;
        TLChamp.Cells[0,ARow]:=V_PGI.DEChamps[iTable,iChamp].Libelle;
        TLChamp.Cells[1,ARow]:=V_PGI.DEChamps[iTable,iChamp].Nom;
      end;
    end;
  ...
  begin
    ...
    for iChamp :=1 to High(V_PGI.DEChamps[iTable]) do
    begin
      if (copy(V_PGI.DEChamps[iTable,iChamp].Nom, 1, 2) = 'T_') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> '')
      and (Copy(V_PGI.DEChamps[iTable,iChamp].Libelle, 1, 1) <> '.') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> 'Error')  then
      begin
         if not ((TLChamp.RowCount = 2) and (TLChamp.Cells[0,1] = '')) then   //1ère ligne vide
            TLChamp.RowCount  := TLChamp.RowCount + 1;
        ARow                  := TLChamp.RowCount -1;
    ...
    iTable              := PrefixeToNum ('YTC');
    for iChamp :=1 to High(V_PGI.DEChamps[iTable]) do
    begin
      if (copy(V_PGI.DEChamps[iTable,iChamp].Nom, 1, 4) = 'YTC_') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> '')
      and (Copy(V_PGI.DEChamps[iTable,iChamp].Libelle, 1, 1) <> '.') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> 'Error') then
      begin
        if (V_PGI.DEChamps[iTable, iChamp].Nom = 'YTC_AUXILIAIRE') or (V_PGI.DEChamps[iTable, iChamp].Nom = 'YTC_TIERS') then
          Continue;
        if not ((TLChamp.RowCount = 2) and (TLChamp.Cells[0,1] = '')) then   //1ère ligne vide
          TLChamp.RowCount  := TLChamp.RowCount + 1;
        ARow                  := TLChamp.RowCount -1;
    ...
    iTable              := PrefixeToNum ('RPR');
    for iChamp :=1 to High(V_PGI.DEChamps[iTable]) do
    begin
      if (copy(V_PGI.DEChamps[iTable,iChamp].Nom, 1, 4) = 'RPR_') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> '')
      and (Copy(V_PGI.DEChamps[iTable,iChamp].Libelle, 1, 1) <> '.') and (V_PGI.DEChamps[iTable,iChamp].Libelle <> 'Error')  then
      begin
        if (V_PGI.DEChamps[iTable, iChamp].Nom = 'RPR_AUXILIAIRE') then
          Continue;
        if not ((TLChamp.RowCount = 2) and (TLChamp.Cells[0,1] = '')) then   //1ère ligne vide
          TLChamp.RowCount  := TLChamp.RowCount + 1;
        ARow                  := TLChamp.RowCount -1;
...
begin
  ...
  Prefixe               := Copy(LeChamp, 1, pos('_', LeChamp)-1);
  Itable                := PrefixeToNum(Prefixe);
  for i := 1 to High(V_PGI.DEchamps[Itable]) do
  begin
    if V_PGI.DEChamps[Itable, i].Nom = LeChamp then
    begin
      LeType            := V_PGI.DEchamps[Itable, i].tipe;
      Break;
    end;
  end;
