...
function FuncSynBPVerifTree(parms: array of variant; nb: integer): variant;
function FuncSynBPDonneLibelleSession(parms: array of variant; nb: integer): variant;

implementation


uses Controls,M3FP,HMsgBox,Sysutils,UCtx,HCtrls,
     {$IFNDEF EAGLCLIENT}
     ...
     {$IFDEF CRM}ParamSoc,{$ENDIF}
     Uutil,InitialisationPrev,BPEclatement,BPPrevCmd,
     CstCommun,BPFctSEssion,BPInitPrev,BPFctArbre,BPBasic,BPUtil;
...
var codeSession,noeudPere,codeaxe,valeuraxe:hString;
    NomTable,chpCode,ChpLib,Chp2,Val2,msg,NewValChecked:hString;
begin
  result:=true;
  codeSession:=hString(parms[0]);
  ...
  NewValChecked:=hString(parms[4]);
  msg:='';
  if valeuraxe<>'' then
  begin
    if NewValChecked='0' then
    begin
    ...
    begin
      //contrôle pour le cas "Nouvelle Embauche" en Paie
      if not ExisteSql('SELECT CC_CODE FROM CHOIXCOD WHERE CC_CODE="'+Copy(valeurAxe,0,2)+'" AND CC_TYPE="PTK"')
      then
      begin
        msg:= Format_( TraduireMemoire( 'La valeur axe entrée est incorrecte pour le code axe %s. ' ), [codeaxe] );
        result:=false;
...
begin
  ...
  if ContextBP=3 then
  begin
   if ExisteSQL('SELECT 1 FROM QBPARBRE WHERE QBR_CODESESSION="'+codeSession+'"')
   then result:=true
   else result:=false
  end
  else result:=true;
...
function FuncSynBPVerifMultiSoc(parms: array of variant; nb: integer): variant;
begin
  if v_pgi.RequeteMultiDossier.count = 0 then result:=false
  else result:=true
end;
...
begin
  ...
  RegisterAglFunc('SynBPVerifTree', False, 1, FuncSynBPVerifTree);
  RegisterAglFunc('SynBPVerifMultiSoc', False, 0, FuncSynBPVerifMultiSoc);
