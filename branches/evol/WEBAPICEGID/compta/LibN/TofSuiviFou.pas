...
procedure TOF_SUIVIFOU.CompteOnExit(Sender: TObject);
begin
...
procedure TOF_SUIVIFOU.DateOnExit(Sender: TObject);
begin
     DoDateOnExit(THEdit(Sender), Ech1, Ech2, DateD, DateF);
end;

procedure TOF_SUIVIFOU.ExoOnChange(Sender: TObject);
begin
...
procedure TOF_SUIVIFOU.JalOnExit(Sender: TObject);
begin
     DoJalOnExit(THEdit(Sender), Jal1, Jal2);
end;

procedure TOF_SUIVIFOU.SensOnChange(Sender: TObject);
begin
...
procedure TOF_SUIVIFOU.TidcOnClick(Sender: TObject) ;
begin
...
Mots clefs ... :
*****************************************************************}
procedure TOF_SUIVIFOU.AuxiElipsisClick( Sender : TObject );
begin
     THEdit(Sender).text:= CPLanceFiche_MULTiers('M;' +THEdit(Sender).text + ';' +THEdit(Sender).Plus + ';');
end;


procedure TOF_SUIVIFOU.OnArgument(Arguments: string);
...
procedure TOF_SUIVIFOU.OnUpdate ;
begin
inherited ;
//if TheData<>nil then Exit ;
end ;

procedure TOF_SUIVIFOU.OnNew ;
begin
inherited ;
{ DblClick }
...
procedure TOF_SUIVIFOU.InitTous;
begin
