...
//mcd 26/01/2006
//utilise traduireMemoire au lieu de traduitGA, car source utilisé en compta,
//qui n'a pas la traduciton affaire et donc pas le fichier DIcoaf
...
uses  StdCtrls,Controls,Classes,M3FP,UTOB,
{$IFDEF EAGLCLIENT}
      eMul,
{$ELSE}
      db,
...
{$ENDIF}
      ...
      ComCtrls,HCtrls,HEnt1,HMsgBox,UTOF, lookup;
...
{$endif}
function GetParamSocGI ( Nom : string ;Def:variant) : variant;
function ChercheLibelleGCZoneLibre ( Code : string ) : string;
implementation
const
TexteMessage: array[1..6] of string 	= (
          {1}  'Proposition d''affaire'
          {2} ,'(Année Début-Année fin d''exercice)'
          {3} ,'(Millésime : Année de fin d''exercice)'
          {4} ,'(Année Début-Mois Début d''exercice)'
          {5} ,'Avenant'
          {6} ,'Paramètre inexistant : '
              );
...
While (Critere <>'') do
   ...
   if (copy(Critere,1,4) ='AUXI')
	then begin
        ...
        if GetParamSocSecur('SO_CPSAISIEAFFAIRE','')='COM' then TableTiers := '##DP##.TIERS'
    ...
    if Copy(Critere,1,6) = 'STATUT' then
      ...
      if copy(critere,8,3)= 'PRO' then begin
         SetControlProperty('AFF_STATUTAFFAIRE','Plus',' AND (CC_LIBRE="" OR CC_LIBRE="PRO")');
         Statut := 'PRO';
         end
      else begin
         SetControlProperty('AFF_STATUTAFFAIRE','Plus',' AND (CC_LIBRE="" OR CC_LIBRE="AFF")');
         Statut := 'AFF';
         end;
      end;
...
if not(GetParamSocGI('SO_AffCo2Visible',false)) then SetCOntrolVisible('AFf_AFFAIRE2',False);
if not(GetParamSocGI('SO_AffCo3Visible',false)) then SetCOntrolVisible('AFf_AFFAIRE3',False);
if not(GetParamSocGI('SO_AffGestionAvenant',false)) then SetCOntrolVisible('AFf_AVENANT',False);
// Caption de la form
if Statut = 'PRO' then Ecran.Caption := TraduireMemoire(TexteMessage[1]);		//'Propositions d''affaires';
UpdateCaption(Ecran);
...
for i:=1 to NbMaxPartieAffaire do
    ...
    case i of
        1: Begin Code:=Part1;
                 lng:=StrToInt(GetParamSocGI('SO_AFFCo1Lng',0));
                 ...
                 End;
        2: Begin Code:=Part2;
                 lng:=StrToInt(GetParamSocGI('SO_AFFCo2Lng',0));
                 TypeCode:=GetParamSocGI('SO_AFFCo2Type','');
                 Valeur:=GetParamSocGI('SO_AFFCo2valeur','');
                 Visible:=GetParamSocGI('SO_AFfCo2Visible',false);
                 Libelle:=GetParamSocGI('SO_AFFCo2Lib','');
                 End;
        3: Begin Code:=Part3;
                 lng:=StrToInt(GetParamSocGI('SO_AFFCo3Lng',0));
                 TypeCode:=GetParamSocGI('SO_AFFCo3Type','');
                 Valeur:=GetParamSocGI('SO_AFFCo3valeur','');
                 Visible:=GetParamSocGI('SO_AFfCo3Visible',false);
                 Libelle:=GetParamSocGI('SO_AFFCo3Lib','');
                 End;
        End;
    If (i <= StrToInt(GetParamSocGI('SO_AFFCodeNbPartie',0))) Then
        ...
        if (Typecode= 'LIS') then                                       // Liste de saisie
            ...
            case i of
              ...
              3 : Code.OnElipsisClick := OnElipsisClickAffairePart3;
            end;
            Code.Width:=45;
            End else
        if ((Typecode='SAI') And (i=2) And (GetParamSocGI('SO_AFFormatExer','')<>'AUC')) then  // Formattage Exercice
            ...
            if (GetParamSocGI('SO_AFFormatExer','')='AA') then
                Code.Hint :=Code.Hint+ TraduireMemoire(TexteMEssage[2])	//'(Année Début-Année fin d''exercice)'
            else if (GetParamSocGI('SO_AFFormatExer','')='A') then
                Code.Hint :=Code.Hint+TraduireMemoire(TexteMEssage[3])	  //(Millésime : Année de fin d''exercice)'
            else
                Code.Hint :=Code.Hint+ TraduireMemoire(TexteMEssage[4]);	//(Année Début-Mois Début d''exercice)';
            END;
...
if Avenant <> Nil then
    BEGIN
    if (GetParamSocGI('SO_AFFGestionAvenant',false)) then
        BEGIN
        Avenant.Visible := true;
        Avenant.Left := PositAvenant; Avenant.Width :=2 * 10 + 5;
        Avenant.Hint := TraduireMemoire(TexteMessage[5]);	//'Avenant';
        Avenant.MaxLength := 2;
...
inherited;
if not(GetParamSocGI('SO_AffCo2Visible',false)) then SetControlText('AFF_AFFAIRE2','');
if not(GetParamSocGI('So_AffCo3Visible',false)) then SetControlText('AFF_AFFAIRE3','');
{$IFDEF EAGLCLIENT}
...
begin
  ...
  if GetParamSocSecur('SO_CPSAISIEAFFAIRE','')='COM' then
...
If ((Nomchamp = 'AFFAIRE1') Or (Nomchamp = 'AFFAIRE2') Or (Nomchamp = 'AFFAIRE3')) Then
  ...
  Case NumPart Of
      ...
      2 : Begin Visible:=GetParamSocGI('SO_AFFCo2Visible',false); Libelle:=GetParamSocGI('SO_AFFCo2Lib',''); End;
      3 : Begin Visible:=GetParamSocGI('SO_AFFCo3Visible',false); Libelle:=GetParamSocGI('SO_AFFCo3Lib',''); End;
      End;
  if not(Visible) then Gr.colwidths[Col]:=0;
  Gr.Cells[Col,0]:=Libelle;
  End;
// Gestion de la colonne Avenant
if ((Nomchamp='AVENANT') and Not(GetParamSocGI('SO_AFFGestionAvenant',false))) then Gr.colwidths[Col]:=0;
end;
...
For i:=0 to Gr.Columns.Count-1 do
    ...
    if (Nomchamp = 'AVENANT') then
        BEGIN
        if Not (GetParamSocGI('SO_AFFGestionAvenant',false)) then  Gr.columns[i].visible:= False;
        END;
    End;
...
begin
  if GetParamSocSecur('SO_CPSAISIEAFFAIRE','')='COM' then
  begin
    Q:=OpenSQL('SELECT * FROM ##DP##.PARAMSOC WHERE SOC_NOM="'+Uppercase(Trim(Nom))+'"',TRUE) ;
    if Not Q.EOF then
    BEGIN
...
{$IFNDEF EAGLSERVER}
      if (V_PGI.SAV) and (not V_PGI.ParamSocLast) then
        PgiInfo (TexteMessage[6]+Nom, TitreHalley); //Paramètre inexistant
{$ENDIF}
    END ;
    Ferme(Q) ;
  end else Result := GetParamSocSecur (Nom,def);
end;
...
begin
  ...
  if GetParamSocSecur('SO_CPSAISIEAFFAIRE','')='COM' then
  begin
    Q := OpenSQL ('SELECT CC_LIBELLE FROM ##DP##.CHOIXCOD WHERE CC_TYPE="ZLT" AND CC_CODE="'+Code+'"',True);
    if not Q.Eof then Result := Q.FindField('CC_LIBELLE').AsString
    else Result := '';
    Ferme (Q);
  end else Result := RechDom ('GCZONELIBRETIE',Code,False);
end;
