...
uses
  ...
  CpteUtil, EdtLegal, CritEDT, HQry, HSysMenu,UtilEdt1, QRRupt, HTB97, HPanel, UiUtil, tCalcCum ;

procedure BalanceAge2TP ;
procedure BalanceAgeZoom2TP(Crit : TCritEdt) ;

type
  TFQRBalAgeTP = class(TFQR)
    HLabel3: THLabel;
  ...
  private    { Déclarations privées }
    DeviseEnCours,LibTPEnCours : String ;
    LMdp, LRupt, LRuptDev, LRecapG : TStringList ;
    TotEdt, TotAux, TotMdp, TotEdtPivot,TotTP : TabTot ;
    TabDate                        : TTabDate4 ;
...
Function TFQRBalAgeTP.QuoiCpt(i : Integer) : String ;
BEGIN
...
Procedure TFQRBalAgeTP.BalAgeZoom(Quoi : String) ;
Var Lp,i: Integer ;
BEGIN
...
BEGIN
PP:=FindInsidePanel ;
QR:=TFQRBalAgeTP.Create(Application) ;
Edition.Etat:=etBalAgeDev ;
QR.QRP.QRPrinter.OnSynZoom:=QR.BalAgeZoom ;
QR.InitType (nbAux,neGlV,msRien,'QRBALAGEETP','',TRUE,FALSE,FALSE,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
try
 QR.QRP.QRPrinter.OnSynZoom:=QR.BalAgeZoom ;
 QR.InitType (nbAux,neGlV,msRien,'QRBALAGEETP','',FALSE,TRUE,FALSE,Edition) ;
 finally
 QR.Free ;
 end ;
Screen.Cursor:=SyncrDefault ;
...
Procedure TFQRBalAgeTP.FinirPrint ;
Var Solde   : Double ;
BEGIN
Inherited ;
QEcr.Close ;
...
procedure TFQRBalAgeTP.InitDivers ;
BEGIN
Inherited ;
...
procedure TFQRBalAgeTP.GenereSQL ;
...
If critEdt.GlV.MultiDevise Then GenereSQLTiersMultidevise(neBal,Age,CritEdt,Q)
                           Else GenereSQLTiers(neBal,Age,CritEdt,Q,TRUE,AvecRevision.State) ;
GenereSQLSub ;
END ;

procedure TFQRBalAgeTP.GenereSQLSub ;
{ Construction de la requête SQL en fonction du multicritère }
BEGIN
Inherited ;
{ Construction de la clause Select de la SQL }
GenereSQLSUBTiers(neBal,Age,CritEdt,QEcr,TRUE) ;
END ;

procedure TFQRBalAgeTP.RenseigneCritere ;
...
procedure TFQRBalAgeTP.ChoixEdition ;
...
procedure TFQRBalAgeTP.RecupCritEdt ;
...
function TFQRBalAgeTP.CritOk : Boolean ;
BEGIN
...
Procedure TFQRBalAgeTP.Calculs(var Report1,Report2 : TReport; i : integer; Bool1, Bool2 : Boolean ;
                              Deb,Cre,Couv : Double) ;
...
Procedure TFQRBalAgeTP.CalculsPivot(i : integer) ;
{ Incrémentations de toutes les variables de l'état }
Var Solde   : Double ;
    Deb,Cre,Couv : Double ;
BEGIN
...
procedure TFQRBalAgeTP.SaisieDates ;
...
procedure TFQRBalAgeTP.FormShow(Sender: TObject);
begin
//HelpContext:=7547010 ;
...
procedure TFQRBalAgeTP.FChoixEcartClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRBalAgeTP.FDateCompta1Exit(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFQRBalAgeTP.FP1Exit(Sender: TObject);
Var St : String ;
    A  : Char   ;
begin
  inherited;
...
procedure TFQRBalAgeTP.PeriodesEnter(Sender: TObject);
begin
  inherited;
FPeriodicite.SetFocus ;
end;

procedure TFQRBalAgeTP.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var LDev : tLDev ;
begin
  inherited;
...
procedure TFQRBalAgeTP.BSubDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
    OkCalcul : Boolean ;
begin
  inherited;
...
procedure TFQRBalAgeTP.BFCompteAuxBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var
    MontantNul : Boolean ;
begin
  inherited;
...
procedure TFQRBalAgeTP.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
Var LeTot : TabTot ;
begin
  inherited;
If CritEdt.GlV.MultiDevise Then
...
procedure TFQRBalAgeTP.BMdpBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRBalAgeTP.FPeriodiciteChange(Sender: TObject);
Var i : Integer ;
    TabD : TTabDate8 ;
begin
  inherited;
...
procedure TFQRBalAgeTP.FTypeParClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRBalAgeTP.TOPREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRBalAgeTP.BOTTOMREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
    TB : TabTot ;
    i : Integer ;
begin
...
procedure TFQRBalAgeTP.QEcrAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFQRBalAgeTP.FNatureCptChange(Sender: TObject);
begin
  inherited;
If QRLoading then Exit ;
...
procedure TFQRBalAgeTP.BNouvRechClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRBalAgeTP.FColl1Change(Sender: TObject);
...
procedure TFQRBalAgeTP.DLRuptNeedData(var MoreData: Boolean);
var TotRupt          : Array[0..15] of Double ;
...
procedure TFQRBalAgeTP.BRuptBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=(CritEdt.Rupture<>rRien) Or CritEdt.GlV.MultiDevise ;
end;

procedure TFQRBalAgeTP.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
If (CritEdt.GlV.TypePar=0) Then
  BEGIN
...
procedure TFQRBalAgeTP.FSansRuptClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRBalAgeTP.FRupturesClick(Sender: TObject);
begin
  inherited;
...
Procedure TFQRBalAgeTP.ActiveSoldePivot ;
...
procedure TFQRBalAgeTP.FMontantClick(Sender: TObject);
begin
  inherited;
ActiveSoldePivot ;
end;

procedure TFQRBalAgeTP.FDevisesChange(Sender: TObject);
begin
  inherited;
ActiveSoldePivot ;
end;

procedure TFQRBalAgeTP.FMultiDeviseClick(Sender: TObject);
Var OkA : Boolean ;
begin
  inherited;
...
procedure TFQRBalAgeTP.QEcrBeforeOpen(DataSet: TDataSet);
begin
  inherited;
If CritEdt.GlV.MultiDevise Then
   BEGIN
...
procedure TFQRBalAgeTP.BRecapGAfterPrint(BandPrinted: Boolean);
begin
  inherited;
BRecapG.ForceNewPage:=FALSE ;
end;

procedure TFQRBalAgeTP.QRDLRecapGNeedData(var MoreData: Boolean);
Var Cod,Lib : String ;
...
procedure TFQRBalAgeTP.BRecapGBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=FALSE ;
If Not CritEdt.GLV.FormatPrint.PrSepCompte[10] Then Exit ;
...
procedure TFQRBalAgeTP.BFCompteAuxAfterPrint(BandPrinted: Boolean);
begin
  inherited;
BFCompteAux.ForceNewPage:=FALSE ;
end;

procedure TFQRBalAgeTP.BENTETETPBeforePrint(var PrintBand: Boolean; var Quoi: String);
Var St : String ;
begin
  inherited;
...
procedure TFQRBalAgeTP.BPIEDTPBeforePrint(var PrintBand: Boolean; var Quoi: String);
begin
  inherited;
...
If CritEdt.GlV.DeviseEtPivot Then
   BEGIN
   SwapSymbolePivotDevise(CritEdt) ; CritEdt.Decimale:=InitDecimaleResol(CritEdt.GlV.Resol,CritEdt.Decimale) ;
   AfficheNewMontant(CritEdt,TotTP[7],TPPivot) ;
   SwapSymbolePivotDevise(CritEdt) ;
   END ;
Fillchar(TotTP,SizeOf(TotTP),#0) ;
end;
