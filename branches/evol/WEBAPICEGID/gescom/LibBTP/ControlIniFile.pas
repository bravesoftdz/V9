...
begin
  ...
  TRY
    ...
    Iexist := FileOpen(IncludeTrailingBackslash(Localrepert)+HalSocIni,fmOpenRead );
    if Iexist <0 then
    begin
...
{$IFNDEF EAGLCLIENT}
  ...
  begin
    IniPgiFile := TiniFile.create (IncludeTrailingBackslash (RepertInifile)+HalSocIni);
    (*
