...
Procedure CalculFacture ( TOBAffaire,TOBPiece,TOBPieceTrait,TOBSSTRAIT,TOBouvrages,TOBouvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG : TOB ; DEV : RDEVISE ;SaisieTypeAvanc : boolean=false; Action: TActionFiche=TaModif; ModifAvanc : boolean=false; LigneACalcule : integer=-1; FromGenfac : boolean=false; FromZero : boolean=false) ;
  ...
  procedure CalculeThisLigneOUv (TOBL : TOB; ImpacteTotauxPiece : boolean=true);
  ...
  BEGIN
    ...
    begin
      ...
      begin
        ...
        CalculeThisLigneOUv (TOBL,not TOBL.getBoolean('GL_GESTIONPTC')); // on precalcule la ligne sans impatcer les montant de la piece
        if TOBL.getBoolean('GL_GESTIONPTC') and (Pos(TOBPiece.getString('GP_NATUREPIECEG'),'DBT;ETU')>0) then
        begin
