...
Var ExistFic : Boolean;
    FicOrig : textfile;
begin
if bTraite then Exit;
LDepart.Visible := False;
LDepartSuite.Visible := False;
LFin.Visible := True;
LFin.Invalidate;
bValider.Enabled := False;
bAbandon.Enabled := False;
bAide.Enabled := False;
if CB1.Checked then
    begin
    ...
    Sleep(2000);
    SB1.SimpleText := 'Effacement de la table Catalogue';
...
//  ouverture du fichier permettant de connaître l'origine
{$I-}
AssignFile(FicOrig, 'C:\PGI01\DAT\S1S3.txt');
FileMode:=0 ;
Reset (FicOrig);
CloseFile(FicOrig);
{$I+}
ExistFic:=(IOResult = 0);
if ExistFic then Origine:='S1';
...
var
    ...
    MessAvance : string ;
    StDisq : string ;
    Nb_Disk,TailleTotal,MaxBar : integer;
    NomFic : array of string;
...
// Gestion du multi-disquettes
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\TiersS*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
    begin
    Nb_Disk:=Nb_Disk+1;
    NomFic[Nb_Disk-1]:=SearchRec.Name;
    TailleTotal:=TailleTotal+SearchRec.Size;
    end;
  FindClose(SearchRec);
  end;
PB2.Position := 0;
NomFic:=Copy(NomFic,0,Nb_Disk);

For Nb_Disk:=Low(NomFic) to High(NomFic) do
begin
  if Nb_Disk>0 then StDisq:='Disquette n° '+IntToStr(Nb_Disk) else StDisq:='';

  //SB1.SimpleText := 'Traitement de la table Tiers';
  SB1.SimpleText := 'Traitement de la table Tiers '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  //AssignFile(FicSeq, 'C:\PGI01\DAT\TiersS3.txt');
  AssignFile(FicSeq, 'C:\PGI01\DAT\'+NomFic[Nb_Disk]);
  Reset (FicSeq);
  {$I+}
  ExistFic:=(IOResult = 0);
  ...
  MessAvance:='Début de traitement Tiers '+StDisq;
  Try
    //  calcul nb enreg du fichier de données
    //FindFirst('C:\PGI01\DAT\TiersS3.txt', faAnyFile, SearchRec);
    FindFirst('C:\PGI01\DAT\'+NomFic[Nb_Disk], faAnyFile, SearchRec);
    i_sizefic := Trunc(SearchRec.Size / Length(stEnreg));
    PB1.Position := 0;
    if Nb_Disk=0 then PB2.Max:=Trunc(TailleTotal / Length(stEnreg));
    Ratio := Round(i_sizefic / 100);
    if Ratio = 0 then begin Ratio := 1; PB1.Max:=i_sizefic; end;
    MaxBar:=Ratio;
    if Ratio > 1000 then Ratio:=1000;
    //st_log := '****** Début du traitement TIERS ******';
    st_log := '****** Début du traitement TIERS  Fichier : '+NomFic[Nb_Disk]+'******';
    ...
    while not EOF(FicSeq) do
        ...
        Inc(Nb_Lus);
        if ((Nb_Lus mod MaxBar) = 0) or (Ratio = 1) then begin PB1.StepIt;PB1.Refresh;end;
        PB2.StepIt;PB2.Refresh;
    ...
    //  mise en place des valeurs importées
        ...
        {$ELSE}
        if Origine='S1' then stAuxi:=Trim(Copy(stEnreg, 2, 10))
                        else stAuxi:=Trim(Copy(stEnreg, 1, 1) + Trim(Copy(stEnreg, 2, 10)));
        {$ENDIF}
    ...
    {    stDate := TobTrav.GetValue('T_AUXILIAIRE');
        ...
        {$ELSE}
        if Origine='S1' then TobTrav.PutValue('T_TIERS', Trim(Copy(stEnreg, 2, 10)))   // <  0> code client     (clé primaire)
                         else TobTrav.PutValue('T_TIERS', Copy(stEnreg, 1, 1) + Trim(Copy(stEnreg, 2, 10)));   // <  0> code client     (clé primaire)
        {$ENDIF}
        ...
        if Origine='S1' then StAuxi:=Trim(Copy(stEnreg, 294, 9))
                        else StAuxi:=Trim(Copy(stEnreg, 293, 10)) ;
        if stAuxi<>'' then
           begin
           AffecteTAuxiliaire(stAuxi);
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\ClilS3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement de la table ADRESSES '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  ...
  MessAvance:='Début de traitement Adresses '+StDisq;
  Try
    //  calcul nb enreg du fichier de données
    ...
    st_log := '****** Début du traitement ADRESSES  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    ...
    //  mise en place des valeurs importées
        ...
        if Origine='S1' then stCode:=Uppercase(Trim(Copy(stEnreg, 2, 10)))
                        else stCode:=Uppercase(Trim(Copy(stEnreg, 1, 1) + Trim(Copy(stEnreg, 2, 10))));
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\ProdS3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement de la table Article '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  ...
  MessAvance:='Début de traitement Article '+StDisq;
  Try
    //  calcul nb enreg du fichier de données
    ...
    st_log := '****** Début du traitement ARTICLE  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    while not EOF(FicSeq) do
    ...
    //  TobTrav.PutValue('GA_COMPTAARTICLE' , Copy(stEnreg, 127, 3));
        ...
        if Origine='S1' then stAuxi := Uppercase(Trim(Copy(stEnreg, 279, 10)))
                        else stAuxi := CodeFour + Uppercase(Trim(Copy(stEnreg, 279, 10)));
        AffecteTAuxiliaire(stAuxi);
        ...
        //TobTrav.PutValue('GA_PVHTCON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
        ...
        //TobTrav.PutValue('GA_DPACON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
        TobTrav.PutValue('GA_PAHT', dbTrav);
        //TobTrav.PutValue('GA_PAHTCON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
        ...
        //TobTrav.PutValue('GA_PMAPCON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
        ...
        //TobTrav.PutValue('GA_MARGEMINICON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\NmclS3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement de la table Nomenclature '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  ...
  MessAvance:='Début de traitement Nomenclature '+StDisq;
  Try
    //  calcul nb enreg du fichier de données
    ...
    st_log := '****** Début du traitement NOMENCLATURE  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    while not EOF(FicSeq) do
...
procedure TFRecupS3.Traite_Catalogue;
var
    stEnreg, stDate, stSigne, stTemp : string;
    FicSeq : textfile;
    TobOut, TobTrav : TOB;
...
begin
if not FileExists('C:\PGI01\DAT\PrfoS3.txt') then Exit;
// Gestion du multi-disquettes
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\PrfoS3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  //SB1.SimpleText := 'Traitement de la table Catalogue';
  SB1.SimpleText := 'Traitement de la table Catalogue '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  //AssignFile(FicSeq, 'C:\PGI01\DAT\PrfoS3.txt');
  AssignFile(FicSeq, 'C:\PGI01\DAT\'+NomFic[Nb_Disk]);
  Reset (FicSeq);
  {$I+}
  ExistFic:=(IOResult = 0);
  ...
  Nb_Lus := 0;
  TobOut := TOB.Create('', nil, -1);
  //MessAvance:='Début de traitement Catalogue ';
  MessAvance:='Début de traitement Catalogue '+StDisq;
  Try
    //  calcul nb enreg du fichier de données
    //FindFirst('C:\PGI01\DAT\PrfoS3.txt', faAnyFile, SearchRec);
    FindFirst('C:\PGI01\DAT\'+NomFic[Nb_Disk], faAnyFile, SearchRec);
    i_sizefic := Trunc(SearchRec.Size / Length(stEnreg));
    PB1.Position := 0;
    if Nb_Disk=0 then PB2.Max:=Trunc(TailleTotal / Length(stEnreg));
    ...
    MaxBar:=Ratio;
    if Ratio > 1000 then Ratio:=1000;
    //st_log := '****** Début du traitement CATALOGUE ******';
    st_log := '****** Début du traitement CATALOGUE  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    while not EOF(FicSeq) do
    ...
    flush (FicLog);
        ...
        PB2.StepIt;PB2.Refresh;
        SB1.Panels.Items[1].Text := IntToStr(Nb_Lus) + ' sur ' + IntToStr(i_Sizefic);
        TobTrav := TOB.Create('CATALOGU', TobOut, -1);
        TobTrav.InitValeurs;
    //  mise en place des valeurs importées
        MessAvance:='Affectation code et Libellé article #13 Enregistrement n° '+IntToStr(Nb_Lus);
        stTemp:=Copy(stEnreg, 1, 15) ;             // <  0>  Code article (clé primaire)
        RemplaceLesBadCaractere(stTemp);
        iDim := 18 + (MaxDimension * 3);
        stDate := format('%-'+IntToStr(iDim)+'.'+IntToStr(iDim)+'sX',[Uppercase(stTemp)]);

        stTemp := 'Select GA_LIBELLE from ARTICLE where GA_ARTICLE="' + stDate + '" And GA_TYPEARTICLE<>"NOM"';
        QQ:=OpenSQL(stTemp,True) ;
        if QQ.Eof then
          begin
          TobTrav.PutValue('GCA_LIBELLE', 'Article référence : '+stDate);
          end else
          begin
          TobTrav.PutValue('GCA_ARTICLE', stDate);
          TobTrav.PutValue('GCA_LIBELLE', QQ.Fields[0].AsString);
          end ;
        Ferme(QQ) ;
        MessAvance:='Affectation référence article #13 Enregistrement n° '+IntToStr(Nb_Lus);
        stTemp:=Copy(stEnreg, 26, 15) ;             // < 25> code article du fournisseur
        if Trim(stTemp)='' then
          begin
          TobTrav.PutValue('GCA_REFERENCE', stDate);
          end else
          begin
          RemplaceLesBadCaractere(stTemp);
          TobTrav.PutValue('GCA_REFERENCE', stDate);
        ...
        MessAvance:='Affectation Code fournisseur et Unité #13 Enregistrement n° '+IntToStr(Nb_Lus);
        if Trim(Copy(stEnreg, 16, 10)) = '' then continue;
        if Origine='S1' then TobTrav.PutValue('GCA_TIERS', Uppercase(Trim(Copy(stEnreg, 16, 10))))   // < 15> code fournisseur (clé prim. + sec.
                        else TobTrav.PutValue('GCA_TIERS', CodeFour + Uppercase(Trim(Copy(stEnreg, 16, 10)))) ;  // < 15> code fournisseur (clé prim. + sec.
        TobTrav.PutValue('GCA_QUALIFUNITEACH', Trim(Copy(stEnreg, 57, 2)));   // < 56> Unité d'achat   UA

        MessAvance:='Affectation Prix et délai #13 Enregistrement n° '+IntToStr(Nb_Lus);
        stDate:=Copy(stEnreg, 108, 13) ;         // <106> Prix d'achat unitaire
        stSigne := Copy(stEnreg, 107, 1);
        iPos:=Pos('.', stDate);
        if iPos>0 then stDate[iPos] := ',';
        dbTrav := 0.0;
        if IsNumeric(stDate) then dbTrav := StrToFloat(stDate);
        if (dbTrav = 0.0) or (stSigne = '-') then  dbTrav := dbTrav * -1; ;
        TobTrav.PutValue('GCA_DPA',dbTrav) ;
        TobTrav.PutValue('GCA_PRIXBASE',dbTrav) ;
        stDate:=Copy(stEnreg, 121, 13) ;         // <120> Délai d'approvisionnement en jours
        iPos:=Pos('.', stDate)-1;
        if iPos>0 then stDate := Trim(Copy(stDate, 0, iPos));
        if IsNumeric(stDate) then TobTrav.PutValue('GCA_DELAILIVRAISON',StrToInt(stDate)) ;

        MessAvance:='Affectation des dates création et modification #13 Enregistrement n° '+IntToStr(Nb_Lus);
        TobTrav.PutValue('GCA_DATECREATION',V_PGI.DateEntree) ;
        TobTrav.PutValue('GCA_DATEMODIF',V_PGI.DateEntree) ;
        TobTrav.PutValue('GCA_DATESUP',IDate2099) ;
        TobTrav.PutValue('GCA_CREATEUR',V_PGI.User) ;
        TobTrav.PutValue('GCA_UTILISATEUR',V_PGI.User) ;
        TobTrav.PutValue('GCA_CREERPAR', 'IMP');
...
var
    ...
    FicSeq : textfile;
    TobArticle, TobA, TobTiers, TobOut, TobTrav,TobTrav2, TobTemp,TobPrec : TOB;
    Nb_Lus,Nb_Duplic, i_sizefic, Ratio, iDim, LgTarif,iPos,MaxTarif,ind,F_ind : integer;
    dbTrav,QtePrec : double;
    Searchrec : TSearchRec;
    Q : TQuery;
    ExistFic,DuplicEnreg : boolean;
    MessAvance : string ;
    StDisq,RacineFic : string ;
    Nb_Disk,TailleTotal,MaxBar : integer;
    NomFic : array of string;
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\'+RacineFic+'*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement de la table Tarif '+StTiers+' '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  ...
  Nb_Lus := 0; MaxTarif:=0; Nb_Duplic:=0 ;
  TobOut := TOB.Create('', nil, -1);
  //MessAvance:='Début de traitement Tarifs '+StTiers;
  MessAvance:='Début de traitement Tarifs '+StTiers+' '+StDisq;
  ...
  Try
    ...
    st_log := '****** Début du traitement TARIFS '+StTiers+'  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    while not EOF(FicSeq) do
    ...
    //  mise en place des valeurs importées
        ...
        TobTrav.PutValue('GF_TARIF', IntToStr(MaxTarif+Nb_Lus+Nb_Duplic));
        ...
        if Trim(Copy(stEnreg, 5, 10)) <> '' then
          begin
          if Origine='S1' then TobTrav.PutValue('GF_TIERS', Uppercase(Trim(Copy(stEnreg, 5, 10))))     // <  4> code client                          //
                          else TobTrav.PutValue('GF_TIERS', CodeDebTiers + Uppercase(Trim(Copy(stEnreg, 5, 10))));     // <  4> code client                          //
          end;
    ...
    //        TobTrav.PutValue('GF_REGIMEPRIX', 'HT')
        ...
        if (dbTrav = 0.0) or (dbTrav = (-999999.0)) then
            ...
            if DuplicEnreg then F_ind:=1 else F_ind:=0 ;
            for ind:=0 to F_ind do
              begin
              if TobOut.Detail.Count>1 then
                TobPrec:=TobOut.Detail[TobOut.Detail.Count-2-ind]
                else TobPrec:=Nil;
              if TobPrec<>nil then
                if (TobTrav.GetValue('GF_ARTICLE')<>TobPrec.GetValue('GF_ARTICLE')) or
                   (TobTrav.GetValue('GF_TIERS')<>TobPrec.GetValue('GF_TIERS')) or
                   (TobTrav.GetValue('GF_TARIFTIERS')<>TobPrec.GetValue('GF_TARIFTIERS')) or
                   (TobTrav.GetValue('GF_TARIFARTICLE')<>TobPrec.GetValue('GF_TARIFARTICLE')) then
                  QtePrec:=0;
              if QtePrec>0 then
                begin
                if TobPrec<>nil then
                   TobPrec.PutValue('GF_BORNESUP', dbTrav);
                end;
              end;
            QtePrec:=dbTrav;
            end;
    ...
    //      end;
        ...
        stDate := Trim(Copy(stEnreg, 77, 8));  // < 76> date effet nouveau PU                //
        while Pos('/', stDate) <> 0 do Delete(stDate, Pos('/', stDate), 1);
        if (IsNumeric(stDate)) and (Length(stDate) = 6) and (StrToInt(stDate) <> 0) then
            begin
            try
            if StrToDate(Trim(Copy(stEnreg, 77, 8)))>Date then DuplicEnreg:=True;
            if DuplicEnreg then
              begin
              TobTrav.PutValue('GF_DATEFIN', Str6ToDate(stDate, 20)-1);
              TobTrav.PutValue('GF_DATEDEBUT', iDate1900);
              end else
              begin
              TobTrav.PutValue('GF_DATEFIN', iDate2099);
              TobTrav.PutValue('GF_DATEDEBUT', Str6ToDate(stDate, 20));
              end;
            except
                TobTrav.PutValue('GF_DATEFIN', iDate2099);
                TobTrav.PutValue('GF_DATEDEBUT', iDate1900);
        ...
        TobTrav.PutValue('GF_DATEMODIF', Date);
        ...
        if IsNumeric(stDate) then dbTrav := StrToFloat(stDate);
        if (dbTrav <> 0.0) and (stSigne = '-') then dbTrav := dbTrav * -1;
        TobTrav.PutValue('GF_PRIXANCIEN', 0);
        if DuplicEnreg then
          begin
          MessAvance:='Duplication ligne tarif pour date effet #13 Enregistrement n° '+IntToStr(Nb_Lus);
          TobTrav2:= TOB.Create('TARIF', TobOut, -1);
          TobTrav2.InitValeurs;
          Inc(Nb_Duplic);
          TobTrav2.Dupliquer(TobTrav,True,True);
          TobTrav.PutValue('GF_PRIXUNITAIRE', dbTrav); // remise de l'ancien prix dans la 1ère ligne de tarif.
          TobTrav2.PutValue('GF_DATEFIN', iDate2099);
          TobTrav2.PutValue('GF_DATEDEBUT',TobTrav.GetValue('GF_DATEFIN')+1);
          TobTrav2.PutValue('GF_TARIF', IntToStr(MaxTarif+Nb_Lus+Nb_Duplic));
          end;
  //      if TobTrav.GetValue('GF_DEVISE') = 'EUR' then TobTrav.PutValue('GF_PRIXCON', Arrondi(dbTrav * 6.55957,V_PGI.OkDecV));
    //    if Trim(Copy(stEnreg, 57, 1)) = 'C' then TobTrav.Free;

    //
        MessAvance:='Mise à jour dans la base #13 Enregistrement n° '+IntToStr(Nb_Lus);
        if ((Nb_Lus mod Ratio) = 0) and (QtePrec=0) then
            begin
            TobOut.SetAllModifie(True);
            if CB1.Checked then
                TobOut.InsertDB(nil)
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\Prm2S3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement des tablettes '+StDisq;
  //  ouverture du fichier de données
  {$I-}
  ...
  st_log := '****** Début du traitement Tablettes  Fichier : '+NomFic[Nb_Disk]+'******';
  writeln (FicLog, st_log);
  flush (FicLog);
  Reset (FicSeq);
  while not EOF(FicSeq) do
  ...
  //            TobTrav.AddChampSup('TTC', True);
              if Copy(stEnreg, 27, 1) = 'T' then
                  TobTrav.PutValue('CC_ABREGE', 'TTC')
                  else
                  TobTrav.PutValue('CC_ABREGE', 'HT');
              end
              else
...
TailleTotal:=0;Nb_Disk:=1;
SetLength(NomFic,99);
if FindFirst('C:\PGI01\DAT\ReglS3*.txt', faAnyFile, SearchRec) = 0 then
  begin
  NomFic[0]:=SearchRec.Name;
  TailleTotal:=SearchRec.Size;
  while FindNext(SearchRec)=0 do
...
begin
  ...
  SB1.SimpleText := 'Traitement de la table Réglement '+StDisq;
  //  ouverture du fichier de données
  ...
  MessAvance:='Début de traitement réglementd '+StDisq;
  Try
    ...
    st_log := '****** Début du traitement REGLEMENT  Fichier : '+NomFic[Nb_Disk]+'******';
    writeln (FicLog, st_log);
    flush (FicLog);
    Reset (FicSeq);
    while not EOF(FicSeq) do
