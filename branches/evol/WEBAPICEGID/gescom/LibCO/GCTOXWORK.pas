...
uses
  ...
  StdCtrls, uTob, ToxMoteur, ParamSoc, HTB97, ExtCtrls,
  HPanel, uTox, Hent1, uToxConst, uToxClasses, uToxDecProc ;


procedure GescomModeNotConfirmeTox ( P: TCollectionEnveloppe; var Atraiter : boolean) ;
procedure GescomModeTraitementTox  ( P: TCollectionEnveloppe ; Index: integer ; TheTob: TOB ; var ttResult: TToxTraitement ; ToFeel: TToxFeelTob = Nil ) ;
...
begin
  ...
  //
  EtatTox := TraitementDeLaTox (TheTob, Tmemo(nil), False, False, False, False, True, Devise, BasculeEuro) ;
  //
  // MAJ du nombre de traitements
  //
  if not TheTob.FieldExists ('NB_TRAITEMENT') then
  ...
  begin
    ...
    // Si Nbre de traitement > Nombre maximum de traitement, la TOX est définitivement rejetée
    //
    if NbTraitement >= GetParamSoc ('SO_GCTOXNBTRT') then  Resultat := [ ttRewrite, ttBad ]
