...
procedure LanceMenuAuto;
var
Nom, St, Value, NomBase, RepertoireServeur, RepertoireSortie,Annee    : string;
i, NumeroMenu     : integer;
Connect           : Boolean;
begin
//  InitMoveProgressForm(Application.MainForm,Application.Title, 'Traitement en cours, veuillez patienter...', 200, False, true) ;
      ...
      begin
        ...
        if Nom='/BASE'     then V_PGI.CurrentAlias :=Value;
        if Nom='/USER'     then V_PGI.UserLogin :=Value;
        if Nom='/PASSWORD' then V_PGI.PassWord:= CryptageSt(Value);
        if Nom='/MENU'     then NumeroMenu:=StrToInt(Value) ;
        if Nom='/REPSERVEUR'      then RepertoireServeur :=Value;
        if Nom='/REPSORTIE'      then RepertoireSortie :=Value;
        if Nom='/ANNEE'      then Annee :=Value;
       end;
      ...
      if connect then LanceSpecifPouchain (NumeroMenu, RepertoireServeur,RepertoireSortie, Annee);
...
begin
  ...
  case Num of
    ...
    15: ; //Avant formshow
    16 :;
    324010 : Begin; LanceTdbcom07('','','',''); RetourForce:=True; End;
    324011 : Begin; LanceTdbcom07('a','','',''); RetourForce:=True; End;
    324012 : Begin; LanceTdbcom07('b','','',''); RetourForce:=True; End;
    324013 : Begin; LanceTdbcom07('c','','',''); RetourForce:=True; End;
    324014 : Begin; LanceTdbcom07('d','','',''); RetourForce:=True; End;
    324015 : Begin; LanceTdbcom07('f','','',''); RetourForce:=True; End;
...
initialization
  ...
  TitreHalley := 'Spécifique POUCHAIN';
