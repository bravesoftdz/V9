...
var ind1, ind2, iTemp, NumTable : integer;
    stSuffixe, stSep : string;
    NumChamps : array of integer;
    NomChamps : array of string;

begin
NumTable := PrefixeToNum('GPA');
ind1 := High(V_PGI.DeChamps[NumTable]) + 1;
SetLength(NumChamps, ind1);
SetLength(NomChamps, ind1);
for ind1 := 1 to High(V_PGI.DeChamps[NumTable]) do
begin
    NumChamps[ind1] := V_PGI.DEChamps[NumTable, ind1].Numero;
    NomChamps[ind1] := V_PGI.DEChamps[NumTable, ind1].Nom;
end;
...
begin
    ...
    begin
        iTemp := ChampToNum(NomChamps[ind1]);
        if V_PGI.DEChamps[NumTable, iTemp].Tipe = 'BOOLEAN' then
            Result := Result + stSep + '"-" as ' + NomChamps[ind1]
        else if (V_PGI.DEChamps[NumTable, iTemp].Tipe = 'DATE') then
            Result := Result + stSep + '"01/01/1900" as ' + NomChamps[ind1]
        else if (V_PGI.DEChamps[NumTable, iTemp].Tipe = 'DOUBLE') or
                (V_PGI.DEChamps[NumTable, iTemp].Tipe = 'RATE') or
                (V_PGI.DEChamps[NumTable, iTemp].Tipe = 'INTEGER') then
            Result := Result + stSep + '0 as ' + NomChamps[ind1]
        else
            Result := Result + stSep + '"" as ' + NomChamps[ind1];
    end;
