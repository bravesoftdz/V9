...
begin
  ...
  begin
    ...
    Req := Req + '(Select SUM(L1.GL_MONTANTHTDEV) FROM LIGNE AS L1 WHERE L1.GL_NUMERO=L0.GL_NUMERO AND L1.GL_PIECEPRECEDENTE LIKE "' + ATrouver + '%" AND (L1.GL_NATUREPIECEG="FBT" OR L1.GL_NATUREPIECEG="ABT")) As MONTANTFAC,';
    ...
    req := Req + ' AND L0.GL_DATEPIECE >="'+ USDATETIME (strtodate(FACTURE.Text))+
    			'" AND L0.GL_DATEPIECE <="'+ USDATETIME (strtodate(FACTURE_.Text))+'"';
