...
//     Procedure SauveDocPgi;
     ...
     procedure TraitementArticle(TOBL: TOB; CodeArticle: string;LignePgi : integer);
...
//          if Valeur(Qte) = 0 then Qte := '1'; // cas ou une désignation est présente dans la quantité
        ...
        //
        // Modified by f.vautrain 24/10/2017 15:30:16 - FS#2753 - NCN : développement selon devis ML-171509-3A
        TraitementArticle(TOBL, CodeArticle, LignePgi);
        //
        if (designation<>'') then
        begin
          if length(designation) > 70 then
  ...
  begin
    ...
    CalculFacture (TOBAFFAire,TOBPiece,TOBPieceTrait,TOBSSTRAIT,TOBOuvrage,TOBOUvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV);
  end;
...
// Modified by f.vautrain 24/10/2017 15:30:16 - FS#2753 - NCN : développement selon devis ML-171509-3A
procedure TPieceEtude.TraitementArticle(TOBL : TOB; CodeArticle : string; LignePgi : integer);
...
begin
  ...
  begin
    ...
    begin
      TraitementArticle(TOBL, '', LignePgi);
    end;
  end;
