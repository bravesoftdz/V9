...
Créé le ...... : 31/01/2002
Modifié le ... :   /  /
Description .. : Planning des absences et des présences
...
PT14 28/06/2007 NA V-80 Ajout planning Presence
}

unit UTOFPGPlanningAbsPre;
...
{$ENDIF}
  ...
  LookUp, entpaie,
  PgOutilsEAgl, HEnt1, PGPlanning, PGPlanningAbsPaie, PgOutils2, Hmsgbox, PGPlanningAbsPre; // pt14
...
var
  StWhere, StAbs, St, StVal , stPres: string; // pt14
    ...
    begin { DEB PT4 }
      ...
      if Assigned(THMultiValComboBox(GetControl('PCN_CODETAPE'))) then
        ...
        begin
          ...
          if StAbs <> '' then StAbs := '(' + Copy(StAbs, 1, Length(StAbs) - 3) + ') ';
          StWhere := StWhere + StAbs;
        end; 
    end
    else
      // deb pt14
      if ARecuperer = 'PRESENCE' then
      begin
        stpres := '';
        St := GetControlText('MOTIFPRESENCE');
        while (St <> '') and (st <> '<<Tous>>') do { PT7 10/10/2005}
        begin
          StVal := ReadTokenSt(St);
          StPres := StPres + ' PCN_TYPECONGE="' + StVal + '" OR';
        end;
        if StPres <> '' then StPres := ' AND (' + Copy(StPres, 1, Length(StPres) - 3) + ') ';
        StWhere := StWhere + StPres;
      end
      else
      // fin pt14
...
begin
  ...
  //(GetControlText('RUPTURE1')='SO_PGSERVICE')
   ...
   PGPlanningAbsPres(StrToDate(GetControlText('DATEABS')), StrToDate(GetControlText('DATEABS_')),
    Utilisat, '', RecupCritere('ABSENCE'),RecupCritere('PRESENCE'), RecupCritere('SALARIE'), Salarie,
    NiveauRupt, (GetControlText('MULTINIVEAU') = 'X'),
    RecupCritere('FILTRE'), RecupCritere('HIERARCHIE'), RecupCritere('CBFILTREABS'), { PT8 } { PT11 }
    StrtoFloat(GetControltext('JOURABS')),StrtoFloat(GetControltext('SOLDEABS')), { PT12 }
    GetControltext('TYPESOLDEABS'),mode); // fin pt14
