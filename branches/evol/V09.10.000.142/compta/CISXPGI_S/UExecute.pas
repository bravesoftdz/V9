...
begin
        ...
        FTable.Close;
        if Result then exit;
        Q1 := OpenSQLADO ('SELECT ' + field+' from '+ParName+ ' Where '+field+'="'+ TRIM(Code)+'"', DMImport.DBGlobalD.ConnectionString );
		...
		begin
			...
			begin
                             ...
                             begin
                                  ...
                                  Q1.DatabaseName := DMImport.DBGlobal.Name;
                                  Q1.SQL.Add ('Select * from '+DMImport.GzImpCorresp.TableName
                                  + ' Where Tablename="'+table+'" and Profile="' + Champ[M].ListProfile.strings[N]+'"');
                                  ...
                                  begin
                                      ...
                                      Q2.ConnectionString := DMImport.DBGlobal.ConnectionString;
                                      Q2.SQL.Add (ReqSQl);
                                      try
                                            Q2.ExecSQL;
                                      except
...
{$ELSE}
                  ...
                  +Cle+'"', DMImport.DBGlobal.ConnectionString );
                  if not QP.EOF then
...
{$ENDIF}
        ...
        begin
             ...
             begin
                 ...
                 TT.Connection := DMImport.DBGLOBALD;
...
{$ENDIF}
        ...
        try
              ...
              Q1 := OpenSQLADO ('Select * from '+ Sortie + Where+ OrderBy, DMImport.DBGlobalD.ConnectionString);
                  ...
                  begin
                                  ...
                                  begin
                                       ...
                                       XWhere    := TF.GetValue ('XWhere');
                                        if Assigned(TT) then TT.Close;
                                       TT := TADOTable.Create(Application);
                                       TT.Tablename := TableLien;
                                       TT.Connection := DMImport.DBGLOBALD;
                                       Try
                                       TT.Open;
                                       except
                                       ...
                                       if TF.Detail.Count = 1 then
                                        ...
                                        begin
                                             Chaine := '';
                                             for jj :=0 to TT.FieldCount - 1 do
                                             begin
                                                  Contenu := TT.FindField(TT.Fields[jj].FieldName).Asstring;
                                                  if TT.FieldDefs[jj].DataType = ftWideString then
                                                       Formatc:= '%-'+IntToStr(TT.FieldDefs[jj].Size)+'.'+ IntToStr(TT.FieldDefs[jj].Size)+'s'
                                                  else
                                                  if TT.FieldDefs[jj].DataType = ftFloat then
                                                  begin
                                                              recupsize (TT.Fields[jj].FieldName);
                                                              Formatc:= '%'+IntToStr(Size)+'.'+ IntToStr(Size)+'s';
                                                  end;
                                                  Chaine := Chaine+Format(Formatc,[Contenu]);
                                             end;
...
(*              Q1 := TQuery.Create(nil);
              Q1.DatabaseName := DMImport.DBGlobalD.Name;
              if Requete <> '' then
                 Q1.SQL.Add ('Select * from '+ Sortie + Where+ OrderBy)
              else
                 Q1.SQL.Add ('Select * from '+ Sortie + Where);
              Q1.Open;
*)
              Q1 := OpenSQLADO ('Select * from '+ Sortie + Where+ OrderBy, DMImport.DBGlobalD.ConnectionString);
