...
uses SysUtils, WinTypes, WinProcs, Messages, Classes, Graphics, Controls,
     ...
     Hctrls, {$IFNDEF DBXPRESS}dbtables, HSysMenu, hmsgbox, Db, Hqry,
  Hcompte, HRegCpte, ComCtrls, HTB97{$ELSE}uDbxDataSet{$ENDIF}, DB, Grids, DBGrids, TabNotBk, CodePost, Hcompte,
     ComCtrls, hmsgbox, Ent1, HEnt1, HSysMenu, Region, HTB97, MajTable,
     Hqry, HRegCpte, HPanel, UiUtil, Saisutil, SaisComm ;

Procedure FicheSociete ( Comment : TActionFiche ; QuellePage : integer ; Verif : Boolean ) ;
...
uses PrintDBG, Axe, QRE ;

Procedure FicheSociete ( Comment : TActionFiche ; QuellePage : integer ; Verif : Boolean ) ;
var FSociete: TFSociete;
    PP       : THPanel ;
begin
...
procedure TFSociete.BImprimerClick(Sender: TObject);
begin
...
procedure TFSociete.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin CanClose:=OnSauve ; end;

procedure TFSociete.BValiderClick(Sender: TObject);
begin Bouge(nbPost) ; end;

procedure TFSociete.BAnnulerClick(Sender: TObject);
begin Bouge(nbCancel) ; end;

Function TFSociete.OnSauve : boolean ;
Var Rep : Integer ;
BEGIN
...
Function TFSociete.EnregOK : boolean ;
BEGIN
Result:=FALSE  ; NextControl(Self) ;
...
Procedure TFSociete.RecupDecDev ;
var Q : TQuery ;
BEGIN
...
procedure TFSociete.BFermeClick(Sender: TObject);
begin Close ; end;

Procedure TFSociete.ChargeEnreg ;
BEGIN
...
Function TFSociete.Bouge(Button: TNavigateBtn) : boolean ;
BEGIN
result:=FALSE  ;
Case Button of
...
Procedure TFSociete.MemoChamps ;
BEGIN
...
procedure TFSociete.FormShow(Sender: TObject);
begin
...
Procedure TFSociete.ActiveLaPage(Sender : TObject ; QuellePage : Integer) ;
BEGIN Pages.ActivePage:=Pages.Pages[QuellePage] ; TWinControl(Sender).SetFocus ; END ;

procedure TFSociete.SO_CODEPOSTALExit(Sender: TObject);
begin VerifCodePostal(TSociete,SO_CODEPOSTAL,SO_VILLE,TRUE) ; end;

procedure TFSociete.SO_VILLEExit(Sender: TObject);
begin VerifCodePostal(TSociete,SO_CODEPOSTAL,SO_VILLE,TRUE) ; (*FALSE*) end;

procedure TFSociete.SO_CODEPOSTALDblClick(Sender: TObject);
begin VerifCodePostal(TSociete,SO_CODEPOSTAL,SO_VILLE,True) ; end;

procedure TFSociete.SO_VILLEDblClick(Sender: TObject);
begin VerifCodePostal(TSociete,SO_CODEPOSTAL,SO_VILLE,False) ; end;

procedure TFSociete.SO_ETABLISCPTAClick(Sender: TObject);
begin
// GP 10/01/97
...
procedure TFSociete.BVentilClick(Sender: TObject);
begin FicheAxeAna ('') ; end;

Procedure TFSociete.RempliTabloCpte ;
BEGIN
...
Function TFSociete.ChangementTabloCpte : Boolean ;
Var i : Integer ;
BEGIN
Result:=False ; i:=1 ;
While i<=10 do
...
Function TFSociete.LongueurCpte(Sender : TObject) : Boolean ;
Var Sql : String ;
    Q : TQuery ;
BEGIN
Result:=True ;
...
Function TFSociete.LongueurCpteOk : Boolean ;
BEGIN
Result:=False ;
...
Function TFSociete.GeneAttenteOk : Boolean ;
Var Q : TQuery ;
    PasOk : Boolean ;
    Ind : Byte ;
BEGIN
...
function TFSociete.ExisteCptesAttente : boolean ;
BEGIN
Result:=False ;
...
Function TFSociete.FourchetteVide(Sender : TObject) : Boolean ;
Var i : Integer ;
    Pref,Indice : String ;
BEGIN
Result:=True ;
...
Function TFSociete.FourchetteOk : Boolean ;
BEGIN
Result:=False ;
...
procedure TFSociete.TSocieteAfterPost(DataSet: TDataSet);
begin
...
Procedure TFSociete.GetCompteFourchette(Sender : TObject ; Var Cpte : TabCpte) ;
...
Function TFSociete.ChevauchementCpteFourchetteOk : Boolean ;
Var Ok : Boolean ;
BEGIN
Ok:=True ;
...
Function TFSociete.CompareTabloCpteOk(Tab1,Tab2 : TabCpte) : Boolean ;
Var i,j : Integer ;
BEGIN
Result:=True ; i:=1 ;
While i<10 do
...
Function TFSociete.ManqueUnCpte : Boolean ;
...
Function TFSociete.NatureCpteOk : Boolean ;
BEGIN
Result:=False ;
...
Function TFSociete.VerifiNatureCpteOk(Sender : TObject ; Tab : TabCpte) : Boolean ;
Var Q : TQuery ;
    i : Integer ;
    Pref : Char ;
    PasOk : Boolean ;
...
Procedure TFSociete.BourreLeCpte(Sender : TObject );
...
procedure TFSociete.SO_BILDEB1Exit(Sender: TObject);
begin BourreLeCpte(Sender) ; end;

Function TFSociete.DevisePrincEnMvt : Boolean ;
Var QLoc : TQuery ;
BEGIN
...
Function TFSociete.TrouveUnEnreg(Q : TQuery ;Sql : String) : Boolean ;
BEGIN
...
procedure TFSociete.Cbj2MouseDown(Sender: TObject; Button: TMouseButton;
...
Procedure TFSociete.LitJourFermeture ;
Var i : Byte ;
    C : TComponent ;
    St : String[7] ;
BEGIN
...
Procedure TFSociete.EcritJourFermeture ;
Var i : Byte ;
    C : TComponent ;
    St : String[7] ;
BEGIN
...
procedure TFSociete.SO_DIVTERRITDblClick(Sender: TObject);
begin
PaysRegion(SO_PAYS,SO_DIVTERRIT,True) ;
end;

procedure TFSociete.SO_PAYSChange(Sender: TObject);
begin
PaysRegion(SO_PAYS,SO_DIVTERRIT,False) ;
end;

procedure TFSociete.Defaut1Exit(Sender: TObject);
begin BourreLeCpte(Sender) ; end;

Function TFSociete.DefautCpteOk : Boolean ;
Var Q : TQuery ;
    i : Byte ;
    Sql,LeWhere : String ;
    St : String ;
...
procedure TFSociete.BAideClick(Sender: TObject);
begin
CallHelpTopic(Self) ;
end;

Function TFSociete.VerifDateEntreeEuroOk : Boolean ;
BEGIN
Result:=True ;
...
function TFSociete.VerifCptesEuro : boolean ;
...
function TFSociete.VerifPariteEuro : boolean ;
Var StParite : String ;
    Parite   : Integer ;
BEGIN
Result:=True ;
...
Function TFSociete.CarBourreOk : Boolean ;
Var Okok : boolean ;
    C : Char ;
    CC : Set of Char ;
BEGIN
...
procedure TFSociete.CacheFonctions35 ;
BEGIN
if ((EstSerie(S3)) or (EstSerie(S5))) then
   ...
   // Axes > 2
   SO_CORSA31.Visible:=False ; SO_CORSA41.Visible:=False ;
   SO_CORSA51.Visible:=False ;
...
procedure TFSociete.SO_DATEDEBUTEUROExit(Sender: TObject);
begin
