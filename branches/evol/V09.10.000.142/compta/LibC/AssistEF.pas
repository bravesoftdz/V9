...
{$IFNDEF DBXPRESS}dbtables,{$ELSE}uDbxDataSet,{$ENDIF}
  StdCtrls, Hctrls, Grids, DBGrids, HDB, ComCtrls,
  Buttons, ExtCtrls, hmsgbox, Ent1, HSysMenu, HTB97, Hqry, ColMemo,
  HPanel, ADODB ;

function LanceAssistEF(Formule : String) : String ;

Type tav = (avNone,avEcr,avAna,avCpt) ;

type
  TFAssistEF = class(TFAssist)
    TRub: THTable;
    ChoixRec: TTabSheet;
    Periode: TTabSheet;
    MsgEF: THMsgBox;
    ...
    procedure FindLibChange(Sender: TObject);
...
Result:='' ;
SourisSablier ;
X:=TFAssistEF.Create(Application) ;
 Try
...
function TFAssistEF.IsBalSit : boolean ;
begin
Result:=FALSE ;
If rCumul.Checked Then result:=(CCBalSit.Checked)
End ;

function TFAssistEF.IsRubrique : boolean ;
begin
result := False ;
...
function TFAssistEF.FirstPage : TtabSheet  ;
begin
result := ChoixFormule ;
end ;

function TFAssistEF.PageNumber : Integer ;
begin
result := 0 ;
...
function TFAssistEF.PreviousPage : TTabSheet ;
begin
result := nil ;
if rCumul.Checked then
   begin
...
function  TFAssistEF.NextPage : TTabSheet ;
begin
result := nil ;
if rCumul.Checked then
   begin
...
*)
procedure TFAssistEF.ChangeSourceItems(n : Integer) ;
var i : Integer ;
begin
rSource.Items.Clear ;
for i:=1 to n do
...
procedure TFAssistEF.SetInitialValues ;
...
procedure TFAssistEF.SetInitialTable(s : String) ;
var st : String ;
    l  : integer ;
begin
st := Trim(Uppercase(s)) ;
...
procedure TFAssistEF.SetInitialRec(s : String) ;
var st : String ;
    Qi : TQuery ;
begin
st := Trim(Uppercase(s)) ;
...
procedure TFAssistEF.SetInitialRec2(s : String) ;
var st : String ;
    Qi : TQuery ;
begin
st := Trim(Uppercase(s)) ;
...
procedure TFAssistEF.SetInitialBalSit(s : String) ;
var st : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
Procedure TFAssistEF.DecodeWhereSup(S : String) ;
Var St,St1 : String ;
    i,j,Max,Ind : integer ;
    Pref,Pref1 : String ;
    ii : tii ;
...
procedure TFAssistEF.SetInitialWhereSup(s : String) ;
Var St : String ;
BEGIN
st := Trim(Uppercase(s)) ;
(*
...
procedure TFAssistEF.SetInitialField(s : String) ;
var st : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
procedure TFAssistEF.SetInitialEcr(s : String) ;
var st,sr : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
procedure TFAssistEF.SetInitialEtab(s : String) ;
var st : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
procedure TFAssistEF.SetInitialSQL(s : String) ;
var st : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
procedure TFAssistEF.SetInitialDev(s : String) ;
var st : String ;
begin
st := Trim(Uppercase(s)) ;
if Pos('"',st) = 0 then // référence cellule
...
procedure TFAssistEF.SetInitialCptVariant(s : String) ;
var st : String ;
    i : Integer ;
begin
st := Trim(Uppercase(s)) ;
...
procedure TFAssistEF.SetInitialDate(s : String ; PourConstante : Boolean) ;
...
procedure TFAssistEF.FindCodeChange(Sender: TObject);
begin
inherited;
Q.Close ;
Q.SQL.Clear ;
if rConstante.Checked then
   begin
   Q.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" AND CC_CODE LIKE "' + FindCode.Text + '%" ORDER BY CC_CODE') ;
   end
else
   begin
   Case rSource.ItemIndex of
      0 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE RB_NATRUB="CPT" AND RB_RUBRIQUE LIKE "' + FindCode.Text + '%" ORDER BY RB_RUBRIQUE') ;
      1 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE ' + WhereBudJal + 'RB_NATRUB="BUD" AND RB_RUBRIQUE LIKE "' + FindCode.Text + '%" ORDER BY RB_RUBRIQUE') ;
      2,10,11,12 : Q.SQL.Add('SELECT G_GENERAL AS CODE, G_LIBELLE AS LIBELLE FROM GENERAUX WHERE G_GENERAL LIKE "' + FindCode.Text + '%" ORDER BY G_GENERAL') ;
      3 : Q.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS WHERE T_AUXILIAIRE LIKE "' + FindCode.Text + '%" ORDER BY T_AUXILIAIRE') ;
      4 : Q.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" AND S_SECTION LIKE "' + FindCode.Text + '%" ORDER BY S_SECTION') ;
      5 : Q.SQL.Add('SELECT BG_BUDGENE AS CODE, BG_LIBELLE AS LIBELLE FROM BUDGENE WHERE BG_BUDGENE LIKE "' + FindCode.Text + '%" ORDER BY BG_BUDGENE') ;
      6 : Q.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" AND BS_BUDSECT LIKE "' + FindCode.Text + '%" ORDER BY BS_BUDSECT') ;
      7 : Q.SQL.Add('SELECT J_JOURNAL AS CODE, J_LIBELLE AS LIBELLE FROM JOURNAL WHERE J_JOURNAL LIKE "' + FindCode.Text + '%" ORDER BY J_JOURNAL') ;
      8 : Q.SQL.Add('SELECT BJ_BUDJAL AS CODE, BJ_LIBELLE AS LIBELLE FROM BUDJAL WHERE BJ_BUDJAL LIKE "' + FindCode.Text + '%" ORDER BY BJ_BUDJAL') ;
      end ;
   end ;
ChangeSQL(Q) ;
Q.Open ;
...
procedure TFAssistEF.FindLibChange(Sender: TObject);
begin
inherited;
Q.Close ;
Q.SQL.Clear ;
if rConstante.Checked then
   begin
   Q.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" AND CC_LIBELLE LIKE "%' + FindLib.Text + '%" ORDER BY CC_CODE') ;
   end
else
   begin
   Case rSource.ItemIndex of
      0 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE RB_NATRUB="CPT" AND RB_LIBELLE LIKE "%' + FindLib.Text + '%" ORDER BY RB_LIBELLE') ;
      1 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE ' + WhereBudJal + 'RB_BUDJAL="' + cBudget.Value + '" RB_NATRUB="BUD" AND RB_LIBELLE LIKE "%' + FindLib.Text + '%" ORDER BY RB_LIBELLE') ;
      2,10,11,12 : Q.SQL.Add('SELECT G_GENERAL AS CODE, G_LIBELLE AS LIBELLE FROM GENERAUX WHERE G_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY G_GENERAL') ;
      3 : Q.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS WHERE T_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY T_AUXILIAIRE') ;
      4 : Q.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" AND S_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY S_SECTION') ;
      5 : Q.SQL.Add('SELECT BG_BUDGENE AS CODE, BG_LIBELLE AS LIBELLE FROM BUDGENE WHERE BG_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY BG_BUDGENE') ;
      6 : Q.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" AND BS_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY BS_BUDSECT') ;
      7 : Q.SQL.Add('SELECT J_JOURNAL AS CODE, J_LIBELLE AS LIBELLE FROM JOURNAL WHERE J_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY J_JOURNAL') ;
      8 : Q.SQL.Add('SELECT BJ_BUDJAL AS CODE, BJ_LIBELLE AS LIBELLE FROM BUDJAL WHERE BJ_LIBELLE LIKE "' + FindLib.Text + '%" ORDER BY BJ_BUDJAL') ;
      end ;
   end ;
ChangeSQL(Q) ;
Q.Open ;
...
procedure TFAssistEF.cPeriodeChange(Sender: TObject);
var i,np : Integer ;
    AA,MM,DD : Word ;
begin
inherited;
...
procedure TFAssistEF.cNatureEcrEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[1] ;
end;

procedure TFAssistEF.cIntegANEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[2] ;
end;

procedure TFAssistEF.cExerciceEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[3] ;
end;

procedure TFAssistEF.cPeriodeEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[4] ;
end;

procedure TFAssistEF.NumPeriodeEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[5] ;
end;

procedure TFAssistEF.cDetailPeriodeEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[6] ;
end;

procedure TFAssistEF.cEtablissementEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[7] ;
end;

procedure TFAssistEF.cDeviseEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[8] ;
end;

procedure TFAssistEF.GridRubEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[9] ;
end;

procedure TFAssistEF.FindCodeEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[10] ;
end;

procedure TFAssistEF.FindLibEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[11] ;
end;

procedure TFAssistEF.bFinClick(Sender: TObject);
begin
inherited;
If WriteFormule Then ModalResult:=mrOk ;
end;

Function TFAssistEF.WriteFormule : Boolean  ;
...
if rCumul.Checked then
   ...
   if FlagExtRec.Checked then s := RefCelRec.Text else
     ...
     If FindCode.Text<>'' Then s := '"' + sm + FindCode.Text + '"'
                          Else s := '"' + sm + Q.FindField('CODE').AsString + '"' ;
     END ;
   sFormule := sFormule + s + '; ' ;
   ...
   If IsCptGeneral Then
     ...
     if FlagExtEcr.Checked then s := RefCelEcr.Text else
       ...
       END ;
     END Else
     //YMO 05/07/2006 FQ14998 pas de choix sur les à-nouveaux dans le cas des rub budgétaires (onglet 'critères' caché)
     if IsBudgete then
     BEGIN
     if FlagExtEcr.Checked then s := RefCelEcr.Text else s := '"' + GetNatureEcr(cNatureEcr.Value) +SMonnaie+ '"' ;
     END Else
     BEGIN
   ...
   If Cum2 Then
     ...
     If rSource.ItemIndex in [10..12] Then
       ...
       if FlagExtRec2.Checked then s := RefCelRec2.Text else s := '"' + sm + Q2.FindField('CODE').AsString + '"' ;
       END Else s := '""' ;
     If IsBalSit Then
       BEGIN
...
if rChamp.Checked then
   ...
   if not IsSociete then
      begin
      if IsSection then sm := AxeCourant else sm := '' ;
      if FlagExtRec.Checked then s := RefCelRec.Text else s := '"' + sm + Q.FindField('CODE').AsString + '"' ;
      sFormule := sFormule + s + '; ' ;
      end
   else // société
      begin
   ...
   if FlagExtField.Checked then s := RefCelField.Text else s := '"' + QFields.FindField('DH_NOMCHAMP').AsString + '"' ;
   sFormule := sFormule + s + '; ' ;
...
if rConstante.Checked then
   ...
   if bLib.Down then sm := '$' + Q.FindField('CODE').AsString else sm := Q.FindField('CODE').AsString ;
...
procedure TFAssistEF.ExpliqueFormule ;
begin
r7.Caption:='' ;
...
If Not rSQL.Checked Then
  ...
  if FlagExtRec2.Checked then
     ...
     if not IsSociete then r2.Caption := r2.Caption + ' ' + Q.FindField('CODE').AsString ;
     // axe éventuel
...
if rChamp.Checked then
   ...
   if FlagExtField.Checked then r3.Caption := MsgF.Mess[15] + ' ' + RefCelField.Text
                           else r3.Caption := QFields.FindField('DH_NOMCHAMP').AsString ;
   end
...
procedure TFAssistEF.FormShow(Sender: TObject);
begin
inherited;
MakeZoomOLE(Handle) ;
Revision.Visible:= True;//(V_PGI.LaSerie=S7) ;
//RRO 28012003
RSQL.visible:= True;//not(V_PGI.Laserie=S3);
...
procedure TFAssistEF.DSDataChange(Sender: TObject; Field: TField);
begin
inherited;
...
if Q.EOF then
   begin
   lRub.Caption := MsgEF.Mess[12] ;
   bFin.Enabled := False ;
   exit ;
   end
else
   begin
   lRub.Caption := Q.FindField('CODE').AsString + ' - ' + Q.FindField('LIBELLE').AsString ;
   if not rChamp.Checked then bFin.Enabled := True ;
   end ;
end;

procedure TFAssistEF.CheckExoOKClick(Sender: TObject);
begin
inherited;
...
procedure TFAssistEF.cNumPeriodeClick(Sender: TObject);
begin
inherited;
NumPeriode.Value := cNumPeriode.ItemIndex + 1 ;
end;

procedure TFAssistEF.FlagExtClick(Sender: TObject);
var i : Integer ;
    C : TControl ;
begin
inherited;
...
procedure TFAssistEF.RefCelEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[27] ;
end;

procedure TFAssistEF.FlagExtEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[28] ;
end;

procedure TFAssistEF.RefCelRecChange(Sender: TObject);
begin
inherited;
lRub.Caption := MsgEF.Mess[29] + ' ' + RefCelRec.Text ;
end;

procedure TFAssistEF.rSourceClick(Sender: TObject);
begin
inherited ;
GC2.Visible:=FALSE ;
...
procedure TFAssistEF.PChange(Sender: TObject);
var titre : Boolean ;
    s : string ;
begin
inherited;
...
if GetPage = ChoixChamp then
   begin
   QFields.Close ;
   QFields.SQL[0] := 'SELECT DH_NOMCHAMP,DH_LIBELLE,DH_TYPECHAMP FROM DECHAMPS WHERE DH_CONTROLE LIKE "%L%" AND DH_PREFIXE = "' + GridFields.Hint + '" ORDER BY DH_NOMCHAMP' ;
   ChangeSQL(QFields) ;
   QFields.Open ;
   bFin.Enabled := True ;
...
if GetPage = ChoixRec2 then
   begin
   If (FindCode2.Text<>'') Or (InitialRec2<>'') Then
     BEGIN
     If InitialRec2<>'' Then BEGIN FindCode2.Text:=InitialRec2 ; InitialRec2:='' ; END Else FindCode2Change(Nil) ;
     END Else
     BEGIN
     Q2.Close ;
     Q2.SQL.Clear ;
     if rConstante.Checked then
        begin
        Q2.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" ORDER BY CC_CODE') ;
        end
     else
        begin
        Case rSource.ItemIndex of
           0 : Q2.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE RB_NATRUB="CPT" ORDER BY RB_RUBRIQUE') ;
           1 : Q2.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE ' + WhereBudJal + 'RB_NATRUB="BUD" ORDER BY RB_RUBRIQUE') ;
           2 : Q2.SQL.Add('SELECT G_GENERAL AS CODE, G_LIBELLE AS LIBELLE FROM GENERAUX ORDER BY G_GENERAL') ;
           3,10 : Q2.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS ORDER BY T_AUXILIAIRE') ;
           4,11 : Q2.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" ORDER BY S_SECTION') ;
           5 : Q2.SQL.Add('SELECT BG_BUDGENE AS CODE, BG_LIBELLE AS LIBELLE FROM BUDGENE ORDER BY BG_BUDGENE') ;
           6,12 : Q2.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" ORDER BY BS_BUDSECT') ;
           7 : Q2.SQL.Add('SELECT J_JOURNAL AS CODE, J_LIBELLE AS LIBELLE FROM JOURNAL ORDER BY J_JOURNAL') ;
           8 : Q2.SQL.Add('SELECT BJ_BUDJAL AS CODE, BJ_LIBELLE AS LIBELLE FROM BUDJAL ORDER BY BJ_BUDJAL') ;
           end ;
        end ;
     ChangeSQL(Q2) ;
     Q2.Open ;
...
if GetPage = ChoixRec then
   begin
   If (FindCode.Text<>'') Or (InitialRec<>'') Then
     BEGIN
     If InitialRec<>'' Then BEGIN FindCode.Text:=InitialRec ; InitialRec:='' ; END Else FindCodeChange(Nil) ;
     END Else
     BEGIN
     Q.Close ;
     Q.SQL.Clear ;
     if rConstante.Checked then
        begin
        Q.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" ORDER BY CC_CODE') ;
        end
     else
        begin
        Case rSource.ItemIndex of
           0 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE RB_NATRUB="CPT" ORDER BY RB_RUBRIQUE') ;
           1 : Q.SQL.Add('SELECT RB_RUBRIQUE AS CODE, RB_LIBELLE AS LIBELLE FROM RUBRIQUE WHERE ' + WhereBudJal + 'RB_NATRUB="BUD" ORDER BY RB_RUBRIQUE') ;
           2,10,11 : Q.SQL.Add('SELECT G_GENERAL AS CODE, G_LIBELLE AS LIBELLE FROM GENERAUX ORDER BY G_GENERAL') ;
           3 : Q.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS ORDER BY T_AUXILIAIRE') ;
           4 : Q.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" ORDER BY S_SECTION') ;
           5,12 : Q.SQL.Add('SELECT BG_BUDGENE AS CODE, BG_LIBELLE AS LIBELLE FROM BUDGENE ORDER BY BG_BUDGENE') ;
           6 : Q.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" ORDER BY BS_BUDSECT') ;
           7 : Q.SQL.Add('SELECT J_JOURNAL AS CODE, J_LIBELLE AS LIBELLE FROM JOURNAL ORDER BY J_JOURNAL') ;
           8 : Q.SQL.Add('SELECT BJ_BUDJAL AS CODE, BJ_LIBELLE AS LIBELLE FROM BUDJAL ORDER BY BJ_BUDJAL') ;
           end ;
        end ;
     ChangeSQL(Q) ;
     Q.Open ;
     ...
     if InitialRec <> '' then
        begin
        Q.Locate('CODE',InitialRec,[]) ;
        bFin.Enabled := True ;
        InitialRec := '' ;
        end Else
        BEGIN
        If FindCode.Text<>'' Then
          BEGIN
          Q.Locate('CODE',FindCode.Text,[]) ;
          END ;
        END ;
     *)
     END ;
...
procedure TFAssistEF.rSourceEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[31] ;
end;

procedure TFAssistEF.cAxeChange(Sender: TObject);
begin
SauveAxe := cAxe.ItemIndex ;
AxeCourant := cAxe.Value ;
end;

procedure TFAssistEF.cAxeEnter(Sender: TObject);
begin
inherited;
lAide.Caption := MsgEF.Mess[32] ;
end;

procedure TFAssistEF.GridRubDblClick(Sender: TObject);
begin
inherited;
if (rCumul.Checked) and (bFin.Enabled) then bFinClick(nil) ;
end;

procedure TFAssistEF.rCumulClick(Sender: TObject);
begin
// BPY le 18/12/2003 => Fiche 13086 active cette coche quelque soit la serie !
{if not(V_PGI.LaSerie=S3) then} GC2.Visible:=TRUE ;
// Fin BPY
...
procedure TFAssistEF.rChampClick(Sender: TObject);
begin
GC2.Visible:=FALSE ;
...
procedure TFAssistEF.FormCreate(Sender: TObject);
begin
inherited;
InitialFormula := '' ;
InitialRec := '' ;
...
procedure TFAssistEF.GridFieldsDblClick(Sender: TObject);
begin
inherited;
if bFin.Enabled then bFinClick(nil) ;
end;

procedure TFAssistEF.DSFieldsDataChange(Sender: TObject; Field: TField);
begin
if not QFields.Active then exit ;
if QFields.EOF then
   begin
   cLibelle.Enabled := False ;
   bFin.Enabled := False ;
   end
else
   begin
   cLibelle.Enabled := (QFields.FindField('DH_TYPECHAMP').AsString = 'COMBO') ;
   if not cLibelle.Enabled then cLibelle.Checked := False ;
   bFin.Enabled := True ;
   end ;
end;

procedure TFAssistEF.rConstanteClick(Sender: TObject);
begin
MULTIDOSSIER.Visible := False;
Regroupement.visible := False;
GC2.Visible:=FALSE ;
...
procedure TFAssistEF.NumPeriodeExit(Sender: TObject);
begin
if NumPeriode.Value > NumPeriode.Max then NumPeriode.Value := 1 ;
end;

procedure TFAssistEF.bValClick(Sender: TObject);
begin
DisplayStep ;
end;

procedure TFAssistEF.bLibClick(Sender: TObject);
begin
DisplayStep ;
end;

procedure TFAssistEF.rBudgetClick(Sender: TObject);
begin
...
procedure TFAssistEF.cCptVariantEnter(Sender: TObject);
begin
  inherited;
...
procedure TFAssistEF.NumPeriodeChange(Sender: TObject);
begin
  inherited;
If NumPeriode1.Value<NumPeriode.Value Then NumPeriode1.Value:=NumPeriode.Value
end;

procedure TFAssistEF.NumPeriode1Exit(Sender: TObject);
begin
  inherited;
if NumPeriode1.Value > NumPeriode1.Max then NumPeriode1.Value := 1 ;
end;

procedure TFAssistEF.DS2DataChange(Sender: TObject; Field: TField);
begin
  inherited;
if not Q2.Active then exit ;
if IsSociete then exit ; // société
...
if Q2.EOF then
   ...
   begin
   lRub.Caption := Q2.FindField('CODE').AsString + ' - ' + Q2.FindField('LIBELLE').AsString ;
   if not rChamp.Checked then bFin.Enabled := True ;
   end ;

end;

procedure TFAssistEF.FindCode2Change(Sender: TObject);
begin
inherited;
...
if rConstante.Checked then
   begin
   Q2.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" AND CC_CODE LIKE "' + FindCode2.Text + '%" ORDER BY CC_CODE') ;
   end
else
   begin
   Case rSource.ItemIndex of
      10 : Q2.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS WHERE T_AUXILIAIRE LIKE "' + FindCode2.Text + '%" ORDER BY T_AUXILIAIRE') ;
      11 : Q2.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" AND S_SECTION LIKE "' + FindCode2.Text + '%" ORDER BY S_SECTION') ;
      12 : Q2.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" AND BS_BUDSECT LIKE "' + FindCode2.Text + '%" ORDER BY BS_BUDSECT') ;
      end ;
   end ;
ChangeSQL(Q2) ;
Q2.Open ;
...
procedure TFAssistEF.FindLib2Change(Sender: TObject);
begin
inherited;
If (rSource.ItemIndex in [10..12])=FALSE Then Exit ;
Q2.Close ;
...
if rConstante.Checked then
   begin
   Q2.SQL.Add('SELECT CC_CODE AS CODE, CC_LIBELLE AS LIBELLE FROM CHOIXCOD WHERE CC_TYPE = "CON" AND CC_LIBELLE LIKE "%' + FindLib2.Text + '%" ORDER BY CC_CODE') ;
   end
else
   begin
   Case rSource.ItemIndex of
      10 : Q2.SQL.Add('SELECT T_AUXILIAIRE AS CODE, T_LIBELLE AS LIBELLE FROM TIERS WHERE T_LIBELLE LIKE "' + FindLib2.Text + '%" ORDER BY T_AUXILIAIRE') ;
      11 : Q2.SQL.Add('SELECT S_SECTION AS CODE, S_LIBELLE AS LIBELLE FROM SECTION WHERE S_AXE = "' + AxeCourant + '" AND S_LIBELLE LIKE "' + FindLib2.Text + '%" ORDER BY S_SECTION') ;
      12 : Q2.SQL.Add('SELECT BS_BUDSECT AS CODE, BS_LIBELLE AS LIBELLE FROM BUDSECT WHERE BS_AXE = "' + AxeCourant + '" AND BS_LIBELLE LIKE "' + FindLib2.Text + '%" ORDER BY BS_BUDSECT') ;
      end ;
   end ;
ChangeSQL(Q2) ;
Q2.Open ;
...
procedure TFAssistEF.RSQLClick(Sender: TObject);
begin
  inherited;
MULTIDOSSIER.Visible := False;
Regroupement.visible := False;
...
procedure TFAssistEF.ChargeAvances(OnLib : Boolean) ;
Var i,j,k : Integer ;
...
procedure TFAssistEF.CBLibClick(Sender: TObject);
begin
  inherited;
ChargeAvances(CBLib.Checked) ;
end;

procedure TFAssistEF.bEffaceAvanceClick(Sender: TObject);
begin
  inherited;
...
Function TFAssistEF.FaitSQLSupp : String ;
...
Mots clefs ... :
*****************************************************************}
Function TFAssistEF.OkAvances(Recharge : Boolean ; ForceIndex : Integer = -1) : tav ;
BEGIN
...
procedure TFAssistEF.rEcrClick(Sender: TObject);
...
procedure TFAssistEF.CCBalSitClick(Sender: TObject);
begin
  inherited;
