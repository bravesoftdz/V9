...
Description .. : Intégration dans la Compta S5 le 05/08/2002
Mots clefs ... :
*****************************************************************}
unit TofBalagee;
...
begin
  // Etat non utilisable sous ORACLE7
    ...
    then PGIInfo( 'Cet état n''est pas utilisable sous Oracle 7. Veuillez utiliser l''état correspondant dans le menu Autres éditions' , 'Balance Agée en situation' )
...
begin
  // Etat non utilisable sous ORACLE7
	if V_PGI.Driver = dbORACLE7
    then PGIInfo( 'Cet état n''est pas utilisable sous Oracle 7. Veuillez utiliser l''état correspondant dans le menu Autres éditions' , 'Balance Ventilée en situation' )
...
If Choix<=-1 Then Choix:=0 ;
   TabD[10]:=StrToDate(FP10.text) ;       { Calcul à partir de la date d'arrêtée }
...
begin
  ...
  case ChoixPeriode of
    ...
    6           :					    // ===> Périodicité de 15 jours
        ...
        for i:=5 downto 1 Do { Pour chaque Fourchette de dates( en partant de la derniére) , en Quinzaine }
           begin
           DAT:=TabD[i+5] ;
           DecodeDate(DAT,an,mois,jour) ;
           JMax:=StrToInt(FormatDateTime('d',FinDeMois(EncodeDate(an,mois,1)))) ;
           if Jour<=15
           	then TabD[i]:=TabD[i+5]-15+1 			{ Date départ = (Date d'arrivée - 15 jours) + 1 jour, si date avant le 15 du mois }
           	else TabD[i]:=TabD[i+5]-(JMax-15)+1 ; { Date départ = (Date d'arrivée - (Nb jours Max du mois - 15 jours) ) + 1 jour, si date aprés le 15 du mois }
           if i>1 then TabD[i+4]:=TabD[i]-1 ;
           end;
    7           :             // ===> Périodicité de 1 semaine
        ...
        for i:=1 to NbPeriodes Do	{ Pour chaque Fourchette de dates( en partant de la première) , en Hebdo }
          ...
          if i<5 then TabD[i+1]:=TabD[i+5]+1 ;
		    ...
		    for i:=5 downto 1 Do  { Pour chaque Fourchette de dates( en partant de la derniére) , en Hebdo }
          begin            		{ Date départ = (Date d'arrivée - (7 jours+ 1 jours) )  }
          TabD[i]:=TabD[i+5]-(7)+1 ;
          if i>1 then TabD[i+4]:=TabD[i]-1 ;
          end ;
