...
Max    :=0; 
{$IFNDEF CISXPGI}
TPar := OpenSQLADO ('Select * from '+DMImport.GzImpPar.TableName +' order by domaine,numero', DMImport.Db );
...
begin
    IF (MENom.Text = '') and  (TV <> nil) and (TV.Selected <> nil) and  (TV.Selected.Level = 2) then
    begin
         PGIInfo ('Renseignez le nom du champ');
         exit;
    end;
    IF (METableName.Text = '') and  (TV <> nil) and (TV.Selected <> nil) and (TV.Selected.Level = 1)  then
    begin
...
begin
{$IFNDEF CISXPGI}
Q := OpenSQLADO ('Select * from '+DMImport.GzImpDomaine.TableName, DMImport.Db );
...
{$ENDIF}
    ...
    TParZE.InsertRecord([ TobF.GetValue('Numero'),
...
//gcvoirtob(TobF);
    TParZE.Edit;
    TParZE.FindField('TableName').asstring    := TobF.GetValue('TableName');
    TParZE.FindField('Nom').asstring         := TobF.GetValue('Nom');
    TParZE.FindField('Contenu').asstring      := TobF.GetValue('Contenu');
    TParZE.FindField('Longueur').asinteger    := TobF.GetValue('Longueur');
    TParZE.FindField('Obligatoire').asstring  := TobF.GetValue('Obligatoire');
    TParZE.FindField('MajusMin').asstring     := TobF.GetValue('MajusMin');
    TParZE.FindField('TypeAlpha').asstring    := TobF.GetValue('TypeAlpha');
    TParZE.FindField('Contenuoblig').asstring := TobF.GetValue('Contenuoblig');
    TParZE.FindField('NomPGI').asstring       := TobF.GetValue('NomPGI');
    TParZE.FindField('Commentaire').asstring      := TobF.GetValue('Commentaire');
    TParZE.FindField('Libre1').asstring       := TobF.GetValue('Libre1');
    TParZE.FindField('Libre2').asstring       := TobF.GetValue('Libre2');
    TParZE.FindField('Libre3').asstring       := TobF.GetValue('Libre3');
    TParZE.FindField('Libre4').asstring       := TobF.GetValue('Libre4');
    TParZE.FindField('Libre5').asstring       := TobF.GetValue('Libre5');
    TParZE.FindField('Libre6').asstring       := TobF.GetValue('Libre6');
    TParZE.FindField('Libre7').asstring       := TobF.GetValue('Libre7');
    TParZE.FindField('Libre8').asstring       := TobF.GetValue('Libre8');
    TParZE.FindField('Libre9').asstring       := TobF.GetValue('Libre9');
    TParZE.FindField('Libre10').asstring      := TobF.GetValue('Libre10');
    cTemp := Copy(TobF.GetValue('Modifiable'), 1, 1);
{$IFNDEF CISXPGI}
    if (cTemp[1] = 'N') or (cTemp[1] = 'O') then
    begin
        if not (V_PGI.PassWord = DayPass(Date)) then
            TParZE.FindField('Modifiable').asinteger := Ord(cTemp[1]) - Ord('N')
            else
            TParZE.FindField('Modifiable').asinteger := 0;
    end
        else
{$ENDIF}
        TParZE.FindField('Modifiable').asinteger := 0;
    ...
    cTemp := Copy(TobF.GetValue('Fige'), 1, 1);
    if (cTemp[1] = 'N') or (cTemp[1] = 'O') then
        TParZE.FindField('Fige').asinteger := Ord(cTemp[1]) - Ord('N')
        else
        TParZE.FindField('Fige').asinteger := 0;
    TParZE.FindField('Alignement').asstring   := TobF.GetValue('Alignement');
    TParZE.FindField('Separateur').asstring   := TobF.GetValue('Separateur');
    TParZE.FindField('Nbdecimal').asstring    := TobF.GetValue('Nbdecimal');
    TParZE.FindField('NumVersion').asstring   := TobF.GetValue('NumVersion');
    TParZE.FindField('Calcul').asstring       := TobF.GetValue('Calcul');
// ajout me
    TParZE.FindField('Famcorresp').asstring   := TobF.GetValue('Famcorresp');
    TParZE.FindField('Ordretri').asstring     := TobF.GetValue('Ordretri');
...
if TParZE = nil then TParZE := TADOTable.Create(Application);
TParZE.Connection :=  DMImport.Db as TADOConnection;
TParZE.TableName := DMImport.GzImpPar.TableName;
if not TParZE.Active then  TParZE.Open;
...
//   Nouvel enregistrement : creation complete.
            ...
            if (cTemp[1] = 'N') or (cTemp[1] = 'O') then
                ...
                TobF.PutValue('Fige', 0);
            ind2 := TParZE.RecordCount + 1;
            while TParZE.Locate('Numero', ind2, [loCaseInsensitive]) do
                Inc(ind2);
        ...
        begin
                if TParZE.Locate('Numero;Domaine', VarArrayOf([TobF.GetValue('Numero'),
...
//   enregistrement modifié : on ne renseigne que les champs modifiés.
            ...
            if not TParZE.Locate('Numero;Domaine', VarArrayOf([TobF.GetValue('Numero'),
                             TobF.GetValue('Domaine')]), [loCaseInsensitive]) then
...
begin
    ...
    if StrLen(PChar(TMELoc.Text)) > int(MELongueur.value) then
    begin
