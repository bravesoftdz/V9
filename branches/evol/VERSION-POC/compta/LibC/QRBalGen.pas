...
uses
  ...
  QR, StdCtrls, hmsgbox, HQuickrp, DB,
  {$IFNDEF DBXPRESS}dbtables,{$ELSE}uDbxDataSet,{$ENDIF}
  Buttons, Hctrls, ExtCtrls,
  Mask, Hcompte, ComCtrls, UtilEdt, HEnt1, Ent1, QRRupt,
...
Procedure TFBalGen.BalGeneZoom(Quoi : String) ;
BEGIN
ZoomEdt(1,Quoi) ;
END ;
...
BEGIN
PP:=FindInsidePanel ;
QR:= TFBalGen.Create(Application) ;
...
procedure TFBalGen.RenseigneCritere;
BEGIN
InHerited ;
...
procedure TFBalGen.ChoixEdition ;
BEGIN
InHerited ;
...
procedure TFBalGen.RecupCritEdt ;
...
Function TFBalGen.CritOk : Boolean ;
Var StEcart : String ;
// CA - 20/12/2001
    BalSit : string;
// CA - 20/12/2001 - FIN
...
Function TFBalGen.ColonnesOK : Boolean ;
BEGIN
...
procedure TFBalGen.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
...
//                    if CritEDT.Bal.PlansCorresp=2 then PrintBand:=(Qr11G_CORRESP2.AsString<>'') ;
   ...
   if (Qr11G_COLLECTIF.AsString='X') And (CritEDT.BAL.FormatPrint.PrColl) Then
      ...
      If CritEdt.Bal.QuelAN<>SansAN Then
         BEGIN
         ExecuteTotCptSolde(QBalC,Qr11G_GENERAL.AsString, CritEDT.Date1,CritEDT.Date1,CritEDT.DeviseSelect,CritEDT.Etab,CritEDT.Exo.Code,TotCpt1M,TotCpt1S,TotCpt1MEURO,TotCpt1SEURO,FALSE,Sommation,CritEdt.Decimale,V_PGI.OkDecE,CritEdt.Monnaie=2,DevEnP,QuelTypeEcr) ;
...
procedure TFBalGen.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFBalGen.DLRuptNeedData(var MoreData: Boolean);
...
Procedure TFBalGen.CalcTotalEdt(AnVD, AnVC, D, C : Double );
...
procedure TFBalGen.BRuptBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=(CritEdt.Bal.RuptOnly<>Sans) ;
end;

procedure TFBalGen.InitDivers ;
BEGIN
InHerited ;
...
procedure TFBalGen.TOPREPORTBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
...
procedure TFBalGen.BOTTOMREPORTBeforePrint(var PrintBand: Boolean; var Quoi: string);
Var MReport : TabTRep ;
begin
  inherited;
...
procedure TFBalGen.FormShow(Sender: TObject);
begin
HelpContext:=7445000 ;
...
procedure TFBalGen.BBilanGenBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=False ;
end;

procedure TFBalGen.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFBalGen.FExerciceChange(Sender: TObject);
begin
FLoading:=TRUE ;
  inherited;
...
procedure TFBalGen.FDateCompta1Change(Sender: TObject);
begin
  inherited;
...
procedure TFBalGen.FSelectCpteChange(Sender: TObject);
begin
  inherited;
VoirQuelAN(FSelectCpte.Value,FExercice.Value,FDateCompta1,FAvecAN) ;
end;

procedure TFBalGen.FAvecANClick(Sender: TObject);
begin
  inherited;
FCol1.Checked:=FAvecAN.Checked ;
end;

procedure TFBalGen.FRupturesClick(Sender: TObject);
begin
  inherited;
...
procedure TFBalGen.FSansRuptClick(Sender: TObject);
begin
  inherited;
...
Procedure TFBalGen.CalculGestion(MO : Array of double ) ;
BEGIN
...
procedure TFBalGen.RecapPourQui(Cpt : String ; LBG : TStringList ; MO : Array of double) ;
Var I      : Integer ;
BEGIN
...
procedure TFBalGen.DLBilNeedData(var MoreData: Boolean);
...
procedure TFBalGen.BGesBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
  // GC - 20/12/2001
  if CritEdt.AvecComparatif then
...
procedure TFBalGen.DLGesNeedData(var MoreData: Boolean);
begin
  inherited;
...
procedure TFBalGen.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
// GC - 20/12/2001
procedure TFBalGen.FAvecComparatifClick(Sender: TObject);
begin
  inherited;
...
// GC - 20/12/2001
procedure TFBalGen.FExercice2Change(Sender: TObject);
var D1, D2 : TDateTime;
begin
  inherited;
...
procedure TFBalGen.OnTypeComparatifClick(Sender: TObject);
begin
  inherited;
  ...
  //GBComparatif2.Enabled := RBCOMPBALSIT.Checked and VH^.TenueEuro and (not (FMontant.ItemIndex=0));
  GBComparatif2.Enabled := RBCOMPBALSIT.Checked;
  CritEdt.CompareBalSit := RBCOMPBALSIT.Checked;
  if CritEdt.CompareBalSit then CritEdt.BalSit := FBALSIT.Text;
end;

procedure TFBalGen.FBALSITChange(Sender: TObject);
begin
  inherited;
  TFLIBELLEBALSIT.Caption := '';
...
procedure TFBalGen.FBALSITElipsisClick(Sender: TObject);
begin
  inherited;
{$IFNDEF IMP}
...
procedure TFBalGen.DLRUPTTRINeedData(var MoreData: Boolean);
...
procedure TFBalGen.BRuptTRIBeforePrint(var PrintBand: Boolean;
  var Quoi: String);
begin
  inherited;
...
procedure TFBalGen.BDetailAfterPrint(BandPrinted: Boolean);
begin
  inherited;
BDetail.ForceNewPage:=FALSE ;
end;

procedure TFBalGen.BRuptAfterPrint(BandPrinted: Boolean);
begin
  inherited;
Brupt.ForceNewPage:=FALSE ;
end;

procedure TFBalGen.FOkRegroupClick(Sender: TObject);
Var QQ : TQuery ;
begin
  inherited;
...
procedure TFBalGen.FRegroupDblClick(Sender: TObject);
Var ST : String ;
begin
  inherited;
St:=FRegroup.Text ;
