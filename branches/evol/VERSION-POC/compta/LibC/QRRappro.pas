...
Créé le ...... : 20/02/2003
Modifié le ... :   /  /    
Description .. : Remplacé en eAGL par CPRAPPRO_TOF.PAS
Mots clefs ... : 
*****************************************************************}
unit QRRappro;
...
uses
  ...
  CpteUtil, DBCtrls, HTB97, HPanel, UiUtil, HRichEdt, tCalcCum;
...
type
  ...
  private
    ...
    QR2E_GENERAL, QR2E_JOURNAL, QR2E_REFINTERNE, QR2E_LIBELLE, QR2E_EXERCICE, QR2E_REFPOINTAGE : TStringField ;
    QR2E_DATECOMPTABLE,QR2E_DATEPOINTAGE           : TDateTimeField ;
...
PasDeMvt:=TRUE ;
Q.Close ; Q.SQL.Clear ;
Q.SQL.Add('Select E_GENERAL, E_JOURNAL, E_DATECOMPTABLE, E_NUMEROPIECE, E_REFINTERNE, E_LIBELLE, E_EXERCICE, E_NUMLIGNE, E_DEVISE, E_REFPOINTAGE, E_DATEPOINTAGE, ');
...
Q.SQL.Add(' E_QUALIFPIECE="N" and E_ECRANOUVEAU<>"OAN" and E_ECRANOUVEAU<>"C" ');
If Not CritEdt.Rap.Tout Then Q.SQL.Add(' AND ((E_REFPOINTAGE<>"" and E_DATEPOINTAGE>"'+USDATETIME(CritEdt.Date2)+'") or (E_REFPOINTAGE=""))');
...
begin
  inherited;
QRAvert.Visible:=PasDeMvt ; FAvert.Visible:=PasDeMvt ;
...
While Not QRef.EOF do
   ...
   if QRef.FindField('EE_DATEPOINTAGE').AsDateTime=CritEdt.Date2 then
      BEGIN
        //==========
        // BPY le 06/10/2003 - 12757 puis 13321
      If CritEdt.Monnaie = 0 Then
        BEGIN
...
begin
  inherited;
If CritEdt.Rap.Tout Then
  BEGIN
  OkOK:=((QR2E_REFPOINTAGE.AsString<>'') AND (QR2E_DATEPOINTAGE.AsDateTime>CritEdt.Date2)) Or (QR2E_REFPOINTAGE.AsString='') ;
//  Q.SQL.Add(' ((E_REFPOINTAGE<>"" and E_DATEPOINTAGE>"'+USDATETIME(CritEdt.Date2)+'") or (E_REFPOINTAGE=""))');
  If Not OkOk Then BEGIN PrintBand:=FALSE ; Exit ; END ;
  END ;
...
InitSoldes ;
QL:=OpenSQL('SELECT G_LIBELLE FROM GENERAUX WHERE G_GENERAL="'+CritEdt.Cpt1+'"',TRUE) ;
If Not QL.Eof Then FLibelCpt.Caption:=QL.Fields[0].AsString Else FLibelCpt.Caption:='' ;
Ferme(QL) ;
END ;
