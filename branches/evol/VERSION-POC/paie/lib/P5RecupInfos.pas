...
PT10: 12/07/2005 SB V_60 FQ 12308 Modification de l'ordre de présentation des bulletins
PT11: 14/04/2006 PH V_65 FQ 11914 Les lignes de saisie par rubrique sont reprises par rapport aux dates du bulletin et non début/fin de mois
PT12: 02/06/2006 SB V_65 Optimisation mémoire
PT13: 15/12/2006 PH V_70 FQ 13419 Saisie par rubrique en dehors date entree du salarié
PT14 : 17/01/2007 GGU V_80 FQ 13404 Saisie par rubrique des bulletins complémentaires
PT15: 26/04/2007 VG V_72 FQ N°14183
PT16: 04/09/2007 PH V_80 FQ N°14728
PT17: 05/09/2008 JS Optimisation GLB
...
{$ENDIF}
  ...
  ExtCtrls, Grids, ImgList, Mask, ClipBrd, richedit, UTOB, P5Def,
  ParamSoc;

procedure RecupereHistoSaisRub(Tob_PSD: tob; DateD, Datef: tdatetime; Salarie: string; ForceDate: Boolean = FALSE; OriginePriseEnCompte: string = 'SRB'); // PT11  //PT14
...
function RecupereNouvellesHistoSaisRub(Tob_PSD: tob; DateD, Datef: tdatetime; Salarie: string; OriginePriseEnCompte: string = 'SRB'): boolean; //PT14
...
{PT15
      ...
      if (AncLib <> '') and (OrigineMvt = 'SRB') and (OrigineMvt = OrigineMvtP) then // ######
//FIN PT15
      begin
...
function WhereOrigine(OriginePriseEnCompte: string): string;
begin
  if (OriginePriseEnCompte = 'SRB') then
    result := ' AND PSD_ORIGINEMVT <> "BCP" ';
  if (OriginePriseEnCompte = 'BCP') then
    result := ' AND PSD_ORIGINEMVT = "BCP" ';
end;

{//PT14
Ajout du parametre OriginePriseEnCompte initialisé à 'SRB' dans la déclaration
de la procédure. Si OriginePriseEnCompte= 'SRB', on ne prends pas en compte les
lignes ayant 'BCP' (comme à l'origine), si OriginePriseEnCompte = 'BCP', on ne
traite que les lignes avec 'BCP'
}

procedure RecupereHistoSaisRub(Tob_PSD: tob; DateD, Datef: tdatetime; Salarie: string; ForceDate: Boolean = FALSE; OriginePriseEnCompte: string = 'SRB');
var
  st: string;
  // XP 10.10.2007 Q: tQuery;
  DD, DF: TDateTime; // PT11
begin
// DEB PT11
  if not ForceDate then
  begin
    DD := DateD;
    Df := DateF;
  end
  ...
  st := 'SELECT * FROM HISTOSAISRUB LEFT JOIN REMUNERATION ON PRM_RUBRIQUE = PSD_RUBRIQUE AND ##PRM_PREDEFINI## WHERE PRM_THEMEREM <> "ZZZ" AND' +
    ' PSD_SALARIE = "' + Salarie + '"' +
    ' AND (PSD_DATEDEBUT >="' + usdatetime(DD) + // PT11 DebutdeMois
    '" AND PSD_DATEFIN <= "' + usdatetime(DF) + '")' + // PT11 FindeMois
...
// PT4 : 01/12/2003 PH V620 Prise en compte en saisie par rubrique des lignes de type MLB provenant fichier import
  ' AND ((PSD_BASE <> 0 OR PSD_TAUX <> 0 OR PSD_COEFF <> 0 OR PSD_MONTANT <> 0) OR (PSD_ORIGINEMVT = "MLB"))'
    + WhereOrigine(OriginePriseEnCompte) //PT14
...
{Flux optimisé
  ...
  Tob_PSD.LoadDetailDBFROMSQL('HISTOSAISRUB', St);
(*PT17
  if Tob_PSD.detail.count <= 0 then
//  else // DEB PT13 Flux optimisé
  begin
//    Ferme(Q);
    st := 'SELECT * FROM HISTOSAISRUB LEFT JOIN REMUNERATION ON PRM_RUBRIQUE = PSD_RUBRIQUE AND ##PRM_PREDEFINI## WHERE PRM_THEMEREM <> "ZZZ" AND' +
      ' PSD_SALARIE = "' + Salarie + '"' +
      ' AND (PSD_DATEDEBUT >="' + usdatetime(DebutdeMois(DD)) +
      '" AND PSD_DATEFIN <= "' + usdatetime(FindeMois(DF)) + '")' +
      ' AND PSD_DATEINTEGRAT <="' + usdatetime(10) + '"' +
      ' AND ((PSD_BASE <> 0 OR PSD_TAUX <> 0 OR PSD_COEFF <> 0 OR PSD_MONTANT <> 0) OR (PSD_ORIGINEMVT = "MLB"))'
      + WhereOrigine(OriginePriseEnCompte) //PT14
      + ' ORDER BY PSD_ORIGINEMVT,PSD_RUBRIQUE';
{Flux optimisé
    Q := OpenSql(st, TRUE);
    if not (Q.eof) then Tob_PSD.LoadDetailDB('HISTOSAISRUB', '', '', Q, False);
    Ferme(Q);
}
    Tob_PSD.LoadDetailDBFROMSQL('HISTOSAISRUB', St);
  end; // FIN PT13
...
function RecupereNouvellesHistoSaisRub(Tob_PSD: tob; DateD, Datef: tdatetime; Salarie: string; OriginePriseEnCompte: string = 'SRB'): boolean;
var
  st: string;
  // XP 10.10.2007 Q: tQuery;
  t: tob;
...
{Flux optimisé
  Q := OpenSql(st, TRUE);
  if (Q.eof) then result := false
  else
  begin
    Ferme(Q);
}
  Tob_PSD.LoadDetailDBFROMSQL('HISTOSAISRUB', St);
  if Tob_PSD.Detail.count <= 0 then result := FALSE  // PT16 correction  du test à l'envers
  else
  begin
    //PT6 : 05/12/2003 PH V620 on exclut les rubriques de themes net à payer
...
{Flux optimisé
    ...
    Tob_PSD.LoadDetailDBFROMSQL('HISTOSAISRUB', st);
    if (Assigned(Tob_PSD)) and (Tob_PSD.detail.count > 0) then //notVide(Tob_PSD, true) then  PT12
    begin
      result := true;
...
begin
  ...
  begin
    ...
    begin
      ...
      if assigned(Tob_PSP) and (Tob_PSP.detail.count > 0) then // if notVide(Tob_PSP, true) then PT12
      begin
        result := true;
...
var
  TW: tob;
begin
  // on supprime d'abord les lignes commentaires crées
...
begin
  ...
  begin
    ...
    THH.PutValue('PHB_OMTSALARIAL', TR.GetValue('PRM_ORDREETAT')); { PT10 }
  end;
end;
