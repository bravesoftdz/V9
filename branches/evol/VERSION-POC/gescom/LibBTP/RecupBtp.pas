...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, (*MemCheck,*)
  StdCtrls,Hent1, HCtrls,ComCtrls, HRichEdt, HRichOLE,UTob, UTom, ParamSoc,
  ExtCtrls, Grids, ImgList, HStatus, Spin, HPanel, UiUtil,
  voirtob,
  variants,
  Buttons, IniFiles, Math, Ent1, EntGC, VisuLogbTP, UtilArticle, AssistUtilBtp, RecupNegSQL7,TarifUtil,UtilPgi,UtilImport,
{$IFDEF EAGLCLIENT}
     MaineAGL
{$ELSE}
     DB,
...
type
  TFRecupbtp = class(TForm)
    ...
    Repertoire: TEdit;
    bStop: TBitBtn;
    bAjouterTout: TBitBtn;
    bEnleverTout: TBitBtn;
...
//    st_Corresp : string;
    ...
    procedure Traiter_Tablette(ChampAssocie, TabLibelle, TabAbrege,Tablette : string);
    ...
    procedure Extraire_Champ(i_ind1 : Integer; NomChampTable, st_trav1 : string; var ValeurSeq : Variant;
                             var ChampSeq : Variant; Var Tablette : variant);
    ...
    procedure Conformite_table(ToTable: TOB);
    procedure COnformiteArticle(TOBRef: TOB; ChampsACalc: array of string;
      MonnaieValeur: string);
    procedure MessageErreur(MessageR: string);
    function RecupModeTrait(chaine: string): string;
    procedure COnformiteTiers(TOBRef: TOB);
    procedure ConformiteArticleTiers(TOBRef: TOB);
    procedure ConformiteCatalogue(TOBRef: TOB);
    procedure ConformiteEntOuvrage(TOBRef: TOB);
    procedure ConformiteLigOuvrage(TOBRef: TOB; MonnaieValeur: string);
  public
...
Procedure RecuperationBtp (WorkingPath : string) ;
procedure Ecrire_DatasbTP(ItemInd : integer; NomChampTable : string; NumTable, NumeroChamp : integer;
...
begin
    ...
    Fo_Recup := TFRecupBtp.Create (Pn_Inside);
...
procedure TFRecupbtp.Conformite_table (ToTable : TOB);
begin
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'GA' then
   begin
   // Table article
   COnformiteArticle(TOBTable,['DPR','PVHT','PVTTC'],ParamGenData.Strings[ParamGenCle.IndexOf('Monnaie')]);
   end;
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'GAT' then
   begin
   COnformiteArticleTiers(TOBTable);
   end;
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'GCA' then
   begin
   COnformiteCatalogue(TOBTable);
   end;
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'GNE' then
   begin
   COnformiteEntOuvrage(TOBTable);
   end;
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'GNL' then
   begin
   COnformiteLigOuvrage(TOBTable,ParamGenData.Strings[ParamGenCle.IndexOf('Monnaie')]);
   end;
if V_PGI.DETables [TOBTable.NumTable].Prefixe = 'T' then
   begin
   COnformiteTiers (TOBTable);
   end;
...
-----------------------------------------------------------------------------}
procedure TFRecupbtp.ImportEnreg ( TTableRecup : TOB; MAJ : string );
var
    vt_trav1 : variant;
    st_trav1, st_Log, CleTable, EnregPGI, OrdreSQL, OrdreSQLCre, OrdreSQLMaj ,cle1,CleTable1: string;
    i_ind1 : integer;
...
//if (MAJ[1] = 'P') or (MAJ[1] = 'M') or (MAJ[2] = 'M') then
    ...
    for i_ind1 := 0 to StringGridAsso.RowCount - 1 do
        ...
        if Pos('Tablette', st_trav1) = 0 then
            begin
            Ecrire_DatasBTP(RGSortie.ItemIndex, st_trav1, TobTable.NumTable, NumChArray[i_ind1],
...
//if (MAJ[1] = 'C') or (MAJ[2] = 'C') then
    ...
    SQL_Where  := '';
    SQL_Sep    := '';
    // Traitement De Mise en conformite des tables
    Conformite_table (TobTable);
    for i_ind1 := 1 to TobTable.NbChamps do
        if V_PGI.DEChamps[TobTable.NumTable, i_ind1].Tipe <> 'BLOB' then
            ...
            Ecrire_Datasbtp(RGSortie.ItemIndex, st_trav1, TobTable.NumTable, i_ind1,
                         vt_trav1, SQL_Champs, SQL_Datas, SQL_Sep, BlobPresent);
            end else
            begin
            st_trav1 := TobTable.GetNomChamp(i_ind1);
            vt_trav1 := TobTable.GetValeur(i_ind1);
            if length (vt_trav1) <> 0 then BlobPresent := true;
            end;
...
if RGSortie.ItemIndex = 0 then
    ...
    //if (MAJ[2]='M') and (MAJ[1]='C') then EnregExist := TobTable.existDB;
    ...
    if (MAJ[1] = 'C') and (MAJ[2] = 'C')  then
        ...
        if (MAJ[1] = 'P') then
            ...
            begin
            if (MAJ[2] = 'M') and EnregExist then
                begin
                OrdreSQL := OrdreSQLMaj;
                VerifieSQL(OrdreSQLMaj);
                ExecuteSQL(OrdreSQLMaj);
...
-----------------------------------------------------------------------------}
procedure TFRecupbtp.BRecupClick (Sender: TObject);
begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bTestClick(Sender: TObject);
begin
    Controle_Ordre_De_Traitement;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Traitement;
var
...
// Modif LS
        ModeGestionTable := RecupModeTrait(ListeATraiter.Items.Strings[i_Ind1]);
//
...
//
            ...
            if (st_trav1 = 'O') and ((st_trav2 = 'O') or (ModeGestionTable ='') or(ModeGestionTable='Création'))  and (i_ind2 = 0) then
//  mode creation et premier fichier de la liste, vide la table et les index
                begin
                if (ModeGestionTable <> 'Modification') and (ModeGestionTable <> 'Partielle') then
                   begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bVisuClick(Sender: TObject);
var
    T1 : TOB;
    FicIni : TIniFile;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bLogClick(Sender: TObject);
    var i_ind1 : integer;
        fdoubles : string;
        SearchRec: TSearchRec;
begin
    FVisuLogbTP := TFVisuLogbTp.Create(FVisuLogbTP);
    FVisuLogbTP.ListBox1.Items.Text := ListeATraiter.Items.Text;
    FVisuLogBTP.ListBox1.ItemIndex := ListeATraiter.ItemIndex;
//{$IFDEF GRC} Modif 06/02/02 rapprochement GC - GRCAB
    ...
    begin
      ...
      if ( (FindFirst(st_chemin +fdoubles+ '.log', faAnyFile, SearchRec) = 0)
        ...
        FVisuLogBTP.ListBox1.Items.append(fdoubles);
    end;
//{$ENDIF} Fin Modif 06/02/02 rapprochement GC - GRCAB
    FVisuLogBTP.CheminLog.text := st_chemin;
    FVisuLogBTP.ShowModal;
    FVisuLogBTP.free;
    FVisuLogBTP :=Nil;
end;

//----------------------------------------------------------------------------
procedure TFRecupbtp.HGrid1RowEnter(Sender: TObject; Ou: Integer;
  var Cancel: Boolean; Chg: Boolean);
  var T2 : TOB;
begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFRecupbtp.FormCreate(Sender: TObject);
var
    SearchRec: TSearchRec;
    FicIni : TIniFile;
    i_ind1 : integer;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.FormShow(Sender: TObject);
var
    SearchRec: TSearchRec;
    FicIni : TIniFile;
    i_ind1 : integer;
...
{$ENDIF}
    ...
    FicIni.ReadSection('General', ParamGenCle);
    for i_ind1 := 0 to ParamGenCle.Count - 1 do
        ParamGenData.Append(FicIni.ReadString('General', ParamGenCle.Strings[i_ind1], ''));
    FicIni.Free;
    ...
    FicIni.ReadSectionValues('Fichier', ListeOptions.Items);
    FicIni.Free;
    //
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bAjouterClick(Sender: TObject);
var
    i_ind1 : integer;
    st_trav1,opt_trait : string;
begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bEnleverClick(Sender: TObject);
var
    i_ind1 : integer;
    st_trav1,opt_trav1 : string;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bStopClick(Sender: TObject);
begin
    l_Arret := True;
end;

//----------------------------------------------------------------------------
procedure TFRecupbtp.bAjouterToutClick(Sender: TObject);
begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.bEnleverToutClick(Sender: TObject);
begin
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.ListePresentDblClick(Sender: TObject);
begin
    bAjouterClick(Sender);
end;

//----------------------------------------------------------------------------
procedure TFRecupbtp.ListeATraiterDblClick(Sender: TObject);
begin
    bEnleverClick(Sender);
end;

//----------------------------------------------------------------------------
procedure TFRecupbtp.Charge_Ini;
var
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Charge_Dict(i_indice : integer; var Traite : Boolean);
var
    Fichier: textfile;
    st_enreg, st_trav1, st_trav2, st_file, st_racine, st_log,st_MAJ,ValRet : string;
...
//
        ...
        // Verif code champs avec fichier texte
            ...
            begin
            if ChampTablette(StringGridFichier.Cells[2, i_nrow]) <> ChampTablette (st_trav2) then
               begin
               ValRet := StringGridFichier.Cells[2, i_nrow];
               AffecteTablette (ValRet,ChampTablette(st_trav2),False,False);
               StringGridAssoStruct.Cells[i_ind2, i_ind1]:= ValRet;
               end else StringGridAssoStruct.Cells[i_ind2, i_ind1] := StringGridFichier.Cells[2, i_nrow];
//                StringGridAssoStruct.Cells[i_ind2, i_ind1] := StringGridFichier.Cells[2, i_nrow];
            end;
...
//----------------------------------------------------------------------------------
function TFRecupbtp.VerifCorrespondance (i_nb_champs : integer;
                                         st_champs : string;
                                         var i_nrow : integer) : boolean;
var
    i_ind : integer;
...
//----------------------------------------------------------------------------------
procedure TFRecupbtp.Ferme_Dict;
begin
    CloseFile (FicLog);
    CloseFile (ErreurLog);
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Traiter_Donnees(i_indfic : Integer);
var
    ...
    ValeurSeq, ChampSeq,Tablette : Variant;
    i_ind1, i_ind2, i_sizefic : integer;
    i_decimale : integer;
    i_PInt, i_PDec : integer;
    Searchrec : TSearchRec;
...
//  Fin Modif 06/02/02 rapprochement GC - GRC
    ...
    ModeGestionTable := RecupModeTrait(Repertoire.Text);
    if ModeGestionTable = '' then st_MAJ := st_trav1 + st_MAJ
                             else BEGIN
                                  if ModeGestionTable = 'Création' then St_MAJ := st_trav1+'C'
                                  else if ModeGestionTable = 'Modification' then St_MAJ := st_trav1+'M'
                                  else St_MAJ := st_trav1+'P';
                                  END;
...
//  fin Modif 07/02/02 rapprochement GC - GRC  clé sur plusieurs champ table
            ...
            begin
                ...
                begin
                    ...
                    Extraire_Champ(1, '', st_trav2, ValeurSeq, ChampSeq,Tablette);
                ...
                begin
                    ...
                    begin
                        if Copy(NomChampSeq, 4, 5) = 'PARAM' then
                            ...
                            st_trav1 := StringGridAssoStruct.Cells[i_ind2, i_ind1];
                        Extraire_Champ(i_ind2, NomChampTable, st_trav1, ValeurSeq, ChampSeq,Tablette);
...
//  Fin Modif 06/02/02 rapprochement GC - GRC
                    begin
                        ...
                        begin
                            ...
                            i_decimale := 0;
                        case i_decimale of
                        0 : ValeurSeq := StrToInt(ValeurSeq);
                        1 : ValeurSeq := i_PInt + (i_PDec / 10);
                        2 : ValeurSeq := i_PInt + (i_PDec / 100);
                        3 : ValeurSeq := i_PInt + (i_PDec / 1000);
                        4 : ValeurSeq := i_PInt + (i_PDec / 10000);
                        5 : ValeurSeq := i_PInt + (i_PDec / 100000);
                        6 : ValeurSeq := i_PInt + (i_PDec / 1000000);
                        7 : ValeurSeq := i_PInt + (i_PDec / 10000000);
                        8 : ValeurSeq := i_PInt + (i_PDec / 100000000);
                        9 : ValeurSeq := i_PInt + (i_PDec / 1000000000);
                        end;
                    end;
...
//  traitement des champs definis en tablette
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    if st_trav1 <> '' then
                    begin
                        Extraire_Champ(2, NomChampTable, st_trav1, ValeurSeq, ChampSeq,Tablette);
                        if VarType(ChampSeq) = varString then
                            TabAbrege := TrimRight(ChampSeq)
                    end;
                    st_trav1 := StringGridAssoStruct.Cells[1, i_ind1];
                    Extraire_Champ(1, NomChampTable, st_trav1, ValeurSeq, ChampSeq,Tablette);
                    if VarType(ChampSeq) = varString then
    ...
    //  si le champ associe est bien dans le fichier en cours de traitement...
                    if Copy(ValeurSeq, 0, 3) = st_racine then
                        Traiter_Tablette(ValeurSeq, TabLibelle, TabAbrege,tablette);
                end;
            end;
        end;
        if l_Arret then Break;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Ecrire_Tob(NumChamp : integer; ValeurSeq : Variant);
var
   st1, st2 : string;
begin
try
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Extraire_Champ(i_ind1 : Integer; NomChampTable, st_trav1 : string; var ValeurSeq : Variant;
                                    var ChampSeq : Variant; Var Tablette : variant);
var
    st_format, st_chassoc, st_trav2,st_Tablette : string;
    i_offset, i_longueur, i_decimale : integer;
    i_PInt, i_PDec, i_trav1 : integer;
...
//  recupere les differentes infos necessaires
        ...
        st_tablette := ReadTokenSt(st_trav1);
        ...
        begin
            ...
            if Copy(NomChampTable, 0, 8) = 'Tablette' then
               begin
               ValeurSeq := st_chassoc;
               Tablette := st_tablette;
               end;
            Exit;
        end;
        if st_format = 'D' then
...
//  Date a 0 : on met le 31/12/2099 par defaut
        ...
        begin
            ...
            case i_decimale of
            0 : ChampSeq := i_PInt;
            1 : ChampSeq := i_PInt + (i_PDec / 10);
            2 : ChampSeq := i_PInt + (i_PDec / 100);
            3 : ChampSeq := i_PInt + (i_PDec / 1000);
            4 : ChampSeq := i_PInt + (i_PDec / 10000);
            5 : ChampSeq := i_PInt + (i_PDec / 100000);
            6 : ChampSeq := i_PInt + (i_PDec / 1000000);
            7 : ChampSeq := i_PInt + (i_PDec / 10000000);
            8 : ChampSeq := i_PInt + (i_PDec / 100000000);
            9 : ChampSeq := i_PInt + (i_PDec / 1000000000);
            end;
//            ChampSeq := d_valeur;
            Exit;
        end;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Traiter_Tablette(ChampAssocie, TabLibelle, TabAbrege,Tablette : string);
var
    st_trav1, st_trav2, st_trav3, st_Champ : string;
    i_ind1, i_offset, indice : integer;
    l_Trouve : boolean;
...
*)
    TobTemp := Nil;
    For indice :=0 to Tob_DeCombos.detail.count -1 do
    begin
      if Pos (st_trav1,Tob_DeCombos.detail[Indice].GetValue('DO_NOMCHAMP')) > 0 then
      begin
        TobTemp := Tob_DeCombos.detail[Indice];
        Break;
      end;
    end;
...
{=============================================================================}
procedure TFRecupbtp.Champ_Tablette(NomChampTable, Valeur : string; i_ind1 : integer);
var
...
{=============================================================================}
procedure TFRecupbtp.Recherche_Tablette(NomChampTable : string; i_ind1 : integer);
var
    st_trav1 : string;
    Query1 : TQuery;
    TobTemp : TOB;
...
{=============================================================================}
function TFRecupbtp.PresenceComp (Fichier,Champs,Valeurs : String ) : Boolean ;
Var
    Q : TQuery ;
    st_trav1 : string;
...
{=============================================================================}
procedure TFRecupbtp.Enreg_Log_Erreur;
var
    i_ind1 : integer;
    st_trav1 : string;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Traiter_Champ_NEGOCE(NomChampTable : string; NomChampSeq : string;
...
//----------------------------------------------------------------------------
procedure TFRecupbtp.Traiter_Champ_PGI(NomChampTable : string; NomChampSeq : string;
                                       var ValeurSeq : Variant);
var
...
//  Code article
    ...
    else if (st_trav3 = 'NOMENCLATURE') and (st_trav2 <> 'GNE') and (st_trav2 <> 'GNL') then
    begin
...
procedure TFRecupbtp.Controle_Ordre_De_Traitement;
begin
...
//  Celui ci.
function TFRecupbtp.Incremente(NomTable, NomChamp : string; Remplace : Boolean;
                               var ValeurSeq : string) : Integer;
var
    i_increment : integer;
    st_trav1, st_trav2 : string;
...
procedure TFRecupbtp.Traiter_Calcul(NomChampTable : string; var ValeurSeq : Variant);
var
    ...
    st_trav1, st_trav2, st_trav3 : string;
    ch_trav1 : char;
    va_temp1, va_temp2,Tablette : variant;
...
//  On peut traiter la formule, le résultat étant mis dans un double
    ...
    begin
        ...
        if st_types[i_variables] = 'F' then
            Extraire_Champ(1, '',st_variables[i_variables], va_temp2, va_temp1,tablette);
...
function  TFRecupbtp.Evaluer_Si(ValeurSeq, NomChampTable : string) : Variant;
var
...
function  TFRecupbtp.Evaluer_Condition(TSI : TOB; NomChampTable : string) : boolean;
var
    TobTemp : TOB;
    Result_Inter : boolean;
    i_ind1 : integer;
    ValeurSeq, ChampSeq1, ChampSeq2,tablette : Variant;
...
if not FctPredefinie(Operande1, NomChampTable, ChampSeq1) then
    ...
    if IsNumeric(Operande1) then
        ...
        Extraire_Champ(0, Operande1, '', ValeurSeq, ChampSeq1,tablette)
    else
        begin
        Result := False;
        Exit;
...
if not FctPredefinie(Operande2, NomChampTable, ChampSeq2) then
    ...
    if IsNumeric(Operande2) then
        ...
        Extraire_Champ(0, Operande2, '', ValeurSeq, ChampSeq2,tablette)
...
if VarType(ChampSeq2) = varString then
    ...
    else if ChampSeq2 = 'VIDE' then champseq2 := '';
    end;
...
function  TFRecupbtp.Evaluer_Valeur(TSI : TOB; NomChampTable : string) : variant;
var
        ...
        if Operateur <> '' then
            ...
            if not FctPredefinie(Operande1, NomChampTable, ChampSeq1) then
                ...
                if Operande1 = 'Parenthese' then
                    ...
                    if ChampSeq1 = 'VIDE' then ChampSeq1 := '';
                    end
                    else
                    begin
                    ChampSeq1 := Operande1;
...
function  TFRecupbtp.FctPredefinie(st_trav1, NomChampTable : string; var ValeurSeq : Variant) : boolean;
var
    st_trav2 : string;
...
function  TFRecupbtp.Fct_MAJUSCULE(var Syntaxe : string) : variant;
var
    ValeurSeq : variant;
...
function  TFRecupbtp.Fct_COMPTEUR(NomChampTable : string) : variant;
var
    Q : TQuery;
...
function  TFRecupbtp.Fct_REPLACECAR(var Syntaxe : string) : Variant;
var
...
function  TFRecupbtp.Fct_CORRESPOND(NomChampTable, NomChampSeq : string) : Variant;
var
    i_ind1 : integer;
    ValeurSeq, ChampSeq,tablette : variant;
...
begin
ValeurSeq := ' ';
Extraire_Champ(0, NomChampSeq, '', ValeurSeq, ChampSeq,tablette);
Result := ChampSeq;
Valeur := ChampSeq;
...
function  TFRecupbtp.Fct_TABLE(NomChampTable, NomChampSeq : string) : Variant;
var
...
function  TFRecupbtp.Fct_SOUSCHAINE(Syntaxe : string) : Variant;
var
    Valeur, st1 : string;
...
function  TFRecupbtp.Fct_SPACE(Syntaxe : string) : Variant;
var
...
function  TFRecupbtp.Fct_INTSTR(Syntaxe : string) : Variant;
begin
Result := RecupValeur(Syntaxe);
end;

function  TFRecupbtp.Fct_STRINT(Syntaxe : string) : Variant;
begin
Result := RecupValeur(Syntaxe);
end;

function  TFRecupbtp.Fct_DATE(Syntaxe : string) : Variant;
begin
Result := StrToDate(Syntaxe);
end;

function  TFRecupbtp.RecupValeur(Syntaxe : string) : Variant;
var
    Valeur, st1 : string;
    ValeurSeq, ChampSeq,tablette : variant;
...
if Pos('+', Syntaxe) = 0 then
    begin
    Extraire_Champ(0, Syntaxe, '', ValeurSeq, ChampSeq,tablette);
    Result := ChampSeq;
    end
    else
    begin
    ...
    while Valeur <> '' do
        ...
        Extraire_Champ(0, st1, '', ValeurSeq, ChampSeq,Tablette);
        if VarType(ChampSeq) = varString then
...
procedure TFRecupbtp.Prepare_Dispo(var traite : boolean);
var
...
//  on extrait le stock initial du champ STO0400
            st1 := StringGridFichier.Cells[2, i_ind1];
            Extraire_Champ(1, '', st1, ValeurSeq, ChampSeq,Tablette);
            StkPhy := ChampSeq;
...
procedure TFRecupbtp.GereConvertError(NomChampTable : string; ValeurSeq : Variant);
var
...
procedure TFRecupbtp.GereEngineError(NomTable, SQL : string);
var
   st_log, st_trav1, st_trav2 : string;
...
procedure TFRecupbtp.VerifieSQL(var Sql : string);
begin
{
if Pos('''', Sql) <> 0 then
    while Pos(':', Sql) <> 0 do
...
Procedure TFRecupbtp.COnformiteArticle (TOBRef:TOB;ChampsACalc:array of string;MonnaieValeur:string) ;
var CodeBase,CodeArrondi,CurChamp: string ;
...
BEGIN
BlocNote := '';
TOBREF.PutValue('GA_ARTICLE',Trim(TOBRef.GetValue('GA_ARTICLE')));
for indice :=0 to length(ChampsAcalc) -1 do
    begin
    CurChamp := ChampsACalc[Indice];
    if (CurChamp = 'DPR') then
       begin
       CodeBase:=TOBRef.getValue('GA_CALCPRIXPR') ;
       CoefCalc:=TOBRef.getValue('GA_COEFFG') ;
       CodeArrondi:=''; ;
       CalCule := (TOBRef.GetValue('GA_DPRAUTO')='X');
       end else if (CurChamp = 'PVHT') then
       begin
       CodeBase:=TOBRef.getValue('GA_CALCPRIXHT') ;
       CoefCalc:=TOBRef.getValue('GA_COEFCALCHT') ;
       CodeArrondi:='' ;
       CalCule := (TOBRef.GetValue('GA_CALCAUTOHT')='X');
       end else if (CurChamp = 'PVTTC') then
       begin
       CodeBase:=TOBRef.getValue('GA_CALCPRIXTTC') ;
       CoefCalc:=TOBRef.getValue('GA_COEFCALCTTC') ;
       CodeArrondi:='' ;
       CalCule := (TOBRef.GetValue('GA_CALCAUTOTTC')='X');
       end;

    If (Calcule) and (CoefCalc<>0) and (CodeBase<>'') and (CodeBase<>'AUC') then
        Begin
        if CodeBase='DPA' then PrixRef:=TOBRef.getValue('GA_DPA')
        else if CodeBase='DPR' then PrixRef:=TOBRef.getValue('GA_DPR')
        else if CodeBase='HT' then PrixRef:=TOBRef.getValue('GA_PVHT')
        else if CodeBase='PAA' then PrixRef:=TOBRef.getValue('GA_PAHT')
        else if CodeBase='PRA' then PrixRef:=TOBRef.getValue('GA_PRHT')
        else if CodeBase='PMA' then PrixRef:=TOBRef.getValue('GA_PMAP')
        else if CodeBase='PMR' then PrixRef:=TOBRef.getValue('GA_PMRP') ;
        BaseTarif:=PrixRef*CoefCalc ;
        if BaseTarif <> 0 then
           begin
           if CodeArrondi <> '' then BaseTarif:=ArrondirPrix(CodeArrondi,BaseTarif) ;
           TOBRef.putValue('GA_'+CurChamp,BaseTarif) ;
           end;
        End ;
    end;
if (VH^.TenueEuro) and (MonnaieValeur <> 'E' ) then
   begin
   // Monnaie de tenue EURO Et franc en entree
   TOBRef.PutValue('GA_DPA',FrancToEuro(TOBRef.GetValue('GA_DPA')));
   TOBRef.PutValue('GA_PAHT',FrancToEuro(TOBRef.GetValue('GA_PAHT')));
   TOBRef.PutValue('GA_PMAP',FrancToEuro(TOBRef.GetValue('GA_PMAP')));
   TOBRef.PutValue('GA_PMRP',FrancToEuro(TOBRef.GetValue('GA_PMRP')));
   TOBRef.PutValue('GA_PRHT',FrancToEuro(TOBRef.GetValue('GA_PRHT')));
   TOBRef.PutValue('GA_DPR',FrancToEuro(TOBRef.GetValue('GA_DPR')));
   TOBRef.PutValue('GA_PVHT',FrancToEuro(TOBRef.GetValue('GA_PVHT')));
   TOBRef.PutValue('GA_PVTTC',FrancToEuro(TOBRef.GetValue('GA_PVTTC')));
   TOBRef.PutValue('GA_PMRP',TOBRef.GetValue('GA_PMRP'));
   end Else if (not VH^.TenueEuro) and (MonnaieValeur <> 'E' ) then
   begin
   // Monnaie de tenue Franc Et franc en entree
   TOBRef.PutValue('GA_DPA',TOBRef.GetValue('GA_DPA'));
   TOBRef.PutValue('GA_PAHT',TOBRef.GetValue('GA_PAHT'));
   TOBRef.PutValue('GA_PMAP',TOBRef.GetValue('GA_PMAP'));
   TOBRef.PutValue('GA_PMRP',TOBRef.GetValue('GA_PMRP'));
   TOBRef.PutValue('GA_PRHT',TOBRef.GetValue('GA_PRHT'));
   TOBRef.PutValue('GA_DPR',TOBRef.GetValue('GA_DPR'));
   TOBRef.PutValue('GA_PVHT',TOBRef.GetValue('GA_PVHT'));
   TOBRef.PutValue('GA_PVTTC',TOBRef.GetValue('GA_PVTTC'));
...
// date de supression
   TOBREF.PutVAlue('GA_DATESUPPRESSION',Str8ToDate('31122099', True));
// --
// gestion Bloc notes
BlocNote := TOBRef.GetValue('GA_BLOCNOTE');
if Length(BlocNote) <> 0 then
   begin
   while Pos('<#CR>', BlocNote) > 0 do
       BlocNote := Copy(BlocNote, 0, Pos('<#CR>', Blocnote) - 1) + #$D#$A +
                   Copy(BlocNote, Pos('<#CR>', BlocNote) + Length('<#CR>'), length(Blocnote));
   TOBRef.PutValue ('GA_BLOCNOTE',BlocNote);
   end;
END ;


Procedure TFRecupbtp.COnformiteTiers (TOBRef:TOB);
var CodeBase,CodeArrondi,CurChamp: string ;
    BaseTarif,CoefCalc,PrixRef: double ;
    Indice : integer;
    CalCule : Boolean;
...
BEGIN
// gestion Bloc notes
lg :=GetParamSoc('SO_LGCPTEAUX');
Newchaine := copy(TobRef.getValue('T_AUXILIAIRE'),1,lg );
TOBRef.putValue('T_AUXILIAIRE',NewChaine);
lg :=GetParamSoc('SO_LGCPTEGEN');
Newchaine := copy(TobRef.getValue('T_COLLECTIF'),1,lg );
TOBRef.putValue('T_COLLECTIF',NewChaine);
BlocNote := TOBRef.GetValue('T_BLOCNOTE');
if Length(BlocNote) <> 0 then
   begin
   while Pos('<#CR>', BlocNote) > 0 do
       BlocNote := Copy(BlocNote, 0, Pos('<#CR>', Blocnote) - 1) + #$D#$A +
                   Copy(BlocNote, Pos('<#CR>', BlocNote) + Length('<#CR>'), length(Blocnote));
   TOBRef.PutValue ('T_BLOCNOTE',BlocNote);
   end;
END ;

Procedure TFRecupbtp.ConformiteArticleTiers (TOBRef:TOB);
BEGIN
TOBRef.PutValue ('GAT_ARTICLE',uppercase (trim(TOBREF.GetValue('GAT_ARTICLE'))));
TOBRef.PutValue ('GAT_REFTIERS',uppercase(trim(TOBREF.GetValue('GAT_REFTIERS'))));
TOBRef.PutValue ('GAT_REFARTICLE',uppercase(trim(TOBREF.GetValue('GAT_REFARTICLE'))));
END ;

Procedure TFRecupbtp.ConformiteCatalogue (TOBRef:TOB);
BEGIN
TOBRef.PutValue ('GCA_ARTICLE',uppercase(trim(TOBREF.GetValue('GCA_ARTICLE'))));
TOBRef.PutValue ('GCA_REFERENCE',uppercase(trim(TOBREF.GetValue('GCA_REFERENCE'))));
END ;

Procedure TFRecupbtp.ConformiteEntOuvrage (TOBRef:TOB);
BEGIN
TOBRef.PutValue ('GNE_ARTICLE',uppercase(trim(TOBREF.GetValue('GNE_ARTICLE'))));
TOBRef.PutValue ('GNE_NOMENCLATURE',uppercase(trim(TOBREF.GetValue('GNE_NOMENCLATURE'))));
END ;

Procedure TFRecupbtp.ConformiteLigOuvrage (TOBRef:TOB; MonnaieValeur: string);
Var TypArt : String;
BEGIN
TOBRef.PutValue ('GNL_ARTICLE',uppercase(trim(TOBREF.GetValue('GNL_ARTICLE'))));
TOBRef.PutValue ('GNL_NOMENCLATURE',uppercase(trim(TOBREF.GetValue('GNL_NOMENCLATURE'))));
if (VH^.TenueEuro) and (MonnaieValeur <> 'E' ) then
   begin
   // Monnaie de tenue EURO Et franc en entree
   TOBRef.PutValue('GNL_PRIXFIXE',FrancToEuro(TOBRef.GetValue('GNL_PRIXFIXE')));
   end;
(* Modif BRL 22/08/05
if copy(TOBRef.GetValue('GNL_SOUSNOMEN'),1,2) <> 'O_' then
*)
TypArt := RenvoieTypeArt (TOBRef.GetValue('GNL_ARTICLE'));
if (TypArt = 'OUV') or (TypArt = 'ARP') then
   begin
   TOBRef.PutValue ('GNL_SOUSNOMEN',uppercase(trim(TOBREF.GetValue('GNL_SOUSNOMEN'))));
   end else
   begin
   TOBREF.PutValue('GNL_SOUSNOMEN','');
   end;
END ;

procedure TFRecupbtp.MessageErreur (MessageR:string);
var chaine : string;
begin
chaine := 'Erreur Prix a 0 sur article '+MessageR;
PGIBox (Chaine,Caption)
end;

function TFRecupbtp.RecupModeTrait( chaine : string):string;
var indice : integer;
    chaineInit,fichier,mode : string;
begin
result := '';
for indice := 0 to ListeOptions.Items.Count -1 do
    begin
    ChaineInit := ListeOptions.Items.Strings [indice];
    if copy ( ChaineInit,0,pos('=',ChaineInit)-1) = Chaine then
       begin
       ChaineINit := copy ( ChaineInit,pos('=',ChaineInit)+1,255);
       fichier := READTOKENST (chaineInit); // 1er fichier
       fichier := READTOKENST (chaineInit); // 2eme fichier
       fichier := READTOKENST (chaineInit); // 3eme fichier
       fichier := READTOKENST (chaineInit); // 4eme fichier
       fichier := READTOKENST (chaineInit); // 5eme fichier
       result := READTOKENST (chaineInit); // Mode
       break;
       end;
    end;
end;
