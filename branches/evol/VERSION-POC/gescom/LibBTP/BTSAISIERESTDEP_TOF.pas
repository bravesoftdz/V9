...
begin
  ...
  begin
    ...
    begin
      TotalDepenseBudgetMTPA        := TotalDepenseBudgetMT;
      TotalDepenseRealiseMTPA       := TotalDepenseRealiseMT;
      TotalDepenseRealiseDepuisMtPA := TotalDepenseRealiseDepuisMt;
      TotalDepenseTheoriqueMTPA     := TotalDepenseTheoriqueMT;
      TotalDepenseFinAffMTPA        := TotalDepenseFinAffMT;
    end;
    *}
  end;
...
begin
  ...
  //Soustraction du Total Prix de Revient du Total Facturation
  TotalBudgetMAR      := CalculTheorique(TotalBudgetMAR     , TotalBudgetDPR);
  TotalRealiseMAR     := CalculTheorique(TotalRealiseMAR    , TotalRealiseDPR);
  TotalReaDepuisMAR   := CalculTheorique(TotalReaDepuisMAR  , TotalReaDepuisMAR);
  TotalTheoMAR        := CalculTheorique(TotalTheoMAR       , TotalTheoDPR);
  //
  if CoefFG <> 0 then
    TotalResteMAR     := CalculTheorique(TotalResteMAR,  (TotalResteDPR * CoefFG))
  else
    TotalResteMAR     := CalculTheorique(TotalResteMAR,   TotalResteDPR);
  //
  TotalFinAffMAR      := CalculTheorique(TotalFinAffMAR,  TotalFinAffDPR);

 if CoefFGFromDomaine then
  begin
    TotalBudgetMARPA    := CalculTheorique(TotalBudgetMARPA,    TotalBudgetDPRPA);
    TotalRealiseMARPA   := CalculTheorique(TotalRealiseMARPA,   TotalRealiseDPRPA);
    TotalReaDepuisMARPA := CalculTheorique(TotalReaDepuisMARPA, TotalReaDepuisMARPA);
    TotalTheoMARPA      := CalculTheorique(TotalTheoMARPA,      TotalTheoDPRPA);
    TotalFinAffMARPA    := CalculTheorique(TotalFinAffMARPA,    TotalFinAffDPRPA);
  end;

  {*
  if TOBL <> nil then
  begin
    TotalBudgetMAR      := CalculTheorique(TOBL.GetDouble('BUDGETMT')     , TotalBudgetDPR);
    TotalRealiseMAR     := CalculTheorique(TOBL.GetDouble('REALISEMT')    , TotalRealiseDPR);
    TotalReaDepuisMAR   := CalculTheorique(TOBL.GetDouble('REALISEDEPUIS'), TotalReaDepuisMAR);
    TotalTheoMAR        := CalculTheorique(TOBL.GetDouble('THEOMT'),        TotalTheoDPR);
    //
    if CoefFG <> 0 then
      TotalResteMAR     := CalculTheorique(TOBL.GetDouble('RESTEMT'), (TotalResteDPR * CoefFG))
    else
      TotalResteMAR     := CalculTheorique(TOBL.GetDouble('RESTEMT'), TotalResteDPR);
    //
    TotalFinAffMAR      := CalculTheorique(TOBL.GetDouble('FINAFFAIRE'), TotalFinAffDPR);
    //
    //FV1 - 20/07/2017 : FS#2639 - GUINIER : dans l'écran RAD, prendre en compte le paramètre de calcul de marge sur PV (% de marque)
    TotalBudgetFAC      := TotalBudgetFAC   + TOBL.GetDouble('BUDGETMT');
    TotalRealiseFAC     := TotalRealiseFAC  + TOBL.GetDouble('REALISEMT') ;
    ...
    if CoefFGFromDomaine then
    begin
      TotalBudgetMARPA    := CalculTheorique(TOBL.GetDouble('BUDGETMTPA'),      TotalBudgetDPRPA);
      TotalRealiseMARPA   := CalculTheorique(TOBL.GetDouble('REALISEMTPA'),     TotalRealiseDPRPA);
      TotalReaDepuisMARPA := CalculTheorique(TOBL.GetDouble('REALISEDEPUISPA'), TotalReaDepuisMARPA);
      TotalTheoMARPA      := CalculTheorique(TOBL.GetDouble('THEOMTPA'),        TotalTheoDPRPA);
      TotalFinAffMARPA    := CalculTheorique(TOBL.GetDouble('FINAFFAIREPA'),    TotalFinAffDPRPA);
    end
    else
    begin
      TotalBudgetMARPA    := TotalBudgetMAR;
  ...
  if TOBL <> nil then
  begin
    TotalRealiseMAR       := CalculTheorique(TotalRealiseMAR,      TOBL.GetDouble('REALISEMT'));
    TotalReaDepuisMAR     := CalculTheorique(TotalReaDepuisMAR,    TOBL.GetDouble('REALISEDEPUIS'));
    TotalTheoMAR          := CalculTheorique(TotalTheoMAR,         TOBL.GetDouble('THEOMT'));
    TotalResteMAR         := CalculTheorique(TotalResteMAR,        TOBL.GetDouble('RESTEMT'));
    TotalFinAffMAR        := CalculTheorique(TotalFinAffMAR,       TOBL.GetDouble('FINAFFAIRE'));
    if CoefFGFromDomaine then
    begin
      TotalRealiseMARPA   := CalculTheorique(TotalRealiseMARPA,    TOBL.GetDouble('REALISEMTPA'));
      TotalReaDepuisMARPA := CalculTheorique(TotalReaDepuisMARPA,  TOBL.GetDouble('REALISEDEPUISPA')) ;
      TotalTheoMARPA      := CalculTheorique(TotalTheoMARPA,       TOBL.GetDouble('THEOMTPA'));
      TotalFinAffMARPA    := CalculTheorique(TotalFinAffMARPA,     TOBL.GetDouble('FINAFFAIREPA'));
      //
    end else
    begin
      TotalRealiseMARPA   := TotalRealiseMAR;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //
        TOBL.PutValue('THEOQTE',          CalculTheorique(TotBudgetQTe,  TotRealiseQte));
        TOBL.PutValue('THEOMTPA',         CalculTheorique(TotBudgetMTPA, TotRealiseMtPA));
        TOBL.PutValue('THEOMT',           CalculTheorique(TotBudgetMT,   TotRealiseMt));
        //
        if DepuisINTheorique then
        begin
          TOBL.PutValue('THEOQTE',        CalculTheorique(TOBL.GetValue('THEOQTE'), TotQteRealiseDep));
          TOBL.PutValue('THEOMTPA',       CalculTheorique(TOBL.GetValue('THEOMTPA'),TotRealiseDepPA));
          TOBL.PutValue('THEOMT',         CalculTheorique(TOBL.GetValue('THEOMT'),  TotRealiseDep));
        end;
...
begin
    ...
    begin
      ...
      //
      TOBL.PutValue('THEOMTPA',         CalculTheorique(TotalBudgetDPRPA, TotalRealiseDPRPA));
      TOBL.PutValue('THEOMT',           CalculTheorique(TotalBudgetDPR,   TotalRealiseDPR));
      //
      if DepuisINTheorique then
      begin
        TOBL.PutValue('THEOMTPA',       CalculTheorique(TOBL.GetValue('THEOMTPA'), TotalReaDepuisDPRPA));
        TOBL.PutValue('THEOMT',         CalculTheorique(TOBL.GetValue('THEOMT')  , TotalReaDepuisDPR));
      end;
    ...
    begin
      ...
      //
      //Calcul du théorique
      TotalTheoriqueDPRPA := CalculTheorique(TotalBudgetDPRPA, TotalRealiseDPRPA);
      TotalTheoriqueDPR   := CalculTheorique(TotalBudgetDPR,   TotalRealiseDPR);
      if DepuisINTheorique then
      begin
        TotalTheoriqueDPRPA := CalculTheorique(TotalTheoriqueDPRPA, TotalReaDepuisDPRPA);
        TotalTheoriqueDPR   := CalculTheorique(TotalTheoriqueDPR  , TotalReaDepuisDPR);
      end;

      TotalTheoriqueDPRPA   := CalculTheorique(TotalTheoriqueDPRPA, TotalTheoFAC);
      TotalTheoriqueDPR     := CalculTheorique(TotalTheoriqueDPR,   TotalTheoFAC);

      TOBL.PutValue('THEOMTPA',         TotalTheoriqueDPRPA);
      TOBL.PutValue('THEOMT',           TotalTheoriqueDPR);
      //
      ...
      TOBL.SetDouble('BASETHEORIQUE',   TotalTheoriqueDPR);
      ...
      TOBL.SetDouble('BASETHEORIQUEPA', TotalTheoriqueDPRPA);
    ...
    begin
      ...
      //TOBL.PutValue('THEOQTE',        CalculTheorique(TotalDepenseBudgetQTE,   TotalDepenseRealiseQte));
      TOBL.PutValue('THEOMTPA',         CalculTheorique(TotalDepenseBudgetMTPA,  TotalDepenseRealiseMTPA));
      TOBL.PutValue('THEOMT',           CalculTheorique(TotalDepenseBudgetMT,    TotalDepenseRealiseMT));

      if DepuisINTheorique then
      begin
        //TOBL.PutValue('THEOQTE',      CalculTheorique(TOBL.GetValue('THEOQTE'),  (TotalDepenseRealiseQte  + TotalDepenseRealiseDepuisQte)));
        TOBL.PutValue('THEOMTPA',       CalculTheorique(TOBL.GetValue('THEOMTPA'), TotalDepenseRealiseDepuisMtPA));
        TOBL.PutValue('THEOMT',         CalculTheorique(TOBL.GetValue('THEOMT'),   TotalDepenseRealiseDepuisMt));
      end;

      TOBL.PutValue('RESTEMT',          TotalDepenseResteMT);
      TOBL.PutValue('FINAFFAIREPA',     TotalDepenseRealiseMTPA  + TotalDepenseResteMT);
      TOBL.PutValue('FINAFFAIRE',       TotalDepenseRealiseMT    + TotalDepenseResteMT);
    end;
...
begin
  ...
  begin
    ...
    begin
      //
      TOBL.PutValue('THEOQTE', CalculTheorique(TOBL.GetDouble('BUDGETQTE'), TOBL.GetDouble('REALISEQTE')));
      TOBL.PutValue('THEOMT',  CalculTheorique(TOBL.GetDouble('BUDGETMT'),  TOBL.GetDouble('REALISEMT')));
      TOBL.PutValue('THEOMTPA',CalculTheorique(TOBL.GetDouble('BUDGETMTPA'),TOBL.GetDouble('REALISEMTPA')));
      //
      if DepuisINTheorique then
      begin
        TOBL.PutValue('THEOQTE', CalculTheorique(TOBL.GetDouble('THEOQTE'), TOBL.GetDouble('REALISEQTEDEPUIS')));
        TOBL.PutValue('THEOMT',  CalculTheorique(TOBL.GetDouble('THEOMT'),  TOBL.GetDouble('REALISEDEPUIS')));
        TOBL.PutValue('THEOMTPA',CalculTheorique(TOBL.GetDouble('THEOMTPA'),TOBL.GetDouble('REALISEDEPUISPA')));
      end;
