...
uses Hent1,Classes,ed_tools,StrUtils,HMsgBox,
     {$IFNDEF EAGLCLIENT}{$IFNDEF DBXPRESS}dbtables{BDE},{$ELSE}uDbxDataSet,{$ENDIF}
     {$else} uTob, {$ENDIF}
     SysUtils,uutil,UQTobIns,CstCommun,BPFctArbre,BPFctSession,
     BPBasic,BPMaille,BPCoeff,BPEclatement,SynScriptBP,BPUtil;
...
var codeRestriction,codeChps,codeChps2,codeChps3,codeChpsSum,codeChpsSumDet,codeTable,codeWhere:String;
    ChpsLibGB:hString;
...
begin
  ...
  Case ContextBP of
    0,1 : begin //Mode-GC
          DonneCodeChpJoinPGINbNivLib(TabCodeAxe,niveau,'LIGNE',ChpsLib,ChpsLibGB,joinLib,Codesession);
          //cherche le code sql champs
    ...
    2 : begin //Compta
          DonneCodeChpJoinPGINbNivLib(TabCodeAxe,niveau,'ANALYTIQ',ChpsLib,ChpsLibGB,joinLib,Codesession);
          //cherche le code sql champs
          if AxeDeviseOK(codeSession) then
    ...
    3 : begin //Paie
          ReqValAffTree(codeSession,ChpsLib,niveau,NbValAff,codeChpsSum,codeChpsSumDet,codeTable,codeWhere);

          DonneCodeChpJoinPGINbNivLib(TabCodeAxe,niveau,'SALARIES',ChpsLib,ChpsLibGB,joinLib,Codesession);

          codeWhere := AnsiReplaceText(codeWhere,'[DATEDEBUT]',USDATETIME(DateRefFin));      //CORRECTION SIC DATEDEBUT -> DATEFIN
          ...
          codeChpsSum := AnsiReplaceText(codeChpsSum,'[DATEDEBUT]',USDATETIME(DateRefFin));  //CORRECTION SIC DATEDEBUT -> DATEFIN
          codeChpsSum := AnsiReplaceText(codeChpsSum,'[DATEFIN]',USDATETIME(DateRefFin));
          codeChpsSumDet := AnsiReplaceText(codeChpsSumDet,'[DATEDEBUT]',USDATETIME(DateRefFin));   //CORRECTION SIC DATEDEBUT -> DATEFIN
  ...
  {EVI / Devise : Suppression du Group By }
  ...
  if ContextBP=3 then
  begin
    LJSQL := AnalyseJoinSQLPaie(LJSQL,codeTable);
    if pos('ETB_HORAIREETABL',codeChpsSum)>0 then
    begin
      LJSQL:=' LEFT JOIN ETABCOMPL ON PSA_ETABLISSEMENT=ETB_ETABLISSEMENT'+LJSQL;
      ChpsLibGB:='ETB_HORAIREETABL,'+ChpsLibGB
    end
  end;
 ...
 ReqSQL := RechercheCodeSqlInsertFeuillesObj(okPremMaille,okDerMaille,
  ...
  Case ContextBP of
    ...
    3 : begin
          ...
          if PrefixH = 'ETP' then PrefixH := 'PSA';
          codeSql := AnsiReplaceText(codeSql0,'[PREFIXE]',PrefixH);
          codeSql := AnsiReplaceText(codeSql,'[CHAMPDETAIL]','');
          codeSql := AnsiReplaceText(codeSql,'[CHAMPDETAILGROUPBY]','');
          codeSqldetail:=AnsiReplaceText(codeSqlDetail0,'[PREFIXE]',PrefixH);
          codeSqldetail:=AnsiReplaceText(codeSqldetail,'[CHAMPDETAIL]',','+PrefixH+'_DATEENTREE');
          codeSqldetail:=AnsiReplaceText(codeSqldetail,'[CHAMPDETAILGROUPBY]',','+PrefixH+'_DATEENTREE,'+PrefixH+'_DATESORTIE');
        end;
  end;
