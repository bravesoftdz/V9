...
type
  ...
  function wFieldValueToString(Const iTable, iChamp:Integer; Const DS:TDataSet):String; overload;
...
begin
  ...
  begin
    ...
    Result := '' ;
    { Indice de la table dans DECHAMPS }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Value := '' ;
            TypeField := V_PGI.DECHAMPS[iTable, iChamp].tipe ;
...
begin
  ...
  Prefixe := WGetPrefixe ( FieldArray[0] );
  if Prefixe = '' then Exit;
  { Récupération du nom de la table }
  Table := PrefixeToTable ( Prefixe );
  ...
  begin
    Inc(i);
	  iChamp := ChampToNum(FieldArray[i]);
    if iChamp <> -1 then
    begin
      TypeField := V_PGI.DECHAMPS[iTable, iChamp].tipe;
      if ( TypeField = 'INTEGER' ) OR ( TypeField = 'SMALLINT' ) then
...
begin
  TypeField := V_PGI.DECHAMPS[iTable,iChamp].tipe;
  if ( TypeField = 'INTEGER' ) OR ( TypeField = 'SMALLINT' ) then
    Result := IntToStr ( DS.FindField ( V_PGI.DECHAMPS[iTable,iChamp].Nom ).AsInteger )
  else if ( TypeField = 'DOUBLE' ) OR ( TypeField = 'RATE' ) OR ( TypeField = 'EXTENDED' ) then
    Result := FloatToStr ( DS.FindField ( V_PGI.DECHAMPS[iTable,iChamp].Nom ).AsFloat )
  else if ( TypeField = 'DATE' ) then
    Result := '"' + DateTimeToStr ( DS.FindField ( V_PGI.DECHAMPS[iTable,iChamp].Nom ).AsDateTime ) + '"'
  else if ( TypeField = 'BLOB' ) OR ( TypeField = 'DATA' ) then
  else
    Result := DS.FindField ( V_PGI.DECHAMPS[iTable,iChamp].Nom ).AsString;
end;
...
begin
  ...
  begin
    ...
    begin
      Inc ( Indice );
      SetLength ( Result, Indice );
      iChamp := ChampToNum(St);
      if iChamp <> - 1 then
      Result[Indice-1] := WFieldValueToString(iTable, iChamp, DS)
      else
        Result[Indice-1] := '';
      St := ReadTokenPipe ( S, ',' );
    end;
...
var
  ...
  FieldsName, LastErrorMsg, MonSelect, Sql : string;
begin
  LastErrorMsg := '';
  iTable := TableToNum(TableName);
  {$IFDEF EAGLCLIENT}
  ...
  if iTable > 0 then
  begin
    { Constitution des champs à insérer }
    {$IFNDEF EAGLSERVER}
    for iField := 1 to High(V_PGI.DeChamps[iTable]) do
    {$ELSE EAGLSERVER}
    for iField := 1 to High(V_PGI.HDeChamps[LookupCurrentSession.SocNum, iTable]) do
    {$ENDIF EAGLSERVER}
    begin
      if Pos('AS ' + V_Pgi.DEChamps[iTable, iField].Nom, Select) > 0 then
        FieldsName := FieldsName + iif(iField > 1, ',', '') + V_Pgi.DEChamps[iTable, iField].Nom;
      if (Pos('AS ' + V_Pgi.DEChamps[iTable, iField].Nom, Select) <= 0) and
         (V_Pgi.DeChamps[iTable, iField].tipe <> 'BLOB') then
      begin
        LastErrorMsg := 'Le champ n''existe pas dans le select (AS ' + V_Pgi.DEChamps[iTable, iField].Nom + ')';
        Break;
      end
      else
      if Pos('AS ' + V_Pgi.DEChamps[iTable, iField].Nom, Select) <= 0 then continue
      else
      begin
        { De }
        De := Pos('AS ' + V_Pgi.DEChamps[iTable, iField].Nom, Select);
        P  := 0;
        ...
        { A }
        A := Pos('AS ' + V_Pgi.DEChamps[iTable, iField].Nom, Select);
...
{$IFNDEF ERADIO}
  ...
  if Assigned(DS) then
  begin
    { Récupération de l'indice de la table }
    iTable := TableToNum( TableName );
    {$IFDEF EAGLCLIENT}
      if High(V_Pgi.DEChamps[iTable]) <= 0 then
        ChargeDeChamps(iTable, TableToPrefixe(TableName));
    ...
    begin
      ...
      begin
        ...
        { Récupère l'indice du champ dans DECHAMPS }
        iChamp := ChampToNum(Champ);
        if iChamp <> -1 then
        begin
          TypeField := V_PGI.DECHAMPS[iTable, iChamp].tipe ;
...
begin
  ...
  { Récupération de l'indice de la table dans V_PGI.DETABLES }
  iTable := TableToNum ( TableName ) ;

  if iTable > 0 then Result := V_PGI.DETABLES[iTable].Cle1 ;
end ;
...
begin
  ...
  begin
	  iChamp := ChampToNum(FieldName);
    FieldType := V_PGI.DECHAMPS[iTable, iChamp].tipe ;
...
begin
  ...
  iChamp := ChampToNum(FieldName);
  if iChamp <> -1 then
    Result := V_PGI.DECHAMPS[iTable, iChamp].Libelle
  else
    Result := '';
end;
  ...
  begin
    ...
    begin
      { Récupère les champs de la table }
			{$IFDEF EAGLSERVER}
	      for i := Low(V_PGI.HDECHAMPS[LookupCurrentSession.SocNum, iTable]) to High(V_PGI.HDECHAMPS[LookupCurrentSession.SocNum, iTable]) do
			{$ELSE  EAGLSERVER}
	      for i := Low(V_PGI.DECHAMPS[iTable]) to High(V_PGI.DECHAMPS[iTable]) do
			{$ENDIF EAGLSERVER}
      begin
        FieldName := V_PGI.DECHAMPS[iTable,i].Nom;
        if FieldName <> '' then
        begin
          TSCodeChamp.Add(FieldName);
          if Assigned(TSLibChamp) then
            TSLibChamp.Add(V_PGI.DECHAMPS[iTable,i].Libelle)
        end;
      end;
    end;
  end;
...
begin
  {$IFDEF EAGLCLIENT}
    if High(V_Pgi.DEChamps[iTable]) <= 0 then
      ChargeDeChamps(iTable, ExtractPrefixe(FieldName));
  {$ENDIF EAGLCLIENT}
  Result := GetSimpleTypeField(V_Pgi.DeChamps[iTable, iField].Tipe)
end;
...
begin
  ...
  begin
    ...
    {$ENDIF EAGLCLIENT}
    iChamp := ChampToNum(FieldName);
    if iChamp <> -1 then Result := V_PGI.DECHAMPS[iTable, iChamp].tipe;
  end;  
end;
...
{$IFNDEF EAGLSERVER}
  ...
  Debug('===========================================');
  Debug(' Fonction: ' + Fonction);
  if Notes <> '' then
    Debug(' Notes   : ' + Notes);
  Debug(' Temps   : ' + FormatDateTime('hh:nn:ss:zzz', GetTime));
  Debug(' IoError : ' + GetIoError);
  Debug('===========================================');
{$ENDIF !EAGLSERVER}
end;
...
begin
  ...
  begin
	  ...
	  for i := Low(V_Pgi.DeChamps[iTable]) to High(V_Pgi.DeChamps[iTable]) do
		{$ENDIF EAGLSERVER}
	    if not IsFieldToExclude(V_Pgi.DeChamps[iTable, i].Nom) and (not ExcludeBlobField or (Pos(V_Pgi.DeChamps[iTable, i].Tipe, 'BLOB;DATA;') = 0)) then
	      Result := Result + iif(Result <> '', ', ', '') + V_Pgi.DeChamps[iTable, i].Nom
	end
end;
...
var
	iTable: Integer;
begin
  iTable := TableToNum(PrefixeToTable(ExtractPrefixe(Trim(UpperCase(FieldName)))));
  Result := (iTable > 0) and (Pos(Trim(UpperCase(FieldName)) + ',', Trim(UpperCase(V_PGI.DETABLES[iTable].Cle1)) + ',') <> 0);
end;
...
begin
  ...
  begin
    ...
    begin
			{$IFDEF EAGLSERVER}
	    for iChamp := Low(V_Pgi.HDeChamps[LookupCurrentSession.SocNum, iTable]) to High(V_Pgi.HDeChamps[LookupCurrentSession.SocNum, iTable]) do
			{$ELSE  EAGLSERVER}
	    for iChamp := Low(V_Pgi.DeChamps[iTable]) to High(V_Pgi.DeChamps[iTable]) do
			{$ENDIF EAGLSERVER}
	      if (V_Pgi.DeChamps[iTable, iChamp].Nom<>'') then
	        T.AddChampSupValeur(V_Pgi.DeChamps[iTable, iChamp].Nom, wGetInitValue(V_Pgi.DeChamps[iTable, iChamp].Nom));
    end;
  end;
end;
