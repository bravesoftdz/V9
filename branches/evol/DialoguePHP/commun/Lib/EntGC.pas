...
type
  LaVariableGC = Class{$IFDEF EAGLSERVER}(TLaVariable){$ENDIF EAGLSERVER}
     ...
     Public
     // Générique
     TOBParPiece,TOBGCE,TOBGCB,TOBEdt : TOB ;
...
{$endif}
     ...
     TOBPcaisse : TOB ;
     // Modif BTP
     BTGestParag,BTPrixMarche,BTSeriaAO, BTSeriaES, BTSeriaLogistique,BTSeriaContrat,BTSeriaIntervention,
     BTSeriaGRC, BTSeriaChantiers,SeriaCoTraitance : boolean;
...
// DBR 27/12/04...donc il faut tester VenteOuAchat <> 1 car si <> 1 est autre que ACH donc VEN et tout le reste
        ...
        if V_PGI.DEChamps[NumTable,NumChamp].tipe = 'DOUBLE' then
          TobTmp1.AddChampSupValeur('VALEURVIDE',0.0,False)
        else if V_PGI.DEChamps[NumTable,NumChamp].tipe = 'DATE' then
          TobTmp1.AddChampSupValeur('VALEURVIDE', iDate1900,False)
        else if V_PGI.DEChamps[NumTable,NumChamp].tipe = 'INTEGER' then
          TobTmp1.AddChampSupValeur('VALEURVIDE', 0,False)
        else if V_PGI.DEChamps[NumTable,NumChamp].tipe = 'BOOLEAN' then
