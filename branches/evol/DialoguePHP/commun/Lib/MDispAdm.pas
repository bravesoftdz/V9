...
uses windows, HEnt1, MenuOLG, sysutils, Messages, HMsgBox, {$IFNDEF VER150}Classes, {$ENDIF}Controls, Hctrls,
  ...
  ,UtomEtabliss,ChangeVersions,BobGestion
...
{$IFDEF BTP}
          ...
          if BOB_IMPORT_PCL_STD('BAT2009','Bob Business BTP',False) = 1 then
          begin
            FMenuG.Quitter;
						Abort;
          end;
...
begin
  ...
  V_PGI.NumVersion := '9.0 Béta' ;
  V_PGI.NumBuild := IntToStr(V_PGI.NumVersionBase) + '.0';
  V_PGI.DateVersion := EncodeDate(2011, 07, 01) ;
