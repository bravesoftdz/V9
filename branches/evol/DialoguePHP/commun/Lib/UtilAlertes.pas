...
begin
  ...
  begin
    ...
    begin
        if Result = '' then
          Result := 'Fiche concernée : '+V_PGI.DECHAMPS[iTable, iChamp].libelle+' = '+ Tobtable.GetString(FieldName[i])
        else
          Result := Result + ', ' + V_PGI.DECHAMPS[iTable, iChamp].libelle+' = '+ Tobtable.GetString(FieldName[i]);
    end
    else
      Ok := False;
  end;
