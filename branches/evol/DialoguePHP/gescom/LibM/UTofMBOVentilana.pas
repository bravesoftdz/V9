...
//  if VH_GC.GCVentAxe4 then Combo.Plus := ' AND (X_AXE= "A1" or X_AXE= "A2" or X_AXE= "A3" or X_AXE= "A4")' else
  ...
  with THValComboBox(GetControl('GDA_TYPECOMPTE')) do  //Affaire - Lien Paie
   if GetParamSoc('SO_AFLIENPAIEANA')
    then Plus := 'AND (CO_LIBRE LIKE "%COMPTA%" OR CO_LIBRE LIKE "%PAIE%")';
...
//  st := RechDom (Tablette,Code,False);
	if Tablette = 'GCZONELIBRE' then st := RechDomZoneLibre (Code, False)
  														else st := RechDom (Tablette,Code,False);
...
BEGIN
  ...
  if wCode ='AC3' then Result :=  VH_GC.CleAffaire.Co3Lib else
   ...
   if wCode = 'CT1' then Result := 'Constante 1' else
   if wCode = 'CT2' then Result := 'Constante 2' else
   if wCode = 'CT3' then Result := 'Constante 3' else
      Exit;
