...
type
	...
	TModifSousDetail = class (Tobject)
      ...
      function EnHt (TOBL : TOB) : boolean;
    	...
    	procedure RenseigneTOBOuv(TOBOuv, TOBL, TobArticle: TOB;libelle: string; AvecPv: boolean=true);
    ...
    public
      ...
      procedure ModifQteSousDet (TOBL : TOB; Qte : double);
      procedure ModifPASousDet (TOBL : TOB; Pua : double);
      procedure ModifPVSousDet (TOBL : TOB; Pu : double);
      procedure ModifUniteSousDet (TOBL : TOB; Unite : string);
    	...
    	procedure ModifPourcentAvancSousDet(TOBL: TOB; Pourcent: double;Typefacturation: string; DEV : Rdevise);
...
uses factureBTP,factOuvrage,UtilPGI,FactArticle,facture,FactTOB,
     ...
     TarifUtil,FactDomaines,BTPUtil;
...
begin
  ...
  if (Pos(GetTypeFacturation,'DAC;AVA')>0) and (Pos(TOBPiece.getValue('GP_NATUREPIECEG'),'FBT;DAC')>0) then Exit;
...
var MontantPA,MontantAchat,MontantDev,QteDuDetail,QteDuPv : double;
begin
  MontantPA :=Arrondi((TOBOD.GetValue('BLO_QTEFACT')*TOBOD.GetValue('BLO_DPA'))/TOBOD.GetValue('BLO_PRIXPOURQTE'),V_PGI.okdecP);
  //
  ...
  begin
    MontantAchat := Arrondi((MontantPA * TOBPere.getValue('GL_QTEFACT'))/(QteDuPv*QteDuDetail),V_PGI.okdecP);
    if EnHt(TOBL) then
    begin
  ...
  //
  if TOBOD.GetValue('BLO_TYPEARTICLE') = 'POU' then TOBL.PutValue('GL_QTEFACT', TOBOD.GetValue('BLO_QTEFACT')/(QteDuPv*QteDuDetail) )
                                               else TOBL.PutValue('GL_QTEFACT', (TOBOD.GetValue('BLO_QTEFACT') * TOBpere.GetValue('GL_QTEFACT')/(QteDuPv*QteDuDetail)));
...
procedure TModifSousDetail.RenseigneTOBOuv(TOBOuv,TOBL, TobArticle : TOB; libelle : string;AvecPv:boolean);
...
begin
  ...
  TOBOUv.putValue('GL_TYPELIGNE',SavTypeLigne);
  ...
  begin
    ...
    CopieOuvFromArt (fTOBPiece,TOBOuv,TobArticle,DEV);
    if AvecPv then
    begin
    ...
    begin
      if fTOBPiece.getValue('GP_APPLICFGST')='X' then
      begin
      ...
      if fTOBPiece.getValue('GP_APPLICFCST')='X' then
      begin
...
begin
  ...
  if TOBref = nil then exit;
  if (TOBL.GetValue('GL_QTEPREVAVANC') <> 0) and (TOBL.GetValue('GL_NATUREPIECEG')='DBT') then
  begin
    PgiError(Traduirememoire('Impossible : une facturation a déjà été effectuée sur cette ligne.'));
    exit;
  end;
  //
  if (TOBRef.GetValue('GL_QTEPREVAVANC') <> 0) and (TOBRef.GetValue('GL_NATUREPIECEG')='DBT') then
  begin
    PgiError(Traduirememoire('Impossible : une facturation a déjà été effectuée sur cette ligne.'));
    exit;
  end;
...
procedure TModifSousDetail.ModifPASousDet(TOBL: TOB; Pua : double);
...
begin
  ...
  begin
    ...
    begin
      ...
      CalculeLigneAcOuv (TOBOD,DEV,TOBOD.GetValue('BLO_NONAPPLICFRAIS')='X',TOBA);
    end;
...
procedure TModifSousDetail.ModifPVSousDet(TOBL: TOB; Pu : double);
var TOBO,TOBOD,TOBA : TOB;
...
procedure TModifSousDetail.ModifQteSousDet(TOBL: TOB; Qte : double);
var TOBO,TOBOD,TOBA : TOB;
...
begin
  ...
  TOBOD.PutValue('BLO_CODEARTICLE', CodeArticle);
  ...
  RenseigneTOBOuv(TOBOD,TOBLO,TOBART,Libelle);
  if isOuvrage(TOBOD) then
  begin
    TOBOD.putValue('BLO_COMPOSE',CodeArticle);
    if not ChargeOuvrage (TOBLO,TOBOD.GetValue('BLO_COMPOSE'), TOBOD , 2,TOBOD.GetValue('BLO_NUMORDRE'),Tresult) then
    begin
...
begin
  ...
  BEGIN
  	st1 := TOBArt.GetValue('GA_ARTICLE');
  END else result := false;

  if st1 <> '' then
  begin
    Q := OpenSQL('Select GNE_NOMENCLATURE,GNE_LIBELLE from NOMENENT Where GNE_ARTICLE="' +
    							TOBArt.GetValue('GA_ARTICLE') + '"', True,-1, '', True);
    st1 := Q.FindField('GNE_NOMENCLATURE').AsString;
    Libelle := Q.FindField('GNE_Libelle').AsString;
    TOBART.putValue('REFARTSAISIE',st1);
    Ferme(Q);
  end;
  Retour := st1;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        RenseigneTOBOuv(TOBDetOuv,TOBL,TobArt,TOBArt.getValue('GA_LIBELLE'));
...
begin
  ...
  begin
  	...
  	TOBL.PutValue('BLF_MTCUMULEFACT',TOBL.GetValue('BLF_MTCUMULEFACT')+Montant);
  end else
  begin
  	TOBL.PutValue('BLF_MTSITUATION',TOBL.GetValue('BLF_MTSITUATION')-Montant);
  	TOBL.PutValue('BLF_MTCUMULEFACT',TOBL.GetValue('BLF_MTCUMULEFACT')-Montant);
...
procedure TModifSousDetail.ModifPourcentAvancSousDet(TOBL: TOB;Pourcent: double; Typefacturation : string; DEV : RDevise);
var TOBO,TOBOD,TOBA : TOB;
		QtePere,QteMaj,QteDuDetail : double;
    zvaleur : T_valeurs;
    Adeduire : double;
