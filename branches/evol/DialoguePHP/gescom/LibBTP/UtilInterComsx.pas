...
begin
    ...
    PathFile := ExtractFileDir(ChargeComsX.FichierEcr)+ '\';
...
Begin
   ...
   if ExisteSql( 'SELECT 1 FROM PARAMSOC WHERE SOC_NOM="SO_MBOCPTAEXCLPA1"' ) then // Test pour mise en place avant création du paramsoc dans la socref officielle
      begin
      sExclure := GetParamSocSecur( 'SO_MBOCPTAEXCLPA1', 'MDP;MDR;DEV;REG;SOU;EXO;TL;SSA;PARAM' ); // Tous par défaut
      if copy( sExclure, Length( sExclure ), 1 ) <> ';' then sExclure := sExclure + ';';
      sExclure := sExclure + GetParamSocSecur( 'SO_MBOCPTAEXCLPA2', '' ); // Aucun par défaut
      end
   else
...
begin
  ...
  if Result = '' then Result := GetParamSocSecur( 'SO_MBOCHEMINCOMSX', '');
