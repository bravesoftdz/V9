...
type TGrilleModeSaisie = (TgsMO, TgsFRS, TgsRES, TgsFOURN,TgsMoext,TgsRecettes);
...
function OKRessource (TOBConso : TOB;MAINDOEUVRE,LeType : string; Var PrestationDefaut : string;WithMajPrix : boolean; fonction : string='';  WithRecupPrestation : boolean = true) : boolean;
...
function IsExisteAffaire (Code : string) : boolean;
function IsExistePhaseAffaire (Code,Phase : string) : boolean;
function GetChantier (Code : string) : string;
...
procedure SetValoFromRessource (TOBL: TOB;TOBRes : TOB;ModeValo : Tmodevalo=[TmvAll]);
...
implementation
uses Utilsuggestion,factdomaines;
...
begin
  ...
  result := false;
  QQ := OpenSql ('SELECT GA_LIBELLE FROM ARTICLE WHERE GA_ARTICLE="'+MATERIAUX+'"',true);
  if not QQ.eof then result := true;
  ferme (QQ);
end;
...
begin
  ...
  begin
    ...
    if TheValeur <> '' then
	  ...
	  TOBConso.PutValue('BCO_DPA',Arrondi(TOBConso.GetValue('_PA_INIT')*Coef,V_PGI.okdecQ));
 	  TOBConso.PutValue('BCO_DPR',Arrondi(TOBConso.GetValue('_PR_INIT')*Coef,V_PGI.okdecQ));
 	  TOBConso.PutValue('BCO_PUHT',Arrondi(TOBConso.GetValue('_PV_INIT')*Coef,V_PGI.okdecQ));
...
begin
  ...
  if (TmvAll in ModeValo) or (TmvPa in ModeValo) then
     TOBL.putValue('BCO_DPA',TOBREs.GetValue('ARS_TAUXUNIT'));
  ...
  CoefMarge := TOBL.GEtValue('COEFMARGE');
...
//if (WithMajPrix) and (TOBRES.GetValue('ARS_ARTICLE')<> '')then
  ...
  if (copy(TOBL.getValue('BCO_AFFAIRE'),1,1)='W') and (getParamSocSecur('SO_BTVALOAPPELS',false)) then
  begin
...
begin
  result := false;
  if Code = '' then exit;
  QQ := OpenSql ('SELECT AFF_LIBELLE FROM AFFAIRE WHERE AFF_AFFAIRE="'+Code+'"',true);
  if not QQ.eof then result := true;
  ferme (QQ);
end;
...
begin
  ...
  {$ELSE}
  CodeAffaireDecoupe (Code,Part0,Part1,Part2,Part3,Avenant,taModif,false);
  {$ENDIF}
  ...
  Stargument := Stargument + ';NOCHANGESTATUT';
