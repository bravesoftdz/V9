...
{$ENDIF}
 ...
 AffaireUtil,FactAdresseBTP,UtilArticle,UtilTOBpiece,uEntCommun,HRichOLE,forms;
...
type TPieceEtude = class
     ...
     fTiers : string;
     fCodeAffaire : string;
     fEtablissement,fDomaine,FDocExcel,fCurSheet : String;
     fEnHT,fSaisieContre : Boolean;
     SaContexte : TModeAff;
     docOpen : boolean;
     DEV : Rdevise;
     ...
     IndiceOuv : integer;
     fColCode,fColDes,fColQte,fColUnite,fColPu,fColMontant : Tcolonnes;
     fPlages : Tlist;
     fcodePrest : string; // il servira pour les etudes secondaires
     fNatureTravail : string;
     fFournisseur : string;
     //
     TOBPIECE,TOBBases,TOBBasesL,TOBEches,TOBPorcs,TOBTiers,TOBArticles,TOBConds,TOBTarif,TOBComms,TOBCatalogu : TOB;
     TOBAdresses,TOBCXV,TOBNomen,TOBDim,TOBLOT,TOBSerie,TOBSerRel,TOBAcomptes,TOBDispoContreM,TOBAffaire : TOB;
     TOBCPTA,TOBANAS,TOBANAP,TOBGSA,TOBOuvrage,TOBLIENOLE,TOBPieceRG,TobBasesRG,(*TOBLignesRg,*)TOBLienXls : TOB;
     TOBOuvragesP : TOB;
     TOBPIECE_O: TOB;
     TOBPieceTrait : TOB;
...
//     Procedure SauveDocPgi;
     ...
     function NewPieceVide(var CleDoc: R_CleDoc; CodeTiers, CodeAffaire,
      Etablissement, Domaine: String; EnHT, SaisieContre: Boolean): TOB;
     ...
     property fournisseur : String write fFournisseur;
     property NatureTravail : String write fNatureTravail ;
...
implementation
uses factureBTP,Factspec,UtilPGI;
...
constructor TPieceEtude.Create;
begin
//
...
procedure TPieceEtude.NewDoc(var CleDoc: R_CleDoc; CodeTiers, CodeAffaire,
  Etablissement, Domaine: String; EnHT, SaisieContre: Boolean);
begin
  fcledoc := cledoc;
...
LoadLesRetenues (TOBPiece,TOBPieceRG,TOBBasesRG,fCleDoc);
// Lecture Ouvrages
{$IFDEF BTP}
...
if WithSousDetail then LoadLesLibDetail(TOBPiece,TOBNomen,TobOuvrage,TOBTiers,TOBAffaire,DEV,False);
{$IFDEF CHR}
...
// génération a travers la structure du fichier Excel
   for Indice := 0 to TobPiece.detail.Count -1 do
       ...
       TOBLXLS := TOBLienXls.findfirst(['BLX_PIECEASSOCIEE','BLX_NUMLIGNE'],[CodPiece,Indice+1],true);
       if TOBLXLS <> nil then
           begin
...
var
    Indice : integer;
    TOBL,TOBLXLS : TOB;
    LignePgi,LigneXls,vide : integer;
    Plage : TPlageExcel;
    NumPrix,designation,qte,unite,pu,montant : string;
    DocValorise : boolean;
    // ARTICLEDIV
...
// génération a travers la structure du fichier Excel
   for Indice := 0 to fPlages.Count -1 do
       ...
       for LigneXls := Plage.mini to Plage.maxi do
           ...
           vide := 0; // reinit nbr de ligne vide
           ...
           if (designation<>'') then
              begin
                if Pos(#$A,designation)> 0 then
                begin
                	LibTmp := copy(designation,1,Pos(#$A,designation)-1);
                	TOBL.putValue('GL_LIBELLE',libTmp);
                  //
                  LibTmp := copy (Designation,Pos(#$A,designation)+1,length(Designation));
                	if Pos(#$A,LibTmp) > 0 then
                  begin
              			libtmp := StringReplace(libTmp,#$A,#$D#$A,[rfReplaceAll]);
                  	StringToRich(Descriptif, libtmp);
                ...
                begin
                  ...
                  if (length(designation) > 70) then
                  begin
                  	TOBL.putValue ('GL_BLOCNOTE',designation);
                  end;
                end;
              end;
           if (Qte<>'') or ((Qte='') and (Pu <> '')) then
              begin
              if ((Qte='') and (Pu <> '')) then
              begin
                if pos(',',pu) > 0 then StringReplace (pu,',','.',[rfReplaceAll]);
                if Valeur(Pu) <> 0 then Qte := '1';
              end;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    TraiteLesOuvrages(nil,TOBPiece, TOBArticles, TOBOuvrage,nil, LignePGI, False, DEV, true);
                    if (designation<>'') then
                    begin
                      if length(designation) > 70 then
                      begin
...
//              if Valeur(Qte) <> 0 then ExcelValue (WorkSheet,LigneXls-1,fcolQte.colonne-1,valeur(Qte));
  ...
  begin
    ...
    CalculFacture (TOBPiece,TOBPieceTrait,TOBOuvrage,TOBOUvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV);
  end;
