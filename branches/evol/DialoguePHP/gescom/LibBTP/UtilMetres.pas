...
{$ENDIF}
  ...
  UtilXlsBTP,EtudesStruct,EtudePiece,FactComm, ComCtrls, uMetreArticle,uEntCommun
  ;
type
...
private
  ...
  procedure AttachesOleOuvrages (TOBOuvrages : TOB);
...
{$ENDIF}
  ...
  For Indice := 0 to ThisDoc.detail.count -1 do
      ...
      if TOBOLE <> nil then
         ...
         TOBL.PutValue('INDICEMETRE',r_CleOle.NumOle); // Ligne de document
         TOBOLE.PutValue('INDICEMETRE',r_CleOle.NumOle); // LIENSOLE
         if r_CleOle.NumOle > IndiceOle then IndiceOle := r_CleOle.NumOle;
         end;
      end;

  if ThisOuvrages <> nil then
     begin
     AttachesOleOuvrages (ThisOuvrages);
     end;

end;

procedure TStockMetre.AttachesOleOuvrages (TOBOuvrages : TOB);
var Indice : integer;
    TOBL, TOBOLE : TOB;
    r_CleOle : cleOle;
begin
  ...
  For Indice := 0 to TOBOuvrages.detail.count -1 do
      ...
      if TOBL.detail.count > 0 then AttachesOleOuvrages (TOBL);
  ...
  TRY
    ...
    if Mns.count > 0 then
       ...
       for Indice := 1 to mns.count do
           begin
           if WorkBook.names.item(indice).name = 'PGI_RESULTAT' then
              begin
  ...
  if sTobMetres.detail.count > 0 then
     ...
     if laTOBMere <> nil then
        ...
        if Action = taCreat then
          ...
          renameFichier (leOldFic,fRepertBase+'\'+fRepert+'\'+LeNewFic);
        ...
        if Duplic then
           ...
           leOldFic := LaTOBOle.GetValue('LO_OBJET');
           LenewFic := GenereNomFichier(LaTOBMere);
          CopieFichier (leOldFic,fRepertBase+'\'+fRepert+'\'+LeNewFic);
          laTOBOle.putvalue('LO_OBJET',fRepertBase+'\'+fRepert+'\'+LeNewFic);
           end;
...
begin
  ...
  TRY
    ...
    Else
       ...
       if fNomFic='' then exit;
       if not FileExists (fNomFic) then CreateTheFichierXls (fnomfic);
       AddVbModuleFromFile ('c:\PGI00\STD\','MetrePgi.xla');
       FileExecAndWait (GetExcelPath + 'EXCEL.exe "' + fNomFic +'"');
       Result := RecupValeurMetre(TOBL);
       End;
...
Begin
  ...
  TRY
    if not FileExists (Fic_Bidon) then CreateTheFichierXls (Fic_Bidon);
