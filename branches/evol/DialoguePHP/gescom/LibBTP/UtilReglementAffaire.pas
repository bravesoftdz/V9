...
begin
  ...
  begin
      result := result + TOBreglements.detail[indice].GetValue('E_CREDITDEV');
...
begin
  ...
  for Indice := 0 to TOBReglements.detail.count -1 do
      ...
      MtRegl := MtRegl + TOBreglements.detail[indice].GetValue('E_CREDITDEV');
      //Modif du 10/01/2008 par FV pour récupération montant exact réglement
      ...
      If TOBreglements.detail[indice].GetValue('E_NATUREPIECE')='OC' then
         MtAcompte := MtAcompte + TOBreglements.detail[indice].GetValue('E_CREDITDEV');
...
begin
  ...
  Sql := 'SELECT S.*,P.GP_REFCOMPTABLE,P.GP_TIERSFACTURE,T.T_AUXILIAIRE FROM BSITUATIONS S '+
         ...
         'WHERE S.BST_AFFAIRE="'+CodeAffaire+'"';
  QQ := OpenSql (Sql,True);
  if not QQ.eof then
  begin
...
begin
  ...
  Sql := 'SELECT GP_NATUREPIECEG,GP_NUMERO,GP_REFCOMPTABLE,GP_TIERSFACTURE,GP_TOTALTTCDEV,T_AUXILIAIRE FROM PIECE '+
