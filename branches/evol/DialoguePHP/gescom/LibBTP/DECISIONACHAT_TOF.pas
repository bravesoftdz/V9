...
Type
  ...
  private
    ...
    QUANTITE_VTE,QUANTITE_ACH,PRIXACHATBRUT,PRIXACHATNET : THNumEdit;
    UNITE_VTE,UNITE_ACH : THLabel;
    ...
    procedure SetInfosFournisseur2Fils(TOBP: TOB; FUV, FUS, FUA, CoefuaUS: double);
...
//    +' Ligne '+InttoStr(cledoc.NumLigne);
  ...
  UNITE_VTE.Caption := TOBL.GetValue('BAD_QUALIFQTEVTE');
  QUANTITE_ACH.Value := TOBL.GetValue('BAD_QUANTITEACH');
  UNITE_ACH.Caption := TOBL.GetValue('BAD_QUALIFQTEACH');
...
begin
  ...
  repeat
    ...
    begin
      ...
      if UnChamp = 'BAD_QUALIFQTESTK' then GS_QUALIFQTESTK := Numero else
...
begin
  ...
  begin
    ListeSaisie := 'OPEN;BAD_TYPEARTICLE;BAD_CODEARTICLE;OPEN1;BAD_LIVCHANTIER;OPEN2;BAD_DEPOT;BAD_AFFAIRE;BAD_TENUESTOCK;BAD_QUANTITESTK;BAD_QUALIFQTESTK;QTEPHY;BAD_FOURNISSEUR;BAD_PABASE;BAD_PRIXACHNET;BAD_PRISENCOMPTE;POSITIONCONSULT;BAD_NUMREMPLACE';
  end else
  begin
    CONSULTFOU.NonUtilisable := true;
    ListeSaisie := 'OPEN;BAD_TYPEARTICLE;BAD_CODEARTICLE;OPEN1;BAD_LIVCHANTIER;OPEN2;BAD_DEPOT;BAD_AFFAIRE;BAD_TENUESTOCK;BAD_QUANTITESTK;BAD_QUALIFQTESTK;QTEPHY;BAD_FOURNISSEUR;BAD_PABASE;BAD_PRIXACHNET;BAD_PRISENCOMPTE;BAD_NUMREMPLACE';
  end;
  NbLigne := 1;
...
begin
  TOBL:=GetTOBDecisAch(TOBdecisionnel,GS.row) ; if TOBL=Nil then Exit ;
	AGLLanceFiche('GC','GCDISPO','',TOBL.GetValue('BAD_ARTICLE')+';'+TOBL.GetValue('BAD_DEPOT')+';-','CONSULTATION') ;
end;
...
begin
  ...
  if CoefuaUs <> 0 then
  begin
  	TOBL.putValue('BAD_QUANTITEACH',Arrondi(TOBL.GetValue('BAD_QUANTITEVTE') * FUV / FUS / CoefuaUs,V_PGI.okdecQ));
  end else
  begin
  	TOBL.putValue('BAD_QUANTITEACH',Arrondi(TOBL.GetValue('BAD_QUANTITEVTE')*FUV/FUA,V_PGI.okdecQ));
  end;
  TOBL.putValue('BAD_QUANTITESTK',Arrondi(TOBL.GetValue('BAD_QUANTITEVTE')*FUV/FUS,V_PGI.okdecQ));

  if TOBL.detail.count > 0 then
  begin
  	SetInfosFournisseur2Fils (TOBL,FUV,FUS,FUA,CoefuaUs);
  end;

end;

procedure TOF_DECISIONACHAT.SetInfosFournisseur2Fils(TOBP: TOB;FUV,FUS,FUA, CoefuaUS : double);
var Indice : integer;
		TOBL : TOB;
begin
	for Indice := 0 to TOBP.detail.count -1 do
  begin
    ...
    if TOBL.detail.count > 0 then SetInfosFournisseur2Fils (TOBL,FUV,FUS,FUA,CoefuaUs);
  end;
//
end;
...
begin
  TOBL:=GetTOBDecisAch(TOBdecisionnel,GS.row) ; if TOBL=Nil then Exit ;
	AGLLanceFiche('GC','GCDISPO_MUL','GA_CODEARTICLE='+TOBL.GetValue('BAD_CODEARTICLE'),'','CONSULTATION') ;
