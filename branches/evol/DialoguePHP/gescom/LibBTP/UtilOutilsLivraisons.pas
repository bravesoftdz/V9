...
begin
  ...
  QQ := OpenSql ('SELECT * FROM LIGNE WHERE GL_PIECEORIGINE="'+TOBL.GetValue('GL_PIECEPRECEDENTE')+
  							 '" AND GL_NATUREPIECEG IN ("BLF","FF") AND GL_QTERESTE <> 0',True);
  if QQ.eof then
  begin
  	ferme (QQ);
    QQ := OpenSql ('SELECT * FROM LIGNE WHERE GL_ARTICLE="'+TOBL.GetValue('GL_ARTICLE')+
                   '" AND GL_AFFAIRE="'+TOBL.GetValue('GL_AFFAIRE')+'" AND GL_NATUREPIECEG IN ("BLF","FF") AND GL_QTERESTE <> 0',True);
  end;
  ...
  begin
    ...
    QQ := OpenSql ('SELECT GL_QUALIFQTEVTE,GL_QTERESTE,GL_PRIXPOURQTE,GL_ARTICLE,GL_CODEARTICLE,GL_AVENANT,GL_NATUREPIECEG,GL_SOUCHE,GL_NUMERO,GL_INDICEG,GL_QTEFACT,GL_DPA,GL_DPR,GL_PUHTDEV FROM LIGNE WHERE GL_NATUREPIECEG="'+Cledoc.NaturePiece+'" AND '+
