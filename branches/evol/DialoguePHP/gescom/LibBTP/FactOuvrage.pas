...
{$ENDIF}
     ...
     UtilArticle,OptimizeOuv,FactDomaines,uEntCommun,UtilTOBPiece ;
...
function TraiteLesOuvrages ( XX: Tform; TOBPiece,TOBArticles,TOBOuvrage,TOBRepart : TOB ; ARow : integer ; LePremier : boolean ;DEV :RDEVISE;OrigineExcel:boolean;OptimizeOuv : TOptimizeOuv = nil;WithDetails : boolean=true;FromBordereau : boolean=false; ModeSaisieAch : boolean= false) : boolean;
...
function ReajusteMontantOuvrage (TOBARticles,TOBPiece,TOBL,TOBOuvrage:TOB;Montantcalc: double;var MontantPr : double;var MontantDemande:double;DEV:RDevise;EnHt : boolean;atributionEcart:boolean=true;EnPR : boolean = false;EnPa : boolean=false) : boolean;
procedure CopieOuvFromArt (TOBPiece,TOBLND,TOBART : TOB;DEV:Rdevise);
...
procedure AttribueEcart (TOBPiece,TobOuvrage : TOB;ArtEcart:string;Montantecart: double;DEV:RDEVISE;EnHt:Boolean;EnPA : boolean=false;OnlyPa:boolean=false);
procedure LoadLesLibDetOuvLig ( TOBPIECE,TOBOuvrage,TOBTiers,TOBAffaire,TOBL : TOB;var Indice: integer; DEV:RDevise);
procedure LoadLesLibDetailOuvrages (TOBPIECE,TOBOuvrage,TOBTiers,TOBAffaire : TOB;DEV:RDevise );
procedure MiseAPlatOuv (TOBPiece,TOBL,TOBOuv,TOBPlat : TOB; TousNiveaux:boolean; GardeReference : boolean=false; WithCalcul : boolean=true; SurLigneStd : boolean=false);
...
procedure InitValoDetail (TOBPiece,TOBOuv,TOBRef : TOB; DEV : Rdevise;CalculPv:boolean=true);
...
procedure GetOuvragePlat (TOBpiece,TOBL,TOBOuvrages,TOBPlat,TOBTiers : TOB;DEv : Rdevise; PourCpta: boolean = true);
...
procedure AppliqueCoefMargDetail (TOBOuvrage: TOB; CoefMarg : double);
...
implementation
uses factgrp,FactVariante,Facture
{$IFDEF BTP}
,UMetreArticle,UtilMetres,PiecesRecalculs,BTStructChampSup
{$ENDIF}
;
...
{$IFDEF BTP}
// Gestion des metres bibliothque dans le cadre d'un Ouvrage
procedure DocMetreBiblioDetail(TOBMere,TOBL:TOB);
Var FicArt: String;
    CodeArt: String;
    TypeArt: String;
...
begin
	...
	if TheMetreShare = nil then exit;

  TypeArt := TOBL.GetValue('BLO_TYPEARTICLE');
  CodeArt := TOBL.GetValue('BLO_ARTICLE');

  // Gestion de l'article en Prix Pos
  if TypeArt = 'ARP' then
     begin
        if ArtPrec = CodeArt then
           exit
        else
           artprec := CodeArt;
     end
  ...
  // Gestion des Metres => FV
  MetreArticle := TMetreArt.create;
  FicArt := MetreArticle.GesDoc(CodeArt,TypeArt);
  MetreArticle.free;

  // rcupration du fichier XLs dans le rpertoire bibliothque
  TheMetreShare.RecupMetreBiblio(TOBL, FicArt);

  if FicArt <> '' then
     TheMetreShare.CopieSousDetail;

end;

//Rcupration du fichier XLS dans le cas d'un Article
procedure DocMetreBiblioEntete(TOBL: TOB; CodeArt,TypeArt:String);
Var FicArt: String;
    MetreArticle: TMetreArt;
begin

  if (TypeArt <> 'OUV') then Exit;
  if TheMetreShare = nil then exit;

  if not GetParamSoc('SO_BTMETREDOC') then exit;
...
if DPR <> 0 then TOBOuv.PutValue('BLO_COEFMARG',arrondi(PuHT/DPR,4));
end;
...
BEGIN
  ...
  begin
    TheMillieme := TOBOUV.GetValue('MILLIEME'+IntToStr(NumCat));
  end;
...
//       TOBArt.SelectDB('"'+RefArticle+'"',Nil) ;
    	 QQ := OpenSql ('SELECT A.*,AC.*,N.BNP_TYPERESSOURCE,N.BNP_LIBELLE,'+
...
//    TOBL.PutValue('QUALIFQTEACH',TOBArt.GetValue('GA_QUALIFUNITEACH')) ;
    ...
    TOBL.PutValue('LIBELLEFOU',TOBArt.GetValue('LIBELLEFOU')) ;
//
...
begin
 ...
 TOBOUV.putValue('BLO_MONTANTPA',Arrondi((TOBOUV.GetValue('BLO_QTEFACT')*TOBOUV.GetValue('BLO_DPA'))/QteDudetail,V_PGI.okDecP));
 //
 ...
 begin
   if TOBOUV.GetValue('BLO_NONAPPLICFG')<>'X' then TOBOUV.putValue('BLO_MONTANTFG',arrondi(TOBOUV.GetValue('BLO_MONTANTPA')*TOBOUV.GetValue('BLO_COEFFG'),4));
   MontantCharge := TOBOUV.GetValue('BLO_MONTANTPA')+TOBOUV.GetValue('BLO_MONTANTFG');
 end else
 begin
   MontantPr := arrondi(QTe * TOBOUV.GetValue('BLO_DPR')/QteDuDetail,V_PGI.okdecP);
   TOBOUV.putValue ('BLO_MONTANTFG',MontantPr-TOBOUV.GetValue('BLO_MONTANTPA'));
 ...
 //
 if TOBOUV.GetValue('BLO_NONAPPLICFC')<>'X' then TOBOUV.putValue('BLO_MONTANTFC',arrondi(MontantCHarge*TOBOUV.GetValue('BLO_COEFFC'),V_PGI.okDecP));
 MontantCharge := TOBOUV.GetValue('BLO_MONTANTPA')+TOBOUV.GetValue('BLO_MONTANTFG')+TOBOUV.GetValue('BLO_MONTANTFC');
 //
 if TOBOUV.GetValue('BLO_NONAPPLICFRAIS')<>'X' then TOBOUV.putValue('BLO_MONTANTFR',arrondi(MontantCHarge*TOBOUV.GetValue('BLO_COEFFR'),V_PGI.okdecP));
 ...
 if Qte <> 0 then TOBOUV.putValue('BLO_DPR',arrondi((MontantCharge/Qte)*QteDudetail,V_PGI.okdecP)) ; // else TOBOUV.PutValue('BLO_DPR',TOBOUV.GetValue('BLO_MONTANTPR')*QteDudetail);
 if WithPuv then
 begin
   if (TOBOUV.GetValue('BLO_COEFMARG') <> 0) and (TOBOUV.GetValue('BLO_DPR')<>0 ) then
   begin
    ...
    InsertionChampSupOuv (TOBLND,false);
{
    ...
    *)
    ...
    if (Pos (TOBArt.getValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne) > 0) then
    begin
...
//    TOBART := TOBA.FIndfirst (['GA_ARTICLE'],[TOBNLD.GetValue('GNL_ARTICLE')],false);
    CopieOuvFromArt (TobPiece,TOBLND,TOBART,DEV);
    //
    ...
    begin
      ...
      else prix := TOBNLD.GetValue ('PVHT');
    ...
    if TOBLND.Getvalue ('BLO_DPR') <> 0 then
    begin
      TOBLND.putvalue ('BLO_COEFMARG',arrondi(TOBLND.Getvalue ('BLO_PUHT')/TOBLND.Getvalue ('BLO_DPR'),4));
    end else
    begin
    	if TOBLND.Getvalue ('BLO_PUHT') <> 0 then
      begin
        TOBLND.putvalue ('BLO_COEFMARG',1);
        TOBLND.putvalue ('BLO_DPR',TOBLND.Getvalue ('BLO_PUHT'));
      end else
      begin
...
var i : integer;
    nomChamp : string;
begin
for i:=1 to High(V_PGI.DEChamps[TOBRef.NumTable]) do
    begin
    Nomchamp := V_PGI.DEChamps[TOBREF.NumTable,i].Nom;
...
BEGIN
  ...
  BEGIN
    ...
    InitTableau (Valeurs);
    result := Entree_LigneOuv(XX,nil,TOBLN,TobArticles,TobL,TOBRepart,False,1,0,DEV,TOBL.Getvalue('GL_QUALIFQTEVTE'),valeurs,tamodif,EnHt,false,ModeSaisieAch,false) ;
...
for Indice:= 0 to TOBOuv.detail.count -1 do
    ...
    if TobDetOuv.Getvalue('BLO_TYPEARTICLE') <> 'POU' then
       ...
       valeur[7] := valeur [7] +Arrondi(((Qte/QteDuPv) * TOBDetOuv.GetValue ('BLO_PMRP')),V_PGI.okdecP);
        //
        if (Pos(TobDetOuv.GetValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne)>0) or (TobDetOuv.GetValue('BLO_TYPEARTICLE')='OUV') then
        begin
          valeur[9] := valeur[9] + Arrondi(((Qte/QteDuPv) * TOBDetOuv.GetValue ('BLO_TPSUNITAIRE')),V_PGI.OkDecP);
        end;
...
for Indice := 0 to TOBOuvrage.detail.count -1 do
    ...
    if ((TypeArticle = 'ARP') and (TOBOUV.Detail.count > 0)) or (TypeArticle = 'PRE') then
       ...
       if (Pos(TOBOuv.GetValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne)>0) then
//       if (Pos(TOBOuv.GetValue('BNP_TYPERESSOURCE'),'SAL;INT;')>0) then
       begin
...
BEGIN
  ...
  WithLigneFac := (pos(cledoc.NaturePiece,'FBT;DAC')>0);
...
BEGIN
  ...
  //if Not TOBOuv.InsertDB(Nil) then V_PGI.IoError:=oeUnknown ;
  // NEW ONE
...
if TOBOuvrage.Detail.Count-1<IndiceNomen-1 then begin result := false; Exit ; end;
    ...
    BEGIN
      ...
      begin
        ...
        begin
          ...
          if (TOBPere.GetValue('BLO_TYPEARTICLE')='ARP') and (TOBLoc.GetValue('BLO_TYPEARTICLE')='PRE') then
          begin
...
//             TOBPere.Putvalue ('BLO_TPSUNITAIRE',TOBLoc.GetValue('BLO_TPSUNITAIRE')*(TOBLoc.getValue('BLO_QTEFACT')/QteDuDetail)) ;
          ...
          if (TOBPere.GetValue('BLO_TYPEARTICLE')='ARP') and (TOBLoc.GetValue('BLO_TYPEARTICLE')='ARP') then
          begin
...
//      Qtedudetail := Qtedetail * TOBL.GetValue('BLO_QTEDUDETAIL');
      ...
      if Valloc[1] <> 0 then TOBL.PutValue('BLO_COEFMARG',arrondi(ValLoc[2]/Valloc[1],4));
    end;
    ...
    begin
      ...
      if (Pos(TOBL.GetValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne)>0) or (TOBL.GetValue('BLO_TYPEARTICLE')='OUV') then
//      if (Pos(TOBL.GetValue('BNP_TYPERESSOURCE'),'SAL;INT;')>0) then
      begin
        Valeurs[9] := Valeurs[9] + Arrondi(((QteLoc/(Qtedetail*QteDuDetail) * TOBL.GetValue ('BLO_TPSUNITAIRE'))),V_PGI.OkDecP);
      end;
    ...
    begin
      ...
      // if (Pos(TOBL.GetValue('BNP_TYPERESSOURCE'),'SAL;INT;')>0) then
      begin
      Valeurs[9] := Valeurs[9] + ((QteLoc/(Qtedetail*QteDuDetail) * TOBL.GetValue ('BLO_TPSUNITAIRE')));
      end;
  ...
  if IndPou >= 0 then
  begin
    ValLoc := CalculSurTableau ('*',ValPou,TOBOuvrage.detail[IndPou].getvalue('BLO_QTEFACT')/100);
    FormatageTableau (ValLoc,V_PGI.OkdecP);
...
InsertionChampSupOuv (TOBEcart,false);
  ...
  MontantEcart := Arrondi(MontantEcart,DEV.Decimale);
...
if not Q.Eof then
   ...
   CopieOuvFromArt (nil,TobEcart,TOBART,DEV);
   ...
   TobEcart.PutValue('BLO_QTEFACT',1) ;
...
//    TobEcart.putvalue ('BLO_PUHT',DeviseToPivotEx (MontantEcart,DEV.Taux,DEV.quotite,V_PGI.okdecP));
   ...
   TobEcart.PutValue ('BLO_QTEDUDETAIL',1);
   ...
   TOBARt.free;
   end else if TOBRef.GetValue('BLO_TYPEARTICLE') = 'MAR' then
   begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TOBLOC.putvalue('BLO_PUHT', devisetopivotEx(valeurs[2], DEV.Taux, DEV.quotite,V_PGI.okdecP));
          TOBLOC.putvalue('BLO_PUTTC', devisetopivotEx(valeurs[3], DEV.Taux, DEV.quotite,V_PGI.okdecP));
          if TOBLoc.GetValue('BLO_DPR')<>0 then TOBLoc.putValue('BLO_COEFMARG',arrondi(TOBLoc.GetValue('BLO_PUHT')/TOBLoc.GetValue('BLO_DPR'),4));
        end;
        TOBLOC.putvalue('BLO_TPSUNITAIRE', valeurs[9]);
      end else
      begin
    ...
    begin
      ...
      begin
        ...
        TOBLOC.detail[0].Putvaleur(IndPvHt, devisetopivotEx(TOBLOC.detail[0].getValeur(IndPvHtdev), DEV.Taux, DEV.quotite, V_PGI.okdecP));
        if TOBLoc.detail[0].GetValue('BLO_DPR')<>0 then TOBLoc.detail[0].putValue('BLO_COEFMARG',arrondi(TOBLoc.detail[0].GetValue('BLO_PUHT')/TOBLoc.detail[0].GetValue('BLO_DPR'),4));
        InitTableau(Valeurs);
        CalculeOuvrageDoc(TOBLOC, 1, 1, true, DEV, valeurs, EnHt);
        TOBLOC.putvalue('BLO_DPA', valeurs[0]);
        TOBLOC.putvalue('BLO_DPR', valeurs[1]);
...
procedure EssayeAffecterEcartQte (TOBL,TOBOuvrage : TOB; Ecart,EcartPr : double ; var SumLoc: double ; var SumPr : double;
																  IndPa,IndPR,IndPVHtDev,IndPVHt : integer; EnPA,EnPr,EnHt : boolean; DEV : Rdevise; TouslesNiveaux : boolean= false);
var Indice : integer;
...
begin
  ...
  begin
    ...
    if (ValTrav > 0) or (ValTrav < 10) then Continue;   // cotraité ou sous traité
    //
    ...
    begin
      ...
      LocSumLocPr := 0;
    	...
    	EssayeAffecterEcartQte (TOBL,TOBO,Encours,EncoursPr,LocSumLoc,LocSumLocPr,IndPa,IndPr,IndPVHtdev,IndPvHt,EnPa,EnPR,EnHt,DEV,TousLesNiveaux);
      ...
      restitue := restitue - (MontantLoc - lastMontantLoc);
      restituePr := restituePr - (MontantLocPr - LastMontantLocPr);
    ...
    begin
      ...
      begin
        ...
        LastMontantLocPr := TOBO.getValue('BLO_MONTANTPR');
        AffecteDifferenceSurLigne (TOBL,TOBO,EnCours,EnCoursPr,IndPa,IndPr,IndPVHtDev,IndPVHt,EnPa,EnPr,EnHt,DEV);
        CalculMontantHtDevLigOuv(TOBO, DEV);
        if EnPa then MontantLoc := TOBO.getValue('BLO_MONTANTPA')
        else if EnHt then MontantLoc := TOBO.getValue('BLO_MONTANTHTDEV')
        else MontantLoc := TOBO.getValue('BLO_MONTANTTTCDEV');
...
begin
  ...
  if CoefApplic=1 then exit; // si on ne change rien pas la peine de faire des calculs
//  if CoefApplic <> 0 then
  ...
  (*          if TOBLoc.getvaleur (IndPr) <> 0 then CoefMG:=TOBLoc.getvaleur (IndPvHtDev)/TOBLoc.getvaleur (IndPr)
        ...
        begin
          if (TobLoc.getvalue('ANCPV') <> PrixInterm) and
            (TobLoc.Detail.count > 0) then
            ReajusteMontantOuvrage(TOBArticles,TOBPiece,TOBL, TOBLoc, TOBLOc.getValue('ANCPV'), PrixPr, PrixInterm, DEV, EnHt, atributionEcart, EnPr);
        end;
        // --
        if ENPA then
        begin
  ...
  //          TOBLOC.Putvaleur(IndPvHt, devisetopivotEx(TOBLOC.Getvaleur(IndPvHtDev), DEV.Taux, DEV.quotite, V_Pgi.okdecP));
      ...
      if ArticleOKInPOUR(TOBLOC.GetValeur(IndTypeArt), ArticleOk) then
        MontantBasePou := MontantBasePou + MontantLoc;
      SumLoc := SumLoc + MontantLoc;
      SumPr := SumPr + MontantLocPr;
    ...
    begin
      ...
      if EnPA then
      begin
        TOBLOC.Putvaleur(IndPa, sumloc);
        TOBLOC.PutValue('GA_PAHT', TOBLOC.GetValue('BLO_DPA'));
        if not CoefFgFixe then TOBLOC.Putvaleur(IndPr, arrondi(sumloc * CoefPaPr, V_PGI.OkdecP));
        if TOBL.GetValue('GL_BLOQUETARIF') = '-' then
        begin
          TOBLOC.Putvaleur(IndPVhtDEV, arrondi(TOBLOC.Getvaleur(IndPr) * CoefPrPv, V_PGI.okdecP));
          TOBLOC.Putvaleur(IndPvHt, devisetopivotEx(TOBLOC.Getvaleur(IndPVHtDEv), DEV.Taux, DEV.quotite, V_PGI.okdecP));
      ...
      begin
        if TOBL.GetValue('GL_BLOQUETARIF') = '-' then
        begin
          TOBLOC.Putvaleur(IndPvHtdev, sumloc);
          TOBLOC.Putvaleur(IndPvHt, devisetopivotEx(MontantBasePou, DEV.Taux, DEV.quotite,V_PGI.okdecP));
        end;
      end;
      CalculMontantHtDevLigOuv(ToBLoc, DEV);
      {Correction suite a erreur dans le calcul prix march-- pb d'arrondi }
    ...
    begin
      ...
      begin
        ...
        if (Arrondi(MontantDemande - SumLoc,V_PGI.OKdecP) <> 0) and (atributionEcart) then
          AttribueEcart(TobPiece,TobOuvrage, ArtEcart, MontantDemande - Sumloc, DEV, ENHt, EnPA);
      end;
    end;
    SumLoc := 0;
    SumPr := 0;
    ...
    begin
      ...
      if TOBLOC.GetValeur(IndPr) <> 0 then TOBLOC.Putvalue('BLO_COEFMARG', arrondi(TOBLOC.GetValeur(IndPvHt)/TOBLOC.GetValeur(IndPr),4));
...
//  MontantPr := arrondi(MontantPr / QteDudetail, V_PGI.okdecP);
  MontantDemande := MontantDemande / QteDudetail;
  MontantPr :=  MontantPr / QteDudetail;
end;
...
begin
  ...
  begin
    ...
    TOBLND.PutValue('BLO_REMISABLELIGNE',TOBArt.GetValue('GA_REMISELIGNE')) ;
    ...
    InitValoDetail (TOBPiece,TOBLND,TOBART,DEV);
  end;
end;
...
var IndOuv,IndiceNomen,IndiceLig,TypePresent,NiveauImbric : integer;
    ...
    QteDuDetail,QteDUPv,Montant,MontantAchat,CumMontant,MontantOuv,MontantLig,MontantPaOuv,MontantLPa : double;
    IndicePou ,RowRef: Integer;
    EnHt : boolean;
    ArticleOk : string;
    Variante : boolean;
    TheTOBref : TOB;
    ModifSousDetail,ModifAvanc,AfficheSousDetail : boolean;
begin
  ModifSousDetail :=  (GetParamSocSecur('SO_BTMODIFSDETAIL',false)) AND (TOBL.GetValue('GLC_VOIRDETAIL')='X');
  AfficheSousDetail := (not GetParamSocSecur('SO_BTMODIFSDETAIL',false)) AND (TOBL.GetValue('GL_TYPEPRESENT')>DOU_AUCUN);
  ModifAvanc := (Pos(TOBPiece.GetValue('AFF_GENERAUTO'),'AVA;DAC')>0);
  IndicePou := -1;
  EnHT:=(TOBPiece.GetValue('GP_FACTUREHT')='X') ;
  CumMontant := 0; MontantPaOuv := 0; MontantLpa := 0;
  TheTOBRef := nil;
  ...
  begin
    ...
    begin
      ...
      TOB.Create('Les sousdetails',TOBL1,-1) ;
      ...
      begin
        MontantAchat := arrondi(TOBOUV.GetValue('BLO_DPA')* TOBL.getvalue('GL_QTEFACT') * (TOBOUV.GetValue('BLO_QTEFACT')/ (QteDuPv * QteDuDetail)),V_PGI.OkdecP);
        if EnHt then
        begin
        ...
        begin
          Montant := arrondi(TOBOUV.GetValue('BLO_PUTTCDEV')* TOBL.getvalue('GL_QTEFACT') * (TOBOUV.GetValue('BLO_QTEFACT')/ (QteDuPv * QteDuDetail)),V_PGI.OkdecP );
        end;
        // VARIANTE
        ...
        // --
        if ArticleOKInPOUR (TOBOUV.getValue('BLO_TYPEARTICLE'),ArticleOk) then CumMontant := CumMontant + Montant;
      end;
  ...
  if (TheTOBRef <> nil) and ((MontantLig <> MontantOuv) or (MontantLPa <> MontantPaOuv))and ((typepresent and DOU_MONTANT) = DOU_MONTANT) and  (not ModifSousDetail) then
  begin
    TOBL1 := TheTOBref;
...
var indice : integer;
    ...
    VoirDetail : boolean;
    TypePresent : integer;
    ModifSousDetail,ModifLeSousDetail : boolean;
begin
...
//    if (ModifSousDetail) and (not VoirDetail) then begin inc(Indice); continue; end;
...
begin
  ...
  for indZone := 1 to high(V_PGI.DeChamps[iTableLig]) do
  begin
    NomZone :=V_PGI.DeChamps[iTableLig,IndZone].Nom;
    ind := Pos ('_',NomZone);
...
procedure AplatOuvrage (TOBpiece,TOBL,TOBOuv,TOBPlat: TOB;Qte,QteDuDetail: double;TousNiveaux:boolean;GardeReference:boolean=false; WithCalcul : boolean=true; SurLigneStd : boolean=false);
...
//  TOBPiece := TOBL.Parent;
  ...
  begin
    ...
    begin
      ...
      APlatOuvrage (TOBPiece,TOBL,TOBLoc,TOBPLat,QTeSuite,QteDuDetailSuite,TousNiveaux,GardeReference,WithCalcul,SurLigneStd);
    end else
    begin
      if SurLigneStd then
      begin
...
APlatOuvrage (TOBPiece,TOBL,TOBLoc,TOBPlat,Qte,1,TousNiveaux,GardeReference,WithCalcul,SurLigneStd);
end;
...
//
  ...
  //
  iTableLig := PrefixeToNum (GetPrefixeTable(TOBLoc));
  for indZone := 1 to high(V_PGI.DeChamps[iTableLig]) do
  begin
    NomZone :=V_PGI.DeChamps[iTableLig,IndZone].Nom;
    ind := Pos ('_',NomZone);
...
for Indice := 0 To TOBOuv.detail.count - 1 do
    ...
    Req := OpenSql ('SELECT GA_PAHT,GA_DPR,GA_PMAP,GA_PMRP FROM ARTICLE WHERE GA_ARTICLE = "' + TOBLOc.getValue('BLO_ARTICLE') + '"',true,-1, '', True);
    if (not Req.eof) then
       begin
...
var i : integer;
    ...
    ChampDest : string;
begin
PrefixeRef := V_PGI.DETables[Tobref.numtable].Prefixe;
PrefixeDest := V_PGI.DETables[TobDest.numTable].Prefixe;
for i:=1 to High(V_PGI.DEChamps[TOBRef.NumTable]) do
     begin
     Suffixe := copy(V_PGI.DEChamps[TOBREF.NumTable,i].Nom,Length(PrefixeRef)+2,255);
     if Suffixe <> '' then
        begin
...
begin
if TOBL.GetValue ('GL_INDICENOMEN') = 0 then exit;
TOBN:=TOBOuvrage.Detail[TOBL.GetValue('GL_INDICENOMEN')-1] ;
if TOBN = nil then exit;
...
prefixe := GetPrefixeTable(TOBL);
TypeArticle := TOBL.getValue(Prefixe+'_TYPEARTICLE');
TypeNomen := TOBL.getValue(Prefixe+'_TYPENOMEN');
if  (TypeArticle = 'OUV') and (TypeNomen='OUV') then result := true;
end;
...
var Indice : integer;
		TOBOUv : TOB;
begin
  Indice := TOBL.getValue('GL_INDICENOMEN'); if Indice = 0 then exit;
  TOBOuv := TOBOuvrage.detail[Indice-1];
  if TOBOUv = nil then exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
      	TOBO.putValue('BLO_DPA',Arrondi(TOBO.GetValue('BLO_MONTANTPA')/TOBO.GetValue('BLO_QTEFACT'),V_PGI.OKDecV));
      	TOBO.putValue('BLO_DPR',Arrondi(TOBO.GetValue('BLO_MONTANTPR')/TOBO.GetValue('BLO_QTEFACT'),V_PGI.OKDecV));
      end else
      begin
    ...
    begin
      ...
      begin
        if (TOBO.GetValue('BLO_COEFMARG') <> 0) and (TOBO.getValue('BLO_DPR')<>0) then
        begin
...
//          TOBO.putValue('BLO_RECALCULER','X');
      ...
      begin
        ...
        begin
          ...
          if Pos(TOBO.getValue('BLO_NATUREPIECEG'),'FBT;ABT;FPR;FAC;AVC')>0 then
          begin
...
//          TOBO.putValue('BLO_COEFMARG',Arrondi(TOBO.getValue('BLO_PUHT')/TOBO.getValue('BLO_DPR'),4));
        end;
        if TOBPiece.getValue('GP_FACTUREHT')='X' then
        begin
          CalculeLigneHTOuv (TOBO,TOBPiece,DEV,true);
...
begin
  ...
  TOBO := TOBOuvrage.detail[IndiceNomen-1];
  GereTraitementOuvrage (TOBO,valeur,false,RazFG);
  TOBL.PutValue('GL_TPSUNITAIRE', Valeur[9] );
...
procedure ValideLesArticlesFromOuv (TOBArticles,TOBOuvrages : TOB);
  ...
  begin
    ...
    begin
      ...
      if (TOBA <> nil) and (TOBA.GetValue('SUPPRIME')<>'X') then TOBA.PutValue('UTILISE','X');
    end;
...
procedure GetOuvragePlat (TOBpiece,TOBL,TOBOuvrages,TOBPlat,TOBTiers : TOB;DEv : Rdevise; PourCpta: boolean = true);
var Indice, IndiceEcart,NumOrdre : integer;
...
//  CalculeLigneHT(TOBL,nil,TOBPiece,DEV,DEV.Decimale ,False,TOBTiers) ;
  ...
  MiseAPlatOuv (TOBpiece,TOBL,TOBouvrages,TOBPlat,true,true,TRUE);
  if (EnHt) then
  begin
    if (avancement) and (TOBL.FieldExists('BLF_MTSITUATION')) then
    begin
  ...
  if (MontantOuv <> MontantLignes) or (MontantOuvAc <> MontantLignesAc)then
  begin
    TOBPlat.detail[IndiceEcart];
    TOBIns:=NewTobLignePlat(TOBPLat,NumOrdre,-1);
    copyLigne (TOBIns,TOBLoc);
    ZeroLigne (TOBIns); TOBIns.putValue('BOP_PUHTDEV',0); TOBIns.putValue('BOP_PUTTCDEV',0);  TOBIns.putValue('BOP_DPA',0);
    ...
    begin
      PuHTDev := arrondi(MontantOuv-MontantLignes,Dev.decimale);
      if EnHt then
      begin
    ...
    if (Not PourCpta) and (ArtEcart <> '') then
    begin
      ...
      begin
        ...
        TobIns.putvalue('BOP_ARTICLE',ArtEcart);
        ArtEcart := Qart.fields[1].AsString;
        TobIns.putvalue('BOP_LIBELLE',ArtEcart);
        ferme (Qart);
      end;
    end;
...
begin
  ...
  QQ := OpenSQl('SELECT * FROM LIGNEOUVPLAT WHERE ' + WherePiece(CleDoc, ttdOuvrageP, False), True,-1, '', True);
...
var Indice : integer;
    TOBB : TOB;
begin
  for Indice := 0 to TOBOuvragesP.detail.count -1 do
  begin
    TOBB := TOBOuvragesP.detail[Indice];
    if TOBB.detail.count > 0 then
    begin
      if TOBB.FieldExists ('BOP_NATUREPIECEG') then
      begin
      ...
      ValideLesOuvPlat (TOBB,TOBPiece);
    end else
    begin
      if TOBB.FieldExists ('BOP_NATUREPIECEG') then
      begin
...
begin
  ...
  begin
    for Indice := 0 to TOBOUvrage.detail.count -1 do
    begin
      AppliqueCoefMargDetail(TOBouvrage.detail[Indice],Coefmarg);
    end;
  end;
end;
...
begin
  ...
  if (Pos(TOBL.detail[1].GetValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne)>0) then
  begin
...
procedure AjoutePrestationPrixPose (TOBPiece,TOBLigPiece,TOBOP,TOBArticles : TOB; PrestDefaut : string; DEV : Rdevise);
  ...
  begin
    ...
    //
    if (Pos (TOBL.getValue('BNP_TYPERESSOURCE'),VH_GC.BTTypeMoInterne) > 0) then
    begin
...
function OuvrageNonDifferencie (TOBPiece,TOBOuvrage : TOB) : boolean;
begin
	result :=  (TOBPiece.getValue('GP_UNIQUEBLO')=0) and (TOBOuvrage.detail.count > 0);
end;
...
begin
  ...
  begin
    ...
    if TOBOO.GetValue('BLF_MTSITUATION')=0 then continue;
