...
implementation
uses UTOF_VideInside,UiUtil,BTTVA_TOF;
...
begin
  ...
  if UneTob <> nil then
     Begin
	   if (length (MemoHaut.Text) <> 0) and (MemoHaut.text <> #$D#$A) and (MemoHaut.text <> '') then
        ...
        UneTob.PutValue('LO_OBJET', RichToString(MemoHaut));
        end;
     end
  Else
  ...
  if UneTob <> nil then
     Begin
        ...
        UneTob.PutValue('LO_OBJET', RichToString(MemoBas));
        end;
     end
  else
...
Begin
  ...
  UneTob.PutValue('LO_OBJET', RichToString(Memo));
