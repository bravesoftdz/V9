...
begin
  ...
  begin
  	if Pos('/01',TOBL.getValue('GL_CODEARTICLE')) > 0 then result.putValue('TYPESERVICE','Optimal')
    																							    else result.putValue('TYPESERVICE','Confort');
  end;
end;
...
//	fdocExcel :=  IncludeTrailingBackslash (TCBPPath.GetCegidDistriStd)+'MODELEPROPALLSE.xlsm';
	fdocExcel :=  'C:\PGI00\STD\MODELEPROPALLSE.xlsm';
//  PGIInfo('Repertoire de stcokage du modele : '+fdocExcel);
...
begin
    ...
    TRY
      ...
      QQ := OpenSql ('SELECT T_JURIDIQUE,T_LIBELLE,T_ADRESSE1,T_ADRESSE2,T_CODEPOSTAL,T_VILLE FROM TIERS WHERE T_TIERS="'+
...
begin
  ...
  PutValueDetail(fTOBPiece, 'GP_RECALCULER', 'X');
	CalculFacture (fTOBPiece,nil,fTOBouvrages,fTOBouvragesP,
  							 fTOBBases,fTOBBasesL,fTOBTiers,fTOBArticles,
                 fTOBPorcs,fTOBPieceRG,fTOBBasesRG, fDEV);
	GereEcheancesGC(fTOBpiece,fTOBTiers,fTOBEches,nil,fTOBPieceRG,taCreat,DEV,False) ;

  CalculeSousTotauxPiece(fTOBPiece);
  RecalculeRG(fTOBPORCS,fTOBPIECE, fTOBPIECERG, fTOBBASES, fTOBBASESRG, fDEV);
end;
...
begin
	fNewNum:=GetNumSoucheG(fTOBPiece.GetValue('GP_SOUCHE')) ;
  fTOBPiece.putValue('GP_NUMERO',fNewNum);
  IncNumSoucheG (fTOBPiece.GetValue('GP_SOUCHE'));
...
begin
  ...
  *)
  ...
  fTOBAffaire.putValue ('AFF_ETATAFFAIRE',	'ENC');
...
begin
  ...
  begin
  	DateDebut := DateDebutPeriode (TypeCalcul, DateDebut,
    															 fTOBAffaire.GetValue ('AFF_INTERVALGENER') ,
