...
uses
  ...
  FactCalc,
  // Modif BTP
  factrg, UMetreArticle,UtilMetres,UtilPhases,
  ...
  BTFactImprTOB,
  UtilArticle,
  AffaireUtil,uEntCommun,UtilTOBPiece;
...
procedure AnalyseDocumentBtp (FF : TForm ; TobArticles,TobPiece,TOBOuvrage : TOB);
...
procedure AppliqueChangementEnteteBtp (FF : TForm;TheTob,TobPiece,TobOuvrage,TOBPorcs : TOB; RegimeTaxe : string ; SavModApplicFg,SavModApplicFC  : boolean);
...
procedure MBDetailNomenClickBtp (FF : Tform ; IndiceNomen : integer;TOBaffaire, TobPiece,TobOuvrage,TobArticles,TOBBases,TOBBasesL,TOBL,TOBrepart : TOB ; addicted : boolean; DEV : Rdevise; ModeSaisieAch : boolean; CotraitOk : boolean);
...
function TraitePrixOuvrage(TOBPiece, TOBL, TOBBases, TOBBasesL,TOBOuvrage: TOB; EnHt: boolean; PuFix,PuReference: double; DEV: Rdevise; EnPA: boolean = false; FromReference : boolean=false): boolean;
...
function ControleSaisiePieceBtp (TOBPiece : TOB;AFFAIRE : string;TheAction : TactionFiche; transfopiece,duplic : boolean; Silencieux : Boolean = False) : boolean;
...
function ReajustePrixDevis (TheForm : TForm;TOBTiers,TOBLigneTarif,TOBTarif,TOBBases,TOBBasesL,TOBPiece,TOBArticles,TOBOuvrages: TOB ; DEV : RDevise) : boolean;
...
procedure TentativeAffectationQtive (TOBL,TOBOuvrage : TOB;EnPa,EnPr,EnHt : boolean;PuSaisi : double ; var SumLoc: double ; DEV:rdevise);
...
procedure PositionneAncienneFacturation(TOBL : TOB; DEV : Rdevise);
...
function ControleChantiertermineBTP (TOBPiece : TOB;Affaire : string; TransfoPiece,DuplicPiece : boolean) : boolean;
var naturepiece,EtatAffaire : string;
begin
	Naturepiece := TOBPiece.GetValue('GP_NATUREPIECEG');
	result := false; // par defaut
  if (Transfopiece) and (Naturepiece='FBT') then exit; // autorise
  if (duplicPiece) and (Naturepiece='ABT') then exit; // autorise
  if (Pos(Naturepiece,'DBT;PBT;CBT;CF;BLF;LFR;CFR;ETU;FBC;ABC;FBT;LBT;BFC;FF;AF;AFS;DEF;BFA;BCE;ABT;DAC')> 0) then
  begin
...
*)
      (not IsDebutParagraphe(TOBL)) then
    begin
      if TOBL <> nil then
      begin
...
//    if copy(TOBLOC.GetValue('GL_TYPELIGNE'), 1, 2) = 'DP' then
    ...
    EntreeAnalyseDocument(TypeAnal,TOBArticles, TOBPiece, TobOuvrage, ['LIBELLE=' + Libelle, Parametre], 2);
  end;
end;
...
procedure AppliqueChangeCoefMargOuvrage (LaTOB,TOBOUvrages : TOB);
...
begin
  ...
  begin
    AppliqueCoefMargDetail (TOBOuvrage,CoefMarg);
  end;
end;
...
begin
  ...
  begin
    ...
    if (LaTob.GetValue('GLC_NONAPPLICFRAIS') <> TOBLO.GetValue('GLC_NONAPPLICFRAIS')) or
    	 ...
    	 (LaTob.GetValue('GLC_NONAPPLICFG') <> TOBLO.GetValue('GLC_NONAPPLICFG'))
    then
    begin
    ...
    begin
      AppliqueChangeCoefMargOuvrage (LaTOB,TOBOUvrage);
    end;
    // VARIANTE
    (* if (copy(TheTOB.GetValue('GL_TYPELIGNE'), 1, 2) = 'DP') or *)
    if (IsDebutParagraphe(LaTOb)) then LaTOB.PutValue('GL_FAMILLETAXE1', '');
//      (copy(TheTOB.GetValue('GL_TYPEARTICLE'), 1, 2) = 'OU') or
//      (TheTOB.GetValue('GL_TYPEARTICLE') = 'ARP') then TheTOB.PutValue('GL_FAMILLETAXE1', '');
    //
    if (LaTob.GetValue('GLC_NONAPPLICFRAIS') <> TOBLO.GetValue('GLC_NONAPPLICFRAIS')) or
    	 (LaTob.GetValue('GLC_NONAPPLICFC') <> TOBLO.GetValue('GLC_NONAPPLICFC')) or
    	 (LaTob.GetValue('GLC_NONAPPLICFG') <> TOBLO.GetValue('GLC_NONAPPLICFG')) then
    begin
    	// dans le cas de modif de l'application des frais --> le recalcul du document remettra tout d'applomb
      // Surtout ne pas passer dans la procédure de modification du prix. 
    end else
    begin
      if ((LaTOB.getValue('GL_TYPEARTICLE') = 'OUV') or (LaTOB.getValue('GL_TYPEARTICLE') = 'ARP')) and
        ((TOBLO.GetValeur(IndicePv) <> LaTob.GetValeur(IndicePv)) or (TOBLO.GetValue('GL_QTEFACT') <> LaTob.GetValue('GL_QTEFACT'))) then
      begin
        IndiceNomen := LaTob.GetValue('GL_INDICENOMEN');
        if IndiceNomen > 0 then
        begin
          TOBNomen := TOBOuvrage.Detail[IndiceNomen - 1];
          if (TOBLO.Getvaleur(indicepv) <> LaTob.Getvaleur(indicepv)) then
          begin
            MontantInterm := LaTob.GetValeur(IndicePv);
            if not ReajusteMontantOuvrage(TOBArticles,TOBPiece,LaTOB,TOBNomen, TOBLO.Getvaleur(indicepv), MontantPr,MontantInterm, DEV, EnHt, true) then
            begin
  //            TheTob.PutValeur(IndicePR, MontantPr);
              if LaTOB.getValue('GL_BLOQUETARIF')='-' then
                  LaTob.PutValeur(IndicePv, MontantInterm);
              TOBPiece.PutValue('GP_RECALCULER', 'X');
              LaTob.PutValue('GL_RECALCULER', 'X');
            end;
          end;
          if (LaTOB.GetValue('GL_TYPEPRESENT') <> DOU_AUCUN) then
          begin
    ...
    begin
		...
		// modif brl suite appel COTE : FQ 12411
    if ((TOBLO.GetValue('GL_DPA') <> LaTob.GetValue('GL_DPA')) or
       (TOBLO.GetValue('GL_DPR') <> LaTob.GetValue('GL_DPR')) or
       (TOBLO.GetValeur(IndicePv) <> LaTob.GetValeur(IndicePv))) then
    begin
...
begin
  ...
  begin
    ...
    if Pos(Naturepiece,'CF;PBT;CBT') <= 0 then
    begin
...
//  chaine := 'UPDATE LIGNE SET GL_VIVANTE="X" WHERE GL_NATUREPIECEG="'+cledoc.NaturePiece+'" AND ' +
  chaine := 'UPDATE LIGNE SET GL_VIVANTE="X",GL_QTERESTE=GL_QTEFACT WHERE GL_NATUREPIECEG="'+cledoc.NaturePiece+'" AND ' +
...
begin
  ...
  if (TOBPiece_O.GetValue('GP_AFFAIREDEVIS') <> '') and (copy(TOBPiece_O.GetValue('GP_AFFAIREDEVIS'), 1, 1) = 'Z') and
      ((GetParamSoc('SO_AFNATAFFAIRE') = TOBPiece_O.GetValue('GP_NATUREPIECEG')) or (TOBPiece_O.GetValue('GP_NATUREPIECEG')='BCE')) then
  begin
...
begin
  ...
  begin
    ...
    MBAnal.visible := (VenteAchat = 'VEN') or (VenteAchat = 'AUT');
    ...
    //
    SimulationdeRentabilit1.visible := (VenteAchat = 'VEN');
    ...
    begin
      BprixMarche.Visible := False;
    	SimulationdeRentabilit1.visible := False;
    ...
    if Not JaiLeDroitConcept(TConcept(gcSaisModifRepres),False) then FFact.GP_REPRESENTANT.Enabled:=False; //confidentialité , concept acces remise
    if Not JaiLeDroitConcept(TConcept(bt113),False) then FFact.GP_REMISEPIED.Enabled:=False; //confidentialité , concept acces remise
    if Not JaiLeDroitConcept(TConcept(bt114),False) then FFact.GP_ESCOMPTE.Enabled:=False; //confidentialité , concept acces escompte
    if Not JaiLeDroitConcept(TConcept(bt115),False) then FFact.SimulationdeRentabilit1.Enabled:=False; //confidentialité , concept acces simulation
		{$ENDIF}    
  end;
end;
...
begin
	LastRow := Arow;
  if Acol = -1 then LastCol := 1 else LastCol := Acol;
  FFAct := TFFacture (FF);
  with FFact do
  begin
    ...
    begin
      IndiceNomen := TOBL.GetValue('GL_INDICENOMEN');
      if (IndiceNomen > 0) and ((TOBL.GetValue('GL_TYPEPRESENT') > DOU_AUCUN) or (TOBL.GetValue('GLC_VOIRDETAIL')='X')  )then
      begin
        GS.BeginUpdate;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AttribueEcart(TobPiece,TobNomen, ArtEcart, Delta, DEV, true, true,True);
      end;
    end;
...
begin
  ...
  begin
    ...
    if (Taction=taModif) and (FFact.ModifAvanc) then Taction := TaConsult;
    TOBNomen := TOBOuvrage.Detail[IndiceNomen - 1];
    if TOBNomen = nil then Exit;
    EnHT := (TOBPiece.GetValue('GP_FACTUREHT') = 'X');
    OkTrait := Entree_LigneOuv(FF,TOBaffaire,TOBNomen, TobArticles, TOBL,TOBRepart, False, 1, 0, DEV, TOBL.GetValue('GL_QUALIFQTEVTE'), valeurs, Taction, Enht, TypeArp, ModeSaisieAch,CotraitOk);
...
begin
  ...
  begin
    ...
    repeat
      ...
      resultOk := ReajusteMontantOuvrage(nil,TOBPiece,TOBL, TOBNomen, PuRef, PrCalc, PuFix, DEV, EnHt, false, false, EnPA);
      if ResultOk then
      begin
        if PuFix = PuSaisi then i := 4
        else
        begin
          Inc(i);
          if i < 4 then
    ...
    until (i = 4);
    if PuFix <> PuSaisi then
    begin
    	EnPr := false;
      TentativeAffectationQtive (TOBL,TOBNomen,EnPa,EnPr,EnHt,PuSaisi,PuFix,DEV);
    end;
    ...
    begin
      ...
      begin
        AttribueEcart(TobPiece,TobNomen, ArtEcart, PuSaisi - PuFix, DEV, ENHt, EnPA);
      end;
      PuFix := PuSaisi;
    end;
...
var OkArt: boolean;
  TOBArt, TOBL: TOB;
  ReferenceSav, UniteSav, RefUnique, LibSav,RefTiers: string;
  QteSav,PUSav: Double;
  FFact : TFFacture;
  PUFixe : boolean;
  QQ : TQuery;
...
begin
  ...
  begin
    ...
    begin
      ...
      PuSav := TOBL.GetValue('GL_PUHTDEV');
      ...
      begin
        ...
        QQ := OpenSql ('SELECT A.*,AC.*,N.BNP_TYPERESSOURCE,N.BNP_LIBELLE FROM ARTICLE A '+
                       ...
                       'LEFT JOIN ARTICLECOMPL AC ON AC.GA2_ARTICLE=A.GA_ARTICLE WHERE A.GA_ARTICLE="'+GS.Cells[SG_RefArt, ARow]+'"',true,-1, '', True);
        TOBArt.SelectDB('',QQ);
...
var mtAchat : double;
		QteReceptionne,FUA,FUV,FUS : double;
    QQ : TQuery;
    Requete : string;
    TOBLIGNes,TOBL : TOB;
    indice : integer;
begin
  ...
  TRY
    ...
    ferme (QQ);
    if (FUS*FUV) <> 0 then Qtereceptionne := QteReceptionne / (FUV*FUS);
...
//  requete := 'SELECT SUM(GL_QTERESTE) AS QTELIVRE FROM LIGNE WHERE GL_PIECEORIGINE="'+RefOrigine+
  requete := 'SELECT SUM(GL_QTERESTE) AS QTELIVRE FROM LIGNE WHERE GL_PIECEPRECEDENTE="'+RefOrigine+
             '" AND GL_NATUREPIECEG IN ("'+GetNaturePieceBLC+'") AND GL_QTERESTE <> 0';
  QQ := OpenSql (requete,true,-1, '', True);
  TRY
    if not QQ.eof then
    begin
  ...
  requete := 'SELECT SUM(GL_QTERESTE) AS QTERECEP,SUM (GL_QTESTOCK*GL_PUHT) AS MTACHAT FROM LIGNE WHERE GL_PIECEORIGINE="'+RefOrigine+
             '" AND GL_NATUREPIECEG IN ('+GetPieceAchat+') AND GL_QTERESTE <> 0';
  QQ := OpenSql (requete,true,-1, '', True);
  TRY
    if not QQ.eof then
    begin
  ...
  QteRecep := Qterecep - QteLivre; if QteRecep < 0 then QteRecep := 0;
...
begin
  ...
  begin
    ...
    begin
  	...
  	if ControleChantiertermineBTP (TOBPiece,Affaire,duplic,transfopiece) then
    begin
    	PgiError ('Saisie impossible, le chantier est clôturé');
    	result := false;
      exit;
    end;
  end;
  ...
  { OPTIMISATIONS LS
    ...
    begin
      ...
      if result = false then
        ...
        if Silencieux = False then PgiBox (TraduireMemoire('Attention, ce chantier n''est pas accepté'),TraduireMemoire('Contrôle chantier'));
    end;
  end;
...
procedure ReajustePrixSousDetail (TOBpiece,TOBLF,TOBOuvrage,TOBArticles : TOB; Mode : integer; DEV : RDevise; EnHt : boolean);
var TOBA,TOBL : TOB;
    Indice : integer;
    TypeArticle : string;
    VenteAchat : string;
...
begin
  ...
  begin
    ...
    begin
      ReajustePrixSousDetail (TOBPiece,TOBLF,TOBL,TOBArticles,Mode,DEV,EnHt);
    ...
    begin
      ...
      //
      if (Mode and 2) = 2 then // Si on reactualise le PR
      begin
      ...
      begin
        ...
        TOBL.putValue('BLO_COEFFG',TOBL.getValue('BLO_DPR')/TOBL.getvalue('BLO_DPA')-1);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ReajustePrixSousDetail (TOBPiece,TOBL,TOBO,TOBArticles,Mode,DEV,EnHt);
          InitTableau (Valeurs);
        ...
        begin
          ...
          if ((Mode and 4) = 4) and (TOBL.getValue('GLC_FROMBORDEREAU')<>'X') then // si on reactualise le PV
          begin
          ...
          begin
            ...
            TOBL.putValue('GL_COEFFG',TOBL.getValue('GL_DPR')/TOBL.getvalue('GL_DPA')-1);
          end;
          if (Mode and 2) <> 2 then
          begin
          ...
          if ((Mode and 4) = 4) and (TOBL.getValue('GLC_FROMBORDEREAU')<>'X') then
          begin
    ...
    FF.CalculeLaSaisie(-1, -1, True);
    DepileLesLignes (TFFacture(TheForm).CalculPieceAutorise, TOBPiece,TOBOuvrages,TOBSauve);
  end;
	TOBSauve.free;
end;
...
begin
  ...
  if (X.TransfoPiece) or (X.DuplicPiece) or (X.SaisieTypeAvanc) then
     begin
     X.ClickValide;
     Exit;
     End;
...
begin
  ...
  begin
    MessageAlerte('ATTENTION : La pièce est d''un montant nul');
		result := false;
  end;
end;
...
function PieceUniteautorisee (Nature : string) : boolean;
begin
  result := (Pos(Nature,'ETU;DBT;FBT;ABT;AFF;BCE;FPR;FAC;')>0);
end;
...
var QteSit,MtSit : double;
begin
  PrixPour := TOBL.GEtValue('GL_PRIXPOURQTE');
