...
begin
  ...
  THValComboBox(GetControl('GP_NATUREPIECEG')).Plus := ' AND GPP_NATUREPIECEG IN ("ABT","FBT","LBT","CBT","DBT","ETU","BCE","BBO","BFC","DAC","FAC","PBT")';
...
begin
  ...
  TRY
    ...
    if F.Name='BTRECALCPIECE_MUL' then
    begin
      if TOBPieces.detail.count > 0 then TraiteRecalculPieces (TOBPieces,CalculPv);
    end
    else if F.Name='BTMANPIECE_MUL' then
    begin
      TraiteManPieces (TOBPieces);
    end;
