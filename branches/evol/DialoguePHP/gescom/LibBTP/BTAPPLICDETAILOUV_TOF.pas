...
begin
  ...
  begin
    ...
    if TOBLL.getValue('GL_ARTICLE')=TOBL.GetValue('GL_ARTICLE') then
    begin
    ...
    if (TOBLL.getValue('GL_ARTICLE')=TOBL.GetValue('GL_ARTICLE')) and
    	 (TOBLL.getValue('GL_RECALCULER')='X') then
    begin
      // Calcul des montants
      InitTableau (Valeurs);
      CalculeOuvrageDoc (TOBOD,1,1,true,DEV,Valeurs,(TOBPiece.getValue('GP_FACTUREHT')='X'));
      TOBLL.Putvalue('GL_PUHTDEV',Valeurs[2]);
      TOBLL.Putvalue('GL_PUTTCDEV',Valeurs[3]);
      TOBLL.Putvalue('GL_PUHT',DeviseToPivot(TOBLL.GetValue('GL_PUHTDEV'),DEV.Taux,DEV.quotite));
      TOBLL.Putvalue('GL_PUTTC',DevisetoPivot(TOBLL.GetValue('GL_PUTTCDEV'),DEV.taux,DEV.quotite));
      TOBLL.Putvalue('GL_PUHTBASE',TOBLL.GetValue('GL_PUHT'));
      TOBLL.Putvalue('GL_PUTTCBASE',TOBLL.GetValue('GL_PUTTC'));
      TOBLL.Putvalue('GL_DPA',Valeurs[0]);
      TOBLL.Putvalue('GL_DPR',Valeurs[1]);
      TOBLL.Putvalue('GL_PMAP',Valeurs[6]);
      TOBLL.Putvalue('GL_PMRP',Valeurs[7]);
      TOBLL.putvalue('GL_TPSUNITAIRE',Valeurs[9]);
//      if not (TFFacture(FF).CalculPieceAutorise) then CalculeLaLigne (TOBPiece,TOBBases,TOBBasesL,TOBLL,TFFacture(FF).TheTOBTiers,DEV,TFFacture(FF).TheAction);
...
begin
  ...
  //
  NumeroteLigneOuv (TOBOD,TOBLL,1,1,0,0,0);
  //
end;
