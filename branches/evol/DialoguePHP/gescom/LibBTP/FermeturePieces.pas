...
begin
  ...
  end else if Pos(NaturePiece,'FBT;DAC')>0 then
  begin
...
begin
  ...
  begin
    If ExecuteSql ('UPDATE PIECE SET GP_VIVANTE="'+VIVANTE+'" WHERE '+
  ...
  if V_PGI.Ioerror = OeOk then
  begin
  	if (not ffermeture) and (pos(fTOBPiece.getValue('GP_NATUREPIECEG'),'DBT;FBT;ABT;FF;AF;AFS')>0) then
    begin
    	sQteMaj := ',GL_QTERESTE=GL_QTEFACT, GL_QTESTOCK=GL_QTEFACT ';
    end else if (not ffermeture) then
    begin
    	sQteMaj := '';
    end else begin
    	sQteMaj := ',GL_QTERESTE=0, GL_QTESTOCK=0 ';
    end;
