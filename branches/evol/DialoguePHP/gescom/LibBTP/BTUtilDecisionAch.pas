...
begin
  ...
  result := result + ' AND GL_QTERESTE <> 0 AND GL_VIVANTE="X" AND GP_VIVANTE="X"';
...
begin
  ...
  if fDepotFin <> '' then result := result + ' AND GL_DEPOT <="'+fDepotFin+'"';
  if fEtablissement <> '' then result := result + ' AND GL_ETABLISSEMENT ="'+fEtablissement+'"';
  result := result + ' AND GL_QTERESTE <> 0 AND GL_VIVANTE="X"';
...
begin
  ...
  begin
    ...
    begin
      ...
      if TOBA.getvalue('GL_COEFCONVQTE')<>0 then
      begin
        result := result + (TOBA.GetValue('GL_QTERESTE') * TOBA.GetValue('GL_COEFCONVQTE'));
      end else
      begin
...
var LivChantier : string;
		FUS,FUV,FUA,PQQ,COEFUAUS : double;
...
begin
  ...
  begin
  	...
  	LaTOBCOurante.putValue('BAD_QUANTITESTK',LaTOBCOurante.GetValue('BAD_QUANTITEINIT')*FUV/FUS);
    if CoefUaUs <> 0 THEN
    begin
			LaTOBCOurante.putValue('BAD_QUANTITEACH',LaTOBCOurante.GetValue('BAD_QUANTITEINIT')*FUV/FUS/CoefUaUs);
    end else
    begin
  		LaTOBCOurante.putValue('BAD_QUANTITEACH',LaTOBCOurante.GetValue('BAD_QUANTITEINIT')*FUV/FUA);
    end;
...
//  	LaTOBCOurante.putValue('BAD_AFFAIRE0',TOBL.GetValue('GL_AFFAIRE0'));
    ...
    begin
      ...
      begin
  		...
  		LaTOBCOurante.putValue('BAD_COEFUAUS',CoefuaUs);
      LaTOBCOurante.putValue('BAD_PRIXACH',TOBL.GetValue('GL_DPA'));
      LaTOBCOurante.putValue('BAD_PRIXACHNET',TOBL.GetValue('GL_DPA'));
      LaTOBCOurante.putValue('BAD_FOURNISSEUR',LeFournisseur);
    end else
    begin
      TarifFourn := GetInfosTarifAch (LeFournisseur,Larticle,TurAchat,false,true,nil,LaTOBCOurante.GetValue('BAD_QUANTITEACH'),TOBL.GetValue('GL_DATELIVRAISON'));
