...
function Entree_Simulation (TOBArticles,TOBPiece,TOBOuvrage,TOBPorcs : TOB; Caption : string;Action:TactionFiche; DEV: Rdevise;Paragraphe : boolean=true):boolean;

implementation
uses FactBordereau,NomenUtil;
{$R *.DFM}
...
if GS_PARAG <> -1 then
   ...
   GS.ColWidths [GS_PARAG] := 150;
   GS.ColLengths [GS_PARAG] := 150;
   GS.ColAligns[GS_PARAG]:=taLeftJustify;
   end;
//
GS.Cells[GS_CATEGORIE,0] := 'Catégorie';
GS.ColWidths [GS_CATEGORIE] := 60;
GS.ColLengths [GS_CATEGORIE] := 60;
GS.ColAligns[GS_CATEGORIE]:=taLeftJustify;
//
GS.Cells[GS_NATURE,0] := 'Nature';
GS.ColWidths [GS_NATURE] := 60;
GS.ColLengths [GS_NATURE] := 60;
...
GS.Collengths [GS_NUMLIG] := 30;
GS.Collengths [GS_OPEN] := 18;
if GS_PARAG <> -1 then GS.ColLengths [GS_PARAG] := 150;
GS.ColLengths [GS_CATEGORIE] := 60;
...
procedure TFSimulRentab.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
TOBSimulPres.free; TOBSimulPres_O.free;
TOBPieceFrais.free;
end;
...
//
CreationTOBSimul(TobSimulPres,TOBPIECE,TOBOuvrage,TOBArticles,Paragraphe,GestionInterdit);
CreationTreeView;
...
changeValue(TscCoefMA,AncHrs,NewHrs,TOBL,gestionInterdit);
FinaliseCalcul (TOBSimulPres);
DesactiveEventGS;
AfficheLaGrille (GS,ListeSaisie,TobSimulPres,Niveau,NbLigne,Paragraphe);
GS.row := SavRow;
...
begin
  ...
  RecalcSuiteModif (TOBPiece,TOBOuvrage,TOBMod,DEV);
  if TOBMod.detail.count = 0 then ValidateOk := False;
  TOBMod.free;
  close;
end;
...
//
if TOBL = nil then exit;
if TOBL.GetValue('NIVEAU') <> 4 then BEGIN TdetailVal.Visible := False; Exit; END;
