...
//	result := true;
  ...
  begin
    ...
    if TOBL.GetValue ('GL_QTEFACT') <> TOBL.GetValue ('GL_QTERESTE') then BEGIN result := false; break; END;
  end;
end;
...
//  if not fusable then exit;
  ...
  if (fMode=tmlchantier) and (Usable) then
     ...
     fIdentifiantWol := 0;
  if (Usable) and (fnaturepiece<> '') and (Pos(fNaturePiece,'ETU;FRC;DBT;FBT;ABT;CBT;LBT;')=0) then
  begin
  	DefiniReferenceLivraison;
  end;
end;
...
begin
	fmode := tmlfournisseur;
  fIdentifiantWol := 0;
  if (fnaturepiece<> '') and (Pos(fNaturePiece,'ETU;FRC;DBT;FBT;ABT;CBT;LBT;')=0) then
  begin
  	DefiniReferenceLivraison;
  end;
end;
...
begin
  ...
  if fMode = tmlChantier then SetAdresseLivChantier (MemoTemp)
  else if fMode = tmlDepot then SetAdresseDepot (TOBL,MemoTemp)
  else MemoTemp.add (TraduireMemoire('Mise à disposition dans vos locaux'));
