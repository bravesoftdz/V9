...
uses BTPUtil,UtilSaisieConso,FactUtil,FactComm;
...
begin
  ...
  if (Naturepiece = 'CF') OR (Naturepiece = 'BLF') OR
     ...
     (Naturepiece = 'FF') OR (Naturepiece = 'PBT') then result := true;
end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      Select := 'SELECT GA_PAHT,GA_DPR,GA_PVHT FROM ARTICLE WHERE GA_ARTICLE="'+TOBLigne.GetValue('GL_ARTICLE')+'"';
      Q := OpenSql (Select,true);
      if not Q.eof then
      begin
      	if Q.findfield('GA_PAHT').AsFloat > 0 then CoefPaPR := Q.findfield('GA_DPR').AsFloat/Q.findfield('GA_PAHT').AsFloat;
      	if Q.findfield('GA_DPR').AsFloat > 0 then CoefPRPV := Q.findfield('GA_PVHT').AsFloat/Q.findfield('GA_DPR').AsFloat;
      end;
      ferme (Q);
    end;
    *)
  ...
  begin
    ...
    begin
      ...
      Select := 'SELECT * FROM CONSOMMATIONS WHERE BCO_NUMMOUV='+inttostr(TOBLigne.GetValue('BLP_NUMMOUV'))+' AND '+
                ...
                'BCO_INDICEG='+InttoStr(TOBLigne.GetValue('GL_INDICEG'));
      Q := OpenSql (select,true);
      if not Q.eof then
      begin
        TOBConso.LoadDetailDB ('CONSOMMATIONS','','',Q,True);
        TOBC := TOBConso.FindFirst (['BCO_NUMMOUV','BCO_INDICE'],[TOBLigne.GetValue('BLP_NUMMOUV'),0],true);
        if TOBC <> nil then
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TOBC.PutValue('BCO_SEMAINE', Semaine);
          TOBC.PutValue('BCO_DATEMOUV', DateMouv);
          TOBC.PutValue('BCO_QUANTITE', TobLigne.GetValue('GL_QTERESTE'));
          TOBC.PutValue('BCO_DPA', TobLigne.GetValue('GL_PUHTNET')/(TOBLigne.getValue('GL_PRIXPOURQTE')));
          TOBC.PutValue('BCO_DPR', Arrondi(TOBC.GEtValue('BCO_DPA')*CoefPaPr,V_PGI.okdecP));
          TOBC.PutValue('BCO_PUHT',Arrondi(TOBC.GEtValue('BCO_DPR')*CoefPRPV,V_PGI.okdecP));
          //
