...
implementation
uses factouvrage,FactArticle,UtilMetres,LigNomen,FactCalc,facture;
// ---------------------------------------------------
...
for i:=0 to TOBNOMEN.Detail.Count-1 do
    ...
    IF (TOBL.GetValue('GL_TYPEARTICLE')='OUV') and
       ...
       Else if TOBL.GetValue('GL_NATUREPIECEG') = 'FBT' then
          DocMetreBiblioDetail (TOBNLD,TOBLND)
...
//Description .. : Appel de la recherche du fichier métrés associé à l'ouvrage
{$IFDEF BTP}
	if (TypeArt='OUV') and (VenteAchat = 'VEN')  then
  if (TOBL.GetValue('GL_NATUREPIECEG') = GetParamSoc('SO_AFNATAFFAIRE')) or
    (TOBL.GetValue('GL_NATUREPIECEG') = 'FBT') or
    (TOBL.GetValue('GL_NATUREPIECEG') = GetParamSoc('SO_AFNATPROPOSITION')) then
  begin
...
{$ENDIF}
   ...
   InitTableau (Valeurs);
   result := Entree_LigneOuv(XX,nil,TOBLN,TobArticles,TobL,TOBRepart,False,1,0,DEV,TOBL.Getvalue('GL_QUALIFQTEVTE'),valeurs,tamodif,EnHt,false,false,false) ;
