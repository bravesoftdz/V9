...
var
Deb, Fin,indice,X : integer;
Critere : string;
MenuPop : Tpopupmenu;
DroitCreat : boolean ;
Chaine,Valeur,ChampMul,ValMul : string;
icol : integer;
CC : THlabel;
BEGIN
Inherited;
...
{$ENDIF}
      ...
      End else if copy(ecran.Name,1,12) = 'BTPREST_RECH' then
      begin
      //
//
        repeat
          ...
          begin
            ...
            begin
              ...
              if ChampMul = 'GA_TYPEARTICLE' then
              begin
      					ConstituePlusTypeArticle (ValMul);
              end else
              if ChampMul = 'TYPERESSOURCE' then
              begin
          			ConstituePlusTypeRessource (ValMul);
              end;
...
DroitCreat:=JaiLeDroitConcept(TConcept(gcArtCreat),False) ;
if not DroitCreat then
begin
  if ButInsert <> nil then ButInsert.visible := false;
  if TtoolBarButton97(GetControl('B_DUPLICATION')) <> nil then
  begin
    TtoolBarButton97(GetControl('B_DUPLICATION')).Visible := false;
  end;
end;
...
var iInd : integer;
    ...
    TobRetour,TobRF : TOB;
begin
  if GetCheckBoxState('RETOUR_CODEARTICLE') = cbChecked then FieldArticle := 'GA_CODEARTICLE'
  else FieldArticle := 'GA_ARTICLE';
  with TFMul(Ecran) do
  begin
...
begin
    ...
    begin
      ...
      SousFamilleTarif.Plus := 'and CC_LIBRE like "'+FamilleTarif.Value+'%"';
    end else
    begin
