...
Begin
  ...
  Begin
	...
	if Champ ='STATUT' then
     ...
     Else if Valeur = 'PRO' then Affaire0 := 'A'
     else Affaire0 := 'A';
     end;
end;
...
begin
  ...
  if GetParamSoc('SO_BTFACAPPELDETAIL') then
     CreerPiecesFromLignes(TobLigne,'FACDETAILCONSO', V_PGI.DateEntree, true, false, TobResult)
  Else
     CreerPiecesFromLignes(TobLigne,'FACCONSO', V_PGI.DateEntree, true, false, TobResult);
  //
...
Begin
	...
	For i := 0 to TobResult.detail.count-1 do
      ...
      For j := 0 To Tobresult.Detail[I].detail.Count-1 Do
          ...
          Req := 'UPDATE AFFAIRE SET AFF_ETATAFFAIRE="FAC" ' +
                 'WHERE AFF_AFFAIRE="' + NumAff + '"';
          ExecuteSQL(Req);
          End;
      End;
...
//  AddLesSupLigne (TOBL,false);
  ...
  TOBL.PutValue('GL_PUHTDEV', TobConso.GetValue('BCO_PUHT'));
