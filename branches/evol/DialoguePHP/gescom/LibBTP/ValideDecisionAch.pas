...
begin
  ...
  begin
    ...
    begin
    	PgiBox (Traduirememoire('Une erreur s''est prpduite lors de la fin de la procédure'),'Validation');
    end else
    begin
    	if fChantierTermine then
      begin
    		PgiInfo (Traduirememoire('Des lignes en procenance de chantier terminé ont été ignoré'),'Validation');
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    MTPAN :=  Arrondi(MTPAN * CoefuaUs,V_PGI.okdecP);
  end else
  begin
    MTPAF :=  Arrondi((MTPAF / FUS ) * FUA,V_PGI.okdecP);
    MTPAN :=  Arrondi((MTPAN / FUS ) * FUA,V_PGI.okdecP);
  end;
  // --
...
begin
  ...
  CalculFacture (TOBPiece,nil,nil,nil,TOBBase,TOBBasesL,TOBTier,TOBArticles,nil,nil,nil,DEVbesoin,false);
