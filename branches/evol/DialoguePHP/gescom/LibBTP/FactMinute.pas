...
var TOTPATOT,TOTPVTOT,TOTHRSTOT : double;
...
for indice := 0 to TOBPiece.detail.count -1 do
    ...
    // VARIANTE
    if ((TypeArticle='MAR') or (TypeArticle = 'PRE') or (TypeArticle = 'POU') or (TypeArticle = 'ARP')) and (not IsVariante(TOBL)) then
       begin
...
lanceEtatTob ('E','GPJ','BMN',TOBMinute,true,false,false,nil,'','Edition de la minute',false);
FINALLY
TOBMinute.free;
END;
end;
