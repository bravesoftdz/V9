...
uses FactOuvrage,NomenUtil,FactTOB,UtilTOBPiece;
const
  TabLse = #$09;
...
begin
  Sql := 'SELECT GP_NATUREPIECEG,GP_SOUCHE,GP_NUMERO,GP_INDICEG,GP_TIERSFACTURE,'+
         'GP_DATEPIECE,GP_MODEREGLE,GPE_MODEPAIE,GPE_DATEECHE,T_AUXILIAIRE '+
         'FROM PIECE '+
...
begin
	result := TrimLeft(trimright(IntToStr(TOBLigne.GetValue('GL_NUMERO'))))+
  					TabLse +
            TrimLeft(trimRight(TOBLigne.GetValue('GL_CODEARTICLE'))) +
  					TabLse +
...
begin
  ...
  Sql := 'SELECT GL_NUMERO,GL_CODEARTICLE,GL_LIBELLE,GL_QTEFACT,GL_PUHTDEV '+
         'FROM LIGNE '+
...
var Indice,IInd : integer;
    ...
    TOBPiece,TOBAdresses,TOBPiecesExp,TOBLignes : TOB;
    F,FD : textfile;
    cledoc : r_cledoc;
    Q : TQuery;
    XX : TForm;
...
begin
 ...
 //
 // SPECIF LSE
 RegisterAglProc( 'BTPExportefactures',True,1,GEPExportefactures);
 //
end;
