...
Type
  ...
  private
    ...
    TOBDEVIS,TOBIntermediaire,TOBCHANTIER,TOBOuvrage,TOBGenere,TOBLiaison,TOBA,TOBFrais,TOBOuvrageFrais :TOB;
    TOBAdresses : TOB;
...
function SetPlannification (TOBDEVIS,TobOuvrage,TOBFrais,TOBOUvrageFrais : TOB) : boolean;

Implementation
uses BTPUtil,Facture;

function SetPlannification (TOBDEVIS,TobOuvrage,TOBFrais,TOBOuvrageFrais : TOB) : boolean;
var st,Param : string;
		leChoix : integer;
    cledocResult : R_Cledoc;
begin
result := true;
...
begin
  ...
  TRY
    ...
    begin
      ...
      (*
      stchaine := EncodeRefPiece (TOBDevis);
      DecodeRefPiece (stchaine,cledoc);
      cledoc.DatePiece := V_PGI.DateEntree;
      cledoc.NaturePiece := getparamsoc('SO_BTNATCHANTIER');
      *)
    end else
    begin
//      G_LigneCommentaireBTP (TOBPiece,TOBgenere.detail[0]);
...
TOBS.PutValue ('FOURNISSEUR',TOBL.GetValue('GL_FOURNISSEUR'));
...
if (ListPhase = nil) or (ListPhase.count = 0) then
  ...
  Begin
    ...
    begin
      result := TOBDebut.findfirst (['PHASE','LIGNEPHASE','OUVRAGE','LIGNEOUV','ARTICLE','FOURNISSEUR'],
                [Phase,LignePhase,CurrentOuv.GetValue('GL_ARTICLE'),CurrentOuv.GetValue('GL_NUMLIGNE'),
                TOBL.GetValue('GL_ARTICLE'),TOBL.GetValue('GL_FOURNISSEUR')],true);
    end else
    begin
      result := TOBDebut.findfirst (['PHASE','LIGNEPHASE','OUVRAGE','LIGNEOUV','ARTICLE'],
                [Phase,LignePhase,CurrentOuv.GetValue('GL_ARTICLE'),CurrentOuv.GetValue('GL_NUMLIGNE'),
  ...
  Begin
  	if TOBL.GetValue('GL_FOURNISSEUR') <> '' then
    begin
      result := TOBDebut.findfirst (['PHASE','LIGNEPHASE','OUVRAGE','LIGNEOUV','ARTICLE','FOURNISSEUR','DPA'],
                [Phase,LignePhase,CurrentOuv.GetValue('GL_ARTICLE'),CurrentOuv.GetValue('GL_NUMLIGNE'),TOBL.GetValue('GL_ARTICLE'),
                TOBL.GetValue('GL_FOURNISSEUR'),TOBL.GetValue('GL_DPA')]
                ,true);
    end else
    begin
      result := TOBDebut.findfirst (['PHASE','LIGNEPHASE','OUVRAGE','LIGNEOUV','ARTICLE'],
...
begin
Inherited ;
if GetParamSoc ('SO_BTLIVCHANTIER') then fLivChantier := True
                                    else fLivChantier := false;
