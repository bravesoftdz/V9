...
{$ENDIF}
 DicoBTP,NomenUtil,FactOuvrage,FactArticle,FactUtil,FactVariante,SysUtils,BTPUtil;
...
procedure AppliqueChangement (Change: T_SimulChg;CoefChange:double;TOBSimulPres:TOB;var ListChange : Tlist;ValeurFixe : double=0; GestionInterdit : TSGestionInterdit=[]);
...
procedure ChangeValue(Change: T_SimulChg;AncValeur,NewValeur:double;TOBSimulPres:TOB; GestionInterdit : TSGestionInterdit);
procedure CreationTOBSimul(TobSimulPres,TOBPIECE,TOBOuvrage,TOBArticles : TOB; AvecParagraphe : boolean; GestionInterdit : TSGestionInterdit);
...
procedure RecalcSuiteModif (TOBPiece,TOBOUvrage,TOBMod : TOB; DEV : Rdevise );
...
begin
  ...
  begin
    ...
    if (TOBL.GetValue('GL_TYPEARTICLE') = 'PRE') or (TOBL.GetValue('GL_TYPEARTICLE') = 'MAR') or (TOBL.GetValue('GL_TYPEARTICLE') = 'FRA') or
...
var QQ : TQUERY;
    ...
    prefixe : string;
begin
if TOBOUV <> nil then BEGIN TOBREF := TOBOUV; Prefixe := 'BLO'; END
                 else BEGIN TOBREF := TOBL; Prefixe := 'GL'; END;
if TOBREF.GetValue(prefixe+'_TYPEARTICLE') = 'PRE' then
   begin
   Prestation := true;
   TOBPREST := TOB.Create ('NATUREPREST',nil,-1);
   QQ := OpenSQL ('SELECT BNP_LIBELLE,BNP_TYPERESSOURCE FROM NATUREPREST WHERE BNP_NATUREPRES="'+TOBA.GetValue('GA_NATUREPRES')+'"',true,-1,'',true);
   if not QQ.eof then
      begin
      TOBPrest.SelectDB ('',QQ);
      Categorie := TOBPRest.GetValue('BNP_TYPERESSOURCE');
      if categorie <> '' then LibCategorie := RechDom ('AFTTYPERESSOURCE',TOBPREST.GetValue('BNP_TYPERESSOURCE'),false)
                         else LibCategorie := 'Non renseigné';
      Nature := TOBA.GetValue('GA_NATUREPRES');
   ...
   if Nature <> '' then LibNature := RechDom ('GCFAMILLENIV1',TOBREF.GetValue(Prefixe+'_FAMILLENIV1'),false)
                   else LibNature := 'Non renseigné';
   Prestation := False;
   end;
...
begin
  ...
  begin
    ...
    if (MontantPAFCH+MontantFGH) <> 0 then TOBS.PutValue ('COEFFCH',Arrondi((MontantPAFCH+MontantFCH+MontantFGH )/(MontantPAFCH+MontantFGH),9))
                         else TOBS.PutValue ('COEFFCH',0);
    if (MontantPAFRH+MontantFCH+MontantFGH) <> 0 then TOBS.PutValue ('COEFFRH',Arrondi((MontantPAFRH+MontantFRH+MontantFCH+MontantFGH)/ (MontantPAFRH+MontantFCH+MontantFGH),9))
    ...
    if (MontantPAFCQ+MontantFGQ) <> 0 then TOBS.PutValue ('COEFFCQ',Arrondi((MontantPAFCQ+MontantFCQ+MontantFGQ )/(MontantPAFCQ+MontantFGQ),9))
                         else TOBS.PutValue ('COEFFCQ',0);
    if (MontantPAFRQ+MontantFCQ+MontantFGQ) <> 0 then TOBS.PutValue ('COEFFRQ',Arrondi((MontantPAFRQ+MontantFRQ+MontantFCQ+MontantFGQ)/ (MontantPAFRQ+MontantFCQ+MontantFGQ),9))
    ...
    if (MontantPAFC+MontantFG) <> 0 then TOBS.PutValue ('COEFFC',Arrondi((MontantPAFC+MontantFC+MontantFG )/(MontantPAFC+MontantFG),9))
                        else TOBS.PutValue ('COEFFC',0);
    if (MontantPAFR+MontantFC+MontantFG) <> 0 then TOBS.PutValue ('COEFFR',Arrondi((MontantPAFR+MontantFR+MontantFC+MontantFG)/ (MontantPAFR+MontantFC+MontantFG),9))
...
begin
  ...
  begin
    ...
    if TOBS.GetValue('NIVEAU') <> 5 then
    begin
      AppliqueChangement(Change,CoefChange,TOBS,ListChange,ValeurFixe,GestionInterdit);
    end;
    if TOBS.GetValue('NIVEAU') = 5 then
    begin
...
begin
  ...
  BEGIN
  	AppliqueChangement (Change,0,TOBSimulPres,ListChange,NewValeur,GestionInterdit);
  END else
  BEGIN
    if AncValeur <> 0 then
...
begin
  ...
  begin
    ...
    if (TOBS.GetValue('NIVEAU')=5) and (TOBS.GetValue('MODIFIE')='X') then
    begin
      TOBL := TOB(TOBS.data);
...
begin
  ...
  begin
    ...
    if Pos(TOBL.GetValue('GL_TYPEARTICLE'),'OUV;ARP;')=0 then continue;
    if TOBL.GetValue('GL_INDICENOMEN') = 0 then continue;
