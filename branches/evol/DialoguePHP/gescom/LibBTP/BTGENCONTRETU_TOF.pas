...
uses factvariante,FactTOB,FactComm, FactUtil,FactGrp;
...
begin
  ...
  IoErr := TRANSACTIONS (ValideLaCtrEtude,1);
  if IOerr <> OeOk then
  begin
...
function TOF_BTGENCONTRETU.GenereTobIntermediaire: boolean;
var Indice : integer;
...
begin
  ...
  begin
    result := copy(trim(TOBL.GetValue('LIBELLE')),1,35);
  end else
  begin
    result := '['+TOBL.GetValue('ARTICLE')+'] '+ copy(trim(TOBL.GetValue('LIBELLE')),1,35);
