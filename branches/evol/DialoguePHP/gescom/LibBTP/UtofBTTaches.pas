...
begin
  ...
  AnneeDeb := FormatDateTime('yyyy',StrToDate(GetControlText('ATA_DATEDEBPERIOD')));
  AnneeFin := FormatDateTime('yyyy',StrToDate(GetControlText('ATA_DATEFINPERIOD')));

  if StrToDate(GetControlText('ATA_DATEDEBPERIOD')) = idate1900 then
     Begin
  ...
  if StrToDate(GetControlText('ATA_DATEFINPERIOD')) = idate2099 then
     Begin
     PGIBoxAF(TexteMessage[62], ecran.Caption);
     SetControlText('ATA_DATEFINPERIOD',dateToStr(PlusDate(now,5,'A')));
     ftobdet.putvalue('ATA_DATEFINPERIOD',GetControlText('ATA_DATEFINPERIOD'));
     AnneeFin := FormatDateTime('yyyy', StrToDate(GetControlText('ATA_DATEFINPERIOD')));
     end;

  if StrToDate(GetControlText('ATA_DATEFINPERIOD')) > PlusDate(now,5,'A') then
     Begin
     PGIBoxAF(TexteMessage[62], ecran.Caption);
     SetControlText('ATA_DATEFINPERIOD',dateToStr(PlusDate(now,5,'A')));
     ftobdet.putvalue('ATA_DATEFINPERIOD',GetControlText('ATA_DATEFINPERIOD'));
...
begin
  ...
  TobAppel.PutValue('AFF_DEVISE', GetControlText('ATA_DEVISE'));

  TobAppel.PutValue('AFF_TIERS', GetControlText('ATA_TIERS'));
  ...
  *)
  ...
  if GetControlText('ATA_UNITETEMPS') = 'H' Then
  begin
    DateDeFin := IncHour(DateDeb,trunc(Duree));
  end else if GetControlText('ATA_UNITETEMPS') = 'J' Then
  begin
    DateDeFin := IncDay(DateDeb,trunc(Duree));
  end else if GetControlText('ATA_UNITETEMPS') = 'MIN' Then
  begin
    DateDeFin := IncMinute(DateDeb,trunc(Duree));
  end;
  ...
  TobAppel.PutValue('AFF_AFFAIREINIT', GetControlText('ATA_AFFAIRE'));
  TobAppel.PutValue('AFF_CHANTIER', fCurAffaire.StChantier);
  TobAppel.PutValue('AFF_NUMEROTACHE', GetControlText('ATA_NUMEROTACHE'));

  //Contrôle du contact
  Contact := GetControlText('CLIENTCONTACT');
  if Contact <> '' then
  ...
  BlocNote :=THRichEditOLE(GetControl('ATA_DESCRIPTIF'));
  if (Length(BlocNote.Text) = 0) or (BlocNote.Text = #$D#$A) then
  Begin
     TobAppel.PutValue('AFF_DESCRIPTIF', GetControlText('ATA_LIBELLETACHE1') + ' ' + GetControlText('ATA_LIBELLETACHE2'));
  end else
  Begin
     TobAppel.PutValue('AFF_DESCRIPTIF', GetControlText('ATA_DESCRIPTIF'));
  end;
  ...
  Ressource := RechRessPrinc(GetControlText('ATA_AFFAIRE'));
  ...
  NoAdresse := 0;
  Req := 'SELECT ADR_NUMEROADRESSE FROM ADRESSES WHERE ADR_REFCODE="' + CodeAppel + '" AND ADR_TYPEADRESSE="INT"';
  QQ := OpenSQL(Req, true);
  if QQ.EOF then
     Begin
  ...
  TobAdrInt.PutValue('ADR_LIBELLE', GetControlText('CLIENTLIBELLE'));
  TobAdrInt.PutValue('ADR_LIBELLE2', GetControlText('CLIENTPRENOM'));
  TobAdrInt.PutValue('ADR_ADRESSE1', GetControlText('CLIENTADRESSE1'));
  TobAdrInt.PutValue('ADR_ADRESSE2', GetControlText('CLIENTADRESSE2'));
  TobAdrInt.PutValue('ADR_ADRESSE3', GetControlText('CLIENTADRESSE3'));
  TobAdrInt.PutValue('ADR_CODEPOSTAL', GetControlText('CLIENTCODEPOSTAL'));
  TobAdrInt.PutValue('ADR_VILLE', GetControlText('CLIENTVILLE'));
  TobAdrInt.PutValue('ADR_PAYS', '');
  TobAdrInt.PutValue('ADR_TELEPHONE', GetControlText('CLIENTTELEPHONE'));

  if Contact <> '' then
     Begin
     TobAppel.PutValue('AFF_NUMEROCONTACT', Contact);
     TobAdrInt.PutValue('ADR_CONTACT', GetControlText('LCONTACT'));
     end;
...
procedure TOF_BTTACHES.DefiniLibelle(TobAppel : Tob);
var Memo: TMemo;
    lng : integer;
    Indice : integer;
begin
