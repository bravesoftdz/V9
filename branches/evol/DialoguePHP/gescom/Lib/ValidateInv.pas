...
uses
  ...
  FactLotSerie, Grids, Hctrls, HImgList,uEntCommun ;
...
//    procedure lv_selectionCustomDrawItem(Sender: TCustomListView; Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);
  ...
  public
    procedure Init(TOBSelection : TOB; Prix1, Prix2, Prix3 : String; RemiseStockZero : Boolean);
    procedure ValidAll;
...
procedure ValideLesListes(Sender : TWinControl; TOBSelection : TOB; Prix1, Prix2, Prix3 : String; RemiseStockZero : Boolean);
...
with TValidateForm.Create(Sender) do
  begin
  Init(TOBSelection, Prix1, Prix2, Prix3, RemiseStockZero);
...
begin
  ...
  try
    ...
    if ConstruireDocInv=True then
      ...
      ExecuteSQL('UPDATE LISTEINVENT SET GIE_VALIDATION="X" WHERE GIE_CODELISTE="'+FBeingValidatedCodeListe+'"');
      ListeValide:=True;
...
begin
  ...
  begin
    ...
    if Not ConstruireLignePiece(CodeArtGen, CodeArtDim, TL, Ecart, IndiceArticle,False) then
    begin
      PGIInfo(TraduireMemoire(TexteMessage[3]),Caption);
      exit;
    end;
  ...
  begin
    ...
    CalculFacture(Tob_Piece, nil,nil,nil,Tob_PiedBase,Tob_LigneBase, Tob_Tiers, Tob_Article, nil,nil,nil, DEV) ;
...
var i,NbArt,x,CountStArt,NbRequete,y : integer;
    Ecart, QteInv : Double;
    StArticle : string;
    TabWhereArticle,TabWhereDispo : array of String;
    TL,TOBDispo,TOBDispoLot,TOBArt,TobDispoArt,TobDispoLotArt : TOB;
    QArticle,QDispo,QDispoLot : TQuery ;
begin
