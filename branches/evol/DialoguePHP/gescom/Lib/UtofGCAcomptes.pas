...
//    AuxiFacture : String;
  ...
  private
    LesColonnes: string;
    GS: THGRID;
    ColMontantdev, colRgt, ColDispo, ColJal, ColModR, ColLib: integer;
...
begin
  ...
  try
    ...
    {$ELSE}
    AGLLanceFiche('GC', 'GCACOMPTE', '', '', 'ACTION=CREATION;ISREGLEMENT=X');
    {$ENDIF}
    TobAcc := TheTob;
    TheTob := nil;
    if TobAcc <> nil then
...
begin
  ...
  try
    ...
    {$ELSE}
    AGLLanceFiche('GC', 'GCACOMPTE', '', '', 'ACTION=' + Action + ';LATOB=' + inttostr(TOBACC.detail[0].getIndex));
    {$ENDIF}
    TobAcc := TheTob;
    TheTob := nil;
    if TobAcc <> nil then
...
begin
  ...
  try
    ...
    {$ELSE}
    AGLLanceFiche('GC', 'GCACOMPTES', '', '', 'ACTION=CONSULTATION');
    {$ENDIF}
    TheTob := nil;
  finally
    TOBPASS.free;
...
//  AuxiFacture := TiersAuxiliaire(TobPiece.GetValue('GP_TIERSFACTURE'));
  ...
  begin
    ...
    begin
      ...
      SetControlText('GAC_LIBELLE', Copy('Ac./Règ. ' + DateToStr(TOBPiece.GetValue('GP_DATEPIECE')) + ' Pce. '+ IntToStr(TOBPiece.GetValue('GP_NUMERO')), 1, 35));
    end;
    {$IFDEF BTP}
  ...
  // Modif BTP
  GetCumulRG(TOBPIeceRG, TRG, TRD);
  THNumEdit(GetControl('TOTALTTCRG')).value := TRD;
  // --
end;
...
begin
  ...
  LastError := 0;
  Erreur := False;
  if getcontroltext('GAC_LIBELLE') = '' then
  begin
    LastError := 1;
...
//
    TOBA.PutLigneGrid(GS, i, false, false, LesColonnes);
  end;
  CalculeMontantTotal;
end;
...
begin
  ...
  begin
    ...
    begin
//      GS.ColFormats[i] := 'CB=TTMODEPAIE';
      ColModR := i;
    end
    else if Nam = 'GAC_JALECR' then
    begin
//      GS.ColFormats[i] := 'CB=TTJALBANQUEOD';
      COlJal := i;
    end
    else if Nam = 'GAC_LIBELLE' then
    begin
      GS.ColLengths[i] := -1;
      ColLib := i;
    end
    else if (Nam = 'GAC_MONTANTDEV') then
    begin
      GS.ColWidths[i] := 80;
      GS.ColFormats[i] := '#,##0.00';
      GS.ColAligns[i] := taRightJustify;
      ColMontantdev := i;
    end
    else if (Nam = 'MONTANTDISPO') then
    begin
      GS.ColWidths[i] := 80;
      GS.ColFormats[i] := '#,##0.00';
      GS.ColAligns[i] := taRightJustify;
    ...
    else if (Nam = 'GAC_ISREGLEMENT') then
    begin
      GS.ColTypes[i] := 'B';
      GS.ColWidths[i] := 15;
...
begin
	MontantUsed := 0;
  QQ := OpenSql ('SELECT SUM(BST_MONTANTACOMPTE) AS MTUSED FROM BSITUATIONS WHERE BST_SSAFFAIRE="'+TOBPiece.getValue('GP_AFFAIREDEVIS')+'"',true);
  ...
  begin
    if not TobAComptes.detail[Indice].FieldExists ('MONTANTINIT') then
    begin
      TobAComptes.detail[Indice].AddChampSupValeur ('MONTANTINIT', TobAComptes.detail[Indice].GetValue('GAC_MONTANTDEV')+MontantUsed);
      TobAComptes.detail[Indice].AddChampSupValeur ('MONTANTDISPO', TobAComptes.detail[Indice].GetValue('GAC_MONTANTDEV'));
    end;

    if (Bprorata <> nil) and (Bprorata.Checked) then
    begin
      MontantDispo := TobAComptes.detail[Indice].GetValue('MONTANTDISPO');
      MontantProrata := (TobAComptes.detail[Indice].GetValue('MONTANTINIT') * Ratio);
      if MontantProrata > MontantDispo then MontantProrata := MontantDispo;
      TobAComptes.detail[Indice].PutValue('GAC_MONTANTDEV',MontantProrata);
    end;
  end;
end;

procedure TOF_GCACOMPTES.ChargeCompta;
  ...
  begin
    ...
    begin
      ...
      TOBA := TOBAcomptes.FindFirst(['GAC_JALECR', 'GAC_NUMECR', 'GAC_AUXILIAIRE', 'GAC_MODEPAIE', 'GAC_QUALIFPIECE' ],
        ...
        True);
{$IFDEF BTP}
      if (TOBA<>Nil) and (TransformationPiece) and (TOBAcomptes.detail.count > 0) then
      begin
...
{$ENDIF}
  ...
  // MODIF LS
  if (not TransformationPiece) or (TOBAcomptes.detail.count = 0) then
  begin
    for Indice := 0 to TOBAcomptes.detail.count -1 do
    begin
      if TobAComptes.detail[Indice].GetValue('TRAITEVIACPTA')<>'X' then
      begin
        if not TobAComptes.detail[Indice].FieldExists ('MONTANTINIT') then
        begin
          TobAComptes.detail[Indice].addchampsupValeur('MONTANTINIT',0);
        end;
        if not TobAComptes.detail[Indice].FieldExists ('MONTANTDISPO') then
        begin
          TobAComptes.detail[Indice].addchampsupValeur('MONTANTDISPO',0);
        end;
        TobAComptes.detail[Indice].PutValue('MONTANTINIT', TobAComptes.detail[Indice].GetValue('GAC_MONTANTDEV'));
        TobAComptes.detail[Indice].PutValue('MONTANTDISPO', TobAComptes.detail[Indice].GetValue('GAC_MONTANTDEV'));
      end;
    end;
  end;
...
begin
  ...
  if VH_GC.GCIfDefCEGID then
  begin
    if TobAcomptes.Detail.count>0 then // JT 29/10/2003 - modif gestion des acomptes
  ...
  TobAcomptes.PutGridDetail(GS, True, True, LesColonnes, True);
  if colJal >= 0 then GS.cells[colJal, 0] := 'Journal';
  if colModR >= 0 then GS.cells[colModr, 0] := 'Mode de paiement';
  if colLib >= 0 then GS.cells[colLib, 0] := 'Libellé';
  if colRgt >= 0 then GS.cells[colRgt, 0] := 'Rgt.';
  if colDispo >= 0 then GS.cells[colDispo, 0] := 'Montant disponible';
  if ColMontantdev >= 0 then GS.cells[ColMontantdev, 0] := 'Montant affecté';
  (* plus besoin maintenant bicose dans calculeMontanttotal
  GS.cells[0,0]:='';
  For i:=GS.fixedrows to GS.RowCount-1 do
     begin
     if GS.isSelected(i) then Total:=Total+valeur(GS.cells[colMontantDev,i])
     else if (GS.cells[1,i]<>'') and (not GS.isSelected(i)) then begin GS.FlipSelection(i);  end ;
     end ;
  ...
  *)
  ...
  CalculeMontantTotal;
  (* ben maintenant si...
  oldMontant:=Valeur(GS.cells[colMontantDev,1]); //initialiser car pas de rowenter sur l'initialisation ???
  *)
  if Sortir then
  begin
    Acol := GS.col;
...
procedure TOF_GCACOMPTES.CalculeMontantTotal;
var i: integer;
begin
  Total := 0;
  for i := GS.fixedrows to GS.RowCount - 1 do
  begin
    if GS.isSelected(i) then Total := Total + valeur(GS.cells[colMontantDev, i])
    else if (GS.cells[1, i] <> '') and (not GS.isSelected(i)) then
    begin
      GS.FlipSelection(i);
    end;
  end;
...
begin
  ...
  begin
    if not (GS.valcombo = nil) then PostMessage(GS.ValCombo.Handle, WM_KEYDOWN, VK_TAB, 0);
  end;
  ...
  for i := GS.fixedrows to GS.RowCount - 1 do
  begin
    if not GS.isselected(i) then TOB(GS.Objects[0, i]).free
    else
    begin
      TOB(GS.Objects[0, i]).GetLigneGrid(GS, i, LesColonnes);
      MajMontants(TOBPiece, TOB(GS.Objects[0, i]));
    end;
  end;
  TobAcomptes.SetAllModifie(false);
end;
...
begin
  ...
  if TobAcomptes.detail.count > 0 then
  begin
    TobAcomptes.GetGridDetail(GS, GS.rowcount - 1, 'ACOMPTES', LesColonnes);
...
begin
  ...
  {$ELSE}
  AGLLanceFiche('GC', 'GCACOMPTE', '', '', 'ACTION=CREATION' + StRegle);
  {$ENDIF}
  TobAcc := TheTob;
  TheTob := nil;
  if TobAcc <> nil then
  begin
    TobAcc.addchampsup('MONTANTDISPO', false);
    TobAcc.putValue('MONTANTDISPO', TobAcc.GetValue('GAC_MONTANTDEV'));
    if not PassCompta then TobAcc.putValue('GAC_NUMECR', LeNumMaxAcompte(ToBAcomptes)+1);
    TobAcc.ChangeParent(TobAcomptes, -1);
  end;
end;
...
begin
  inherited;
  if (GS.Col = colMontantDev) and (Action <> taConsult) then
  begin
    GridSetEditing;
    exit;
  end;
  TheTob := LaTob;
  {$IFDEF BTP}
  if (ExisteAffectation(Tob(GS.Objects[0, GS.Row]))) or (Action = TaConsult) then
  begin
  ...
  AGLLanceFiche('BTP', 'BTACOMPTE', '', '', LACTION + ';LATOB=' + intTostr(Tob(GS.Objects[0, GS.Row]).GetIndex));
//  ChargeCompta;
  if PassCompta then
  begin
  	ChargeCompta;
  ...
  OnLoad;
  {$ELSE}
  AGLLanceFiche('GC', 'GCACOMPTE', '', '', 'ACTION=CONSULTATION;LATOB=' + intTostr(Tob(GS.Objects[0, GS.Row]).GetIndex));
  {$ENDIF}
...
begin
  if Action = taConsult then Exit;
  if not (GS.valCombo = nil) then GS.valCombo.visible := false;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TobPiece.GetValue('AVANCSAISIE') - TOBPiece.GetValue('AVANCPREC') < valeur(GS.cells[colMontantDev, ARow]) then
        begin
    ...
    if valeur(GS.cells[colDispo, ARow]) < valeur(GS.cells[colMontantDev, ARow]) then
    begin
      PGIBox('Vous depassez le montant disponible', TFVierge(ECran).caption);
      Cancel := True;
      ACol := ColMontantDev;
    ...
    begin
      Total := Total - OldMontant + valeur(GS.cells[colMontantDev, ARow]);
      SetControlText('TOTAL', FloatToStrF(Total, ffNumber, 15, 2));
      oldMontant := valeur(GS.cells[colMontantDev, ARow]);
      TOBA := Tob(GS.Objects[0, Arow]);
      if TOBA = nil then Exit;
      TobA.putValue('GAC_MONTANTDEV', valeur(GS.cells[colMontantDev, ARow]));
    end;
  end;
  GS.options := GS.Options - [goEditing];
  GS.editormode := false;
...
begin
  if GS.isselected(Arow) then
  begin
  	Total := Total - valeur(GS.cells[colMontantDev, ARow])
  end else
  begin
  	if PassCompta then
    begin
      Journal := GS.cells[ColJal, ARow];
...
begin
  oldMontant := Valeur(GS.cells[colMontantDev, ou]);
  GS.InvalidateRow(ou);
end;
...
begin
  ...
  if (Acol = ColJal) and (Arow >= GS.FixedRows) then
  begin
    ARect := GS.CellRect(ACol, ARow);
    TOBA := TobAcomptes.detail[Arow-1]; if TOBA = nil then exit;
  ...
  else if (Acol = ColModR) and (Arow >= GS.FixedRows) then
  begin
    ARect := GS.CellRect(ACol, ARow);
    TOBA := TobAcomptes.detail[Arow-1]; if TOBA = nil then exit;
...
begin
  if goEditing in GS.Options then Exit;
  with GS do
  begin
