...
function PassageUAUV (TOBRef,TOBcatalogue : TOB;MTPAF: double) : double;
{$ENDIF}
...
{$IFDEF GPAOLIGHT}
//if ExisteSQL('SELECT WAN_ARTICLE FROM WARTNAT WHERE WAN_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WARTNAT', 'WAN_NATURETRAVAIL', 'WAN_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='10' ;
  MessError:=TraduireMemoire('Utilisé dans la nature de travail ') + sInfo ;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WNT_ARTICLE FROM WNOMETET WHERE WNT_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WNOMETET', 'WNT_NATURETRAVAIL', 'WNT_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='11' ;
  MessError:=TraduireMemoire('Utilisé dans une entête de nomenclature de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WNL_COMPOSANT FROM WNOMELIG WHERE WNL_COMPOSANT="'+Article+'"')  then
sInfo := GetColonneSQL ('WNOMELIG', 'WNL_NATURETRAVAIL', 'WNL_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='12' ;
  MessError:=TraduireMemoire('Utilisé dans une ligne de nomenclature de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WGT_ARTICLE FROM WGAMMETET WHERE WGT_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WGAMMETET', 'WGT_NATURETRAVAIL', 'WGT_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='13' ;
  MessError:=TraduireMemoire('Utilisé dans une gamme de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WCO_ARTICLE FROM WCOLORIS WHERE WCO_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WCOLORIS', 'WCO_COLORIS', 'WCO_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='14' ;
  MessError:=TraduireMemoire('A un colori ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WEA_ARTICLE FROM WDEPOTAPP WHERE WEA_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WDEPOTAPP', 'WEA_DEPOTENC', 'WEA_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='15' ;
  MessError:=TraduireMemoire('Rattaché à un dépot d''encours ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WOA_ARTICLE FROM WORDREAUTO WHERE WOA_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WORDREAUTO', 'WOA_CODEAUTO', 'WOA_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='16' ;
  MessError:=TraduireMemoire('Utilisé dans les automatismes de production de code ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WOB_COMPOSANT FROM WORDREBES WHERE WOB_COMPOSANT="'+Article+'"')  then
sInfo := GetColonneSQL ('WORDREBES', 'WOB_NATURETRAVAIL', 'WOB_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='17' ;
  MessError:=TraduireMemoire('Utilisé dans les besoins de production de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WOL_ARTICLE FROM WORDRELIG WHERE WOL_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WORDRELIG', 'WOL_NATURETRAVAIL', 'WOL_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='18' ;
  MessError:=TraduireMemoire('Utilisé dans les lignes de production de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WOT_ARTICLE FROM WORDRETET WHERE WOT_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WORDRETET', 'WOT_NATURETRAVAIL', 'WOT_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='19' ;
  MessError:=TraduireMemoire('Utilisé dans les entêtes de production de nature ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WPL_ARTICLE FROM WPDRLIG WHERE WPL_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WPDRLIG', 'WPL_IDENTIFIANTWPE', 'WPL_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='20' ;
  MessError:=TraduireMemoire('Utilisé dans les lignes de prix de revient de l''entête de PdR ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WPE_ARTICLE FROM WPDRTET WHERE WPE_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WPDRTET', 'WPE_IDENTIFIANT', 'WPE_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='21' ;
  MessError:=TraduireMemoire('Utilisé dans l''entête de prix de revient ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WPN_ARTICLE FROM WPARCNOME WHERE WPN_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WPARCNOME', 'WPN_IDENTIFIANT', 'WPN_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='22' ;
  MessError:=TraduireMemoire('Utilisé dans la nomenclature de S.A.V. ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WVS_ARTICLE FROM WVERSION WHERE WVS_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WVERSION', 'WVS_IDENTIFIANT', 'WVS_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='23' ;
  MessError:=TraduireMemoire('Utilisé dans la version de S.A.V. ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
//if ExisteSQL('SELECT WPC_ARTICLE FROM WPARC WHERE WPC_ARTICLE="'+Article+'"')  then
sInfo := GetColonneSQL ('WPARC', 'WPC_IDENTIFIANT', 'WPC_ARTICLE="' + Article + '"' );
if sInfo <> '' then
begin
  NumError:='24' ;
  MessError:=TraduireMemoire('Utilisé dans le parc S.A.V. ') + sInfo;
  TabErreur[i]:= NumError+';'+MessError ;
  inc(i) ;
end ;
{$ENDIF}
...
function nbDecimales(NomChamp,Typ,DimDecQte,DimDecPrix : string) : string ;
begin
if (Typ='DOUBLE') or (Typ='RATE') then Result := DimDecPrix
else if (Typ='INTEGER') or (Typ='SMALLINT') then Result := DimDecQte
else Result := '';
...
begin
  ...
  begin
    ...
    Req := 'SELECT * FROM CATALOGU WHERE GCA_ARTICLE="'+TOBRef.GetValue('GA_ARTICLE')
        +'" AND GCA_TIERS="'+TOBRef.GetValue('GA_FOURNPRINC')+'"';
    QQ := OpenSql (Req,true,-1,'',true);
  ...
  begin
    ...
    //
    if (Pos(TOBtarif.getValue('GF_CASCADEREMISE'),'CAS;MIE')>0) or (TOBtarif.getValue('GF_CASCADEREMISE')='') then
    begin
...
begin
  ...
  if (TOBCatalogue.GetValue('GCA_QUALIFUNITEACH') = '') or
     (TOBREf.GetValue('GA_QUALIFUNITEVTE') = '') then exit;
  QuotiteAchat := RatioMesure('PIE',TOBCatalogue.GetValue('GCA_QUALIFUNITEACH'));
  if QuotiteAchat = 0 then QuotiteAchat := 1;
  QuotiteVte := RatioMesure('PIE',TOBRef.GetValue('GA_QUALIFUNITEVTE'));
  if QuotiteVte = 0 then QuotiteVte := 1;
  QuotiteSto := RatioMesure('PIE',TOBRef.GetValue('GA_QUALIFUNITESTO'));
  ...
  result := result / TobRef.GetValue('GA_PRIXPOURQTE');
(*
QQUALIFACH:=OpenSql('Select GME_QUOTITE From MEA where GME_MESURE="'+TOBCatalogue.GetValue('GCA_QUALIFUNITEACH')+'"',True,-1,'',true) ;
QQUALIFVTE:=OpenSql('Select GME_QUOTITE From MEA where GME_MESURE="'+TOBREf.GetValue('GA_QUALIFUNITEVTE')+'"',True,-1,'',true) ;
If (not QQUALIFACH.Eof) And (not QQUALIFVTE.Eof) then
Begin
  QuotiteAchat:=Valeur(QQUALIFACH.FindField('GME_QUOTITE').AsString) ;
  QuotiteVte:=Valeur(QQUALIFVTE.FindField('GME_QUOTITE').AsString) ;
  Result:=MTPAF * QuotiteVte/QuotiteAchat ;
  if TobRef.GetValue('GA_PRIXPOURQTE') = 0 then TobRef.PutValue('GA_PRIXPOURQTE',1);
End;
Ferme(QQUALIFACH) ;
Ferme(QQUALIFVTE) ;
*)
...
begin
  ...
  try
{$IFNDEF AGL570}
    if wLoadTobFromSql('PARPIECE', sql, TobGPP) then
{$ELSE}
...
{$IFDEF BTP}
          MetreArticle.CopieMetre(OldCode, NewCode);
          MetreArticle.CopieVariable(OldCode, NewCode);
{$ENDIF}
        END else
        begin
