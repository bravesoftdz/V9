...
{$ENDIF}
    ...
    TobEnt : TOB;
begin
//TV_NLIG.OnMouseDown := TV_NLIGMouseDown;
...
{$IFDEF LINE}
  TGNE_NOMENCLATURE.Caption := TraduireMemoire('Ouvrage');
...
{$ELSE}
Caption := TraduireMemoire('Saisie de la nomenclature ' + GNE_NOMENCLATURE.Text);
{$ENDIF}
...
begin
  {$IFDEF LINE}
  G_NLIG.ListeParam:='BTNOMENLIG_S1';
  {$ELSE}
...
begin
  ...
  if SGN_FOURN <> -1 then
  begin
  	if Action <> taConsult then G_NLIG.ColLengths[SGN_FOURN] := -1;
  end;
  ...
  if SGN_PVHT <> -1 then
  begin
  	if Action <> taConsult then G_NLIG.ColLengths[SGN_PVHT] := -1;
  end;
  if SGN_PREST <> -1 then
  begin
  	if Action <> taConsult then G_NLIG.ColLengths[SGN_PREST] := -1;
  end;
  if SGN_TPS <> -1 then
  begin
  	if Action <> taConsult then G_NLIG.ColLengths[SGN_TPS] := -1;
  end;
  if SGN_MO <> -1 then
  begin
...
{$ELSE}
  ...
  {$ELSE}
  G_NLIG.ColAligns[SGN_Joker] := taCenter;
  {$ENDIF}
{$ENDIF}
...
begin
  ...
  for indice := 1 to G_NLIG.RowCount - 1 do
     begin
     if G_NLIG.Cells[SGN_Comp, Indice] = '' then break;
     ChampSupNomen (TOB(G_NLIG.Objects[SGN_NumLig, Indice]));
     if TOB(G_NLIG.Objects[SGN_NumLig, Indice]).GetValue('GNL_SOUSNOMEN') = '' then
        begin
        renseigneValoArt (TOB(G_NLIG.Objects[SGN_NumLig, Indice]),Tob(G_NLIG.Objects[SGN_IdComp, indice]),nil);
        end;
     Tob(G_NLIG.objects [SGN_NUMLIG,Indice]).putvalue('UNITENOMENC',Tob(G_NLIG.Objects[SGN_IdComp, Indice]).getvalue('GA_QUALIFUNITEVTE'));
     G_NLIG.cells[SGA_UVTE,Indice] := Tob(G_NLIG.Objects[SGN_IdComp, Indice]).getvalue('GA_QUALIFUNITEVTE');
     if Tob(G_NLIG.Objects[SGN_IdComp, Indice]).getvalue('GA_TYPEARTICLE') = 'POU' then
        begin
        IPercent := Indice;
        continue;
     ...
     if TOB(G_NLIG.Objects[SGN_NumLig, Indice]).GetValue('GNL_SOUSNOMEN') <> '' then
        begin
        chargePrixOuvrage (TOB(G_NLIG.Objects[SGN_NumLig, Indice]),
                          TOB(G_NLIG.Objects[SGN_NumLig, Indice]).getvalue('GNL_SOUSNOMEN'),
                          Indice);
        if Tob(G_NLIG.Objects[SGN_IdComp, Indice]).getvalue('GA_TYPEARTICLE') = 'ARP' then
          begin
          RecupInfoPrixPose (TOB(G_NLIG.Objects[SGN_NumLig, Indice]));
          end;
        end;
     InitTableau (TValloc);
     Tob(G_NLIG.objects[SGN_Numlig,Indice]).putvalue('QTEDUDETAIL',1);
     CumuleElementTable (Tob(G_NLIG.objects[SGN_Numlig,Indice]),ExAucun, TValLoc);
...
{$IFDEF LINE}
N3.visible := false;
PopG_NLIG_D.visible := false;
TLIB_PREST.visible := false;
TGA_FOURNPRINC.visible := false;
HLabel7.visible :=false;
NEPVTTC.visible := false;
HLabel6.visible :=false;
NEPVFORFAIT.visible := false;
{$ENDIF}
end;
...
{$ELSE}
                    stWhere := 'GA_CODEARTICLE=' + Trim (Copy (ART.Text, 1, 18));
{$ENDIF}
                    DispatchRecherche (ART, 1, '',stWhere, '');
                    if ART.Text <> '' then
                        G_NLIG.Cells[G_NLIG.Col,G_NLIG.Row]:= Trim(copy(ART.Text, 0, Length(ART.Text) - 1));
                    END;
                END;
    VK_INSERT : BEGIN
                if FocusGrid then
...
//  Demande de validation
    ...
    case w_rep of
    mrYes    : BEGIN BValiderClick(Sender); if ModalResult = 0 then exit; END;
...
Repeat
    ...
    if NomCol<>'' then
        ...
        if ichamp>=0 then
            BEGIN
{$IFDEF BTP}
            if (Pos('GNL_',NomCol) > 0) and (Pos('X',V_PGI.Dechamps[iTableLigne,ichamp].Control)>0) then ColsInter[icol]:=True
            else if (Pos('GNL_',NomCol) = 0) then ColsInter[icol] := True; (* Pour les autres tables *)
{$ELSE}
            if Pos('X',V_PGI.Dechamps[iTableLigne,ichamp].Control)>0 then begin ColsInter[icol]:=True end;
{$ENDIF}
...
var
    i_ind1 : integer;
// {$IFDEF BTP} // DBR Fiche 10995
    tn1 : TTreeNode;
// {$ENDIF} // DBR Fiche 10995
begin
...
(* {$ELSE} // DBR Fiche 10995
for i_ind1 := 0 to G_NLIG.RowCount - 1 do
    begin
    if G_NLIG.Cells[SGN_Comp, i_ind1] = '' then break;
    G_NLIG.Objects[SGN_Comp, i_ind1] := ChercheNode(TV_NLIG, nil, G_NLIG.Cells[SGN_SSNom, i_ind1]);
    if G_NLIG.Objects[SGN_Comp, i_ind1] = nil then
        G_NLIG.Objects[SGN_Comp, i_ind1] := ChercheNode(TV_NLIG, nil, G_NLIG.Cells[SGN_Comp, i_ind1]);
    end;
{$ENDIF} *)
...
if (Arow >= 1) then
begin
  if (TOB(G_NLIG.Objects[SGN_NumLig, ARow])<>nil) and (TOB(G_NLIG.Objects[SGN_NumLig, ARow]).GetValue ('GNL_CODEARTICLE')='') then exit;
  if (TOB(G_NLIG.Objects[SGN_NumLig, ARow-1])<>nil) and (TOB(G_NLIG.Objects[SGN_NumLig, ARow-1]).GetValue ('GNL_CODEARTICLE')='') then exit;
end;
...
//--------------------------------------------------------------------------
G_NLIG.Objects[SGN_NumLig, ARow] := TOB.Create ('NOMENLIG', TOBLig, ARow-1) ;
//ChampSupNomen (TOB(G_NLIG.Objects[SGN_NumLig, ARow])); // DBR Fiche 10995
ChampSupNomen (TOB(G_NLIG.Objects[SGN_NumLig, ARow]), False); // DBR Fiche 10995

TOB(G_NLIG.Objects[SGN_NumLig, ARow]).PutValue('GNL_NOMENCLATURE',GNE_NOMENCLATURE.Text);
TOB(G_NLIG.Objects[SGN_NumLig, ARow]).PutValue('GNL_NUMLIGNE',StrToInt(G_NLIG.Cells[0, ARow]));
TOB(G_NLIG.Objects[SGN_NumLig, ARow]).PutValue('GNE_DOMAINE',Domaine);
//TOB(G_NLIG.Objects[SGN_NumLig, ARow]).AddChampSup('LIBELLE', False);
TOBTemp := TOB(G_NLIG.Objects[SGN_NumLig, ARow]);
AffecteLibelle(TOBTemp, 'GNL_CODEARTICLE');
//if Arow <= IPercent then Ipercent := IPercent +1;
Renumerote(ARow + 1);
Ztype := false;
Acol := SGN_Comp;
G_NLIG.col := Acol;
...
{$IFDEF BTP}
    if i_ind = SGN_Prix then
        G_NLIG.Cells [i_ind, ARow] := strs (0,V_PGI.OkDecP) else
{$ENDIF}
...
try
  Q := OpenSQL('Select * from NOMENLIG where GNL_NOMENCLATURE="' + GNE_NOMENCLATURE.Text + '"', true,-1, '', True);
  TOBTemp.LoadDetailDB('NOMENLIG', '', '', Q, False);
  Ferme(Q);
  ...
  for i_ind1 := 0 to TOBLig.Detail.Count - 1 do
      ...
      if (TOBLIG.Detail[i_ind1].getvalue('GNL_ARTICLE')) <> '' then
         begin
         TOBTemp.Dupliquer(TOBLig.Detail[i_ind1], False, True);
...
begin
    //  Suppression des records de l'ancienne version de la nomenclature GP
    TOBTemp := TOB.Create('', nil, -1);
    try
      Q := OpenSQL('Select * from NOMENENT where GNE_ARTICLE="' + GNE_ARTICLE.Text +
                   '" and GNE_NOMENCLATURE="' + GNE_NOMENCLATURE.Text + '"', true,-1, '', True);
      TOBTemp.SelectDB('', Q);
      Ferme(Q);
      LastIdent := TOBTemp.GetValue('GNE_IDENTIFIANTWNT');
    finally
      TobTemp.Free;
    end;
    ...
    if (LastIdent <> 0) then
    begin
        stSQL := 'Select * from WNOMETET where WNT_IDENTIFIANT="' + IntToStr(LastIdent) + '"';
        //  Suppression des records de l'ancienne version de la nomenclature GP
        TOBTemp := TOB.Create('WNT', nil, -1);
        try
          ...
          // 1° ON vire les lignes existantes
          wDeleteTable('WNOMELIG', 'WNL_ARTICLE="' + TobTemp.GetValue('WNT_ARTICLE') +
                       '" and WNL_MAJEUR="' + TobTemp.GetValue('WNT_MAJEUR') + '"');

          // 2° On recrée les lignes
          TobTemp.ClearDetail;
          for i_ind1 := 0 to TOBLig.Detail.Count - 1 do
          begin
            if (TOBLIG.Detail[i_ind1].GetValue('GNL_ARTICLE')) <> '' then
            begin
              TOBTempL := TOB.Create('WNL', TOBTemp, -1);

              { Champs }
              TOBTempL.AddChampSupValeur('WNL_COMPOSANT'    , TOBLIG.Detail[i_ind1].GetValue('GNL_ARTICLE'));
              TOBTempL.AddChampSupValeur('WNL_CODECOMPOSANT', Copy(TOBTempL.GetValue('WNL_COMPOSANT'), 1, 18));
              TOBTempL.AddChampSupValeur('WNL_QLIENSAIS'    , TOBLIG.Detail[i_ind1].GetValue('GNL_QTE'));
              TOBTempL.AddChampSupValeur('WNL_PHASE'        , 'AGC');
              TOBTempL.AddChampSupValeur('WNL_OPEITI'       , '110');
              TOBTempL.AddChampSupValeur('WNL_OPEITIAPPRO'  , '110');
              TOBTempL.AddChampSupValeur('WNL_MODECONSO'    , 'LAN');
              TOBTempL.AddChampSupValeur('WNL_QLOTSAIS', 1.0);
            end;
          end;
          ...
          try
            ...
            Q := OpenSQL('SELECT GA_TENUESTOCK FROM ARTICLE WHERE GA_ARTICLE="' +TobTemp.GetValue('WNT_ARTICLE') + '"', True,-1, '', True);
            TOBA.SelectDB('', Q);
            Ferme(Q);
...
//  derniere ligne
   ...
   ((G_NLIG.Cells[SGN_Comp,ARow] = '') and (G_NLIG.Cells[SGN_Comp,ARow + 1] = '')) then
    begin
//    G_NLIG.Rows[ARow].Clear;
    Exit;
    end;
...
if (ACol = SGN_Comp) or (ACol = SGN_Qte) then
    begin
{$IFDEF BTP}
