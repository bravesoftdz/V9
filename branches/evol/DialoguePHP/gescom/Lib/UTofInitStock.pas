...
{$IFDEF BTP}
        ColEmplacement,ColOk, ColQteMini : integer;
{$ENDIF}
        d_dpa : double;
...
begin
  ...
  begin
    ...
    TobIS.PutLigneGrid (GIS,GIS.row,false,false,
                        'FIXED;' + stColListe + ';QTEINIT;QTEMINI;EMPLACEMENT;SELECTOK');
    Key := 0;
  end;
end;
...
begin
  GIS.ElipsisButton := (GIS.col = ColEmplacement);
end;
...
TobIS.PutLigneGrid (GIS,Arow,false,false,'FIXED;' + stColListe + ';QTEINIT;QTEMINI;EMPLACEMENT;SELECTOK');
{$ENDIF}
end;
...
TobInitStk.PutGridDetail (GIS, False, False,
                          'FIXED;' + stColListe + ';QTEINIT;QTEMINI;EMPLACEMENT;SELECTOK', False);
{$ELSE}
TobInitStk.PutGridDetail (GIS, False, False,
...
{$IFDEF BTP}
GIS.ColCount := nbColListe + 5; // pour fixed, QteInit, QteMini,Emplacement et OK
st := stColListe + ';QTEINIT';
{$ELSE}
...
ColQteInit := GIS.ColCount - 4;
ColDPR := GIS.ColCount - 5;
ColPMAP := GIS.ColCount - 6;
ColDPA := GIS.ColCount - 7;
{$ELSE}
...
for iInd := 1 To GIS.ColCount - 1 do
    begin
{$IFDEF BTP}
    if (iInd <> ColQteInit) and (iInd <> ColDPA) and
