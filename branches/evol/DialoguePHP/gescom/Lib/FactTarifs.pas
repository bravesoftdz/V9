...
function TarifVersLigne(TobA, TobTiers, TobL, TobLigneTarif, TobPiece, TobTarif: Tob; ForceTous, Totale: boolean; DEV: RDevise): T_ActionTarifArt;
...
*)
        ...
        END else Px := TOBL.GetValue('GL_PUHTDEV');
      end else
      BEGIN
      {$ENDIF}
        Px := TOBTarif.GetValue('GF_PRIXUNITAIRE');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TOBL.PutValue('GL_PUHTDEV', PivotToDevise(Px, DEV.Taux, DEV.Quotite, DEV.Decimale));
      	  TOBL.PutValue ('GL_COEFMARG',0);
        end else
        begin
          TOBL.PutValue('GL_PUTTC', Px);
          // MODIF MODE LM
          Px := Px * V_PGI.TauxEuro;
          TOBL.PutValue('GL_PUTTCDEV', PivotToDevise(Px, DEV.Taux, DEV.Quotite, DEV.Decimale));
      	  TOBL.PutValue ('GL_COEFMARG',0);
        end;
        // MODIF MODE LM
  ...
  if ((Result = ataOk) and (Rem <> 0)) then
  begin
    if Totale then
    begin
