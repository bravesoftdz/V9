...
function GetQtePrixDateListe(Article, Depot : String; DateListe : TDateTime; TobL : TOB = nil ; TobLN : TOB = nil) : TQtePrixRec;
...
Q := OpenSQL('SELECT COUNT(GQ_ARTICLE) AS NBCLOT FROM DISPO WHERE GQ_ARTICLE="'+Article+
             '" AND GQ_DEPOT="'+Depot+'" AND GQ_CLOTURE="X"',true,-1, '', True);
if not Q.EOF then nbClot := Q.FindField('NBCLOT').AsInteger else nbClot := 0;
Ferme(Q);
if nbClot = 0 then
...
else Q := OpenSQL('SELECT GQ_DATECLOTURE, GQ_PHYSIQUE AS QTESTOCK, GQ_DPA, GQ_PMAP, GQ_DPR, GQ_PMRP FROM DISPO '+
...
if not Q.EOF then
  ...
  result.DateDerniereCloture := Q.FindField('GQ_DATECLOTURE').AsDateTime;
  //DateCloture := Q.FindField('GQ_DATECLOTURE').AsDateTime;
  result.Qte := Q.FindField('QTESTOCK').AsFloat;
  result.QtePlus := 0;
  result.QteMoins := 0;
...
const Fieldz = 'GL_QUALIFQTESTO, GL_QUALIFQTEACH, GL_QUALIFQTEVTE, GL_NATUREPIECEG, '+
               ...
               'GL_PIECEPRECEDENTE, GL_QTESTOCK, GL_QTERESTE, GL_QTERELIQUAT, GL_DPA, GL_DPR, '+
               'GL_PUHTNET, GL_PUTTCNET, GL_PRIXPOURQTE, GL_COEFCONVQTE ';
...
//                  'AND GL_NUMLIGNE=' + inttostr(CleDoc.NumLigne) + ' ' +
  ...
  begin
    QLigne := OpenSQL('SELECT GL_NATUREPIECEG, GL_SOUCHE, GL_NUMERO, GL_INDICEG, ' +
           'GL_NUMLIGNE,GL_NUMORDRE,GL_QTESTOCK,GL_QTERESTE,GL_PIECEPRECEDENTE FROM LIGNE WHERE ' + WhereLigne, true,-1, '', True);
  ...
  begin
    QLigne := OpenSQL('SELECT GLN_NATUREPIECEG, GLN_SOUCHE,GLN_NUMERO,GLN_INDICEG,GLN_NUMLIGNE,GLN_ORDRECOMPO,'+
                 'GL_QTESTOCK,GL_QTERESTE,GL_PIECEPRECEDENTE,GLN_COMPOSE, GLN_QTE '+
...
if TobL = nil then
   begin
   stWhere := RecupWhereNaturePiece('PHY','GL');
   if stWhere <> '' then stWhere := ' AND ' +stWhere;
   Q := OpenSQL('SELECT '+Fieldz+' FROM LIGNE  '+
...
if TOBLN = nil then
   begin
   stWhere := RecupWhereNaturePiece('PHY','GLN');
   if stWhere <> '' then stWhere := ' AND ' +stWhere;
