...
if Result then
  ...
  QQSuite:=OpenSql('Select * from ARTICLEQTE where GAF_ARTICLE="'+
                   TOBArt.GetValue('GA_ARTICLE')+'"',True,-1, '', True);
  if Not QQSuite.Eof then TOBADet:=TOB.CreateDB('ARTICLEQTE',TobFormule,-1,QQSuite) ;
  Ferme(QQSuite);
  if TOBADet<> Nil then
    begin
    QQSuite:=OpenSql('Select * from ARTFORMULEVAR where GAV_ARTICLE="'+
                   TOBArt.GetValue('GA_ARTICLE')+'" and GAV_VENTEACHAT="'+VteAch+'"',True,-1, '', True);
