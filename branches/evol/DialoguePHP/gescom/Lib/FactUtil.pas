...
uses {$IFDEF VER150} variants,{$ENDIF} HEnt1, HCtrls, UTOB, Ent1, LookUp, Controls, ComCtrls, StdCtrls, ExtCtrls,
  ...
  ParamSoc, HPanel  ,uEntCommun,UtilTOBPiece;
...
function GetCommentaireLigne(CodeCom: string = ''): TStrings;
...
Function PreAffecteLigne ( TobPiece, TobL, TobLigneTarif, TobA, TobTiers, TobTarif, TobConds, TobAffaire, TobCata, TobArtRef: Tob; ARow: integer; DEV: RDEVISE; NewQte: double; CalcTarif : boolean = True ): T_ActionTarifArt;
...
procedure GetSommeAcomptes(TOBAcomptes: TOB; var XP, XD: Double);
procedure LoadLesAcomptes(TOBPiece, TOBAcomptes: TOB; CleDoc: R_CleDoc; TOBAcptesIni: TOb = nil);
// MOdif BTP
procedure LoadLesRetenues(TOBPiece, TOBPieceRG, TOBBasesRG: TOB; CleDoc: R_cledoc;Action : TactionFiche= taCreat);
procedure LoadLesLibDetail(TOBPiece, TOBNomenclature, TobOuvrage, TOBTiers, TOBAffaire: TOB; DEV: Rdevise; saisieTypeAvanc: boolean);
...
procedure RecalculPiedPort(Action: TActionFiche; TOBPiece, TOBPiedPort: Tob; MontantbaseFixe : double=0);
function PivotToSaisie(TheDEV: RDevise; Base, Taux, Quotite: Double; Decimale: integer): double;
procedure CalculMontantsPiedPort(TOBPiece, TOBPL: TOB);
...
procedure ReactualisePr (TOBA : TOB);
begin
  if (TOBA.getvalue('GA_COEFFG') <> 0) and
  	 ((TOBA.getValue('GA_DPRAUTO')='X') OR (TOBA.GetValue('GA_DPR')=0)) and
     (TOBA.GetValue('GA_CALCPRIXPR')='PAA') then
  begin
    TOBA.PutValue('GA_DPR',Arrondi(TOBA.getvalue('GA_PAHT')*TOBA.getvalue('GA_COEFFG'),V_PGI.okdecP));
  end;
end;
...
function IsExisteFraisRepartit (Cledoc : R_Cledoc) : boolean;
var QQ : TQuery;
    TOBPort : TOB;
    Indice : integer;
begin
...
begin
  ...
  begin
    if (GetParamSoc('SO_GCDESACTIVECOMPTA')) or
      (GetInfoParPiece(Nature, 'GPP_TYPEPASSCPTA') = 'DIF') then
      Result := False;
  end;
end;
...
begin
  ...
  if TheTypeAffaire = 'I' then
     begin
     StChamps := 'AFF_STATUTAFFAIRE=INT';
     if not bChangeStatut then StArgument := 'STATUT:INT' else stArgument := 'STATUT:';
     end
  else if TheTypeAffaire = 'W' then
     begin
     StChamps := 'AFF_STATUTAFFAIRE=APP';
     if not bChangeStatut then StArgument := 'STATUT:APP' else stArgument := 'STATUT:';
     end
	else if TheTypeAffaire = 'P' then
     Begin
     StChamps := 'AFF_STATUTAFFAIRE=PRO';
     if not bChangeStatut then StArgument := 'STATUT:PRO' else stArgument := 'STATUT:';
     end
  else
     begin
     Stchamps := 'AFF_STATUTAFFAIRE=AFF';
     if (sContexte = 'ACH') or (bChangeStatut) then StArgument := 'STATUT:'
     																					 else StArgument := 'STATUT:AFF';
     TheTypeAffaire := 'A';
     end;
  ...
  if not (bChangeTiers) and (Tiers.Text <> '') then
    Stargument := Stargument + ';NOCHANGETIERS';
...
begin
  ...
  if LaTob.NomTable <> 'LIGNETARIF' then
    ...
    begin
      ...
      begin
        SQL := 'SELECT * FROM CATALOGU WHERE GCA_TIERS="' + CodeTiers + '" AND GCA_ARTICLE="' + CodeArt + '" ' +
          'AND GCA_DATESUP>="' + USDateTime(DatePiece) + '" AND GCA_DATEREFERENCE<="' + USDateTime(DatePiece) + '"';
    ...
    begin
      ...
      begin
        Q := OpenSql ('SELECT A.*,AC.*,N.BNP_TYPERESSOURCE,N.BNP_LIBELLE,'+
      						 		'(SELECT T_LIBELLE FROM TIERS WHERE T_NATUREAUXI="FOU" AND T_TIERS=GA_FOURNPRINC) AS LIBELLEFOU FROM ARTICLE A '+
                       'LEFT JOIN NATUREPREST N ON N.BNP_NATUREPRES=A.GA_NATUREPRES '+
                       'LEFT JOIN ARTICLECOMPL AC ON AC.GA2_ARTICLE=A.GA_ARTICLE WHERE A.GA_ARTICLE=(Select GAT_ARTICLE From ARTICLETIERS Where GAT_REFTIERS="' + CodeTiers + '" AND GAT_REFARTICLE="'+ RefSais + '") ' + WhereNat,true,-1, '', True);
        if not Q.EOF then
        begin
          TOBArt.SelectDB('', Q);
          InitChampsSupArticle (TOBART);
      ...
      begin
        SQL := 'Select A.*,AC.*,N.BNP_TYPERESSOURCE,N.BNP_LIBELLE,'+
               ...
               'AND GCA_DATESUP>="' + USDateTime(DatePiece) + '" AND GCA_DATEREFERENCE<="' + USDateTime(DatePiece) + '" ' + WhereNat;
...
begin
  Result := nil;
  if CodeCom = '' then CodeCom := AGLLanceFiche('GC', 'GCMULCOMMENTAIRE', '', '', '');
  if CodeCom = '' then exit;
  QQ := OpenSql('Select GCT_CONTENU From COMMENTAIRE WHERE GCT_CODE="' + CodeCom + '"', True,-1, '', True);
  if not QQ.Eof then
  begin
    Contenu := TStringList.Create;
    {$IFDEF EAGLCLIENT}
    Contenu.Text := QQ.FindField('GCT_CONTENU').AsString;
    {$ELSE}
    if not TMemoField(QQ.FindField('GCT_CONTENU')).IsNull then
      Contenu.Assign(TMemoField(QQ.FindField('GCT_CONTENU')));
    {$ENDIF}
    Result := Contenu;
  end;
  Ferme(QQ);
...
begin
  ...
  if TOBD <> nil then
  begin
    if Champ = 'GQ_PHYSIQUE' then Result := StrF00(TOBD.GetValue('GQ_PHYSIQUE'), V_PGI.OkDecQ) else
      if Champ = 'GQ_DISPONIBLE' then
      Result := StrF00(TOBD.GetValue('GQ_PHYSIQUE') - TOBD.GetValue('GQ_RESERVECLI') - TOBD.GetValue('GQ_PREPACLI'), V_PGI.OkDecQ) else
      if Champ = 'STOCNET' then
      Result := StrF00((TOBD.GetValue('GQ_PHYSIQUE') - TOBD.GetValue('GQ_RESERVECLI') - TOBD.GetValue('GQ_PREPACLI')) + TOBD.GetValue('GQ_RESERVEFOU'),
        V_PGI.OkDecQ) else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        TOBL.PutValue('GL_DPR', TOBD.GetValue('GQ_DPR'));
      end;
    end
    else
       TOBL.PutValue('GL_DPR', TOBD.GetValue('GQ_DPR'));
  ...
  begin
    ...
    begin
      ReactualisePr (TOBA);
      if TOBA.GetValue('GA_PAHT') <> 0 then CoefPapr := TOBA.GetValue('GA_DPR')/TOBA.GetValue('GA_PAHT')
                                       else CoefPapr := 1;
...
begin
  ...
  begin
    VenteAchat := GetInfoParPiece(TOBL.GetValue('GL_NATUREPIECEG'), 'GPP_VENTEACHAT');
    if (TOBA.FieldExists('GCA_LIBELLE'))
      and (TOBL.GetValue('GL_REFARTSAISIE') <> TOBL.GetValue('GL_CODEARTICLE')) then
    begin
...
begin
  ...
  begin
    ...
    TOBL.PutValue('GL_REMISABLELIGNE', TOBArtRef.GetValue('GA_REMISELIGNE'));
    {Familles, collection}
...
//  Result:=ataOK ;
  ...
  if CalcTarif and (not GetParamSoc('SO_PREFSYSTTARIF')) then
    Result := TarifVersLigne(TobA, TobTiers, TobL, TobligneTarif, TobPiece, TobTarif, False, True, DEV)
  else
    Result := ataOk;
end;
...
var TOTD, TOTP: Double;
begin
  GetSommeAcomptes(TOBAcomptes, TOTP, TOTD);
  TOBPiece.PutValue('GP_ACOMPTE', TOTP);
  TOBPiece.PutValue('GP_ACOMPTEDEV', TOTD);
end;
...
begin
  ...
  if (TOBAcomptes_O <> nil) and (TOBAcomptes_O.detail.count <> 0) then
  begin
    for i := 0 to TOBAcomptes.Detail.Count - 1 do
    begin
      TOBACC := TOBAcomptes.Detail[i];
...
//    
    ...
    if TOBRACC.detail.count > 0 then if not TOBRACC.InsertOrUpdateDB(false) then
      begin
        MessageValid := 'Erreur mise à jour acomptes (1)';
  ...
  if not TOBAcomptes.InsertDB(nil) then
  begin
    MessageValid := 'Erreur mise à jour acomptes (2)';
    V_PGI.IoError := oeUnknown;
  end;
end;
...
procedure LoadLesRetenues(TOBPiece, TOBPieceRG, TOBBasesRG: TOB; CleDoc: R_cledoc; Action : TactionFiche);
var Q: Tquery;
  i: integer;
  IndiceRg: integer;
  TOBL, TOBB: TOB;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          CautionU := CautionU - TOBB.GEtValue('PRG_MTTTCRG');
          if CautionU < 0 then CautionU := 0;
          CautionUDev := CautionUDev - TOBB.GEtValue('PRG_MTTTCRGDEV');
          if CautionUDev < 0 then CautionUDev := 0;
        end;
        TOBB.AddChampSupValeur ('CAUTIONUTIL',CautionU);
        TOBB.AddChampSupValeur ('CAUTIONUTILDEV',CautionUDev);
      end;
...
begin
  ...
  begin
  	LoadLesLibDetailOuvrages(TOBPiece, TOBOuvrage, TOBTiers, TOBAffaire, DEV);
  end;
  {$ENDIF}
...
begin
  ...
  if not TOBPieceRG.InsertDB(nil) then
  begin
    MessageValid := 'Erreur mise à jour des RG';
    V_PGI.IoError := oeUnknown;
  end;
end;
...
var
  ...
  { Main }
begin
  if DEV.Code <> TOBPiece.GetValue('GP_DEVISE') then
  begin
...
begin
  ...
  begin
    ...
    if EnHT then base := TobPort.GetValue('GPO_PVHT') else base := TobPort.GetValue('GPO_PVTTC');
    Base := Arrondi(PivotToSaisie(DEV, Base, DEV.Taux, DEV.Quotite, DEV.Decimale), DEV.decimale);
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          if EnHT then TobPied.detail[i].PutValue('GPT_BASEHTDEV', Base)
          else TobPied.detail[i].PutValue('GPT_BASETTCDEV', Base);
          CalculBasePiedPort(TobPied.Detail[i], EnHT);
        end else
        begin
          // frais repartis
      ...
      CalculMontantsPiedPort(TOBPiece, TOBPied.detail[i]);
