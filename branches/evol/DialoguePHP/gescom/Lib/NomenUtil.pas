...
{$IFDEF BTP}
    UtilTarif,
{$ENDIF}
    SaisUtil, comctrls, HEnt1, UtilArticle, HDimension,Factcalc,UtilPgi,ENtGC;
...
Type T_Valeurs = array [0..22] of double;
//
// Modif BTP
//
...
procedure renseigneValoArt (TOBIndice,TOBART,TOBMereLig : TOB);
...
//
  Select := 'SELECT NOMENLIG.*,N.BNP_TYPERESSOURCE,GNE_DOMAINE,GNE_QTEDUDETAIL AS QTEDUDETAIL,'
  					+	'GA_TYPEARTICLE,GA_FOURNPRINC,GA_PAHT,GA_PVHT,GA_NATUREPRES,'
...
TRY
   ...
   if (TOBRef.GetValue('GA_TYPEARTICLE') = 'MAR') or (TOBRef.GetValue('GA_TYPEARTICLE')='ARP') or (TOBREF.GetValue('GA_TYPEARTICLE') = 'PRE') then
      ...
      TOBLIG.putValue('GCA_QUALIFUNITEACH',TOBCatalog.GetValue('GCA_QUALIFUNITEACH'));
...
begin
  ...
  Select := 'Select NOMENLIG.*,GNE_DOMAINE,GA_PRIXPASMODIF,GA_TYPEARTICLE,GA_ARTICLE,GA_CODEARTICLE,GA_FOURNPRINC,GA_QUALIFUNITEVTE,'
            ...
            + 'GA_CALCAUTOHT,GA_COEFCALCHT,GA_COEFCALCTTC,GA_CALCPRIXHT,GA_CALCPRIXTTC,'
            + 'GA_DPA,GA_DPR,GA_PMAP,GA_PMRP,GA_PAHT,GA_PVHT,GA_PVTTC FROM NOMENLIG '
