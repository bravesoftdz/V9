...
var CodeListe, GaArticle, GereParLot, Depot : String;
    FUS,FUV,FPPQ : Double;
    FEMPL : String;
    FPHY,FDPA,FPMAP,FDPR,FPMRP : Double;
    LM, ListeMere : TOB;
    QP : TQtePrixRec;
begin
  result := false;
...
var RequeteSQL : String;  // Requête SQL générée par les critères
    ...
    SQLArtNonValid, StockVide, ListeDepot : String;
    TOBDepot, TOBD : TOB ;
    QA, QD : TQuery ;
    i,NbLig,NbListe : integer;
    H : THeader;
begin
...
if CheckGoodFields then
  ...
  for i:=0 to TobDepot.Detail.Count-1 do
    ...
    if (ctxMode in V_PGI.PGIContexte) then
      begin
      if FCriTOB.GetValue('CHKARTMOUV')='X' then
        begin
        //Désactivé en 4.2 pour être réactivé en 4.3 quand tous les cas seront gérés
        if FCriTOB.GetValue('CHKDIFFZERO')='X' then StockVide := ' AND GQ_PHYSIQUE<>0'
        else StockVide := '';
        RequeteSQL := 'SELECT GA_ARTICLE, GA_CODEARTICLE, GA_LOT, GA_FOURNPRINC, GQ_DEPOT, GQ_EMPLACEMENT, GQ_PHYSIQUE, GQ_DPA, GQ_PMAP, GQ_DPR, GQ_PMRP, GA_DPA, GA_PMAP, GA_DPR, GA_PMRP, GA_QUALIFUNITESTO, GA_QUALIFUNITEVTE, GA_PRIXPOURQTE '+
                      'FROM DISPO LEFT JOIN ARTICLE ON GA_ARTICLE = GQ_ARTICLE '+
                      'WHERE GA_TENUESTOCK="X" AND GA_STATUTART<>"GEN" AND '+
                      'GQ_CLOTURE="-" AND GQ_DEPOT="'+FDepot+'"'+StockVide
        end
      else
        RequeteSQL := 'SELECT GA_ARTICLE, GA_CODEARTICLE, GA_LOT, GA_FOURNPRINC, GQ_DEPOT, GQ_EMPLACEMENT, GQ_PHYSIQUE, GQ_DPA, GQ_PMAP, GQ_DPR, GQ_PMRP, GA_DPA, GA_PMAP, GA_DPR, GA_PMRP, GA_QUALIFUNITESTO, GA_QUALIFUNITEVTE, GA_PRIXPOURQTE '+
                      'FROM ARTICLE LEFT JOIN DISPO ON GQ_ARTICLE=GA_ARTICLE AND GQ_CLOTURE="-" AND GQ_DEPOT="'+FDepot+'" '+
                      'WHERE GA_TENUESTOCK="X" AND GA_STATUTART<>"GEN"';

      RequeteSQL := RequeteSQL + AvecLesAND([GoodCondition('GA_CODEARTICLE','GA_CODEARTICLE'),
                                     ...
                                     GoodCondition('GA_COLLECTION','GA_COLLECTION'),
                                     GoodCondition('GA_FOURNPRINC','GA_FOURNPRINC') ],false);
      end
      else
      begin
      RequeteSQL := 'SELECT GA_ARTICLE, GA_CODEARTICLE, GA_LOT, GQ_DEPOT, GAT_REFTIERS, GQ_EMPLACEMENT, GQ_PHYSIQUE, GQ_DPA, GQ_PMAP, GQ_DPR, GQ_PMRP, GA_DPA, GA_PMAP, GA_DPR, GA_PMRP, GA_QUALIFUNITESTO, GA_QUALIFUNITEVTE, GA_PRIXPOURQTE '+
                    'FROM DISPO LEFT JOIN ARTICLE ON GA_ARTICLE = GQ_ARTICLE '+
                    'LEFT JOIN ARTICLETIERS ON GA_CODEARTICLE = GAT_ARTICLE '+
                    'WHERE GA_TENUESTOCK="X" AND GA_STATUTART<>"GEN" AND GQ_CLOTURE="-" AND GQ_DEPOT="'+FDepot+'"'
    ...
    Ferme(FQ);
    TOBArtNonValid.Free ; TOBArtNonValid := Nil;
    FiniMove;
    FBigMother.InsertDB(nil, true);
    DoReport;
    FBigMother.Free; FBigMother := Nil;
