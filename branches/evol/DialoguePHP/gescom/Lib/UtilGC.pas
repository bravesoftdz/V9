...
{$IFDEF GCGC}
Function  GCMAJChampLibre (FF:TForm; Edition:Boolean; TypeChp:String; Name:String; Nbre:Integer; Suffixe:String='') : integer;
Function  GCXXWhereChampLibre (FF:TForm; xx_where:String; TypeChp:String; Name:String; Nbre:Integer; Suffixe:String='') : string;
Function  GCTitreZoneLibre (Name : String; Var TitreZone : String): Boolean;
...
Procedure MajChampsLibresArticle(FF: Tform; stPrefixe:string='GA';MasqueSiVide:boolean=true;TSTablesLibres:string='PTABLESLIBRES';TSZonesLibres : string='PZONESLIBRES' );
  //mcd 07/07/03
function AfPlusNatureAchat(BLC:boolean=FALSE) :string;
{$ENDIF}
...
if (TypeCAB='E13') or (TypeCAB='EA8') or (TypeCAB='39C') or (TypeCAB='ITC') or (TypeCAB='ITF')
   then LgMaxi:=LgMaxi-1;                  //Clé de contrôle sur le dernier caractère
if (LgMaxi < 4) and (LgMaxi>0) then LgMaxi:=VH^.Cpta[fbAux].Lg;
LgPrefixe := Length(Prefixe);
if (LgPrefixe >= LgMaxi) and (LgMaxi>0) then begin {LgPrefixe:=0;} Prefixe:=''; end;
...
While NbBoucle <= 3 do
  ...
  if ((LgPrefixe+LgChrono) > LgMaxi) and (NbBoucle < 3) and (LgMaxi>0) then // on est au bout du compteur
     ...
     if (NbBoucle = 1) and (LgPrefixe > 0) then   // Si préfixe, on le racourci
        begin
...
MetreArticle.SuppVariable(cle);
MetreArticle.Supprime(Cle);
MetreArticle.free;
{$ENDIF}
end;
...
if ctxScot in V_PGI.PGIContexte  then result:=''
 ...
 begin
 {$IFDEF CCS3}
   // en attente de la GA S3, pour S5 on prend tout, pour S3 que cde,bl,facture
 Result := ' AND ((GPP_NATUREPIECEG="CF") or (GPP_NATUREPIECEG="BLF") or (GPP_NATUREPIECEG="FF") or (GPP_NATUREPIECEG="AF")';
 If BLC then  result:= result +' or (GPP_NATUREPIECEG="BLC")' ;
 Result:= Result +' )';
  {$ENDIF}
 end;
end;
...
For i:=Depart to fin do
    ...
    GCTitreZoneLibre(Name+Num,Titre);
...
if (nom = 'TYTC') or (nom = 'YTC') then begin
        ind := Pos ('FOU',name);
        if (ind <> 0) then
            begin
            code := 'F';
...
Var iTable, iChamp : integer;
    ...
    procedure MasqueCube ( iTable,iChamp : integer );
    begin
     if (Length(V_PGI.DEChamps[iTable,iChamp].Libelle)>0) and (copy(V_PGI.DEChamps[iTable,iChamp].Libelle,1,2)='.-') and (pos( 'C',V_PGI.DEChamps[iTable,iChamp].Control)<>0) then
           V_PGI.DEChamps[iTable,iChamp].Control:= FindEtReplace(V_PGI.DEChamps[iTable,iChamp].Control,'C','',True);
    end;
    procedure GereLibellesLibres (stPrefixe,stTablette : string; isGRC : boolean) ;
    var iTable, iChamp : integer;
        NLib : string;
     begin
     iTable := PrefixeToNum (stPrefixe);
     ChargeDeChamps(iTable,stPrefixe) ;
     for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
       begin
       if isGRC then NLib:=GRCCodeTableLibre (V_PGI.DEChamps[iTable,iChamp].Nom) else NLib := CodeTableLibre_2 (V_PGI.DEChamps[iTable,iChamp].Nom);
       if NLib <> '' then
{$IFDEF CCS3}
          if ((stPrefixe='YTC')  and ((copy(NLib,3,1)='A') or (strToInt(copy(NLib,3,1))>3))) or ((stPrefixe='C')  and ((copy(NLib,3,1)='A') or (strToInt(copy(NLib,3,1))>3))) or (stPrefixe='RD1') then  V_PGI.DEChamps[iTable,iChamp].Libelle :='.-invisible'
             else
{$ENDIF}
          V_PGI.DEChamps[iTable,iChamp].Libelle :=rechdom (stTablette,NLib,False);
       MasqueCube(iTable,iChamp);
       end;
     end;
begin
...
if (ctxAffaire in V_PGI.PGIContexte) or (ctxGCAFF in V_PGI.PGIContexte) then begin
...
ChargeDeChamps(iTable,'GA') ;
for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
    begin
    if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_FAMILLENIV1' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF1',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_FAMILLENIV2' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF2',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_FAMILLENIV3' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF3',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_CODEDIM1' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI1',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_CODEDIM2' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI2',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_CODEDIM3' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI3',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_CODEDIM4' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI4',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_CODEDIM5' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI5',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_COLLECTION' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCZONELIBRE','AS0',False)
    else if (ctxMode in V_PGI.PGIContexte) and (V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_PVHT') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Prix Négoce (HT)'
    else if (ctxMode in V_PGI.PGIContexte) and (V_PGI.DEChamps[iTable,iChamp].Nom = 'GA_PVTTC') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Prix Détail (TTC)'
    else begin
         NLib := CodeTableLibre_2 (V_PGI.DEChamps[iTable,iChamp].Nom);
         if NLib <> '' then
{$IFDEF CCS3}
          if ((copy(NLib,3,1)='A') or (strToInt(copy(NLib,3,1))>3)) then  V_PGI.DEChamps[iTable,iChamp].Libelle :='.-invisible'
             else
{$ENDIF}
             V_PGI.DEChamps[iTable,iChamp].Libelle :=rechdom ('GCZONELIBRE',NLib,False);
         MasqueCube(iTable,iChamp);
         end;
    end;

// ARTICLECOMPL
iTable := PrefixeToNum ('GA2');
ChargeDeChamps(iTable,'GA2') ;
for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
    begin
    if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_FAMILLENIV4' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF4',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_FAMILLENIV5' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF5',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_FAMILLENIV6' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF6',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_FAMILLENIV7' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF7',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_FAMILLENIV8' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF8',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_STATART1' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCZONELIBRE','AS1',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GA2_STATART2' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCZONELIBRE','AS2',False) ;
    end;

// DIMMASQUE
iTable := PrefixeToNum ('GDM');
ChargeDeChamps(iTable,'GDM') ;
for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
    begin
    if V_PGI.DEChamps[iTable,iChamp].Nom = 'GDM_TYPE1' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI1',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GDM_TYPE2' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI2',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GDM_TYPE3' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI3',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GDM_TYPE4' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI4',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GDM_TYPE5' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCCATEGORIEDIM','DI5',False) ;
    end;
...
if not VH_GC.GCMultiDepots then
   begin
   // DISPO - dépôt
   iTable := PrefixeToNum ('GQ');
   for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
       begin
       if (V_PGI.DEChamps[iTable, iChamp].Nom = 'GQ_DEPOT') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Etablissement';
       end;
   // LISTEINVENT - dépôt
   iTable := PrefixeToNum ('GIE');
   for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
       begin
       if (V_PGI.DEChamps[iTable, iChamp].Nom = 'GIE_DEPOT') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Etablissement';
       end;
   // TRANSINVENT - dépôt
   iTable := PrefixeToNum ('GIT');
   for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
       begin
       if (V_PGI.DEChamps[iTable, iChamp].Nom = 'GIT_DEPOT') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Etablissement';
       end;
   end;
...
for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
    begin
    if V_PGI.DEChamps[iTable,iChamp].Nom = 'GL_FAMILLENIV1' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF1',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GL_FAMILLENIV2' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF2',False)
    else if V_PGI.DEChamps[iTable,iChamp].Nom = 'GL_FAMILLENIV3' then V_PGI.DEChamps[iTable,iChamp].Libelle := RechDom('GCLIBFAMILLE','LF3',False)
    else if (not VH_GC.GCMultiDepots) and (V_PGI.DEChamps[iTable, iChamp].Nom = 'GL_DEPOT') then V_PGI.DEChamps[iTable,iChamp].Libelle := 'Etablissement'
    else begin
         NLib := CodeTableLibre_2 (V_PGI.DEChamps[iTable,iChamp].Nom);
         if NLib <> '' then V_PGI.DEChamps[iTable,iChamp].Libelle :=rechdom ('GCZONELIBRE',NLib,False);
         end;
    end;
if VH_GC.GCIfDefCEGID then
begin
  iTable := PrefixeToNum ('T');
  ChargeDeChamps(iTable,'T') ;
  for iChamp := 1 to high(V_PGI.DeChamps[iTable]) do
      begin
         if (V_PGI.DEChamps[iTable, iChamp].Nom = 'T_RVA') then
             V_PGI.DEChamps[iTable,iChamp].Libelle := 'Site Web';
      end;
end;      
end;
...
For i:=1 to 10 do if i<10 then AuMoinsUn:=(ChangeLibre2('T'+stPrefixe+'_LIBREART'+intToStr(i),FF) or AuMoinsUn)
                          else AuMoinsUn:=(ChangeLibre2('T'+stPrefixe+'_LIBREARTA',FF)or AuMoinsUn);
