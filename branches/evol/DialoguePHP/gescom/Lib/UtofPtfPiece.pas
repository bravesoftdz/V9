...
{$ENDIF}
      UTOF, AglInit, Agent, Ent1, UtilGc ;
...
if stArgument='ACHAT' then
    ...
    st_plus:=st_Plus + ' AND ((GPP_NATUREPIECEG="DEF") or (GPP_NATUREPIECEG="CF") or (GPP_NATUREPIECEG="BLF") or (GPP_NATUREPIECEG="AF") or (GPP_NATUREPIECEG="AFS") or (GPP_NATUREPIECEG="FF"))';
    ...
    End
else if stArgument = 'DEVIS' then
    begin
    SetControlEnabled ('GP_NATUREPIECEG',false);
    SetControlVisible('AFF_GENERAUTO',False) ;
    end
else if stArgument = 'COMPTOIR' then
    begin
    SetControlVisible('AFF_GENERAUTO',False) ;
    SetControlVisible('TETATAFFAIRE',false);
...
{$IFDEF NOMADE}
if stArgument = 'ACHAT'
  then St_Plus := St_Plus + ' AND (GPP_NATUREPIECEG="CF" OR GPP_TRSFACHAT="X")'
  else St_Plus := St_Plus + ' AND (GPP_NATUREPIECEG IN ("CC","DE") OR GPP_TRSFVENTE="X")' ;
{$ENDIF}
...
// JT, eQualité 10782
if Ecran.FindChildControl('CODEARTICLE') <> nil then 
begin
