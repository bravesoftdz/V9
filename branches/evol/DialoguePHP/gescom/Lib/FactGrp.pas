...
{$ENDIF}
     ...
     UCotraitance ; // DBR : Depot unique chargé
...
Function RegroupeLesPieces ( TOBSource : TOB ; NewNat : String ; Eclate,DeGroupeRemise,AvecComment : boolean ; ChoixEclateDomaine : integer ;
                              ...
                              TraitFinTravaux:boolean=false;CodeAffCde:string='';WithVariantes: boolean=false) : integer ;
Function  TransfoBatchPiece ( OldNat,NewNat,Souche : String ; Numero,Indice : integer ; NewDate : TDateTime ) : integer ;
...
Var NewNature,VenteAchat : String ;
    ...
    // Modif BTP
    TOBOuvrage,TOBOuvragesP,TOBLIENOLE,TOBPIECERG,TOBBasesRg,TOBLiaison,TOBPieceTrait,TOBTimbres : TOB;
    indiceRg : Integer;
...
// VARIANTE
	 if (ModeGenere <> 'DEVTOCHAN') and (ModeGenere <> 'ETUTODEV') then SupprimeLesVariantes (TOBpiece);
// --
...
BEGIN
  ...
  {$IFDEF BTP}
  if (NewNature = 'FBT') or (NewNature='DAC') then
  begin
    MajLignesSituation (TOBGenere, DEV);
  end;
  {$ENDIF}
  ...
  BEGIN
    ...
    if CodeAff = '' then
    begin
      if ModeGeneration <> 'CBTTOLIVC' then
      begin
  ...
  *)
    calculPv :=  (NewNature <> 'FBT') and (NewNature<>'DAC');
...
if EclateParPiece then    // PCS 12/03/2003  CDISCOUNT
   ...
   NewNum:=GetNumSoucheG(NewSouchePiece) ;
...
if Not PassationComptable(TOBGenere,TOBOuvragesP,TOBBases,TOBBasesL,TOBEches,TOBPieceTrait,nil,TOBTiers,TOBArticles,TOBCpta,TOBAcomptes,TOBPorcs,TOBPIECERG,TOBBasesRG,TOBAnaP,TOBAnaS,DEV,OldEcr,OldStk,Not(bContinuOnError))
...
var
  Ind, iNoTypeDim, iNoCodeDim, iNoCodeArt, iNoQteReste, iNoQteReliq: integer;
  iNoQteStock, iNoQteFact: integer;
  TOBL, TOBGen: TOB;
  TypeDim, CodeDim: string;
  Qte, QteReste, QteReliq, QteStock, QteFact: double;
...
{$IFDEF BTP}
Procedure G_EnregistreLaPieceCumule(TOBPiece,TOBPieceTrait : TOB; GestionReliquat:boolean=false;FromLigneDetail : boolean=false);
...
TOBBasesL.ClearDetail;
CalculFacture(TOBGenere,nil,TOBOuvrage,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV) ;
{$IFDEF BTP}
...
if NeedVisaEnd then
   ...
   END ;
{Echéances}
TOBEches.ClearDetail ; GereEcheancesGC(TOBGenere,TOBTiers,TOBEches,TOBAcomptes,TOBPieceRG,taCreat,DEV,False) ;
G_InitToutModif ;
G_TraiteEuro ;
// Modif MODE
...
procedure GenereEcartArticle (TOBPiece : TOB; var TotalPa,TotalPr,TotalPV : double);
...
Procedure G_GenereLaPiece ( TOBPiece,TOBPieceTrait : TOB ;RendActive : boolean=false; GestionReliquat : boolean = false; FromLigneDetail : boolean=false; ModeGeneration : string='') ;
Var Domaine,Nature,Lib : String ;
    NumPiece : Integer;
    NeedVisaEnd : boolean ;
    MontantVisaEnd : Double ;
...
CalculFacture(TOBGenere,TOBPieceTrait,TOBOuvrage,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG,DEV) ;
{$IFDEF BTP}
  ...
  begin
    ...
    begin
      ...
      begin
        GenereEcartArticle (TOBgenere,TotalPa,TotalPr,TotalPV);
			  NumeroteLignesGC(Nil,TOBGenere, true);
        ZeroFacture (TOBgenere);
        for Ind := 0 to TOBgenere.detail.count -1 do ZeroLigneMontant (TOBgenere.detail[Ind]);
        PutValueDetail (TOBgenere,'GP_RECALCULER','X');
...
GereEcheancesGC(TOBGenere,TOBTiers,TOBEches,TOBAcomptes,TOBPieceRG,taCreat,DEV,False) ;
G_InitToutModif ;
G_TraiteEuro ;
...
{$IFDEF BTP}
  if V_PGI.IoError=oeOk then UG_reajusteStockPiecePrecedent(TobGenere,TobPiece,TobArticles);
{$ENDIF}
if V_PGI.IoError=oeOk then ValideLesLignesCompl(TOBGenere, nil);
if V_PGI.IoError=oeOk then ValideLesAdresses(TOBGenere,TOBPiece,TOBAdresses) ;
if V_PGI.IoError=oeOk then ValideLesArticles(TOBGenere,TOBArticles) ;
...
if V_PGI.IoError=oeOk then ValideLesOuv(TOBOuvrage,TOBgenere) ;
// NEW ONE (OPTIMISATION CALCUL)
if (V_PGI.IoError = oeOk) and (Pos(TOBgenere.GetValue('GP_NATUREPIECEG'),'FBT;ABT')>0) then ValideLesOuvPlat (TOBOuvragesP, TOBgenere);
if V_PGI.IoError = oeOk then ValideLesBases(TOBgenere,TobBases,TOBBasesL);
//DetruitLignesVirtuellesOuv (TOBGenere,DEV);
{$ENDIF}
if V_PGI.IoError=oeOk then TOBGenere.InsertDBByNivel(False) ;
...
if V_PGI.IoError=oeOk then ValideLesPieceTrait(TOBgenere, TOBPieceTrait,DEV);
...
if (V_PGI.IoError=oeOk) and
	...
	begin
  	MajSousAffaire (TOBGenere,nil,'00',TaCreat,false,false);
	end;
// MODIF BTP -- LS
if (V_PGI.IoError=oeOk) and (FromLigneDetail) then UG_ReajustePiecesPrecedente (TOBGenere,TOBPiece,TOBArticles);
...
if V_PGI.IoError=oeOk then
begin
  if Pos(Nature + ';', GetParamSoc('SO_WMISEENPROD')) <> 0 then
    wReceptionneWOLFromGL(TobGenere);
end;
{$ENDIF GPAOLIGHT}
{$IFDEF EDI}
  if V_PGI.IoError = oeOk then
    if StrToBool_(GetInfoParPiece(TOBGenere.GetValue('GP_NATUREPIECEG'), 'GPP_PIECEEDI')) then
      if IsEDITiers(TOBGenere) then
        EDICreateETR(TOBGenere, EDIGetFieldFromETS('ETS_CODEMESSAGE', EDIGetCleETS(TOBGenere.GetValue('GP_TIERS'), TOBGenere.GetValue('GP_NATUREPIECEG'))));
{$ENDIF EDI}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        end else if OldTobl.FieldExists ('QTERESTE') then
        begin
...
Function G_yaRuptureEntete ( TOBPiece : TOB ) : Boolean ;
Var i : integer ;
    Champ,Junk : String ;
BEGIN
Result:=True ;
...
Var i,k,LastI,iacc : integer ;
    TOBL,TOBPiece,TOBPieceTiers,NewL,TOBTP : TOB ;
...
InitRem := False;
LocalModeGestion := '';
if WithVariantes then LocalModegestion := 'ETUTODEV';
...
{$ENDIF}
    if DeGroupeRemise then
       ...
       END ;
    if G_yaRuptureEntete (TOBPiece) then
       BEGIN
       NewPiece:=True ;
       TOBRefPiece.Dupliquer(TOBPiece,False,True) ;
       END ;
...
{$ENDIF}
             ...
             // MODIF_DBR_FIN
              if i>0 then G_GenereLaPiece(TOBPieceTiers.Detail[i-1],TOBPieceTrait,false,false,false,LocalModegestion )
                     else G_GenereLaPiece(TOBPieceTiers.Detail[0],TOBPieceTrait,false,false,false,LocalModegestion ) ;
              LastI:=i ;
              END ;
...
{$ENDIF}
        ...
        begin
          ...
          Ecart := Arrondi(NewL.GetValue('GL_QTERESTE') - Qte, 6);
...
if TOBPieceTiers.Detail.Count>0 then G_GenereLaPiece(TOBPieceTiers.Detail[TOBPieceTiers.Detail.Count-1],TOBPieceTrait,false,false,false,LocalModegestion);
TOBPieceTiers.Free;
END ;
...
// --
  ...
  {$IFDEF BTP}
      if (GenereMode = 'CHANTOCBT') or (GenereMode = 'CBTTOLIVC') or (Generemode = 'DEVTOCHAN') then
      begin
...
//           Prix:=0;
     ...
     G_GenereLaPiece(TOBPiece,nil,RendActive,GestionReliquat,True,GenereMode);
     // Modif BTP
     ...
     begin
        ...
        if (self.GenereMode = 'CHANTOCBT') or
           ...
           (self.GenereMode = 'CBTTOLIVC') then
        begin
{$IFDEF BTP}
...
for j:=Low(LesNumPieceTiers)to High(LesNumPieceTiers) do
   ...
   if LesNumPieceTiers[j].Souche <> '' then
      BEGIN
      LesNumPieceTiers[j].NumDebP := SetNumberAttribution (LesNumPieceTiers[j].Souche,LesNumPieceTiers[j].NbPiece);
...
EclateParPiece := Eclate; bContremarque:=False;
...
// PA 02/10/2001
    ...
    if io=oeOk then io:=Transactions(GenVal.G_GenereParTiers,1);
    GenVal.Free ;
    Case io of
      oeOk : G_MajEvent ;
      oeUnknown  : BEGIN
...
FromShellOLE:=True ;
Result:=RegroupeLesPieces(TOBSource,NewNat,True,True,False,0,NewDate,False,False) ;
FromShellOLE:=False ;
...
begin
    ...
    TOBL.PutValue('GL_DPA',TOBL.GetValue('GL_PUHTNET'));
...
var TOBSource:TOB;
    ...
    TOBL:TOB;
    QteReliquat : double;
    StRendu,TrierPar,RuptureCol,ValPrec:String;
...
{$ENDIF}
      ...
      if (GenereMode = 'FACDETAILCONSO') or (GenereMode = 'FACCONSO') then
         ...
         if Getparamsoc('SO_FACTPROV') then
            ...
            NaturePiece:='FAC';
         TrierPar:='GL_AFFAIRE; GL_TIERS';
         RuptureCol:= 'GL_AFFAIRE;';
         if (GenereMode = 'FACCONSO') then
          	begin
            TrierPar:='GL_TIERS;GL_CHANTIER';
...
{$ENDIF}
  ...
  begin
     ...
     TOBLigne.Detail.Sort('GL_AFFAIRE');
  end;
...
{$IFDEF BTP}
    ...
    begin
      ...
      if (TOBL.getValue('GL_TYPEARTICLE')='FRA')  Then continue;
		...
		if (GenereMode = 'CBTTOLIVC') then
    begin
      QteLivrable := GetQtelivrable (TOBL,GestionPhase,nil,SansStock);
//      if Qtelivrable = 0 then continue; // si rien a livrer sur cette ligne --> ligne suivante
    end else QteLivrable := TOBL.GetValue('GL_QTERESTE');
{$ENDIF}
    if ((GenereMode = 'REAP')or (GenereMode = 'REAPECLAT') or
    	  (GenereMode = 'VALIDDECIS') or (GenereMode = 'VALIDDECISECLAT') ) and
        (TOBL.GetValue('GL_TIERS')='') then continue;
    //
...
{$IFDEF BTP}
      ...
      if (GenereMode = 'CHANTOCBT') or (GenereMode = 'CBTTOLIVC') then
      begin
...
{$ENDIF}
      if (GenereMode = 'DEVTOCHAN') or (GenereMode = 'RFOTOLIVC') or
      	 (GenereMode = 'CBTTOLIVC') or (GenereMode = 'FACCONSO') or
...
{$IFDEF BTP}
    ...
    // là on a forcement les lignes de réceptions a livrer sur chantiers sans Besoin préalable
      ...
      Qte:=NewTOBL.GetValue('GL_QTEFACT') * RatioStk;
      //
    ...
    if (UpperCase(GenereMode) = 'CBTTOLIVC')  then
    begin
    	QteReliquat := Abs(TOBL.getValue('GL_QTESTOCK'))-Abs(QTeLivrable);
      if QTereliquat <= 0 then NewTOBL.PutValue('GL_QTERELIQUAT',0)
      										else NewTOBL.PutValue('GL_QTERELIQUAT',TOBL.getValue('GL_QTESTOCK')-QTeLivrable);
//    if NewTOBL.GetValue('GL_QTERELIQUAT') < 0 then NewTOBL.PutValue('GL_QTERELIQUAT',0);
...
{$ENDIF}
  // --
  io:=Transactions(GenPiece.GenereLesPieces,1) ;
  Case io of
     oeOk       : BEGIN
                  G_MajEvent;
                  StRendu:='Le traitement s''est correctement effectué. #13#10'+IntToStr(NbP)+' pièce(s) générée(s) de nature '
                  ...
                  if TOBResult <> nil then
                  begin
                    TOBresult.Dupliquer (GenPiece.TOBResult,true,true);
                  end;
                  // --
                  result:=true;
                  END ;
...
begin
  ...
  begin
    ...
    ferme (QQ);
    //
    newNum := SetNumberAttribution (NewCleDoc.Souche,1);
...
if (NewCodeTiers <> '') then
   ...
   if Not(ModeRegleTiers) then
      TobGenere.PutValue('GP_MODEREGLE', TobPiece.GetValue('GP_MODEREGLE'));
   end;
...
begin
  ...
  TRY
    ...
    AddLesSupEntete (TOBGenere);
    UG_VideLesTobs (TOBBases,TOBEches,TOBNomenclature,TOBAcomptes,TOBPorcs,TOBSerie,TOBOuvrage,
                       TOBLienOle,TOBPieceRG,TOBBasesRg,TobVariable,TobRevision,TobLigneTarif  );
...
//         Prix:=0;
  ...
  ZeroFacture(TOBGenere) ;
  //
  G_EnregistreLaPieceCumule(TOBgenere,nil,GestionReliquat,true);
  //
  if TOBResult <> nil then
  begin
