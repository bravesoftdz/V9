...
{$IFNDEF CCS3}
  TarifCond,
{$ENDIF}
  UtilArticle, AglInitGC, M3FP, EntGC ;
...
Function SaisieTarifCliArtKnown (NatTiers, CodeT ,Article: string; Action : TActionFiche; TarifTTC : Boolean=False) : boolean ;
...
type
  TFTarifCliArt = class(TForm)
    ...
    TGA_PRIXPOURQTE: THLabel; //JT eQualité 10807
...
if (tav = tavFam) or (tav = tavTout) then
    ...
    QQ := OpenSQL('SELECT TARIF.*, CC_LIBELLE FROM TARIF LEFT JOIN CHOIXCOD ON CC_TYPE="BFT" AND CC_CODE=GF_SOUSFAMTARART WHERE '+
                  WhereTarifCli (CodeTiers, '', CodeDevise, ttdCliFam, False)+
                  ' ORDER BY GF_TARIFARTICLE, GF_SOUSFAMTARART, GF_ARTICLE, GF_BORNEINF',True,-1,'',true) ;
...
DEV.Code := CodeDevise ;
GetInfosDevise (DEV) ;
if CodeTiers <> '' then PrepareEntete else InitialiseEntete ;
if TarifTTC then
begin
...
if G_Fam.Col = SF_SFam then
    ...
    FAM.Plus := ' AND CC_LIBRE="'+G_FAM.cells[SF_FAM,G_Fam.row]+'"';
    GetFamilleRecherche (FAM) ;
    if FAM.Text <> '' then G_Fam.Cells[G_Fam.Col,G_Fam.Row]:= FAM.Text;
    FAM.Destroy;
    END ;
...
BEGIN
  ...
  if St <> '' then
  BEGIN
  	if ExisteSousFamille (G_Fam.Cells [SF_Fam, ARow],St) then
    BEGIN
      TOBL.PutValue ('GF_SOUSFAMTARART', St);
      G_Fam.Cells [SF_LibSFam, ARow] := RechDom('BTSOUSFAMTARART',St,False);
    END else
    BEGIN
