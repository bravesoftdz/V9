...
{$ENDIF}
         ...
         begin
           ...
           begin
           	if Transactions(SetVISA,3) <> oeOK then PGIBox('Impossible de restaurer le visa de la pièce n° '+Q.FindField('GP_NUMERO').AsString, Caption);
           end;
         end else
         begin
           ...
           if Transactions(SetVISA,3) <> oeOK then PGIBox('Impossible de viser la pièce n° '+Q.FindField('GP_NUMERO').AsString, Caption);
         end;
         MoveCur(False);
         END;
...
inherited;
  ...
  //mcd 08/07/03
if (ctxAffaire in V_PGI.PGIContexte) and Not(ctxGCAFF in V_PGI.PGIContexte) and (GetControlText('GP_VENTEACHAT')='ACH') then StPlus:= StPLus+  AfPlusNatureAchat;

if StPlus <>'' then SetControlProperty('GP_NATUREPIECEG','Plus',StPlus);
...
Begin
	...
	if Champ='RESTORE' then
     ...
     Begin
     SetControlText('GP_VENTEACHAT',Valeur);
     ...
     Else if valeur = 'AFF' then
          ...
          {$IFDEF LINE}
          SetControlProperty('GP_NATUREPIECEG', 'Plus', ' AND GPP_NATUREPIECEG IN ("FBT","ABT","DBT","DAC")');
          ...
          {$ELSE}
          SetControlProperty('GP_NATUREPIECEG', 'Plus', ' AND GPP_NATUREPIECEG IN ("FBT","FBC","ABT","ABC","DBT","ETU","BCE","DAC")');
