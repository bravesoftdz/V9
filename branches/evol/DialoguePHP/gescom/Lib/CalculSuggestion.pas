...
uses SysUtils, Classes, UTOB, Hctrls, HStatus, FactUtil, StdCtrls, math,
     EntGC, Ent1, SaisUtil, FactComm, FactCalc, UTofListeInv,AglInit,UtilPGI, FactTOB, FactPiece,
{$IFNDEF EAGLCLIENT}
...
procedure CalculEvolutionStock(TOBArt, TobMvts : TOB; var TobEvol : TOB;LigneStd:boolean=true;TOBLTraite : TOB = nil;FromCalculDocuments : boolean=false);
...
CleDoc.NaturePiece:='REA' ; CleDoc.DatePiece:=V_PGI.DateEntree ;
...
if DepartTraitement = 'A' then Result := LanceTraitementFromStock (TOBPiece)
                          else Result := LanceTraitementFromDocs (TOBPiece);
// --
end;
...
if (not AvecHistoriqueConso) and (TOBArt.GetValue('GA_TENUESTOCK')='X') then
    ...
    if TobEvol.Detail.Count > 0 then
...
TobMvts.Free; TobMvts := nil;
TobEvol.Free; TobEvol := nil;
end;

procedure CalculHistoStock(TOBArt : TOB; var TobMvts : TOB; LigneStd : boolean);
var
    TobLig, TobTempL, TobTempM : TOB;
    i_ind1 : integer;
    Q : TQuery;
...
begin
  ...
  begin
    ...
    if TousDepots then Depot := '' else Depot := TobTempL.GetValue('GQ_DEPOT');
...
//  Calcul des coef pondérateurs
  ...
  begin
    ...
    st2 := IntToStr(StrToInt(st1) - 1) +
           Copy(TobTempL.GetValue('PERIODE'), Pos('/', TobTempL.GetValue('PERIODE')), 255);
...
*)
procedure CalculEvolutionStock(TOBArt, TobMvts : TOB; var TobEvol : TOB;LigneStd:boolean;TOBLTraite : TOB;FromCalculDocuments : boolean);
var
    TobTempE, TobTempL, TobLig : TOB;
    Q : TQuery;
    FUS, FUA, FUV, CoefUaUs : double;
    i_ind1, i_ind2 : integer;
    Select, st1, st2, Annee, Mois, Jour : string;
    NewPeriode : boolean;
    d_temp1 : TDateTime;
...
Select:='Select GL_NATUREPIECEG,GL_NUMERO,GL_SOUCHE,GL_INDICEG,GL_NUMLIGNE,GL_DATEPIECE,GL_ARTICLE,' +
        (* Ajout Ls *)
        'GL_PIECEPRECEDENTE,GL_PIECEORIGINE,GL_COEFCONVQTE,' +
        (* --- *)
        'GL_DEPOT,GL_QTESTOCK,GL_QTEFACT,GL_QTERESTE,GL_DATELIVRAISON,GL_QUALIFQTESTO,GL_QUALIFQTEVTE,GL_QUALIFQTEACH,GP_VIVANTE from LIGNE ' +
        'left join PARPIECE on GPP_NATUREPIECEG=GL_NATUREPIECEG ' +
        'left join PIECE on (GP_NATUREPIECEG=GL_NATUREPIECEG) and (GP_NUMERO=GL_NUMERO) ' +
        'where GL_ARTICLE="' + TOBArt.GetValue('GA_ARTICLE') + '" ' +
        'and GP_VIVANTE="X" ' +
...
Select:='Select GL_NATUREPIECEG,GL_NUMERO,GL_SOUCHE,GL_INDICEG,GL_NUMLIGNE,GL_DATEPIECE,GL_ARTICLE,' +
        ...
        'GL_PIECEPRECEDENTE,GL_PIECEORIGINE,GL_COEFCONVQTE,' +
        (* --- *)
        'GL_DEPOT,GL_QTESTOCK,GL_QTEFACT,GL_QTERESTE,GL_DATELIVRAISON,GL_QUALIFQTESTO,GL_QUALIFQTEVTE,GL_QUALIFQTEACH,(GL_QTERESTE * GLN_QTE) as GL_QTERESTE,GL_DATELIVRAISON,GP_VIVANTE from LIGNENOMEN ' +
...
begin
  Select:='Select GQ_STOCKMIN, GQ_STOCKMAX,GQ_DEPOT from DISPO where GQ_ARTICLE="' +
          TOBArt.GetValue('GA_ARTICLE') + '" and GQ_CLOTURE="-"';
  Q := OpenSQL(Select, True,-1,'',true);

  if not Q.EOF then TobDepot.LoadDetailDB('DISPO', '', '', Q, False);
//    else
//    TobDepot.Free; ?? pourquoi ca il est ferme plus tard ??
	Ferme(Q);
end;
...
for i_ind1 := 0 to TobEvol.Detail.Count - 1 do
    ...
    if (i_ind1 = 0) and TousDepots then stkphy := TobTemp.GetValue('Q_PHYSIQUE');
//    if i_ind1 = 0 then stRuptDepot := TobTemp.GetValue('DEPOT');
    if (not TousDepots) and (TobTemp.GetValue('DEPOT') <> stRuptDepot) and (stRuptDepot <> '') then
        begin
        ...
        DetermineQteAffecte (TOBL.GetValue('LIENLIGNE'),StkPhy);
        //
        ...
        begin
          if stkmax <> 999999999999 then
            stkdisp := stkmax
        end
        else                          stkdisp := -stkdisp;
        if stkdisp > 0 then
            begin
            MemoTemp.Text := sDate + ';';
            MemoTemp.Add(sPiece + ';');
...
//  Creation de la ligne de document associée au calcul
            ...
            InitLigne(TOBPiece, TobArt, TobSuggest);
						TobSuggest.PutValue('GL_PUHTBASE',TOBL.getValue('GL_DPA'));
						TobSuggest.PutValue('GL_QTEPREVAVANC',TOBL.getValue('GL_QTEFACT'));
            {$IFDEF BTP}
            if FromCalculDocuments then
              TOBSuggest.addChampSupValeur ('LIENLIGNE',TOBL.GetValue('LIENLIGNE'),false);
            {$ENDIF}
//            if (not LigneStd) and (TOBL <> nil) then
            begin
              TOBSuggest.PutValue('GL_TIERS',TOBL.GetValue('GL_FOURNISSEUR')) ;
              TOBSuggest.PutValue('GL_FOURNISSEUR','') ;
              TobSuggest.PutValue('GL_PUHT', TobL.GetValue('GL_DPA'));
              TobSuggest.PutValue('GL_PUHTDEV', TobL.GetValue('GL_DPA'));
            end;
...
TOBDepot.Free;
if stkdisp <= 0 then begin MemoTemp.Free; Exit; end;
...
for i_ind1 := 0 to TobMvts.Detail.Count - 1 do
    begin
    if ((i_ind1 Div 12) Mod 2) = 0 then Continue;
    TobTemp := TobMvts.Detail[i_ind1];
    if TobTemp.GetValue('DEPOT') <> stRuptDepot then Continue;
    sDate  := sDate  + sSep + TobTemp.GetValue('PERIODE');
    sCli   := sCli   + sSep + StrF00(TobTemp.GetValue('QSOR'), GetParamSoc('SO_DECQTE'));
    sSep := ';';
    end;
MemoTemp.Add(sDate + ';');
MemoTemp.Add(sCli + ';');
...
begin
  ...
  if coefUaUS <> 0 then
  begin
  	TobSuggest.PutValue('GL_QTEPREVAVANC',TOBL.getValue('GL_QTEFACT')/CoefUAUS);
  	TobSuggest.PutValue('GL_PUHTBASE',MtPa*coefuaUs);
  end else
  begin
  	TobSuggest.PutValue('GL_QTEPREVAVANC',TOBL.getValue('GL_QTEFACT')/FUA*FUS);
  	TobSuggest.PutValue('GL_PUHTBASE',MtPa/FUS*FUA);
  end;
...
begin
  ...
  TRY
    GetInfosDevise(DEV);
    CalculFacture (TOBPiece,nil,nil,nil,TobBases,TOBBasesL,nil,nil,nil,nil,nil,DEV);
  FINALLY
...
//  Result := false;
  ...
  TRY
    ...
    CalculStockPhysique(TOBArt, TobEvol);
    CalculEvolutionStock(TOBArt, TobMvts, TobEvol,LigneStandard,TOBL,True);
    if (TobEvol.Detail.Count > 0) or (TobMvts.Detail.Count > 0) then
    begin
...
begin
  ...
  requete := 'SELECT SUM(GL_QTERESTE) AS QTERECEP,GL_QUALIFQTEACH,GL_COEFCONVQTE FROM LIGNE WHERE GL_PIECEORIGINE="'+RefOrigine+
  ...
  if ModifOk then
  begin
    TOBF := TOBDest.FindFirst (['GL_ARTICLE'],[TOBL.GetValue('GL_ARTICLE')],true);
  end;
