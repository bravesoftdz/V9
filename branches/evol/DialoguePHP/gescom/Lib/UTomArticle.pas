...
uses {$IFDEF VER150} variants,{$ENDIF} Classes, Utom, UTob, UTof, HDimension, UDimArticle, sysutils, LookUp, HEnt1, HCtrls,
  ...
  {$IFDEF BTP}
  UtilTarif,TarifCliart,olectnrs, UMetreArticle,  UtilXlsBTP,FileCtrl,
  {$ENDIF}
...
type
  ...
  private
    ...
    Prefixe: string;
    TOBCatalog, TOBTarif, TOBEDetail, TOBDetail, TOBPrestation, TOBTiers: TOB;
    TPrestation: THEdit;
    DQtePrest: THNumEdit;
    // Gestion des Métrés
    TABMetre: TTabSheet;
    ...
    //
    ModifChampHT, ModifChampTTC, FaireCalcul, CalculEnCours: Boolean;

    ArtToDuplic: string;
    DuplicFromFiche, DuplicFromMenu: boolean;
    DuplicArt: string;
    ...
    IsLine : boolean;
    bCalculPrixDuplicHT, bCalculPrixDuplicTTC : boolean; // Pour ne pas mettre de message 37 à l'ouverture de la fiche en duplication
    ...
    function PassageUAUV(TOBcatalogue: TOB; MTPAF: double): double;
...
const
  ...
  TexteMessage: array[1..59] of string = (
...
begin
  ...
  begin
    ...
    if BMonoFournisseur = True
      then SetField('GCA_TIERS', FournPrinc)
    else SetField('GCA_REFERENCE', trim(copy(ArticlePrin, 1, 18))); // Référence article chez le fournisseur par défaut
  ...
  if BMonoFournisseur = True
    then SetFocusControl('GCA_REFERENCE')
  else SetFocusControl('GCA_TIERS');
end;
...
begin
  ...
  begin
    ...
    begin
      if not LookupValueExist(GetControl('_ARTICLE_GEN')) then
      begin
...
begin
  ...
  {$ENDIF}
  if BMonoFournisseur = True
    then SetControlEnabled('GCA_TIERS', False)
  else SetControlEnabled('GCA_TIERS', (DS.State in [dsInsert]));
  SetControlEnabled('GCA_REFERENCE', (DS.State in [dsInsert]));
  OldArticle := GetControlText('GCA_ARTICLE');
  connexion := False;
  if Ecran.Name <> 'GCCATALOGU_SAISI3' then exit;
  {$IFDEF BTP}
  if (DS.state = dsInsert) and (ArticleParam <> '') then
  begin
    ArticleFourn := true;
...
var CodeUnique, CodeGenerique, CodeDim: string;
begin
  if CodeArticleUnique2(GetControlText('_ARTICLE_GEN'), '') <> OldArticle then
    if not (DS.State in [dsInsert, dsEdit]) then DS.edit; // pour passer DS.state en mode dsEdit

  if GetControlText('_ARTICLE_GEN') = Trim(Copy(GetField('GCA_ARTICLE'), 1, 18)) then exit;
  if GetControlText('_ARTICLE_GEN') = '' then
  begin
  ...
  CodeDim := '';
  CodeGenerique := GetControlText('_ARTICLE_GEN');
  CodeUnique := CodeArticleUnique2(CodeGenerique, '');
  if not BMonoFournisseur then
  begin
    // Sélection éventuelle d'un article dimensionné
    if ExisteSQL('Select GA_ARTICLE from ARTICLE where GA_CODEARTICLE="' + CodeGenerique + '" AND GA_STATUTART="GEN"')
      then CodeDim := SelectUneDimension(CodeUnique);
  end;
  if CodeDim <> '' then SetField('GCA_ARTICLE', CodeDim) else SetField('GCA_ARTICLE', CodeUnique);
  THLabel(GetControl('LIBARTICLE')).Caption := RechDom('GCARTICLEGENERIQUE', CodeGenerique, False);
  if THLabel(GetControl('LIBARTICLE')).Caption = 'Error' then THLabel(GetControl('LIBARTICLE')).Caption := '';
end;
...
begin
  ...
  if TToolbarButton97(ecran.FindComponent('BAPPELVARIABLE')) <> nil then
     ...
     SetControlProperty('BAppelVariable', 'Visible', True);
  ...
  if TFFiche(Ecran).fTypeAction <> taConsult then
  	 begin
     if not JaiLeDroitConcept(TConcept(gcArtCreat), False) then
    		begin
  ...
  else if copy(TypeArticle,1,2) = 'PA' then  // articles parcs
  	 begin
...
{$IFNDEF LINE}
          if Pos(typeArticle,'MAR;ARP') <> 0 then
          begin
            THlabel(GetControl('TGA_ARTICLEPARC')).visible := true;
...
{$ENDIF}
  ...
  Else if (copy(ecran.name, 1, 12) = 'BTPRESTATION') then
     Begin
     if (TypeArticle = 'PRE') then OpenOleContainer;
     if (TypeArticle = 'FRA') then SetControlProperty('BAppelVariable', 'Visible', False);
     end
  Else if (copy(ecran.name, 1, 13) = 'BTARTPOURCENT') then
     Begin
     if (TypeArticle = 'POU') then OpenOleContainer;
     end;

  {JLD Confidentialité concepts}
  if ((TFFiche(Ecran).fTypeAction = taModif) and (not JaiLeDroitConcept(TConcept(gcArtModif), False))) then
     begin
...
Begin
  ...
  if (copy(ecran.name, 1, 9) = 'BTARTICLE') then
    ...
    // Gestion des métrés
    TabMetre := TTabSheet(ecran.FindComponent('P_METRES'));
    TabMetre.TabVisible := false;
  ...
  //gestion des métrés
  if (TypeArticle = 'MAR') or (TypeArticle = 'ARP') then OpenOleContainer;
...
begin
  ...
  //
  {$IFDEF BTP}
  RecalcAutoPR := boolean(GetField('GA_DPRAUTO') = 'X');
  {$ELSE}
  RecalcAutoPR := (CtxMode in V_PGI.PGIContexte) and boolean(GetField('GA_DPRAUTO') = 'X');
  {$ENDIF}
  RecalcAutoHT := boolean(GetField('GA_CALCAUTOHT') = 'X');
  RecalcAutoTTC := boolean(GetField('GA_CALCAUTOTTC') = 'X');
  {$IFDEF GPAOLIGHT}
    SetBtArtNatState;
  {$ENDIF}
...
{$ENDIF}
  ...
  begin
    if (GetField('GA_CALCAUTOHT') = 'X') then SetControlProperty('GA_PVHT', 'Color', clbtnFace)
  ...
  begin
    ...
    begin
      ...
      if GetControl('FOURNPRINC') <> nil then SetControlText('FOURNPRINC', RechDom('GCTIERS', GetField('GA_FOURNPRINC'), False));
      {$IFDEF BTP}
      if (copy(ecran.name, 1, 9) = 'BTARTICLE')  then
      begin
      	if not DuplicFromMenu then GetInfosPrixTarif(GetField('GA_FOURNPRINC'), GetField('GA_ARTICLE'),
    ...
    // DCA - Gestion Commerciale PGI 10790
    else if GetControl('FOURNPRINC') <> nil then SetControlText('FOURNPRINC', '');
    ArticlePrin := GetField('GA_ARTICLE');
    // DCA - Gestion Commerciale PGI 10790
    if BMonoFournisseur and (GetControl('GA_FOURNPRINC') <> nil) then FournPrinc := GetControlText('GA_FOURNPRINC');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //  BBI, fin fiche correction 10410
        if GetField('GA_CALCAUTOHT')='X' then  SetControlProperty('GA_PVTTC', 'Color', clbtnFace)
        																 else SetControlProperty('GA_PVTTC', 'Color', clWindow);
      end;
    end;
  end;
  ...
  if ((Getfield('GA_CALCPRIXPR') = 'AUC') and
    (GetField('GA_CALCPRIXTTC') = 'AUC') and (GetField('GA_CALCPRIXHT') = 'AUC')) or
    ((copy(GetField('GA_CALCPRIXHT'), 1, 1) = 'S') and (copy(GetField('GA_CALCPRIXTTC'), 1, 1) = 'S') and
    (copy(GetField('GA_CALCPRIXPR'), 1, 1) = 'S')) then SetControlEnabled('BACTUPRIX', False)
  ...
  if ((GetField('GA_CALCPRIXTTC') = 'AUC') and (GetField('GA_CALCPRIXHT') = 'AUC')) or ((copy(GetField('GA_CALCPRIXHT'), 1, 1) = 'S') and
    (copy(GetField('GA_CALCPRIXTTC'), 1, 1) = 'S')) then
    SetControlEnabled('BACTUPRIX', False)
  else if (GetField('GA_TYPENOMENC') <> 'MAC') then SetControlEnabled('BACTUPRIX', (not Consultation));
  {$ENDIF}
  // ----
  ...
  if (NomChamps = 'GA_COEFCALCHT') then
  begin
    if (DS.State in [dsEdit, dsInsert]) and RecalcAutoHT then CalculBaseTarifHT(False);
    if DS.State in [DsInsert] then TestFaireCalcul
    else if DS.State in [DsEdit] then
...
begin
  ...
  begin
    ModifChampPR := not boolean(GetField('GA_DPRAUTO') = 'X');
    FaireCalcul := True;
  end;
  ...
  if (ctxMode in V_PGI.PGIContexte) then
  begin
    if (CalcPAPR = 'PAA') and (GetField('GA_PAHT') <> 0) then
    begin
      ModifChampPR := not StringToCheck(GetField('GA_DPRAUTO'));
      FaireCalcul := True;
    end
    else if (CalcPAPR = 'PRA') and (GetField('GA_PRHT') <> 0) then
    begin
      ModifChampPR := not StringToCheck(GetField('GA_DPRAUTO'));
      FaireCalcul := True;
    end;
  ...
  begin
    ModifChampHT := not StringToCheck(GetField('GA_CALCAUTOHT'));
    FaireCalcul := True;
  end;
  ...
  begin
    ModifChampTTC := not StringToCheck(GetField('GA_CALCAUTOTTC'));
    FaireCalcul := True;
  end;
...
begin
  ...
  begin
    ...
    if (GetField('GA_TYPEARTICLE') = 'ARP') then
      Begin
        ...
        begin
          if not MetreArticle.validate(getfield('GA_Article')) then
             begin
               if (GetActiveTabSheet('PAGES').Name = 'P_METRES') then
                  MetreArticle.ModifMetres;
             end;
        end;
      end
    else if(GetField('GA_TYPEARTICLE') = 'MAR') then
      Begin
        if MetreArticle <> nil then
        begin
          if not MetreArticle.validate(getfield('GA_Article')) then
    ...
    else if(GetField('GA_TYPEARTICLE') = 'PRE') then
      Begin
        if MetreArticle <> nil then
        begin
          if not MetreArticle.validate(getfield('GA_Article')) then
    ...
    else if(GetField('GA_TYPEARTICLE') = 'POU') then
      Begin
        if MetreArticle <> nil then
        begin
          if not MetreArticle.validate(getfield('GA_Article')) then
  ...
  begin
    ...
    else if (ModifChampHT and (GetField('GA_CALCPRIXHT') <> 'AUC') and (getField('GA_CALCAUTOHT')='X')) then
    begin
      SetLastError(33, 'GA_CALCPRIXHT');
      exit
    end
    else if (ModifChampTTC and (GetField('GA_CALCPRIXTTC') <> 'AUC') and (getField('GA_CALCAUTOTTC')='X')) then
    begin
  ...
  //
  {$IFDEF BTP}
  if ((GetField('GA_CODEARTICLE') = '') or (copy(GetField('GA_CODEARTICLE'), length(prefixe) + 1, 255) = '')) and (GetParamsoc('SO_GCNUMARTAUTO') = TRUE) and
    (GetParamsoc('SO_GCTYPSUFART') = TRUE) then
  begin
    CodeArt := AttribNewCodeB('ARTICLE', 'GA_CODEARTICLE', GetParamsoc('SO_GCLGNUMART'), prefixe, Getfield('GA_LIBELLE'));
    SetField('GA_CODEARTICLE', CodeArt);
    if (GetField('GA_STATUTART') = 'GEN') then DimensionsArticle.CodeArticle := CodeArt;
  end;
  if (TFFiche(Ecran).fTypeAction = taCreat) and (GetField('GA_CODEARTICLE') <> '') and (Length(Prefixe) > 0) and (copy(GetField('GA_CODEARTICLE'), 1,
    Length(prefixe)) <> prefixe) then
  begin
    lngCodeArt := GetParamsoc('SO_GCLGNUMART');
    SetField('GA_CODEARTICLE', copy(Prefixe + GetField('GA_CODEARTICLE'), 1, lngCodeArt));
  end;
  {$ELSE}
  //************************ Gestion des chronos Article automatiques *****************
  if (GetField('GA_CODEARTICLE') = '') and (GetParamsoc('SO_GCNUMARTAUTO') = TRUE) and
    (TypeArticle = 'MAR') then
  begin
    // Récupération du chrono article : gestion multi-utilisateurs
    iRecupChrono := 0;
    repeat
      inc(iRecupChrono);
      stChronoArt := GetParamSoc ( 'SO_GCCOMPTEURART', True ) ;
      CodeArt := AttribNewCode('ARTICLE', 'GA_CODEARTICLE', GetParamsoc('SO_GCLGNUMART'), trim(GetParamsoc('SO_GCPREFIXEART')), stChronoArt, '',
        GetParamsoc('SO_GCCABARTICLE'));
  ...
  if (GetField('GA_ARTICLE') = '') then // A revoir avec les dimensions
    SetField('GA_ARTICLE', CodeArticleUnique2(GetField('GA_CODEARTICLE'), ''));
...
*)
  ...
  begin
    ...
    if MetreArticle <> Nil then
    begin
      MetreArticle.CopieMetre(DuplicArt, GetField('GA_ARTICLE'));
      MetreArticle.CopieVariable(DuplicArt, GetField('GA_ARTICLE'));
    end;
    DuplicArt := ''
  end;
  {$ENDIF}
...
var I, x: Integer;
  OngletDimension: TTAbSheet;
  PanelDimension: TPanel;
  IsMacro, IsNomenc, IsAucOuStock,IsParc, IsNomenParc,IsEnabled: boolean;
  stTemp, ArtSpecif: string;
  {$IFDEF BTP}
  // Modif BTP
  NbDeclinaison : integer;
  OngletCaracteristique, OngletTarif: TTAbSheet;
  PanelCaracteristique, PanelTarif: TPanel;
  LibTypNomenc: string;
  {$ENDIF}
  Requete,CodeSSTarif: string;
  QQ: TQuery;
begin
  inherited;
  ...
  if (copy(ecran.name, 1, 12) = 'BTPRESTATION') then
    ...
    if (TypeArticle = 'PRE') then
       begin
         if MetreArticle <> nil then
         begin
           metreArticle.TypeArticle := TypeArticle;
           metrearticle.Article := Getfield('GA_ARTICLE');
         end;
       end;
    {$ENDIF}
...
//MODIF BRL 10/07/06      SetCOntrolVisible ('GB_COMPTA',false);
  ...
  // Modif BTP
 {$IFDEF BTP}
  if (copy(ecran.name, 1, 13) = 'BTARTPOURCENT') then
  begin
    if oleExcel <> nil then
       begin
         CloseOleContainer;
...
{$IFDEF BTP}
  ...
  begin
    ...
    // contrôle du type d'article pour intégration de l'onglet métrés
    ...
    else if TypeArticle = 'OUV' then
        begin
        // Controle si l'ouvrage est décliné et si oui on n'affiche pas le métré
          NbDeclinaison := CompteDeclinaison(getfield('GA_ARTICLE'));
          if NbDeclinaison = 1 then
             begin
               OpenOleContainer;
             end
          else
...
{$ENDIF}
  ...
  begin
    ...
    SetControlEnabled('GA_CALCAUTOTTC', True);
...
//      SetControlProperty('TGA_PMAP', 'Caption', 'PMAP ' + RechDom('GCDEPOT', VH_GC.GCDepotDefaut, False));
  ...
  begin
		CodeSousTarif.Plus := 'AND CC_LIBRE="'+GetField('GA_TARIFARTICLE')+'"';
  end;
  {$ENDIF}
...
begin
  ...
  begin
    //item := FindItemToFocused(DimensionsArticle.Dim.GridDim, DimensionsArticle.MasqueDim, TypeMasque);
    //if item <> nil then itemDimSetFocus(DimensionsArticle, item, '');
    FocusedFirstItem(THDimension(GetControl('FDIM')));
  end;

// Gestion des métrés => Click sur Onglet
{$IFDEF BTP}
   ...
   if (GetActiveTabSheet('PAGES').Name = 'P_METRES') then
    ...
    begin
      SetControlVisible('BFirst', True);
      SetControlVisible('BPrev', True);
      setControlVisible('BNext', True);
      SetControlVisible('BLast', True);
      if Ok_MetreActif = true then
      begin
        if MetreArticle <> nil then
        begin
          //if TypeArticle <> 'PRE' then
          //begin
          MetreArticle.SauveMetres(False);
          //end;
          MetreArticle.DesactiveTimer;
        end;
        Ok_MetreActif := false;
      end;
    end;
...
{$IFNDEF GPAO}
  ...
  begin
    if (GetParamSoc('SO_GCPROFILART') <> '') and (GetParamSoc('SO_GCPROFILART') <> '...') then
      ...
      if GetParamsoc('SO_GCPROFILARTAUTO') then
        AppliqueProfilArticle(False, True);
  end;
{$ELSE}
...
// Gestion des métrés => Abandon saisie
{$IFDEF BTP}
 if (copy(UpperCase(ecran.name), 1, 9) = 'BTARTICLE') THEN
     if TypeArticle = 'MAR' then
        begin
          CloseOleContainer;
        end
  ...
  if (copy(UpperCase(ecran.name), 1, 13) = 'BTARTPOURCENT') THEN
     if TypeArticle = 'POU' then
        begin
          CloseOleContainer;
        end;

  if (copy(UpperCase(ecran.name), 1, 12) = 'BTPRESTATION') THEN
     if TypeArticle = 'PRE' then
        begin
          CloseOleContainer;
        end;
...
begin
  ...
  if (copy(ecran.name, 1, 9) = 'BTARTICLE') then
     ...
     if (TypeArticle = 'OUV') Then
        begin
        NbDeclinaison := CompteDeclinaison(getfield('GA_ARTICLE'));
        if NbDeclinaison = 1 then CloseOleContainer;
        end;
     end;
...
{$ENDIF}
  ...
  if GetParamsoc('SO_GCNUMARTAUTO') then SetFocusControl('GA_LIBELLE')
  else SetFocusControl('GA_CODEARTICLE');
...
var TOBFourn, TOBFournFille: TOB;
  QFourn: TQuery;
  SQL: string;
...
begin
  ...
  begin
    ...
    SQL := 'Select GCA_TIERS, GCA_REFERENCE, T1.T_LIBELLE from CATALOGU '
      + ' left outer join TIERS T1 on GCA_TIERS=T_TIERS '
      + ' where (GCA_ARTICLE="' + CodArtic + '")';
    if TypeRech = '2' then SQL := SQL + ' and (GCA_TIERS="' + CodFourn + '")'; // Sélection restrictive sur le fournisseur de l'article
    NbCatalogue := 0;
    QFourn := OpenSql(SQL, True);
...
begin
  ...
  begin
    ...
    if CtxMode in V_PGI.PGIContexte then
    begin
      if CalcPAPR = 'AR' then SetField('GA_PRHT', FloatToStr(BaseTarifPR))
      else if CalcPAPR = 'RA' then SetField('GA_PAHT', FloatToStr(BaseTarifPR))
    end
    else if GetField('GA_DPRAUTO')='X' then SetField('GA_DPR', FloatToStr(BaseTarifPR));
    if (GetField('GA_DPRAUTO') = 'X') then SetControlProperty('GA_DPR', 'Color', clbtnFace)
																		  else SetControlProperty('GA_DPR', 'Color', clWindow);
    ClickActuPrix := Click;
  end;
...
function TOM_ARTICLE.PassageUAUV(TOBcatalogue: TOB; MTPAF: double): double;
var (*QQUALIFACH, QQUALIFVTE: TQuery; *)
//  QuotiteAchat, QuotiteVte: double;
  PrixPourQte: double;
    QuotiteAchat,QuotiteVte,QuotiteSto,CoefUaUs : double;
begin
(*
  result := MTPAF;
  PrixPourQte := GetField('GA_PRIXPOURQTE');
  QQUALIFACH := OpenSql('Select GME_QUOTITE From MEA where GME_MESURE="' + TOBCatalogue.GetValue('GCA_QUALIFUNITEACH') + '"', True);
  QQUALIFVTE := OpenSql('Select GME_QUOTITE From MEA where GME_MESURE="' + GetField('GA_QUALIFUNITEVTE') + '"', True);
  if not QQUALIFACH.Eof then QuotiteAchat := Valeur(QQUALIFACH.FindField('GME_QUOTITE').AsString) else QuotiteAchat := 1;
  if not QQUALIFVTE.Eof then QuotiteVte := Valeur(QQUALIFVTE.FindField('GME_QUOTITE').AsString) else QuotiteVte := 1;
...
*)
  ...
  if (TOBCatalogue.GetValue('GCA_QUALIFUNITEACH') = '') or
     (GetField('GA_QUALIFUNITEVTE') = '') then exit;
  PrixPourQte := GetField('GA_PRIXPOURQTE');
  if PrixPourQte = 0 then PrixPourQte := 1;
  QuotiteAchat := RatioMesure('PIE',TOBCatalogue.GetValue('GCA_QUALIFUNITEACH'));
  if QuotiteAchat = 0 then QuotiteAchat := 1;
  QuotiteVte := RatioMesure('PIE',GetField('GA_QUALIFUNITEVTE'));
  if QuotiteVte = 0 then QuotiteVte := 1;
  QuotiteSto := RatioMesure('PIE',GetField('GA_QUALIFUNITESTO'));
...
var Pu, PRIXPOURQTEAC, PrixPourQte: double;
    pa,pr,pv,pvc : double;
    TRefMarge : THValComboBox;
begin
  ...
  begin
//    THNumEdit(GetControl('GCA_PRIXBASE')).Value := (TOBCatalog.GetValue('GCA_PRIXBASE') / PrixPourQteAC) * PrixPourQte;
  ...
  begin
//    THNumEdit(GetControl('GCA_PRIXBASE')).Value := (TOBCatalog.GetValue('GCA_PRIXVENTE') / PrixPourQteAC) * PrixPourQte;
...
//  THNumEdit(GetControl('GF_PRIXUNITAIRE')).Value := TOBTarif.GetValue('GF_PRIXUNITAIRE') * PrixPourQte;
  ...
  begin
    if TOBCatalog.GetValue('GCA_PRIXBASE') <> 0 then Pu := (TOBCatalog.GetValue('GCA_PRIXBASE') / prixPourQteAC)
    else Pu := (TOBCatalog.GetValue('GCA_PRIXVENTE') / PrixPourQTeAC);
  end;
  ...
  Begin
    ...
    if NbDeclinaison = 1 then
      begin
        if MetreARticle <> nil then
        begin
        	MetreArticle.ValDec(Getfield('GA_ARTICLE'));
          if GetActiveTabSheet('PAGES').Name = 'P_METRES' then
          begin
            if OleExcel <> nil then
            begin
              oleExcel.doverb(ovHide);
              CloseOleContainer;
            end;
          end;
        end;
    ...
    if NbDeclinaison = 1 then
      Begin
        OpenOleContainer;
        if GetActiveTabSheet('PAGES').Name = 'P_METRES' then
        if MEtreARticle <> nil then MetreArticle.ModifMetres;
      end else
      begin
        TabMetre.TabVisible := false;
      end;
      ...
      if GetActiveTabSheet('PAGES').Name = 'P_METRES' then
      begin
        if OleExcel <> nil then oleExcel.doverb(OvShow);
      end;
  end;
end;
...
procedure TOM_Article.OpenOleContainer;
var dest : string;
begin
{
  // gestion des Variables : déclaration bouton
  ...
  if not OK_MetreDoc then exit;
  OleExcel := tolecontainer.Create(TabMetre);
  OleExcel.Parent := TabMetre;
  OleExcel.AutoActivate := aaManual;
  OleExcel.Align := alClient;

  MetreArticle := TMetreArt.create(Tform(Ecran), TabMetre, OleExcel);

  if not MetreArticle.actif then
     Begin
       SetControlProperty('BAppelVariable', 'Visible', False);
       exit;
     end;

  TabMetre.TabVisible := OK_MetreDoc;

  if (typeArticle = 'OUV') and (MetreArticle <> nil) then
     Begin
       MetreArticle.TypeArticle := TypeArticle;
       MetreArticle.Article :=  Getfield('GA_ARTICLE');
     End;
...
*****************************************************************}
procedure TOM_Article.CloseOleContainer;
begin
  if not OK_MetreDoc then exit;
  ...
  if oleExcel <> nil then FreeAndNil(OleExcel);

  DelVBModule('ArticlePgi.xla');
...
begin
	if CodeSousTarif = nil then exit;
  CodeSousTarif.value := '';
  CodeSousTarif.Plus := ' AND CC_LIBRE="'+CodeTarif+'"';
end;
