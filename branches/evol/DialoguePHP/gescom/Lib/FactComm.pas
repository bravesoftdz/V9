...
procedure ValideLesLignes(TOBPiece, TOBArticles, TOBCatalogu, TOBNomenclature, TOBOuvrage, TOBPieceRG, TOBBasesRG: TOB; Reliquat, Rafale: boolean; Ajout : boolean = false;GenereAuto:boolean=false;TOBOldPiece : TOB=nil; AutoriseDecoup : boolean=false);
...
procedure InverseStock (TOBPiece_O, TOBArticles, TOBCatalogu, TOBN_O : TOB) ; { NEWPIECE }
procedure InverseStockTransfo(TOBPiece_O, TobPiece, TOBArticles, TOBCatalogu, TOBN_O : TOB; Litige: Boolean = False); { NEWPIECE }
...
// Modif BTP
function GereEcheancesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG: TOB; Action: TActionFiche; DEV: RDevise; Ouv: boolean): Boolean;
// -----
{$IFDEF CHR}
...
// modif BTP
procedure ProrateEchesGC(TOBPiece, TOBEches, TOBAcomptes, TOBPieceRG: TOB; DEV: RDEVISE);
// --
function PieceEncoreVivante(TOBPiece: TOB): Boolean;
function GCPieceCorrecte(TOBPiece, TOBArticles, TOBCatalogu: TOB; ModeBordereau : boolean = false): integer;
...
function UpdateResteALivrer(TobPiece, TobPiece_O, TobArticles, TobCatalogu, TobN_O: Tob; Action: T_UpDateReliquat): Boolean; { NEWPIECE }
...
procedure GereLesLignesSoldees(TobPiece, TobPiece_O, TobArticles, TobCatalogu, TobN_O: Tob);
...
begin
  ...
  if (CtxMode in V_PGI.PGIContexte) and (SuppTOX) then
    Nb := ExecuteSQL('UPDATE PIECE SET GP_SUPPRIME="X",GP_VIVANTE="-",GP_DATEMODIF="' + USTime(NowH) + '",' +
      'GP_ESCOMPTE=0,GP_TOTALESC=0,GP_TOTALESCDEV=0,GP_TOTESCTTC=0,' +
      'GP_TOTESCTTCDEV=0,GP_REMISEPIED=0,GP_TOTALREMISE=0,' +
      'GP_TOTALREMISEDEV=0,' +
      'GP_TOTREMISETTC=0,GP_TOTREMISETTCDEV=0,GP_TOTALHT=0,' +
      'GP_TOTALHTDEV=0,GP_TOTALTTC=0,GP_TOTALTTCDEV=0,' +
      'GP_TOTALQTEFACT=0,GP_TOTALQTESTOCK=0,GP_TOTALBASEREMDEV=0,GP_TOTALBASEREM=0,' +
      'GP_TOTALBASEESCDEV=0,GP_TOTALBASEESC=0 ' +
      'WHERE ' + WherePiece(CleDoc, ttdPiece, False) + ' AND GP_DATEMODIF="' + USTime(OldD) + '"')
  else
...
*)
  ...
  //
  if Pos (cledoc.NaturePiece,'FBT;DAC') > 0 then
  begin
    ExecuteSQL('DELETE FROM LIGNEFAC WHERE ' + WherePiece(CleDoc, ttdLigneFac , False));
  end;
  //
  if TOBPieceRG_O <> nil then if TOBPieceRG_O.detail.count > 0 then
    begin
  ...
  if TOBBasesRG_O <> nil then if TOBBasesRG_O.Detail.count > 0 then
    begin
...
begin
  ...
  Nb := ExecuteSQL('DELETE FROM LIGNEBASE WHERE ' + WherePiece(CleDoc, ttdLigneBase, False));
  Nb := ExecuteSQL('DELETE FROM PIEDECHE WHERE ' + WherePiece(CleDoc, ttdEche, False));
  Nb := ExecuteSQL('DELETE FROM NOMENLIG WHERE ' + WherePiece(CleDoc, ttdNomen, False));
...
begin
  ...
  if not TOBLiens.InsertOrUpdateDB(False) then
  BEGIN
    MessageValid := 'Erreur écriture / LIENOLE';
    V_PGI.IoError := oeUnknown;
  END;
end;
...
//  if GetInfoParPiece(NaturePieceG, 'GPP_MAJPRIXVALO') = '' then exit;
  ...
  begin
    MessageValid := 'Erreur mise à jour articles';
    V_PGI.IoError := oeUnknown;
  end;
end;
...
//    OldQteReste := Max(0.0, Double(TobLigPiecePrecedente.G('GL_QTERESTE')));
    ...
    begin
      ...
      end else { Cas de la modification d'une ligne dans une pièce transformée (Modif. d'une ligne de livraison) }
    	begin
...
//      NewQteReste := Max(0 , Qte);
			if reliquat < 0 then qte := 0;
			newQtereste := Qte;
    end
    else
      NewQteReste := 0;
...
procedure _InverseStock(TOBPiece_O, TOBPiece, TOBArticles, TOBCatalogu, TOBN_O: TOB; Transfo: Boolean; Litige: Boolean = False);
var
  i, IndiceNomen, iArt, iTypeRef: integer;
  TheTobPiece, TOBL, TOBA, TOBNomen, TOBCata: TOB;
  NewTobLig: TOB;
  OldQteStock: Double;
...
begin
  ...
  begin
    if NaturePieceG = 'TRE' then
      DepotRecepteurGererSurSite := DepotGererSurSite(TheTobPiece.GetValue('GP_DEPOTDEST'));
  end;
  ...
  begin
    ...
    // -----
    NewTobLig := nil; OldQteStock := TobL.G('GL_QTESTOCK');
//    Maj := True;
    ...
    { Modif BTP }
    if (IndiceNomen > 0) and (TOBL.GetValue('GL_TYPEARTICLE') <> 'OUV') and (TOBL.GetValue('GL_TYPEARTICLE') <> 'ARP') then TOBNomen := TOBN_O.Detail[IndiceNomen - 1]
                                                                                                                       else TOBNomen := nil;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // --
          Tobl.P('GL_QTESTOCK', GetReliquatQteReste(Tobl, NewTobLig, nil));
          // Gestion des pièces d'avoir
          if (GetInfoParPiece(TOBPiece.GetValue('GP_NATUREPIECEG'), 'GPP_ESTAVOIR') = 'X') then
          begin
            InverseX(NewTobLig,'GL_QTESTOCK') ;
      ...
      begin
        ...
        begin
          if DepotRecepteurGererSurSite then
            MajQteStock(TobL, TOBArticles, TOBNomen, ColPlus, ColMoins, False, RatioVA);
        end
        else
        begin
...
procedure InverseStock(TOBPiece_O, TOBArticles, TOBCatalogu, TOBN_O: TOB);
begin
  _InverseStock(TOBPiece_O, nil, TOBArticles, TOBCatalogu, TOBN_O, False);
end;

{ NEWPIECE }

procedure InverseStockTransfo(TOBPiece_O, TobPiece, TOBArticles, TOBCatalogu, TOBN_O: TOB; Litige: Boolean = False);
begin
  _InverseStock(TOBPiece_O, TobPiece, TOBArticles, TobCatalogu, TOBN_O, True, Litige);
end;
...
begin
  ...
  if not TOBPorcs.InsertDB(nil) then
  begin
    MessageValid := 'Erreur mise à jour des Ports/frais';
    V_PGI.IoError := oeUnknown;
  end;
end;
...
begin
  ...
  begin
    ...
    if TheLignePrec.GetValue('GL_QTERESTE') > TheLignePrec.GetValue('GL_QTEFACT') then
      TheLignePrec.PutValue('GL_QTERESTE', TheLignePrec.GetValue('GL_QTEFACT'));
...
var
  i: integer;
  TOBL, TOBA, TOBCATA, TOBNomen: TOB;
...
begin
  ...
  if CtxMode in V_PGI.PGIContexte then
  begin
    if (NaturePieceG = 'TRE') and (TOBPiece.GetValue('GP_DEPOTDEST')<>'') then
      DepotRecepteurGererSurSite := DepotGererSurSite(TOBPiece.GetValue('GP_DEPOTDEST'))
  end;
  ...
  begin
    ...
    if ((TOBA <> nil) and (not Reliquat)) then
      TOBA.PutValue('UTILISE', 'X');
    TOBL.PutValue('GL_QUALIFMVT', QualifMvt);
    if TOBL.GetValue('GL_DEPOT') = '' then
    ...
    // fin Modif BTP
    TobNomen := nil;
    if (IndiceOuv > 0) and (TobOuvrage <> nil) then
    begin
      {$IFDEF BTP}
      if (not ReAffecteLigOuv(IndiceOuv, TobL, TobOuvrage)) then
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          if CtxMode in V_PGI.PGIContexte then
            DepotEmetteur := TOBPiece.GetValue('GP_DEPOT')
          else
          begin
            DepotEmetteur := TOBPiece.GetValue('GP_DEPOTDEST');
            TOBPiece.PutValue('GP_DEPOTDEST', '');
          end;
          RetourMajStock := MajQteStock(TOBL, TOBArticles, TOBNomen, ColPlus, ColMoins, True, RatioVA, DepotEmetteur);
        end
        else
          break;
      end
      ...
      begin
        ...
        begin
          RetourMajStock := MajQteStock(TOBL, TOBArticles, TOBNomen, ColPlus, ColMoins, True, RatioVA);
...
(*
          RestitueQteOrigine (TOBOldPiece,TOBL,TOBARTICLES);
*)
...
begin
  NewNum := 0;
  if Souche = '' then Exit;
  NewNum := SetNumberAttribution(Souche, 1);
end;
...
begin
  ...
  if NumPiece <> -1 then CleDoc.NumeroPiece := Numpiece
                    else CleDoc.NumeroPiece := GetNumSoucheG(CleDoc.Souche);
  if CleDoc.NumeroPiece <= 0 then
  begin
    PGIBox('Souche associée à la nature de pièce non valide', 'Création de pièce');
    Exit;
...
begin
  ...
  if ((ctxGRC in V_PGI.PGIContexte) and (V_PGI.IoError = oeOK)) then
    MajProspectClient(TOBPiece, TobTiers);
  {$ENDIF}
  TOBTiers.PutValue('T_DATEDERNPIECE', TOBPiece.GetValue('GP_DATEPIECE'));
  TOBTiers.PutValue('T_NUMDERNPIECE', TOBPiece.GetValue('GP_NUMERO'));
  TOBTiers.PutValue('T_TOTDERNPIECE', TOBPiece.GetValue('GP_TOTALHT'));
  ...
  SQL := 'UPDATE TIERS SET T_DATEDERNPIECE="' + UsDateTime(TOBPiece.GetValue('GP_DATEPIECE')) + '", '
    ...
    + 'T_NATUREAUXI="' + TOBTiers.GetValue('T_NATUREAUXI') + '", '
    + 'T_DATEPROCLI="' + UsDateTime(TOBTiers.GetValue('T_DATEPROCLI')) + '" '
    + 'WHERE T_TIERS="' + TOBTiers.GetValue('T_TIERS') + '"';

  Nb := ExecuteSQL(SQL);
  if Nb <> 1 then
  BEGIN
    MessageValid := 'Erreur écriture adresses';
...
{$IFDEF BTP}
        ...
        if NomCol = 'GL_HEURE' then SG_TEMPSTOT := icol else
...
{$ENDIF}
        ...
        // ----------
        {$IFDEF CHR}
        if NomCol = 'GL_NOFOLIO' then SG_Folio := icol else
        if NomCol = 'GL_DATEPRODUCTION' then SG_DateProd := icol else
        if NomCol = 'GL_REGROUPELIGNE' then SG_Regrpe := icol else
        {$ENDIF} // FIN AJOUT CHR
...
function AdapteCond(TOBL, TOBConds: TOB; var Qte: Double): boolean;
var CodeCond, RefUnique: string;
  TOBC: TOB;
  LastCond: string;
  XQ, LastQte, NbArt, PEntSup, CoefN: Double;
begin
  Result := False;
  CodeCond := TOBL.GetValue('GL_CODECOND');
  if CodeCond = '' then Exit;
  RefUnique := TOBL.GetValue('GL_ARTICLE');
  ...
  LastCond := '';
  // Cas particulier
  if Qte = Trunc(Qte) then
  begin
    TOBC := TOBConds.FindFirst(['GCO_ARTICLE', 'GCO_CODECOND'], [RefUnique, CodeCond], False);
    if TOBC <> nil then
    begin
  ...
  // Boucle sur les conditionnements
  ...
  begin
    ...
    begin
      PEntSup := Qte / NbArt;
      if TOBC.GetValue('GCO_DECOND') = 'X' then
      begin
        if Qte = Trunc(Qte) then LastQte := Qte else LastQte := Trunc(Qte) + 1;
        LastCond := TOBC.GetValue('GCO_CODECOND');
        Break;
      end else
      begin
    ...
    TOBC := TOBConds.FindNext(['GCO_ARTICLE'], [RefUnique], False);
  end;
  if LastCond <> '' then
  begin
    TOBL.PutValue('GL_CODECOND', LastCond);
    Qte := LastQte * CoefN;
    Result := True;
  end;
end;
...
{$IFDEF BTP}
      VC := ((TOBL.GetValue('GL_NATUREPIECEG') = 'FAC') or (TOBL.GetValue('GL_NATUREPIECEG') = 'AVS')
        ...
        or (TOBL.GetValue('GL_NATUREPIECEG') = 'ABT'))
{$ELSE}
      VC := ((TOBL.GetValue('GL_NATUREPIECEG') = 'FAC') or (TOBL.GetValue('GL_NATUREPIECEG') = 'AVS')
...
procedure ModeToEchesGC(Mode: T_ModeRegl; TOBPiece, TOBEches: TOB; NbLigAcpte : integer);
var TOBH: TOB;
  i: integer;
  MtDed, MtMaj : double;
begin
...
function InitModeGC(TOBPiece, TOBTiers, TOBAcomptes, TOBPieceRG: TOB; DEV: RDEVISE): T_ModeRegl;
var Mode: T_ModeRegl;
  XP, XD : Double;
  DD: TDateTime;
  // Modif BTP
...
function FabriqueEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG: TOB; DEV: RDEVISE): T_ModeRegl;
var Mode: T_ModeRegl;
    nbacpt : Integer;
begin
  {$IFDEF CHR}
  ...
  {$ELSE}
  Mode := InitModeGC(TOBPiece, TOBTiers, TOBAcomptes, TOBPieceRG, DEV);
  {$ENDIF} // FIN CHR

  CalculModeRegle(Mode, True);
  if (TOBAcomptes = NIL)  then nbacpt := 0 else nbacpt := TOBAcomptes.Detail.count;
  ModeToEchesGC(Mode, TOBPiece, TOBEches, nbacpt);
  Result := Mode;
end;
...
begin
  ...
  if TotD = 0 then Exit;
  // Modif BTP
  Coef := (TOBPiece.GetValue('GP_TOTALTTCDEV') - AccD - RGD) / TotD;
  ...
  begin
    ...
    begin
      XD := Arrondi(TOBPiece.GetValue('GP_TOTALTTCDEV') - AccD - TotD - RGD, DEV.Decimale);
      TOBH.PutValue('GPE_MONTANTDEV', XD);
      XP := Arrondi(TOBPiece.GetValue('GP_TOTALTTC') - AccP - TotP - RGP, V_PGI.OkDecV);
      TOBH.PutValue('GPE_MONTANTECHE', XP);
    end;
  end;
end;

function RenseigneEchesGC (TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPIECERG: TOB;
                           DEV: RDEVISE; Action : TActionFiche): T_ModeRegl;
var Mode: T_ModeRegl;
begin
  {$IFDEF CHR}
  Mode := InitModeChr(TOBPiece, TOBTiers, TOBEches, DEV);
  //ProrateEchesGC(TOBPiece,TOBEches,TOBAcomptes,TOBPieceRG,DEV) ;
  // CHR : On ne repartit pas l'échéance par ligne
  {$ELSE}
  Mode := InitModeGC(TOBPiece, TOBTiers, TOBAcomptes, TOBPieceRG, DEV);
...
begin
  ...
  if TOBEches.Detail.Count <= 0 then Mode := FabriqueEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG, DEV)
  else Mode := RenseigneEchesGC(TOBPiece, TOBTiers, TOBEches, TOBAcomptes, TOBPieceRG, DEV, Action);
  if Ouv then
  begin
    if Action = taConsult then Mode.Action := taConsult;
    if Mode.TotalAPayerD = 0 then Okok := True else Okok := SaisirEcheance(Mode);
    if Okok then ModeToEchesGC(Mode, TOBPiece, TOBEches, TOBAcomptes.Detail.count) else Result := False;
  end;
  if Result then
  begin
...
var i, iArt, iMt, iCirc: integer;
		iartcli : integer;
  TOBL: TOB;
  OkArt, OkMtant, OkDate, OkNatAffaire, OkCircuit : boolean;
  MtAcc: Double;
  CodeD, StDev, NaturePieceG: string;
begin
  NaturePieceG := TobPiece.GetValue('GP_NATUREPIECEG');
  {$IFDEF BTP}
...
begin
  ...
  begin
    Result := TheTob.FindFirst(['GL_NATUREPIECEG', 'GL_DATEPIECE', 'GL_SOUCHE'
      , 'GL_NUMERO', 'GL_INDICEG', 'GL_NUMORDRE']
        , [CleDoc.NaturePiece, CleDoc.DatePiece, CleDoc.Souche
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TobModifFille.PutValue('OLD_SOLDERELIQUAT', TobLigneOld.GetValue('GL_SOLDERELIQUAT'));
          end;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not VerifQuantitePieceSuivante(TobLigne, TobLigne.GetValue('GL_QTEFACT')) then
        Result := i;
    end;
  end;
end;
...
var
  ...
  TobLigne, TobLignePiecePrecedente: Tob;
  TobNomen: Tob;
  ...
  SolderReliquat, DesolderReliquat: Boolean;
  NewReste, QteTransfo, Delta,Temp: Double;
...
begin
  ...
  try
    ...
    begin
      ...
      if (Diff <> 0) or SolderReliquat or DesolderReliquat then
      begin
              ...
              try
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if TobLignePiecePrecedente.GetValue('GL_QTERESTE') > 0 then
                        Diff := -1 * TobLignePiecePrecedente.GetValue('GL_QTERESTE')
                    end
                ...
                begin
                  Signe := '+';
                  if Delta > TobLignePiecePrecedente.GetValue('GL_QTEFACT') then
                  	Diff := TobLignePiecePrecedente.GetValue('GL_QTEFACT') - TobLignePiecePrecedente.GetValue('GL_QTERESTE');
                end else
                begin
                  Signe := '-';
                  if Delta < 0 then
                  	Diff := TobLignePiecePrecedente.GetValue('GL_QTERESTE') * (-1);
                ...
                begin
                  ...
                  Sql := 'UPDATE LIGNE SET GL_QTERESTE = GL_QTERESTE ' + Signe + ' ' + StrFPoint(Diff)
                    + ' WHERE ' + WherePiece(CleDocPrecedente, ttdLigne, True, True);
                  ...
                  begin
                    ...
                    begin
                      ...
                      Sql := 'UPDATE PIECE SET GP_DATEMODIF="' + USTime(NowH) + '" WHERE ' + WherePiece(CleDocPrecedente, ttdPiece, False);
                      Result := (ExecuteSQL(Sql) = 1);
...
//                        NewQteReste := Max(0, Double(TobLignePiecePrecedente.G('GL_QTERESTE')) + Diff);
                        ...
                        NewQteReste := TobLignePiecePrecedente.G('GL_QTERESTE');
                        ...
                        begin
                          ...
                          { Récupère la TobNomen - TP - Bug 10528 }
                          IndiceNomen := TobLignePiecePrecedente.GetValue('GL_INDICENOMEN');
                          if (IndiceNomen > 0) and Assigned(TobN_O) and (TOBN_O.detail.count >0)  then
                            TobNomen := TobN_O.Detail[IndiceNomen - 1]
                          else
                            TobNomen := nil;
                          { Ajout de la TobNomen dans les paramètres }
                          OkStock := MajQteStock(TobLignePiecePrecedente, TobArticles, TobNomen, ColPlus, ColMoins, True, RatioVA);
                        end;
                      end;
...
begin
  ...
  begin
    if (TobL.GetValue('GL_PIECEPRECEDENTE') <> '')
{$IFDEF BTP}
    and IsPieceGerableReliquat(TOBL)
{$ENDIF}
    then
    begin
...
//      if TobL.GetValue('GL_QTERELIQUAT') < 0 then
//        TobL.PutValue('GL_QTERELIQUAT', 0);
    end;
  end;
end;
...
var
  CleDoc, PrevCleDoc, CurrPieceCleDoc: R_CleDoc;
  i, MaxIndice: Integer;
begin
  MaxIndice := -1;
  //
...
var
  TobPiece, TobDim: Tob;
  i, iTypeDim, iCodeArticle, iQteReste: Integer;
  ...
  begin
    ...
    begin
      Result := (TobL.GetValue('GL_QTERESTE') = 0)
                 or (TobL.GetValue('GL_SOLDERELIQUAT') = 'X');
    end else
    begin
      Result := ((TobL.GetValue('GL_QTERESTE') = 0) and (TOBL.GetValue('GL_QTEFACT') <> 0))
                 or (TobL.GetValue('GL_SOLDERELIQUAT') = 'X');
    end;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          while (TobDim.GetValeur(iTypeDim) = 'DIM') and (TobDim.GetValeur(iCodeArticle) = CodeArticleGen)
                                                     and Result do
          begin
...
begin
  ...
  begin
    ...
    TRY
      ...
      if not TobLignesCompl.InsertDB (nil,true) then
      begin
        MessageValid := 'Erreur écriture LIGNECOMPL';
        V_PGI.Ioerror := OeUnknown;
      end;
    finally
      TobLignesCompl.Free;
    ...
    begin
      ...
      try
        ...
        if (IndiceNomen > 0) and Assigned(TobN_O) and (TOBN_O.detail.count >0)  then
          ...
          TobNomen := nil;
        { Ajout de la TobNomen dans les paramètres }
        MajQteStock(TOBL, TobArticles, TobNomen, ColPlus, ColMoins, Inverse, RatioVA);
      finally
  ...
  begin
    ...
    begin
      ...
      if TransfoPiece then
        ...
        TOBL_O := GetTOBPrecBySQL(TOBL, 'GL_SOLDERELIQUAT,GL_QTERELIQUAT,GL_QTERESTE');
...
function IsgestionAvanc (TOBpiece : TOB) : boolean;
begin
	result := (Pos(TOBpiece.getValue('GP_NATUREPIECEG'),'FBT;DAC')>0) and
