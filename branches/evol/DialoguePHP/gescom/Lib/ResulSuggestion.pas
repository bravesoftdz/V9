...
//      TOB(G_LIG.Objects[0, ARow]).PutValue('GL_PUHTBASE', Valeur(G_LIG.Cells[ACol, ARow]));
  ...
  begin
    ...
    begin
      ...
      begin
        Q := OpenSQL('Select T_TIERS from TIERS where T_TIERS="'+G_LIG.Cells[ACol, ARow]+'"', True,-1,'',true);
        if Q.EOF then
        begin
          Cancel := True;
...
{$IFNDEF PGIMAJVER}
{$IFNDEF BTP}
Q := OpenSQL('Select Count(*) from CATALOGU where GCA_ARTICLE="'+G_LIG.Cells[0, G_LIG.Row]+'"', True,-1,'',true);
Nb := Q.Fields[0].AsInteger;
Ferme(Q);
if Nb = 0 then
    st1 := AGLLanceFiche('GC', 'GCFOURNISSEUR_MUL', 'T_NATUREAUXI=FOU', '', 'SELECTION')
    else
    st1 := AGLLanceFiche('GC', 'GCCATALOGU_RECH', 'GCA_ARTICLE='+G_LIG.Cells[0, G_LIG.Row], '', '');
if st1 <> '' then
    begin
    TOB(G_LIG.Objects[0, G_LIG.Row]).PutValue('GL_TIERS', st1);
    G_LIG.Cells[5, G_LIG.Row] := st1;
    end;
{$ELSE}
...
(*
   ...
   begin
      ...
      MtPa := RecupTarifAch (st1,Article);
      TOBL.PutValue('GL_PUHT', MtPa);
      TOBL.PutValue('GL_PUHTDEV', MtPa);
      TOBL.PutLigneGrid (G_LIG,G_LIG.row,false,false,LesCols);
   end;
*)
{$ENDIF}
{$ENDIF}
...
{$IFDEF BTP}
		ua := '';
    MtPa := RecupTarifAch (MaCle,Article,Ua,CoefUaUs,TurStock);
{$ENDIF}
    TOBL.PutValue('GL_PUHT', MtPa);
    TOBL.PutValue('GL_PUHTDEV', MtPa);
*)
...
var FUS,FUA,CoefuaUs,PQQ : double;
