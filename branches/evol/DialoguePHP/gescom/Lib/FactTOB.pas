...
uses {$IFDEF VER150} variants,{$ENDIF} HEnt1, UTOB, UtilPGI, EntGC, SysUtils, Math, HCtrls,uEntCommun
{$IFDEF EAGLCLIENT}
{$ELSE}
...
const
  SG_NL: integer = 0        ;
  SG_ContreM: integer = -1  ; SG_RefArt: integer = 1 ; SG_Lib: integer = 2     ; SG_QF: integer = 3       ; SG_QS: integer = -1    ;
  SG_Px: integer = 5        ; SG_Rem: integer = 6    ; SG_Aff: integer = 7     ; SG_Rep: integer = 8      ; SG_Dep: integer = 9    ;
  SG_RV: integer = 10       ; SG_RL: integer = 11    ; SG_Montant: integer = 12; SG_DateLiv: integer = -1 ; SG_Total: integer = -1 ;
  SG_QR: integer = -1       ; SG_QReste: integer = -1; SG_Motif: integer = -1  ; SG_QA: integer = -1      ; SG_Pct: integer = -1   ;
  SG_Unit: integer = -1     ; SG_TypA: integer = -1  ; SG_Folio: integer = -1  ; SG_DateProd: integer = -1; SG_Regrpe: integer = -1;
  SG_LibRegrpe: integer = -1; SG_PxNet: integer = -1 ; SG_TypeRem: integer = -1;
  SG_Circuit: integer = -1;
  SG_PXAch : integer = -1; SG_MontantAch : integer = -1;
  SG_REFTiers : integer = -1; SG_DETAILBORD : Integer = -1; SG_DEJAFACT : integer =-1; SG_montantSit : integer = -1; SG_MontantMarche : integer = -1;
  SG_QTEPREVUE : integer = -1; SG_POURCENTAVANC: integer = -1; SG_QTESITUATION: integer = -1; SG_MTDEJAFACT : integer = -1;
  SG_TEMPS : integer = -1; SG_TEMPSTOT : integer = -1;
  SG_QUALIFTEMPS : integer = -1; SG_CODTVA1 : integer = -1; SG_NATURETRAVAIL : integer = -1; SG_FOURNISSEUR : integer = -1; SG_VOIRDETAIL : integer = -1;


implementation
uses
  FactTarifs,FactComm,FactDomaines
  ;
...
procedure AddLesSupEntete(TOBE: TOB);
begin
  if (not TOBE.FieldExists('AFF_GENEREAUTO')) then TOBE.AddChampsupValeur('AFF_GENEREAUTO', '', False);
  if (not TOBE.FieldExists('AFF_OKSIZERO')) then TOBE.AddChampsupValeur('AFF_OKSIZERO', '-', False);
...
function FindChamps(ITable : integer;Nomchamp : string) : integer;
var Indice : integer;
begin
  result := -1;
  for Indice := 0 to High(V_PGI.Dechamps[iTable]) do
  begin
    if V_PGI.Dechamps[iTable, Indice].Nom = NomChamp then
    begin
      result := Indice;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
      	prefixe := '';
        if Pos('_',NomChamp)> 0 then
        begin
        	prefixe := copy (NomChamp,1,Pos('_',NomChamp)-1);
        	iTable := PrefixeToNum(prefixe);
          if iTable >= 0 then
          begin
            Indice := FindChamps(ITable,Nomchamp);
            if Indice >= 0 then
            begin
            	ConvertTypeChamps (TOBDest,TOBProv,NomChamp,V_PGI.Dechamps[iTable, Indice].tipe);
              ConvertOk := true;
            end;
          end;
        end;
