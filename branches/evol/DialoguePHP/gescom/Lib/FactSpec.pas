...
// Modif BTP
Function BeforeCalcul ( FF : TForm ; TOBPiece,TOBBases,TOBTiers,TOBArticles,TOBOuvrage,TOBCpta,TOBCata,TOBAffaire,TOBGCS,TOBPorcs : TOB ; DEV : RDEVISE; var Arow: integer ; CalculPv : boolean=true; RazFg : boolean=false; OnlyPv : boolean=false) : boolean ;
...
// Modif BTP
{$IFDEF BTP}
Procedure MajLignesSituation ( TOBPiece: TOB; DEV : RDEVISE) ;
{$ENDIF}
Procedure AfterShowForLine (FF : TForm; NaturePiece : String);

procedure InitLigneValoPR (TOBL : TOB);
procedure BeforeTraitementCalculCoef (TOBArticles,TOBPiece,TOBPorcs,TOBOUvrages : TOB; DEV : Rdevise; SaisieTypeAvanc : boolean=false);
...
begin
  ...
  if TOBPiece.GEtVAlue('AFF_OKSIZERO')='X' Then result := true;
end;

Procedure MajLignesSituation ( TOBPiece : TOB; DEV : RDEVISE ) ;
Var i,j, niveau : integer ;
    TOBL : TOB ;
    QtePrevue, montantreel, montantcalcule, montantcumule, montantprecedent : double ;
    TypFac, Typlig : String;
    UneLigneFacturee: Array [1..9] of Boolean ;
BEGIN
TypFac:=RenvoieTypeFact (TOBPiece.GetValue('GP_AFFAIREDEVIS'));

i:=0;
While (i < TOBPiece.Detail.Count) Do
  BEGIN
  TOBL:=TOBPiece.Detail[i] ;
  TypLig := Copy(TOBL.GetValue('GL_TYPELIGNE'),1,2);
  Niveau := TOBL.GetValue('GL_NIVEAUIMBRIC');

(* MODIFBRL : 22/07/04 : on conserve les sauts de page déjà présents dans le devis
...
*)
  ...
  if (TypLig ='DP') then
    begin
    UneLigneFacturee[niveau]:=False;
    end
  else if (TOBL.GetValue('GL_TYPELIGNE')='ART') then
    begin
    // Pour un article de type pourcentage, la quantité facturée est
    // forcément la quantité prévue
    if TOBL.GetValue('GL_TYPEARTICLE')='POU' then
      ...
      if (TOBL.GetValue('GL_QTEPREVAVANC')=0) and (TOBL.GetValue('GL_QTESIT')=0) and(not IsQteZeroAutorise(TOBPiece)) then
      begin
        TOBL.Free; // pas de génération pour une ligne à zéro
        Continue; //pas d'incrémentation du compteur de ligne
      end else
      begin

      // MAJ indicateur de ligne facturée sauf pour les lignes d'article en pourcentage
      if TOBL.GetValue('GL_TYPEARTICLE')<>'POU' then
        begin
        for j:=niveau downto 1 do
          begin
          UneLigneFacturee[j]:=True;
          end;
        end;

      QtePrevue := TOBL.GetValue('GL_QTEFACT'); // la qté prévue est la qté en devis

      if (TypFac = 'AVA') or (TypFac = 'DAC') then
        // Traitement des situations
        begin
        TOBL.PutValue('GL_QTEFACT',TOBL.GetValue('GL_QTEPREVAVANC')-TOBL.GetValue('GL_QTESIT')) ; // la quantité facturée est la quantité "avancée"
        (*
        // calcul si écart entre montant reel = qté facturée * PU
        // et montant calculé =(qté cumulée*PU) - (Qté précédente*PU)
        // dans ce cas affectation de l'écart sur la remise ligne
        montantreel := TOBL.GetValue('GL_QTEFACT')*TOBL.GetValue('GL_PUHTNETDEV');
        montantcumule := TOBL.GetValue('GL_QTEPREVAVANC')*TOBL.GetValue('GL_PUHTNETDEV');
        montantprecedent := TOBL.GetValue('GL_QTESIT')*TOBL.GetValue('GL_PUHTNETDEV');
        montantreel := arrondi(montantreel,DEV.Decimale);
        montantcalcule := arrondi(montantcumule,DEV.Decimale) - arrondi(montantprecedent,DEV.Decimale);

        if (montantreel <> montantcalcule) then
          TOBL.Putvalue('GL_VALEURREMDEV',TOBL.Getvalue('GL_VALEURREMDEV') - (montantreel-montantcalcule));
        *)
        TOBL.PutValue('GL_QTESIT',TOBL.GetValue('GL_QTEPREVAVANC')) ; // stockage Qté cumulée
        end
      else
        // traitement facture directe
        begin
        TOBL.PutValue('GL_QTEFACT',TOBL.GetValue('GL_QTEPREVAVANC')) ; // la quantité facturée est la quantité saisie
        TOBL.PutValue('GL_QTESIT',TOBL.GetValue('GL_QTESIT')+TOBL.GetValue('GL_QTEPREVAVANC')) ; // stockage Qté cumulée
        end;

      TOBL.PutValue('GL_QTEPREVAVANC',QtePrevue) ; // on conserve la qté prévue dans la qté avancement de la facture

      // Suite à la nouvelle gestion des reliquats, on force GL_QTERESTE à GL_QTESTOCK
      // car si le devis n'est plus vivant, la moulinette de la 608 ne l'a pas traité.
      // Ces deux valeurs ne sont donc pas identiques et sont récupérées telles quelles
      // dans la facture. Ceci pose problème en modification éventuelle de cette facture.
      TOBL.PutValue('GL_QTESTOCK',TOBL.GetValue('GL_QTEFACT')) ;
      TOBL.PutValue('GL_QTERESTE',TOBL.GetValue('GL_QTESTOCK')) ;
        InitLigneValoPR (TOBL);
        TOBL.PutValue('GL_MONTANTPA',TOBL.GetValue('GL_QTEFACT')*TOBL.GetValue('GL_DPA')) ;
        TOBL.PutValue('GL_MONTANTPAFG',TOBL.GetValue('GL_MONTANTPA')) ;
        TOBL.PutValue('GL_MONTANTPR',TOBL.GetValue('GL_QTEFACT')*TOBL.GetValue('GL_DPR')) ;
        TOBL.PutValue('GL_MONTANTFG',TOBL.GetValue('GL_MONTANTPR')-TOBL.GetValue('GL_MONTANTPA')) ;
        if TOBL.GetValue('GL_MONTANTPA')<>0 then
        begin
          TOBL.PutValue('GL_COEFFG',TOBL.GetValue('GL_MONTANTFG')/TOBL.GetValue('GL_MONTANTPA'));
        end;
        //
        TOBL.PutValue('GL_QTESTOCK',TOBL.GetValue('GL_QTEFACT')) ;
        TOBL.PutValue('GL_QTERESTE',TOBL.GetValue('GL_QTESTOCK')) ;
      end;
    end else if (Typlig = 'TP') and (UneLigneFacturee[niveau] = False) then
    begin
    // suppression de tout le paragraphe
    TOBL.free;
    j:=i-1;
    While (j>=0) and (Copy(TOBPiece.Detail[j].GetValue('GL_TYPELIGNE'),1,2) <>'DP') Do
      begin
      TOBPiece.Detail[j].free;
      j:=j-1;
      end;
    TOBPiece.Detail[j].free;
    i:=j;
    Continue;
    end else if (Typlig = 'TP') and (UneLigneFacturee[niveau]) then
    begin
      InitLigneValoPR (TOBL);
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if Pos(TOBL.getValue('GL_NATUREPIECEG'),'FBT;ABT;FPR;FAC;AVC')>0 then
            begin
            ...
            begin
              if (not FromImportBordereaux) and (TOBL.GetValue('GL_BLOQUETARIF')='-') then
              begin
                TOBL.putValue('GL_COEFMARG',CoefMarg);
                TOBL.putValeur(indPuHt,Arrondi(TOBL.getValeur(IndPr)*CoefMarg,V_PGI.okdecP));
  ...
  begin
    ...
    begin
    	...
    	TOBpiece.putValue('GP_MONTANTFG',0); // car deja calcule precedemment
			AppliqueFraisPiece (TOBPiece,TOBOuvrage,true,calculPv,(TOBPiece.getVAlue('GP_FACTUREHT')='X'),DEV);
      TOBPiece.PutValue('GP_RECALCACHAT','-');
      Arow := -1; // force le calcul pour l'ensemble des lignes
    end else
    begin
  	  CalculeMontantsDoc (TOBPiece,TOBOuvrage,calculPv);
    end;
  end else
  begin
  	CalculeMontantsDoc (TOBPiece,TOBOuvrage,calculPv);
...
Var Ind  : Integer ;
    ...
    FFact : TFFacture;
    Requete : String;
    RefInt, s,NextPiece: String;
BEGIN
FFact := TFFacture(FF);
Result:=True ;
NextPiece := GetParamSocSecur('SO_BTCHOIXPIECEPROPOS','DBT');
...
if (ctxGRC in V_PGI.PGIContexte) and
   ...
   (TobPiece.getValue('GP_NATUREPIECEG')=NextPiece) and
...
if (ctxGRC in V_PGI.PGIContexte) and
	 (TobPiece.getValue('GP_PERSPECTIVE')<>0) and
   (TobPiece.getValue('GP_NATUREPIECEG')=NextPiece) and (Old_TOTALHT <> 0 ) then
   begin
{$IFDEF GRC}
{$IFNDEF CCS3}
...
{$IFDEF GPAO}
   ...
   begin
      PgiInfo(wGetLastErrorMessageFromWIL(s)
              + ' (' + TraduireMemoire('Sur la ligne de pièce N° ') + ' ' + IntToStr(Ind + 1) + ')', '');
      Result := False;
      EXIT;
   end;
{$ENDIF}
...
begin
  ...
  FFact.MBanal.Visible := false;
  FFact.SimulationdeRentabilit1.Visible := false;
...
BEGIN
  ...
  begin
    ...
    Complmentvariables1.visible := false;
...
begin
  ...
  begin
    ...
    begin
    	TOBOUV.PutValeur(IndCoefFG, (TOBOUV.getValeur(IndPR)/TOBOUV.getValeur(IndPa))-1);
    end;
  end;
end;
...
procedure AffecteNewZonesLigne (TOBArticles,TOBPiece : TOB; DEV : Rdevise; SaisieTypeAvanc : boolean=false);
var Indice : integer;
		TOBL : TOB;
    TOBA : TOB;
    IndTPsUnitaire,IndTypeRes,IndTypeLig,IndTypeArt,IndArt,IndFourn,IndCoefMarg,IndCoeffG,IndPvHtdev,IndCoefDev : integer;
    IndPvHt,IndPR,IndPrxbase,IndPourcentAvanc,IndTotalHtDev,IndMontantFact,IndMontantSit : integer;
...
begin
  ...
  begin
    ...
    if (Pos (TOBL.GetValeur(IndTypeRes),VH_GC.BTTypeMoInterne) > 0) then
    begin
...
procedure AffecteNewZonesOuv (TOBArticles,TOBOuvrages : TOB);
var Indice : integer;
		TOBL,TOBA : TOB;
    IndTPsUnitaire,IndTypeRes,IndTypeLig,IndTypeArt,IndCoefMarg,IndFourn : integer;
    IndPuHt,IndPR,IndArt,IndPuHTdev,IndCoefDev : integer;
    first : boolean;
    CoefFGLig : double;
begin
	first := true;
...
begin
  ...
  AffecteNewZonesLigne (TOBArticles,TOBPiece,DEV,SaisieTypeAvanc);
  AffecteNewZonesOuv (TOBArticles,TOBOuvrages);
end;
