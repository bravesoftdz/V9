...
begin
if not bParamHT and not bParamTTC and not bParamPxBase and
    not CALCAUTOPV.Checked and not CALCPASAUTOPV.Checked then exit;
...
{$IFDEF BTP}
       RecalculPr(TOBD,Nil);
{$ELSE}
       RecherchePxAffecte(TOBD,'PAA', TOBD.GetValue('GA_PAHT'));
{$ENDIF}
       end;
        ...
        if (CALCPASAUTOPV.Checked = true) and (StrToFloat (ECART.Text) > 0.0) and
           ...
           (TOBD.GetValue ('GA_CALCAUTO' + stTypePrix) = '-') then
            ...
            if TOBD.GetValue ('GA_PV' + stTypePrix) <> 0.0 then
                begin
                if (TOBD.GetValue ('GA_PV' + stTypePrix) - dPrix)*100.0/
                        TOBD.GetValue ('GA_PV' + stTypePrix) < StrToFloat (ECART.Text) then
                    begin
                    TOBD.PutValue ('GA_PV' + stTypePrix, dPrix);
                    end;
                end else TOBD.PutValue ('GA_PV' + stTypePrix, dPrix);
            end else if CALCAUTOPV.Checked = true then
            begin
            if stTypePrix = 'HT' then TOBD.PutValue ('GA_CALCAUTOTTC', '-')
            else TOBD.PutValue ('GA_CALCAUTOHT', '-');
...
if TypeMAJ = 'P01' then Prix:=Valeur
   else if TypeMAJ = 'P02' then Prix := PrixRef * Valeur
       else if TypeMAJ = 'P03' then Prix := (PrixRef * (1 + Valeur));
