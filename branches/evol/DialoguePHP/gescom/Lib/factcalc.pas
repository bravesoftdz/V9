...
{$ENDIF}
     ...
     UtilPGI, UTofPiedPort,TarifUtil,UtilArticle,Forms,FactLigneBase ;

Procedure CalculFacture ( TOBPiece,TOBPieceTrait,TOBouvrages,TOBouvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPorcs,TOBPieceRG,TOBBasesRG : TOB ; DEV : RDEVISE ;SaisieTypeAvanc : boolean=false; Action: TActionFiche=TaModif; ModifAvanc : boolean=false; LigneACalcule : integer=-1; FromGenfac : boolean=false; FromZero : boolean=false) ;
// --
...
procedure CalculeLaLigne (TOBPiece,TOBPieceTrait,TOBOUvrages,TOBOUvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TOBPOrcs,TOBPieceRG,TOBBasesRG,TOBL : TOB;DEV : Rdevise; NbDec : integer; saisieAvanc : boolean =false; ModifAvanc : boolean=false; FF : Tform = nil);
{$IFDEF BTP}
procedure CalculTaxesLigneBTP ( TOBL,TOBPiece,TOBTiers,TOBTaxesL : TOB ; DEV : Rdevise; NbDec : integer ; Base : Double ; EnHT : Boolean ) ;
{$ENDIF}
...
X:=-TOBL.GetValeur(ii) ; if X<>0 then TOBL.PutValeur(ii,X) ;
END ;
...
if NomTable='PIECE' then
   ...
   for k:=0 to TOBP.Detail.Count-1 do
       BEGIN
       TOBL:=TOBP.Detail[k] ;
       InverseX(TOBL,'GL_QTESTOCK') ; InverseX(TOBL,'GL_QTEFACT') ; InverseX(TOBL,'GL_QTERESTE') ; { NEWPIECE }
...
Var Base,Taux,MontantTaxe,MontantTaxeDev,BaseDev : Double ;
    TOBA,TOBT : TOB ;
    RegimeTaxe,FormuleTaxe,Categorie : String ;
...
begin
	TheMillieme := TOBL.GetValue('MILLIEME'+IntToStr(NumCat));
end;
{$ENDIF}
...
Var TOBB,TOBA : TOB ;
    i : integer ;
    FamilleTaxe,Categorie,Fournisseur : String ;
BEGIN
  // Modif BTP
...
BEGIN
  ...
  // --
  QteS:=TOBL.GetValue(prefixe+'_QTESTOCK') ; RatioVA:=GetRatio(TOBL,Nil,trsDocument) ;
  ...
  X:=RatioMesure('TEM',TOBL.GetValue(prefixe+'_QUALIFHEURE'))    ; TOBL.PutValue(prefixe+'_TOTALHEURE',X*RatioVA*QteS*TOBL.GetValue(prefixe+'_HEURE')) ;
END ;
...
var Indice : integer;
    ...
    BaseHt,baseHtDev,DelTa,baseTTC,baseTTCDEV,MTTva,MtTvaDev : double;
...
begin
  ...
  //
  if (TOBBasesL.detail.count > 1) and (TOBP.getValue('GL_TYPENOMEN')='OU1') then
  begin
  ...
  begin
    ...
    if Delta <> 0 then
    begin
      TOBP.PutValue('GL_TOTALTAXE'+InttOStr(Indice),TOBP.GetValue('GL_TOTALTAXE'+InttOStr(Indice))+Delta);
    end;
    Delta := Arrondi(TMontantTvaDev[Indice] - TOBP.GetValue('GL_TOTALTAXEDEV'+InttOStr(Indice)),V_PGI.OkdecV) ;
    if Delta <> 0 then
    begin
      TOBP.PutValue('GL_TOTALTAXEDEV'+InttOStr(Indice),TOBP.GetValue('GL_TOTALTAXEDEV'+InttOStr(Indice))+Delta);
    end;
  end;
  //
  if EnHt then
  begin
    ...
    if DelTa <> 0 then
    begin
      TOBP.putValue('GL_TOTALTTC',TOBP.GetValue('GL_TOTALTTC')+Delta);
    end;
    //
    Delta := (BaseHtDev+MTTvaDev) - TOBP.GetValue('GL_TOTALTTCDEV');
    if DelTa <> 0 then
    begin
      TOBP.putValue('GL_TOTALTTCDEV',TOBP.GetValue('GL_TOTALTTCDEV')+Delta);
    end;
  end else
  begin
...
BEGIN
  ...
  if (TOBL.GetValue(prefixe+'_TYPEREF') <> 'CAT') and (TOBL.GetValue(prefixe+'_ARTICLE') = '') then Exit ;
  {$IFDEF BTP}
  if (not IsCentralisateurBesoin(TOBL))
  ...
  begin
    ...
    PUHT:=arrondi(TOBP.GetValue(prefixeD+'_MONTANTHT')*PQ/QteF,V_PGI.OkdecP) ;
  	// reequilibrage des Ht via TTC (PU)
...
BEGIN
  ...
  {$IFDEF BTP}
      {and} (not IsCentralisateurBesoin(TOBL))
      and (TOBL.GetValue(prefixe+'_TYPEARTICLE') <> '$#$')
  {$ENDIF} then
  begin
...
//  if (FactureAvanc) and (TOBL.FieldExists ('BLF_MTSITUATION')) and (prefixe <> 'BOP') then
  if (FactureAvanc) and (TOBL.FieldExists ('BLF_NATUREPIECEG')) and (TOBL.GetValue ('BLF_NATUREPIECEG')<>'')  then
  begin
...
BEGIN
  ...
  RemLigne:=0 ; RemPied:=0 ; Escompte:=0 ;
  // Informations issues de l'article
  if (not isVariante(TOBL)) {$IFDEF BTP} and (not IsCentralisateurBesoin(TOBL)){$ENDIF} then
  begin
...
{$IFDEF BTP}
       (TOBL.GetValue('GL_TYPEARTICLE') = '$#$') or
			 (IsCentralisateurBesoin(TOBL)) or
{$ENDIF}
       (Typl = 'TOT') or
       (Typl = 'COM')) or
       (TOBL.getvalue('GL_ARTICLE') = '' ) then continue;
...
procedure recalculeRGLoc (TOBtiers,TOBPiece,TOBPieceTrait,TOBBasesL,TOBPieceRG,TOBBasesRG,TOBporcs: TOB;DEV:RDevise;FromGenFac:boolean);
var TOBL : TOB;
    I : Integer;
    T_RG :R_CPercent;
begin
  ...
  begin
    ...
    if I > 0 then
      ...
      T_RG.Fin := 0;
    CalculBaseRG (TOBTiers,TOBPiece,TOBPieceTrait,TOBBasesL,TOBPieceRg,TOBBasesRg,TOBL,tobporcs,T_RG,DEV,FromGenFac);
...
{***********A.G.L.***********************************************
Auteur  ...... : Jean-Louis DECOSSE  -- Modifie par LS
Cr le ...... : 19/01/2000
...
Var i,NbDec,NbLig,Indice : integer ;
    ...
    NumOrdre : integer;
    IndiceDepart,IndiceArrive,Indrecalc : integer;
...
BEGIN
  ...
  begin
    ...
    (*
  	for Indice := indiceDepart+1 to TOBPiece.detail.count -1 do
    begin
    	if Indice = IndiceDepart +1 then
      begin
      	Indrecalc := TOBPiece.detail[Indice].GetNumChamp ('GL_RECALCULER');
      end;
    	if TOBPiece.detail[Indice].getValeur(IndRecalc)='X' then
      begin
      	IndiceArrive := Indice;
      end;
    end;
  ...
  BEGIN
    TOBL:=TOBPiece.Detail[i] ; if TOBL=Nil then Break ;
    //
    ...
    //
    if (not SaisieTypeAvanc) and (Pos(TOBL.GetValeur(IndTypeLigne),'ART;ARV;CEN')>0) then
    begin
      // -------------------------------------------------------
      // Deduction des montants ligne avant l'ajout de celle-ci recalcule
      // -------------------------------------------------------
      if (TOBL.GetValeur(IndTypeLigne)<>'CEN') and (not FromZero) and (not FromGenFac) then
      begin
        DeduitLigneModifie (TOBL,TOBpiece,TOBPieceTrait,TOBouvrages,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,DEV, Action);
      end;
      // --------
      if IsOuvrage (TOBL) and (TOBL.GetValeur(IndIndiceNomen)<>0) then
      begin
        ...
        begin
          ...
          SommePieceTrait (TOBpieceTrait,TOBOL);
        end;
        // cumul final
      ...
      begin
        ...
        BEGIN
          ...
          begin
            SommeLignePiedHT(TOBL,TOBPiece) ;
          	SommePieceTrait (TOBpieceTrait,TOBL);
          end;
        END else
        BEGIN
    ...
    begin
      ...
      begin
        MontantCum := Arrondi((TOBL.GetValue('GL_POURCENTAVANC')/100)* TOBL.GetValue('GL_TOTALHTDEV'),DEV.Decimale);
        TOBL.PutValue('MONTANTSIT',MontantCum - TOBL.GetValue('MONTANTFACT'));
      end else
      begin
        if TOBL.fieldExists('MONTANTSIT') then
        begin
          TOBL.PutValue('MONTANTSIT',TOBL.GetValue('GL_TOTPREVDEVAVAN'));
        end;
      end;
//      TOBPiece.PutValue('MONTANTSIT',TOBPiece.GetValue('MONTANTSIT')+TOBL.GetValue('MONTANTSIT'));
    end;
  ...
  //FV1 : Qd on arrive des appel d'offre TOBPIECE n'est pas renseigné
    recalculeRGloc (TOBTiers,TOBPieceTrait,TOBPiece,TOBBasesL,TOBPieceRG,TOBBasesRG,TOBporcs,DEV,FromGenFac);
  end;
// --
  TOBTaxesL.free;
END ;
...
var Indice : integer;
    ...
    Taxe,Valeurcalc,delta,deltaDev,SommeTaxe,SommeTaxeDev : double;
begin
...
begin
  ...
  if (Avancement) then
  BEGIN
  	if DEV.decimale = 0 then DEV.Decimale := Nbdec;
    CalculAvancementPiece (TobPiece,TOBL,DEV,ModifAvanc);
  END;
  if saisieAvanc then
  begin
    ...
    if (TypeFacturation = 'AVA') or (TypeFacturation = 'DAC') then
    begin
      MontantCum := (TOBL.GetValue('GL_POURCENTAVANC')/100)* TOBL.GetValue('GL_TOTALHTDEV');
      TOBL.PutValue('MONTANTSIT',MontantCum - TOBL.GetValue('MONTANTFACT'));
    end else
    begin
      TOBL.PutValue('MONTANTSIT',TOBL.GetValue('GL_TOTPREVAVANC'));
    end;
    TOBPiece.PutValue('MONTANTSIT',TOBPiece.GetValue('MONTANTSIT')+TOBL.GetValue('MONTANTSIT'));
    if FF <> nil then TFFacture(FF).rafraichitPiedAvanc;
  end;
  *)
  //
  ...
  CalculFacture (TOBPiece,TOBpiecetrait,TOBOUvrages,TOBOuvragesP,TOBBases,TOBBAsesL,TOBTiers,TOBArticles,TOBPOrcs,TOBPieceRG,TOBBasesRG,DEV,saisieAvanc,TaModif,ModifAvanc,LaLIgne);
