...
{$IFDEF BTP}
    		 TOBEtude: TOB;
{$ENDIF}
         function RechercheTiers (Tiers : string; NatureTiers, stPayeur : string;
...
const
  ...
  TexteMessage: array[1..58] of string  = (
(*
...
// confidentialité , concept Modif & Creation Prospects
            ...
            BEGIN
              if (Not JaiLeDroitConcept(TConcept(bt110),False)) and (TFFiche(Ecran).fTypeAction<>taCreat) then  //confidentialité , concept Modif Prospect
              begin
              	TFFiche(Ecran).fTypeAction := taConsult;
                SetControlVisible('BDUPLICATION',False) ;
              end;

              if Not JaiLeDroitConcept(TConcept(gcProspectCreat),False) then  //confidentialité , concept Creation Prospect
                BEGIN
                SetControlVisible('BINSERT',False) ;
                SetControlVisible('BDUPLICATION',False) ;
                END ;
...
if StNatureAuxi = 'PRO' then
begin
  if Not JaiLeDroitConcept(TConcept(bt110),False) then TFFiche(Ecran).fTypeAction := taConsult;
end else
	if Not JaiLeDroitConcept(TConcept(bt111),False) then TFFiche(Ecran).fTypeAction := taConsult;

// confidentialité , concept Creation Clients
if TFFiche(Ecran).fTypeAction<>taConsult then
   BEGIN
   if ((StNatureAuxi = 'PRO') and (Not JaiLeDroitConcept(TConcept(gcProspectCreat),False))) or
      ((StNatureAuxi = 'CLI') and (Not JaiLeDroitConcept(TConcept(gcCLICreat),False))) then
      BEGIN
      SetControlVisible('BINSERT',False) ;
      SetControlVisible('BDUPLICATION',False) ;
      END ;
   ...
   if Not JaiLeDroitConcept(TConcept(gcAdminECommerce),False) then
   begin
...
begin
   ...
   if (Toujours = True) or (Length(Tampon)<>LgCode) then
      begin
      if Length(Tampon)>LgCode then Tampon:=Copy(Tampon,1,LgCode)
      else if Length(Tampon)<LgCode then Tampon:=BourreLaDonc(Tampon,fbAux);
...
if CtiRaccrocherFiche then
    SetControlText('INFOSCTI','le correspondant a raccroché');
{$ENDIF}
...
{$IFDEF MODE}
   if (F.FieldName = 'T_TIERS') and (GetField('T_AUXILIAIRE')='') and (GetField('T_TIERS')<>'') then
      ...
      else AffecteTAuxiliaire(GetField('T_TIERS'), True) ;
      end;
{$ELSE}
   if (F.FieldName = 'T_TIERS') and (GetField('T_AUXILIAIRE')='') and (GetField('T_TIERS')<>'') then begin
                  if (stNatureAuxi = 'FOU') then AffecteTAuxiliaire(Trim(GetParamSoc('SO_GCPREFIXEAUXIFOU'))+GetField('T_TIERS'), True)
                   else if (stNatureAuxi = 'CLI') or (stNatureAuxi = 'PRO') then AffecteTAuxiliaire(Trim(GetParamSoc('SO_GCPREFIXEAUXI'))+GetField('T_TIERS'), True)
                   else AffecteTAuxiliaire(GetField('T_TIERS'), True)
                   end;
{$ENDIF}
...
if (F.FieldName = 'T_COLLECTIF')  then
   ...
   if Length(tampon1)>VH^.Cpta[fbGene].Lg then
      tampon1:=Copy(Tampon1,1,VH^.Cpta[fbGene].Lg) ;
   if Length(tampon1)<VH^.Cpta[fbGene].Lg then
   begin
...
{$IFDEF GRC}
if stNatureAuxi <>'CON' then
if (ctxGRC in V_PGI.PGIContexte) and (ds<>nil) and not RTControleModifTiers(TFFiche(Ecran),nil,(DS.State<>dsInsert)) then
begin LastError :=1; exit;end;
{$ENDIF}
...
if (GetField ('T_NATUREAUXI') = 'CLI') or (GetField ('T_NATUREAUXI') = 'PRO')
   or (GetField ('T_NATUREAUXI') = 'CON') then  // GRC
   begin
   ...
   if (ds<>nil) then if client_particulier = TRUE then SetField ('T_PARTICULIER','X') else SetField ('T_PARTICULIER','-');
...
if GetField ('T_NATUREAUXI')='FOU' then SetField('T_FACTUREHT','X');       //JCF
...
begin
  ...
  begin
    ...
    try
      if (Qr.FindField('PY_MEMBRECEE').AsString = 'X')
         and (LePays <> GetParamSoc('SO_GCTIERSPAYS'))
         and (GetField('T_REGIMETVA') <> VH_GC.TypeTVAIntraComm) then
        begin
...
if (stNatureAuxi = 'PRO') and (DS.State <> dsInsert)
   and ( JaiLeDroitConcept(TConcept(gcCLICreat),False) ) then SetControlVisible ('BCLIENT',True);
...
if (stNatureAuxi = 'CLI')or (stNatureAuxi = 'FOU') then begin
if (ctxscot in V_PGI.PGIContexte) and (GetParamsoc('SO_AFLIENDP') = true)
    then begin
      if (Getfield('T_TIERS') <>'') then begin
        codeper:=StrToInt(GetcontrolText('XXCODEPER'));
{$IFDEF DP}
...
{$endif}
      ...
      if (codeper = 0)  and (staction<>TaConsult) and (origine <> 'DP')
              then begin
              DS.Edit;
...
if GetField ('T_NATUREAUXI') = 'CLI' then
    ...
    if GetField('T_MULTIDEVISE') = 'X' then
        begin
        QLoc := OpenSql ('SELECT GP_DATEPIECE FROM PIECE WHERE GP_TIERS="' +
                        GetField('T_TIERS') + '"', True);
        if not QLoc.Eof then
            begin
            ...
            begin
            Ferme (Qloc);
            QLoc := OpenSql('SELECT E_DATECOMPTABLE FROM ECRITURE WHERE E_AUXILIAIRE="' +
                        GetField('T_AUXILIAIRE') + '"', True);
            if not QLoc.Eof then
                begin
                SetControlEnabled ('T_MULTIDEVISE', False);
                end;
...
*)
  if (GetField ('T_PAYEUR')<>'') then
      ...
      Ferme(QLoc);
      end else begin     //mcd 05/03/01
          old_payeur:='';
          SetCOntrolText('TE_PAYEUR','');
      end;
  if (GetField ('T_FACTURE')<>'') then
      begin
      if GetField('T_FACTURE') = GetField('T_AUXILIAIRE')
       then begin
  ...
  //            SetCOntrolText('TE_FACTURE', QLoc.Fields[0].AsString);
    ...
    { BUG : 10368 }
    if (GetField ('T_NATUREAUXI') = 'FOU') then Thedit(GetControl('T_FACTURE')).UpdateLibelle { GPAO }
    else Thedit(GetControl('TE_FACTURE')).UpdateLibelle; { GPAO }
(* end; *)
...
if GetField ('T_NATUREAUXI') = 'FOU' then
   ...
   if ctxMode in V_PGI.PGIContexte then
    begin
        if GetParamsoc('SO_GCCABFOURNIS')=True
          then SetControlEnabled('BCODEBARRE', True)
          else SetControlEnabled('BCODEBARRE', False);
    end else SetControlVisible('BCODEBARRE', False);
   end;

// prospect : nature prospect
//if GetField ('T_NATUREAUXI') = 'PRO' then
if (ctxGRC in V_PGI.PGIContexte) and
   ( ( GetField ('T_NATUREAUXI') = 'PRO') or ( GetField ('T_NATUREAUXI') = 'CLI') ) then
begin
...
else SetControlEnabled('T_TIERS', FALSE);
If GetField ('T_NATUREAUXI') = 'FOU'
     then begin
...
SetControlvisible ('BTCONTRAT',true);
if (GetField ('T_NATUREAUXI') = 'CLI') then
   Begin
      ...
      If QContrat.Eof then
      	SetControlEnabled  ('BTCONTRAT',False)
      else
      	SetControlEnabled ('BTCONTRAT',true);
      Ferme(QContrat);
   End;
...
if (GetField ('T_NATUREAUXI') = 'CLI') then
   Begin
{$IFNDEF LINE}
      SetControlVisible ('BTAPPELS',True);
{$ENDIF}
   End;
...
Begin
  ...
  begin
    if GetParamSoc('SO_BTAFFAIRERECHLIEU') then
    	AGLLanceFiche('BTP', 'BTMULAPPELSINT','AFF_AFFAIRE0=W','','AFF_TIERS=' + GetField('T_TIERS')+ ';NUMTEL='+AppelantCTI+ ';APPELANT='+stContact + ';ETAT=ENC;STATUT=APP')
    else
    	AglLanceFiche('BTP','BTMULAPPELS','AFF_AFFAIRE0=W','AFF_TIERS=' + GetField('T_TIERS'),'AFF_TIERS=' + GetField('T_TIERS') + ';NUMTEL='+AppelantCTI+ ';APPELANT='+stContact+';ETAT=ENC;STATUT=APP'); // Affaires
  end else
  begin
...
begin
   ...
   AglLanceFiche('BTP','BTAFFAIRE_MUL','AFF_AFFAIRE0=I','','AFF_TIERS=' + GetField('T_TIERS') + ';ETAT=ENC;STATUT=INT');   // Affaires
...
if (F is TFFiche) then OM:=TFFiche(F).OM else exit;
if (OM is TOM_TIERS)
    then TOM_TIERS(OM).SetEtatBoutonCti
    else exit;
end;
