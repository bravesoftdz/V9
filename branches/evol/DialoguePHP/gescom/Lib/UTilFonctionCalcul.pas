...
Type R_FonctCal = RECORD
  ...
  function  SupprimeCaracteresSpeciaux(St: string ; Accent,AltGr,Operateur : boolean): string ;
...
For ind:=1 to Length(St) do
  begin
  if (Accent and (St[ind] in Car1)) then Delete(St,ind,1) ;
  if (AltGr and (St[ind] in Car2)) then Delete(St,ind,1) ;
  if (Operateur and (St[ind] in Car3)) then Delete(St,ind,1) ;
  end;
Result:=St;            
end;
