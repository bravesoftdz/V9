...
begin
  Inherited ;
GA.VidePile(false) ; GA.AllSelected := False;
TobA.ClearDetail ;
F:=TFMul(Ecran);
ListeChampGrid := '';
...
{$ENDIF}
ListeChampGrid := ListeChampGrid + 'QTEPHY;DATEPIECE';
GA.Titres[0] := ListeChampGrid;
ColPhy := GA.ColCount-2;
ColDat := GA.ColCount-1;
GA.Cells[ColDat, 0] := TraduireMemoire('Dernier mouvement');
GA.ColTypes[ColDat] := 'D';   
if GetControlText('DEPOT')='' then GA.Cells[ColPhy, 0] := TraduireMemoire('Total stock')
else GA.Cells[ColPhy, 0] := TraduireMemoire('Stock');
GA.ColWidths[ColDat]:=130; GA.ColWidths[ColPhy]:=130;
GA.ColAligns[ColDat] := taCenter; GA.ColAligns[ColPhy] := taRightJustify;
...
for iInd := 0 to TobA.Detail.Count -1 do
    ...
    if not QQ.Eof then TobAD.PutValue('QTEPHY',QQ.FindField('QTEPHY').AsFloat);
    Ferme(QQ);
    MoveCur(False) ;
    {$IFDEF EAGLCLIENT}
