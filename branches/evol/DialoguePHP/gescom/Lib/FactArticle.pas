...
function GetArticleRecherche(GS: THGrid; TitreSel, NaturePieceG, DomainePiece, SelectFourniss: string; stWhere: string = ''): boolean;
begin
  if GetParamSocSecur('SO_GCRECHARTAV', False) then Result := GetArticleMul(GS, TitreSel, NaturePieceG, DomainePiece, SelectFourniss, stWhere)
  else Result := GetArticleLookUp(GS, TitreSel, NaturePieceG, DomainePiece, SelectFourniss, stWhere);
end;
...
begin
  if TOBA = nil then Exit;
  if TOBL = nil then Exit;
  Q := OpenSQL('SELECT * FROM CATALOGU WHERE GCA_TIERS="' + TOBL.GetValue('GL_TIERS') + '" AND GCA_ARTICLE="' + TOBL.GetValue('GL_ARTICLE') + '"', True,-1, '', True);
  if not Q.EOF then
  begin
