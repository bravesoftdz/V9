...
type
  ...
  private
    ...
    FColonneSaisie, FColonneEcart, FColonneSaisi, FColonneOrdi : integer;
...
implementation
uses UTOFListeInv, InvUtil, DimUtil, AGLInitGC, UtilDispGC;
...
{$IFDEF BTP}
	if (NomCol='GIL_LOTINTERNE') or (NomCol='GIL_SERIEINTERNE') then
  begin
...
begin
GetLigneTOB(G_Inv.Row);
G_Inv.ElipsisButton := (TOBCurrentLigne <> nil) and IsCurrentLigneCumul;
...
if TOBCurrentLigne <> nil then
  ...
  if IsCurrentLigneCumul and
     (TOBLots.Somme('GLI_INVENTAIRE', ['GLI_ARTICLE'], [TOBCurrentLigne.GetValue('GIL_ARTICLE')], false) <> vale) then
    begin
    ...
    if (vale <> 0) or (TOBCurrentLigne.GetValue('GIL_QTEPHOTOINV') = 0) then  // Mettre automatiquement la coche 'Saisi'
...
begin
if G_Inv.Col = FColonneSaisie then UpdateElipsis
                              else G_Inv.Col := FColonneSaisie; // Forcer la colonne active à la colonne de saisie
end;

procedure TFSaisieInv.G_InvCellExit(Sender: TObject; var ACol, ARow: Integer; var Cancel: Boolean);
begin
if csDestroying in ComponentState then exit;
if (G_Inv.Col = FColonneSaisie) and (ARow <= TOBLignes.Detail.Count)
  then Cancel := not TOBizeLaSaisie(ARow); // Si la TOBization renvoi faux, refuser le changement de cellule
end;
