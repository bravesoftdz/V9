...
uses  StdCtrls,Controls,Classes,forms,sysutils,ComCtrls,
     ...
     {$IFDEF BTP}
      UMetreArticle, olectnrs, UtilXlsBTP, FileCtrl,
     {$ENDIF}
{$IFDEF EAGLCLIENT}
      MaineAGL, eFichList,
{$ELSE}
...
{$IFDEF BTP}
//        property TheMetreArticle: TmetreArt read MetreArticle write MetreArticle;
{$ENDIF}        // ----
     end ;
...
tpageControl(ecran.findComponent('PAGE')).Onchange := OnChangeOnglet;
Ok_MetreActif := false;
{$IFDEF EAGL}
{$ELSE}
(*
...
TabMetre.TabVisible := true;
{$ENDIF}
{$IFDEF GPAOLIGHT}
if VH_GC.OASeria then
    TobGP := TOB.create('WNOMETET',nil,-1);
{$ENDIF }
...
//begin
  ...
  begin
    if not MetreArticle.validate(getfield('GNE_NOMENCLATURE')) then
       if (GetActiveTabSheet('PAGE').Name = 'P_METRES') then
...
begin
  ...
  // Controle si l'ouvrage est décliné et si oui on affiche le métré
  if LastOuvrage <> '' then
  begin
    if MetreArticle <> nil then
    begin
      MetreArticle.validate(LastOuvrage);
    end;
...
TobLoc := TOBDetailEntete.FindFirst (['GNE_NOMENCLATURE'],critrech,true);
// FV => Gestion des métrés
if MetreArticle <> nil then MetreArticle.supprime(getfield('GNE_NOMENCLATURE'));
...
// FV => Gestion des métrés
begin
  if MetreArticle <> nil then MetreArticle.free;
  if OleExcel <> nil then OleExcel.Free;
end;
{$ENDIF}
TOBE.free;
TOBD.free;
...
{$ENDIF}
   ...
   if MetreArticle <> nil then
   begin
     MetreArticle.CopieMetre(OuvrageRef, TobResult.GetValue('CODENOMEN'));
     MetreArticle.CopieVariable(OuvrageRef, TobResult.GetValue('CODENOMEN'));
   end;
...
{$IFDEF BTP}
   if (CodeRes <> '') and (oleExcel <> nil) then
      Begin
        if MetreArticle <> nil then FreeAndNil (MetreArticle);
...
{$IFDEF BTP}
procedure TOM_NOMENENT.OnChangeOnglet(Sender: TObject);
begin
    ...
    begin
      if Ok_MetreActif = true then
        Begin
         if MetreARticle <> nil then
         begin
//           if OleExcel.Modified then if not (DS.State in [dsInsert, dsEdit]) then DS.edit;
           if (OleExcel <> nil) and (OleExcel.Modified) then
           begin
              SetField ('GNE_QTEDUDETAIL',THDBSpinEdit(GetControl('GNE_QTEDUDETAIL')).Value+1);
              SetField ('GNE_QTEDUDETAIL',THDBSpinEdit(GetControl('GNE_QTEDUDETAIL')).Value-1);
           end;
...
procedure TOM_NOMENENT.OpenOleContainer;
var Ok_Excel: boolean;
    dest    : string;
begin
  ...
  MetreArticle.Article := Getfield('GNE_NOMENCLATURE');

  If Ok_Excel = True then MetreArticle.ModifMetres;

end;

procedure TOM_NOMENENT.CloseOleContainer;
begin
  //MetreArticle.Annule;
  //metreArticle.free;
//  if (GetActiveTabSheet('PAGE').Name = 'P_METRES') then
