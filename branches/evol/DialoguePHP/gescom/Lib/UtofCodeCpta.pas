...
Type
     ...
     private
        LesColonnes : string ;
        GS : THGRID ;
        ColCpteVte,ColCpteAch,ColCpteStock,ColCpteVarStk,ColTiers : integer ;
        TobToDelete : Tob ;
...
for i:=0 to GS.ColCount-1 do
   ...
   begin
   	GS.ColFormats[i]:='CB=GCCOMPTAARTICLE';
		GS.ColWidths [I] := 120;
   end else if Nam='GCP_COMPTATIERS' then
   begin
   	GS.ColFormats[i]:='CB=GCCOMPTATIERS';
    ColTiers := i;
   end else if Nam='GCP_COMPTAAFFAIRE' then GS.ColFormats[i]:='CB=AFCOMPTAAFFAIRE'
   else if Nam='GCP_ETABLISSEMENT' then GS.ColFormats[i]:='CB=TTETABLISSEMENT'
   else if Nam='GCP_REGIMETAXE' then GS.ColFormats[i]:='CB=TTREGIMETVA'
   else if Nam='GCP_FAMILLETAXE' then GS.ColFormats[i]:='CB=GCFAMILLETAXE1'
