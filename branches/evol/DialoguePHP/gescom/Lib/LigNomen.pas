...
{$IFDEF BTP}
function Entree_LigneOuv (XP : Tform; TOBaffaire,TobNomen, TobArticles,TOBLigPiece,TOBRepart : TOB ; Emploi : boolean ; Niveau,OrdreCompo : integer;DEV:Rdevise; UniteVente:String;var Valeurs:T_valeurs;TAction:TActionFiche;Enht:boolean;TypeARp:boolean;ModeSaisieAch : boolean; CotraitOk : boolean): boolean;
{$ENDIF}
...
//  evenements sur le treeview
    ...
    procedure G_NLIGBeforeFlip(Sender: TObject; ARow: Integer;
      var Cancel: Boolean);
    ...
    procedure G_NLIGMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
  private
    ...
    Cell_Text : string;
    TOBLig, TobArticles : TOB;
    TobSauv : TOB;
    iTableLigne : integer;
    InfoNiv,InfoOrdre : integer;
...
{$ENDIF}
    ...
    prefixe : string;
    DEV:RDevise;
    TOBLigPiece,TOBPiece,TOBaffaire : TOB;
    QteDUdetail : double;
    GestionHt : boolean;
...
//
{$IFDEF BTP}
    TotalOuv, ValPourcent : T_Valeurs;
{$ENDIF}
    TOBImp : TOB;
    // ----------
//  Gestion du grid
...
{$IFDEF BTP}
uses factOuvrage,UtilMetres,UMetreArticle,UImportLigneNomen;
{$ENDIF}
...
for Indice := 1 to G_NLIG.RowCount - 1 do
    ...
    QteDudetail := TOBL.getValue('BLO_PRIXPOURQTE'); if QteDudetail = 0 then QteduDetail := 1;
//    QteDudetail := TOBL.getValue('BLO_QTEDUDETAIL'); if QteDudetail = 0 then QteduDetail := 1;
    TotalOuv[0] := TotalOuv [0] + Arrondi(TOBL.getvalue('BLO_DPA') * (Qte/QteDudetail),V_PGI.okdecP);
    TotalOuv[1] := TotalOuv [1] + Arrondi(TOBL.getvalue('BLO_DPR') * (Qte/QteDudetail),V_PGI.okdecP) ;
    TotalOuv[2] := TotalOuv [2] + Arrondi(TOBL.getvalue('BLO_PUHTDEV') * (Qte/Qtedudetail),V_PGI.okdecP) ;
    TotalOuv[3] := TotalOuv [3] + Arrondi(TOBL.getvalue('BLO_PUTTCDEV') * (Qte/QteDudetail),V_PGI.okdecP);
    TotalOuv[6] := TotalOuv [6] + Arrondi(TOBL.getvalue('BLO_PMAP') * (Qte / QTEduDetail),V_PGI.OkdecP);
    TotalOuv[7] := TotalOuv [7] + Arrondi(TOBL.getvalue('BLO_PMRP')* (Qte/QTeduDetail),V_PGI.okdecp) ;
//    if TOBL.getValue('BNP_TYPERESSOURCE')='SAL' then
    ...
    begin
      TotalOuv[9] := TotalOuv [9] + TOBL.getvalue('BLO_TPSUNITAIRE')* (Qte/QTeduDetail) ;
    end;
    TotalOuv[10] := TotalOuv[10] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTPAFG'))),4);
    TotalOuv[11] := TotalOuv[11] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTPAFR'))),4);
    TotalOuv[12] := TotalOuv[12] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTPAFC'))),4);
    TotalOuv[13] := TotalOuv[13] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTFG'))),4);
    TotalOuv[14] := TotalOuv[14] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTFR'))),4);
    TotalOuv[15] := TotalOuv[15] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTFC'))),4);
    TotalOuv[16] := TotalOuv[16] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTPA'))),V_PGI.okdecP);
    TotalOuv[17] := TotalOuv[17] + Arrondi((((QteDuDetail) * TOBL.GetValue ('BLO_MONTANTPR'))),V_PGI.okdecP);


    if  not ArticleOKInPOUR (TOBL.GetValue('BLO_TYPEARTICLE'),ArticleOk) then continue;
    MontPourcent[0] := MontPourcent [0] + Arrondi(TOBL.getvalue('BLO_DPA') * (Qte/QteDudetail),V_PGI.okdecp);
    MontPourcent[1] := MontPourcent [1] + Arrondi(TOBL.getvalue('BLO_DPR') * (Qte/QteDudetail),V_PGI.okdecp) ;
    MontPourcent[2] := MontPourcent [2] + Arrondi(TOBL.getvalue('BLO_PUHTDEV') * (Qte/Qtedudetail) ,V_PGI.okdecp);
    MontPourcent[3] := MontPourcent [3] + Arrondi(TOBL.getvalue('BLO_PUTTCDEV') * (Qte/QteDudetail),V_PGI.okdecp);
    MontPourcent[6] := MontPourcent [6] + Arrondi(TOBL.getvalue('BLO_PMAP') * (Qte / QTEduDetail),V_PGI.okdecp);
    MontPourcent[7] := MontPourcent [7] + Arrondi(TOBL.getvalue('BLO_PMRP')* (Qte/QTeduDetail),V_PGI.okdecp);
    end;
...
if IndPou >= 0 then
   ...
   valPourcent[0] := MontPourcent[0] * (Qte/QteDudetail);
   valPourcent[1] := MontPourcent[1] * (Qte/QteDudetail) ;
   valPourcent[2] := arrondi (MontPourcent[2] * (Qte/Qtedudetail),V_PGI.OkDecP ) ;
   valPourcent[3] := MontPourcent[3] * (Qte/QteDudetail);
   valPourcent[6] := MontPourcent[6] * (Qte/QteduDetail);
   valPourcent[7] := MontPourcent[7] * (Qte/QTeduDetail) ;
...
begin
  ...
  TOBOUv := TOB(G_NLIG.Objects[SGN_Numlig, ARow]); if TOBOuv = nil then exit;
...
{$IFDEF BTP}
  ...
  begin
    ...
    begin
      if TheMetreShare.MetreAttached (TOBOUV) then
      begin
...
if (SGN_PVHT <> -1) then
begin
	G_NLIG.colformats [SGN_PVHT] := '# ### ##0.'+ formatDec ('0',V_PGI.okdecP);
  if TOBLigpiece.getValue('GL_BLOQUETARIF')='X' then
  begin
  	G_NLIG.ColWidths  [SGN_PVHT] := 0;
		G_NLIG.ColLengths [SGN_PVHT] := -1;
  end;
end;
...
if SGN_LIBFOU > 0 then
begin
  G_NLIG.ColEditables[SGN_LIBFOU] := False;
end;
...
if SGN_PAHT <> -1 then
   begin
   if Action <> TaConsult then G_NLIG.ColLengths [SGN_PAHT] := -1;
   end;
*)
if SGN_PVHTP <> -1 then
   begin
...
if Ouvrage then
   ...
   height := screen.Desktopheight - 80;
   end;
...
if TobVide(TobLig) then
    ...
    end else TOBLig.PutGridDetail(G_NLIG, False, False, LesColNLIG);
// Modif BTP
...
//   if (TypeArp) and (MetreDoc) then Appel_Xl.Visible := true;
   ...
   end else Caption := TraduireMemoire ('Décomposition de la nomenclature ');
if action <> taconsult then
begin
	BintegreExcel.Visible := true;
end;
...
{$IFDEF LINE}
 N4.visible := false;
 POPG_NLIG_Copier.visible := false;
 POPG_NLIG_Coller.visible := false;
 Rappeler1.visible := false;
{$ENDIF}
SetControlsEnabled ;
G_NLIGEnter (self);
end;
    ...
    if NomCol<>'' then
        ...
        if ichamp>=0 then
            ...
            if (Pos(prefixe+'_',NomCol) > 0 ) and
               (Pos('X',V_PGI.Dechamps[iTableLigne,ichamp].Control)>0) then ColsInter[icol]:=True
            ...
            END  else
            begin
              if NomCol = 'MONTANTHTDEV' Then SGN_MONTANT:= icol else
            end;
        END ;
    Inc(icol) ;
...
//=============================================================================
procedure TFLigNomen.G_NLIGCellExit(Sender: TObject; var ACol,
                                    ARow: Integer; var Cancel: Boolean);
begin
...
if (TOBL.GetValue('BLO_TYPEARTICLE')= 'ARP') and (TOBL.detail.count >0) then
   ...
   AppliqueCoefDomaineActOuv (TOBL.detail[0]);
...
//Copie du fichier XLS dans le répertoire document
    if (TheMetreShare <> nil) and (TobTemp.Detail.Count = 0) then
      TheMetreShare.DocMetreBiblio (TOBTemp);
{$ENDIF}
...
{$IFDEF BTP}
    ...
    if Val <> 0 then
       begin
        ...
        TOBtemp.PutLigneGrid (G_NLIG, G_NLIG.row,False, False, LesColNLIG);
        if G_NLIG.col = SGN_Qte then
          begin
...
begin
Result := True;
if G_NLIG.Cells[SGN_Comp, ARow]='' then exit;
{$IFNDEF BTP}
...
{$IFDEF BTP}
        ...
        END else if (Acol = SGN_UV) then StC:=uppercase(Trim(St));
{$ENDIF};
...
var i_ind1 : integer ;
{$IFDEF BTP}
    valeurpv,valeuranc,ValeurPr,Qte : double;
...
for i_ind1 := 0 to G_NLIG.ColCount - 1 do
    ...
    if i_ind1 = SGN_Comp  then
        // Modif BTP
        TOBL.PutValue(prefixe+'_CODEARTICLE', G_NLIG.Cells[i_ind1, Ou]);
    if i_ind1 = SGN_Lib   then
        // Modif BTP
        TOBL.PutValue(prefixe+'_LIBELLE', G_NLIG.Cells[i_ind1, Ou]);
    if (i_ind1 = SGN_Qte) and (not Ouvrage)   then
        TOBL.PutValue('GLN_QTE', G_NLIG.Cells[i_ind1, Ou]);
    if (i_ind1 = SGN_Qte) and (Ouvrage)   then
        TOBL.PutValue('BLO_QTEFACT', G_NLIG.Cells[i_ind1, Ou]);
{$IFDEF BTP}
...
(*
     TOBOuv.PutValue(prefixe+'_DPR',TobArticle.GetValue('GA_DPA') * TOBPiece.GetValue ('COEFFGFORCE') );
     TOBOuv.PutValue(prefixe+'_PMRP',TobArticle.GetValue('GA_DPR'));
     TOBOuv.PutValue(prefixe+'_DPR',TobArticle.GetValue('GA_DPR'));
     TOBOuv.PutValue(prefixe+'_PMRP',TobArticle.GetValue('GA_PMRP'));
*)
...
if CodeNomen <> '' then
   ...
   Select := 'SELECT GNL_SOUSNOMEN,GNL_ARTICLE,GNL_CODEARTICLE,GNL_LIBELLE,GNL_QTE '+
       'FROM NOMENLIG WHERE GNL_NOMENCLATURE = "' + CodeNomen + '"';
...
if Ouvrage then
   ...
   OkTrait := Entree_LigneOuv(XP,nil,TobLND,TobArticles,TOBLigPiece, TOBREpart, True , InfoNiv+1,TobLND.GetValue(prefixe+'_NUMORDRE'),DEV,TOBLND.GetValue(prefixe+'_QUALIFQTEVTE'),valloc,Lastaction,GestionHt,false,ModeSaisieAch,false);
...
if not EnregistreOK then BEGIN Modalresult := 0 ; OnValidation := false; exit; end;
CompleteTobArticles(TobLig);
Indice := 0;
repeat
	if TOBlig.Detail[Indice].getValue('BLO_CODEARTICLE')='' then TOBlig.Detail[Indice].free
  																												else inc(Indice);
...
begin
  MontantpaU.Value := Total [0]/QTEDETAIL.Value;
  MontantprU.value := Total [1]/QTEDETAIL.Value;
  MontantHTU.value := Total [2]/QTEDETAIL.Value;
  MontantTTCU.value := Total [3]/QTEDETAIL.Value;
  MontantFGU.value := (Total[1]-Total[0])/QTEDETAIL.Value;
  MontantMargU.value := (devisetopivot (Total[2],DEV.Taux,DEV.Quotite ) -Total[1])/QTEDETAIL.Value;
  TPSUNITAIRE.value := Total[9]/QTEDETAIL.Value;
end;
...
begin
  AppliqueCoefMargDetail (TOBL,TOBL.GetValue('BLO_COEFMARG'));
end;
//
...
{$IFDEF BTP}
   ...
   Result := TheMetreShare.ModifieMetre (TOBO,self);
   if result <> 0 then
   begin
