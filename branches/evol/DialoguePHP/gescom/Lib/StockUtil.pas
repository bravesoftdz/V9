...
{$ENDIF}
     SysUtils, Dialogs, Utiltarif, FactUtil, FactArticle, FactTOB, EntGC, UTofPrixRevient, ParamSoc ;

Type T_RatioStock = (trsDocument,trsStock,trsVente) ;

Function MajQteStock(TOBL, TOBArticles, TOBNomen : TOB; QtePlus, QteMoins: String; Plus: boolean; RatioVA: Double; DepotEmetteur : String='') : Boolean;
...
Var VenteAchat : String ;
    RatioVA,FUA,FUS,FUV,CoefUAUS : Double ;
BEGIN
RatioVA:=1.0 ;
FUA := 1.0; FUV := 1.0 ; FUS := 1.0; CoefUaUs := 0;
...
Case trs of
   trsStock : BEGIN
              if VenteAchat='ACH' then
                 ...
                 if TOBN<>Nil then
                 begin
                 	RatioVA:=CalcRatioVA(TOBN.GetValue('GLN_QUALIFQTEACH'),TOBN.GetValue('GLN_QUALIFQTESTO'))
                 end else
                 begin
                 	if CoefuaUs <> 0 then
                  begin
                 ...
                 END else
                 BEGIN
                 if TOBN<>Nil then RatioVA:=CalcRatioVA(TOBN.GetValue('GLN_QUALIFQTEVTE'),TOBN.GetValue('GLN_QUALIFQTESTO'))
...
trsDocument : BEGIN
              if VenteAchat='ACH' then
                 BEGIN
                   if TOBN<>Nil then
                   begin
                    RatioVA:=CalcRatioVA(TOBN.GetValue('GLN_QUALIFQTEVTE'),TOBN.GetValue('GLN_QUALIFQTEACH'))
                   end else
                   begin
                   	if CoefuaUs <> 0 then
                    begin
...
Var RefUnique,Depot : String ;
    ...
    TOBDepot,TOBPlat,TOBA,TOBPD,TOBDepotEmetteur : TOB ;
    i                : integer ;
    RatN             : Double ;
...
if ((TOBL.GetValue('GL_TENUESTOCK')='-') and (TOBNomen=Nil)) then Exit ;
// infos lignes
NaturePieceG:=TOBL.GetValue('GL_NATUREPIECEG') ;
Qte:=TOBL.GetValue('GL_QTESTOCK') ; if ((Qte=0) and (Naturepieceg<>'INV')) then Exit ;
Depot:=TOBL.GetValue('GL_DEPOT') ;
...
if TOBNomen=Nil then
   ...
   begin
     // Ajout LS pour mise à jour de la valo du stock
     UpdateValoStock(TOBDepot,TOBL);
     if Qte=0 then exit;
   end;
   // ----
   ...
   if (CalcRupt<>'AUC') and (CalcRupt<>'') and (not ForceRupt) and (QteDispo<0) then
      BEGIN
   		UpdateQteStock(TOBDepot,Qte,RatioVA,QtePlus,QteMoins,(not Plus)) ; // On le rejoue en marche arriere
      Result:=False;
      END;
   END else
   BEGIN
   ...
   NomenAPlat(TOBNomen,TOBPlat,1) ;
   for i:=0 to TOBPlat.Detail.Count-1 do
       BEGIN
       TOBPD:=TOBPlat.Detail[i] ;
       RefUnique:=TOBPD.GetValue('GLN_ARTICLE') ; if RefUnique='' then Continue ;
       if TOBPD.GetValue('GLN_TENUESTOCK') <> 'X' then Continue ;
       TOBA:=TOBArticles.FindFirst(['GA_ARTICLE'],[RefUnique],False) ; TOBA.PutValue('UTILISE','X') ;
       RatN:=GetRatio(TOBL,TOBPD,trsStock) ; QteN:=TOBPD.GetValue('GLN_QTE') ;
       // Calculs
       TOBDepot:=TOBA.FindFirst(['GQ_DEPOT'],[Depot],False) ;
       if TOBDepot=Nil then
          BEGIN
          ...
          END ;
       UpdateQteStock(TOBDepot,Qte*QteN,RatN,QtePlus,QteMoins,Plus) ;
       QteDispo := CalcDispo(TOBL, TobDepot);
       if (CalcRupt<>'AUC') and (CalcRupt<>'') and (not ForceRupt) and (QteDispo<0) then
          BEGIN
       		UpdateQteStock(TOBDepot,Qte*QteN,RatN,QtePlus,QteMoins,(not Plus)) ;
          Result:=False;  Break;
          END;
       END ;
...
var
  iCompteur: integer ;
  TypeChamp, NomChamp, sChamps, sValues  : String ;
begin
  ...
  With TOBA do
    ...
    if NumTable > 0 then
      begin
      for iCompteur:=1 to High(V_PGI.DeChamps[NumTable]) do
        begin
        NomChamp  := V_PGI.DEChamps[NumTable,iCompteur].Nom ;
        TypeChamp := V_PGI.DEChamps[NumTable,iCompteur].tipe ;

        if ( NomChamp = 'US_UTILISATEUR' ) then
        begin
          if sChamps='' then AddS(sChamps,NomChamp)
          else AddS(sChamps,', '+NomChamp) ;
          if sValues='' then AddS(sValues,GetValue( NomChamp ) )
          else AddS(sValues,', "'+GetValue( NomChamp ) + '"' ) ;
        end
        else if Not (TypeChamp='BLOB') AND Not (TypeChamp='DATA') then
          begin
          if sChamps='' then AddS(sChamps,V_PGI.DEChamps[NumTable,iCompteur].Nom)
          else AddS(sChamps,', '+V_PGI.DEChamps[NumTable,iCompteur].Nom) ;
          if sValues='' then AddS(sValues,ValeurDuChamp(TOBA,TypeChamp,NomChamp,False,True))
          else AddS(sValues,', '+ValeurDuChamp(TOBA,TypeChamp,NomChamp,True,True)) ;
          end ;
        end ;
...
var
  ...
  TypeChamp,sChamps,NomChamp : String ;
begin
  ...
  With TOBA do
    ...
    if NumTable > 0 then
      ...
      for iCompteur:=1 to High(V_PGI.DeChamps[NumTable]) do
        begin
        NomChamp  := V_PGI.DEChamps[NumTable,iCompteur].Nom ;
        if IsFieldModified(NomChamp) then
          begin
            if (ChampsAutorise(NomChamp,LesChampsValo)) then
            begin
              Inc(nChamp) ;
              TypeChamp := V_PGI.DEChamps[NumTable,iCompteur].tipe ;
              if (TypeChamp <> 'BLOB') then
              begin
                if sChamps='' then sChamps:=V_PGI.DEChamps[NumTable,iCompteur].Nom + '='
                  else AddS(sChamps,', '+V_PGI.DEChamps[NumTable,iCompteur].Nom+'=') ;

                  AddS(sChamps,ValeurDuChamp(TOBA,TypeChamp,NomChamp)) ;
              end;
            end;
          end ;
        end ;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if Not UpdateMaTOB(TOBDisp,LesChamps) then begin Result := False; Break; end;
      end;
      ...
      begin
        ...
        if Not InsertOk then
        begin
          if Not UpdateMaTOB(TOBDispLot,LesChamps) then begin Result := False; Break; end;
        end;
      end;
    end;
  end;
