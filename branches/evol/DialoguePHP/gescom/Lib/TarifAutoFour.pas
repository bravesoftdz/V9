...
{$ENDIF} // EAGLCLIENT
  HDimension, Menus, Buttons, Hqry, ImgList, HImgList;
...
for iChamp := 0 to ListeChampArtC.Count - 1 do
    begin
    TobPar := TobParFouLig.FindFirst (['GFL_CHAMP'],
                                      [ListeChampArtC.Strings[iChamp]], False);
    AjouteLSChamp (ListeChampArtC.Strings [iChamp], ListTypeChampArtC [ichamp]);
//    if (ListeChampArtC.Strings[iChamp] <> 'GA2_ARTICLE') and (TobPar = Nil) then
        begin
        InsereListeChamp (ListeChampArtC.Strings [iChamp],
                          ListeLibChampArtC.Strings [iChamp]);
        end;
    end;
...
for iChamp := 0 to ListeChampCat.Count - 1 do
    begin
    TobPar := TobParFouLig.FindFirst (['GFL_CHAMP'],
                                      [ListeChampCat.Strings[iChamp]], False);

    AjouteLSChamp (ListeChampCat.Strings [iChamp],ListTypeChampCat [ichamp]);

    if (ListeChampCat.Strings[iChamp] <> 'GCA_TIERS') and
       (ListeChampCat.Strings[iChamp] <> 'GCA_DATESUP') and
       (TobPar = Nil) then
       begin
       InsereListeChamp (ListeChampCat.Strings [iChamp],
                         ListeLibChampCat.Strings [iChamp]);
       end;
    end;
...
for iChamp := 0 to ListeChampArt.Count - 1 do
    begin
    TobPar := TobParFouLig.FindFirst (['GFL_CHAMP'],
                                      [ListeChampArt.Strings[iChamp]], False);
    ...
    if (ListeChampArt.Strings[iChamp] <> 'GA_ARTICLE') and (TobPar = Nil) then
        begin
        InsereListeChamp (ListeChampArt.Strings [iChamp],
                          ListeLibChampArt.Strings [iChamp]);
        end;
    end;
...
BEGIN
SAF_Champ := -1; ;SAF_Longueur := -1;
SAF_Offset := -1; SAF_Libelle := -1;
SAF_SEPDecimale    := -1;
SAF_NbrDecimale := -1;
...
begin
if TOBPARFOU.GetValue('GRF_TIERS') = '' then exit; // evite de reinitialiser
...
if pos ('GCA', stChamp) > 0 then
    ...
    end else if Pos ('GA2',StChamp) > 0 then
    BEGIN
...
while (iInd < ListeChamp.Count) and (ListeChamp.Strings [iInd] <> stChamp) do
    iInd := iInd + 1;
if iInd < ListeChamp.Count then Result := ListeLib.Strings [iInd]
else Result := 'Champ inconnu';
end;
...
TOBListeChamps.free; 	TOBListeChamps := nil;
