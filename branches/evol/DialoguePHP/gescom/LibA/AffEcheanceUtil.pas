...
function DateDebutPeriode(AcsPeriodicite:string; AcdDateDebut:TDateTime;AciInterval : integer;MethodeCalcul  : string):TDateTime;
...
Procedure UtilCalculEcheances(MtGlobal:Double; Profil, CodeAff, TypeCalcul, zRepAct, ztier, ztyp_fac : string; TypeModif:T_TypeModifAff; zinterval: integer;
...
if (TobAff.GetValue('AFF_GENERAUTO')='CON') and (TobAff.GetValue('AFF_METHECHEANCE')='CIV') then
    DateDebGen:= DateDebutPeriode ( TobAff.GetValue('AFF_PERIODICITE'), DateDebGen, TobAff.GetValue('AFF_INTERVALGENER'),methodeCalcul);

bMtPeriodique := True; // principe : on part du montant périodique
TypeModif := [tmaDate];
  //mcd 03/07/03 ajout profilgener
UtilCalculEcheances(TobAff.GetValue('AFF_TOTALHTGLODEV'),TobAff.GetValue('AFF_PROFILGENER'),CodeAff, TobAff.GetValue('AFF_PERIODICITE'),
   ...
   TobAff.GetValue('AFF_DATERESIL'),DEV, SaisieContre, bMtPeriodique, bPourcentage, TobAff.GetValue('AFF_MULTIECHE'),ModeMontantDoc,MethodeCalcul,DateDebutfac,TobAff.GetValue('AFF_TERMEECHEANCE'));
...
BEGIN
  ...
  TobDet.PutValue('AFA_MONTANTECHE',MtPivot);
END;


function CalcDaysInPeriod (DateDeb : TdateTime; NbMois : integer) : double;
...
Var QQ : Tquery;
    ...
    bDejaFac:boolean;
    DateEcheance, DateFinEcheance, DateEcheanceSuivante, DerniereDateFac, DateFinTraite:TDateTime;
...
Begin
  ...
  if (bDejaFac) then
  begin
  	DateDebut := DateDebutPeriode (TypeCalcul, DateDebutFac, zinterval,MethodeCalcul)  ;
  end;
  DateEcheance := DateDebut;
  DateFinEcheance := PlusDate( DateEcheance, zinterval, TypeCalcul );
  ...
  while (DateDebCal <> iDate1900) And (DateDebCal > DateEcheance) And (TypeCalcul <>'P') do
  begin
    Inc(Cpt);
    DateEcheance := PlusDate( DateDebut, Cpt * zinterval, TypeCalcul );
    DateEcheance := RecaleEcheanceFinMois( DateEcheance, DateDebut );
    DateFinEcheance := PlusDate( DateEcheance, zinterval, TypeCalcul );
    DateFinEcheance := PlusDate( DateFinEcheance, -1, 'J' );
  end;
  ...
  begin
    DateEcheance := DateDebut;
     DateFinEcheance := PlusDate( DateEcheance, zinterval, TypeCalcul );
     DateFinEcheance := PlusDate( DateFinEcheance, -1, 'J' );
    if (DateEcheance <= DateFinTraite) and (DateEcheance >=DateDebCal) then
       GetTOBEcheance(TobEch, iMax,Profil, '-', CodeAff, zRepAct,  ztier, ztyp_fac, zmont, DateEcheance, DateFinEcheance , Dev,
                      SaisieContre,TermeEche,MethodeCalcul,TypeCalcul);
  end else
  begin
      // calcul des échéances
    indiceEche := 1;
    while (DateEcheance <= DateFinTraite)  do
    Begin
      //
      // cas des contrats annuels dont les dates ne couvrent pas des périodes annuelles
      //
      DeCodeDate (DateEcheance,year,Month,Day); DateEcheance := EncodeDateTime (Year,Month,Day,0,0,0,0);
      ...
      begin
        ...
        begin
          NbDaysInPeriod := CalcDaysInperiod (DateEcheance,zinterval);
          NbMonthInPeriod := zinterval;
        end;
        CouvertureperiodeIncomplete := (Arrondi(DaySpan(DateEcheance,DateFinEcheance),0) <> NbDaysInPeriod);
        DeltaMois := Arrondi(MonthSpan(DateEcheance,DateFinEcheance),0);
        DeltaJour := Arrondi(DaySpan(DateEcheance,DateFinEcheance),0);
        D1 := DateEcheance;
        D2 := DateFinEcheance;
      end;
      //
      ...
      begin
        if D1 <> DateEcheance then TypeEcart := 'D' else TypeEcart := 'F';
      ...
      begin
        ...
        Inc(iMax);
        GetTOBEcheance(TobEch, iMax,Profil, '-', CodeAff, zRepAct,  ztier, 'ACT', 0, DateEcheance, DateFinEcheance, Dev, SaisieContre,TermeEche,MethodeCalcul,TypeCalcul);
      end;
      ///////////////////////////////////////////////

      Inc(iMax); inc(indiceEche);
      DateEcheance := PlusDate (DateEcheance,zinterval,TypeCalcul);
      DateEcheance := RecaleEcheanceFinMois( DateEcheance, DateDebut );
      DateFinEcheance := PlusDate( DateEcheance, zinterval, TypeCalcul );
      DateFinEcheance := PlusDate( DateFinEcheance, -1, 'J' );
      //
      ...
      begin
        DateEcheanceSuivante:=PlusDate (DateEcheance,zinterval,TypeCalcul);
...
// PA prise en compte de la date de résil si antérieure à la date de fin
if (DateResil < DateFinTraite) then DateFinTraite := DateResil; 
chp:= 'AFA_DATEECHE,AFA_AFFAIRE,AFA_NUMECHE,AFA_NUMECHEBIS,AFA_TIERS,AFA_ECHEFACT,AFA_LIQUIDATIVE,AFA_POURCENTAGE';
//chp:=chp+',AFA_MONTANTECHE,AFA_MONTANTECHECON,AFA_MONTANTECHEDEV,AFA_REPRISEACTIV,AFA_TYPECHE';
chp:=chp+',AFA_MONTANTECHE,AFA_MONTANTECHEDEV,AFA_REPRISEACTIV,AFA_TYPECHE';
chp:=chp+',AFA_GENERAUTO,AFA_DEVISE,AFA_LIBELLEECHE';
...
{$ENDIF}
  ...
  Begin
    ...
    Begin
      ...
      begin
        // PL le 13/11/02 : on conserve l'indice le plus élevé des echeances (non liquidative)
        ...
        Begin     // A traiter
//          DateDebutFac := StrToDate (TobDet.GetValue('AFA_DATEECHE'));
        End Else
        Begin  // Facturé
          bDejaFac :=true;
          ...
          DerniereDateFac := StrToDate (TobDet.GetValue('AFA_DATEECHE'));
          DateTmp := getFinPeriodefac(TobDet.GetValue('AFA_LIBELLEECHE'));
          if (DateTmp <> '') then
          begin
            DateDebutFac := PlusDate( StrToDate(DateTmp), 1, 'J' );
          end else
          begin
            DateTmp := TobDet.GetValue('AFA_DATEECHE');
            DateDebutfac := PlusDate( StrToDate(DateTmp), zinterval, TypeCalcul );
          end;
          ////////////////////////
        End;
      end;
      ...
      begin
        ...
        Begin // mcd 13/11/02 revu pour cas activite et date liquid
          ...
          CodeAff, zRepAct, ztier, ztyp_fac,TobDet.getValue('AFA_MONTANTECHEDEV'), DateLiquid,IDate1900, Dev,
          SaisieContre,TermeEche,MethodeCalcul,TypeCalcul);
        End;
      end;
    End;   // for
...
If (TypeCalcul = 'P') And (bDejaFac) then Exit;
...
{$IFNDEF RECALCULAFF}
   ...
   ExecuteSQL('DELETE FROM FACTAFF Where AFA_AFFAIRE="'+CodeAff+
              '" And AFA_DATEECHE >"'+usdatetime(DerniereDateFac)+'" And AFA_ECHEFACT="-" AND AFA_TYPECHE="NOR"' + stWhereDateCal);
{$ENDIF}
   ...
   DateFinEcheance := PlusDate( DateFinEcheance, -1, 'J' );
   (*
   if (bDejaFac) And (DateDebut=DerniereDateFac) then
        begin
        DateEcheance := PlusDate( DateDebut, zinterval, TypeCalcul );
        DateEcheance := RecaleEcheanceFinMois( DateEcheance, DateDebut );
         DateFinEcheance := PlusDate( DateEcheance, zinterval, TypeCalcul );
         DateFinEcheance := PlusDate( DateFinEcheance, -1, 'J' );
        end;
   *)
   ...
   // recup max numeche
   iMax := 1;
   if (DateEcheance <= DateFinTraite) then
      begin
      // PL le 06/03/02 : INDEX 3
   ...
   if (TypeCalcul ='P') then
      ...
      while (DateEcheance <= DateFinTraite)  do
            ...
            begin
              ...
              if MethodeCalcul = 'CIV' then CouvertureperiodeIncomplete := (Arrondi(DaySpan(DateDebutFac,DateFinEcheance),0) <> NbDaysInPeriod);
              DeltaMois := Arrondi(MonthSpan(DateDebutFac,DateFinEcheance),0);
              DeltaJour := Arrondi(DaySpan(DateDebutFac,DateFinEcheance),0);
              D1 := DateDebutfac;
              D2 := DateFinEcheance;
            ...
            begin
              ...
              if MethodeCalcul = 'CIV' then CouvertureperiodeIncomplete := (Arrondi(DaySpan(DateEcheance,DateFinEcheance),0) <> NbDaysInPeriod);
              DeltaMois := Arrondi(MonthSpan(DateEcheance,DateFinEcheance),0);
              DeltaJour := Arrondi(DaySpan(DateEcheance,DateFinEcheance),0);
              D1 := DateEcheance;
              D2 := DateFinEcheance;
...
//            IndiceEcheance:=GetTOBEcheance(TobEch, iMax,Profil, '-', CodeAff, zRepAct,  ztier, ztyp_fac, Lemontant, DateEcheance,DateFinEcheance, Dev, SaisieContre);
   ...
   // Traitement de la facture liquidative
   // PL le 16/05/2002 : on incremente une fois de trop...
   //Inc(iMax);
   ///////////////////
(*   TobEch := Tob.Create('Echéance liquidative',nil,-1);
   try
   if (DateLiquid<>iDate1900) and (DateLiquid<>iDate2099) then
      GetTOBEcheance(TobEch, iMax, 'X', CodeAff, zRepAct, ztier, ztyp_fac, 0, DateLiquid, Dev, SaisieContre);
   finally
   if (TobEch.Detail.Count > 0) Then TobEch.InsertOrUpdateDB;
...
if (GetParamSoc('SO_AFGERELIQUIDE') = true) and bCreerEcheLiquid then
  begin
    ...
    try
      if (DateLiquid<>iDate1900) and (DateLiquid<>iDate2099) then
        GetTOBEcheance(TobEch, IndiceDerniereEche + 1,Profil, 'X', CodeAff, zRepAct, ztier, ztyp_fac, 0, DateLiquid, IDate1900, Dev, SaisieContre,TermeEche,MethodeCalcul,typeCalcul);
    finally
      if (TobEch.Detail.Count > 0) Then TobEch.InsertOrUpdateDB;
      TobEch.free;
    End;
...
function RecaleEcheanceFinMois( AcdDateEntree, AcdDateDebEch : TDateTime ) : TDateTime;
var
jourDeb, jourEch,  mois, annee : word;
iNbJFinMois : integer;
begin
...
if (sLiquidative='X') then
    ...
    If (Posit <> 0) then Begin delete (St,Posit,2);Insert(TobDet.Getvalue('AFA_NUMECHEBIS'),St,Posit); end;     //mcd 25/05/03 ajout Bis
    ...
    Posit := Pos ('**',St);            // $$  pour reprendre le numéro d'echeance
    If (Posit <> 0) then
    Begin delete (St,Posit,2);Insert(TobDet.Getvalue('AFA_NUMECHEBIS'),St,Posit); end;   //mcd 23/05/03 mise bis
    Posit := Pos ('$$',St);            // $$  pour reprendre la date
    If (Posit <> 0) then
    Begin
    	delete (St,Posit,2);
