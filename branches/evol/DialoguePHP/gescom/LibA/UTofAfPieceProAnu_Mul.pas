...
BEGIN
  ...
  ExecuteSQL ('DELETE FROM VENTANA WHERE YVA_IDENTIFIANT ="'+EncodeRefCPGescom(TOBPiece)+'"');
  ...
  Begin
    ...
    if DevisExiste then
    begin
      ExecuteSQL('UPDATE LIGNE SET GL_VIVANTE="X",GL_QTERESTE=GL_QTEFACT WHERE GL_AFFAIRE="'+ CodeAffaire + '" AND GL_NATUREPIECEG="DAP"');
