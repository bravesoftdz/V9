...
BEGIN
NumTable := PrefixeToNum(Prefixe);
for i:=1 to High(V_PGI.DeChamps[NumTable]) do
  if V_PGI.DEChamps[NumTable,i].Nom<>'' then
     BEGIN
     V_PGI.DEChamps[NumTable,i].Libelle:=TraduitGA(V_PGI.DEChamps[NumTable,i].Libelle);
     // traduction des libellés des 3 parties de l'affaire
     if Pos ('AFFAIRE1',V_PGI.DEChamps[NumTable,i].Nom) > 0 then
          V_PGI.DEChamps[NumTable,i].Libelle := VH_GC.CleAffaire.Co1Lib;
     if Pos ('AFFAIRE2',V_PGI.DEChamps[NumTable,i].Nom) > 0 then
          V_PGI.DEChamps[NumTable,i].Libelle := VH_GC.CleAffaire.Co2Lib;
     if Pos ('AFFAIRE3',V_PGI.DEChamps[NumTable,i].Nom) > 0 then
          V_PGI.DEChamps[NumTable,i].Libelle := VH_GC.CleAffaire.Co3Lib;
     END;
END;
