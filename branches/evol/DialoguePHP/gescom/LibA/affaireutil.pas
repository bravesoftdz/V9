...
Procedure MajAffaire(TobPiece,TOBAcomptes : Tob; CodeAffaireAvenant, typeaction : string;Action : TActionFiche; Duplic:Boolean;SaisieAvanc:boolean);
...
Var i, AnneeDeb, AnneeFin, Mois : integer;
    ...
    StatutReduit,CodeAvenant, P1, P2, P3 : string;
Begin
Result:='';
AviPartErreur:=0;
...
for i:=1 to VH_GC.CleAffaire.NbPartie do
    ...
    if ((FTypeAction=taCreat) And (SaisieAffaire)) then
        ...
        begin
        	if Code.text = '' then Code.Text := CompteurPartieAffaire(i, bProposition,True);
        end;
        if (Code.Text ='') then
...
*****************************************************************}
function  CodeAffaireRegroupe(Var Part0, Part1 ,Part2 ,Part3, Avenant : string ; FTypeAction:TActionFiche ; SaisieAffaire,bGarderCpt,bUpdateCpt : Boolean): String;
Var codepartie, SQL, TypeTab : String;
    i : integer;
    TypeCode:String3;
    bProposition : Boolean;
    ...
    if ((FTypeAction=taCreat) And (SaisieAffaire)) then
        ...
        if (Pos(TypeCode ,'CPT;CPM')>0) then
           ...
           if Not(bGarderCpt) or (CodePartie = '') then
              CodePartie := CompteurPartieAffaire(i,bProposition,bUpdateCpt);
           end;
...
Begin
  ...
  try
    if FromAppel then
    begin
      Q := OpenSQL('SELECT SOC_DATA FROM PARAMSOC WHERE SOC_NOM="SO_APPCO' + IntToStr(NumPartie) + 'VALEUR'+ suffixe + '"',True);
    end else
    begin
    	Q := OpenSQL('SELECT SOC_DATA FROM PARAMSOC WHERE SOC_NOM="SO_AFFCO' + IntToStr(NumPartie) + 'VALEUR'+ suffixe + '"',True,-1,'',true);
    end;
...
if Affaire0 <> '' then
    ...
    BEGIN if bProposition then StatutReduit := 'P' else StatutReduit := 'A'; END;
...
For i:=0 to Gr.fliste.Columns.Count-1 do
    Begin
    StChamp := Gr.Q.FormuleQ.GetFormule(Gr.FListe.Columns[i].FieldName); //21/10/2005 on récupère le nom ttoaldu champ
...
if (TobPiece.Getvalue ('GP_NATUREPIECEG') = VH_GC.AFNatAffaire) or
   ...
   begin
	 MajSousAffaire(TobPiece,TOBAComptes,CodeAffaireAvenant,Action,Duplic,SaisieAvanc);
   exit;
   end;
...
BEGIN
  ...
  begin
    result := TOBlocal.getValue(Nomchamp);
  end;
END;
...
begin
  ...
  QQ := OpenSql ('SELECT * FROM PIECE WHERE GP_AFFAIRE="'+CodeAffaire+'"',true,-1,'',true);
  TRY
    if not QQ.eof then
    begin
      ...
      begin
        ...
        if (nature <> 'DBT') then
        begin
          PgiInfo ('Impossible des documents d''achats ou d''exploitation sont en cours');
          okok := false;
          break;
        end;
        if (nature = 'DBT') then
