...
uses  StdCtrls,Controls,Classes, forms,sysutils,ComCtrls,paramsoc,
      ...
      utilarticle,FactComm, FactCalc,Facture,UtilPGI,
...
uses FactureBTP,FactOuvrage,CalcOlegenericBtp,FactTimbres;
//
...
BEGIN
  ...
  Begin
     ...
     io:=Transactions(RechNumeroPiece,5) ;
...
// je vide la tobarticle si elle devient trop importante
	if (tobarticles.detail.count > 200) then
...
//---
    CalculFacture(TOBPiece_OK,nil,TobOuvrages ,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TobPorcs,Nil,Nil,DEV);
//    RecalculeSousTotaux;
...
BEGIN
  ...
  if  RegSurCaf  then
    ...
    // Verif du payeur, s'il est différent on le garde
   	if  RegSurCaf  and
     		(TobPiece.Getvalue('GP_TIERS')=TobPiece.Getvalue('GP_TIERSPAYEUR')) then
...
//  GestionDuVisa;
  GestionDuVisa(TobPiece_ok);
   ...
   GereEcheancesGC(TOBPiece_OK,TOBTiers,TOBEches,NIL,Nil,taCreat,DEV,False);
...
for zz:=0  to Tobpiece_ok.Detail.count-1 do
   ...
   PassationComptable(TOBPiece_OK, TOBOuvragesP,TOBBases,TOBBasesL, TOBEches, nil,nil,TOBTiers, TOBArticles, TOBCpta, Nil, TOBPorcs, TOBPieceRG, Nil, nil,nil,DEV, OldEcr, OldStk, True);
   LibereParamTimbres;
...
//      LoadTobVarQte(Nil,TobVarQte_ACT,'ACT');
  ...
  if (GenFac.zcomaffsup <> '') then     //Commentaire Affaire
    ...
    Begin
        ...
        CalculFacture(TOBPiece,nil,TobOuvrages ,TOBOuvragesP,TOBBases,TOBBasesL,TOBTiers,TOBArticles,TobPorcs,Nil,Nil,DEV);
...
begin
  ...
  begin
    ...
    TOBL.putValue('GL_QTEFACT',1); TOBL.putValue('GL_QTESTOCK',1); TOBL.putValue('GL_QTERESTE',1);
