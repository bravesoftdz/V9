...
begin
  ...
  if stRange = 'GRC' then
    ...
    CodeRes := AFLanceFiche_Rech_Ressource(stWhere, stRange);
...
begin
  ...
  LookupList(G_Ressource, 'RESSOURCES', 'RESSOURCE', 'ARS_RESSOURCE', 'ARS_LIBELLE', StWhere,
    'ARS_RESSOURCE', False, 6);

  if (G_Ressource.Text <> '') and (G_Ressource.Text <> G_CodeRessource.Text) then
    G_CodeRessource.Text := G_Ressource.Text;
end;
