...
uses  StdCtrls,Controls,Classes,forms,sysutils,ComCtrls, windows,
      ...
      utilarticle,Dicobtp,AFPrepFact,Facture,Hstatus,TiersUtil, CBPPath,
{$IFDEF EAGLCLIENT}
      MaineAGL, eMul,
{$ELSE}
...
BEGIN
    ...
    sttiersfact := TobFaff.getValue('T_FACTURE');
    ...
    Begin
      ...
      Begin
        ...
        //  1 : Affaire étant rattaché à une affaire de référence
        1 : Begin
            if ((sttiersfact = TobFaff.getValue('T_FACTURE')) and
...
begin
{$IFDEF BTP}
AGLLanceFiche ('AFF','AFPREPFACT_MUL','AFF_AFFAIRE0=I','','STATUT=INT');
{$ELSE}
AGLLanceFiche ('AFF','AFPREPFACT_MUL','','','');
{$ENDIF}
end;
