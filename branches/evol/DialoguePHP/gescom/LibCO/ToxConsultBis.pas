...
uses
  ...
  {$IFNDEF DBXPRESS} dbTables, {$ELSE} uDbxDataSet, {$ENDIF} ToxMoteur, HSysMenu, uToxClasses, uToxConst, HEnt1, UIUtil, utoz ;
...
////////////////////////////////////////////////////////////////////////////////
  procedure ConsultManuelTox ;
  procedure ConsulteAutoTox ( FileName : string; MessageDonnee: boolean);
...
begin
  ...
  PEnveloppes.LoadEnveloppe(FileNameEnv) ;
    ...
    begin
      ...
      //
      // Recherche de la devise d'émission
      //
      Devise:=GetInfoComp('DEVISE', Devise);
      if Devise = '' then  Devise := 'Inconnue';
      Bascule:=GetInfoComp('BASCULEEURO', Bascule);
    end ;
    if Bascule then
    begin
      DeviseEmis.Text   := 'EUR' ;
      DeviseContre.Text := Devise;
    end else
    begin
      DeviseEmis.Text   := Devise ;
      DeviseContre.Text := 'EUR'  ;
    end;
  FINALLY
    //
...
procedure TConsultMessageToxBis.FormShow(Sender: TObject);
begin
  // Appel de la fonction d'empilage dans la liste des fiches
  AglEmpileFiche(Self) ;
end;

procedure TConsultMessageToxBis.baideClick(Sender: TObject);
begin
  CallHelpTopic(Self) ;
end;
