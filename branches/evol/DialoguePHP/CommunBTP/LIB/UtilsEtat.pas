...
Type
    ...
    public
      ...
      //
      constructor Create (Tip, Nat, Modele, Titre, StSQL : String; Apercu, DeuxPages, First, ListeExport, Duplicata : Boolean; Spages : TPageControl; Etat : THValComboBox);
...
BEGIN
    ...
    if fFirst then iDef:=-1 else OldDef:=VC.Value;
    ...
    While Not QQ.EOF do
    BEGIN
      i_ind := VC.Items.Add(QQ.FindField('MO_LIBELLE').AsString) ;
      ...
      begin
        ...
        begin
         Idef   := i_ind ;
         Modele := QQ.findField('MO_CODE').AsString;
        end;
      end;
      VC.Values.Add(QQ.FindField('MO_CODE').AsString) ;
      QQ.Next
    END ;
    ...
    if fFirst then
      if idef>0 then VC.ItemIndex:=idef else VC.ItemIndex:=VC.Values.IndexOf(Modele)
    else
      if VC.Values.IndexOf(OldDef)>=0 then VC.Value:=OldDef else VC.ItemIndex:=0 ;
...
BEGIN
  ...
  if fTip='E' then
    EditEtat(fTip, fNat, fModele, False, fSpages, fstSQL, fTitre)
  else
...
begin
  ...
  if Assigned(TobEdition) then
    Result := LanceEtatTob(fTip, fNat, fModele, TobEdition, fApercu, fListeExport, fDeuxPages, fSpages, fstSQL, fTitre, fDuplicata)
  else
