...
uses
  ...
  hcompte, HEnt1, HSysMenu, MajTable, SaisUtil, ADODB ;
...
if MonnaieIN then
   ...
   if CodeDevise.Value<>'' then
      BEGIN
      RGsens.Items[0]:=FindEtReplace(MsgBox.Mess[16],'$',CodeDevise.Value,True) ;
      RGsens.Items[1]:=FindEtReplace(MsgBox.Mess[17],'$',CodeDevise.Value,True) ;
      END ;
   END ; 
end;
