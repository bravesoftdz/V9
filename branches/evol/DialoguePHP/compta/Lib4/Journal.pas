...
uses WinTypes, WinProcs, Classes, Graphics, Forms, Controls, StdCtrls, Tabs,
  ...
  Hcompte, HRegCpte, Hctrls, ADODB, udbxDataset{$ELSE}uDbxDataSet{$ENDIF}, DBCtrls, Hctrls, HDB,
...
begin
Case Comment of
   taCreat,taCreatEnSerie,taCreatOne : if Not JaiLeDroitConcept(TConcept(ccJalCreat),True) then Exit ;
              taModif,taModifEnSerie : if Not JaiLeDroitConcept(TConcept(ccJalModif),True) then Exit ;
   END ;
...
begin if JaiLeDroitConcept(TConcept(ccJalCreat),True) then Bouge(nbInsert) ; end;
