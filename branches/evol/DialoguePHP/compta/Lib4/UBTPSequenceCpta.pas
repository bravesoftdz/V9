...
function ReadCurrentSequence(Cle: string) : integer;
function CreateSequence(Cle: string; valeur : integer) : Boolean;
function GetNextSequence(Cle: string) : integer;
...
function ExistSequence(Cle: string) : Boolean;
begin
	Result := ExisteSQL('SELECT DSQ_CODE FROM DESEQUENCE WHERE DSQ_CODE="'+Cle+'"');
end;
...
begin
  Result := -1;
  try
  	QQ := OpenSql ('SELECT DSQ_VALEUR FROM DESEQUENCE WHERE DSQ_CODE="'+Cle+'"',True,1,'',true);
    if not QQ.eof then
    begin
...
begin
  ...
  okOk := false;
  repeat
    lastValue := ReadCurrentSequence(Cle);
    if lastValue = -1 then
    begin
    ...
    II := lastValue + 1;
    if ExecuteSQL('UPDATE DESEQUENCE SET DSQ_VALEUR='+IntToStr(II)+' WHERE DSQ_VALEUR='+IntToStr(lastValue)) > 0 then
    begin
    	OkOk := true;
      Result := II;
    end;
    if (not OkOk) and (Nbr < 50) then Sleep(200);
