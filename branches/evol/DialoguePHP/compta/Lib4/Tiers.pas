...
uses
  ...
  ExtCtrls, Buttons{$ELSE}uDbxDataSet{$ENDIF}, Grids, DBGrids, StdCtrls, Buttons, ExtCtrls, DBCtrls, Mask,
...
BEGIN
Case Comment of
   taCreat,taCreatEnSerie,taCreatOne : if Not JaiLeDroitConcept(TConcept(ccAuxCreat),True) then Exit ;
              taModif,taModifEnSerie : if Not JaiLeDroitConcept(TConcept(ccAuxModif),True) then Exit ;
   END ;
...
begin if JaiLeDroitConcept(TConcept(ccAuxCreat),True) then Bouge(nbInsert) ; end;
