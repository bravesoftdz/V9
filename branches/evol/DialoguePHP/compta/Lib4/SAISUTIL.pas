...
Type RMVT = RECORD
            ...
            DateC,DateTaux,LastDateEche : TDateTime ;
...
{=====================================================================}
  {$IFNDEF EAGLSERVER}
Function InitVariant ( Nom : String ) : Variant ;
Var Pref,Tipe : String ;
    Pos1,it,j,jt : integer ;
BEGIN
Result:='' ;
Pos1:=Pos('_',Nom) ; if Pos1<=0 then Exit ;
Pref:=Trim(Copy(Nom,1,Pos1-1)) ; it:=PrefixeToNum(Pref) ; if it<=0 then Exit ;
jt:=0 ;
For j:=1 to High(V_PGI.DEChamps[it]) do
  if AnsiCompareText(V_PGI.DeChamps[it,j].Nom,Nom)=0 then BEGIN jt:=j ; Break ; END ;
if jt<=0 then Exit ; Tipe:=V_PGI.DeChamps[it,jt].Tipe ;
...
Var ii : integer ;
BEGIN
if Not JaiLeDroitConcept(TConcept(ccSaisEcritures),True) then Result:=True else
   BEGIN
   ii:=ControleDate(DateToStr(DD)) ; Result:=(ii>0) ;
...
Var ii : integer ;
BEGIN
ii:=ControleDate(DateToStr(DD)) ; Result:=((ii>0) and (not GetParamSocSecur('SO_GCDESACTIVECOMPTA',False))) ;
...
Var NomEcr,NomAna : String ;
    ...
    TOBAxe        : TOB ;
BEGIN
FNumAna:=PrefixeToNum('Y') ; FNumEcr:=PrefixeToNum('E') ;
for i:=1 to TOBEcr.NbChamps do
    BEGIN
    NomEcr:=TOBEcr.GetNomChamp(i) ;
    // le champs e_vision de la table ecriture est un varchar de 3 et celui
    ...
    if j>0 then
       ...
       if V_PGI.DECHAMPS[FNumEcr,i].Tipe=V_PGI.DECHAMPS[FNumAna,j].Tipe
          then TOBAna.PutValeur(j,TOBEcr.GetValeur(i)) ;
       END ;
    END ;
...
begin
  ...
  try
      ...
      if (iNumTable = 0) then Exit;

      for i := 1 to TT.NbChamps do
        TT.AddChampSup(V_PGI.DECHAMPS[iNumTable,i].Nom ,False);

      for i := 1 to TT.NbChamps do begin
        if ((V_PGI.DECHAMPS[iNumTable,i].tipe <> 'MEMO') and (V_PGI.DECHAMPS[iNumTable,i].tipe <> 'BLOB')) then begin
