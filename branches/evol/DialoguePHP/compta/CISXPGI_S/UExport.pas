...
begin
  ...
  tbcharger.Connection := DMImport.DBGLOBAL;
...
begin
   ...
   try
              ...
              FTable.Connection := DMImport.DBGLOBAL;
              FTable := DMImport.GzImpReq;
              FTable.Open;
...
(*                      if (TobExport.detail.Count > 1) and (not Memefichier.checked) then
                         Name := TF.detail[iet].GetValue('Sortief')+TF.GetValue('Indice')+'.DB'
                      else
*)
...
{$ENDIF}
                            ...
                            (*if (FFScript.Champ[0].LienInter <> '') and (AScript.FileDest = 0) then
                            begin
                               Lien :=  InterpreteVarSyntaxSt(FFScript.Champ[0].LienInter);
                               if Pos ('|', Lien) <> 0 then
                               begin
                                     St := ReadTokenPipe(Lien,'|');
                                     ListeLien := TOB.Create ('', nil, -1);
                                     While (St <> '') do
                                     begin
                                          TF1 := TOB.Create('fille', ListeLien, -1);
                                          TF1.AddChampSupValeur('ListeLien', St);
                                          TableLien :=FFScript.name+Copy(St, 0, Pos('/',St)-1)+'.DB';
                                          TF1.AddChampSupValeur('TableLien', TableLien);
                                          TF1.AddChampSupValeur('XWhere', '');
                                          TF1.AddChampSupValeur('TableName', '');
                                          St1 := Copy(St,Pos('/',St)+1,length(St)-2);
                                          St2 := ReadTokenPipe(St1,',');
                                          While (St2 <> '') do
                                          begin
                                              TF2 := TOB.Create('fille', TF1, -1);
                                              TF2.AddChampSupValeur(St2, St2);
                                              St2 := ReadTokenPipe(St1,',');
                                          end;
                                          St := ReadTokenPipe(Lien,'|');
                                     end;
                               ...
                               TableLien :=TF.detail[iet].GetValue('TABLENAME')+Copy(TF.detail[iet].GetValue('Lieninter'), 0, Pos('/',TF.detail[iet].GetValue('Lieninter'))-1)+'.DB'
                            end
                            else
                            begin
                               Lien := '';
...
(*                            if (AScript.FileDest = 0) and (not TF.detail[iet].GetValue('SousEnsemble')) then
                            begin
                             if ModeRlV <>'IMPORT' then
                                      EcritureFichierExport(FichierIE, FTable.Tablename, FieldName, Domaine, DatedebutExo, DatefinExo, Lien, '', TableLien, Orderby, LPays, ListeLien)
                             else
                                      EcritureDanslaBase(FTable.Tablename, FieldName, Domaine, TEcr, TReleve, CpteGeneral, RefPointage, NumReleve,  TSV, '', OrderBy);
                            end;
*)
...
(*                         if (AScript.FileDest = 1) then
                         begin
                               DeleteFile (CurrentDonnee+'\'+Name+'.txt');
                               DeleteFile (CurrentDonnee+'\'+Name+'.sch');
                         end
                         else
                               DeleteFile (CurrentDonnee+'\'+Name+ '.DB');
*)
                         DMImport.CmdDonnee.CommandText:='DROP TABLE ' + Name;
...
{$IFNDEF DBXPRESS}
              if UnEnreg then
                  ScriptVisu (GD.Cells[1, SavRow], DMImport.DBGlobalD.DatabaseName,nil, FALSE,0)
              else
                  ScriptVisu (GD.Cells[1, SavRow], DMImport.DBGlobalD.DatabaseName, ListeEnreg, TRUE,0);
{$ENDIF}
{$ENDIF}
              ListeEnreg.free;
end;
...
{$ELSE}
                  ...
                  QP.SQL.Add ('SELECT CLE, Domaine from '+ DMImport.GzImpReq.TableName
                  + ' Where CLE="'+Table.Text+'"');
