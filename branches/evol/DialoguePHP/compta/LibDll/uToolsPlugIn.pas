...
function GetTabletteSqlPortail (DataType: string; Plus: string = ''; WithoutBy: Boolean = False) : string;

type
//  HString = string; //PGR 3/11/2006 n'importe quoi
...
//    procedure SetSqlMutliDossier(var SQL: string);
  ...
  public
    ...
    function GetBoolean(NomChp: string; Critere: Boolean = True) : Boolean; {Renvoie True si le Booléen est à X}
    ...
    procedure SetTypeCol(Grille, Chp, TypeCol : string);
    ...
    function GetCritereVisible(AControlName: string): boolean;
    procedure SetCritereVisible(AControlName: string; AVisible: boolean);
    class function IsForceDataBaseName(var databasename: string; PlugInName: string): boolean;
    function ExecuteSelect(Requete: string; ADossier: string = ''): string;
  ...
  private
    FStack: string;
  public
    function OpenSelectInCache(ARequete: string; ADossier: string = ''; AStack : string = ''): TOB;
    procedure SetControlProperty(ControlName, PropertyName: string; value: variant);
{GC_FR_VIGGC10035_DEBUT}
    procedure ModifControlPropriete(ControlName, PropertyName: string; value: variant);
{GC_FR_VIGGC10035_FIN}
  end;
...
var
  ...
  StTable, StCode, StWhere, StPrefixe, StLib: string;
  StTitre: hstring;
  NumDisp: Integer;
begin
  DecodeTT (DataType, StTable, StCode, StWhere, StPrefixe, StLib, StTitre, NumDisp) ;

  //    okGraph := Pos(';GRAPHIQUE;',';'+DO_NOMCHAMP.Text+';')>0;
  //    if ((StPrefixe = 'CC') or (StPrefixe = 'CO') or (StPrefixe = 'YX')) then
  //      Query := 'Select * FROM ' + StTable
  //    else if OkGraph then
  //      SQL:='Select '+DO_CODE.Text+', '+DO_CHAMPLIB.Text+', '+Pref+'_GRAPHIQUE FROM '+PrefixeToTable(Pref)
  //    else
  //    begin
  if (StPrefixe = 'CC') or (StPrefixe = 'CO') or (StPrefixe = 'YX') then
    Query := 'SELECT ' + StCode + ', ' + StLib + ', ' + StPrefixe + '_ABREGE' + ', ' + StPrefixe + '_LIBRE' + ' FROM ' + StTable
  else if StLib <> '' then
    Query := 'SELECT ' + StCode + ', ' + StLib + ' FROM ' + StTable
  else
    Query := 'SELECT ' + StCode + ' FROM ' + StTable;
  //    end;

  ChangeWhereTT (StWhere, '', WithoutBy) ;
  ...
  begin
    Query := Query + ' WHERE (' + Plus + ')';

    if StWhere <> '' then
    begin
      if Pos (' WHERE ', StWhere) = 1 then
        StWhere := copy (StWhere, 7, Length (StWhere) - 7) ;

      Query := Query + ' AND ' + StWhere;
    end;
  end;
...
//    DecompressTextObjectStream(MemoryStream);
  ...
  TobFormesXml := Tob.Create('_SFORMESXMLP_', nil, -1);
  TobFormesXml.LoadDetailFromSQL('SELECT YFX_XMLDATA FROM SFORMESXMLP WHERE (YFX_TYPEFORME = "' + Nature + '") AND (YFX_FORME = "' + Forme + '")');

  if TobFormesXml.Detail.Count > 0 then
  begin
// à mettre du coté client apres avoir avoir vu avec P Gronchi
    Buffer := TobFormesXml.Detail[0]['YFX_XMLDATA'].Valeur;
    MemoryStream := TMemoryStream.Create;
    MemoryStream.Clear;
//    DecompressTextObjectStream(MemoryStream);
    MemoryStream.writeBuffer(Pointer(Buffer)^,Length(Buffer));
    ...
    TobScript := Tob.Create('T_SCRIPTP', nil, -1);
    TobScript.LoadDetailFromSQL('SELECT YSC_DATA FROM SCRIPTSP WHERE YSC_TYPESCRIPT="' + Nature + '" AND YSC_SCRIPT="' + Forme + '"');

    if TobScript.Detail.Count > 0 then
    begin
      TobScript.Detail[0].ReaffecteNomTable('T_SCRIPTP', True);
      TobScript.Detail[0].ChangeParent(TobFormeXml, -1, True);
    end;
...
begin
  ...
  begin
//EPZ 14.04.2006if false then 
    if TobCtrl.getString('DATATYPE') <> '' then
    begin
      stSql := GetTabletteSqlPortail(TobCtrl.getString('DATATYPE'), TobCtrl.getString('PLUS'));
      if ADossier <> '' then
  ...
  else if TobCtrl.NomTable = 'T_IMAGE' then
  begin
  end
...
begin
  ...
  FStack := '';
//  if TobIn.FieldExists('DBNAME') then
//    FStack := FStack + TobIn.GetValue('DBNAME');
  if TobIn.FieldExists('DOMAIN') then
    FStack := FStack + TobIn.GetValue('DOMAIN');
  if TobIn.FieldExists('ANCETRE') then
    FStack := FStack + TobIn.GetValue('ANCETRE');
  if TobIn.FieldExists('FORMNAME') then
    FStack := FStack + TobIn.GetValue('FORMNAME');
...
begin
  ...
  if (Result = nil) and (DatasRequest<>nil) then
...
var
    ...
    FSources, FChamps, FLiaison, FFPerso, FParams, FLargeurs, FJustifs, FTris : string;
    FTitres, FLibelle, FNumCols : WideString;
...
begin
  ...
  begin
    ...
    //EPZ 19.03.2008
    TNew.AddChampSupValeur('CLEARSELECTED', BooltoStr(Datas.detail.count > 0), false);
    //EPZ 19.03.2008
...
begin
  ...
  begin
    if T.FieldExists('NAME') then
      T.PutValue('NAME', ControlName)
    else
      T.AddChampSupValeur('NAME', ControlName);

    if T.FieldExists('VALUE') then
      T.PutValue('VALUE', ControlValue)
    else
      T.AddChampSupValeur('VALUE', ControlValue);
  end;
end;
...
begin
  try
    ...
    ItemsTob := Tob.Create('T_ITEMSCOMBOBOX', NewData, -1);
    if assigned(InfoTob) then
    ...
    begin
      ...
      NewTob := Tob.Create('T_ITEMVALUE', ItemsTob, -1);
//EPZ 21.04.2006 NewTob := Tob.Create('T_ITEMVALUE', NewData, -1);
...
procedure TVignettePlugin.SetControlProperty(ControlName, PropertyName: string; value : variant);
var
  T: TOB;
begin
  T := DatasRequest.FindFirst(['NAME'], [ControlName], True);
  if T <> nil then
    if T.FieldExists(PropertyName) then
      T.PutValue(PropertyName, Value);
end;

{GC_FR_VIGGC10035_DEBUT}
procedure TVignettePlugin.ModifControlPropriete(ControlName, PropertyName: string; value : variant);
var
  T: TOB;
begin
  T := GetPropInstance(PropertyName,ControlName);
  if T = nil then
  begin
    T := Tob.Create('T_CONTROLPROP', PropsToSend, -1);
    T.AddChampSupValeur('PROPNAME', PropertyName, CSTString);
    T.AddChampSupValeur('CONTROLNAME', ControlName, CSTString);
  end;
  T.AddChampSupValeur('VALUE', Value, CSTInteger);
end;
{GC_FR_VIGGC10035_FIN}
...
function TVignettePlugin.GetCritereValue(AControlName: string): hString;
var
//  i: integer;
  Name, Nom: string;
  T: TOB;
...
function TVignettePlugin.GetCritereVisible(AControlName: string): Boolean;
var
//  i: integer;
  Name, Nom: string;
  T: TOB;
...
begin
  Result := nil;
  if not assigned(CritToSend) then
    exit;
...
begin
  ...
  if (VarType(V) > 1) then
  try
    if (VarToStr(V) = '') or (VarToStr(V) = '/  /    ') then
      Result := iDate1900
    else
...
function TVignettePlugIn.GetBoolean(NomChp: string; Critere: Boolean): Boolean;
begin
  Result := GetString(NomChp, Critere) = 'X';
end;

//Retourne le dossier
function TVignettePlugIn.GetDossier: string;
var
  stDossier, stPlugIn: string;
  bforce: boolean;
...
begin
  ...
  if FPlugInName<>'' then
  begin
    bforce := IsForceDataBaseName(stDossier,FPlugInName);
  end;
  if not bforce then
  begin
    stPlugIn := LookupCurrentSession.FAction;
    FPlugInName := ReadTokenPipe(stPlugIn,'.');
//    bforce := IsForceDataBaseName(stDossier,FPlugInName);
  end;
...
//    bforce := true;
  //end;
  if stDossier <> '' then
    ChargeDBParams(stDossier, StDriver, StServer, StPath, Stbase, StUser, StPassWord, StODBC, StOptions, StGroup, StShare)
  else
...
function TVignettePlugIn.ExecuteSelect(Requete: string; ADossier: string = ''): string;
var
  stSQL,database: hstring;
begin
  if ADossier = '' then
    database := ListeSoc
  else
  ...
  if database <> '' then
    stSQL := sqlToMultiBases(Requete, database);
  ExecuteSQL(stSQL);
  result :=  stSQL;
end;

function TVignettePlugIn.OpenSelectInCache(ARequete: string; ADossier: string = ''; AStack : string = ''): TOB;
var
  _dossier,_stack: string;
begin
  if ADossier = '' then
    _dossier := ListeSoc
  else
  ...
  ddWriteLN('PGIVIGNETTES BASE : ' + _dossier);
  result := LookupcurrentSession.OpenSelectInCache(ARequete, _dossier, _stack);
end;
...
{Definit le Type des données d'une colonne de la grille
 'M' pour mémo, 'B' pour boolean ....}
procedure TVignettePlugIn.SetTypeCol(Grille, Chp, TypeCol : string);
var
  T: TOB;
begin
  T := GetTobProperty(Grille, Chp);
  if T <> nil then
    T.SetString('TYPE', TypeCol);
end;
...
class function TVignettePlugin.IsForceDataBaseName(var databasename: string; PlugInName: string): boolean;
var
  ini: TIniFile;
begin
databasename := '';
...
procedure TVignettePlugin.SetCritereVisible(AControlName: string;
  AVisible: boolean);
var
  T: TOB;
begin
