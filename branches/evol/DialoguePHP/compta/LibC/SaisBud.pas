...
Créé le ...... : 25/01/2005
Modifié le ... :   /  /
Description .. : Remplacé en eAGL par eSaisBud.PAS
Mots clefs ... :
*****************************************************************}
unit SaisBud;
...
{$ENDIF}
  ...
  BudJal,   // FicheBudjal
  BudGene,  // FicheBudGene
  BudSect,  // FicheBudSect
...
Uses HZoomSP; // CatExiste, ChoisirValCat
...
ChangeSQL(TBudEcr) ; ModeSaisBud:=msbGene ; OkControle:=True ;
...
procedure TFSaisBud.CompleteEnreg ;
BEGIN
TBudEcr.FindField('BE_ETABLISSEMENT').AsString:=BE_ETABLISSEMENT.Value ;
TBudEcr.FindField('BE_DATEMODIF').AsDateTime:=NowFutur ;
TBudEcr.FindField('BE_DATECREATION').AsDateTime:=Date ;
TBudEcr.FindField('BE_TYPESAISIE').AsString:=VUE.Value ;
TBudEcr.FindField('BE_RESOLUTION').AsString:=RESOL.Value ;
END ;
...
for ACol:=1 to GB.ColCount-1 do for ARow:=1 to GB.RowCount-1 do
    ...
    for i:=0 to T.Count-1 do
        ...
        if ((O.GetDebitSaisi<>0) or (O.GetCreditSaisi<>0)) then
           ...
           O.EgalChamps(TBudEcr) ;
           TMemoField(TBudEcr.FindField('BE_BLOCNOTE')).Assign(O.M) ;
           CompleteEnreg ;
           TBudEcr.Post ;
           END ;
        END ;
...
BEGIN
Result:=False ;
if BE_BUDSECT.ExisteH<=0 then BEGIN BE_BUDSECT.SetFocus ; HM.Execute(6,'','') ; Exit ; END ;
if Pos(BE_BUDSECT.Text+';',LesSections)<=0 then BEGIN BE_BUDSECT.SetFocus ; HM.Execute(6,'','') ; Exit ; END ;
Result:=True ;
END ;
...
BEGIN
Result:=False ;
if BE_BUDGENE.ExisteH<=0 then BEGIN BE_BUDGENE.SetFocus ; HM.Execute(7,Caption,'') ; Exit ; END ;
if Pos(BE_BUDGENE.Text+';',LesComptes)<=0 then BEGIN BE_BUDGENE.SetFocus ; HM.Execute(7,Caption,'') ; Exit ; END ;
Result:=True ;
END ;
...
Var ind : integer ;
BEGIN
if Col<=PERDEB.Items.Count-PERDEB.ItemIndex then
   BEGIN
   ...
   if ExosInter.Values.Count<=0 then
      BEGIN
      Ind:=Col-PERDEB.Items.Count-1 ;
      Result:=StrToDate(PERFIN.Values[ind]) ;
      END else
      BEGIN
      if Col>(PERDEB.Items.Count-PERDEB.ItemIndex)+ExosInter.Values.Count then
         BEGIN
