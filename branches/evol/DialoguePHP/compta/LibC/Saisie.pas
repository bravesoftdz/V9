...
OkPasModif:=((Action=taModif) and (Not JaiLeDroitConcept(TConcept(ccSaisEcritures),False))) ;
// GG COM
...
if Not JaiLeDroitConcept(TConcept(ccSaisSolde),True) then Exit ;
if Not EstRempli(GS,GS.Row) then Exit ;
...
if C=SA_Gen then
   BEGIN
   {$IFNDEF GCGC}
   if ((Not JaiLeDroitConcept(TConcept(ccGenModif),False)) or (ModeLC)) then A:=taConsult ;
   ...
   BEGIN
   if ((Not JaiLeDroitConcept(TConcept(ccAuxModif),False)) or (ModeLC)) then A:=taConsult ;
...
if Not JaiLeDroitConcept(TConcept(ccSaisCreatGuide),True) then Exit ;
