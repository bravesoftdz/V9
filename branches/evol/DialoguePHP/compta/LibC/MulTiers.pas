...
if TypeAction=taModifEnSerie then
   ...
   END;
// MODIF PACK AVANCE
if ((JaiLeDroitConcept(TConcept(ccAuxCreat),False)) and (TypeAction<>taConsult)) then BInsert.Visible:=True ;
{$IFDEF CCS3}
...
if Q.FindField('T_AUXILIAIRE')<>Nil then
   ...
   if TypeAction<>taModifEnSerie then
      ...
      if (TypeAction=taModif) then
        ...
        Ferme(Q1) ;
        END ;
      if ((TypeAction=taModif) and (Not JaiLeDroitConcept(TConcept(ccAuxModif),False))) then AA:=taConsult ;
      (*{$IFDEF ESP}
