...
uses
  ...
  Hctrls, StdCtrls, Ent1,
  hmsgbox,HStatus,HEnt1,
  {$IFNDEF EAGLCLIENT}
...
{$IFDEF MODENT1}
  ...
  CPObjetGen,
{$ENDIF MODENT1}
  Math, UtilTrans, UTOB, RecupUtil, SaisComm;
...
Procedure RecupSectionSISCO(Var St,St1 : String ; Var Infos : TInfoGeneSISCO; FInfoCpta : TZInfoCpta=nil) ;
...
Function EcritPiece(Var NewFichier : TextFile ; ListePiece : HTStringList ; Var IdentPiece : TIdentPiece ; Var Infos : TInfoGeneSISCO ;
                    Var PieceEcrite : Boolean ;
                    InfoImp : PtTInfoImport = Nil) : Integer ;
Function FaitPiece(St : String ; ListePiece : HTStringList ; Var IdentPiece : TIdentPiece ;
...
If RemplaceLeFichier Then
  ...
  RenameFile(StNewFichier,StFichier) ;
  END Else
  BEGIN
  If NewRep='' Then
    BEGIN
  ...
  AssignFile(NewFichier,StNewFichier2) ; Erase(NewFichier) ;
  {$i+}
  RenameFile(StNewFichier,StNewFichier2) ;
  END ;
...
If IdentPiece.CodeMontant<>'' Then
  BEGIN
  If (GetTenuEuro) And (IdentPiece.CodeMontant[1]='F') Then Result:=V_PGI.OkDecE Else
  If (Not GetTenuEuro) And (IdentPiece.CodeMontant[1]='E') Then Result:=V_PGI.OkDecE ;
  END ;
If Oppose Then
  BEGIN
...
If Racine[Length(Racine)]<>';' Then Racine:=Racine+';' ;
While Racine<>'' Do
  BEGIN
...
If GetRecupPCL Then EstAux:=Cpt[1] In ['0','9','a'..'z','A'..'Z']
                Else EstAux:=Cpt[1] In ['0','9','C','F'] ;
if GetRecupComSx and (not EstAux) then // ajout me 12-02-02
...
If Not EstAux Then
  BEGIN
    If (GetRecupPCL or GetRecupSISCOPGI ) Then
    BEGIN
...
{$IFDEF NOVH}
      if (Gen='') and (BourreEtLess(Cpt,fbGene) = GetParamsoc('SO_DEFCOLSAL')) then
        Aux := GetParamSoc('SO_SALATTEND') ;
{$ELSE}
...
Function RetoucheListePiece(ListePiece : HTStringList ; Var IdentPiece : TIdentPiece ; InfoImp : PtTInfoImport = Nil) : Integer ;
Var i : Integer ;
...
{$IFNDEF NOVH}
  If GetRecupSISCOPGI Then St1:=Insere(St1,Format_String(VH^.Cpta[fbGene].AxGenAttente,17),14+DecalListe,17)
...
{$ENDIF}
  ...
  If GetRecupPCL Then LibelCGE:=Format_String('Ecart de conversion',35) Else
    If GetRecupSISCOPGI Then LibelCGE:=Format_String('Equilibrage bordereau',35) Else
  ...
  *)
  ...
  {$ENDIF}
  END ;
If GetRecupSISCOPGI Then OkMajTP:=FALSE ;
...
ULiBExercice.WhatDate(St1,DD1,DD2,Err,Exo) ;
Result:=DD1 ;
END ;
...
{$ENDIF}
        ...
        if GetRecupComSx then // ajout me 20-03-02
        begin
...
{$IFNDEF NOCONNECT}
        If CptLuJ.Nature='BQE' Then
          ...
          If NatG='COC' Then StSISCO.TP:='RC' Else
            If NatG='COF' Then StSISCO.TP:='RF' Else If GetRecupSISCOPGI Then
              BEGIN
...
{$ENDIF}
        ...
        If GetRecupPCL And (Arrondi(IdentPiece.LigneEnCours.DE+IdentPiece.LigneEnCours.CE,2)<>0)
        ...
        If (GetRecupCegid) And (InfoImp<>NIL) Then
          BEGIN
        ...
        StSisco.RefP:='' ;
        If (GetRecupSISCOPGI or GetRecupPCL) and (DecalAna=0) Then
          BEGIN
        ...
        If RecupSISCO or (GetRecupComSx and (InfoImp <> nil)) Then
        BEGIN
             ll:=Pos('&[+',St) ;
             ...
             If (IdentPiece.DateP >= GetEncours.Deb) Then
             BEGIN
                  Lettre:=Trim(Copy(St,44,1)) ;
                  If (Lettre<>'') And (Lettre<>'*') And (DecalAna=0) Then
                    BEGIN
                    If GetRecupLTL Or GetRecupSISCOPGI Then StSISCO.RefReleve:=Format_String(Lettre+CptOrigineCEGID,10) ;
             ...
             If ((InfoImp<>NIL) And (GetRecupSISCOPGI or GetRecupPCL) And (DecalAna=0))
             // ajout me 10/07/2003 pour les codes stat
           {JP 01/09/03 : or au lieu de and sinon, lorsque l'on n'est en ComSx, il n'est pas possible de récupérer les codes stats}
               or (GetRecupComSx and (InfoImp <> nil)) Then
             BEGIN
                  CodeStat:=Trim(Copy(St,72,4)) ;
                  If (CodeStat<>'') and (InfoImp^.LCS.indexOf(CodeStat) = -1) Then
                     InfoImp^.LCS.Add(CodeStat) ;
             END ;
             If GetRecupCegid Or GetRecupLTL Then StSISCO.Affaire:=Format_String(CptOrigineCEGID,17)
             ...
             else if GetRecupPCL or GetRecupComSx then
             begin
...
Procedure MAJListeSISCO(Var StSISCO : TStSISCO ; ListePiece : HTStringList ;
...
With StSISCO Do
  BEGIN
  If GetRecupCegid And (InfoImp<>NIL) And (InfoImp^.BaseOrigine='CLI') Then
    BEGIN
  ...
  If RecupSISCO Then
    ...
    If GetRecupCegid Or GetRecupLTL Or GetRecupSISCOPGI Then AffaireCegid:=Affaire ;
    If GetRecupLTL Or GetRecupSISCOPGI Then LaRefReleve:=StSISCO.RefReleve ;
    St1:=St1+RefExterne ;
    StSup:='' ;
    If GetRecupLTL Or GetRecupSISCOPGI Then
    ...
    If GetRecupCegid Then
       ...
       BEGIN
        ...
        if GetRecupPCL then AffaireCegid := Affaire;
    ...
    BEGIN
         ...
         begin
              ...
              Format_String(' ',704)
              +StSISCO.datepaquetmin+ '01011900'+
              StSISCO.lettre + ' --TL '+StSISCO.RefLibre   //AJOUT ME CORRECTION fiche 10184
              else
...
{$ENDIF}
If GetRecupSISCOPGI Then okLigne:=TRUE ;
OkOk:=TRUE ;
...
Case St[1] Of
  ...
  'J' : BEGIN
        ...
        If GetRecupCegid And (InfoImp<>NIL) And (InfoImp.BaseOrigine='CLI') Then IdentPiece.JalP:='Z'+IdentPiece.JalP ;
        If GetRecupSISCOPGI  Then
          BEGIN
          NewJal:=TRUE ;
  ...
  'E','G' : BEGIN
        ...
        If (GetRecupPCL) And RuptOrigineSaisie And (IdentPiece.codeMontant<>'') And (Not OkCROS) Then
          BEGIN
...
{$ENDIF}
        ...
        If OkOk Then
          ...
          If (GetRecupPCL) And (Not OkCROS) And RuptOrigineSaisie And (IdentPiece.codeMontant<>'') Then
            BEGIN
            If CodeMontantSISCO='F' Then CodeMontantSISCO:='E' Else CodeMontantSISCO:='F' ;
            END ;
        If ForceTenue Then
          ...
          FaitStSISCO(St, StSISCO, OldStSISCO, IdentPiece, Infos, RuptOrigineSaisie, CodeMontantSISCO, nil) ;
{$IFDEF CEGID}
...
if GetRecupComSx and Infos.Troncaux then // ajout me 20-03-02
   Cpt2 := Copy (Cpt,2,Length(Cpt)-1);
If GetRecupSISCOPGI And (InfoImp<>NIL) And (InfoImp^.LGSISCO>0) Then CptOrigine:=BourreEtLess(CptOrigine,fbGene,InfoImp^.LGSISCO) ;
...
If Not OkNat1 Then
  ...
  If Cpt[1] in ['C','9'] Then
    ...
    END Else
    BEGIN
        if GetRecupComSx then // ajout me 30-04-2002
        begin
...
If GetRecupSISCOPGI Then Lib:=Lib+'µ'+CptOrigine ;
...
If Copy(Cpt,1,1)='6' Then
  ...
  Result:='CHA' ;
  END ;
If GetRecupPCL And (Result='DIV') Then
  BEGIN
...
St1:='***CGE'+Format_String(Cpt,17)+Format_String(Copy(St,12,25),35)+Nat+Let+Point+
     Vent+Vide+Sens ;
{$IFDEF NOVH}
If GetRecupPCL And (Trim(GetParamsoc ('SO_GENATTEND', TRUE))='') And (Copy(Cpt,1,2)='47') Then
{$ELSE}
If GetRecupPCL And (Trim(VH^.Cpta[fbGene].Attente)='') And (Copy(Cpt,1,2)='47') Then
{$ENDIF}
  BEGIN
  Lib:=AnsiUpperCase(Trim(Copy(St,12,25))) ;
  If Pos('ATTENT',Lib)<>0 Then
    BEGIN
         SetParamSoc('SO_GENATTEND',Trim(Cpt)) ;
{$IFNDEF NOVH}
         VH^.Cpta[fbGene].Attente:=Trim(Cpt) ;
{$ENDIF}
    END Else
    BEGIN
         If Copy(Cpt,1,3)='471' Then
         begin
              SetParamSoc('SO_GENATTEND',Trim(Cpt)) ;
{$IFNDEF NOVH}
              VH^.Cpta[fbGene].Attente:=Trim(Cpt) ;
{$ENDIF}
    ...
    END ;
  END ;
If ((GetRecupPCL) or (GetRecupSISCOPGI)) And AuxACreer Then
  BEGIN
  Coll:=TrouveCptSISCO(Cpt,Infos.FourchetteSISCO,Nat1) ;
  Coll:=AnsiUpperCase(Format_String(Coll,17)) ;
  Coll:=CptRemplace(Coll,Infos.CharRG) ;
  ...
  Vide:=Format_String(' ',320) ;
  If (RecupSISCO or (GetRecupSISCOPGI)) Then
    BEGIN
...
BEGIN
St1:='' ;
If Not GetRecupSISCOPGI Then Exit ;
C:=St[3] ;
NumTL:=0 ;
...
{$ENDIF}
    ...
    TL[i]:=BourreLaDoncSurLaTable('S0'+IntToStr(i-1),TL[i], 6) ;
  ...
  END ;
{$IFNDEF NOVH}
If GetRecupPCL And (Trim(VH^.Cpta[fbaxe1].Attente)='') And (Copy(Cpt,1,2)='99') Then
  BEGIN
...
{$IFDEF MODENT1}
St:=Insere(St,Lib,10,35) ;
St:=Insere(St,'LIB',74,3) ;
{$ELSE}
...
If GetRecupCegid And (InfoImp<>NIL) And (InfoImp.BaseOrigine='CLI') Then Jal:='Z'+Jal ;
   ...
   If Jal='EC' Then Nat:='REG' Else
    If (Jal='RB') Or (Jal='OD') Or (Jal='RE') Then Nat:='OD' Else
      ...
      If (Typ='T') Or (Typ='B') or (Typ='P') Then
        ...
        END ;
      END ;
If GetRecupSISCOPGI Then MS:='-' Else
  BEGIN
  MS:='LIB' ;
  if GetRecupComSx and ((Typ='A') Or (Typ='V')) Then MS:='-' else   // fiche 10340
  If (Typ='A') Or (Typ='V') Then MS:='BOR' ;
  If Nat='ANO' Then MS:='-' ;
  END ;
if (Nat='ANO') and (GetRecupPCL) then SoucheSim:=''
else SoucheSIM:='SIM';  //If Typ='X' Then SoucheSim:='SIS' ;
If Typ='X' Then
  BEGIN
  SoucheSim:='SIS' ;
  If GetRecupSISCOPGI And (InfoImp<>NIL) Then
    BEGIN
...
If (Nat<>'CAI') And (Nat<>'BQE') Then
  ...
  END Else CA:='' ;
If (InfoImp<>NIL) And GetRecupSISCOPGI Then
  BEGIN
  ...
  END ;
If GetRecupSISCOPGI Then If Typ='B' Then BEGIN CA:='***' ; InfoImp^.AuMoinsUneODA:=TRUE ; END ;
...
If Q.Eof Then
  ...
  if GetRecupPCL then Q.FindField('EX_ETATCPTA').AsString:='ZZZ'
  else
  begin
...
If GetRecupCegid And (InfoImp<>NIL) And (InfoImp.BaseOrigine='CLI') And (i=8) Then Exit ;
{$ENDIF}
{$ENDIF}
Case i Of
  0 : BEGIN
...
//        MiseANiveau(InfoImp) ;
        END ;
    1 : BEGIN
          if not GetRecupPCL Then  // Mis à jour à la création du dossier en PCL par DP
          begin
...
//        If Info.AnaSurSISCO Then
    ...
    8 : BEGIN
        ...
        if not ExisteSql ('SELECT CR_CORRESP FROM CORRESP WHERE CR_TYPE="SIS" AND CR_CORRESP="'+Copy(St,3,10)+'"') then
        begin
    ...
    50 : BEGIN
        ...
        If Not Q.Eof Then
          BEGIN
          //SG6 14.03.05 FQ 12617
          if Q.FindField('X_LONGSECTION').AsInteger < ll1 then
          begin
            PGIInfo('La longueur des sections paramétrés dans Sisco est supérieur à la longueur des sections PGI', 'Récupération Sisco');
{$IFNDEF NOVH}
            VH^.STOPRSP:=TRUE ;
{$ENDIF}
...
If Result=0 Then
  BEGIN
  Info.ExoSISCO:=Exo ;
  If Exo.Code=GetEnCours.Code Then Info.StPlusExo:='' Else
    If Exo.Code=GetSuivant.Code Then Info.StPlusExo:='+' Else
      If Exo.Code=GetPrecedent.Code Then Info.StPlusExo:='-' Else
...
Function TraiteEnr(Var NewFichier : TextFile ; ListePiece : HTStringList ;
...
If Copy(St,1,2)='00' Then
  ...
  dec := StrToInt(Copy(St, 23, 1));
  if V_PGI.OkDecV <> dec then
  begin
    V_PGI.OkDecV := dec;  // pour récupération du nombre decimal fiche 10577
    ExecuteSQL('UPDATE DEVISE SET D_DECIMALE='+ Copy(St, 23, 1) +
          ' WHERE D_DEVISE="' + GetParamSocSecur('SO_DEVISEPRINC', 'EUR') + '"');
    SetParamsoc('SO_DECVALEUR', dec);
  end;
{$IFNDEF NOCONNECT}
  If Not Info.InitPCL Then
    BEGIN
    If QuelExoDate(Info.ExoSISCO.Deb,Info.ExoSISCO.Fin,MonoExo,Exo1) Then
      BEGIN
      If Not MonoExo Then Result:=1 Else
        If RecupSISCO Then
          ...
          END Else
          BEGIN
          If (Exo1.Code=GetEnCours.Code) Or ((GetSuivant.Code<>'') And (Exo1.Code=GetSuivant.Code)) Then Else Result:=2 ;
          END ;
      END Else Result:=1 ;
...
If Not Info.InitPCL Then
  If Copy(St,1,2)='05' Then
    ...
    Case St[1] Of
  {$IFNDEF NOCONNECT}
      'C' : BEGIN RecupCptSISCO(St,St1,StSup,Info,InfoImp) ; If St1<>'' then WriteLn(NewFichier,St1) ; If GetRecupPCL And (StSup<>'') Then WriteLn(NewFichier,StSup) ; END ;    // Généraux & Auxiliaires
...
{$ENDIF}
{$ENDIF}
            If (GetRecupLTL Or GetRecupSISCOPGI ) And PieceAvecMvt And ForceDecoupe Then CLoseOpenAFaire:=TRUE ;
      ...
      'J' : BEGIN
            //YYSoldeAuJour:=
            EcritPiece(NewFichier,ListePiece,IdentPiece,Info,PieceAvecMvt,InfoImp) ;
            FaitPiece(St, ListePiece, IdentPiece, StSISCO, Info, InfoImp, FALSE) ;
            IdentPiece.NumChronoEclate:=0 ; ChronoEclateIncremente:=FALSE ;
            END ; // Journal
      'F' : BEGIN
            ...
            If GetRecupPCL Then
              ...
              If GetRecupCegid Or (Arrondi(IdentPiece.TotDP-IdentPiece.TotCP,DecimSISCO(IdentPiece,FALSE))=0) Then
...
//              If RuptureJour(St,StSisco) (*Or PieceEquilibree(IdentPiece)*) Or RuptureOrigineSaisie(St,IdentPiece) Then YYSoldeAuJour:=EcritPiece(NewFichier,ListePiece,IdentPiece,Info,PieceAvecMvt,InfoImp) ;
              END Else
              BEGIN
              If RuptureJour(St,StSisco) Or PieceEquilibree(IdentPiece) Or RuptureOrigineSaisie(St,IdentPiece,ChronoEclateIncremente) Then
                //YYSoldeAuJour:=
                EcritPiece(NewFichier,ListePiece,IdentPiece,Info,PieceAvecMvt,InfoImp) ;
              END ;
  ...
  {$IFNDEF NOCONNECT}
      'v','w' : BEGIN FaitPiece(St, ListePiece, IdentPiece, StSISCO, Info, InfoImp, FALSE) ; END ; // Ecriture analytique
  {$ENDIF}
...
While Not Q.Eof Do
  ...
  END ;
Ferme(Q) ;
If FourchetteVide And GetRecupPCL And (Not GetRecupSISCOPGI) Then
  BEGIN
...
If GetRecupSISCOPGI Then If InfoImp<>NIL Then InfoImp^.RecupODA:=TRUE ;
...
InfoGeneSISCO.LJalLu:=HTStringList.Create ; CompteurFichier:=-1 ;
{$IFNDEF NOCONNECT}
...
{$ENDIF}
{$ENDIF}
If (GetRecupLTL Or GetRecupSISCOPGI) And ForceDecoupe Then CompteurFichier:=0 ;
...
{$ENDIF}
{$ENDIF}
  If (GetRecupLTL Or GetRecupSISCOPGI) And CloseOpenAFaire And ForceDecoupe Then CloseOpenNewFile(NewFichier,StNewFichier,RemplaceLeFichier,StFichier,NewRep,CompteurFichier)  ;
...
If ForceDecoupe And GetRecupSISCOPGI And (CompteurFichier>=0)  Then NbFic:=CompteurFichier+1 ;
...
Var Fichier,NewFichier : TextFile ;
    ...
    CloseOpenAFaire : Boolean ;
BEGIN
Result:=0 ; Pb:=FALSE ; ListePiece := nil;
InitMove(1000,TraduireMemoire('Chargement du fichier en cours...')) ;
...
{$ELSE}
lg := GetParamsoc ('SO_LGCPTEGEN', TRUE);
{$ENDIF}
If Lg>10 Then Lg:=10 ;
...
Var Q : TQuery ;
    St : String ;
    Naturecpte : string;
    Listecpte  : HTStringList;
    i          : integer;
BEGIN
if bExit then Exit ;
NatureCpte := 'G';
Listecpte := HTStringList.Create;
