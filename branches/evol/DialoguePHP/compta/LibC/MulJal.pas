...
inherited;
if ((V_PGI.OutLook) and (JaiLeDroitConcept(TConcept(ccJalCreat),False)) and (TypeAction<>taConsult)) then BInsert.Visible:=True ;
...
if Zoom then
   ...
   if (TypeAction=taModif) then
     ...
     Ferme(Q1) ;
     END ;
   if ((TypeAction=taModif) and (Not JaiLeDroitConcept(TConcept(ccGenModif),False))) then AA:=taConsult ;
