...
if S_SECTION.CanFocus then S_SECTION.SetFocus ;
  inherited;
// MODIF PACK AVANCE
if ((JaiLeDroitConcept(TConcept(ccSecCreat),False)) and (TypeAction<>taConsult)) then BInsert.Visible:=True ;
{$IFDEF CCS3}
...
if TypeAction<>taModifEnSerie then
   ...
   if (TypeAction=taModif) and ((S_Ferme.State=cbGrayed) or (S_Ferme.State=cbChecked)) then
     ...
     Ferme(Q1) ;
     END ;
   if ((TypeAction=taModif) and (Not JaiLeDroitConcept(TConcept(ccSecModif),False))) then AA:=taConsult ; 
