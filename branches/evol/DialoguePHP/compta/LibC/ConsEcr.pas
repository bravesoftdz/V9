...
MakeZoomOLE(Handle) ;
GeneCharge:=True ;
DroitEcritures:=JaiLeDroitConcept(TConcept(ccSaisEcritures),False) ;
...
if TOBGene.GetValue('G_GENERAL')='' then Exit ;
  inherited;
AA:=taModif ; if ((not JaiLeDroitConcept(TConcept(ccGenModif),False)) or (Not ToutAcc)) then AA:=taConsult ;
...
if TOBTiers.GetValue('T_AUXILIAIRE')='' then Exit ;
  inherited;
AA:=taModif ; if ((not JaiLeDroitConcept(TConcept(ccAuxModif),False)) or (Not ToutAcc)) then AA:=taConsult ;
