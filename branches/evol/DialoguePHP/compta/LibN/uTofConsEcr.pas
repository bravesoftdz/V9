...
//  FOkCreateModif  := False;  LG - 26/06/2007 - supprimer suite au deplacement du Inherited ds le OnLoad.
  ...
  FDroitEcritures := JaiLeDroitConcept(TConcept(ccSaisEcritures), False);
...
begin
  ...
  inherited;
  lAction := TaModif;
  if ((not JaiLeDroitConcept(TConcept(ccGenModif), False)) or
      (not FOkCreateModif)) then
    lAction := taConsult;
...
begin
  ...
  inherited;
  lAction := taModif;
  if ((not JaiLeDroitConcept(TConcept(ccAuxModif), False)) or (not
    FOkCreateModif)) then
    lAction := taConsult;
