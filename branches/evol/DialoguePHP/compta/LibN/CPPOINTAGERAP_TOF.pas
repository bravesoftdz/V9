...
begin //OK@@
  ...
  {$ELSE}
  FBoDroitEcritures := JaiLeDroitConcept(TConcept(ccSaisEcritures), False);
  {$ENDIF TRESO}
  FBoDroitMouvement := JaiLeDroitConcept(TConcept(ccSaisMvtBqe   ), False);
...
begin //OK@@
  ...
  {12/07/07 : FQ 21052 : on accède toujours au mouvement en consultation pour les mouvements pointés
   18/07/07 : FQ 21126 : en consultation s'il s'agit d'une session de pointage automatique}
    ...
    if JaiLeDroitConcept(ccSaisMvtBqe, True) and (FListe.Cells[COL_POINTE, FListe.Row] = '') and FPtgeManuel then begin
