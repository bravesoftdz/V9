...
begin
  ...
  if GetField(StrNature) = 'SUS' then     // Si fiche Suspect
    ...
    V_PGI.DispatchTT(8, ParamMixte.Action, GetField(StrCode), '', 'MONOFICHE;T_NATUREAUXI=CLI')
  else if GetField(StrNature)='PRO' then    // si fiche prospect
    V_PGI.DispatchTT(8, ParamMixte.Action, GetField(StrCode), '', 'MONOFICHE;T_NATUREAUXI=PRO');
