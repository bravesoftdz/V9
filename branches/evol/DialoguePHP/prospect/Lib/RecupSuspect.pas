...
begin
  ...
  if (not JaiLeDroitConcept(TConcept(gcImportModifTiers), False)) then
  begin
...
begin
   ...
   begin
      NumeroChamp := TobImport.GetNumChamp ('RSU_LIBELLE');
      TypeChamp := V_PGI.DEChamps[TobImport.NumTable, NumeroChamp].Tipe;
   ...
   begin
      NumeroChamp := TobImport.GetNumChamp ('RSU_ADRESSE1');
      TypeChamp := V_PGI.DEChamps[TobImport.NumTable, NumeroChamp].Tipe;
      PosMaxi := TipeToLength(TypeChamp);
      if Length(ValChamp) > PosMaxi then
      begin
...
NumeroChamp := TobImport.GetNumChamp (Champ);
TypeChamp := V_PGI.DEChamps[TobImport.NumTable, NumeroChamp].Tipe;
...
begin
  ...
  for i := 0 to TobParSuspectLig.Detail.Count - 1 do
    begin
      ...
      TypeChamp := V_PGI.DEChamps[TobImport.NumTable, Numchamp].tipe;
...
begin
  ...
  NumeroChamp           := TobM.GetNumChamp(LeChamp);
  TypeChamp             := V_PGI.DEChamps[TobM.NumTable, NumeroChamp].Tipe;
  if TypeChamp = 'DATE' then
    begin
