...
stProduitPgi := '';
if copy(TFMul(Ecran).name,1,2) = 'RT' then stProduitPgi:='GRC'
 else if copy(TFMul(Ecran).name,1,2) = 'RF' then stProduitPgi:='GRF' ;
...
if (ecran <> Nil) and ((TFMul(Ecran).name = 'RTACTIONS_MUL_RAP') or (TFMul(Ecran).name = 'RTACTIONS_MUL')
    or (TFMul(Ecran).name = 'RTACTIONS_DUPLI') or (TFMul(Ecran).name = 'RFACTIONS_MUL') or (TFMul(Ecran).name = 'RFACTIONS_DUPLI')) then
    begin
    	// Modif BTP
			Fliste.OnDblClick := GSDblClick;
      if assigned (TToolbarButton97(GetControl('Binsert'))) then
      begin
      	TToolbarButton97(GetControl('Binsert')).OnClick := BinsertClick;
      end;
      if assigned (ThEdit(GetCOntrol('RAC_TIERS'))) then
      begin
      	ThEdit(GetCOntrol('RAC_TIERS')).OnChange :=RAC_TIERS_OnChange;
      end;
      if assigned (TToolbarButton97(GetControl('BDUPLICATION'))) then
      begin
      	TToolbarButton97(GetControl('BDUPLICATION')).OnClick := BDUPLICATION_OnClick;
      end;
      //
...
{$ENDIF}
      ;
  if pos('PREVU',Arguments) <> 0 then
   begin //mcd 21/12/2005 equalite 10392
   SetControlText ('RAC_ETATACTION','PRE');
   end;
...
{$ENDIF GRCLIGHT}
  ...
  if (ecran <> Nil) and ( (TFMul(Ecran).name = 'RTACTIONS_MUL')
    or (TFMul(Ecran).name = 'RTACTIONS_TIERS') ) then
 ...
 if (StrArgs <> nil) then
   ...
   if (StrArgs.Values['MONTIERS'] <> '') then
     SetControlText('MONTIERS', StrArgs.Values['MONTIERS']);
