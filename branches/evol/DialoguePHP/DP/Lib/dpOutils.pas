...
var
   ...
   TOBLibelle        :TOB;
begin
     // Table ANNUBIS (on charge les champs si pas déjà fait)
     iTable := PrefixeToNum ('ANB');
     if iTable < 1 then
        exit;
     ChargeDeChamps (iTable, 'ANB');
     ...
     iChamp := ChampToNum ('ANB_CHOIXLIBRE1');
     strLibelle := Trim (RechDom ('YYANNUPARAM', 'AN1', FALSE));
     if strLibelle <> '' then
        V_PGI.DEChamps [iTable, iChamp].Libelle := strLibelle;
     iChamp := ChampToNum ('ANB_CHOIXLIBRE2');
     strLibelle := Trim (RechDom ('YYANNUPARAM', 'AN2', FALSE));
     if strLibelle <> '' then
        V_PGI.DEChamps [iTable, iChamp].Libelle := strLibelle;
     iChamp := ChampToNum ('ANB_CHOIXLIBRE3');
