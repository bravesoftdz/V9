...
{$ENDIF}
     ...
     HEnt1,
     HMsgBox,UTOM,
     UTob,extctrls,htb97,UtilAlertes,HRichOLE,HPanel,Hdb,buttons,hDebug,
     ParamSoc,HSysMenu ;
...
begin
  ...
  Repeat
    ...
    if Prefixe <> '' then
      ...
      i:= high(V_PGI.DeChamps[iTable]) ;
      for iChamp := 1 to i do
        begin
        { en attendant que le problème sur ce champ soit corrigé
        if V_PGI.Dechamps[iTable,iChamp].nom = 'GA_COLLECTION' then continue;}
        if V_PGI.Dechamps[iTable,iChamp].libelle = '' then continue;
        if (Pos(ListeDhControle,V_PGI.Dechamps[iTable,iChamp].Control)>0) and (V_PGI.Dechamps[iTable,iChamp].libelle[1]<>'.') then
            if (TypeChamp<>'D') or ((TypeChamp='D') and (V_PGI.Dechamps[iTable,iChamp].tipe='DATE')) then
              ListeInterm.add (V_PGI.Dechamps[iTable,iChamp].libelle+';'+V_PGI.Dechamps[iTable,iChamp].nom);
        end;
      end;
  until Prefixe='';
