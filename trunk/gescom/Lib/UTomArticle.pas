...
Begin
  ...
  Begin
    ...
    //
    if TypeArticle='OUV' then BCodeBarre.Visible := False;
    //
    MaxTaxe := 1;
    //
...
{$IFDEF BTP}
  ...
  begin
    ...
    if (not MetreArticle.ControleMetre) or (not MetreArticle.OkExcel) then
    begin
      // Modified by f.vautrain 14/11/2017 14:35:24 - FS#2788 - MICHEL SA : Violation d'accès sur ouverture article pourcentage
      if assigned(BExcel) then BExcel.visible := False;
      BAppelVariable.Visible := false;
    end
    else
    begin
      // Modified by f.vautrain 14/11/2017 14:35:31 - FS#2788 - MICHEL SA : Violation d'accès sur ouverture article pourcentage
