...
implementation
uses FactComm,UtilPGI,FactTOB,FactPiece,FactRG,FactUtil,ParamSOc,ENt1,AglInit,M3FP,cbpPath;
...
begin
  ...
  TheLance := IncludeTrailingBackslash(TcbpPath.GetCegid)+'Specif-POC\APP\MarcheST.exe /Serveur='+ServerName+' /BaseDeDonnees='+DBName+' /CodeChantier="'+CodeChantier+'" /SousTraitant='+SousTrait+' /CodeMarche='+CodeMarche+' /Action=M';
...
begin
  ...
  DBName := V_PGI.DBName;
  ServerName := GetServerName;
  TheLance := IncludeTrailingBackslash(TcbpPath.GetCegid)+'Specif-POC\APP\MarcheST.exe /Serveur='+ServerName+' /BaseDeDonnees='+DBName+' /CodeChantier="'+CodeChantier+'" /Action=C';
  FileExecAndWait (TheLance);
end;
