...
uses Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
     ...
     Utof,Utob,FE_Main,HRichEdt, HRichOLE, SaisBor, TofVerCptaBor;

type TOF_RepCptaBor = class(TOF)
     private
       ...
       procedure LanceReparation ;
       procedure LanceRepEcr(TobEcr, TobAna : TOB) ;
       procedure LanceRepAna(TobAnaTotal, TobEcr : TOB) ;
...
procedure TOF_RepCptaBor.OnLoad ;
var Err : integer ;
begin
...
procedure TOF_RepCptaBor.OnClose ;
begin
MsgBox1.Free ;
MsgBox2.Free ;
TCritere.Free ;
...
procedure TOF_RepCptaBor.OnUpdate ;
var Err : integer ;
begin
LeNombreErreur:=0 ;
...
BlocNote.Lines.Add(TraduireMemoire('Réparation en cours ...')) ;
BlocErr.Lines.Clear ;
...
procedure TOF_RepCptaBor.OnClickBStop(Sender : TObject) ;
begin
if TraitEnCours=False then Exit ;
if Erreur1(19)=mrYes then Sortie:=True ;
end ;

procedure TOF_RepCptaBor.OnTropErreurs ;
begin
if TraitEnCours=False then Exit ;
if Erreur1(18)=mrYes then Sortie:=True ;
end ;

procedure TOF_RepCptaBor.OnChangeExo(Sender : TObject) ;
var DateTmp : THedit ;
begin
...
procedure TOF_RepCptaBor.RecupCriteres(var Err : integer) ;
...
procedure TOF_RepCptaBor.InitForm(var Err : integer) ;
var Button : TButton ;
begin
//init des controls
...
procedure TOF_RepCptaBor.ChargeJournaux ;
...
procedure TOF_RepCptaBor.BuildWhere(var Where : string;Prefix : string);
...
function TOF_RepCptaBor.CreerTobAnaPArtiel(TobAnaTotal,TobEcr : Tob) : Tob ;
...
//*                            *
//******************************
procedure TOF_RepCptaBor.LanceReparation ;
...
//        Where:='E_JOURNAL="'+Jal+'" AND E_PERIODE='+IntToStr(Periode) ;
    ...
    if TobEcr<>nil then LanceRepEcr(TobEcr, TobAna)
                   else LanceRepAna(TobAna, nil) ;
...
procedure TOF_RepCptaBor.LanceRepEcr(TobEcr, TobAna : TOB) ;
...
for i:=0 to TobEcr.Detail.Count-1 do
  ...
  if EquGen.Checked then
    ...
    if ChangeBordereau(T1,CtrlBor) then
      ...
      if T1.GetValue('E_ANA')='X' then LanceRepAna(TobAna,T1) ;
...
procedure TOF_RepCptaBor.LanceRepAna(TobAnaTotal,TobEcr : Tob) ;
...
//*                            *
//******************************
function TOF_RepCptaBor.ChangeBordereau(T1 : Tob;CtrlBor : TCtrlBor) : boolean ;
var BorTmp : integer ;
begin
...
function TOF_RepCptaBor.ChangePiece(T1 : Tob;CtrlBor : TCtrlBor) : boolean ;
...
function TOF_RepCptaBor.ChangeVentil(T1 : Tob;CtrlBor : TCtrlBor) : boolean ;
...
//*                            *
//******************************
function TOF_RepCptaBor.ControlIntegriteGen(T1 : Tob) : integer ;
...
function TOF_RepCptaBor.ControlIntegriteAna(T1 : Tob) : integer ;
...
function TOF_RepCptaBor.ControlIntegritePie(T1 : Tob;CtrlBor : TCtrlBor) : integer ;
...
function TOF_RepCptaBor.ControlIntegriteBor(T1 : Tob;CtrlBor : TCtrlBor) : integer ;
begin
...
function TOF_RepCptaBor.ControlIntegriteVen(T1 : Tob;CtrlBor : TCtrlBor) : integer ;
begin
...
//*                            *
//******************************
function TOF_RepCptaBor.ControlExisteChamps(T1 : Tob;Prefix : string) : integer ;
var Ret : integer ;
begin
...
function TOF_RepCptaBor.EcritureEtVentilation(TVen,TEcr : Tob) : integer ;
...
function TOF_RepCptaBor.ControlCumulOk(CtrlBor : TCtrlBor;Bor,Piece : Boolean) : integer ;
begin
...
function TOF_RepCptaBor.ControlCumulAnaOk(CtrlBor : TCtrlBor;TEcr : Tob) : integer ;
...
function TOF_RepCptaBor.GeneralEtAutreOK(T1 : Tob;Prefix : string) : integer ;
...
function TOF_RepCptaBor.EcrAnouveauOK(T1 : Tob;Prefix : string) : boolean ;
...
function TOF_RepCptaBor.NatureGenEtNatureAuxOK(NatureGen,NatureAux : string) : boolean ;
begin
...
function TOF_RepCptaBor.DeviseOK(T1 : Tob;Prefix : string) : integer ;
...
function TOF_RepCptaBor.MontantAnaOK(T1 : Tob) : integer ;
...
//*                            *
//******************************
procedure TOF_RepCptaBor.MajCtrlBor(Pref: string; var CtrlBor: TCtrlBor; T1: TOB; RazCumulBor, RazCumulPiece: Boolean) ;
begin
...
procedure TOF_RepCptaBor.AjoutCumulCtrlBor(Pref: string; var CtrlBor: TCtrlBor; T1: TOB) ;
...
function TOF_RepCptaBor.RecupMonnaieAutre : string ;
begin
...
//*                            *
//******************************
procedure TOF_RepCptaBor.CreateTheTob ;
begin
TheTob:=Tob.Create('_THETOB', nil, -1) ;
AddTheTob('EXERCICE','SELECT EX_EXERCICE FROM EXERCICE') ;
AddTheTob('JOURNAL','SELECT J_JOURNAL,J_NATUREJAL,J_MODESAISIE,J_LIBELLE FROM JOURNAL WHERE J_MODESAISIE="BOR" OR J_MODESAISIE="LIB"') ;
...
procedure TOF_RepCptaBor.AddTheTob(Table : string ; Sql : string) ;
var Q : TQuery ; T1 : TOB ;
begin
...
function TOF_RepCptaBor.CatchTheTob(QuelTob : string ) : TOB ;
begin
Result:=nil ;
...
function TOF_RepCptaBor.RecupTheTob(QuelTob, QuelChamp : string ; NomChamp : array of string ; ValeurChamp : array of Variant ; MultiNiveau : Boolean) : Variant ;
var T1, T2 : TOB ;
begin
...
function TOF_RepCptaBor.ExisteTheTob(QuelTob :string ; NomChamp : array of String ; ValeurChamp : array of Variant ; MultiNiveau : Boolean) : Boolean ;
var T1,T2 : TOB ;
begin
...
procedure TOF_RepCptaBor.DestroyTheTob ;
begin
...
function TOF_RepCptaBor.Erreur1(Err : integer) : integer ;
begin
Result:=0 ;
...
Procedure TOF_RepCptaBor.InitMsgBox1;
var i : integer ;
begin
if MsgBox1=nil then exit ;
...
procedure TOF_RepCptaBor.Erreur2(Prefix : string;T1 : tob;Err : Integer) ;
...
Procedure TOF_RepCptaBor.InitMsgBox2;
var i : integer ;
begin
if MsgBox2=nil then exit ;
Err_Ligne:=0 ;
...
procedure TOF_RepCptaBor.ExportErreur ;
...
Initialization
registerclasses([TOF_RepCptaBor]);
end.
