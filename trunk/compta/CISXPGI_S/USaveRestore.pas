...
TDomaine.Connection := DMImport.DBGLOBAL;
TDomaine := DMImport.GzImpDomaine;
...
{    TobTVTable := TobTVDomaine.FindFirst(['NomTable'], [FNom.AsString + '-' +FNomChamp.AsString], False);
    if TobTVTable = nil then
        begin
        TobTVTable := TOB.Create(FNom.AsString + '-' +FNomChamp.AsString, TobTVDomaine, -1);
        TobTVTable.AddChampSupValeur('NomTable', FNom.AsString + '-' +FNomChamp.AsString, False);
    ...
    TDelim := DMImport.GzImpDelim;
    DMImport.Cmd.CommandText := 'SELECT * FROM PGZIMPREQ WHERE CLE="'+ TBase.FieldByName('CLE').AsString +'"';
...
{$IFNDEF CISXPGI}
  try  // ajout me
    if (TobL <> nil) and (TBase.TableName = DMImport.GzImpPar.TableName) then
    begin
        QP := TQuery.Create(nil);
        QP.DatabaseName := DMImport.DBGlobal.Name;
        QP.SQL.Add('Delete from ' + DMImport.GzImpPar.TableName + ' where Domaine="'+TobL.GetValue('LeDomaine')+'" and TableName="'+TobL.GetValue('LaTable')+'"');
        QP.ExecSQL;
        QP.Free;
    end;
  except
...
begin
    TBase := DMImport.GzImpPar;
    RecordExist := TBase.Locate('Numero;Domaine', VarArrayOf([TobF.GetValue('Numero'), TobF.GetValue('Domaine')]), [loCaseInsensitive])
...
begin
    ...
    RecordExist := TDelim.Locate('Cle', VarArrayOf([TobF.GetValue('DELIM-CLE')]), [loCaseInsensitive]);
    if not RecordExist then
        TDelim.Insert
        else
        TDelim.Edit;
