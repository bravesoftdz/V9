...
uses
  ...
  CpteUtil, EdtLegal, CritEDT, HQry,UtilEdt1, QRRupt, HTB97, HPanel, UiUtil, tCalcCum ;


procedure GLVentileTP ;
procedure GLVentileZoomTP(Crit : TCritEdt) ;

type
  TFQRGLVenTP = class(TFQR)
    HLabel3: THLabel;
...
Function TFQRGLVenTP.QuoiCpt(i : Integer) : String ;
BEGIN
  ...
  0 : BEGIN
      Case i Of
        ...
        1 : Result:=QR2E_AUXILIAIRE.AsString+' '+Q.FindField('T_LIBELLE').AsString+' '+
             '#'+StJal+' N° '+StNumP+' '+DateToStr(QR2E_DateComptable.AsDAteTime)+'-'+
              PrintSolde(Qr2DEBIT.AsFloat,Qr2Credit.AsFloat,CritEDT.Decimale,CritEDT.Symbole,CritEDT.AfficheSymbole)+' '+
              '@'+'5;'+StJal+';'+UsDateTime(QR2E_DATECOMPTABLE.AsDateTime)+';'+StNumP+';'+QR2E_EXERCICE.asString+';'+
              StNumL+';'
        end ;
      END ;
  1 : BEGIN
...
Procedure TFQRGLVenTP.GLVentZoom(Quoi : String) ;
Var Lp,i: Integer ;
BEGIN
...
BEGIN
PP:=FindInsidePanel ;
QR:=TFQRGLVenTP.Create(Application) ;
Edition.Etat:=etGlVen ;
QR.QRP.QRPrinter.OnSynZoom:=QR.GLVentZoom ;
QR.InitType (nbAux,neGlV,msRien,'QRGLVENTP','',TRUE,FALSE,FALSE,Edition) ;
if PP=Nil then
   BEGIN
   try
    QR.ShowModal ;
...
try
 QR.QRP.QRPrinter.OnSynZoom:=QR.GLVentZoom ;
 QR.CritEdt:=Crit ;
 QR.InitType (nbAux,neGlV,msRien,'QRGLVENTP','',FALSE,TRUE,FALSE,Edition) ;
 finally
 QR.Free ;
 end ;
Screen.Cursor:=SyncrDefault ;
...
procedure TFQRGLVenTP.InitDivers ;
BEGIN
Inherited ;
...
procedure TFQRGLVenTP.GENERESQL ;
{ Construction de la requête SQL en fonction du multicritère }
BEGIN
Inherited ;
GenereSQLTiers(neGL,Ventile,CritEdt,Q,TRUE,AvecRevision.State) ;
GenereSQLSUB ;
END ;

procedure TFQRGLVenTP.GenereSQLSUB ;
{ Construction de la requête SQL en fonction du multicritère }
BEGIN
Inherited ;
GenereSQLSUBTiers(neGL,Ventile,CritEdt,QEcr,TRUE) ;
END ;

procedure TFQRGLVenTP.RenseigneCritere ;
...
procedure TFQRGLVenTP.ChoixEdition ;
...
procedure TFQRGLVenTP.FinirPrint ;
Var SOlde : Double ;
BEGIN
Inherited ;
QEcr.Close ;
...
if OkMajEdt Then
   ...
   if Solde<0
      then MajEdition('GLV', '', DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date1),
                      '', TotEdt[6].TotDebit, TotEdt[6].TotCredit, Solde, 0)
      else MajEdition('GLV', '', DateToStr(CritEdt.Date1), DateToStr(CritEdt.Date1),
                      '', TotEdt[6].TotDebit, TotEdt[6].TotCredit, 0, Solde) ;
   END ;
END ;

function  TFQRGLVenTP.CritOk : Boolean ;
BEGIN
...
procedure TFQRGLVenTP.RecupCritEdt ;
...
Procedure TFQRGLVenTP.Calculs(Solde : TQRLabel; var Report1,Report2 : TReport; i : integer; Bool1,Bool2 : Boolean ) ;
...
procedure TFQRGLVenTP.SaisieDates ;
Var i : byte ;
...
procedure TFQRGLVenTP.FormShow(Sender: TObject);
begin
//HelpContext:=7559010 ;
...
FNatureEcr.ItemIndex:=0 ; FNatureEcr.Enabled:=FALSE ;
end;

procedure TFQRGLVenTP.FColl2Change(Sender: TObject);
...
procedure TFQRGLVenTP.BDetailBeforePrint(var PrintBand: Boolean;  var Quoi: string);
...
procedure TFQRGLVenTP.BFCompteAuxBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
PrintBand:=Affiche ;
...
procedure TFQRGLVenTP.BMdpBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLVenTP.BFinEtatBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLVenTP.FChoixEcartClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLVenTP.FTypeGLClick(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFQRGLVenTP.FP5Exit(Sender: TObject);
Var St : String ;
    A  : Char   ;
begin
  inherited;
...
if (TMaskEdit(Sender).Text<>'  /  /    ') and IsValidDate(TMaskEdit(Sender).Text)  then
   ...
   END else
   BEGIN
   MsgRien.Execute(1,Caption,' '+TMaskEdit(Sender).Text) ;
...
procedure TFQRGLVenTP.FDateCompta1Exit(Sender: TObject);
begin
  inherited;
If FChoixEcart.ItemIndex=1 then SaisieDates ;
end;

procedure TFQRGLVenTP.tabSupEnter(Sender: TObject);
begin
  inherited;
FPeriodicite.SetFocus ;
end;

procedure TFQRGLVenTP.FTypeParClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLVenTP.FPeriodiciteChange(Sender: TObject);
Var i : Integer ;
    TabD : TTabDate8 ;
begin
  inherited;
...
procedure TFQRGLVenTP.TOPREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
begin
  inherited;
...
procedure TFQRGLVenTP.BOTTOMREPORTBeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport : TabTRep ;
begin
  inherited;
...
procedure TFQRGLVenTP.QAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFQRGLVenTP.QEcrAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
QR2E_PIECETP       :=TStringField(QEcr.FindField('E_PIECETP')) ;
...
procedure TFQRGLVenTP.BDetail2BeforePrint(var PrintBand: Boolean;  var Quoi: string);
var MReport  : TabTRep ;
    OkCalCul : Boolean ;
    St,St1,StNumE : String ;
begin
  inherited;
...
If OkCalCul then
   BEGIN
   St:=Qr2E_PIECETP.AsString ; StNumP:='' ; StNumL:='' ; StJal:='' ;
   If St<>'' Then
     BEGIN
     St1:=ReadTokenSt(St) ;
     StNumP:=ReadTokenSt(St) ;
     StNumL:=ReadTokenSt(St) ;
     StNumE:=ReadTokenSt(St) ;
     StJal:=ReadTokenSt(St) ;
     END ;
   E_PIECELIGECH.Caption:=StNumP+' / '+StNumL ;
   LE_JOURNAL.Caption:=StJal ;
...
procedure TFQRGLVenTP.FNatureCptChange(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLVenTP.BFCompteAuxAfterPrint(BandPrinted: Boolean);
begin
  inherited;
InitReport([2],CritEdt.GlV.FormatPrint.Report) ;
end;

procedure TFQRGLVenTP.BNouvRechClick(Sender: TObject);
begin
  inherited;
If FCollJoker.Visible then FCollJoker.Text:='' ;
end;

procedure TFQRGLVenTP.DLRuptNeedData(var MoreData: Boolean);
var TotRupt          : Array[0..13] of Double ;
...
procedure TFQRGLVenTP.BRuptBeforePrint(var PrintBand: Boolean; var Quoi: string);
begin
  inherited;
PrintBand:=(CritEdt.Rupture<>rRien) ;
end;

procedure TFQRGLVenTP.FSansRuptClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLVenTP.FRupturesClick(Sender: TObject);
begin
  inherited;
...
procedure TFQRGLVenTP.BPIEDTPBeforePrint(var PrintBand: Boolean; var Quoi: String);
begin
  inherited;
LIBTP2.Caption:=TraduireMemoire(' TOTAL Tiers payeur :   ')+Q.FindField('PAYEUR').AsString+'   '+LibTPEnCours ;
TPCum5.Caption:=PrintSolde(TotTP[5].TotDebit, TotTP[5].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPCum4.Caption:=PrintSolde(TotTP[4].TotDebit, TotTP[4].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPCum3.Caption:=PrintSolde(TotTP[3].TotDebit, TotTP[3].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPCum2.Caption:=PrintSolde(TotTP[2].TotDebit, TotTP[2].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPCum1.Caption:=PrintSolde(TotTP[1].TotDebit, TotTP[1].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPCum0.Caption:=PrintSolde(TotTP[0].TotDebit, TotTP[0].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
TPTotal.Caption:=PrintSolde(TotTP[6].TotDebit, TotTP[6].TotCredit, CritEdt.Decimale,CritEdt.Symbole,CritEdt.AfficheSymbole) ;
Fillchar(TotTP,SizeOf(TotTP),#0) ;

end;

procedure TFQRGLVenTP.BENTETETPBeforePrint(var PrintBand: Boolean;  var Quoi: String);
Var St : String ;
begin
  inherited;
St:=Q.FindField('PAYEUR').AsString ;
LibTPEnCours:=TrouveLibTP(St,TRUE) ;
LIBTP1.Caption:=TraduireMemoire('Tiers payeur :   ')+Q.FindField('PAYEUR').AsString+'   '+LibTPEnCours ;
end;
