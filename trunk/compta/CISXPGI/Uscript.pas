       ...
       begin
                    ...
                    begin
                            ...
                            begin
                              ...
                              Q1 := OpenSQLADO (SQL, DMImport.DbDonnee);
                              if not Q1.EOF then
                              begin
...
begin
  ...
  SR.FASCIIMODE := 0; // FASCIIMODE; (* 0:Fixe 1:Delimite *)
...
begin
  ...
  if FileDest = 0 then
    ...
    ATablerec.FTable.Connection := DMImport.DBDonnee as TADOConnection;
...
begin { CreerTables }
  ...
  begin { TTable TDatabase }
    ...
    DefFields(FFields);
   try
    DMImport.DBDonnee.Execute ('DROP TABLE ' + ParName);
(*     DMImport.CmdDonnee.CommandText:='DROP TABLE ' + ParName;
     DMImport.CmdDonnee.Execute;
*)
    except end;
    try;
     DMImport.DBDonnee.Execute('CREATE TABLE ' + ParName + ' (' + S);
(*
...
try
  ...
  STable.Connection := DMImport.Db as TADOConnection;
  STable.Tablename := DMImport.GzImpReq.TableName;
  STable.Open;
  AStreamTable := TmemoryStream.create;
  if Table <> '' then
  begin
       if not STable.Locate('CLE', table, [loCaseInsensitive]) then exit;
  ...
  begin
      ...
      QP := OpenSQLADO (' SELECT Table1,CLE,COMMENT,CLEPAR,DATEDEMODIF,Table0,Domaine from '+ DMImport.GzImpReq.TableName + ' Where (CLEPAR<>"SQL" or CLEPAR="") '+ SQL, DMImport.Db );
...
begin
      ...
      Q := TQuery.Create(nil);
      Q.SqlConnection := DMImport.DbDonnee;
      Q.SQL.Add (requete);
      Q.Open;
