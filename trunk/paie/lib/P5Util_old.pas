...
PT4   : 29/08/2001 PH V547 correction bornes de validité remuneration sur 4 mois
...
PT8   : 03/09/2001 PH V547 on force dans tous les cas le calcul de l'arrondi sur
                           le montant calculé
PT9   : 05/09/2001 SB V547 L'acquisition annuelle des CP ne correspond plus à 25
...
PT11  : 10/09/2001 SB V547 Fiche de bug n°266 : Gestion des msg d'erreur CP en
                           prep auto . fn SalIntegreCP modifiée (Aussi ds source
                           SaisBul)
...
PT60  : 07/10/2002 PH V585 Recherche d'un element national par préférence
                           DOS,STD,CEG
PT61  : 17/10/2002 SB V585 FQ n°10235 Intégration du calcul des variables 28,29 heures ouvrées ouvrables
PT62  : 07/11/2002 PH V591 Prise en compte du decalage de paie sur les éléments nationaux en fonction du champ
        decalage
...
PT69-1  14/01/2003 SB V591 Suppression des lignes de commentaire CP inopérandes si modification date de fin de bulletin
PT69-2  16/01/2003 SB V591 Calcul variable 50 : Integration du solde que si salarié sorti
...
PT75-2: 04/06/2003 SB V595 FQ 10523 Intégration de l'acquis bulletin calculé sur une variable de paie
PT76  : 04/06/2003 PH V_421 FQ 10425,10620,10655 nvelles variables
PT77  : 04/06/2003 PH V_421 FQ 10689 Salarié sort dans la période mais la cotisation ne se calcule que si le salarié est present le dernier jour du mois
PT78  : 05/06/2003 PH V_421 FQ 10700 Gestion origine de la rubrique dans les bases de cotisations
PT79-1: 06/06/2003 SB V595 Test minimum conventionnel en alpha même pour du numerique
...
PT82-2 : 25/06/2003 SB V_42 FQ 10717 Calcul nombre de mois anciennete erroné : Nombre de mois entier
PT83-3 : 27/06/2003 SB V_42 FQ 10628 Intégration procedure à part de l'init des variables heures et jours travaillés
...
PT88   : 10/10/2003 SB V_42 Intégration d'un indicateur des CP pris non payés par manque d'acquis
...
PT90-1 : 18/11/2003 SB V_50 FQ 11121 Contrôle rubrique alimenté pour génération libellé CP
...
PT94   : 18/12/2003 PH V_50 FQ 11028 Calcul des cotisations que si présent en fin de mois-traitement de la date à idate1900
PT95   : 12/01/2004 PH V_50 FQ 11024 Rubrique sur le bulletin en origine salarié reprend les valeurs même si elt variable
PT96   : 24/02/2004 SB V_50 FQ 11130 Calcul Ancienneté érronné si salarié entrée sortie le même jour
...
PT100-1: 09/04/2004 SB V_50 FQ 11136 Ajout Gestion des congés payés niveau salarié
PT100-2: 09/04/2004 SB V_50 FQ 11237 Ajout Gestion idem etab jours Cp Suppl
PT101    07/05/2004 SB V_50 FQ 11199 Dysfonctionnement Calcul Indemnité CP sur 1er bulletin
...
PT104    21/06/2004 PH V_50 FQ 10793 Indicateur de contrôle de modification des salariés pdt la saisie du bulletin
...
PT108    05/07/2004 PH V_50 FQ 11052 Modif Fiche ETABSOCIAL et contrôle sur les dates d'édition
...
PT112    10/08/2004 PH V_50 FQ 11464 Prise en compte de la période en cours pour les variables de type rem,cot
...
PT115    19/08/2004 PH V_50 FQ 10991 Suite : inversion du calcul en fonction du code de la rubrique Mois et Année
PT116    20/08/2004 PH V_50 FQ 10959 Récupération du champ sortie définitive
PT117    23/08/2004 PH V_50 FQ 11459 Limitation des trentièmes si 2 bulletins dans le même mois
...
PT120    24/08/2004 PH V_50 FQ 11454 Suite Si pas élément Nat trouvé alors on rend 0
...
PT126    23/12/2004 PH V_60 Prise en compte variable de type cumul sur période annuelle
                            pour le 1er mois dans le cas du décalage de paie
...
PT128    03/02/2005 SB V_60 FQ 11089 Calcul erronné ancienneté en jours variable 0010
PT129    08/02/2005 MF V_60 Traitement des commentaires des règlements d'IJSS (2 nvelles fcts)
}
unit P5Util;
...
var
  ...
  BullCompl, ProfilSpec: string; // Bulletin compléméntaire X ou -  correspondra à un Boolean dans la table et son profil
...
function EvalueRem(Tob_Rub: TOB; const Rub: string; var Base, Taux, Coeff, Montant: Double; var lib: string; DateDeb, DateFin: TDateTime; ActionCalcul: TActionBulletin; Ligne:
  Integer): Boolean; // Calcul de la rémunération
function EvalueChampRem(Tob_Rub: TOB; const TypeChamp, Champ, Rub: string; const DateDeb, DateFin: TDateTime): Double; // Calcul d'un champ d'une Remunération
function RechCasCodeCalcul(const Cc: string): WORD; // Recherche du cas code calcul
function ValVariable(const VariablePaie: string; const DateDeb, DateFin: TDateTime; TOB_Rub: TOB): double; // Fonction d'évaluation d'une variable
...
procedure CalculBulletin(Tob_Rub: TOB); // procedure qui valorise les lignes de bulletins en fonction du paramètrage et A PARTIR de TOB_Rub
...
procedure RecupTobSalarie(const Salarie: string); // Rechargement de la TOB_Salarié
...
begin
  ...
  begin // recup des lignes du dernier bulletin fait pour le salarié/etablissement
    ...
    begin
      ...
      if ActionB = taCreation then
      begin
        MajRubriqueBulletin(TPE, TPR, CodSal, Etab, DateDeb, DateFin) // controle si la rubrique est presénte dans le bulletin et récup élément permanent
      end
      else
      begin
...
var
  ...
  St: string;
begin
  ret := 0; // initialisation dans le cas peu probable où on ne trouve pas
...
begin
  ...
  begin // Rubrique existante = recup libelle
    ...
    if NatureRub = 'AAA' then RecupEltPermanent(TRech, TPR, Rubrique); // les éléments permanents ne sont que sur les Rémunérations
  end
  else
...
begin
  ...
  begin
    if TOB_Salarie.GetValeur(iPSA_SALARIE) <> Salarie then RecupTobSalarie(Salarie); // Reprend le contenu de la table Salarié au moment où l'on écrit Entete Bulletin
  end;
...
function EvalueCot(Tob_Rub: TOB; const Rub: string; var Base, TxSal, TxPat, MtSal, MtPat: Double; var lib: string; DateDeb, DateFin: TDateTime; ActionCalcul: TActionBulletin; var
  At: Boolean): Boolean; // Calcul de la rémunération
var
  T1, TRecup: TOB;
...
{$ENDIF}
  ...
  begin
    ...
    if TypeChamp = 'REM' then result := T_Rech.GetValeur(iPHB_MTREM) // Montant de Rémunération
    else
...
function EvalueBas(Tob_Rub: TOB; const Rub: string; var Base, Plafond, Plf1, Plf2, Plf3, TR1, TR2, TR3: Double; var lib: string; const DateDeb, DateFin: TDateTime; ActionCalcul:
  TActionBulletin; const BaseForcee, TrancheForcee: Boolean): Boolean; // Calcul de la rémunération
var
  T1, TRecup: TOB;
...
begin
  ...
  if TypeChamp = 'ELN' then result := ValEltNat(Champ, DateFin) // Recup elelment national
    ...
    if TypeChamp = 'REM' then result := T_Rech.GetValue('PHB_MTREM'); // Montant de Rémunération
end;
...
begin
  ...
  {juste pour voir 08-02        BCPACQUIS  := Arrondi(RendCumulSalSess ('11'),DCP);
       ...
       Remarque : Si on est en modification, on peut compter sur le date validité
                  qui correspond à la date fin de bulletin puisque celle ci n'est pas
                  modifiable en modif bulletin.
                  Par contre en création bulletin, impossible de compter sur cette date
                  car elle peut être modifiée à n'importe quel moment. On aura dc stocké
                  le n° ordre du mvt
...
begin
     ...
     Q:=OpenSql(st, TRUE);
     if Q.eof then // pas de congés pris
        exit;
...
{$ENDIF}
    ...
    100: MtCalcul := ValEltNat('0001', DateFin) * Trentieme; // Plafond salarié proratisé  30eme
...
begin
  ...
  case i of
    2: result := ValEltNat(ChampBase, DateFin); // cas d'un élement national
    3: result := ValVariable(ChampBase, DateDebut, DateFin, TOB_Rub); // cas d'une variable
    4: result := VALEUR(ChampBase); // cas d'une valeur
    5..8: result := EvalueVarRub(TypeB, T_Rech, DateDebut, DateFin, TOB_Rub, TypeB, ChampBase); // cas d'une rémunération dans la session de paie
    9:
      begin
...
{$ENDIF}
 ...
 recherche des lignes de commentaire que sur la nature non sur les dates, etab et salarié
  ...
  if (TOB_Libelle <> nil) and (Rub <> '') then //PT90-1 Gestion libellé si rubrique existant
  begin
    i := 1;
